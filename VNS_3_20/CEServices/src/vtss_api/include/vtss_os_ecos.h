...
#define VTSS_MSLEEP(msec)       HAL_DELAY_US(msec*1000)    /**< Sleeping for "msec" mili seconds */
#define VTSS_NSLEEP(nsec)       HAL_DELAY_US((nsec)/1000)  /**< Sleeping for "nsec" nano seconds */
 ...
 */
#define VTSS_OS_CTZ(val32) __builtin_ctz(val32)
 ...
 *
 * Note: __builtin_ctzll() below returns 31 if val64 is 0, hence the additional check.
 * I have tested it for all other values (including all-ones and MSbit set, and it works).
 */
