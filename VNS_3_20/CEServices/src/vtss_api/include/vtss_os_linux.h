...
} /**< Sleep for "nsec" nano seconds */
#define VTSS_MSLEEP(msec) { \
    struct timeval tve, tv; \
    gettimeofday(&tve,NULL); \
    tve.tv_usec+=(msec*1000);\
    if (tve.tv_usec>=1000000) { tve.tv_sec+=tve.tv_usec/1000000; tve.tv_usec%=1000000; } \
    do { gettimeofday(&tv,NULL); } \
    while ( timercmp(&tv,&tve,<) ); \
    } /**< Sleep for "msec" mili seconds */
...
__extension__ extern long long int llabs (long long int __x) __THROW __attribute__ ((__const__));
...
#define VTSS_NSLEEP(nsec) ndelay(nsec)
#define VTSS_MSLEEP(msec) mdelay(msec)
 ...
 */
#define VTSS_OS_CTZ(val32) __builtin_ctzl((unsigned long)val32)
 ...
 */
#ifdef __KERNEL__
#define VTSS_OS_MALLOC(size, flags) kmalloc(size, GFP_KERNEL)
#else
 ...
 */
#ifdef __KERNEL__
#define VTSS_OS_FREE(ptr, flags) kfree(ptr)
#else
