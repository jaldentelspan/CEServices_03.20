...
{
  T_N("Locking function %s",function);
...
{
  T_N("Unlocking function %s",function);
...
static void packet_poll(vtss_board_t *board)
{
    ...
    for (queue = VTSS_PACKET_RX_QUEUE_START; queue < VTSS_PACKET_RX_QUEUE_END; queue++) {
        ...
        T_D("received frame on port_no: %u, queue: %u, length: %u",
...
static int parse_options(int argc, const char **argv, int *count)
{
    int  i;
    const char *s;
    ...
    for (i = 1; i < argc; i++) {
        ...
        switch (*s) {
        case 'h': /* Help */
            printf("vtss_appl [-v] [-t[:<layer>][:<group>][:<level>]] <board-specific args>\n");
            printf("-v: Show version\n");
            printf("-t: Show or set trace level\n");
            return 1;
        case 'v': /* Version */
            printf("Version: %s\n", vtss_version_string);
            printf("Date   : %s\n", vtss_compile_time);
        ...
        case 't':
        {
            vtss_trace_group_t group, group_all = (VTSS_TRACE_GROUP_COUNT + 1), grp;
            vtss_trace_conf_t  conf;
            vtss_trace_layer_t layer, lyr;
            vtss_trace_level_t level, lvl;
            BOOL               first = 1;
            char               c, c2;
            ...
            lyr = (c == 'a' ? VTSS_TRACE_LAYER_AIL :
                   c == 'c' ? VTSS_TRACE_LAYER_CIL : VTSS_TRACE_LAYER_COUNT);

            /* Parse trace group */
            if (lyr != VTSS_TRACE_LAYER_COUNT && (s = strstr(s + 1, ":")) != NULL) {
                c = s[1];
                c2 = s[2];
            }
            grp = (c == 'm' ? VTSS_TRACE_GROUP_COUNT :
                   c == 'd' ? VTSS_TRACE_GROUP_DEFAULT :
                   c == 'p' ? (c2 == 'o' ? VTSS_TRACE_GROUP_PORT : c2 == 'h' ?
                               VTSS_TRACE_GROUP_PHY : VTSS_TRACE_GROUP_PACKET) : 
                   c == 'q' ? VTSS_TRACE_GROUP_QOS :
                   c == 'l' ? VTSS_TRACE_GROUP_L2 : group_all);

            /* Parse trace level */
            if (grp != group_all && (s = strstr(s + 1, ":")) != NULL)
                c = s[1];
            lvl = (c == 'o' ? VTSS_TRACE_LEVEL_NONE :
                   c == 'e' ? VTSS_TRACE_LEVEL_ERROR :
                   c == 'i' ? VTSS_TRACE_LEVEL_INFO :
                   c == 'd' ? VTSS_TRACE_LEVEL_DEBUG :
                   c == 'n' ? VTSS_TRACE_LEVEL_NOISE : VTSS_TRACE_LEVEL_COUNT);

            for (layer = 0; layer < VTSS_TRACE_LAYER_COUNT; layer++) {
                if (lyr != VTSS_TRACE_LAYER_COUNT && lyr != layer)
                    continue;
                
                for (group = 0; group <= VTSS_TRACE_GROUP_COUNT; group++) {
                    if (grp != group_all && grp != group)
                        continue;
                    ...
                    if (lvl == VTSS_TRACE_LEVEL_COUNT) {
                        /* Show levels */
                        if (first) {
                            first = 0;
                            printf("Layer  Group    Level\n");
                            printf("-----  -------  -----\n");
                        } 
...
#endif
        if (board->board_init(argc - count, argv + count, board))
            return 1;
