...
static u8 si5326_val[BS_MAX][SI5326_MAX_FIELDS] = {
   ...
   [BS_66_64]       = {0x00, 0x01, 0x00, 0x0A, 0x02, 0x01, 0x02, 0x07, 0x40, 0x00, 0x00, 0x03, 0x00, 0x00, \
...
static vtss_rc dd_get_block_status(int block, vtss_config_mode_t mode, int *block_state)
{
...
vtss_rc d_init_channel_clock_sw(int channel, vtss_config_mode_t mode, u32 xfi_clk)
{
    int target;
    int portoffset;
    int block_status;
    ...
    VTSS_RC(dd_get_block_status(BLOCK_SI5338_2, mode, &block_status));
    if ((mode == VTSS_CONFIG_MODE_PEE_P || mode == VTSS_CONFIG_MODE_PEE_MP)&& (xfi_clk == 161))
    ...
    VTSS_RC(dd_get_block_status(BLOCK_SI5338_1, mode, &block_status));
    ...
    VTSS_RC(dd_get_block_status(BLOCK_SI5326_2, mode, &block_status));
    ...
    VTSS_RC(dd_get_block_status(BLOCK_SI5326_1, mode, &block_status));
...
vtss_rc daytona_eval_init(int inst, vtss_config_mode_t mode_0, vtss_config_mode_t mode_1, u32 xfi_clk0, u32 xfi_clk1)
{
    /* Initialize board */
#ifdef BOARD_TALLADEGA
    if (inst == 0)
    ...
    if (inst == 0 && mode_0 != VTSS_CONFIG_MODE_DISABLED)
      if (d_init_channel_clock_sw(0, mode_0, xfi_clk0) != VTSS_RC_OK) {
        T_E("d_init_channel_clock_sw returned Failure for channel 0");
        return VTSS_RC_ERROR;
      }
    if (inst == 0 && mode_1 != VTSS_CONFIG_MODE_DISABLED)
      if (d_init_channel_clock_sw(1, mode_1, xfi_clk1) != VTSS_RC_OK) {
        T_E("d_init_channel_clock_sw returned Failure for channel 1");
        return VTSS_RC_ERROR;
      }

#ifdef BOARD_TALLADEGA
    if (inst == 1 && mode_0 != VTSS_CONFIG_MODE_DISABLED)
      if (d_init_channel_clock_sw(2, mode_0, xfi_clk0) != VTSS_RC_OK) {
        T_E("d_init_channel_clock_sw returned Failure for channel 2");
        return VTSS_RC_ERROR;
      }

    if (inst == 1 && mode_1 != VTSS_CONFIG_MODE_DISABLED)
      if (d_init_channel_clock_sw(3, mode_1, xfi_clk1) != VTSS_RC_OK) {
                           ...
                           u32                  *const value)
{
    ...
    *value=0;
    ...
    if (ioctl(pi_fd,VITGENIO_16BIT_READ,&pibuf) < 0) {
        VTSS_E ("PI Read operation failed: %u\n", *value);
        rc = VTSS_RC_ERROR;
    } else {
        *value = pibuf.value;
                            ...
                            const u32            value)
{
    ...
    if (ioctl(pi_fd,VITGENIO_16BIT_WRITE,&pibuf) < 0) {
        VTSS_E ("PI Write operation failed: %u\n", value);
        rc = VTSS_RC_ERROR;
    }
...
//        printf("rabbit_ip: Rabbit board IP address\n\n");
    ...
    if(ev_init) {
        ...
        if(daytona_eval_init(inst, conf->conf_mode[0], conf->conf_mode[1], conf->xfi_clk[0], conf->xfi_clk[1]) != VTSS_RC_OK) {
...
static void daytona_board_init(void)
{
    ...
    if (ioctl(pi_fd, VITGENIO_CS_SETUP_TIMING, &timing) < 0) {
        VTSS_E("Failure in PI timing setups\n");
    }
    VTSS_D("Using CS%d", timing.cs);
    if (ioctl(pi_fd, VITGENIO_CS_SELECT, timing.cs) < 0) {
        VTSS_E("Failure in PI chip selection\n");
    } 

    /* Control the PI_HOLD and PI_WAIT */
    pibuf.offset = ((VTSS_TO_CFG_IF_FAST_REGS << 16) + (0x3 << 1)) >> 1;
    pibuf.value  = 0x00000778;

    if (ioctl(pi_fd,VITGENIO_16BIT_WRITE,&pibuf) < 0) {
        VTSS_E ("PI write failed..%u\n", value);
        value=0;
    } 
#endif /* !DAYTONA_CHIP_SIMULATE */
...
void vtss_board_daytona_init(vtss_board_t *board)
{
