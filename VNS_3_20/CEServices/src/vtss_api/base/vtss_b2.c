...
{ \
    vtss_rc rc; \
    if ((rc = b2_rd(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, port, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_rd(VTSS_TGT_##tgt, VTSS_ADDX_##tgt##_##addr(x), 0, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_rd(VTSS_TGT_##tgt, VTSS_ADDXY_##tgt##_##addr(x,y), 0, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_wr(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, port, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_wr(VTSS_TGT_##tgt, VTSS_ADDX_##tgt##_##addr(x), 0, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    u32 val_tmp; \
    vtss_rc rc; \
    if ((rc = b2_rd(VTSS_TGT_##tgt, VTSS_ADDX_##tgt##_##addr(x), 0, &val_tmp))<VTSS_RC_OK) \
        return rc; \
...
{ \
    vtss_rc rc; \
    if ((rc = b2_wr(VTSS_TGT_##tgt, VTSS_ADDXY_##tgt##_##addr(x,y), 0, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_rdf(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, VTSS_OFF_##tgt##_##addr##_##fld, VTSS_LEN_##tgt##_##addr##_##fld, port, value))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_wrf(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, VTSS_OFF_##tgt##_##addr##_##fld, VTSS_LEN_##tgt##_##addr##_##fld, port, value))<VTSS_RC_OK) \
        return rc; \
}
...
/* Read/write register */
static vtss_rc b2_rd_wr(u32 tgt, u32 addr, u32 port, u32 *value, BOOL read)
{
    switch (tgt) {
    case VTSS_TGT_ASM:
    ...
    VTSS_RC(b2_pi_rd_wr(tgt, addr, value, read));
...
/* Read register */
static vtss_rc b2_rd(u32 tgt, u32 addr, u32 port, u32 *value)
{
    return b2_rd_wr(tgt, addr, port, value, 1);
}

/* Write register */
static vtss_rc b2_wr(u32 tgt, u32 addr, u32 port, u32 value)
{
...
/* Read register field */
static vtss_rc b2_rdf(u32 tgt, u32 addr, u32 offset, u32 length, u32 port, u32 *value)
{
...
/* Write register field */
static vtss_rc b2_wrf(u32 tgt, u32 addr, u32 offset, u32 length, u32 port, u32 value)
{
    u32 val, mask;
...
static vtss_rc b2_rev_c_check(const char *feature)
{
    if (vtss_state->chip_id.revision < 3) {
...
static BOOL b2_port_is_host(int chip_port) 
{
...
static vtss_rc b2_miim_cmd(u32 cmd, u32 sof, vtss_miim_controller_t miim_controller,
                           u8 miim_addr, u8 addr, u16 *data, BOOL report_errors)
{
  static BOOL  miim_addr_err[256];
    u32 i, n;
    u32 value;


    if (miim_addr_err[miim_addr] ) {
      return VTSS_RC_ERROR;      
    }
 ...
 mmd_error:
    ...
    miim_addr_err[miim_addr] = TRUE;
    return VTSS_RC_ERROR;

}

static vtss_rc b2_miim_read(vtss_miim_controller_t miim_controller, 
                            u8 miim_addr, u8 addr, u16 *value, BOOL report_errors)
{
    return b2_miim_cmd(PHY_CMD_READ, 1, miim_controller, miim_addr, addr, value, report_errors);
}

static vtss_rc b2_miim_write(vtss_miim_controller_t miim_controller, 
                             u8 miim_addr, u8 addr, u16 value, BOOL report_errors)
{
    return b2_miim_cmd(PHY_CMD_WRITE, 1, miim_controller, miim_addr, addr, &value, report_errors);
}

/* MMD (MDIO Management Devices (10G)) read */
static vtss_rc b2_mmd_read(vtss_miim_controller_t miim_controller, 
                           u8 miim_addr, u8 mmd, u16 addr, u16 *value, BOOL report_errors)
{
    VTSS_RC(b2_miim_cmd(PHY_CMD_ADDRESS, 0, miim_controller, miim_addr, mmd, &addr, 1));
    return b2_miim_cmd(PHY_CMD_READ, 0, miim_controller, miim_addr, mmd, value, 1);
}

/* MMD (MDIO Management Devices (10G)) write */
static vtss_rc b2_mmd_write(vtss_miim_controller_t miim_controller, 
                            u8 miim_addr, u8 mmd, u16 addr, u16 data, BOOL report_errors)
{
    VTSS_RC(b2_miim_cmd(PHY_CMD_ADDRESS, 0, miim_controller, miim_addr, mmd, &addr, 1));
    return b2_miim_cmd(PHY_CMD_WRITE, 0, miim_controller, miim_addr, mmd, &data, 1);
}
...
static vtss_rc b2_ct_setup(u32 chip_port, const vtss_port_ct_t *ct, int maxlen)
{
    ...
    VTSS_RC(b2_rev_c_check("Cut-through"));
    ...
    dport = vtss_state->dep_port[aport];
    ...
    if (host_mode < VTSS_HOST_MODE_8) {
        ...
        for (aport = aport_min; aport <= aport_max; aport++) {
            B2_WRX(QSS, PORT_Q_CT_TH, aport, tx_threshold);    
            B2_WRX(QSS, PORT_Q_CT_TH, vtss_state->dep_port[aport], rx_threshold);    
        }         
    }
    return VTSS_RC_OK;
}
...
static vtss_rc b2_port_setup(u32 port, vtss_port_conf_t *conf)
{
    const u8 *mac;
    ...
    VTSS_RC(b2_ct_setup(port, &conf->ct, conf->max_frame_length));  

    /* PFCI */
    if (conf->flow_control.pfci_enable) {
      VTSS_D("PFCI enabled");
      VTSS_RC(b2_rev_c_check("PFCI")); // Print error message if wrong chip revision
    }
    VTSS_RC(b2_wrf(VTSS_TGT_QSS, VTSS_ADDR_QSS_PFCI_LPORT_ENA, b2_port2line(port), 
...
/* Determine queue ID */
static u32 b2_qid(u32 dport, u32 queue)
{
    return ((dport - (vtss_state->init_conf.host_mode < VTSS_HOST_MODE_4 ? 24 : 48))*B2_PRIOS + queue);
}

/* Flush queue */
static vtss_rc b2_queue_flush(u32 port, u32 queue)
{
    u32 i, value;
...
static vtss_rc b2_port_conf_1g_set(u32 port, vtss_port_conf_t *conf)
{
    ...
    /* Common port setup */
    VTSS_RC(b2_port_setup(port, conf));
...
static vtss_rc b2_port_conf_10g_set(u32 port, vtss_port_conf_t *conf)
{
    u32               disabled;
    vtss_host_mode_t  host_mode;
    BOOL              hih;
...
static vtss_rc b2_port_conf_set(vtss_port_no_t port_no)
{
    u32              port = vtss_state->port_map[port_no].chip_port;
    vtss_port_conf_t *conf = &vtss_state->port_conf[port_no];

    if (b2_port_is_host(port)) 
        return VTSS_RC_ERROR;
       
    
    if (VTSS_PORT_IS_10G(port))
        return b2_port_conf_10g_set(port, conf);
    else
        return b2_port_conf_1g_set(port, conf);
}

static vtss_rc b2_cnt(u32 tgt, u32 addr, u32 port, vtss_chip_counter_t *counter, BOOL clear)
{
    vtss_rc rc;
    u32     value;
    
    /* Read chip counter */
    if ((rc = b2_rd(tgt, addr, port, &value)) != VTSS_RC_OK)
        return rc;
...
{ \
    vtss_rc rc; \
    if ((rc = b2_cnt(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, port, counter, clear))<VTSS_RC_OK) \
        return rc; \
}
...
{ \
    vtss_rc rc; \
    if ((rc = b2_cnt(VTSS_TGT_##tgt, VTSS_ADDX_##tgt##_##addr(x), 0, counter, clear))<VTSS_RC_OK) \
        return rc; \
}

static vtss_rc b2_port_counters_cmd(const vtss_port_no_t port_no, 
                                    vtss_port_counters_t *const counters, BOOL clr)
{
    ...
    port = vtss_state->port_map[port_no].chip_port;
    ...
    aport = b2_port2line(port);
    dport = vtss_state->dep_port[aport];
    c = &vtss_state->port_counters[port_no].counter.b2;
...
static vtss_rc b2_port_counters_update(const vtss_port_no_t port_no)
{
    return b2_port_counters_cmd(port_no, NULL, 0);
}

static vtss_rc b2_port_counters_clear(const vtss_port_no_t port_no)
{
    return b2_port_counters_cmd(port_no, NULL, 1);
}

static vtss_rc b2_port_counters_get(const vtss_port_no_t port_no,
                                    vtss_port_counters_t *const counters)
{
    memset(counters, 0, sizeof(*counters)); 
    return b2_port_counters_cmd(port_no, counters, 0);
}

/* ================================================================= *
 *  Port Filters
 * ================================================================= */
static vtss_rc b2_port_filter_set(const vtss_port_no_t port_no, 
                                  const vtss_port_filter_t * const filter)
{
    ...
    line_port = b2_port2line(vtss_state->port_map[port_no].chip_port);
...
static u32 b2_min_weight(u32 weight)
{
...
static vtss_rc b2_weight(vtss_bitrate_t rate, u32 *weight)
{
    u32 max;
    u64 w;
...
static vtss_rc b2_scheduler_check(void)
{
    ...
    if (host_mode == VTSS_HOST_MODE_0 || host_mode == VTSS_HOST_MODE_10) /* XAUI0 */
        rate = vtss_state->xaui[0].qos.shaper.rate;
    else if (host_mode == VTSS_HOST_MODE_1 || host_mode == VTSS_HOST_MODE_11) /* XAUI1 */
        rate = vtss_state->xaui[1].qos.shaper.rate;
    else { /* BOTH, take the largest value*/
        if (vtss_state->xaui[1].qos.shaper.rate > vtss_state->xaui[0].qos.shaper.rate)
            rate = vtss_state->xaui[1].qos.shaper.rate;
        else
            rate = vtss_state->xaui[0].qos.shaper.rate;
    }
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        if (host_mode > VTSS_HOST_MODE_6) {
            ...
            rate = vtss_state->qos_port_conf[port_no].shaper_port.rate;
            chip_port = vtss_state->port_map[port_no].chip_port;
        ...
        if (vtss_state->port_map[port_no].chip_port < 0)
            continue;
        rate = b2_min_rate(vtss_state->qos_port_conf[port_no].scheduler.rate);
    ...
    if (host_mode > VTSS_HOST_MODE_6) {
        ...
        for (lport_no = 0; lport_no < count; lport_no++) {
            ...
            rate = b2_min_rate(vtss_state->qos_lport[lport_no].scheduler.rx_rate);
            spi4_rate += rate;
            
            /* Tx rate towards XAUI */
            rate = b2_min_rate(vtss_state->qos_lport[lport_no].scheduler.tx_rate);
            if (lport_no < (count/2))
                x0_rate += rate;
            else
                x1_rate += rate;
...
/* Setup shaper */
static vtss_rc b2_shaper_setup(u32 port, const vtss_shaper_t *shaper)
{
    u32 value;
...
/* Setup lport shaper */
static vtss_rc b2_shaper_lport_setup(uint lport, const vtss_shaper_t *shaper)
{
    u32 value, value2;
    ...
    if (lport < 32 ) {
        VTSS_RC(b2_wrf(VTSS_TGT_SCH, VTSS_ADDR_SCH_PP_SPR_PORT_ENABLE_LS, lport, 1, 0, 
               shaper->rate == VTSS_BITRATE_DISABLED ? 0 : 1));
    } else {
        VTSS_RC(b2_wrf(VTSS_TGT_SCH, VTSS_ADDR_SCH_PP_SPR_PORT_ENABLE_MS, lport-32, 1, 0, 
               shaper->rate == VTSS_BITRATE_DISABLED ? 0 : 1));
    }
...
static vtss_rc b2_policer_setup(int offset, const vtss_policer_t *policer, BOOL queue)
{
    u32 cfg, level, rate;
...
static vtss_rc b2_port_qos_set(int chip_port, const vtss_qos_port_conf_t *qos)
{
    ...
    VTSS_RC(b2_wrf(VTSS_TGT_ANA_CL, VTSS_ADDR_ANA_CL_DSCP_REMAP_IDX_CFG, line_port, 1, 0, qos->dscp_table_no));
    ...
    for (indx = 0; indx < VTSS_PORT_POLICERS; indx++) {
        VTSS_RC(b2_policer_setup(VTSS_PORT_POLICERS * line_port + indx, 
    ...
    VTSS_RC(b2_shaper_setup(line_port, &qos->shaper_port));

    /** Channel shaper (1 shaper per lport) **/
    VTSS_RC(b2_shaper_lport_setup(line_port, &qos->shaper_host));
...
static vtss_rc b2_qos_port_conf_set(vtss_port_no_t port_no)
{
    vtss_host_mode_t host_mode;
    vtss_lport_no_t  lport_no;
    u32              weight;
    ...
    if (vtss_state->port_map[port_no].chip_port == CHIP_PORT_SPI4)
        return VTSS_RC_ERROR;
    ...
    VTSS_RC(b2_port_qos_set(vtss_state->port_map[port_no].chip_port, qos));
    ...
    if (host_mode < VTSS_HOST_MODE_8) {
        ...
        if (host_mode > VTSS_HOST_MODE_3) {
            ...
            if (vtss_state->spi4.ob.frame_interleave == 0 ) { 
                /* Burst interleave */
...
/* Global chip QoS  */
static vtss_rc b2_qos_conf_set(BOOL changed)
{
    int glb_no, filt_no, pair_no, offset;
    vtss_qos_conf_t *qos;

    qos = &vtss_state->qos_conf;
    ...
    for (pair_no = 0; pair_no < VTSS_UDP_TCP_PAIR_COUNT; pair_no++) {        
        ...
        VTSS_RC(b2_wrf(VTSS_TGT_ANA_CL, VTSS_ADDR_ANA_CL_ENDPT_GLOBAL_TCPUDP_RNG_CFG, pair_no, 1, 0, qos->udp_tcp[pair_no].range));
    }    
...
static vtss_rc b2_dscp_table_set(const vtss_dscp_table_no_t table_no)
{
    uint indx, offset;
    ...
    dscp_table = vtss_state->dscp_table[table_no];
...
static vtss_rc b2_chip_id_get(vtss_chip_id_t *const chip_id)
{
    u32 value;
...
static vtss_rc b2_reg_read(const vtss_chip_no_t chip_no, const u32 reg, u32 *const value)
{
    return b2_pi_rd_wr(reg >> 16, reg & 0xffff, value, 1);
}

static vtss_rc b2_reg_write(const vtss_chip_no_t chip_no, const u32 reg, const u32 value)
{
    u32 val = value;
    return b2_pi_rd_wr(reg >> 16, reg & 0xffff, &val, 0);
}

/* Poll bit until zero */
static vtss_rc b2_poll_bit(u32 tgt, u32 addr, u32 offset)
{
    u32 value, count = 0;
...
{ \
    vtss_rc rc; \
    if ((rc = b2_poll_bit(VTSS_TGT_##tgt, VTSS_ADDR_##tgt##_##addr, VTSS_OFF_##tgt##_##addr##_##fld)) < VTSS_RC_OK) \
        return rc; \
}

/* Initialize ANA_CL and ANA_AC */
static vtss_rc b2_ana_init(vtss_init_conf_t *conf)
{
    u32 port, queue, i;
...
/* Initialize ASM */
static vtss_rc b2_asm_init(vtss_host_mode_t host_mode)
{
...
static vtss_rc b2_qss_setup(void)
{
    ...
    vtss_spi4_conf_t *spi4 = &vtss_state->spi4;
    vtss_qs_conf_t   *qs;
    int              aport, dport;
    vtss_host_mode_t host_mode;
    vtss_port_no_t   port_no;
    ...
    if (host_mode < VTSS_HOST_MODE_8) {
        ...
        lport_count = 0;
        for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
            if ((chip_port = vtss_state->port_map[port_no].chip_port) < 0 || b2_port_is_host(chip_port))
                continue;
            lport_count++;
        ...
        for (port = 0; port < lport_count; port++) {
            ...
            vtss_state->dep_port[aport] = dport;
            vtss_state->dep_port[dport] = aport;
        }
    }
    ...
    for (port = port_min; port <= port_max; port++) {
        for (chan = chan_min; chan <= chan_max; chan++) {
            aport = (host_mode > VTSS_HOST_MODE_6 ? 
                     ...
                     b2_port2line(port));
            if ((dport = vtss_state->dep_port[aport]) < 0)
            ...
            for (i = rx; i <= tx; i++) {
                ...
                if (i == rx) {
                    ...
                    if (qs->qs_6 == VTSS_QUEUE_STRICT_RESERVE) {
                        ...
                        B2_WRX(QSS, Q_ENA, b2_qid(dport, 6), 0); 
                    }    /* Default enabled */
                    ...
                    if (qs->qs_7 == VTSS_QUEUE_STRICT_RESERVE) {
                        ...
                        B2_WRX(QSS, Q_ENA, b2_qid(dport, 7), 0); 
                    } /* Default enabled */

                    for (a=0; a<6; a++) {
                        if (qs->qw[a] == VTSS_QUEUE_WEIGHT_MAX_ENABLE) {
                            B2_WRX(QSS, Q_MAX, b2_qid(dport, a), q_max);
                        } else if (qs->qw[a] == VTSS_QUEUE_WEIGHT_DISABLE) {
                            B2_WRX(QSS, Q_ENA, b2_qid(dport, a), 0); 
                        } /* Default enabled */
                    }
                } else {
                    /* Tx */
...
/* Initialize QSS */
static vtss_rc b2_qss_init(void)
{    
...
static vtss_rc b2_host_flow_ctrl(void) {
  BOOL              fc_spi4, fc_xaui0, fc_xaui1, ibfc_xaui0, ibfc_xaui1 ;
  

/* Channel flowcontrol */
  fc_spi4 = (vtss_state->spi4.fc.enable ? 1 : 0);
  fc_xaui0 = (vtss_state->xaui[0].fc.channel_enable ? 1 : 0);
  fc_xaui1 = (vtss_state->xaui[1].fc.channel_enable ? 1 : 0);
  ibfc_xaui0 = (vtss_state->xaui[0].fc.ib.enable ? 1 : 0);
  ibfc_xaui1 = (vtss_state->xaui[1].fc.ib.enable ? 1 : 0);
...
/* Initialize SCH */
static vtss_rc b2_sch_init(vtss_init_conf_t *conf)
{
    u32 i, w;
    ...
    /* Scheduler weights */
    VTSS_RC(b2_weight(VTSS_BITRATE_DISABLED, &w));
...
static u32 b2_lport_count(void)
{
    u32 iport, count = 0;

    for (iport = 0; iport < VTSS_INT_PORT_COUNT; iport++)
        if (vtss_state->dep_port[iport] >= 0)
            count++;
    return (count/2);
}

/* Initialize SPI4.2 */
static vtss_rc b2_spi4_conf(void)
{
    int              i, count;
    vtss_spi4_conf_t *spi4;
    u32              vco, cmu, w;
    ...
    if (host_mode < VTSS_HOST_MODE_4) {
        ...
        if (spi4->ob.fcs_strip || spi4->ob.hih_length_update)
            VTSS_RC(b2_rev_c_check("SPI4 FCS stripping"));
        ...
        B2_WR(DEVCPU_GCB, CHIP_MODE, 0,
              ...
              ((vtss_state->xaui[0].status_select == VTSS_XAUI_STATUS_XAUI_1 ? 1 : 0) <<
              ...
              ((vtss_state->xaui[0].hih.format == VTSS_HIH_PRE_SFD ? 1 : 0) << 
        ...
        fc_xaui0 = (vtss_state->xaui[0].fc.three_level_enable ? 1 : 0);
        fc_xaui1 = (vtss_state->xaui[1].fc.three_level_enable ? 1 : 0);
        B2_WR(QSS, FC_3LVL, 0,
        ...
        if (vtss_state->chip_id.revision == 2) {
...
static vtss_rc b2_xaui_ibfc_setup(u32 port, vtss_xaui_conf_t *xaui)
{  
    ...
    if (port == CHIP_PORT_10G_0) {
        ...
        for(aport=0; aport <= 23; aport++) {
            ...
            if ((vtss_state->dep_port[aport]) == -1) 
                lport = 0x3f; /*  logical port is disabled */
...
/* Initialize XAUI */
static vtss_rc b2_xaui_conf(vtss_port_no_t port_no)
{
    ...
    /* XAUI setup */
    xaui = &vtss_state->xaui[port-CHIP_PORT_10G_0];

    /* Check revision C features */
    if (xaui->status_select != VTSS_XAUI_STATUS_BOTH)
        VTSS_RC(b2_rev_c_check("single XAUI status channel"));
    if (xaui->status_select != VTSS_XAUI_STATUS_BOTH && 
    ...
    B2_WR(DEVCPU_GCB, CHIP_MODE, 0,
          ...
          (0 << VTSS_OFF_DEVCPU_GCB_CHIP_MODE_SYNC_E_CLK_A_DRIVE_EN) |
          (0 << VTSS_OFF_DEVCPU_GCB_CHIP_MODE_STD_PREAMBLE_ENA) |
          ((vtss_state->spi4.ob.frame_interleave ? 0 : 1) << 
           VTSS_OFF_DEVCPU_GCB_CHIP_MODE_SPI4_INTERLEAVE_MODE) |
    ...
    vtss_state->xaui[port-CHIP_PORT_10G_0?0:1].status_select = xaui->status_select;
    vtss_state->xaui[port-CHIP_PORT_10G_0?0:1].hih.format = xaui->hih.format;

    /* 3 level flowcontrol */
    fc_spi4 = (vtss_state->spi4.fc.three_level_enable ? 1 : 0);
    fc_xaui0 = (vtss_state->xaui[0].fc.three_level_enable ? 1 : 0);
    fc_xaui1 = (vtss_state->xaui[1].fc.three_level_enable ? 1 : 0);
    B2_WR(QSS, FC_3LVL, 0,
          (2 << VTSS_OFF_QSS_FC_3LVL_DISABLED_PORT_FC) |
    ...
    VTSS_RC(b2_port_conf_10g_set(port, &conf));
    /* Store the host port conf in vtss_state */
    for (p = VTSS_PORT_NO_START; p < VTSS_PORT_NO_END; p++) {
        if ((vtss_state->port_map[p].chip_port) == port) {
            vtss_state->port_conf[p] = conf;
            break;
        }
    }
    ...
    VTSS_RC(b2_xaui_ibfc_setup(port, xaui));
...
static vtss_rc b2_host_conf_set(vtss_port_no_t port_no)
{
    u32     port = vtss_state->port_map[port_no].chip_port;
   
    /* Setup flow control */
    VTSS_RC(b2_host_flow_ctrl());

    if (VTSS_PORT_IS_10G(port))
        return b2_xaui_conf(port_no);
...
/* Setup port map */
static vtss_rc b2_port_map_set(void)
{
    vtss_port_no_t   port_no;
    int              chip_port;
    vtss_host_mode_t host_mode;
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        chip_port = vtss_state->port_map[port_no].chip_port;
        if (chip_port < 0 || b2_port_is_host(chip_port))
            continue;
        lport_no = vtss_state->port_map[port_no].lport_no;
        ...
        VTSS_D("API port:%u->chip port:%d->arr_port:%d->dep_port:%d. Arr_port:%d->dep_port:%d",
               port_no,chip_port,b2_port2line(chip_port),dport,dport,vtss_state->dep_port[dport]);
...
static vtss_rc b2_port_forward_state_set(vtss_port_no_t      port_no)
{
    int rx, tx, queue;
    vtss_host_mode_t host_mode;
    ...
    rx = VTSS_PORT_RX_FORWARDING(vtss_state->port_forward[port_no]);
    ...
    tx = VTSS_PORT_TX_FORWARDING(vtss_state->port_forward[port_no]);
    if (tx) {
        /* Enable Tx queue */
    ...
    for (iport = 0; iport < VTSS_INT_PORT_COUNT; iport++) {
        if (vtss_state->dep_port[iport] == aport) {
...
/* Setup PSI */
static vtss_rc b2_psi_set(const u32 clock)
{
...
/* Get PSI clock */
static vtss_rc b2_psi_get(u32 * clock)
{
  // See PSI Configuration Register in datasheet for understanding this function.
...
static vtss_rc b2_100fx_status_get(const vtss_port_no_t  port_no,
                                   vtss_port_status_t *status)
{
    u32 value;
    uint port = vtss_state->port_map[port_no].chip_port;
...
static vtss_rc b2_10g_status_get(vtss_port_no_t port_no, vtss_port_status_t * const status)
{
    uint port = vtss_state->port_map[port_no].chip_port;
    u32 value;
    ...
    status->fdx = 1;
    status->speed = vtss_state->port_conf[port_no].speed;
...
static vtss_rc b2_port_status_get(const vtss_port_no_t  port_no, 
                                   vtss_port_status_t    *const status)
{
    vtss_port_conf_t *conf = &vtss_state->port_conf[port_no];

    if (conf->if_type == VTSS_PORT_INTERFACE_100FX) {
        return b2_100fx_status_get(port_no, status);
    }  else if (conf->if_type == VTSS_PORT_INTERFACE_XAUI) {
        return b2_10g_status_get(port_no, status);
    } else 
...
static vtss_rc b2_port_clause_37_control_set(const vtss_port_no_t port_no)
{
    u32 value;
...
static vtss_rc b2_port_clause_37_status_get(const vtss_port_no_t         port_no, 
...
static vtss_rc b2_vid2port_set(const vtss_vid_t     vid, 
                        const vtss_port_no_t port_no)
{
    vtss_host_mode_t host_mode;
    u32 enabled = 0, offset = 0, xaui = 0;

    VTSS_RC(b2_rev_c_check("VID to line port mapping"));
    ...
    if (port_no != VTSS_PORT_NO_NONE) {
        enabled = 1;
        offset = vtss_state->port_map[port_no].lport_no;
    ...
    VTSS_D("port = %d, enabled =%d, offset = %d, zaui = %d, host_mode =%d, vid =%d, vtss_state->port_map[port_no].lport_no = %d",
           port_no,enabled,offset,xaui,host_mode,vid,vtss_state->port_map[port_no].lport_no);
...
static vtss_rc b2_vid2lport_set(const vtss_vid_t      vid, 
                         const vtss_lport_no_t lport_no)
{
    vtss_host_mode_t host_mode;
    u32 enabled = 0, offset = 0, xaui = 0;

    VTSS_RC(b2_rev_c_check("VID to logical port mapping"));
...
static vtss_rc b2_qos_lport_conf_set(const vtss_lport_no_t lport_no)
{
    ...
    VTSS_RC(b2_shaper_lport_setup(lport_no, &qos->shaper));    
...
/* Set GPIO mode */
static vtss_rc b2_gpio_mode(const vtss_chip_no_t  chip_no, 
                            ...
                            const vtss_gpio_mode_t mode)
{
    ...
    case VTSS_GPIO_OUT:
    case VTSS_GPIO_IN:
        VTSS_RC(b2_wrf(VTSS_TGT_DEVCPU_GCB, VTSS_ADDR_DEVCPU_GCB_GPIO_OUTPUT_ENA, gpio_no, 1, 0, mode == VTSS_GPIO_OUT ? 1 : 0));
        break;
    default:
...
/* Read GPIO input pin */
static vtss_rc b2_gpio_read(const vtss_chip_no_t  chip_no, 
                            const vtss_gpio_no_t  gpio_no, 
                             BOOL                  *const value)
{
    u32 val;
    ...
    if (gpio_no < VTSS_GPIO_NO_END) {
        VTSS_RC(b2_rdf(VTSS_TGT_DEVCPU_GCB, VTSS_ADDR_DEVCPU_GCB_GPIO_I, gpio_no, 1, 0, &val));
        *value = VTSS_BOOL(val);
        return VTSS_RC_OK;
    } 
...
static vtss_rc b2_gpio_write(const vtss_chip_no_t  chip_no, 
                             const vtss_gpio_no_t  gpio_no, 
                             const BOOL            value)
{
    if (gpio_no < VTSS_GPIO_NO_END) {
        VTSS_RC(b2_wrf(VTSS_TGT_DEVCPU_GCB, VTSS_ADDR_DEVCPU_GCB_GPIO_O, gpio_no, 1, 0, value));
        return VTSS_RC_OK;
    }
...
static vtss_rc b2_gpio_clk_set(const vtss_gpio_no_t         gpio_no, 
                               const vtss_port_no_t         port_no,
                               const vtss_recovered_clock_t clk)
{
    ...
    /* Feature only supported in Chip Rev C  */
    VTSS_RC(b2_rev_c_check("recovered clock"));
    ...
    if (port_no == VTSS_PORT_NO_NONE) {
        ...
        if (gpio_no == 0) {
            ...
            for (p = VTSS_PORT_NO_START; p < VTSS_PORT_NO_END; p++) {
                port = vtss_state->port_map[p].chip_port;                                       
                if (VTSS_PORT_IS_1G(port))
    ...
    VTSS_RC(b2_gpio_mode(0, gpio_no, VTSS_GPIO_OUT));
...
static vtss_rc b2_intr_mask_set(vtss_intr_t *mask) {
  vtss_port_no_t port_no;
  u32            chip_port;    
  ...
  for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {        
    ...
    if_100fx = 0;
    if_xaui = 0;
    if (vtss_state->port_conf[port_no].if_type == VTSS_PORT_INTERFACE_100FX) {
      if_100fx = 1;
    } else if (vtss_state->port_conf[port_no].if_type == VTSS_PORT_INTERFACE_XAUI) {
      if_xaui = 1;
    } else if (vtss_state->port_conf[port_no].if_type != VTSS_PORT_INTERFACE_SERDES) {
      continue;
    } 
    
    /* Link change interrupt not supported for XAUI and 100FX in chip rev A/B     */
    if (if_xaui || if_100fx)
      VTSS_RC(b2_rev_c_check("XAUI/100FX link change interrupt"));
...
static vtss_rc b2_intr_status_get(vtss_intr_t *status) {
  vtss_port_no_t port_no;
...
static vtss_rc b2_init_conf_set(void)
{
    u32              value;
    int              port;
    vtss_host_mode_t host_mode;
    ...
    /* Read chip ID */
    VTSS_RC(b2_chip_id_get(&vtss_state->chip_id));
    ...
    if (host_mode > VTSS_HOST_MODE_9)
        VTSS_RC(b2_rev_c_check("host mode 10/11"));
    ...
    for (port = 0; port < 26; port++) {
        ...
        VTSS_RC(b2_port_qos_set(port, &vtss_state->qos_port_conf[0]));
        VTSS_RC(b2_port_filter_set(port, &vtss_state->port_filter[0]));
    }

    /* SCH initialization */
    VTSS_RC(b2_sch_init(conf));
    

    return VTSS_RC_OK;
}

vtss_rc vtss_b2_inst_create(void)
{
    vtss_cil_func_t *func = &vtss_state->cil_func;

    /* Initialization */
    func->init_conf_set = b2_init_conf_set;

    /* Miscellaneous */
    func->reg_read = b2_reg_read;
    func->reg_write = b2_reg_write;
    func->chip_id_get = b2_chip_id_get;
#if defined(VTSS_GPIOS)
    func->gpio_mode = b2_gpio_mode;
    func->gpio_read = b2_gpio_read;
    func->gpio_write = b2_gpio_write;
    func->gpio_clk_set = b2_gpio_clk_set;
#endif /* VTSS_GPIOS */
    /* Port control */
    func->miim_read = b2_miim_read;
    func->miim_write = b2_miim_write;
    func->mmd_read = b2_mmd_read;
    func->mmd_write = b2_mmd_write;
    func->port_status_interface_set = b2_psi_set;
    func->port_status_interface_get = b2_psi_get;
    func->port_forward_set  = b2_port_forward_state_set;
    func->port_map_set = b2_port_map_set;
    func->port_conf_set = b2_port_conf_set;
    func->port_counters_update = b2_port_counters_update;
    func->port_counters_clear = b2_port_counters_clear;
    func->port_counters_get = b2_port_counters_get;
    func->port_status_get = b2_port_status_get;
    func->port_clause_37_status_get = b2_port_clause_37_status_get;
    func->port_clause_37_control_set = b2_port_clause_37_control_set;
    func->host_conf_set = b2_host_conf_set;
    func->vid2port_set = b2_vid2port_set;
    func->vid2lport_set = b2_vid2lport_set;
    func->qos_port_conf_set = b2_qos_port_conf_set;
    func->qos_lport_conf_set = b2_qos_lport_conf_set;
    func->qos_conf_set = b2_qos_conf_set;
    func->dscp_table_set = b2_dscp_table_set;
    func->port_filter_set = b2_port_filter_set;

#if defined(VTSS_FEATURE_INTERRUPTS)
    func->intr_mask_set = b2_intr_mask_set;
    func->intr_status_get = b2_intr_status_get;
#endif

    /* State data */
    vtss_state->prio_count = B2_PRIOS;
#if defined(VTSS_GPIOS)
    vtss_state->gpio_count = VTSS_GPIOS;
#endif /* VTSS_GPIOS */
