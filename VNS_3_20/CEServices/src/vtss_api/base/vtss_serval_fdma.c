...
#include "vtss_common.h" /* For vtss_cmn_XXX() */
...
#define VTSS_FDMA_INJ_HDR_SIZE_BYTES 20 // Number of bytes in the injection header (16) + 4 for a VLAN tag (is @switch_frm == TRUE)
...
static vtss_rc xtr_cfg(vtss_state_t *const vstate, const vtss_fdma_ch_t xtr_ch, const vtss_fdma_ch_cfg_t *const cfg)
{
    ...
    ch_state->xtr_grp   = xtr_ch; // One-to-one correspondence between exxtraction group and channnel number.
    ...
    if (!props->dont_touch_ifh) {
        ...
        if (props->switch_frm) {
            ...
            if (props->ptp_action == VTSS_PACKET_PTP_ACTION_NONE && afi_id != VTSS_AFI_ID_NONE) {
                ...
                // since we don't call cil_func.tx_hdr_encode(), which will otherwise to it.
...
/*****************************************************************************/
static vtss_rc serval_fdma_inj_restart_ch(vtss_state_t *const vstate, vtss_fdma_ch_t inj_ch, fdma_sw_dcb_t *head, BOOL do_start, vtss_trace_group_t trc_grp)
{
    if (do_start) {
        // Channel must be inactive
        FDMA_CHECK_RC(trc_grp, (SRVL_RD(vstate, VTSS_ICPU_CFG_FDMA_FDMA_CH_SAFE) & VTSS_BIT(inj_ch)) != 0);
...
static FDMA_INLINE void inj_irq_tfr_done(vtss_state_t *vstate, void *const cntxt, vtss_fdma_ch_t inj_ch)
{
...
#if defined(VTSS_FEATURE_AFI_SWC)
    if (FDMA_SW_DCB(sof_l)->afi_id != VTSS_AFI_ID_NONE) {
...
#endif /* defined(VTSS_FEATURE_AFI_SWC) */
    {
        ...
        (void)vtss_fdma_cmn_dcb_release(vstate, 0 /* doesn't matter in FDMA v3+ */, FDMA_SW_DCB(sof_l), TRUE /* thread safe */, FALSE /* only used for XTR DCBs */, FALSE /* only used for XTR DCBs */);
#endif /* VTSS_OPT_FDMA_VER */
    }
}
...
/****************************************************************************/
static void afi_unlink(vtss_state_t *const vstate, vtss_afi_id_t afi_id)
{
                               ...
                               const vtss_fdma_inj_props_t *const props)
{
    ...
    if (props->ccm_cancel) {
        ...
        FDMA_CHECK_RC(FALSE, afi_alloc(vstate, sw_dcb, props->ccm_fps, &afi_id) == VTSS_RC_OK);
                              ...
                              BOOL                  afi_cancel)
{
    ...
    if (afi_cancel) {
        // Find the AFI frame
        vtss_afi_id_t afi_id = afi_find_frame(vstate, FDMA_USER_DCB(sw_dcb)->frm_ptr);
        FDMA_ASSERT(NORMAL, afi_id != VTSS_AFI_ID_NONE, return VTSS_RC_ERROR;);
        ...
        FDMA_CHECK_RC(FALSE, afi_alloc(vstate, sw_dcb, fdma_info->afi_fps, &tx_info->afi_id) == VTSS_RC_OK);

        // Indicate that this is an AFI frame.
        sw_dcb->afi_id = tx_info->afi_id;
        // Fall through to the injection stuff.
    } else {
        // Indicate that this is not an AFI frame.
        sw_dcb->afi_id = tx_info->afi_id = VTSS_AFI_ID_NONE;
    }
#endif /* defined(VTSS_FEATURE_AFI_SWC) */
...
/*****************************************************************************/
void serval_fdma_func_init(void)
{
