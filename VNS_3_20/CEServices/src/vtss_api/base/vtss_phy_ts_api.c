...
#define VTSS_PHY_TS_READ_CSR(p, b, a, v)    \
                      vtss_phy_ts_read_csr(p , b, a, v)

#define VTSS_PHY_TS_WRITE_CSR(p, b, a, v)   \
                      vtss_phy_ts_write_csr(p , b, a, v)
...
#ifdef VTSS_CHIP_10G_PHY
vtss_rc vtss_phy_10g_id_get_priv(const vtss_port_no_t   port_no,
                                 vtss_phy_10g_id_t *const phy_id);
#endif

static vtss_rc vtss_phy_ts_base_port_get_priv(const vtss_port_no_t port_no,
...
typedef enum {
    ...
    VTSS_PHY_TS_OPER_MODE_INV = 2  /* Invalid Mode */
...
static vtss_rc vtss_daytona_chip_id_get_priv(vtss_chip_id_t *const chip_id);
#endif

static vtss_rc vtss_phy_ts_rev_get_priv(const vtss_port_no_t port_no,
                                       u16              *const revision);

#ifdef VTSS_CHIP_CU_PHY
vtss_rc vtss_phy_id_get_priv(const vtss_port_no_t   port_no,
                             vtss_phy_type_t *phy_id);
#endif

#if defined(VTSS_CHIP_CU_PHY) && defined(VTSS_PHY_TS_SPI_CLK_THRU_PPS0)
static vtss_rc vtss_phy_ts_channel_id_get_priv(const vtss_port_no_t port_no,
                                               u16     *const channel_id);
static vtss_rc vtss_phy_ts_spi_pause_priv(const vtss_port_no_t port_no);
static vtss_rc vtss_phy_ts_spi_unpause_priv(const vtss_port_no_t port_no);

static BOOL    is_spi_paused;
static u16     spi_prev_status;
...
{ \
    vtss_rc err; \
    if (vtss_state->phy_ts_port_conf[(p)].new_spi_conf.phy_type == VTSS_PHY_TYPE_8574) { \
        if ((err = vtss_phy_ts_spi_pause_priv((p))) != VTSS_RC_OK) { \
...
{ \
    vtss_rc err; \
    if (vtss_state->phy_ts_port_conf[(p)].new_spi_conf.phy_type == VTSS_PHY_TYPE_8574) { \
        if ((err = vtss_phy_ts_spi_unpause_priv((p))) != VTSS_RC_OK) { \
    ...
    vtss_phy_ts_oper_mode_t *oper_mode)
{
    ...
    if (vtss_phy_10g_id_get_priv(port_no, &phy_id_10g) == VTSS_RC_OK) {
    ...
    if (vtss_phy_id_get_priv(port_no, &phy_id_1g) == VTSS_RC_OK) {
...
#ifdef VTSS_ARCH_DAYTONA
    if (vtss_daytona_chip_id_get_priv(&chip_id) == VTSS_RC_OK) {
...
static vtss_rc vtss_phy_ts_read_csr(const vtss_port_no_t port_no,
                                    ...
                                    u32 *const value)
{
    ...
    VTSS_RC(vtss_phy_ts_register_access_type_get(cfg_port, &phy_type, &clause45, &oper_mode));
    ...
    case VTSS_PHY_TYPE_8487:
    {
        ...
        if (!clause45) return VTSS_RC_ERROR;

        if ((blk_id == VTSS_PHY_TS_PROC_BLK_ID(0)) || (blk_id == VTSS_PHY_TS_PROC_BLK_ID(1))) {
            reg_value = 0;
            VTSS_RC(mmd_read_func(cfg_port, VTSS_PHY_TS_10G_DATA_CLK_MMD_ADDR,
        ...
        VTSS_RC(mmd_read_inc_func(cfg_port, mmd_addr, reg_addr, (u16 *)&value32, 2));
        *value = value32;
        break;
    }
    case VTSS_PHY_TYPE_8488:
    ...
    case VTSS_PHY_TYPE_8574:
    {
         ...
         */
        /* 1588 - Page Selection */
        VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_ADDR_EXT_REG, 0x1588));
         ...
         */
        VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_BIU_ADDR_REG, reg_value));

        reg_value = 0;
        max_read = 0;
        do {
            max_read++;
            VTSS_RC(miim_read_func(cfg_port, VTSS_PHY_TS_1G_BIU_ADDR_REG, &reg_value));
         ...
         */
        VTSS_RC(miim_read_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_UPPER, &reg_value_upper));
        /* Read the lower word (lower 16 bits) from register 17
         */
        VTSS_RC(miim_read_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_LOWER, &reg_value_lower));
        /* Restore standard page
         */
        VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_ADDR_EXT_REG, VTSS_PHY_PAGE_STANDARD));

        *value = ((reg_value_upper << 16) | reg_value_lower);
        VTSS_I("Read CSR: port %u, blk_id %d, adr %x, value %x", port_no, blk_id, csr_address, *value);
        break;
    }
#endif /* VTSS_CHIP_CU_PHY */
#ifdef VTSS_ARCH_DAYTONA
    ...
    case VTSS_DEV_TYPE_8494:
    {
        ...
        if (port_no == 0 || port_no == 1) return VTSS_RC_ERROR;
        VTSS_RC(daytona_port_2_target(port_no, daytona_blk_id_map[actual_blk_id], &target));
...
static vtss_rc vtss_phy_ts_write_csr(const vtss_port_no_t port_no,
                                     const vtss_phy_ts_blk_id_t blk_id,
                                     const u16 csr_address,
                                     const u32 *const value)
{
    ...
    case VTSS_PHY_TYPE_8487:
    {
        ...
        if ((blk_id != VTSS_PHY_TS_PROC_BLK_ID(0)) && (blk_id != VTSS_PHY_TS_PROC_BLK_ID(1))) {
            ...
            VTSS_RC(mmd_write_func(cfg_port, mmd_addr, (reg_addr | 1), reg_value_upper));
            /* Write the Lower 2 Bytes */
            VTSS_RC(mmd_write_func(cfg_port, mmd_addr, reg_addr, reg_value_lower));
    ...
    case VTSS_PHY_TYPE_8574:
    {
        ...
        VTSS_RC(miim_read_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_UPPER, &base_reg_value));
        ...
        if((blk_id == 6 || blk_id == 7) && (csr_address == 0x10 ||
                ...
                if(!reg_value_upper) {
                    VTSS_I("Ignore upper value for blk_id %d, csr_address %x", blk_id, csr_address);
                } else {
                    base_reg_update = TRUE;
                    VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_UPPER, reg_value_upper));
                }
        } else {
            VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_UPPER, reg_value_upper));
        }
        /* Write the lower word data (lower 16 bits) to register 17 */
        VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_LOWER, reg_value_lower));
        ...
        if (base_reg_update != FALSE) {
            VTSS_RC(miim_write_func(cfg_port, VTSS_PHY_TS_1G_CSR_DATA_UPPER, base_reg_value));
        }
        break;
    }
#endif /* VTSS_CHIP_CU_PHY */
                                    ...
                                    const u32 *const value)
{
    ...
    VTSS_ENTER();
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) != VTSS_RC_OK) break;
        if ((rc = vtss_phy_ts_register_access_type_get(port_no, &phy_type, &clause45, &oper_mode)) != VTSS_RC_OK) break;

#ifdef VTSS_CHIP_10G_PHY
        if ((phy_type == VTSS_PHY_TYPE_8487) || (phy_type == VTSS_PHY_TYPE_8488)) {
            if ((rc = vtss_phy_ts_base_port_get_priv(port_no, &cfg_port)) != VTSS_RC_OK) break;
        }
#endif
#ifdef VTSS_CHIP_CU_PHY
...
static vtss_rc vtss_daytona_chip_id_get_priv(vtss_chip_id_t *const chip_id)
{
    ...
    if ((vtss_state->cil_func.chip_id_get != NULL) &&
        ((rc = vtss_state->cil_func.chip_id_get(chip_id)) == VTSS_RC_OK)) {
...
static vtss_rc vtss_phy_ts_block_init(const vtss_port_no_t port_no)
{
    u16                        phy_type = VTSS_PHY_TYPE_NONE;
    BOOL                       clause45 = FALSE;
    vtss_phy_ts_oper_mode_t    oper_mode = VTSS_PHY_TS_OPER_MODE_INV;
    ...
    VTSS_RC(vtss_phy_ts_register_access_type_get(port_no, &phy_type, &clause45, &oper_mode));
    VTSS_RC(vtss_phy_ts_base_port_get_priv(port_no, &base_port_no));
    ...
    case VTSS_PHY_TYPE_8488:
    {
        ...
        if (!clause45) return VTSS_RC_ERROR;
        VTSS_RC(mmd_read_func(base_port_no, mmd_addr, reg_addr, &reg_value));
        if (reg_value == 0xff01) {
            /* Already Enabled */
            return VTSS_RC_OK;
        } else {
        ...
        VTSS_RC(mmd_write_func(base_port_no, mmd_addr, reg_addr, reg_value));
    ...
    case VTSS_PHY_TYPE_8574:
        ...
        /* 1588 - Page Selection */
        VTSS_RC(vtss_state->init_conf.miim_write(base_port_no, VTSS_PHY_TS_1G_ADDR_EXT_REG, 0x1588));
        /* Write the lower word data to register 29 */
        VTSS_RC(vtss_state->init_conf.miim_write(base_port_no, 29, 0x7ae0));
        /* Write the upper word data to register 30 */
        VTSS_RC(vtss_state->init_conf.miim_write(base_port_no, 30, 0xb71c));
        break;
#endif /* VTSS_CHIP_CU_PHY */
#ifdef VTSS_ARCH_DAYTONA
    case VTSS_DEV_TYPE_8492:
    ...
    case VTSS_DEV_TYPE_8494:
    {
        ...
        VTSS_RC(VTSS_FUNC_COLD(pcs_10gbase_r_config_set, port_no));

        /* Configure the GPIO Pin Mux for 1PPS Output; pin 2 for 1PPS output
           for channel 0 and pin 10 for channel 1 */
        VTSS_RC(daytona_port_2_target(port_no, DAYTONA_BLOCK_DEVCPU_GCB, &target));
        ...
        VTSS_RC(vtss_state->cil_func.gpio_mode(0, ((port_no == 2) ? 2: 10), VTSS_GPIO_OUT));
        /* GPIO Pin Direction for 1PPS IN i.e. Load/Save: input */
        VTSS_RC(vtss_state->cil_func.gpio_mode(0, 1, VTSS_GPIO_IN));
        break;
    }
#endif /* VTSS_ARCH_DAYTONA */
    default:
...
static vtss_rc vtss_phy_ts_port_init(const vtss_port_no_t port_no,
                                     const vtss_phy_ts_init_conf_t *const conf)
{
    ...
    if ((phy_type == VTSS_DEV_TYPE_8492) || (phy_type == VTSS_DEV_TYPE_8494)) {
        ...
        VTSS_RC(vtss_state->cil_func.gpio_mode(0, 0, VTSS_GPIO_OUT));
        VTSS_RC(vtss_state->cil_func.gpio_mode(0, 8, VTSS_GPIO_OUT));
        VTSS_RC(vtss_state->cil_func.gpio_mode(0, 9, VTSS_GPIO_OUT));
    }
#endif
    ...
    switch (phy_type) {
        ...
        case VTSS_DEV_TYPE_8494:
        {
            ...
            VTSS_RC(daytona_port_2_mode(port_no, &channel_mode));
#ifdef VTSS_FEATURE_OTN
...
static vtss_rc vtss_phy_ts_analyzer_init_priv(const vtss_port_no_t port_no)
{
    vtss_phy_ts_blk_id_t blk_id;
...
static vtss_rc warm_wr_masked(const vtss_port_no_t port_no, u32 blok, u32 addr, u32 value, u32 mask, const char *function, const u16 line)
{
    u32 val;

    VTSS_RC(VTSS_PHY_TS_READ_CSR(port_no, blok, addr, &val));
    if (sync_calling_private) { /* Warm start sync - check for changes */
...
static vtss_rc vtss_phy_ts_csr_set_priv(const vtss_port_no_t port_no,
        const vtss_phy_ts_proc_conf_t conf)
{
    ...
    case VTSS_PHY_TS_RATE_ADJ_SET:
    {
        ...
        if (ti != 0) {
            ...
            if (val > 1000000000ULL) val = 1000000000ULL;
            value =
    ...
    case VTSS_PHY_TS_PORT_ENA_SET:
        ...
        if (enable) {
            ...
            value = VTSS_PHY_TS_CLR_BITS(value, VTSS_F_PTP_IP_1588_TOP_CFG_STAT_INTERFACE_CTL_BYPASS);
        } else {
    ...
    case VTSS_PHY_TS_PORT_EVT_MASK_SET:
        ...
        VTSS_RC(warm_wr_masked(port_no, VTSS_PHY_TS_PROC_BLK_ID(0), VTSS_PTP_INGR_IP_1588_CFG_STAT_INGR_INT_MASK, mask, w_mask, __FUNCTION__,  __LINE__));
        ...
        VTSS_RC(warm_wr_masked(port_no, VTSS_PHY_TS_PROC_BLK_ID(0), VTSS_PTP_EGR_IP_1588_CFG_STAT_EGR_INT_MASK, mask, w_mask, __FUNCTION__,  __LINE__));
...
static vtss_rc vtss_phy_ts_csr_ptptime_arm_priv(const vtss_port_no_t port_no)
{
    u32 value;
...
static vtss_rc vtss_phy_ts_csr_ptptime_set_done_priv(const vtss_port_no_t port_no)
{
    u32 value = 0;
...
static vtss_rc vtss_phy_ts_csr_event_poll_priv(const vtss_port_no_t  port_no,
...
static vtss_rc vtss_phy_ts_stats_get_priv(const vtss_port_no_t port_no,
...
static vtss_rc vtss_phy_ts_correction_overflow_get_priv(
                                                   const vtss_port_no_t port_no,
...
static vtss_rc vtss_phy_ts_csr_adj_1ns_set_priv(const vtss_port_no_t port_no,
                                                   BOOL incr)
{
    u32          value = 0;
...
static vtss_rc vtss_phy_ts_signature_set_priv(const vtss_port_no_t port_no)
{
    u32 value = 0, pos = 0;
    i32 byte_ct = 0;
    u8 sig_sel[16];
        ...
        for (engine_id = VTSS_PHY_TS_PTP_ENGINE_ID_0; engine_id <= VTSS_PHY_TS_PTP_ENGINE_ID_1;
            ...
            if (vtss_state->phy_ts_port_conf[port_no].egress_eng_conf[engine_id].eng_used == TRUE) {
                ...
                if ((encap_type == VTSS_PHY_TS_ENCAP_ETH_IP_PTP) ||
                    ...
                    VTSS_RC(vtss_phy_ts_ip1_sig_mask_set_priv(port_no, engine_id, blk_id));
                } else if (encap_type == VTSS_PHY_TS_ENCAP_ETH_IP_IP_PTP) {
                    VTSS_RC(vtss_phy_ts_ip2_sig_mask_set_priv(port_no, engine_id, blk_id));
                }
            }
        }
        ...
        for (engine_id = VTSS_PHY_TS_PTP_ENGINE_ID_0; engine_id <= VTSS_PHY_TS_PTP_ENGINE_ID_1;
            ...
            if (vtss_state->phy_ts_port_conf[port_no].egress_eng_conf[engine_id].eng_used == TRUE) {
                ...
                if ((encap_type == VTSS_PHY_TS_ENCAP_ETH_PTP) ||
                    ...
                    VTSS_RC(vtss_phy_ts_eth1_sig_mask_set_priv(port_no, engine_id, blk_id));
                } else if ((encap_type == VTSS_PHY_TS_ENCAP_ETH_ETH_PTP)      ||
                           (encap_type == VTSS_PHY_TS_ENCAP_ETH_ETH_IP_PTP)   ||
                           (encap_type == VTSS_PHY_TS_ENCAP_ETH_MPLS_ETH_PTP) ||
                           (encap_type == VTSS_PHY_TS_ENCAP_ETH_MPLS_ETH_IP_PTP)) {
                    VTSS_RC(vtss_phy_ts_eth2_sig_mask_set_priv(port_no, engine_id, blk_id));
                }
            }
        }
...
static vtss_rc vtss_phy_ts_sw_table_wr_priv(const vtss_port_no_t port_no,
...
#if defined(VTSS_PHY_TS_ENBL_TS_TABLE)
            /* Save the timestamp into SW TS table */
            vtss_phy_ts_sw_table_wr_priv(port_no, &signature, &ts, status);
#else
                                        ...
                                        const vtss_timeinterval_t    *const latency)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            if ((rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_ING_LATENCY_SET)) != VTSS_RC_OK) {
                                       ...
                                       const vtss_timeinterval_t    *const latency)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_EGR_LATENCY_SET)) != VTSS_RC_OK) {
                VTSS_E("Latency set fail, port %u", port_no);
                /* don't break, needs to unpause */
            }
            VTSS_PHY_TS_SPI_UNPAUSE(port_no);
                                   ...
                                   const vtss_timeinterval_t    *const path_delay)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PATH_DELAY_SET)) != VTSS_RC_OK) {
                                        ...
                                        const vtss_timeinterval_t   *const delay_asym)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_DELAY_ASYM_SET)) != VTSS_RC_OK) {
                VTSS_E("Latency set fail, port %u", port_no);
                /* don't break, needs to unpause */
            }
            VTSS_PHY_TS_SPI_UNPAUSE(port_no);
                                ...
                                const vtss_phy_timestamp_t  *const ts)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_ptptime_set_priv(port_no, ts)) != VTSS_RC_OK) {
                                ...
                                const vtss_port_no_t  port_no)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_ptptime_arm_priv(port_no)) != VTSS_RC_OK) {
                                ...
                                const vtss_port_no_t  port_no)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_ptptime_set_done_priv(port_no)) != VTSS_RC_OK) {
                                ...
                                vtss_phy_timestamp_t  *const ts)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_ptptime_get_priv(port_no, ts)) !=  VTSS_RC_OK) {
                                      ...
                                      const vtss_phy_ts_scaled_ppb_t *const adj)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_RATE_ADJ_SET)) != VTSS_RC_OK) {
                                   ...
                                   const BOOL            incr)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            if ((rc = vtss_phy_ts_csr_adj_1ns_set_priv(port_no, incr)) !=  VTSS_RC_OK) {
                                 ...
                                 const vtss_phy_ts_fifo_sig_mask_t sig_mask)
{
    ...
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_MSG_TYPE)  len += 1; /* PTP Msg Type = 1Byte */
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_DOMAIN_NUM) len += 1; /* PTP Dom Nm = 1Byte */
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_SOURCE_PORT_ID) len += 10; /* SRC Port Identity = 10 Bytes */
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_SEQ_ID) len += 2; /* Sequence Number = 2 Bytes*/
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_DEST_IP) len += 4; /* Dest IP = 4 Bytes */
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_SRC_IP) len += 4;  /* Src IP = 4 Bytes */
    if (sig_mask & VTSS_PHY_TS_FIFO_SIG_DEST_MAC) len += 6;  /* Dest MAC = 6 Bytes */
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {   
            ...
            if ((rc = vtss_phy_ts_base_port_get_priv(port_no, &base_port_no)) != VTSS_RC_OK) break;
            ...
            if((rc = vtss_phy_ts_signature_set_priv(base_port_no)) != VTSS_RC_OK) {
                               ...
                               const vtss_port_no_t    port_no)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            rc = vtss_phy_ts_fifo_empty_priv(inst, port_no);
        }
    } while(0);
...
#if defined(VTSS_PHY_TS_ENBL_TS_TABLE)
typedef struct {
    vtss_phy_ts_fifo_status_t status; /* HW TSFIFO status */
    vtss_phy_ts_fifo_sig_t    sig; /* signature */
    vtss_phy_timestamp_t      ts; /* timestamp */
} vtss_phy_ts_sw_tbl_entry_t;

typedef struct {
    u8                          num_entry;
    vtss_phy_ts_sw_tbl_entry_t  entry[VTSS_PHY_TS_FIFO_SIZE];
} vtss_phy_ts_sw_tbl_t;

static vtss_phy_ts_sw_tbl_t sw_ts_tbl[VTSS_PORTS];
...
{
    u32 index, num_entry;
    vtss_phy_ts_sw_tbl_entry_t *entry;

    num_entry = sw_ts_tbl[port_no].num_entry;
    if (num_entry < VTSS_PHY_TS_FIFO_SIZE) {
        entry = &sw_ts_tbl[port_no].entry[num_entry - 1];
        entry->status = status;
        memcpy(&entry->sig, sig, sizeof(vtss_phy_ts_fifo_sig_t));
        memcpy(&entry->ts, ts, sizeof(vtss_phy_timestamp_t));
        sw_ts_tbl[port_no].num_entry += 1;
    } else {
        /* table is full, delete the oldest entry which is at index 0 */
        for (index = 1; index < VTSS_PHY_TS_FIFO_SIZE; index++) {
            memcpy(&sw_ts_tbl[port_no].entry[index - 1],
                   &sw_ts_tbl[port_no].entry[index],
                   sizeof(vtss_phy_ts_sw_tbl_entry_t));
            /* change the status overflow as table gets overflow */
            sw_ts_tbl[port_no].entry[index - 1].status = VTSS_PHY_TS_FIFO_OVERFLOW;
        }
        /* copy the new entry into above created space */
        sw_ts_tbl[port_no].entry[VTSS_PHY_TS_FIFO_SIZE - 1].status = status;
        memcpy(&sw_ts_tbl[port_no].entry[VTSS_PHY_TS_FIFO_SIZE - 1].sig, sig,
               sizeof(vtss_phy_ts_fifo_sig_t));
        memcpy(&sw_ts_tbl[port_no].entry[VTSS_PHY_TS_FIFO_SIZE - 1].ts, ts,
               sizeof(vtss_phy_timestamp_t));
        /* num entry is same as one id deleted, new one added */
    }

    return VTSS_RC_OK;
}

vtss_rc vtss_phy_ts_fifo_entry_read(const vtss_inst_t            inst,
                                    const vtss_port_no_t         port_no,
                                    const vtss_phy_ts_fifo_sig_t *const sig,
                                    vtss_phy_timestamp_t         *const ts,
                                    vtss_phy_ts_fifo_status_t    *const status)
{
    u32 index, num_entry;

    VTSS_PHY_TS_ASSERT(sig == NULL);
    VTSS_PHY_TS_ASSERT(ts == NULL);

    VTSS_ENTER();
    VTSS_RC(vtss_inst_port_no_check(inst, port_no));

    num_entry = sw_ts_tbl[port_no].num_entry;
    for (index = 0; index < num_entry; index++) {
        if (!memcmp(&sw_ts_tbl[port_no].entry[index].sig, sig,
            sizeof(vtss_phy_ts_fifo_sig_t))) {
            break;
        }
    }
    if (index < num_entry) {
        /* found the match, return the timestamp */
        memcpy(ts, &sw_ts_tbl[port_no].entry[index].ts,
               sizeof(vtss_phy_timestamp_t));
        *status = sw_ts_tbl[port_no].entry[index].status;
        /* delete the entry */
        for ( ; index < (num_entry - 1); index++) {
            memcpy(&sw_ts_tbl[port_no].entry[index],
                   &sw_ts_tbl[port_no].entry[index + 1],
                   sizeof(vtss_phy_ts_sw_tbl_entry_t));
        }
        memset(&sw_ts_tbl[port_no].entry[index], 0,
               sizeof(vtss_phy_ts_sw_tbl_entry_t));
        sw_ts_tbl[port_no].num_entry -= 1;
    } else {
        memset(ts, 0, sizeof(vtss_phy_timestamp_t));
        *status = VTSS_PHY_TS_FIFO_OVERFLOW;
    }
                           ...
                           const vtss_phy_ts_ip_conf_t    *const def_conf)
{
    ...
    if (double_ip) {
        ...
        VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, def_conf->comm_opt.ip_mode, TRUE));
        ...
        /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
        VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, def_conf->comm_opt.ip_mode, FALSE));
                           ...
                           const vtss_phy_ts_ip_conf_t    *const def_conf)
{
    ...
    /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
    VTSS_RC(vtss_phy_ts_ip2_different_offset_set(port_no, blk_id, def_conf->comm_opt.ip_mode));
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_eth1_flow_conf_set_priv(eng_parm, eng_conf, new_flow_conf)));
    return VTSS_RC_OK;
}
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_eth2_flow_conf_set_priv(eng_parm, eng_conf, new_flow_conf)));
    return VTSS_RC_OK;
}
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_mpls_flow_conf_set_priv(eng_parm, eng_conf, new_flow_conf)));
    return VTSS_RC_OK;
}
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_ach_flow_conf_priv( eng_parm, eng_conf, new_flow_conf)));
    ...
    if (double_ip) {
        if (old_ip_conf->comm_opt.ip_mode != new_ip_conf->comm_opt.ip_mode) {
            if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_4) {
                ...
                VTSS_RC(vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 20)); /* default IPv4 header length */

                /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
                VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_4, TRUE));
            } else if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_6) {
                /* IP1 next protocol i.e. number of bytes in this header */
                VTSS_RC(vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 40)); /* default IPv6 header length */
                /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
                VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_6, TRUE));
            }
        }
        ...
        if (old_ip_conf->comm_opt.ip_mode != new_ip_conf->comm_opt.ip_mode) {
            if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_4) {
                /* IP1 next protocol i.e. number of bytes in this header */
                VTSS_RC(vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 28)); /* IPv4+UDP header length */
                /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
                VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_4, FALSE));
            } else if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_6) {
                /* IP1 next protocol i.e. number of bytes in this header */
                VTSS_RC(vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 48)); /* IPv6+UDP header length */
                /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
                VTSS_RC(vtss_phy_ts_ip1_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_6, FALSE));
            }
        }
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_ip1_flow_conf_set_priv( eng_parm, double_ip, eng_conf, new_flow_conf)));
    return VTSS_RC_OK;
}
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    if (old_ip_conf->comm_opt.ip_mode != new_ip_conf->comm_opt.ip_mode) {
        if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_4) {
            ...
            VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_ip2_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 28))); /* IPv4+UDP header length */
            /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
            VTSS_RC(vtss_phy_ts_ip2_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_4));
        } else if (new_ip_conf->comm_opt.ip_mode == VTSS_PHY_TS_IP_VER_6) {
            /* IP2 next protocol i.e. number of bytes in this header */
            VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_ip2_next_comp_set_priv(port_no, blk_id, VTSS_PHY_TS_NEXT_COMP_NONE, 48))); /* IPv6+UDP header length */
            /* set IP mode, IP1_PROT_MATCH_1, PROT_OFFSET_2, checksum */
            VTSS_RC(vtss_phy_ts_ip2_different_offset_set(port_no, blk_id, VTSS_PHY_TS_IP_VER_6));
        } else {
                    ...
                    const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_ip2_flow_conf_set_priv( eng_parm, eng_conf, new_flow_conf)));
    return VTSS_RC_OK;
}

static vtss_rc vtss_phy_ts_ptp_def_conf_priv(BOOL                  ingress,
                                             vtss_ts_engine_parm_t *eng_parm);

static vtss_rc vtss_phy_ts_oam_def_conf_priv(BOOL                  ingress,
                                             vtss_ts_engine_parm_t *eng_parm);
    ...
    if (vtss_state->phy_ts_port_conf[port_no].sig_mask &
       (VTSS_PHY_TS_FIFO_SIG_DEST_IP | VTSS_PHY_TS_FIFO_SIG_SRC_IP)) {
        ...
        if (flow_conf->flow_conf.ptp.ip2_opt.comm_opt.ip_mode
            ...
            value |= VTSS_F_ANA_IP2_NXT_PROTOCOL_IP2_FRAME_SIG_CFG_IP2_FRAME_SIG_OFFSET(20) ;
        }
        VTSS_RC(VTSS_PHY_TS_WRITE_CSR (port_no, blk_id,
                         VTSS_ANA_IP2_NXT_PROTOCOL_IP2_FRAME_SIG_CFG, &value));
...
static vtss_rc vtss_phy_ts_engine_init_priv(const BOOL        ingress,
                        vtss_ts_engine_parm_t                 *eng_parm,
                        const vtss_phy_ts_engine_flow_match_t flow_match_mode)
{
    vtss_rc rc = VTSS_RC_OK;
    ...
    /* PTP application not allowed in OAM engine because of no TSFIFO support.
    ...
    if (eng_id == VTSS_PHY_TS_OAM_ENGINE_ID_2A ||
        ...
        if (eng_id == VTSS_PHY_TS_OAM_ENGINE_ID_2B) {
            /* Engine 2B only support OAM-over-ETH */
    ...
    if (eng_id == VTSS_PHY_TS_OAM_ENGINE_ID_2A ||
        ...
        if (alt_eng_conf->eng_used) {
            ...
            /* flow match should be same as other engine */
            if (flow_match_mode != alt_eng_conf->flow_match_mode) {
                VTSS_D("port: %u:: flow match mode is not consistent with engine ID %d",
            ...
            VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_action_flow_init(port_no, blk_id, eng_id)));
        }
    } else {
    ...
    /* flow match should be same in ingress and egress for same engine ID */
    if (ingress) {
    ...
    if (alt_eng_conf->eng_used && flow_match_mode != alt_eng_conf->flow_match_mode) {
        VTSS_D("port: %u:: flow match mode is not consistent in ingress and egress engine ID %d", (u32)port_no, eng_id);
    }
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_flow_match_mode_set_priv(port_no, blk_id, eng_id, flow_match_mode)));
    ...
    case VTSS_PHY_TS_ENCAP_ETH_PTP:
        ...
        rc = VTSS_RC_COLD(vtss_phy_ts_eth1_next_comp_etype_set_priv(port_no, blk_id,
                              eng_id, VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 0x88F7));
        if (rc != VTSS_RC_OK) break;
        /* set eth1 conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_eth1_def_conf_priv(eng_parm,
        ...
        rc = vtss_phy_ts_ptp_def_conf_priv(ingress, eng_parm);
        break;
    case VTSS_PHY_TS_ENCAP_ETH_IP_PTP:
        ...
        rc = VTSS_RC_COLD(vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id,
                             VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 28));
        if (rc != VTSS_RC_OK) break;
        /* set IP1 conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_ip1_def_conf_priv(eng_parm, FALSE,
        ...
        if (ingress == FALSE) {
            rc = VTSS_RC_COLD(vtss_phy_ts_ip1_sig_mask_set_priv(port_no, eng_id, blk_id));
        }
        memset(action_conf, 0, sizeof(vtss_phy_ts_engine_action_t));
        action_conf->action_ptp = TRUE;
        memcpy(&action_conf->action.ptp_conf[0],
    ...
    case VTSS_PHY_TS_ENCAP_ETH_IP_IP_PTP:
        ...
        if (rc != VTSS_RC_OK) break;
        /* set IP1 conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_ip1_def_conf_priv(eng_parm, TRUE,
                             &flow_conf->flow_conf.ptp.ip1_opt,
        ...
        rc = VTSS_RC_COLD(vtss_phy_ts_ip2_next_comp_set_priv(port_no, blk_id,
                                      VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 28));
        if (rc != VTSS_RC_OK) break;
        /* set IP2 conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_ip2_def_conf_priv(eng_parm,
                             &flow_conf->flow_conf.ptp.ip2_opt,
                             &vtss_phy_ts_def_inner_ip_conf));

        /* signature mask config */
        if (rc == VTSS_RC_OK && ingress == FALSE) {
            rc = VTSS_RC_COLD(vtss_phy_ts_ip2_sig_mask_set_priv(port_no, eng_id, blk_id));
        }
        if (rc != VTSS_RC_OK) break;
    ...
    case VTSS_PHY_TS_ENCAP_ETH_ETH_PTP:
        ...
        /* set eth2 next comparator */
        rc = VTSS_RC_COLD(vtss_phy_ts_eth2_next_comp_etype_set_priv(port_no, blk_id,
                              eng_id, VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 0x88F7));
        if (rc != VTSS_RC_OK) break;
        /* set eth2 conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_eth2_def_conf_priv(eng_parm,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_ETH_IP_PTP:
        ...
        /* signature mask config */
        if (ingress == FALSE) {
            rc = vtss_phy_ts_ip1_sig_mask_set_priv(port_no, eng_id, blk_id);
        }
        if (rc != VTSS_RC_OK) break;
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_IP_PTP:
        ...
        /* set MPLS next comparator */
         rc = VTSS_RC_COLD(vtss_phy_ts_mpls_next_comp_set_priv(port_no, blk_id,
                              eng_id, VTSS_PHY_TS_NEXT_COMP_IP1));
        if (rc != VTSS_RC_OK) break;
        /* set MPLS conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_mpls_def_conf_priv(eng_parm,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_ACH_PTP:
        ...
        rc = VTSS_RC_COLD(vtss_phy_ts_ach_next_comp_set_priv(port_no, blk_id, encap_type,
                                      VTSS_PHY_TS_NEXT_COMP_PTP_OAM));
        if (rc != VTSS_RC_OK) break;
        /* set ACH conf */
        rc = VTSS_RC_COLD(vtss_phy_ts_ach_def_conf_priv(eng_parm,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_OAM:
        ...
        rc = vtss_phy_ts_oam_def_conf_priv(ingress, eng_parm);
        break;
...
static vtss_rc vtss_phy_ts_engine_clear_priv(const BOOL  ingress,
                        ...
                        const vtss_phy_ts_engine_flow_match_t flow_match_mode)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) != VTSS_RC_OK) break;
        if ((rc = vtss_phy_ts_base_port_get_priv(port_no, &base_port)) != VTSS_RC_OK) break;
        ...
        if ((rc = vtss_phy_ts_ana_blk_id_get(eng_id, TRUE, &blk_id)) == VTSS_RC_OK) {
            ...
            rc = vtss_phy_ts_engine_init_priv(TRUE, &eng_parm, flow_match_mode);
                         ...
                         const vtss_phy_ts_engine_t         eng_id)
{
    ...
    do {
        ...
        VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
        rc = VTSS_RC_COLD(vtss_phy_ts_engine_clear_priv(TRUE, inst, port_no,
                                           base_port, eng_id));
        VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
        ...
        if (action_conf->action_ptp) {
            for (i = 0; i < 6; i++) {
                if (eng_conf->action_flow_map[i]) {
                    rc = VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_delete_priv(port_no, blk_id, i));
            ...
            for (i = 0; i < 6; i++) {
                if (eng_conf->action_flow_map[i]) {
                    rc = VTSS_RC_COLD(vtss_phy_ts_engine_oam_action_flow_delete_priv(port_no, blk_id, eng_id, i));
                    eng_conf->action_flow_map[i] = 0;
                }
            }
    ...
    do {
        ...
        if ((rc = vtss_phy_ts_ana_blk_id_get(eng_id, FALSE, &blk_id)) == VTSS_RC_OK) {
            vtss_ts_engine_parm_t eng_parm = {port_no, base_port, blk_id,
                                              ...
                                              flow_st_index, flow_end_index};
            VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
            rc = vtss_phy_ts_engine_init_priv(FALSE, &eng_parm, flow_match_mode);
            VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
                         ...
                         const vtss_phy_ts_engine_t        eng_id)
{
    ...
    do {
        ...
        VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
        rc = VTSS_RC_COLD(vtss_phy_ts_engine_clear_priv(FALSE, inst, port_no,
                                           base_port, eng_id));
        VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
        /* clear the engine config */
        memset(flow_conf, 0, sizeof(vtss_phy_ts_engine_flow_conf_t));
...
static vtss_rc vtss_phy_ts_engine_config_priv(BOOL                   ingress,
...
static vtss_rc vtss_phy_ts_engine_flow_set_priv(BOOL       ingress,
                      vtss_ts_engine_parm_t                *eng_parm,
                      ...
                      const vtss_phy_ts_engine_flow_conf_t *const new_flow_conf)
{
    ...
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_config_priv(ingress, port_no, eng_id, FALSE)));
    }

    switch (eng_conf->encap_type) {
    case VTSS_PHY_TS_ENCAP_ETH_PTP:
        if ((rc = vtss_phy_ts_eth1_conf_priv(eng_parm,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_IP_PTP:
        ...
        if ((rc = vtss_phy_ts_ip1_conf_priv(eng_parm, FALSE,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
        break;
    case VTSS_PHY_TS_ENCAP_ETH_IP_IP_PTP:
        ...
        if ((rc = vtss_phy_ts_ip1_conf_priv(eng_parm, TRUE,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
        if ((rc = vtss_phy_ts_ip2_conf_priv(eng_parm,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
    ...
    case VTSS_PHY_TS_ENCAP_ETH_ETH_PTP:
        ...
        if ((rc = vtss_phy_ts_eth2_conf_priv(eng_parm,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
        break;
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_IP_PTP:
        ...
        if ((rc = vtss_phy_ts_mpls_conf_priv(eng_parm,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
        if ((rc = vtss_phy_ts_ip1_conf_priv(eng_parm, FALSE,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_ACH_PTP:
        ...
        if ((rc = vtss_phy_ts_ach_conf_priv(eng_parm,
                           eng_conf, new_flow_conf)) != VTSS_RC_OK) {
            break;
        }
        break;
    ...
    if (new_flow_conf->eng_mode ||
        (alt_eng_conf && alt_eng_conf->flow_conf.eng_mode)) {
    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_config_priv(ingress, port_no, eng_id, TRUE)));
    }
                        ...
                        const vtss_phy_ts_engine_flow_conf_t   *const flow_conf)
{
    ...
    do {
        ...
        if ((rc = vtss_phy_ts_ana_blk_id_get(eng_id, TRUE, &blk_id)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
            rc = vtss_phy_ts_engine_flow_set_priv(TRUE, &eng_parm,
                                                  eng_conf, flow_conf);
            VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
                        ...
                        const vtss_phy_ts_engine_flow_conf_t  *const flow_conf)
{
    ...
    do {
        ...
        if ((rc = vtss_phy_ts_ana_blk_id_get(eng_id, FALSE, &blk_id)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
            rc = vtss_phy_ts_engine_flow_set_priv(FALSE, &eng_parm,
                                                  eng_conf, flow_conf);
            VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
            /* save the flow config */
            if (rc == VTSS_RC_OK) {
                      ...
                      const vtss_phy_ts_ptp_msg_type_t    msg_type)
{
    ...
    case PTP_ACTION_CMD_SUB:
        ...
        value |= VTSS_F_ANA_PTP_FLOW_PTP_ACTION_2_PTP_NEW_CF_LOC(8); /* CF offset */
        VTSS_RC(VTSS_PHY_TS_WRITE_CSR(port_no, blk_id, VTSS_ANA_PTP_FLOW_PTP_ACTION_2(flow_index), &value));
        break;
    ...
    case PTP_ACTION_CMD_SUB_ADD:
        ...
        value = VTSS_F_ANA_PTP_FLOW_PTP_ACTION_2_PTP_REWRITE_OFFSET(8); /* nothing to write */
        value |= VTSS_F_ANA_PTP_FLOW_PTP_ACTION_2_PTP_REWRITE_BYTES(8); /* for CF */
        value |= VTSS_F_ANA_PTP_FLOW_PTP_ACTION_2_PTP_NEW_CF_LOC(8); /* correction field offset */
        VTSS_RC(VTSS_PHY_TS_WRITE_CSR(port_no, blk_id, VTSS_ANA_PTP_FLOW_PTP_ACTION_2(flow_index), &value));
        /* clear frame bytes */
    ...
    case PTP_ACTION_CMD_SAVE_IN_TS_FIFO:
        ...
        value |= VTSS_F_ANA_PTP_FLOW_PTP_ACTION_2_PTP_NEW_CF_LOC(8); /* update on CF */
        VTSS_RC(VTSS_PHY_TS_WRITE_CSR(port_no, blk_id, VTSS_ANA_PTP_FLOW_PTP_ACTION_2(flow_index), &value));
        break;
    default:
        break;
...
/* PTP action is applicable to PTP engine which is handled by action_ptp field */
static vtss_rc vtss_phy_ts_engine_ptp_action_flow_add_priv(BOOL    ingress,
                      const vtss_port_no_t                  port_no,
                      const vtss_phy_ts_blk_id_t            blk_id,
                      const vtss_phy_ts_ptp_msg_type_t      msg_type,
                      const u8                              flow_index,
    ...
    case PTP_MSG_TYPE_SYNC:
        if (ingress) {
            if (action_conf->clk_mode == VTSS_PHY_TS_PTP_CLOCK_MODE_BC1STEP &&
                ...
                VTSS_RC(vtss_phy_ts_engine_ptp_action_flow_conf_priv(port_no,
                                 blk_id, flow_index,
...
static vtss_rc vtss_phy_ts_engine_y1731_oam_action_flow_add_priv(BOOL    ingress,
                      const vtss_port_no_t                    port_no,
                      const vtss_phy_ts_blk_id_t              blk_id,
                      const vtss_phy_ts_engine_t              eng_id,
...
static vtss_rc vtss_phy_ts_engine_ietf_oam_action_flow_add_priv(BOOL    ingress,
                      const vtss_port_no_t                    port_no,
                      const vtss_phy_ts_blk_id_t              blk_id,
...
static vtss_rc vtss_phy_ts_engine_ptp_action_add_priv(BOOL  ingress,
                      const vtss_port_no_t                  port_no,
                      const vtss_port_no_t                  base_port_no,
                      const vtss_phy_ts_blk_id_t            blk_id,
                      vtss_phy_ts_eng_conf_t                *eng_conf,
                      ...
                      const u8                              action_index)
{
	...
	u32 i, empty_cnt = 0;
    u8 num_flow = ptp_action_max_flow[action_conf->clk_mode][action_conf->delaym_type];

    /* PTP action is only on PTP engine i.e. 0 and 1; 
       we need to add new flow for the action */
    ...
    if (action_conf->clk_mode == VTSS_PHY_TS_PTP_CLOCK_MODE_BC1STEP &&
        action_conf->delaym_type == VTSS_PHY_TS_PTP_DELAYM_E2E) {
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_SYNC, flow_index[0], action_conf)));
        eng_conf->action_flow_map[flow_index[0]] = action_index + 1;
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_DELAY_REQ, flow_index[1], action_conf)));
        ...
        eng_conf->action_flow_map[flow_index[0]] = action_index + 1;
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_PDELAY_REQ, flow_index[1], action_conf)));
        eng_conf->action_flow_map[flow_index[1]] = action_index + 1;
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_PDELAY_RESP, flow_index[2], action_conf)));
        ...
        eng_conf->action_flow_map[flow_index[1]] = action_index + 1;
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_PDELAY_REQ, flow_index[2], action_conf)));
        eng_conf->action_flow_map[flow_index[2]] = action_index + 1;
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_add_priv(ingress,
                port_no, blk_id, PTP_MSG_TYPE_PDELAY_RESP, flow_index[3], action_conf)));
...
static vtss_rc vtss_phy_ts_add_grp_mask_cold_priv(const vtss_port_no_t                  port_no,
                                                  const vtss_phy_ts_blk_id_t            blk_id,
                                                  const vtss_phy_ts_engine_t            eng_id,
                                                  const u8                              flow_id)
{
...
static vtss_rc vtss_phy_ts_engine_y1731_oam_action_add_priv(BOOL  ingress,
                      const vtss_port_no_t                  port_no,
                      const vtss_port_no_t                  base_port_no,
                      const vtss_phy_ts_blk_id_t            blk_id,
                      const vtss_phy_ts_engine_t            eng_id,
                      ...
                      const u8                              action_index)
{
    ...
    u8 num_flow = y1731_oam_action_max_flow[action_conf->oam_conf.y1731_oam_conf.delaym_type];
        ...
        if (found) {
            for (i = 0; i < 6; i++) {
                if (action_flow_map[i] == other_action_index) {
                    ...
                    VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_add_grp_mask_cold_priv(port_no, blk_id, eng_id, i)));
                }
            }
            return VTSS_RC_OK;
        }
    ...
    if (action_conf->oam_conf.y1731_oam_conf.delaym_type == VTSS_PHY_TS_Y1731_OAM_DELAYM_1DM) {    
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_y1731_oam_action_flow_add_priv(ingress,
...
static vtss_rc vtss_phy_ts_engine_ietf_oam_action_add_priv(BOOL  ingress,
                      const vtss_port_no_t                  port_no,
                      const vtss_port_no_t                  base_port_no,
                      const vtss_phy_ts_blk_id_t            blk_id,
                      vtss_phy_ts_eng_conf_t                *eng_conf,
                      ...
                      const u8                              action_index)
{
    ...
    if (action_conf->oam_conf.ietf_oam_conf.delaym_type == VTSS_PHY_TS_IETF_MPLS_ACH_OAM_DELAYM_DMM) {
        VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ietf_oam_action_flow_add_priv(ingress,
                                             ...
                                             vtss_ts_engine_parm_t *eng_parm)
{
    ...
    VTSS_RC(vtss_phy_ts_engine_ptp_action_add_priv(ingress, port_no, base_port_no,
                                             ...
                                             vtss_ts_engine_parm_t *eng_parm)
{
    ...
    if (def_action.y1731_en) {
        VTSS_RC(vtss_phy_ts_engine_y1731_oam_action_add_priv(ingress, port_no, base_port_no,
                        blk_id, eng_id, eng_conf, &def_action, 0));
    } else {
        VTSS_RC(vtss_phy_ts_engine_ietf_oam_action_add_priv(ingress, port_no, base_port_no,
                        blk_id, eng_conf, &def_action, 0));
    }
...
static vtss_rc vtss_phy_ts_engine_action_set_priv(BOOL  ingress,
                      vtss_port_no_t                    port_no,
                      vtss_port_no_t                    base_port_no,
                      vtss_phy_ts_engine_t              eng_id,
                      vtss_phy_ts_eng_conf_t            *eng_conf,
        ...
        for (i = 0; i < 2; i++) {
            if (memcmp(&old_action_conf->action.ptp_conf[i],
                ...
                if (old_action_conf->action.ptp_conf[i].enable) {
                    ...
                    for (j = 0; j < 6; j++) {
                        if (eng_conf->action_flow_map[j] == (i + 1)) {
                            VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_ptp_action_flow_delete_priv(port_no, blk_id, j)));
                ...
                if (new_action_conf->action.ptp_conf[i].enable) {
                    VTSS_RC(vtss_phy_ts_engine_ptp_action_add_priv(ingress,
        ...
        for (i = 0; i < 6; i++) {
            if (memcmp(&old_action_conf->action.oam_conf[i],
                ...
                if (old_action_conf->action.oam_conf[i].enable) {
                    ...
                    for (j = 0; j < 6; j++) {
                        if (eng_conf->action_flow_map[j] == (i + 1)) {
                            VTSS_RC(VTSS_RC_COLD(vtss_phy_ts_engine_oam_action_flow_delete_priv(port_no, blk_id, eng_id, j)));
                            eng_conf->action_flow_map[j] = 0;
                        }
                    }
                }
                ...
                if (new_action_conf->action.oam_conf[i].enable) {
                    if (new_action_conf->action.oam_conf[i].y1731_en) {
                        VTSS_RC(vtss_phy_ts_engine_y1731_oam_action_add_priv(ingress,
                                     port_no, base_port_no, blk_id, eng_id, eng_conf,
                                     &new_action_conf->action.oam_conf[i], i));
                    } else {
                        VTSS_RC(vtss_phy_ts_engine_ietf_oam_action_add_priv(ingress,
                                     port_no, base_port_no, blk_id, eng_conf,
                                     &new_action_conf->action.oam_conf[i], i));
                    }
                }
                          ...
                          const vtss_phy_ts_engine_action_t *const action_conf)
{
    ...
    do {
        ...
        VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
        rc = vtss_phy_ts_engine_action_set_priv(TRUE, port_no, base_port_no,
                          ...
                          const vtss_phy_ts_engine_action_t *const action_conf)
{
    ...
    do {
        ...
        VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
        rc = vtss_phy_ts_engine_action_set_priv(FALSE, port_no, base_port_no,
                                             eng_id, eng_conf, action_conf);
        VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
        /* save the action config */
        if (rc == VTSS_RC_OK) {
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            if (enable)
                ...
                vtss_state->phy_ts_port_conf[port_no].event_mask &= ~ev_mask;
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_EVT_MASK_SET);
            VTSS_PHY_TS_SPI_UNPAUSE(port_no);
        }
    } while(0);
                               ...
                               vtss_phy_ts_event_t   *const status)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            rc = vtss_phy_ts_csr_event_poll_priv(port_no, status);
            VTSS_PHY_TS_SPI_UNPAUSE(port_no);
        }
    } while(0);
                              ...
                              vtss_phy_ts_stats_t   *const statistics)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            rc = vtss_phy_ts_stats_get_priv(port_no, statistics);
            VTSS_PHY_TS_SPI_UNPAUSE(port_no);
        }
    } while(0);
                        ...
                        BOOL                  *const egr_overflow)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE(port_no);
            rc = vtss_phy_ts_correction_overflow_get_priv(port_no,
                             ...
                             const BOOL            enable)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            VTSS_PHY_TS_SPI_PAUSE_COLD(port_no);
            rc = VTSS_RC_COLD(vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_ENA_SET));
            VTSS_PHY_TS_SPI_UNPAUSE_COLD(port_no);
        }
    } while(0);
...
#ifdef VTSS_CHIP_CU_PHY
static vtss_rc vtss_phy_ts_phy_latency_set_priv(vtss_port_no_t port_no)
{
    u16 dly_egr;
    u32 dly_egr32;
    u16 reg18;
    ...
    VTSS_RC(vtss_phy_wr( port_no, 31, 0 ));
    ...
    VTSS_RC(vtss_phy_wr( port_no, 18, 0x0E ));   //      bit 23-12          bit 11-0
    VTSS_RC(vtss_phy_wr( port_no, 17, 0xF164 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9f90 )); //Write speed1000_lat_igr, speed100_lat_igr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x06 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0xaB7A ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9f92 )); //Write speed10_lat_egr, speed10_lat_igr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x01 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x805B ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9f94 )); //Write speed1000_lat_egr, speed100_lat_egr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x11 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x4057 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9f96 )); //Write fi_100fx_lat_igr, fi_1000_lat_igr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x01 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x801E ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9f98 )); //Write fi_100fx_lat_egr, fi_1000_lat_egr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x32 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x0320 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fa8 )); //Write i1588_byp_speed10_lat_igr, i1588_byp_speed10_lat_egr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x08 ));   //      bit 23-16                    bit 15-8                     bit 7-0    
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x0850 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fac )); //Write i1588_byp_speed1000_lat_igr, i1588_byp_speed1000_lat_egr, i1588_byp_speed100_lat_igr
    ...
    VTSS_RC(vtss_phy_wr( port_no, 18, reg18 ));   //      bit 23-12                     bit 11-0
    VTSS_RC(vtss_phy_wr( port_no, 17, reg17 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fae )); //Write i1588_speed1000_proc_dly_egr, i1588_speed1000_proc_dly_igr
    ...
    VTSS_RC(vtss_phy_wr( port_no, 18, reg18 ));   //      bit 21-14                   bit 13-0
    VTSS_RC(vtss_phy_wr( port_no, 17, reg17 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fb0 )); //Write i1588_byp_speed100_lat_egr, i1588_speed100_proc_dly_egr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x00 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x0d68 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fb2 )); //Write i1588_speed100_proc_dly_igr
    ...
    VTSS_RC(vtss_phy_wr( port_no, 17, reg17));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fb4 )); //Write i1588_speed10_proc_dly_egr

    VTSS_RC(vtss_phy_wr( port_no, 18, 0x00 ));
    VTSS_RC(vtss_phy_wr( port_no, 17, 0x8490 ));
    VTSS_RC(vtss_phy_wr( port_no, 16, 0x9fb6 )); //Write i1588_speed10_proc_dly_igr
                         ...
                         const vtss_phy_ts_init_conf_t  *const conf)
{
    ...
    do {
        ...
        if ((rc = vtss_phy_ts_base_port_get_priv(port_no, &base_port_no)) != VTSS_RC_OK) break;
#if defined(VTSS_CHIP_CU_PHY) && defined(VTSS_PHY_TS_SPI_CLK_THRU_PPS0)
        if ((rc = vtss_phy_ts_channel_id_get_priv(port_no, &channel_id)) != VTSS_RC_OK) break;
#endif
        if ((rc = vtss_phy_ts_rev_get_priv(port_no, &revision)) != VTSS_RC_OK) break;
        if ((rc = vtss_phy_ts_register_access_type_get(port_no, &phy_type, &clause45, &oper_mode)) != VTSS_RC_OK) {
        ...
        if ((phy_type == VTSS_DEV_TYPE_8492) || (phy_type == VTSS_DEV_TYPE_8494)) {
            phy_type_match = TRUE;
            if ((rc = daytona_port_2_mode(port_no, &channel_mode)) != VTSS_RC_OK) break;
            switch (channel_mode) {
        ...
        if ((phy_type == VTSS_PHY_TYPE_8574) && (conf->clk_freq == VTSS_PHY_TS_CLOCK_FREQ_500M)) {
            rc = VTSS_RC_ERROR;
        ...
        if ((rc = VTSS_RC_COLD(vtss_phy_ts_block_init(port_no))) != VTSS_RC_OK) {
            VTSS_E("block init failed, port_no %u, base_port %u", port_no, base_port_no);
            break;
        }

        if ((rc = VTSS_RC_COLD(vtss_phy_ts_port_init(port_no, conf))) != VTSS_RC_OK) {
        ...
        if (phy_type == VTSS_PHY_TYPE_8574) {
            ...
            if (revision == VTSS_PHY_TESLA_REV_A) {
                if (vtss_state->phy_ts_port_conf[base_port_no].eng_init_done == FALSE) {
                    ...
                    if ((rc = VTSS_RC_COLD(vtss_phy_ts_analyzer_init_priv(base_port_no))) != VTSS_RC_OK) {
            ...
            if ((rc = vtss_phy_ts_phy_latency_set_priv(port_no)) != VTSS_RC_OK) {
...
static vtss_rc vtss_phy_ts_spi_pause_priv(const vtss_port_no_t port_no)
{
    ...
    do {
        ...
        if ((rc = PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, VTSS_PHY_TS_FIFO_CONF_STATUS)) != VTSS_RC_OK) break;
        /* Poll on 18G.15 to clear */
        if ((rc = vtss_phy_wait_for_micro_complete(port_no)) != VTSS_RC_OK) break;
        /* Above function moves back to std page, change to GPIO page */
        if ((rc = vtss_phy_page_gpio(port_no)) != VTSS_RC_OK) break;
        /* Now it can be read */
        if ((rc = PHY_RD_PAGE(port_no, VTSS_PHY_MICRO_PAGE, &val_18g)) != VTSS_RC_OK) break;
        spi_prev_status = val_18g;
        ...
        if ((blk_id == VTSS_PHY_TS_BLOCK_A) && (val_18g & 0x500)) {
            ...
            if ((rc = PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, (val_18g & 0xA00) | VTSS_PHY_TS_CONFIG_BASE)) != VTSS_RC_OK) break;
            /* Poll on 18G.15 to clear */
            if ((rc = vtss_phy_wait_for_micro_complete(port_no)) != VTSS_RC_OK) break;
        } else if ((blk_id == VTSS_PHY_TS_BLOCK_B) && (val_18g & 0xA00)) {
            is_spi_paused = TRUE;
            /* pause both ports in block B */
            if ((rc = PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, (val_18g & 0x500) | VTSS_PHY_TS_CONFIG_BASE)) != VTSS_RC_OK) break;
            /* Poll on 18G.15 to clear */
            if ((rc = vtss_phy_wait_for_micro_complete(port_no)) != VTSS_RC_OK) break;
        }
    } while (0);
...
static vtss_rc vtss_phy_ts_spi_unpause_priv(const vtss_port_no_t port_no)
{
    ...
    do {
        ...
        if ((rc = (PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, (spi_prev_status & 0xF00) | VTSS_PHY_TS_CONFIG_BASE))) != VTSS_RC_OK) break;
        /* Poll on 18G.15 to clear */
        if ((rc = (vtss_phy_wait_for_micro_complete(port_no))) != VTSS_RC_OK) break;
    } while (0);
...
static vtss_rc vtss_phy_ts_new_spi_mode_set_priv(const vtss_port_no_t    port_no,
                                                 const BOOL              enable)
{
    vtss_rc  rc = VTSS_RC_OK;
    u16      val_18g;
    ...
    if ((rc = vtss_phy_ts_spi_pause_priv(port_no)) != VTSS_RC_OK) {
    ...
    if ((rc = vtss_phy_ts_spi_unpause_priv(port_no)) != VTSS_RC_OK) {
    ...
    do {
        ...
        if ((rc = (PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, VTSS_PHY_TS_CONFIG_BASE | val_18g))) != VTSS_RC_OK) break;
        /* Poll on 18G.15 to clear */
        if ((rc = (vtss_phy_wait_for_micro_complete(port_no))) != VTSS_RC_OK) break;
    } while (0);
...
static vtss_rc vtss_phy_ts_new_spi_mode_sync_priv(const vtss_port_no_t    port_no,
                                                 const BOOL              enable)
{
    vtss_rc  rc = VTSS_RC_OK;
    u16      val_18g;
                                     ...
                                     const BOOL                 enable)
{
    ...
    do {
        ...
        if ((rc = VTSS_RC_COLD(vtss_phy_ts_new_spi_mode_set_priv(port_no, enable))) != VTSS_RC_OK) {
                                         ...
                                         const vtss_port_no_t       port_no)
{
    ...
    do {
        if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
            ...
            if (vtss_phy_ts_register_access_type_get(port_no, &phy_type, &clause45, &oper_mode) != VTSS_RC_OK) {
            ...
            if (VTSS_RC_COLD(vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_ING_LATENCY_SET)) != VTSS_RC_OK) {
                VTSS_E("Ingress Latency set fail, port %u", port_no);
                rc = VTSS_RC_ERROR;
                break;
            }
            if (VTSS_RC_COLD(vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_EGR_LATENCY_SET)) != VTSS_RC_OK) {
                VTSS_E("Egress Latency set fail, port %u", port_no);
                rc = VTSS_RC_ERROR;
                break;
            }
            if (VTSS_RC_COLD(vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_OPER_MODE_CHANGE_SET)) != VTSS_RC_OK) {
...
static vtss_rc vtss_phy_ts_fec_mode_set_priv(const vtss_port_no_t port_no,
                                 ...
                                 const vtss_otn_och_fec_t *const cfg)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && cfg) {
        ...
        if ((u16)cfg->type < (u16)VTSS_OTN_OCH_FEC_MAX) {
            rc = VTSS_RC_COLD(vtss_phy_ts_fec_mode_set_priv(port_no, cfg));
        }
    }
    VTSS_EXIT();
    return rc;
...
static vtss_rc vtss_phy_ts_block_sync(const vtss_port_no_t port_no, BOOL *block_sync)
{
    u16                        mmd_addr = (u16)0x1E;
    u16                        reg_value_1 = 0;
    u16                        reg_value_2 = 0;
    ...
    VTSS_RC(vtss_phy_ts_register_access_type_get(port_no, &phy_type, &clause45, &oper_mode));
    if (clause45) {
        VTSS_RC(mmd_read_func(port_no, mmd_addr, 0x001A, &reg_value_1));
        VTSS_RC(mmd_read_func(port_no, mmd_addr, 0x0102, &reg_value_2));
        *block_sync = (((reg_value_1 == (u16)0xFF01) && (reg_value_2 == (u16)0x8100)) ? TRUE : FALSE);
        if (vtss_state->phy_ts_port_conf[port_no].port_ts_init_done != *block_sync) {
            ...
            VTSS_RC(mmd_write_func(port_no, mmd_addr, 0x001A, reg_value_1));
            reg_value_2 = (vtss_state->phy_ts_port_conf[port_no].port_ts_init_done) ? 0x8100 : 0;
            VTSS_RC(mmd_write_func(port_no, mmd_addr, 0x0102, reg_value_2));
        }
    } else {
        /* initial setup of extended register 29 and 30 in 1588 extended page */
        /* 1588 - Page Selection */
        VTSS_RC(vtss_state->init_conf.miim_write(port_no, VTSS_PHY_TS_1G_ADDR_EXT_REG, 0x1588));
        /* Read the lower word data from register 29
         */
        VTSS_RC(vtss_state->init_conf.miim_read(port_no, 29, &reg_value_1));
        /* Read the upper word data from register 30
         */
        VTSS_RC(vtss_state->init_conf.miim_read(port_no, 30, &reg_value_2));
        *block_sync = (((reg_value_1 == 0x7AE0) && (reg_value_2 == 0xB71C)) ? TRUE : FALSE);
        if (vtss_state->phy_ts_port_conf[port_no].port_ts_init_done != *block_sync) {
            ...
            VTSS_RC(vtss_state->init_conf.miim_write(port_no, 29, reg_value_1));
            reg_value_2 = (vtss_state->phy_ts_port_conf[port_no].port_ts_init_done) ? 0xB71C : 0;
            VTSS_RC(vtss_state->init_conf.miim_write(port_no, 30, reg_value_2));
...
static vtss_rc vtss_phy_ts_mode_sync(const vtss_port_no_t port_no,
                                     const vtss_port_no_t base_port_no,
                                     BOOL *phy_engine_sync_reqd)
{
    ...
    *phy_engine_sync_reqd = FALSE;
    ...
    port_actual_state = ~base_port_value & VTSS_F_PTP_IP_1588_TOP_CFG_STAT_INTERFACE_CTL_BYPASS;
    ...
    if (vtss_state->phy_ts_port_conf[port_no].port_ena) {
        ...
        *phy_engine_sync_reqd = FALSE;
        base_port_value |= VTSS_F_PTP_IP_1588_TOP_CFG_STAT_INTERFACE_CTL_BYPASS;
        VTSS_RC(VTSS_PHY_TS_WRITE_CSR(port_no, VTSS_PHY_TS_PROC_BLK_ID(0),
                                               VTSS_PTP_IP_1588_TOP_CFG_STAT_INTERFACE_CTL, &base_port_value));
    }
...
static vtss_rc vtss_phy_ts_engine_sync_priv(BOOL  ingress,
                                             const vtss_port_no_t port_no,
                                             const vtss_port_no_t base_port_no,
                                             const vtss_phy_ts_engine_t eng_id,
                                             const vtss_phy_ts_blk_id_t blk_id,
...
{
    u32 value,hw_eng_conf;
    vtss_phy_ts_engine_t engine_id;

    VTSS_RC(VTSS_PHY_TS_READ_CSR(port_no, VTSS_PHY_TS_PROC_BLK_ID_0,
                     VTSS_PTP_IP_1588_TOP_CFG_STAT_ANALYZER_MODE, &value));
    if (ingress) {
        hw_eng_conf = (VTSS_X_PTP_IP_1588_TOP_CFG_STAT_ANALYZER_MODE_INGR_ENCAP_ENGINE_ENA(value));
    } else {
        hw_eng_conf = (VTSS_X_PTP_IP_1588_TOP_CFG_STAT_ANALYZER_MODE_EGR_ENCAP_ENGINE_ENA(value));
    }

    engine_id = (eng_id < VTSS_PHY_TS_OAM_ENGINE_ID_2A) ?  eng_id :
                                                        VTSS_PHY_TS_OAM_ENGINE_ID_2A;

    *eng_ena = ((hw_eng_conf & (0x01 << engine_id))  >> engine_id) ? TRUE : FALSE;
    ...
    hw_eng_conf = VTSS_X_PTP_IP_1588_TOP_CFG_STAT_ANALYZER_MODE_ENCAP_FLOW_MODE(value);

    *eng_match_mode =  ((hw_eng_conf & (0x01 << engine_id))  >> engine_id) ?
...
static vtss_rc vtss_phy_ts_engine_flow_conf_sync_priv(BOOL                           ingress,
                                                      vtss_ts_engine_parm_t          *eng_parm,
                                                      vtss_phy_ts_eng_conf_t         *hw_eng_conf,
                                                      vtss_phy_ts_engine_flow_conf_t *sw_flow_conf,
                                                      BOOL *hw_update_reqd)
{
    ...
    case VTSS_PHY_TS_ENCAP_ETH_PTP:
        rc = vtss_phy_ts_eth1_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd);
        break;
    case VTSS_PHY_TS_ENCAP_ETH_IP_PTP:
        if ((rc = vtss_phy_ts_eth1_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        if ((rc = vtss_phy_ts_ip1_conf_sync_priv(eng_parm, FALSE,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        break;
    case VTSS_PHY_TS_ENCAP_ETH_IP_IP_PTP:
        ...
        if ((rc = vtss_phy_ts_ip1_conf_sync_priv(eng_parm, TRUE,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        if ((rc = vtss_phy_ts_ip2_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        } 
        break;
    case VTSS_PHY_TS_ENCAP_ETH_ETH_PTP:
        ...
        if ((rc = vtss_phy_ts_eth2_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        break;
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_IP_PTP:
        ...
        if ((rc = vtss_phy_ts_mpls_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        if ((rc = vtss_phy_ts_ip1_conf_sync_priv(eng_parm, FALSE,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_ACH_PTP:
        ...
        if ((rc = vtss_phy_ts_ach_conf_sync_priv(eng_parm,
                           hw_eng_conf, sw_flow_conf, hw_update_reqd)) != VTSS_RC_OK) {
            break;
        }
        break;
...
static vtss_rc vtss_phy_ts_engine_encap_conf_sync_priv(BOOL ingress,
                                                       vtss_port_no_t port_no,
                                                       vtss_port_no_t base_port_no,
                                                       vtss_phy_ts_engine_t eng_id,
                                                       vtss_phy_ts_blk_id_t blk_id,
...
static vtss_rc vtss_phy_ts_engine_next_comp_sync_priv(const BOOL                      ingress,
                                                      const vtss_port_no_t            port_no,
                                                      const vtss_phy_ts_blk_id_t      blk_id,
                                                      const vtss_phy_ts_engine_t      eng_id,
                                                      const vtss_phy_ts_encap_t       encap_type)
{
    ...
    case VTSS_PHY_TS_ENCAP_ETH_PTP:
        /* PTP allows only in PTP engine */
        /* next comp: PTP */        
        rc = vtss_phy_ts_eth1_next_comp_etype_set_priv(port_no, blk_id,
                                     eng_id, VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 0x88F7);
        break;
    case VTSS_PHY_TS_ENCAP_ETH_IP_PTP:
        /* No OAM engine for this encap type */
        ...
        /* set IP next comparator: PTP */
        rc = vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id,
                             VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 28);
        if (rc != VTSS_RC_OK) break;
    ...
    case VTSS_PHY_TS_ENCAP_ETH_IP_IP_PTP:
        ...
        if (rc != VTSS_RC_OK) break;
        /* set IP2 next comparator */
        rc = vtss_phy_ts_ip2_next_comp_set_priv(port_no, blk_id,
                                      VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 28);
        /* signature mask config */
        if (rc == VTSS_RC_OK && ingress == FALSE) {
            rc = vtss_phy_ts_ip2_sig_mask_set_priv(port_no, eng_id, blk_id);
        }
        break;
    case VTSS_PHY_TS_ENCAP_ETH_ETH_PTP:
        /* No OAM engine for this encap type */
        ...
        /* set eth2 next comparator */
        rc = vtss_phy_ts_eth2_next_comp_etype_set_priv(port_no, blk_id,
                              eng_id, VTSS_PHY_TS_NEXT_COMP_PTP_OAM, 0x88F7);
        if (rc != VTSS_RC_OK) break;
        break;
    case VTSS_PHY_TS_ENCAP_ETH_ETH_IP_PTP:
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_IP_PTP:
        ...
        /* set MPLS next comparator */
        rc = vtss_phy_ts_mpls_next_comp_set_priv(port_no, blk_id,
                              eng_id, VTSS_PHY_TS_NEXT_COMP_IP1);
        if (rc != VTSS_RC_OK) break;
        /* set IP1 next comparator */
        rc = vtss_phy_ts_ip1_next_comp_set_priv(port_no, blk_id,
    ...
    case VTSS_PHY_TS_ENCAP_ETH_MPLS_ACH_PTP:
        ...
        if (rc != VTSS_RC_OK) break;
        /* set ACH next comparator */
        rc = vtss_phy_ts_ach_next_comp_set_priv(port_no, blk_id, encap_type,
                                      VTSS_PHY_TS_NEXT_COMP_PTP_OAM);
        break;
    case VTSS_PHY_TS_ENCAP_ETH_OAM:
        /* set eth1 next comparator */
...
static ptp_action_flow_conf_t ptp_action_flow_ingr_conf_TC_A[PTP_MSG_TYPE_PDELAY_RESP + 1]
    ...
    {{{PTP_ACTION_CMD_WRITE_NS_P2P, PTP_ACTION_ASYM_ADD},   /* write(RX_timestamp,  Reserved), add(PathDelay+Asymmetry, correctionField) BC1, P2P*/
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_ADD}},  /* write(RX_timestamp,  Reserved), add(Asymmetry, correctionField) BC1, E2E */
     {{PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_ADD},   /* write(RX_timestamp,  Reserved), add(Asymmetry, correctionField) BC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_ADD}},  /* write(RX_timestamp,  Reserved), add(Asymmetry, correctionField) BC2, E2E */
     {{PTP_ACTION_CMD_SUB,          PTP_ACTION_ASYM_ADD},   /* write(RX_timestamp,  Reserved), add(PathDelay+Asymmetry, correctionField) TC1, P2P*/
      {PTP_ACTION_CMD_SUB,          PTP_ACTION_ASYM_ADD}},  /* write(RX_timestamp,  Reserved), add(Asymmetry, correctionField) TC1, E2E*/
     {{PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE},  /* write(RX_timestamp,  Reserved) TC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}}},/* write(RX_timestamp,  Reserved) TC2, E2E */

    /* Delay_Req */
    {{{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP BC1, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}},  /* write(RX_timestamp,  Reserved) BC1, E2E */
     {{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP BC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}},  /* write(RX_timestamp,  Reserved) BC2, E2E */
     {{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP TC1, P2P */
      {PTP_ACTION_CMD_SUB,          PTP_ACTION_ASYM_NONE}},  /* write(RX_timestamp,  Reserved) TC1, E2E */
    ...
    {{{PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE},   /* write(RX_timestamp,  Reserved) BC1, P2P */
      {PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE}},  /* NOP BC1, E2E */
     {{PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE},   /* write(RX_timestamp,  Reserved) BC2, P2P */
      {PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE}},  /* NOP BC2, E2E */
...
static ptp_action_flow_conf_t ptp_action_flow_ingr_conf_TC_B[PTP_MSG_TYPE_PDELAY_RESP + 1]
    ...
    {{{PTP_ACTION_CMD_WRITE_NS_P2P, PTP_ACTION_ASYM_ADD},   /* write(RX_timestamp,  Reserved), add(PathDelay+Asymmetry, correctionField) BC1, P2P*/
     ...
     {{PTP_ACTION_CMD_WRITE_NS_P2P, PTP_ACTION_ASYM_ADD},   /* write(RX_timestamp,  Reserved), add(PathDelay+Asymmetry, correctionField) TC1, P2P*/
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_ADD}},  /* write(RX_timestamp,  Reserved), add(Asymmetry, correctionField) TC1, E2E*/
     {{PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE},  /* write(RX_timestamp,  Reserved) TC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}}},/* write(RX_timestamp,  Reserved) TC2, E2E */
    ...
    {{{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP BC1, P2P */
     ...
     {{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP BC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}},  /* write(RX_timestamp,  Reserved) BC2, E2E */
     {{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP TC1, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}},  /* write(RX_timestamp,  Reserved) TC1, E2E */
     {{PTP_ACTION_CMD_NOP,          PTP_ACTION_ASYM_NONE},   /* NOP TC2, P2P */
      {PTP_ACTION_CMD_WRITE_NS,     PTP_ACTION_ASYM_NONE}}}, /* write(RX_timestamp,  Reserved) TC2, E2E */
...
static ptp_action_flow_conf_t ptp_action_flow_egr_conf_TC_A[PTP_MSG_TYPE_PDELAY_RESP + 1]
    ...
    {{{PTP_ACTION_CMD_WRITE_1588,      PTP_ACTION_ASYM_NONE},   /* write(TX_timestamp, originTimestamp) BC1, P2P */
      {PTP_ACTION_CMD_WRITE_1588,      PTP_ACTION_ASYM_NONE}},  /* write(TX_timestamp, originTimestamp) BC1, E2E */
     {{PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE},   /* save(TX_timestamp, TXFifo) BC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}},  /* save(TX_timestamp, TXFifo) BC2, E2E */
     {{PTP_ACTION_CMD_ADD,             PTP_ACTION_ASYM_NONE},   /* Subtract_add(TX_timestamp, Reserved, correctionField) TC1, P2P */
      {PTP_ACTION_CMD_ADD,             PTP_ACTION_ASYM_NONE}},  /* Subtract_add(TX_timestamp, Reserved, correctionField) TC1, E2E */
     {{PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE},   /* save(TX_timestamp, TXFiFo) TC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}}}, /* save(TX_timestamp, TXFiFo) TC2, E2E */

    /* Delay_Req */
    {{{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP BC1, P2P */
#if defined(VTSS_FEATURE_PHY_TS_ONE_STEP_TXFIFO_OPTION)
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_SUB}},   /* save(TX_timestamp, TXFiFo);sub(Asymmetry, correctionField) BC1, E2E */
#else 
      {PTP_ACTION_CMD_SUB_ADD,         PTP_ACTION_ASYM_SUB}},   /* Subtract_add(TX_timestamp - Asymmetry, Reserved, correctionField) BC1, E2E */
#endif /* VTSS_FEATURE_PHY_TS_ONE_STEP_TXFIFO_OPTION */
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP BC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}},  /* save(TX_timestamp, TXFiFo) BC2, E2E*/
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP TC1, P2P */
      {PTP_ACTION_CMD_ADD,             PTP_ACTION_ASYM_SUB}},   /* Subtract_add(TX_timestamp - Asymmetry, Reserved, correctionField) TC1, E2E */
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP TC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}}}, /* save(TX_timestamp, TXFiFo) TC2, E2E */
...
static ptp_action_flow_conf_t ptp_action_flow_egr_conf_TC_B[PTP_MSG_TYPE_PDELAY_RESP + 1]
    ...
    {{{PTP_ACTION_CMD_WRITE_1588,      PTP_ACTION_ASYM_NONE},   /* write(TX_timestamp, originTimestamp) BC1, P2P */
     ...
     {{PTP_ACTION_CMD_SUB_ADD,         PTP_ACTION_ASYM_NONE},   /* Subtract_add(TX_timestamp, Reserved, correctionField) TC1, P2P */
      {PTP_ACTION_CMD_SUB_ADD,         PTP_ACTION_ASYM_NONE}},  /* Subtract_add(TX_timestamp, Reserved, correctionField) TC1, E2E */
     {{PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE},   /* save(TX_timestamp, TXFiFo) TC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}}}, /* save(TX_timestamp, TXFiFo) TC2, E2E */
    ...
    {{{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP BC1, P2P */
     ...
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP BC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}},  /* save(TX_timestamp, TXFiFo) BC2, E2E*/
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP TC1, P2P */
      {PTP_ACTION_CMD_SUB_ADD,         PTP_ACTION_ASYM_SUB}},   /* Subtract_add(TX_timestamp - Asymmetry, Reserved, correctionField) TC1, E2E */
     {{PTP_ACTION_CMD_NOP,             PTP_ACTION_ASYM_NONE},   /* NOP TC2, P2P */
      {PTP_ACTION_CMD_SAVE_IN_TS_FIFO, PTP_ACTION_ASYM_NONE}}}, /* save(TX_timestamp, TXFiFo) TC2, E2E */
...
static vtss_rc vtss_phy_ts_engine_ptp_action_flow_sync_priv(BOOL    ingress,
                      const vtss_port_no_t                  port_no,
                      const vtss_phy_ts_blk_id_t            blk_id,
                      const vtss_phy_ts_ptp_msg_type_t      msg_type,
                      const u8                              flow_index,
                      ...
                      BOOL *hw_update_reqd)
{
    ...
    if (vtss_state->phy_ts_port_conf[port_no].tc_op_mode == VTSS_PHY_TS_TC_OP_MODE_A) {
        if (ingress) {
            VTSS_RC(vtss_phy_ts_engine_ptp_action_flow_conf_sync_priv(port_no,
                                     blk_id, flow_index,
                                 ...
                                 BOOL *hw_update_reqd)
{
    ...
    if (action_conf->clk_mode == VTSS_PHY_TS_PTP_CLOCK_MODE_BC1STEP &&
        action_conf->delaym_type == VTSS_PHY_TS_PTP_DELAYM_E2E) {
        VTSS_RC(vtss_phy_ts_engine_ptp_action_flow_sync_priv(ingress,
    ...
    *hw_update_reqd = (hw_action_conf->action_ptp != sw_action_conf->action_ptp) ? TRUE : FALSE;
    ...
    for(flow_index = 0; flow_index < 6; flow_index++) {
        ...
        *hw_update_reqd = ((hw_conf->enable      != sw_conf->enable) ||
        ...
        if (*hw_update_reqd)  break;
    }

    if (*hw_update_reqd)  return VTSS_RC_ERROR;
    return VTSS_RC_OK;
}
                                           ...
                                           BOOL *hw_update_reqd)
{
    ...
    if(actbitmask[0] == state_actbitmask[0]) {
        ...
        if(hw_action_conf->action.ptp_conf[0].enable) {
            ...
            if (vtss_phy_ts_engine_ptp_action_sync_priv( ingress, port_no, blk_id, sync_index[0],
    ...
    if(actbitmask[1] == state_actbitmask[0]) {
        ...
        if (hw_action_conf->action.ptp_conf[1].enable) {
            ...
            if (vtss_phy_ts_engine_ptp_action_sync_priv( ingress, port_no, blk_id, sync_index[1],
                                              &sw_action_conf->action.ptp_conf[0], hw_update_reqd) == VTSS_RC_OK) {
                bool3 = TRUE;
                                           ...
                                           BOOL *hw_update_reqd)
{
    ...
    if (action_ptp) {
        rc = vtss_phy_ts_engine_ptp_action_flow_conf_extract_from_hwpriv(ingress, port_no, eng_id, blk_id,hw_eng_conf,sw_eng_conf, &hw_sync_error);
    } else {
        rc = vtss_phy_ts_engine_oam_action_flow_conf_extract_from_hwpriv(ingress, port_no, eng_id, blk_id, hw_eng_conf,sw_eng_conf, &hw_sync_error);
    }
    ...
    *hw_update_reqd = hw_sync_error;
    if (hw_sync_error) {
        ...
        for(flow_idx = 0; flow_idx < 6; flow_idx++) {
            ...
            VTSS_RC(vtss_phy_ts_engine_config_priv(ingress, port_no, eng_id, FALSE));
            if (action_ptp) {
                VTSS_RC(vtss_phy_ts_engine_ptp_action_flow_delete_priv( port_no, blk_id, flow_idx));
            } else {
                VTSS_RC(vtss_phy_ts_engine_oam_action_flow_delete_priv(port_no, blk_id, eng_id, flow_idx));
            }
            /* Clear the action_flow_map */
            sw_eng_conf->action_flow_map[flow_idx] = 0;
            VTSS_RC(vtss_phy_ts_engine_config_priv(ingress, port_no, eng_id, TRUE));
        }
    }
...
static vtss_rc vtss_phy_ts_engine_action_hwupdate(BOOL ingress,
                                                  BOOL action_ptp,
                                                  vtss_phy_ts_engine_t    engine_id,
                                                  vtss_phy_ts_encap_t     encap_type,
                                                  vtss_port_no_t          port_no,
                                                  ...
                                                  vtss_phy_ts_eng_conf_t *hw_eng_conf)
{
    ...
    /* Engine should be disabled before configuring the registers */
    VTSS_RC(vtss_phy_ts_engine_config_priv(ingress, port_no, engine_id, FALSE));
    ...
    if (action_ptp) {
        ...
        for(action_idx = 0; action_idx < VTSS_PHY_TS_MAX_ACTION_PTP; action_idx++) {
            ...
            if (ptp_action_conf->enable) {
                VTSS_RC(vtss_phy_ts_engine_ptp_action_add_priv(ingress, port_no, base_port_no,
                                                         blk_id, hw_eng_conf, ptp_action_conf,
                                                                                 action_idx));
            }
        }
    } else {
        ...
        for(action_idx = 0; action_idx < VTSS_PHY_TS_MAX_ACTION_OAM; action_idx++) {
            ...
            if (oam_action_conf->enable) {
                if (oam_action_conf->y1731_en) {
                    VTSS_RC(vtss_phy_ts_engine_y1731_oam_action_add_priv(ingress, port_no,
                                             base_port_no, blk_id, engine_id, hw_eng_conf,
                                                            oam_action_conf, action_idx));
                } else {
                    VTSS_RC(vtss_phy_ts_engine_ietf_oam_action_add_priv(ingress, port_no,
    ...
    VTSS_RC(vtss_phy_ts_engine_config_priv(ingress, port_no, engine_id, TRUE));
    return VTSS_RC_OK;
}
...
static vtss_rc vtss_phy_ts_engine_flow_hwsync(BOOL ingress,
                                       BOOL action_ptp,
                                       ...
                                       vtss_phy_ts_eng_conf_t *hw_eng_conf) 
{
    ...
    VTSS_RC(vtss_phy_ts_engine_flow_conf_sync_priv(ingress, &eng_parm,
                     hw_eng_conf, sw_flow_conf, engine_update_required));
    VTSS_RC(vtss_phy_ts_engine_action_flow_conf_check_hwpriv(ingress, port_no, engine_id,
                     blk_id, action_ptp, hw_eng_conf, sw_eng_conf, hw_action_update_reqd));
    return VTSS_RC_OK;
}

static vtss_rc vtss_phy_ts_engine_flow_hwupdate(BOOL ingress,
                                         vtss_phy_ts_engine_t    engine_id,
                                         vtss_phy_ts_encap_t     encap_type,
                                         vtss_port_no_t          port_no,
                                         vtss_port_no_t          base_port_no,
                                         ...
                                         vtss_phy_ts_eng_conf_t *hw_eng_conf) 
{
    ...
    if (sw_eng_conf->eng_used) {
        ...
        VTSS_RC(vtss_phy_ts_engine_flow_set_priv(ingress, &eng_parm, hw_eng_conf,
                                                                  sw_flow_conf));
    } else {
...
static vtss_rc vtss_phy_ts_port_hwsync(const vtss_port_no_t port_no,
...
static vtss_phy_ts_eng_conf_t  hw_eng_conf;
vtss_rc vtss_phy_ts_sync(const vtss_port_no_t port_no)
{
    vtss_rc rc = VTSS_RC_OK;
    u8   ingress_ct;
    u16  phy_type;
    ...
    VTSS_RC(vtss_phy_ts_block_sync(port_no, &block_sync));
    ...
    VTSS_RC(vtss_phy_ts_port_hwsync(port_no, &init_conf));
    ...
    if ((vtss_state->phy_ts_port_conf[port_no].clk_src   != init_conf.clk_src) ||
        ...
        VTSS_RC(vtss_phy_ts_port_init(port_no, &init_conf));
    }

#if defined(VTSS_CHIP_CU_PHY) && defined(VTSS_PHY_TS_SPI_CLK_THRU_PPS0)
    if ((phy_type == VTSS_PHY_TYPE_8574) &&
        (vtss_state->phy_ts_port_conf[port_no].tx_fifo_mode == VTSS_PHY_TS_FIFO_MODE_SPI)) {
        VTSS_RC(vtss_phy_ts_new_spi_mode_sync_priv(port_no, vtss_state->phy_ts_port_conf[port_no].new_spi_conf.enable));
    }
#endif

    /* Check if the channel is enabled */
    base_port_no = vtss_state->phy_ts_port_conf[port_no].base_port;
    VTSS_RC(vtss_phy_ts_mode_sync(port_no, base_port_no, &generic_phy_engine_sync_reqd));
    ...
    for (engine_id = VTSS_PHY_TS_PTP_ENGINE_ID_0; engine_id < VTSS_PHY_TS_ENGINE_ID_INVALID; engine_id++) {
        for (ingress_ct = 0; ingress_ct <= 1; ingress_ct++) {
            ...
            VTSS_RC(vtss_phy_ts_ana_blk_id_get(engine_id, ingress, &blk_id));
            VTSS_RC(vtss_phy_ts_engine_sync_priv(ingress, port_no, base_port_no, engine_id,
                                              blk_id, &engine_enabled, &flow_match_mode));
            if (vtss_phy_ts_engine_encap_conf_sync_priv(ingress, port_no, base_port_no,
            ...
            if ((hw_eng_conf.eng_used        != sw_eng_conf->eng_used) || 
                ...
                if(sw_alt_eng_conf && sw_alt_eng_conf->eng_used) {
                    ...
                    rc = vtss_phy_ts_engine_config_priv(ingress, port_no, engine_id, FALSE);
                }
                if(sw_eng_conf->eng_used) {
                    ...
                    rc = vtss_phy_ts_engine_next_comp_sync_priv(ingress, port_no,
                                           blk_id, engine_id, sw_eng_conf->encap_type);
                    encap_type = sw_eng_conf->encap_type;
                    rc = vtss_phy_ts_flow_match_mode_set_priv(port_no, blk_id,
                                           engine_id, sw_eng_conf->flow_match_mode);
            ...
            if (phy_engine_sync_reqd) {
                ...
                if((rc = vtss_phy_ts_engine_flow_hwsync(ingress, action_ptp, &engine_update_required,
            ...
            if (engine_enable_reqd) {
                VTSS_PHY_TS_SPI_PAUSE_PRIV(port_no);
                if ((rc = vtss_phy_ts_engine_config_priv(ingress, port_no, engine_id, TRUE))
                                                                       != VTSS_RC_OK) {
            ...
            if (engine_update_required) {
                VTSS_PHY_TS_SPI_PAUSE_PRIV(port_no);
                if ((rc = vtss_phy_ts_engine_flow_hwupdate(ingress, engine_id, encap_type,
                                                      port_no, base_port_no, blk_id,
                                                      sw_eng_conf, &hw_eng_conf)) != VTSS_RC_OK) {
 ...
 */
            if (hw_action_update_reqd) {
                 ...
                 */
                VTSS_PHY_TS_SPI_PAUSE_PRIV(port_no);
                if ((rc= vtss_phy_ts_engine_action_hwupdate(ingress, action_ptp, engine_id, encap_type,
    ...
    if(!generic_phy_engine_sync_reqd && vtss_state->phy_ts_port_conf[port_no].port_ena) {
        ...
        rc = vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_ENA_SET);
    }

    VTSS_PHY_TS_SPI_PAUSE_PRIV(port_no);
    /* Update the ingress_latency and egress_latency. */
    if (vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_ING_LATENCY_SET) != VTSS_RC_OK) {
        VTSS_E("Ingress Latency set fail, port %u", port_no);
        rc = VTSS_RC_ERROR;
    }
    if (vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_EGR_LATENCY_SET) != VTSS_RC_OK) {
        VTSS_E("Egress Latency set fail, port %u", port_no);
        rc = VTSS_RC_ERROR;
    }
    if (vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_OPER_MODE_CHANGE_SET) != VTSS_RC_OK) {
        VTSS_E("PHY operational mode change set failed, port %u", port_no);
        rc = VTSS_RC_ERROR;
    }
    VTSS_PHY_TS_SPI_UNPAUSE_PRIV(port_no);
    ...
    VTSS_SYNC_RC(vtss_phy_ts_csr_set_priv(port_no, VTSS_PHY_TS_PORT_EVT_MASK_SET));
