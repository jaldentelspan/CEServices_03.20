...
static vtss_rc daytona_oha_config_set(const vtss_port_no_t port_no)
{
    ...
    VTSS_RC(daytona_port_2_target(port_no, DAYTONA_BLOCK_EWIS, &target_wis));
...
vtss_rc vtss_daytona_oha_restart_conf_set(void)
{
    vtss_port_no_t port_no;
    vtss_rc rc;

    for (port_no = 0 ; port_no < vtss_state->port_count; port_no++) {
        rc = daytona_oha_config_set(port_no);
 ...
 * \return Return code.
 **/
vtss_rc vtss_daytona_inst_oha_create(void)
{
