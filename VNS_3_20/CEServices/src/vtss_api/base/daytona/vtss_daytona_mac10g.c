...
static vtss_rc daytona_mac10g_config_set(const vtss_port_no_t port_no)
{
    u32 target;
    u32 value;
    u32 value_mac_clk = 0;
    ...
    VTSS_RC(daytona_port_2_target(port_no, DAYTONA_BLOCK_MAC10G, &target));
    ...
    if (vtss_state->mac10g_state[port_no].mac10g_cfg.rx_tx_loopback != FALSE) { /*Tx to Rx Loopback Enabled */
         ...
         */
        if (side == 1)  {
          VTSS_RC(daytona_port_2_mode(port_no, &mode));
          if (mode != VTSS_CONFIG_MODE_DISABLED) {
            VTSS_RC(vtss_daytona_blk_oper_mode_get(port_no, channel,BLOCK_GCB, &blk_mode));
...
static vtss_rc daytona_mac10g_status_get(const vtss_port_no_t port_no,
...
static vtss_rc daytona_mac10g_counters(const vtss_port_no_t port_no, 
                                       vtss_port_counters_t *const counters,
                                       BOOL clr)
{

    u32 target;
    vtss_port_mac10g_counters_t        *c = &vtss_state->port_counters[port_no].counter.mac10g;
...
static vtss_rc daytona_mac10g_counters_update(const vtss_port_no_t port_no)
{
    return daytona_mac10g_counters(port_no, NULL, 0);
}

static vtss_rc daytona_mac10g_counters_clear(const vtss_port_no_t port_no)
{
    return daytona_mac10g_counters(port_no, NULL, 1);
}

static vtss_rc daytona_mac10g_counters_get(const vtss_port_no_t port_no, vtss_port_counters_t *const counters)
{
    return daytona_mac10g_counters(port_no, counters, 0);
}

/* ================================================================= *
 *  Defects/Events
 * ================================================================= */
static vtss_rc daytona_mac10g_event_enable(const vtss_port_no_t        port_no,
                                           const BOOL                  enable,
...
static vtss_rc daytona_mac10g_event_poll(const vtss_port_no_t    port_no,
...
static vtss_rc daytona_mac10g_event_poll_without_mask(const vtss_port_no_t    port_no,
                                                      vtss_mac10g_event_t     *const status)
{
    u32 target;
    u32 sticky;
 ...
 * ================================================================= */
vtss_rc vtss_daytona_mac10g_restart_conf_set(void)
{
    vtss_port_no_t port_no;
    vtss_rc rc;

    for (port_no = 0 ; port_no < vtss_state->port_count; port_no++) {
        rc = daytona_mac10g_counters_clear(port_no);
        rc = daytona_mac10g_config_set(port_no);
...
static vtss_mac10g_cfg_t mac10g_default = {
    ...
    .max_tags           = VTSS_DAYTONA_MAC10G_DEFAULT_MAX_TAGS,
 ...
 * \return Return code.
 **/
vtss_rc vtss_daytona_inst_mac10g_create(void)
{
    vtss_port_no_t port_no;
