...
static vtss_rc vtss_daytona_rab_config_set(const vtss_port_no_t port_no)
{
    u32 target;
    u32 value;
    vtss_rab_cfg_t *cfg;

    VTSS_RC(daytona_port_2_target(port_no, DAYTONA_BLOCK_RAB, &target));
...
static vtss_rc vtss_daytona_rab_counters_get(const vtss_port_no_t port_no,
...
vtss_rc vtss_daytona_rab_restart_conf_set(void)
{
    ...
    for (port_no = 0 ; port_no < vtss_state->port_count; port_no++) {
        ...
        rc = vtss_daytona_rab_config_set(port_no);
 ...
 * \return Return code.
 **/
vtss_rc vtss_daytona_inst_rab_create(void)
{
    vtss_port_no_t port_no;
