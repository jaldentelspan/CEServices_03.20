...
#define DAYTONA_RD(p, value)                            \
    {                                                  \
        vtss_rc __rc__ = daytona_pi_rd(0,p,value); \
        if (__rc__ != VTSS_RC_OK)                          \
            return __rc__;                                 \
    }

#define DAYTONA_RD_D(p, value)  daytona_pi_rd(0,p,value)

/* Write register */
#define DAYTONA_WR(p, value)                            \
    {                                              \
        vtss_rc __rc__ = daytona_pi_wr(0,p,value); \
        if (__rc__ != VTSS_RC_OK)                          \
            return __rc__;                                 \
    }
...
#define DAYTONA_WRF(p,o,w,value)                            \
    {                                              \
        vtss_rc __rc__ = daytona_wrf(0,p,o,w,value); \
        if (__rc__ != VTSS_RC_OK)                          \
            return __rc__;                                 \
    }
...
#define DAYTONA_RDF(p,o,w,value)                            \
    {                                              \
        vtss_rc __rc__ = daytona_rdf(0,p,o,w,value); \
        if (__rc__ != VTSS_RC_OK)                          \
            return __rc__;                                 \
    }
...
#define DAYTONA_WRM(p, value, mask)                            \
    {                                                         \
        vtss_rc __rc__ = daytona_wrm(0, p, value, mask); \
        if (__rc__ != VTSS_RC_OK)                                 \
            return __rc__;                                        \
    }
 ...
 * \return Return code.
 **/
vtss_rc daytona_port_2_mode(vtss_port_no_t port, vtss_config_mode_t *mode);
 ...
 * \return Return code.
 **/
vtss_rc daytona_port_2_target(vtss_port_no_t port, daytona_block_id_t fb, u32 *target);
 ...
 * \return Return code.
 **/
vtss_rc daytona_port_2_channel_side(vtss_port_no_t port, daytona_channel_t *channel, daytona_side_t *side);
 ...
 * \return Return code.
 **/
vtss_rc daytona_pi_rd(vtss_chip_no_t chip_no, u32 addr, u32 *value);
 ...
 * \return Return code.
 **/
vtss_rc daytona_pi_wr(vtss_chip_no_t chip_no, u32 addr, const u32 value);
 ...
 * \return Return code.
 **/
vtss_rc daytona_rdf(vtss_chip_no_t chip_no, u32 reg, u32 bit_offset, u32 bit_width, u32 *value);
 ...
 * \return Return code.
 **/
vtss_rc daytona_wrf(vtss_chip_no_t chip_no, u32 reg, u32 bit_offset, u32 bit_width, u32 value);
 ...
 * \return Return code.
 **/
vtss_rc daytona_wrm(vtss_chip_no_t chip_no, u32 reg, u32 value, u32 mask);
