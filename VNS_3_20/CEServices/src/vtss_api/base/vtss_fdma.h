...
#include "vtss_serval.h" /* For VTSS_AFI_SLOT_CNT */
...
typedef struct {
    ...
    vtss_rc (*fdma_inj_restart_ch)       (struct vtss_state_s *const vstate, vtss_fdma_ch_t inj_ch, fdma_sw_dcb_t *head, BOOL do_start, vtss_trace_group_t trc_grp);
#if VTSS_OPT_FDMA_VER >= 3
...
typedef struct {
    ...
    // do this read.
    vtss_rc (*rx_frame_get_internal)(vtss_chip_no_t       chip_no,
    ...
    vtss_rc (*rx_frame_discard_internal)(vtss_chip_no_t       chip_no,
                                         vtss_packet_rx_grp_t grp,
                                         u32                  *discarded_bytes);
