                             ...
                             const vtss_xaui_cfg_t *const cfg)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && cfg) {
        ...
        rc = VTSS_FUNC_COLD(xaui_config_set, port_no);
    }
    VTSS_EXIT();
    return rc;
}
                             ...
                             vtss_xaui_status_t *const status)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && status) {
        rc = VTSS_FUNC(xaui_status_get, port_no, status);
    }
    VTSS_EXIT();
    return rc;
}
                               ...
                               vtss_xaui_pm_cnt_t *const cnt)
{
    ...
    VTSS_D("port_no: %u", port_no);
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && cnt) {
        rc = VTSS_FUNC(xaui_counters_get, port_no, cnt);
    }
    VTSS_EXIT();
    return rc;
}
                                 ...
                                 const vtss_port_no_t port_no)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = VTSS_FUNC(xaui_counters_clear, port_no);
    }
    VTSS_EXIT();
    return rc;
}
