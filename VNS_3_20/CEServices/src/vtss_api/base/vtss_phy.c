...
static BOOL sync_calling_private = FALSE; // Global variable signaling if registers only shall be updated if the value is different from the current value.

static BOOL link_down_due_to_port_reset[VTSS_PORTS]; // We need to keep track of if the link has been down due to port reset
...
vtss_rc vtss_phy_micro_assert_reset(const vtss_port_no_t port_no)
{
    ...
    case VTSS_PHY_FAMILY_TESLA:
      ...
      VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x800f));
      ...
      break;
    default: 
      VTSS_RC(vtss_phy_wr_masked(port_no, 0|0x10<<5, 0, 0x8000));
      break;
    }
...
static const char *vtss_phy_media_if2txt(vtss_phy_media_interface_t media_if_type)
{
    ...
    case VTSS_PHY_MEDIA_IF_CU:              return "VTSS_PHY_MEDIA_IF_CU";
    case VTSS_PHY_MEDIA_IF_SFP_PASSTHRU:    return "VTSS_PHY_MEDIA_IF_SFP_PASSTHRU";
    case VTSS_PHY_MEDIA_IF_FI_1000BX:       return "VTSS_PHY_MEDIA_IF_FI_1000BX";
    case VTSS_PHY_MEDIA_IF_FI_100FX:        return "VTSS_PHY_MEDIA_IF_FI_100FX"; 
    case VTSS_PHY_MEDIA_IF_AMS_CU_PASSTHRU: return "VTSS_PHY_MEDIA_IF_AMS_CU_PASSTHRU"; 
    case VTSS_PHY_MEDIA_IF_AMS_FI_PASSTHRU: return "VTSS_PHY_MEDIA_IF_AMS_FI_PASSTHRU";
    case VTSS_PHY_MEDIA_IF_AMS_CU_1000BX:   return "VTSS_PHY_MEDIA_IF_AMS_CU_1000BX";
    case VTSS_PHY_MEDIA_IF_AMS_FI_1000BX:   return "VTSS_PHY_MEDIA_IF_AMS_FI_1000BX";
    case VTSS_PHY_MEDIA_IF_AMS_CU_100FX:    return "VTSS_PHY_MEDIA_IF_AMS_CU_100FX";
    case VTSS_PHY_MEDIA_IF_AMS_FI_100FX:    return "VTSS_PHY_MEDIA_IF_AMS_FI_100FX";
    }
...
static vtss_rc vtss_phy_i2c_wait_for_ready (vtss_port_no_t port_no) {
    u16 timeout = 500;
    u16 reg_value;

    VTSS_RC(vtss_phy_page_gpio(port_no));
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_I2C_MUX_CONTROL_2, &reg_value));
...
static vtss_rc vtss_phy_i2c_wr_private(vtss_port_no_t port_no, u8 i2c_mux, u8 i2c_reg_addr, u8 i2c_device_addr, u8 *value, u8 cnt) 
{
    u8 i;
    u32 reg_val;
    ...
    VTSS_RC(vtss_phy_i2c_wait_for_ready(port_no));
    ...
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_I2C_MUX_CONTROL_1,
    ...
    for (i = 0; i < cnt; i++) {
        ...
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_I2C_MUX_DATA_READ_WRITE, value[i]));
        
        // Execute the write
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_I2C_MUX_CONTROL_2,
                                 VTSS_F_PHY_I2C_MUX_CONTROL_2_MUX_READY |
                                 VTSS_F_PHY_I2C_MUX_CONTROL_2_PHY_PORT_ADDR(i2c_mux) |
                                 VTSS_F_PHY_I2C_MUX_CONTROL_2_ENA_I2C_MAX_ACCESS | 
...
// In: port_no - The PHY port number starting from 0.
static vtss_rc vtss_phy_i2c_rd_private(vtss_port_no_t port_no, u8 i2c_mux, u8 i2c_reg_addr, u8 i2c_device_addr, u8 *value, u8 cnt) {
    ...
    for (i = 0; i < cnt; i++) {
        ...
        VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_I2C_MUX_DATA_READ_WRITE, &reg_val16));
                                     ...
                                     const u16            mask)
{
    ...
    /* Change page */
    if (page) {
        rc = write_func(port_no, 31, page);
    }

    if (rc == VTSS_RC_OK) {
        if (read) {
            /* Read */
            rc = read_func(port_no, reg, value);
        } else if (mask != 0xffff) {
            /* Read-modify-write */
            if ((rc = read_func(port_no, reg, &val)) == VTSS_RC_OK)
                rc = write_func(port_no, reg, (val & ~mask) | (*value & mask));
        } else {
            /* Write */
            
            rc = write_func(port_no, reg, *value);
        }
    }

    /* Restore standard page */
    if (page && rc == VTSS_RC_OK) {
        rc = write_func(port_no, 31, VTSS_PHY_PAGE_STANDARD);
    }
...
static vtss_rc  vtss_phy_do_page_chk(const vtss_port_no_t port_no,
                                     const u16            page,
                                     const u16            line) {
    u16 current_page;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    default:
        // Do the page check
        VTSS_RC(vtss_phy_rd(port_no, 31, &current_page));
...
{
    if (do_page_chk) {
        VTSS_RC(vtss_phy_do_page_chk(port_no, page, line));
    }
    return vtss_phy_rd_wr_masked(1, port_no, addr, value, 0);
}
                                ...
                                const u16            line)
{
    ...
    return vtss_phy_rd_wr_masked(0, port_no, addr, &val, mask);
}
...
static BOOL warm_start_reg_changed[VTSS_PORTS];
...
#define VTSS_PHY_WARM_WR_MASKED(port_no, page_addr, value, mask) vtss_phy_warm_wr_masked(port_no, page_addr, value, mask, 0xFFFF, __FUNCTION__,  __LINE__)
...
#define VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, page_addr, value, mask, chk_mask) vtss_phy_warm_wr_masked(port_no, page_addr, value, mask, chk_mask, __FUNCTION__, __LINE__)
...
static vtss_rc vtss_phy_warm_wr_masked(const vtss_port_no_t port_no,
                                       const u16            page,
                                       const u32            addr,
                                       const u16            value,
                                       const u16            mask,
                                       ...
                                       
{
    ...
    if (sync_calling_private){
        VTSS_RC(vtss_phy_rd_page(port_no, page, addr, &val, line));
        if ((val ^ value) & mask & chk_mask) { /* Change in bit field */
            ...
            VTSS_I("warm_start_reg_changed:%d, chk_mask:%d", warm_start_reg_changed[port_no], chk_mask);

            warm_start_reg_changed[port_no] = TRUE; // Signaling the a register for this port port has changed.
            
            VTSS_RC(vtss_phy_wr_masked_page(port_no, page, addr, value, mask, line)); 
        }
    } else {
        VTSS_RC(vtss_phy_wr_masked_page(port_no, page, addr, value, mask, line));
    }
...
#define VTSS_PHY_WARM_WR(port_no, page_addr, value) vtss_phy_warm_wr(port_no, page_addr, value, __FUNCTION__, __LINE__)
...
//      function - Pointer to string containing the name of the calling function.
//      line     - Line number calling the function.
static vtss_rc vtss_phy_warm_wr(const vtss_port_no_t port_no,
                                const u16            page,
                                const u32            addr,
                                const u16            value,
                                const char           *function,                                 
                                const u16            line)
{
    return vtss_phy_warm_wr_masked(port_no, page, addr, value, 0xFFFF, 0xFFFF, function, line);
}


// Clause 45 register access setup. Setups the MMD register access with the address.
//
 static vtss_rc vtss_phy_mmd_reg_setup(const vtss_port_no_t port_no, 
                                       const u8  devad,
                                       const u16 addr) {
     VTSS_RC(vtss_phy_page_std(port_no));
     VTSS_RC(vtss_phy_wr(port_no, 13, devad)); // Setup cmd=address + devad address
     VTSS_RC(vtss_phy_wr(port_no, 14, addr));  // Setup address
     VTSS_RC(vtss_phy_wr(port_no, 13, (1 << 14) + devad)); // Setup cmd=data + devad address
     return VTSS_RC_OK;
}


// Clause 45 writes
static vtss_rc vtss_phy_mmd_wr(const vtss_port_no_t port_no, 
                               const u8  devad,
                               const u16 addr,
                               const u16 data_val) {
    VTSS_RC(vtss_phy_mmd_reg_setup(port_no, devad, addr));
    VTSS_RC(vtss_phy_wr(port_no, 14, data_val));  // Write data
...
// Clause 45 reads 
static vtss_rc vtss_phy_mmd_rd(const vtss_port_no_t port_no, 
                               const u8 devad,
                               const u16 addr,
                               u16   *value) {
    VTSS_RC(vtss_phy_mmd_reg_setup(port_no, devad, addr));
    VTSS_RC(vtss_phy_rd(port_no, 14, value));  // read data
...
static vtss_rc vtss_phy_mmd_reg_wr_masked(const vtss_port_no_t port_no, 
                                          const u16 devad,
                                          const u16 addr,
                                          const u16 data_val,
                                          const u16 mask) {
    u16 current_reg_val;
    VTSS_RC(vtss_phy_mmd_rd(port_no, devad, addr, &current_reg_val)); // Read current value of the register
    VTSS_RC(vtss_phy_wr(port_no, 14, (current_reg_val & ~mask) | (data_val & mask) ));  // Modify current value and write.
...
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_STANDARD);
}

vtss_rc vtss_phy_page_ext(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_EXTENDED);
}

vtss_rc vtss_phy_page_ext2(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_EXTENDED_2);
}

vtss_rc vtss_phy_page_ext3(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_EXTENDED_3);
}

vtss_rc vtss_phy_page_gpio(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_GPIO);
}

vtss_rc vtss_phy_page_test(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_TEST);
}

vtss_rc vtss_phy_page_tr(vtss_port_no_t port_no)
{
    return vtss_phy_wr(port_no, 31, VTSS_PHY_PAGE_TR);
}


// Function for mapping the port_no giving to a function to the  physical port number.
static vtss_port_no_t vtss_phy_chip_port(vtss_port_no_t port_no) {
    u16 reg_val;
    u16 current_page;
    VTSS_RC(vtss_phy_rd(port_no, 31, &current_page));
    VTSS_RC(vtss_phy_page_ext(port_no));
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_EXTENDED_PHY_CONTROL_4, &reg_val));
    VTSS_D("reg_val:0x%X, chip_no:%d, port_no:%u", reg_val,  (reg_val & VTSS_M_VTSS_PHY_EXTENDED_PHY_CONTROL_4_PHY_ADDRESS) >> 11, port_no);
    VTSS_RC(vtss_phy_wr(port_no, 31, current_page));
...
static vtss_rc port_reset(vtss_port_no_t port_no) {
    u16 reg;
    vtss_mtimer_t         timer;

    VTSS_RC(atom_patch_suspend(port_no, TRUE)); // Suspend the micro patch while resetting 
    
    // If warm starting we don't want to reset the port unless some register has changed.
    if (!sync_calling_private || warm_start_reg_changed[port_no]) {
        VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_MODE_CONTROL, 
    ...
    while (1) {
        if (PHY_RD_PAGE(port_no, VTSS_PHY_MODE_CONTROL, &reg) == VTSS_RC_OK && (reg & VTSS_F_PHY_MODE_CONTROL_SW_RESET) == 0)
            break;
        VTSS_MSLEEP(20);
    ...
    VTSS_RC(atom_patch_suspend(port_no, FALSE)); // Restart micro patch
    return VTSS_RC_OK;
}
...
static vtss_rc vtss_phy_optimize_receiver_init(vtss_port_no_t port_no)
{
    ...
    case VTSS_PHY_FAMILY_TESLA:
        break;
    default:
        VTSS_RC(vtss_phy_wr_masked(port_no, 12, 0x0200, 0x0300)); // Not needed for ATOM/TESLA
    }

    if (ps->power.vtss_phy_power_dynamic) {
        /* Restore changed dsp setting used by dynamic power saving algorithm */
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_TEST_PAGE_12, 0x0000, 0xfc00));
        ...
        case VTSS_PHY_FAMILY_TESLA:
            ...
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0xafe4));
            VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
            reg17 = (reg17 & 0xffef); //Clear half_adc as desired
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_17, reg17));
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0x8fe4));
            break;
        default:
            /* Restore 1000-BT LD-Q Current Control */
            // For now don't restore at warm start
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, VTSS_PHY_TEST_PAGE_24, 0x2000, 0x2000, 0x0));
        }
...
static vtss_rc vtss_phy_optimize_dsp (vtss_port_no_t port_no)
{
    ...
    VTSS_RC(vtss_phy_page_tr(port_no));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xaf8a));               /*- Read PMA internal Register 5 */
    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0000, 0x0000)); /*- Configure PMA */
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0008, 0x000c)); /*- Register 5 */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f8a));               /*- Write PMA internal Register 5 */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xaf86));               /*- Read PMA internal Register 3 */
    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0008, 0x000c)); /*- Configure PMA */
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0000, 0x0000)); /*- Register 3 */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f86));               /*- Write PMA internal Register 3 */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xaf82));               /*- Read PMA internal Register 1 */
    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0000, 0x0000)); /*- Configure PMA */
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0100, 0x0180)); /*- Register 1 */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f82));               /*- Write PMA internal Register 1 */
...
// Function that returns TRUE is the port's speed is 10BASE-T 
// In   : iport - port starting for 0.
// Return : TRUE if port is in 10base mode else FALSE
/*static BOOL is_speed_10base_t (vtss_port_no_t iport) {
    u16 status;
    
    return FALSE;
    VTSS_RC(vtss_phy_rd(iport, 28, &status)); // getting status

    // Bit 4:3 is the speed. 0 = 10BASE-T
    if ((status & 0x18) != 0) {
        return FALSE;
    } else {
        return TRUE;
...
static u16 ecpdset[] = { 0, 5, 9, 12, 14 };
...
static vtss_rc vtss_phy_power_opt(const vtss_port_no_t port_no)
{
    ...
    VTSS_RC(vtss_phy_page_tr(port_no));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no,VTSS_PHY_PAGE_TR_16, 0xa3aa));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_18, 0x0003));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_17, reg17));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0x83aa));
    ...
    {
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_TEST_PAGE_12, ((ecpdset[ecpd_idx]<<12)|(ncpd<<10)), 0xfc00));
        ...
        if (done != 0) {
            ...
            /*- Read MSE A and B */
            VTSS_RC(vtss_phy_page_tr(port_no));
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0xa3c0));
            VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
            VTSS_RC(vtss_phy_rd(port_no, 18, &reg18));
            ...
            /*- Read MSE C and D */
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0xa3c2));
            VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
            VTSS_RC(vtss_phy_rd(port_no, 18, &reg18));
    ...
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0xa3aa));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_18, 0x0000));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_17, reg17));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0x83aa));
    VTSS_RC(vtss_phy_page_std(port_no));
...
static vtss_rc vtss_phy_optimize_receiver_reconfig(vtss_port_no_t port_no)
{
    ...
    VTSS_RC(vtss_phy_page_tr(port_no));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_PAGE_TR_16, 0xaff0));
    VTSS_RC(vtss_phy_rd(port_no, 17, &vga_state_a));
    ...
    if ((vga_state_a < 16) || (vga_state_a > (max_vga_state_to_optimize & 0x1f))) {
        ...
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_TEST_PAGE_12, 0x0000, 0x0300));
        ...
        if (ps->power.vtss_phy_power_dynamic) {
            ...
            default:
                ...
                VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x0000, 0x2000));
                break;
            }

            /*- Dynamic Power Reduction */
            VTSS_RC(vtss_phy_power_opt(port_no));
        } else {
            ps->power.pwrusage = VTSS_PHY_LINK_UP_FULL_PWR;
        }
    }
...
static vtss_rc vtss_phy_optimize_jumbo(vtss_port_no_t port_no)
{
    ...
    VTSS_RC(vtss_phy_wr(port_no, 17, 0x0E35));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x9786));
    VTSS_RC(vtss_phy_page_test(port_no));
...
static vtss_rc vtss_phy_optimize_rgmii_strength(vtss_port_no_t port_no)
{
    /* BZ 2094 */
    VTSS_RC(vtss_phy_page_test(port_no));
    VTSS_RC(vtss_phy_wr(port_no, 3, 0xf082));
    VTSS_RC(vtss_phy_wr(port_no, 3, 0xf082));
    return vtss_phy_page_std(port_no);
}
...
static vtss_rc vtss_phy_mdi_control_reg(vtss_port_no_t port_no) {
    vtss_port_status_t   status;
    VTSS_RC(vtss_phy_status_get_private(port_no, &status));
                               
  
    if (status.speed == VTSS_SPEED_1G) {
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_BYPASS_CONTROL, 0x0000, 
                                        VTSS_F_PHY_BYPASS_CONTROL_HP_AUTO_MDIX_AT_FORCE)); 
        VTSS_N("speed:%d", status.speed); 
    } else {
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_BYPASS_CONTROL, VTSS_F_PHY_BYPASS_CONTROL_HP_AUTO_MDIX_AT_FORCE, 
                                        VTSS_F_PHY_BYPASS_CONTROL_HP_AUTO_MDIX_AT_FORCE)); 
    }

    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_BYPASS_CONTROL, 
                                    0,
...
static vtss_rc vtss_phy_mdi_setup(const vtss_port_no_t port_no) {
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    case VTSS_PHY_MDIX_AUTO:
        ...
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_MODE_CONTROL, 
    ...
    case VTSS_PHY_MDIX:
        ...
        VTSS_RC(vtss_phy_mdi_control_reg(port_no));
...
static vtss_rc vtss_phy_detect(const vtss_port_no_t port_no)
{
    vtss_rc               rc = VTSS_RC_OK;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    u16                   reg2, reg3, model;
    ...
    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(vtss_phy_rd(port_no, 2, &reg2));
    VTSS_RC(vtss_phy_rd(port_no, 3, &reg3));
    oui = ((reg2 << 6) | ((reg3 >> 10) & 0x3F));
    model = ((reg3 >> 4) & 0x3F);
    ps->revision = (reg3 & 0xF);

    ps->conf_none = (oui == 0x005043 && model == 2 && ps->revision == 4 ? 1 : 0);
    ...
    if (oui == 0x0001C1) {
        ...
        case 0x0a: // VSC8574 Tesla 
            ...
            ps->type.port_cnt = 4;
            ps->type.channel_id = vtss_phy_chip_port(port_no);
            ps->type.phy_api_base_no = (ps->type.channel_id >= 2) ? port_no - 2 : port_no;
            break;
    ...
    VTSS_I("port_no:%u, Detect oui:0x%X, model:0x%X, revision:%d, detected:%s PHY:%d_%d (model:0x%02x, features:0x%02x, family:%s)",
           port_no,
           oui, model, ps->revision,
           ps->type.part_number != VTSS_PHY_TYPE_NONE ? "Vitesse" : "Unknown",
           ps->type.part_number, ps->revision, model, ps->features, vtss_phy_family2txt(ps->family));

    return rc;
}

// Function that returns the chip type/id for a given port.
// In     : iport  - Internal port (starting from 0)
// In/Out : phy_id - Pointer to phy_id to be returned.
static vtss_rc vtss_phy_id_get_private(const vtss_port_no_t port_no, vtss_phy_type_t *phy_id) 
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    *phy_id = ps->type;
...
static vtss_rc vtss_phy_enab_smrt_premphasis (vtss_port_no_t port_no)
{
	/* BZ 2675 */
	VTSS_RC(vtss_phy_page_tr(port_no));
	VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7fa));
	VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0000, 0x0000));
	VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0008, 0x0008));
	VTSS_RC(vtss_phy_wr(port_no, 16, 0x87fa));
	VTSS_RC(vtss_phy_page_std(port_no));
...
void vga_adc_debug(u8 vga_adc_pwr, u8 phy) {
    ...
    (void) vtss_phy_wr(phy, 31, 0x0010);	// switch to micro page (glabal to all 12 PHYs)
    (void) PHY_WR_PAGE(phy, VTSS_PHY_MICRO_PAGE, 0x9024);
    
    (void) vtss_phy_wr(phy, 31, 0x52b5);	// Switch to token-ring register page
    (void) vtss_phy_wr(phy, 18, 0x0001);
    (void) vtss_phy_wr(phy, 17, 0x40b9 | (vga_adc_pwr<<1));
    (void) vtss_phy_wr(phy, 16, 0x8fda);	// for 100
    (void) vtss_phy_wr(phy, 18, 0x0000);
    (void) vtss_phy_wr(phy, 17, 0x0159 | (vga_adc_pwr<<1));
    (void) vtss_phy_wr(phy, 16, 0x8fd6);	// for 1000
    
    // turn micro VGA patch back on to allow correct PHY start-up
    (void) vtss_phy_wr(phy, 31, 0x0010);	// switch to micro page (glabal to all 12 PHYs)
    (void) PHY_WR_PAGE(phy, VTSS_PHY_MICRO_PAGE, 0x9004);
    (void) vtss_phy_page_std(phy);
}

// Function for testing phy features
static inline BOOL vtss_phy_can(const vtss_port_no_t port_no, vtss_phy_feature_t feature)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
...
static vtss_rc vtss_phy_eee_ena_private(const vtss_port_no_t port_no) 
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    vtss_rc rc = VTSS_RC_OK;
    ...
    case VTSS_PHY_FAMILY_ATOM:
    case VTSS_PHY_FAMILY_LUTON26:
        if (ps->revision == VTSS_PHY_ATOM_REV_B) {
            if (vtss_state->phy_state[port_no].eee_conf.eee_mode == EEE_ENABLE) {
                VTSS_RC(atom_patch_suspend(port_no, FALSE)); // Make sure that 8051 patch is running
    ...
    case VTSS_PHY_FAMILY_TESLA:
        ...
        if (reconfigure) {
            ...
            if (ps->eee_conf.eee_ena_phy) {
                VTSS_RC(vtss_phy_page_ext2(port_no));
                VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EEE_CONTROL, 
                                                      VTSS_F_PHY_EEE_CONTROL_ENABLE_10BASE_TE, 
                                                      VTSS_F_PHY_EEE_CONTROL_ENABLE_10BASE_TE)); // Enable EEE (EEE control, Adress 17E2, bit 15)
                VTSS_RC(vtss_phy_mmd_reg_wr_masked(port_no, 7, 60, 6, 0x0006)); // Enable advertisement
                
                VTSS_RC(vtss_phy_wr(port_no, VTSS_PHY_PAGE, 0x2a30));                      // Switch to test-register page
                VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_TEST_PAGE_25, 0, 1));                 // Clear EEE bit (bit 0)
            } else {
                VTSS_RC(vtss_phy_page_ext2(port_no));
                VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EEE_CONTROL, 0x0, VTSS_F_PHY_EEE_CONTROL_ENABLE_10BASE_TE));  // Disable EEE (EEE control, Adress 17E2, bit 15)
                VTSS_RC(vtss_phy_mmd_reg_wr_masked(port_no, 7, 60, 0, 0x0006));    // Disable advertisement
            }

            VTSS_RC(vtss_phy_page_std(port_no));   // Go back to standard page.            
            ...
            if (ps->setup.mode == VTSS_PHY_MODE_ANEG) {
                VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_MODE_CONTROL, 
...
static vtss_rc vtss_phy_eee_link_partner_advertisements_get_private(const vtss_port_no_t        port_no,
                                                                    u8  *advertisements) {
    ...
    case VTSS_PHY_FAMILY_LUTON26:
    case VTSS_PHY_FAMILY_TESLA:
        VTSS_RC(vtss_phy_mmd_rd(port_no, 7, 61, &reg_val)); // Get the link partner advertisement.
...
static vtss_rc vtss_phy_eee_power_save_state_get_private(const vtss_port_no_t        port_no,
    ...
    case VTSS_PHY_FAMILY_LUTON26:
    case VTSS_PHY_FAMILY_TESLA:
        VTSS_RC(vtss_phy_mmd_rd(port_no, 3, 1, &reg_val)); // Get PCS status
...
static vtss_rc vtss_phy_mac_media_if_cobra_setup(const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf)
{
    u16 reg = 0;
    ...
    VTSS_RC(vtss_phy_wr(port_no, 23, reg));
    return VTSS_RC_OK;


// Configuration error.
conf_error: VTSS_E("Cobra family, Port %u, Media interface:%s not supported for mac if:%s", 
...
static vtss_rc vtss_phy_init_seq_cobra(vtss_phy_port_state_t *ps,
...
//
static vtss_rc vtss_phy_atom12_cfg_ib_cterm_ena_private(vtss_port_no_t port_no, u8 ib_cterm_ena, u8 ib_eq_mode)
{
    u8 tmp, tmp1;
    u16 reg18g = 0;
    vtss_rc rc = VTSS_RC_OK;
    ...
    // fetch MCB data
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8113));     // Read MCB for 6G macro 1 into PRAM
    ...
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0xd7d9));     // set mem_addr to cfg_buf[10] (0x47d9)
    (void) vtss_phy_wait_for_micro_complete(port_no);

    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8007));     // read the value of cfg_buf[10] w/o post-incr.
    (void) vtss_phy_wait_for_micro_complete(port_no);

    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    // get bits 11:4 from return value and OR in ib_rst (bit 6)
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MICRO_PAGE, &reg18g));
    tmp1 = (u8)(((reg18g >> 4) & 0xff) | 0x40);

    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x9006 | (tmp1 << 4)));    // write the value back to cfg_buf[10] w/ post-incr.
    (void) vtss_phy_wait_for_micro_complete(port_no);

    // update ib_cterm_ena and ib_eq_mode
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8007));     // read the value of cfg_buf[11] w/o post-incr.
    (void) vtss_phy_wait_for_micro_complete(port_no);
    ...
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8006 | (tmp << 4)));    // write the value back to cfg_buf[11] w/o post-incr.
    (void) vtss_phy_wait_for_micro_complete(port_no);

    // Update MCB w/ ib_rst asserted
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0xd7ce));     // set mem_addr to 0x47ce (addr_vec)
    (void) vtss_phy_wait_for_micro_complete(port_no);

    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x80e6));     // set addr_vec for macros 1-3
    (void) vtss_phy_wait_for_micro_complete(port_no);

    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x9cc0));     // Write MCB for 6G macros 1-3 from PRAM
    (void) vtss_phy_wait_for_micro_complete(port_no);
    ...
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8006 | (tmp1 << 4)));    // write the value back to cfg_buf[10] w/o post-incr.
    (void) vtss_phy_wait_for_micro_complete(port_no);
...
static vtss_rc vtss_phy_atom12_patch_setttings_get_private(vtss_port_no_t port_no, u8 *mcb_bus, u8 *cfg_buf, u8 *stat_buf) {
    u16 idx;
    u16 reg18g;
    u8 slave_num = 0;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    // fetch MCB data
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8003 | (slave_num << 8) | (*mcb_bus << 4)));     // Read MCB macro into PRAM
    (void) vtss_phy_wait_for_micro_complete(port_no);
    
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
    VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0xd7cf));     // set mem_addr to cfg_buf (0x47cf)
    (void) vtss_phy_wait_for_micro_complete(port_no);

    // read out cfg_buf
    for (idx=0; idx<MAX_CFG_BUF_SIZE; idx++) {  // MAX_CFG_BUF_SIZE is 36
        VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x9007));     // read the value of cfg_buf[idx] w/ post-incr.
        (void) vtss_phy_wait_for_micro_complete(port_no);
    ...
    for (idx=0; idx<MAX_STAT_BUF_SIZE; idx++) {  // MAX_STAT_BUF_SIZE is 6
        VTSS_RC(vtss_phy_wr(port_no, 31, 0x0010));     // Switch back to micro/GPIO register-page
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x9007));     // read the value of stat_buf[idx] w/ post-incr.
        (void) vtss_phy_wait_for_micro_complete(port_no);
...
static vtss_rc vtss_phy_cfg_ob_post0_private(vtss_port_no_t port_no, u8 val)
{
    u8 tmp;
    u16 reg18g = 0;
    vtss_rc rc = VTSS_RC_OK;
    ...
    switch (ps->family) {
    case VTSS_PHY_FAMILY_ATOM:
        VTSS_RC(vtss_phy_page_gpio(port_no));       // Switch to micro/GPIO register-page
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8113));  // Read MCB for 6G macro 1 into PRAM
        (void)  vtss_phy_wait_for_micro_complete(port_no);
        
        VTSS_RC(vtss_phy_page_gpio(port_no));       // Switch back to micro/GPIO register-page
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0xd7d8));  // set mem_addr to cfg_buf[9] (0x47d8)
        (void)(vtss_phy_wait_for_micro_complete(port_no));
        
        
        VTSS_RC(vtss_phy_page_gpio(port_no));       // Switch back to micro/GPIO register-page
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8007));  // read the value of cfg_buf[9] w/o post-incr.
        (void) vtss_phy_wait_for_micro_complete(port_no);
        ...
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x9006 | ((u16)tmp << 4)));
        (void) vtss_phy_wait_for_micro_complete(port_no);
        ...
        VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8006 | ((u16)tmp << 4)));
        (void) vtss_phy_wait_for_micro_complete(port_no);
...
/*lint -esym(459, mac_if_old) */
/*lint -esym(459, init_done) */
static BOOL mac_if_changed(vtss_port_no_t port_no, vtss_port_interface_t current_mac_if) {
    BOOL mac_if_chged;
    static vtss_port_interface_t mac_if_old[VTSS_PORTS];
    static BOOL init_done = FALSE;
    vtss_port_no_t i;

    // Initialize array the first time we come into this function.
    if (!init_done) {
        for (i = 0; i < VTSS_PORTS; i++) {
            mac_if_old[i] = VTSS_PORT_INTERFACE_NO_CONNECTION;
            init_done = TRUE;
        }
    }
    
    // Find out if interface has changed.
    if (mac_if_old[port_no] == current_mac_if) {
        mac_if_chged = FALSE;
    } else {
        mac_if_chged = TRUE;
    }

    mac_if_old[port_no] = current_mac_if; // Remember the last mac interface configured for this port. 
    
    VTSS_N("port:%u, mac_if_changed:%d, mac_if_old[port_no]:%d, current_mac_if:%d", port_no, mac_if_old[port_no] != current_mac_if, mac_if_old[port_no], current_mac_if); 

    return mac_if_chged;
}

static vtss_rc vtss_phy_mac_media_if_atom_setup(const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf)
{
    u16 reg23 = 0;
    u16 reg19g = 0;
    ...
    if (vtss_state->phy_state[port_no].family == VTSS_PHY_FAMILY_ATOM) {
        ...
        if (mac_if_changed(port_no, conf->mac_if)) {
            if (conf->mac_if ==  VTSS_PORT_INTERFACE_QSGMII) {
                ...
                VTSS_RC(vtss_phy_atom12_cfg_ib_cterm_ena_private(port_no, vtss_state->init_conf.serdes.ib_cterm_ena, 1));
                
                // ob post called to override the ROM default of 2.
                VTSS_RC(vtss_phy_cfg_ob_post0_private(port_no, vtss_state->init_conf.serdes.qsgmii.ob_post0));
#endif
            } else {
                VTSS_RC(vtss_phy_page_gpio(port_no));
                //  Bugzero#48512, Changing speed at one port gives CRC errors at other ports.
                VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x80B0));// Seee TN1085
            } 
        ...
        VTSS_RC(vtss_phy_page_gpio(port_no));
        if (fi_100_mode) {
            VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE , 0x8091 | 0x0100 << (vtss_phy_chip_port(port_no) % 4)));
            VTSS_D("MCB : Setting 100Fx mode, 0x%X", 0x8091 | 0x0100 << (vtss_phy_chip_port(port_no) % 4));
        }
        
        if (fi_1000_mode) {
            VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x8081 | 0x0100 << (vtss_phy_chip_port(port_no) % 4)));
            VTSS_D("MCB : Setting 1000x mode, 0x%X", 0x8081 | 0x0100 << (vtss_phy_chip_port(port_no) % 4));
        }
    ...
    VTSS_D("reg23:0x%X, media_if:%s, mac if:%d, port_no:%u, chip_no:%u", 
           reg23, vtss_phy_media_if2txt(conf->media_if), conf->mac_if, port_no, vtss_phy_chip_port(port_no) % 12);
    VTSS_RC(vtss_phy_wr(port_no, 23, reg23));


    // Port must be reset in order to update the media setting for register 23
    VTSS_RC(vtss_phy_wr_masked(port_no, VTSS_PHY_PAGE_STANDARD, VTSS_F_PHY_MODE_CONTROL_SW_RESET, VTSS_F_PHY_MODE_CONTROL_SW_RESET));
    VTSS_MSLEEP(10);
    ...
    if (conf->mac_if == VTSS_PORT_INTERFACE_QSGMII) {
      VTSS_RC(vtss_phy_page_ext3(port_no));
      VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_MAC_SERDES_STATUS, 0x0004, 0x0004));
    }
...
BOOL vtss_phy_veriphy_running(vtss_port_no_t port_no, BOOL set, u8 running) {
    static BOOL init_done = FALSE;
    static BOOL  veriphy_running_bit_vector[VTSS_PORTS]; // Vector that indicates if veriphy is running for a port. The bit in the vector correspond to the port number
    vtss_port_no_t port_idx;
    
    // Initialize to not running the fir time this function is entered.
    if (!init_done) {
        for (port_idx = 0; port_idx < VTSS_PORTS; port_idx++) {
            veriphy_running_bit_vector[port_idx] = FALSE;
        }
        init_done = TRUE;
    }
    ...
    if (set) {
        if (running) {
            veriphy_running_bit_vector[port_no] = TRUE;  // Set the running bit in the vector
        } else {
            veriphy_running_bit_vector[port_no] = FALSE; // Clear the running bit in the vector
        }
    }
    VTSS_N("set =%d, running = %d, veriphy_running_bit_vector[%d] = 0x%X", set, running, port_no, veriphy_running_bit_vector[port_no]);

    
    // Return TRUE is any port has veriphy running 
    for (port_idx = 0; port_idx < VTSS_PORTS; port_idx++) {
        if (veriphy_running_bit_vector[port_idx]) {
            return TRUE;
        }
    }
...
vtss_rc atom_patch_suspend(vtss_port_no_t iport, BOOL suspend) {
    u16 word;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[iport];


    switch (vtss_state->phy_state[iport].family) {
    case VTSS_PHY_FAMILY_TESLA:
        if (ps->revision == VTSS_PHY_TESLA_REV_A) {
    ...
    case VTSS_PHY_FAMILY_LUTON26:
        ...
        VTSS_RC(vtss_phy_page_gpio(iport)); // Change to GPIO page

        VTSS_N("atom_patch_suspend = %d, port = %d", suspend, iport);
        ...
        if (vtss_phy_veriphy_running(iport, FALSE, FALSE)) {
            VTSS_N("atom_patch_suspend, port = %d", iport);
            VTSS_RC(PHY_WR_PAGE(iport, VTSS_PHY_MICRO_PAGE, 0x800F)); // Suspend the micro patch
            VTSS_RC(vtss_phy_wait_for_micro_complete(iport));
            VTSS_RC(vtss_phy_page_std(iport)); // Back to standard page
            return VTSS_RC_OK;
        } 
        
        if (suspend) {
            // Suspending 8051 patch
            if (vtss_state->phy_state[iport].family != VTSS_PHY_FAMILY_TESLA &&  ps->revision == VTSS_PHY_ATOM_REV_A) {
                // From JimB
                ...
                VTSS_RC(PHY_WR_PAGE(iport, VTSS_PHY_MICRO_PAGE, 0x9014)); // Suspend vga patch 
                VTSS_RC(vtss_phy_wait_for_micro_complete(iport)); 
            }  else {
                // See comment below.
                VTSS_RC(PHY_RD_PAGE(iport, VTSS_PHY_MICRO_PAGE, &word));
                if (!(word & 0x4000)) {
                    VTSS_RC(PHY_WR_PAGE(iport, VTSS_PHY_MICRO_PAGE, 0x8009)); // Suspend 8051 EEE patch 
                }
                VTSS_RC(vtss_phy_wait_for_micro_complete(iport)); 
            }
            VTSS_RC(vtss_phy_wait_for_micro_complete(iport));
            ...
            // Resuming 8051 patch
            if (vtss_state->phy_state[iport].family != VTSS_PHY_FAMILY_TESLA && ps->revision == VTSS_PHY_ATOM_REV_A) {
                VTSS_RC(PHY_WR_PAGE(iport, VTSS_PHY_MICRO_PAGE, 0x9004)); 
                VTSS_RC(vtss_phy_wait_for_micro_complete(iport)); 
                ...
                VTSS_RC(PHY_RD_PAGE(iport, VTSS_PHY_MICRO_PAGE, &word));
                if (word & 0x4000) {
                    VTSS_RC(PHY_WR_PAGE(iport, VTSS_PHY_MICRO_PAGE, 0x8009)); 
                }
                VTSS_RC(vtss_phy_wait_for_micro_complete(iport)); 
            }
            VTSS_RC(vtss_phy_wait_for_micro_complete(iport));
    ...
    VTSS_RC(vtss_phy_page_std(iport)); // Change to standard page
...
static vtss_rc vtss_phy_coma_mode_private(const vtss_port_no_t port_no, BOOL low) 
{
...
// Bit 12 is the bit to drive the COMA_MODE pin to when the output is enabled.    
    VTSS_RC(vtss_phy_page_gpio((port_no)));
    if (low) {
        VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_GPIO_CONTROL_2, 0x0000, 
        ...
        VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_GPIO_CONTROL_2, VTSS_F_PHY_GPIO_CONTROL_2_COMA_MODE_OUTPUT_ENABLE,
...
static vtss_rc vtss_phy_set_private_atom(const vtss_port_no_t port_no, const vtss_phy_mode_t mode)
{
    // Clear Cl40AutoCrossover in forced-speed mode, but set it in non-forced modes
    VTSS_RC(vtss_phy_wr(port_no, 31, 0x52b5));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7fa));  // issue token-ring read request
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, (mode == VTSS_PHY_MODE_FORCED)
                               ? 0x0000 : 0x1000, 0x1000));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87fa));  // issue token-ring write request

    VTSS_RC(vtss_phy_wr(port_no, 16, 0xaf82));
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x2, 0xf));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f82));

    VTSS_RC(vtss_phy_page_std(port_no));

    // Enable HP Auto-MDIX in forced-mode (by clearing disable bit)
    VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_BYPASS_CONTROL, 0x0000, VTSS_F_PHY_BYPASS_CONTROL_HP_AUTO_MDIX_AT_FORCE)); 
    return VTSS_RC_OK;
}
...
/************************************************************************/
static vtss_rc vtss_phy_mac_media_if_tesla_setup(const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf)
{
    ...
    switch (conf->mac_if) {
    case VTSS_PORT_INTERFACE_SGMII:
      VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_MAC_MODE_AND_FAST_LINK, 
   ...
   //  Bugzero#48512, Changing speed at one port gives CRC errors at other ports.
    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MODE_CONTROL, 0x4040));

    VTSS_RC(vtss_phy_page_gpio(port_no));
    if (mac_if_changed(port_no, conf->mac_if)) {
        if (conf->mac_if ==  VTSS_PORT_INTERFACE_QSGMII) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, VTSS_PHY_MICRO_PAGE, 0x80E0, 0xFFFF, 0x0)); // Configure SerDes macros for QSGMII MAC interface (See TN1080)
        } else {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, VTSS_PHY_MICRO_PAGE, 0x80F0, 0xFFFF, 0)); // Configure SerDes macros for 4xSGMII MAC interface (See TN1080)
        }
        VTSS_RC(vtss_phy_wait_for_micro_complete(port_no)); 
    }
    VTSS_MSLEEP(10);
    ...
    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MODE_CONTROL, 0x0000));
    // end Bugzero#48512
    ...
    if (conf->media_if != VTSS_PHY_MEDIA_IF_CU) {
        ...
        VTSS_RC(vtss_phy_page_gpio(port_no));        
        VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MICRO_PAGE, 0x80C1 | ((1 << (port_no % 4)) << 8) | micro_cmd_100fx));
        VTSS_RC(vtss_phy_wait_for_micro_complete(port_no)); 
        VTSS_MSLEEP(10);
    ...
    reg_val  = VTSS_F_PHY_EXTENDED_PHY_CONTROL_MEDIA_OPERATING_MODE(media_operating_mode) | 
      ...
      (mac_interface_mode ? VTSS_F_PHY_EXTENDED_PHY_CONTROL_MAC_INTERFACE_MODE : 0);
    

    reg_mask  = VTSS_M_PHY_EXTENDED_PHY_CONTROL_MEDIA_OPERATING_MODE | VTSS_F_PHY_EXTENDED_PHY_CONTROL_AMS_PREFERENCE | VTSS_F_PHY_EXTENDED_PHY_CONTROL_MAC_INTERFACE_MODE;
    
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_PHY_CONTROL, reg_val, reg_mask));
    ...
    if (!sync_calling_private || warm_start_reg_changed[port_no]) {
        ...
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_MODE_CONTROL, 0x8000, 0x8000));
        VTSS_MSLEEP(10);
    }
    return VTSS_RC_OK;
}
...
static vtss_rc vtss_phy_mac_media_if_quattro_setup (const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf)
{
	u16 reg = 0;
    switch (conf->mac_if) {
    case VTSS_PORT_INTERFACE_SGMII:
...
static vtss_rc vtss_phy_init_seq_quattro(vtss_phy_port_state_t *ps,
                                         vtss_port_no_t        port_no)
{
    u16 reg = 0;
    ...
    VTSS_RC(vtss_phy_page_test(port_no));
    VTSS_RC(vtss_phy_wr_masked(port_no, 2, 0x8000, 0x8000));
    VTSS_RC(vtss_phy_page_std(port_no));
    ...
    if (ps->revision < 3) {
        ...
        VTSS_RC(vtss_phy_wr(port_no, 17, 0x0671));
        ...
        VTSS_RC(vtss_phy_wr(port_no, 17, 0x492a));
        VTSS_RC(vtss_phy_wr(port_no, 16, 0x8fa4));
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 8, 0x0000, 0x0200));
        VTSS_RC(vtss_phy_page_std(port_no));
    ...
    VTSS_RC(vtss_phy_page_tr(port_no));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xaf82));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
    reg = (reg & 0xffef) | 0;           /*- Enable DFE in 100BT */
    VTSS_RC(vtss_phy_wr(port_no, 17, reg));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f82));
    VTSS_RC(vtss_phy_page_test(port_no));
    VTSS_RC(vtss_phy_wr_masked(port_no, 8, 0x0000, 0x0200));
    VTSS_RC(vtss_phy_page_std(port_no));
    ...
    VTSS_RC(vtss_phy_wr(port_no, 19 | VTSS_PHY_REG_EXTENDED,
                        ps->reset.media_if == VTSS_PHY_MEDIA_IF_CU ?
                        0x0002 : 0x0001));

    /* Disable down shifting */
    VTSS_RC(vtss_phy_wr_masked(port_no, 20 | VTSS_PHY_REG_EXTENDED, 0x0000, 0x0010));
...
static vtss_rc vtss_phy_mac_media_if_spyder_gto_setup (const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf, \
...
static vtss_rc vtss_phy_init_seq_spyder(vtss_phy_port_state_t *ps,
                                        vtss_port_no_t        port_no)
{
    u16 reg = 0;
    ...
    VTSS_RC(vtss_phy_page_ext(port_no));
    VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x8000, 0x8000));
    VTSS_RC(vtss_phy_page_std(port_no));
    ...
    if (ps->type.part_number == VTSS_PHY_TYPE_8538 || ps->type.part_number == VTSS_PHY_TYPE_8558) {
        if (ps->revision == 0) { /* VSC8538/58 Rev A */
            /* BZ 2020 */
            VTSS_RC(vtss_phy_page_test(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 27, 0x8000, 0x8000));
            VTSS_RC(vtss_phy_wr_masked(port_no, 19, 0x0300, 0x0f00));
            VTSS_RC(vtss_phy_page_std(port_no));

            /* BZ 2063 */
            VTSS_RC(vtss_phy_page_tr(port_no));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0xa60c));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0xa60c));
            VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
            if ((reg & (1<<3)) == 0) {
                /* !RxTrLock */
                VTSS_RC(vtss_phy_wr(port_no, 17, 0x0010));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x8604));
                VTSS_RC(vtss_phy_wr(port_no, 17, 0x00df));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x8600));
                VTSS_RC(vtss_phy_wr(port_no, 17, 0x00ff));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x8600));
            ...
            /* BZ 2069/2086 */
            VTSS_RC(vtss_phy_page_ext(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0000, 0x0001));
            VTSS_RC(vtss_phy_rd(port_no, 23, &reg)); /* PHY address at bit 11-15 */
            VTSS_RC(vtss_phy_page_std(port_no));
            ...
            if ((reg & (0x7 << 11)) == 0) {
                ...
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5000)); /*- Dummy write to start off */
                VTSS_RC(vtss_phy_wr(port_no, 11, 0xffff)); /*- Dummy write to start off */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5002)); /*- Dummy write to addr 16384= 02 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5040)); /*- Dummy write to addr 16385= 40 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x500C)); /*- Dummy write to addr 16386= 0C */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5002)); /*- Dummy write to addr 16387= 02 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5040)); /*- Dummy write to addr 16388= 40 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5021)); /*- Dummy write to addr 16389= 21 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5002)); /*- Dummy write to addr 16390= 02 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5040)); /*- Dummy write to addr 16391= 40 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5022)); /*- Dummy write to addr 16392= 22 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5002)); /*- Dummy write to addr 16393= 02 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5040)); /*- Dummy write to addr 16391= 40 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5023)); /*- Dummy write to addr 16392= 23 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50C2)); /*- Dummy write to addr 16396= C2 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50AD)); /*- Dummy write to addr 16397= AD */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50C2)); /*- Dummy write to addr 16396= C2 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CA)); /*- Dummy write to addr 16399= CA */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5075)); /*- Dummy write to addr 16400= 75 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CB)); /*- Dummy write to addr 16401= CB */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x509A)); /*- Dummy write to addr 16402= 9A */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5075)); /*- Dummy write to addr 16400= 75 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CA)); /*- Dummy write to addr 16399= CA */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5046)); /*- Dummy write to addr 16405= 46 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5085)); /*- Dummy write to addr 16406= 85 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CB)); /*- Dummy write to addr 16401= CB */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CD)); /*- Dummy write to addr 16408= CD */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5085)); /*- Dummy write to addr 16406= 85 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CA)); /*- Dummy write to addr 16399= CA */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CC)); /*- Dummy write to addr 16411= CC */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50D2)); /*- Dummy write to addr 16412= D2 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50CA)); /*- Dummy write to addr 16399= CA */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50D2)); /*- Dummy write to addr 16414= D2 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x50AD)); /*- Dummy write to addr 16415= AD */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5022)); /*- Dummy write to addr 16416= 22 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5022)); /*- Dummy write to addr 16416= 22 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5022)); /*- Dummy write to addr 16416= 22 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x5022)); /*- Dummy write to addr 16416= 22 */
                VTSS_RC(vtss_phy_wr(port_no, 12, 0x0000)); /*- Clear internal memory access */
                VTSS_RC(vtss_phy_wr(port_no,  0, 0x4099)); /*- Allow 8051 to run again, with patch enabled */
                VTSS_RC(vtss_phy_wr(port_no,  0, 0xc099)); /*- Allow 8051 to run again, with patch enabled */
                VTSS_RC(vtss_phy_page_std(port_no));
            }
            ...
            if (ps->type.part_number == VTSS_PHY_TYPE_8558) {
                VTSS_RC(vtss_phy_wr(port_no, 16, 0xa606));    /*- Request read, Media SerDes Control */
                VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
                reg = (reg & 0xfff8) | 5;                        /*- Optimize sample delay setting */
                VTSS_RC(vtss_phy_wr(port_no, 18, 0x0000));    /*- Is this OK? */
                VTSS_RC(vtss_phy_wr(port_no, 17, reg));
                VTSS_RC(vtss_phy_wr(port_no,16, 0x8606));    /* Write Media SerDes Control Word */
            }
            VTSS_RC(vtss_phy_wr(port_no, 16, 0xae06));       /*- Request read, MAC SerDes control */
            VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
            reg = (reg & 0xfff8) | 5;                           /*- Optimize sample delay setting */
            VTSS_RC(vtss_phy_wr(port_no, 18, 0x0000));       /*- Is this OK? */
            VTSS_RC(vtss_phy_wr(port_no, 17, reg));
            VTSS_RC(vtss_phy_wr(port_no,16, 0x8e06));        /* Write MAC SerDes Control Word */
            VTSS_RC(vtss_phy_page_std(port_no));
        }
        ...
        /* BZ 2221*/
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x1800, 0x1800));
        VTSS_RC(vtss_phy_page_std(port_no));

        /* BZ 2012 */
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x6000, 0x6000));  /* MDI Impedance setting = +2 */
        VTSS_RC(vtss_phy_page_std(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0xa002, 0xe00e));  /* 100-BT Amplitude/Slew Rate Control */
        ...
        if (ps->revision == 0) {
            ...
            /* 100-Base FX Clock Data Recovery Improvement */
            {
                ...
                VTSS_RC(vtss_phy_page_tr(port_no));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0xae0e));
                VTSS_RC(vtss_phy_rd(port_no, 18, &reg18));
                VTSS_RC(vtss_phy_rd(port_no, 17, &reg17));
                VTSS_RC(vtss_phy_wr(port_no, 18, reg18));
                VTSS_RC(vtss_phy_wr(port_no, 17, (reg17&0xffef)));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x8e0e));
                VTSS_RC(vtss_phy_page_std(port_no));
            }
        } /* ps->revision == 0 */
    }   /* (ps->type.part_number == VTSS_PHY_TYPE_8658) */
...
static vtss_rc vtss_phy_init_seq_cooper(vtss_phy_port_state_t *ps,
                                        vtss_port_no_t        port_no)
{
    u16 reg = 0;
    ...
    if (ps->revision == 0) {    /*- Rev A */
        /* BZ 2231 */
        VTSS_RC(vtss_phy_optimize_dsp(port_no));

        /* BZ 2253 */
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0010, 0x0010)); /*- Enab Enh mode for Reg17E chg */
        VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0007, 0x0007)); /*- sets the LEDs combine/separate */
        VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0000, 0x0010)); /*- sets back to simple mode */
        VTSS_RC(vtss_phy_page_std(port_no));

        /* BZ 2234 */
        VTSS_RC(vtss_phy_wr_masked(port_no, 9, 0x0000, 0x0400));    /*- Port Type #2234 */
        VTSS_RC(vtss_phy_wr_masked(port_no, 27, 0x0004, 0x0004));   /*- nVidia Req */
    }

    /* BZ 2644 - Improvement for marginal 10-Base T settings */
    VTSS_RC(vtss_phy_page_tr(port_no));

    VTSS_RC(vtss_phy_wr(port_no, 18, 0x9e));
    VTSS_RC(vtss_phy_wr(port_no, 17, 0xdd39));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87aa));

    VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7b4));
    VTSS_RC(vtss_phy_rd(port_no, 18, &reg));
    VTSS_RC(vtss_phy_wr(port_no, 18, reg));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
    reg = (reg & ~0x003f) | 0x003c;
    VTSS_RC(vtss_phy_wr(port_no, 17, reg));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87b4));

    VTSS_RC(vtss_phy_wr(port_no, 16, 0xa794));
    VTSS_RC(vtss_phy_rd(port_no, 18, &reg));
    VTSS_RC(vtss_phy_wr(port_no, 18, reg));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
    reg = (reg & ~0x003f) | 0x003e;
    VTSS_RC(vtss_phy_wr(port_no, 17, reg));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x8794));

    VTSS_RC(vtss_phy_wr(port_no, 18, 0xf7));
    VTSS_RC(vtss_phy_wr(port_no, 17, 0xbe36));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x879e));

    VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7a0));
    VTSS_RC(vtss_phy_rd(port_no, 18, &reg));
    VTSS_RC(vtss_phy_wr(port_no, 18, reg));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
    reg = (reg & ~0x003f) | 0x0034;
    VTSS_RC(vtss_phy_wr(port_no, 17, reg));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a0));

    VTSS_RC(vtss_phy_wr(port_no, 18, 0x3c));
    VTSS_RC(vtss_phy_wr(port_no, 17, 0xf3cf));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a2));

    VTSS_RC(vtss_phy_wr(port_no, 18, 0x3c));
    VTSS_RC(vtss_phy_wr(port_no, 17, 0xf3cf));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a4));

    VTSS_RC(vtss_phy_wr(port_no, 18, 0x3c));
    VTSS_RC(vtss_phy_wr(port_no, 17, 0xd287));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a6));

    VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7a8));
    VTSS_RC(vtss_phy_rd(port_no, 18, &reg));
    VTSS_RC(vtss_phy_wr(port_no, 18, reg));
    VTSS_RC(vtss_phy_rd(port_no, 17, &reg));
    reg = (reg & ~0x0fff) | 0x0125;
    VTSS_RC(vtss_phy_wr(port_no, 17, reg));
    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a8));
...
static vtss_rc vtss_phy_init_seq_luton(vtss_phy_port_state_t *ps,
                                       vtss_port_no_t        port_no)
{
    /* BZ 1801, 1973 */
    ...
    if (ps->family == VTSS_PHY_FAMILY_LUTON && ps->revision == 0) {
        ...
        VTSS_RC(vtss_phy_page_tr(port_no));
        VTSS_RC(vtss_phy_wr(port_no, 18, 0x0000));
        VTSS_RC(vtss_phy_wr(port_no, 17, 0x0689));
        ...
        VTSS_RC(vtss_phy_wr(port_no, 23, 0xFF80));
        VTSS_RC(vtss_phy_wr(port_no, 23, 0x0000));
        VTSS_RC(vtss_phy_page_std(port_no));
    }
    ...
    VTSS_RC(vtss_phy_optimize_receiver_init(port_no));
    VTSS_RC(vtss_phy_page_test(port_no));                       /*- Additional Init for parts that */
    VTSS_RC(vtss_phy_wr_masked(port_no, 8, 0x0200, 0x0200));  /*- lack Viterbi decoder */
    VTSS_RC(vtss_phy_page_tr(port_no));                         /*- to prevent DSP drift from */
    VTSS_RC(vtss_phy_wr(port_no, 16, 0xb68a));               /*- leading to bit errors */
    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0003, 0xff07));
    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x00a2, 0x00ff));
    ...
    if (ps->family == VTSS_PHY_FAMILY_LUTON24) {
        VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0xa002, 0xe00e));
    } else if (ps->family == VTSS_PHY_FAMILY_LUTON && ps->revision == 0) {
        /* BZ 2012 - Applies to VSC 7385/7388 Rev A */
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x4000, 0x6000)); /* Trim MDI Termination Impedance */
        VTSS_RC(vtss_phy_page_std(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0xa00e, 0xe00e)); /* Trim 100/1000 Tx amplitude & edge rate */
    } else {
        /* BZ 2043 - Applies to VSC 7385/7388 Rev B and later, VSC 7395/7398 all revs */
        VTSS_RC(vtss_phy_page_test(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 22, 0x0240, 0x0fc0)); /* Trim reference currents */
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x4000, 0x6000)); /* Trim termination impedance */
        VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x0030, 0x0038)); /* 1000-BT Edge Rate Ctrl */
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x2000, 0xe000)); /* Trim 1000 Base-T amplitude */
        VTSS_RC(vtss_phy_page_std(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x8002, 0xe00e)); /* 100-BT Edge Rate Ctrl */
    }
...
// In - port_no - PHY port number.
static vtss_rc vtss_phy_event_enable_private(const vtss_port_no_t  port_no)
{
    u16     mask = 0;
    vtss_rc rc = VTSS_RC_OK;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    case VTSS_PHY_FAMILY_LUTON26:
        if (ps->revision == VTSS_PHY_ATOM_REV_B) {
            if (vtss_state->phy_state[port_no].eee_conf.eee_mode == EEE_ENABLE) {
                ...
                rc = VTSS_PHY_WARM_WR(port_no, VTSS_PHY_INTERRUPT_MASK, 0);
            }
        }
        break;
    case VTSS_PHY_FAMILY_COBRA:
        ...
        if (conf->mode ==  VTSS_PHY_MODE_POWER_DOWN) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_INTERRUPT_MASK, 0x0, 0xFFFF));
            return VTSS_RC_OK;
        }
        break;
    default:
    ...
    if(!vtss_phy_can(port_no, VTSS_CAP_INT)) {
        return (VTSS_RC_ERROR);
    }
    ...
    VTSS_RC(vtss_phy_mmd_rd(port_no, 7, 60, &reg_val)); // Read current value of the register
    ...
    if (ev_mask & VTSS_PHY_LINK_FFAIL_EV) {
        ...
        if (reset_conf->media_if == VTSS_PHY_MEDIA_IF_FI_100FX || reset_conf->media_if == VTSS_PHY_MEDIA_IF_AMS_FI_100FX) {
            VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MODE_STATUS, &reg));
            ...
            if (!(reg & VTSS_F_PHY_STATUS_LINK_STATUS)) {
                ...
                rc = VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_INTERRUPT_MASK, 0, VTSS_F_PHY_INTERRUPT_MASK_FAST_LINK_MASK);
                }
    ...
    if (ev_mask & VTSS_PHY_LINK_AMS_EV) {
        if (ps->family == VTSS_PHY_FAMILY_COBRA) {
            VTSS_D("PHY Family %s doesn't support \"FDX state change\" interrupt", vtss_phy_family2txt(ps->family));
        } else {
            mask |= VTSS_F_PHY_INTERRUPT_MASK_AMS_MEDIA_CHANGE_MASK;
        }
    }
    
    rc = vtss_phy_page_std(port_no);
    if(mask) {
        mask |= VTSS_F_PHY_INTERRUPT_MASK_INT_MASK;         /* Add Interrupt Status */
    }
    VTSS_N("port_no:%d, ev_mask:0x%X, mask:0x%X", port_no, ev_mask, mask);
    rc = VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_INTERRUPT_MASK, mask, mask);
...
static vtss_rc vtss_phy_event_poll_private(const vtss_port_no_t  port_no,
                                           vtss_phy_event_t      *const events)
{
    ...
    *events = 0;
    ...
    if (!vtss_phy_can(port_no, VTSS_CAP_INT))
        return VTSS_RC_ERROR;
    rc = vtss_phy_page_std(port_no);
    rc = PHY_RD_PAGE(port_no, VTSS_PHY_INTERRUPT_STATUS, &pending); /* Pending is cleared by read */
    rc = PHY_RD_PAGE(port_no, VTSS_PHY_INTERRUPT_MASK, &mask);
...
// IN : port_no - Any port within the chip.
vtss_rc vtss_phy_post_reset_private(const vtss_port_no_t port_no)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    case VTSS_PHY_FAMILY_LUTON26:
    case VTSS_PHY_FAMILY_TESLA:
        VTSS_RC(vtss_phy_coma_mode_private(port_no, TRUE));
        break;
    default:
...
static vtss_rc vtss_phy_mac_media_if_enzo_setup(const vtss_port_no_t port_no, const vtss_phy_reset_conf_t *const conf)
{
    u16 reg = 0;
...
static vtss_rc vtss_phy_init_seq_enzo(vtss_phy_port_state_t *ps,
                                      vtss_port_no_t        port_no)
{
    u16 reg = 0;
    ...
    if ((reg & (0x3 << 11)) == 0) {
        ...
        if (ps->revision == 0) {    /*- Rev A */
            ...
            VTSS_RC(vtss_phy_page_ext(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 19, 0x0800, 0x0800));

            /* BZ 2637 */
            /* 100/1000 Base-T amplitude compenstation */
            VTSS_RC(vtss_phy_page_test(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x0003, 0x0007)); // ->(val & ~mask) | (*value & mask) i.e (value at 24 & 0xfff8) | (0x0003 & 0x0007)
            VTSS_RC(vtss_phy_page_std(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x0004, 0x000e)); // ->(val & ~mask) | (*value & mask) i.e(value at 24 & 0xfff1) | (0x0004 & 0x000e)

            /* BZ 2639 / BZ 2642 */
            /* Improve robustness of 10Base-T performance */
            VTSS_RC(vtss_phy_page_tr(port_no));
            VTSS_RC(vtss_phy_wr(port_no, 18, 0x0));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x3f));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x8794));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0xf7));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xadb4));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x879e));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x0));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x32));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a0));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x41));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x410));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a2));
            ...
            VTSS_RC(vtss_phy_wr(port_no, 18, 0x41));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x284));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a6));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x92));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xbcb8));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87a8));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x3));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xcfbf));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87aa));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x49));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x2451));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87ac));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x1));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x1410));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87c0));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x10));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xb498));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87e8));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x71));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xe7dd));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87ea));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x69));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x6512));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87ec));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x49));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x2451));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87ee));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x45));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x410));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87f0));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x41));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x410));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87f2));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x0));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x10));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87f4));

            VTSS_RC(vtss_phy_page_test(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 9, 0x0040, 0x0040));
            VTSS_RC(vtss_phy_wr_masked(port_no, 22, 0x0010, 0x0010));
            VTSS_RC(vtss_phy_page_std(port_no));

            /* BZ 2643 */
            /* Performance optimization - 100Base-TX/1000Base-T slave */
            VTSS_RC(vtss_phy_page_test(port_no));
            VTSS_RC(vtss_phy_wr_masked(port_no, 0, 0x0060, 0x00e0));
            VTSS_RC(vtss_phy_page_tr(port_no));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x12));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x480a));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f82));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x0));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x422));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f86));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x3c));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0x3800));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x8f8a));

            VTSS_RC(vtss_phy_wr(port_no, 18, 0x8));
            VTSS_RC(vtss_phy_wr(port_no, 17, 0xe33f));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x83ae));
            VTSS_RC(vtss_phy_page_std(port_no));
        }   /*- Rev A */
...
vtss_rc vtss_phy_init_conf_set(void)
{
    ...
    if (conf->restart_info_src == VTSS_RESTART_INFO_SRC_CU_PHY &&
        vtss_phy_detect(port_no) == VTSS_RC_OK &&
        vtss_state->phy_state[port_no].family != VTSS_PHY_FAMILY_NONE) {
        /* Get restart information from Vitesse 1G PHY */
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(vtss_phy_rd(port_no, 29, &reg)); /* 16 MSB at register 29E1 */
        value = reg;
        value = (value << 16);
        VTSS_RC(vtss_phy_rd(port_no, 30, &reg)); /* 16 LSB at register 30E1 */
        value += reg;
        VTSS_RC(vtss_phy_page_std(port_no));
...
vtss_rc vtss_phy_restart_conf_set(void)
{
    ...
    if (conf->restart_info_src == VTSS_RESTART_INFO_SRC_CU_PHY &&
        ...
        VTSS_RC(vtss_phy_wr(port_no, 29, reg)); /* 16 MSB at register 29E1 */
        reg = (value & 0xffff);
        VTSS_RC(vtss_phy_wr(port_no, 30, reg)); /* 16 LSB at register 30E1 */
        VTSS_RC(vtss_phy_page_std(port_no));
    }
...
vtss_rc vtss_phy_reset_private(const vtss_port_no_t port_no)
{
    ...
    case VTSS_PHY_FAMILY_BLAZER:
        ...
        if (ps->revision < 4 || ps->revision> 6) {
            VTSS_E("port_no %u, unsupported Blazer revision: %u", port_no, ps->revision);
            return VTSS_RC_ERROR;
        }
        break;
    case VTSS_PHY_FAMILY_COBRA:
        VTSS_RC(vtss_phy_mac_media_if_cobra_setup(port_no, conf));
        break;
    case VTSS_PHY_FAMILY_QUATTRO:
        VTSS_RC(vtss_phy_mac_media_if_quattro_setup (port_no, conf));
        break;
    case VTSS_PHY_FAMILY_SPYDER:
        if (ps->revision == 0) {
            /* BZ 2027 */
            VTSS_RC(vtss_phy_page_gpio(port_no));
            VTSS_RC(vtss_phy_wr(port_no, 0, 0x4c19));
            VTSS_RC(vtss_phy_micro_assert_reset(port_no));
            VTSS_RC(vtss_phy_page_std(port_no));
        }
        VTSS_RC(vtss_phy_mac_media_if_spyder_gto_setup(port_no, conf, ps->type));
        break;
    case VTSS_PHY_FAMILY_ENZO:
        VTSS_RC(vtss_phy_mac_media_if_enzo_setup(port_no, conf));
        break;
    case VTSS_PHY_FAMILY_COOPER:
    {
        ...
        VTSS_RC(vtss_phy_page_ext(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no, 28, reg, 0xf000));
        VTSS_RC(vtss_phy_page_std(port_no));
        break;
    }
    case VTSS_PHY_FAMILY_LUTON:
    ...
    case VTSS_PHY_FAMILY_LUTON26:
        ...
        VTSS_RC(vtss_phy_wr(port_no, 31, 0x2a30));
        VTSS_RC(vtss_phy_wr_masked(port_no, 28, 0x8 , 0x8));
        VTSS_RC(vtss_phy_page_std(port_no));
        // Fall through on purpose
    case VTSS_PHY_FAMILY_ATOM:
        VTSS_RC(vtss_phy_mac_media_if_atom_setup(port_no, conf));
        break;

    case VTSS_PHY_FAMILY_TESLA:
        VTSS_RC(vtss_phy_mac_media_if_tesla_setup(port_no, conf));
        break;
    ...
    default:
        if (ps->conf_none) {
            switch (conf->mac_if) {
            case VTSS_PORT_INTERFACE_RGMII:
                VTSS_RC(vtss_phy_wr(port_no, 27, 0x848B));
                VTSS_RC(vtss_phy_wr(port_no, 20, 0x0CE2));
                break;
            case VTSS_PORT_INTERFACE_RTBI:
                VTSS_RC(vtss_phy_wr(port_no, 27, 0x8489));
                VTSS_RC(vtss_phy_wr(port_no, 20, 0x0CE2));
                break;
            default:
                break;
    ...
    case VTSS_PHY_FAMILY_BLAZER:
        VTSS_RC(vtss_phy_init_seq_blazer(ps, port_no));
        ...
        reg |= (((ps->revision == 6 ? 1 : 0)<<9) | /* 2.5V */
                ...
                (0x14<<0)); /* Reserved */
        VTSS_RC(vtss_phy_wr(port_no, 23, reg));
        VTSS_RC(vtss_phy_wr(port_no, 22, 0x3000));
        break;

    case VTSS_PHY_FAMILY_COBRA:
        VTSS_RC(vtss_phy_init_seq_cobra(ps, port_no));
        break;

    case VTSS_PHY_FAMILY_QUATTRO:
        VTSS_RC(vtss_phy_init_seq_quattro(ps, port_no));
        break;

    case VTSS_PHY_FAMILY_SPYDER:
        VTSS_RC(vtss_phy_init_seq_spyder(ps, port_no));
        break;

    case VTSS_PHY_FAMILY_COOPER:
        VTSS_RC(vtss_phy_init_seq_cooper(ps, port_no));
        break;

    case VTSS_PHY_FAMILY_LUTON:
    case VTSS_PHY_FAMILY_LUTON_E:
    case VTSS_PHY_FAMILY_LUTON24:
        VTSS_RC(vtss_phy_init_seq_luton(ps, port_no));
        break;

    case VTSS_PHY_FAMILY_LUTON26:
        VTSS_RC(vtss_phy_init_seq_atom(ps, port_no, TRUE));

#if defined(VTSS_FEATURE_EEE)
        vtss_state->phy_state[port_no].eee_conf.eee_mode = EEE_REG_UPDATE;
        VTSS_RC(vtss_phy_eee_ena_private(port_no));
#endif
        break;
    case VTSS_PHY_FAMILY_ATOM:
        VTSS_RC(vtss_phy_init_seq_atom(ps, port_no, FALSE));
    ...
    case VTSS_PHY_FAMILY_TESLA:
        ...
        break;
    case VTSS_PHY_FAMILY_ENZO:
        VTSS_RC(vtss_phy_init_seq_enzo(ps, port_no));
        break;
...
static vtss_rc vtss_phy_conf_set_private(const vtss_port_no_t port_no)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    vtss_phy_conf_t       *conf = &ps->setup;
    u16                   reg, revision;
    ...
    case VTSS_PHY_MODE_ANEG:
        ...
        reg = (((conf->aneg.asymmetric_pause ? 1 : 0)<<11) |
               ...
               (1<<0));
        VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_DEVICE_AUTONEG_ADVERTISEMENT, reg));

        /* Setup register 9 */
        reg = ((conf->aneg.speed_1g_fdx ? 1 : 0)<<9);
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_1000BASE_T_CONTROL, reg, 0x0200));
        ...
        switch (family) {
            case VTSS_PHY_FAMILY_COOPER:
                ...
                VTSS_RC(vtss_phy_page_tr(port_no));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0xA7F8));
                VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0000, 0x0018));
                VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0, 0));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x87F8));
                VTSS_RC(vtss_phy_page_std(port_no));
                break;
	     ...
	     case VTSS_PHY_FAMILY_ATOM:
      	     case VTSS_PHY_FAMILY_LUTON26:
             case VTSS_PHY_FAMILY_TESLA:
	         VTSS_RC(vtss_phy_set_private_atom(port_no, conf->mode));
		 break;
        ...
        if (!sync_calling_private || warm_start_reg_changed[port_no]) {
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MODE_CONTROL,
                                     VTSS_F_PHY_MODE_CONTROL_AUTO_NEG_ENA | VTSS_F_PHY_MODE_CONTROL_RESTART_AUTO_NEG));
    ...
    case VTSS_PHY_MODE_FORCED:
        ...
        VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MODE_CONTROL, reg));
        ...
        if (conf->forced.speed != VTSS_SPEED_1G) {
            ...
            case VTSS_PHY_FAMILY_QUATTRO:
                ...
                VTSS_RC(vtss_phy_wr(port_no, 17, 0x2803));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x87fa));
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no,  8, 0x0000, 0x0200));
                VTSS_RC(vtss_phy_page_std(port_no));
            ...
            case VTSS_PHY_FAMILY_LUTON:
                if (revision == 0) {
                    ...
                    VTSS_RC(vtss_phy_page_std(port_no));
                    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x0000, 0x0080));
                }
                break;
            case VTSS_PHY_FAMILY_SPYDER:
            case VTSS_PHY_FAMILY_LUTON_E:
            ...
            case VTSS_PHY_FAMILY_LUTON24:
                ...
                VTSS_RC(vtss_phy_wr_masked(port_no, 8, 0x0200, 0x0200));
                VTSS_RC(vtss_phy_page_tr(port_no));
                VTSS_RC(vtss_phy_wr(port_no, 18, 0x0092));
                VTSS_RC(vtss_phy_wr(port_no, 17, 0x2803));
                VTSS_RC(vtss_phy_wr(port_no, 16, 0x87FA));
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 8, 0x0000, 0x0200));
            ...
            case VTSS_PHY_FAMILY_COOPER:
                ...
                if (conf->forced.speed == VTSS_SPEED_100M) {
                    VTSS_RC(vtss_phy_page_tr(port_no));
                    VTSS_RC(vtss_phy_wr(port_no, 16, 0xA7F8));
                    VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0018, 0x0018));
                    VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0, 0));
                    VTSS_RC(vtss_phy_wr(port_no, 16, 0x87F8));
                    VTSS_RC(vtss_phy_page_std(port_no));
                }
    ...
    case VTSS_PHY_MODE_POWER_DOWN:
        /* Setup register 0 */
        VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_MODE_CONTROL, 1<<11));
        if (family == VTSS_PHY_FAMILY_QUATTRO) {
            /* Briefly power up-down to make sure the link status bit clears */
            VTSS_RC(vtss_phy_wr(port_no, 0, 0<<11));
            VTSS_RC(vtss_phy_wr(port_no, 0, 1<<11));
        }
    ...
    if (family == VTSS_PHY_FAMILY_COBRA) {
        VTSS_N("Port:%d  Bugzilla#7489 - Interrupt must be disable when powered down.", port_no);
        VTSS_RC(vtss_phy_event_enable_private(port_no));
    }

    VTSS_RC(vtss_phy_mdi_setup(port_no));
...
static vtss_rc squelch_workaround_private(vtss_port_no_t port_no, BOOL enable) {
    u16 reg_value;
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    
    switch (ps->family) {
    case VTSS_PHY_FAMILY_TESLA:
        if (ps->revision == VTSS_PHY_TESLA_REV_A || ps->revision == VTSS_PHY_TESLA_REV_B) {
            //Enable workaround by writing command to 18G
            //Command 18G instruction
            VTSS_RC(vtss_phy_page_gpio(port_no));	// switch to micro page (glabal to all 12 PHYs)
            if (enable) {
                VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x801d));
            } else {
                VTSS_RC(PHY_WR_PAGE(port_no, VTSS_PHY_MICRO_PAGE, 0x800d));
            }
            ...
            VTSS_RC(vtss_phy_page_gpio(port_no));	// switch to micro page (glabal to all 12 PHYs)
            VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MICRO_PAGE, &reg_value));
            if (reg_value & (1 << 14)) {
                VTSS_E("squelch workaround not loaded");
                return VTSS_RC_ERROR;
            }
            VTSS_D("squelch_workaround setup reg_value = 0x%X", reg_value);
        }
        break;
    default:
        break;
...
static void vtss_phy_link_speeed_decode_status(vtss_port_no_t port_no, u16 lp_1000base_t_status_reg, u16 mii_status_reg, const vtss_phy_conf_t phy_setup, vtss_port_status_t *const status) {
    ...
    VTSS_N("port:%d, lp_1000base_t_status_reg:0x%X, mii_status_reg:0x%X, speed:%d, ps->setup.aneg.speed_1g_fdx:%d", 
           port_no, lp_1000base_t_status_reg, mii_status_reg, status->speed, ps->setup.aneg.speed_1g_fdx);
}
...
void vtss_phy_reg_decode_status(vtss_port_no_t port_no, u16 lp_auto_neg_advertisment_reg, u16 lp_1000base_t_status_reg, u16 mii_status_reg, const vtss_phy_conf_t phy_setup, vtss_port_status_t *const status) {
    ...
    // Link up/down
    status->link = (mii_status_reg & (1<<2) ? 1 : 0);
    status->link_down = (mii_status_reg & (1<<2) ? 0 : 1);
    status->remote_fault = (mii_status_reg & (1<<4) ? 1 : 0);
                                    ...
                                    vtss_port_status_t   *const status)
{
    ...
    VTSS_RC(vtss_phy_rd(port_no, 1, &reg));

    // Check if link has been down due to port has been reset
    if (link_down_due_to_port_reset[port_no]) {
        status->link_down = TRUE; // Link due the port reset
        VTSS_D("Setting link_down_due_to_port_reset, port_no = %d", port_no);
        link_down_due_to_port_reset[port_no] = FALSE; // OK - Link down is now detected by the Application. 
    } else {
        status->link_down = (reg & (1<<2) ? 0 : 1);
    }
    status->remote_fault = (reg & (1<<4) ? 1 : 0);
    if (status->link_down) {
    ...
    if (status->link) {
        ...
        case VTSS_PHY_MODE_ANEG:
            if ((reg & (1<<5))==0) {
                 VTSS_RC(vtss_phy_rd(port_no, 28, &reg));
		 ...
		 break;
            ...
            VTSS_RC(vtss_phy_rd(port_no, 5, &reg));
            ...
            if (ps->family == VTSS_PHY_FAMILY_NONE) {
                ...
                VTSS_RC(vtss_phy_rd(port_no, 10, &reg10));
    ...
    if ((!status->link || status->link_down) && ps->status.link) {
        ...
        case VTSS_PHY_FAMILY_COBRA:
            /* BZ ? */
            VTSS_RC(vtss_phy_rd(port_no, 0, &reg));
            if ((reg & (1<<11))==0) {
                /* Briefly power down PHY to avoid problem sometimes seen when
                   changing speed on SmartBit */
                VTSS_RC(vtss_phy_wr(port_no, 0, reg | (1<<11)));
                status->link = 0;
                VTSS_MSLEEP(100);
                VTSS_RC(vtss_phy_wr(port_no, 0, reg));
            }
        ...
        case VTSS_PHY_FAMILY_LUTON24:
            ...
            if (ps->family != VTSS_PHY_FAMILY_QUATTRO) {
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear MDI Impedance Force Bit - Safety  */
                VTSS_RC(vtss_phy_page_std(port_no));
            }
            ...
            if (ps->family == VTSS_PHY_FAMILY_LUTON_E) {
                ...
                VTSS_RC(vtss_phy_wr_masked(port_no, 22, 0x0800, 0x0c00));
            }
            break;
        ...
        case VTSS_PHY_FAMILY_LUTON26:
            ...
            if (revision == VTSS_PHY_ATOM_REV_B || revision == VTSS_PHY_ATOM_REV_A) {
                VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MODE_CONTROL, &reg));
                if (!(reg & VTSS_F_PHY_MODE_CONTROL_POWER_DOWN)) {
                    ...
                    if (conf->media_if == VTSS_PHY_MEDIA_IF_CU && ps->setup.mode == VTSS_PHY_MODE_ANEG) {
                        ...
                        VTSS_RC(PHY_WR_MASKED_PAGE(port_no, VTSS_PHY_MODE_CONTROL, 0, VTSS_F_PHY_MODE_CONTROL_POWER_DOWN));
                    }
                }
            }
	    // Fall through - On purpose.
		...
		if (reg & VTSS_F_PHY_STATUS_LINK_STATUS) {
		  VTSS_I("Link is up, port:%d", port_no);
			...
			VTSS_RC(PHY_WR_MASKED_PAGE(port_no,VTSS_PHY_EXTENDED_CONTROL_AND_STATUS, 
			...
			VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MODE_STATUS, &reg)); // Clear the status register, to get rid of the false link up, due to that we have just forced the link up/down.
   	    ...
   	    // Work-around solution for bugzilla#6859 from James McIntosh
            ...
            //to ~511 ms.
            VTSS_D("Work-around for bugzilla#6859 - Link up long when going to 100Mbit forced mode from Auto or 10Mbit forced mode. Port:%d", port_no); 
            VTSS_RC(vtss_phy_wr(port_no, 31, 0x52b5));
            VTSS_RC(vtss_phy_wr(port_no, 16, 0xa7fa));  // issue token-ring read request
            
            // Register 17 is bits 15:0
            // Register 18 is bits 24:16
            VTSS_RC(vtss_phy_wr_masked(port_no, 17, 0x0, 0xE000));  // set bit 22 to 1 and bits 21:13 to
            VTSS_RC(vtss_phy_wr_masked(port_no, 18, 0x50, 0x07F));  // 0x080; leave other bits the same
            VTSS_RC(vtss_phy_wr(port_no, 16, 0x87fa));  // issue token-ring write request
            VTSS_RC(vtss_phy_page_std(port_no));
    ...
    if (status->link && (!ps->status.link || status->link_down)) {
        ...
        case VTSS_PHY_FAMILY_SPYDER:
            if (status->speed == VTSS_SPEED_1G) {
                ...
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x2000, 0x6000));  /*- MDI Impedance offset -1 */
                VTSS_RC(vtss_phy_page_std(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0xa004, 0xe00e));   /*- 10/100 Edge Rate/Amplitude Control */
            } else if (status->speed == VTSS_SPEED_10M) {
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x6000, 0x6000));  /*- MDI Impedance Offset = +1 */
                VTSS_RC(vtss_phy_page_std(port_no));
            }
            break;
        ...
        case VTSS_PHY_FAMILY_LUTON:
            if (status->speed == VTSS_SPEED_1G) {
                ...
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x6000));  /*- MDI Impedance offset default */
                VTSS_RC(vtss_phy_page_std(port_no));
                /*- MII 24 can be 0xa000(0xe000), if customer has 100-BT rise time issues */
                VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x8002, 0xe00e));   /*- 10/100 Edge Rate/Amplitude Control */
            } else if (status->speed == VTSS_SPEED_10M) {
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x2000, 0x6000));  /*- MDI Impedance Offset = -1 */
        ...
        case VTSS_PHY_FAMILY_LUTON_E:
            if (status->speed == VTSS_SPEED_1G) {
                VTSS_RC(vtss_phy_optimize_receiver_reconfig(port_no));
                VTSS_RC(vtss_phy_page_ext(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x2000, 0xe000));  /*- 1000-BT DAC Amplitude control = +2% */
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0x0030, 0x0038));  /*- 1000-BT Edge Rate Control */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x4000, 0x6000));  /*- MDI Impedance Offset = +2 */
                VTSS_RC(vtss_phy_page_std(port_no));
            } else if (status->speed == VTSS_SPEED_100M) {
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                ...
                VTSS_RC(vtss_phy_wr_masked(port_no, 22, 0x0000, 0x0c00));
            }
            break;
        ...
        case VTSS_PHY_FAMILY_LUTON24:
            if (status->speed == VTSS_SPEED_1G) {
                ...
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x6000));   /*- MDI Impedance offset default */
                VTSS_RC(vtss_phy_page_std(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 24, 0xa002, 0xe00e));    /*- 10/100 Edge Rate/Amplitude Control */
            } else if (status->speed == VTSS_SPEED_10M) {
                VTSS_RC(vtss_phy_page_test(port_no));
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x0000, 0x1000));  /*- Clear Force Bit */
                VTSS_RC(vtss_phy_wr_masked(port_no, 20, 0x2000, 0x6000));   /*- MDI Impedance Offset = -1 */
    ...
    /* Save status */
    ps->status = *status;
    VTSS_N("status->link:%d, port:%d, status->link_down:%d", status->link, port_no, status->link_down);
    return VTSS_RC_OK;
}



static vtss_rc vtss_phy_power_conf_set_private(const vtss_port_no_t port_no)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    ...
    case VTSS_PHY_POWER_ACTIPHY:
        ...
        if (ps->family == VTSS_PHY_FAMILY_COOPER) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, COOPER_VTSS_PHY_AUXILIARY_CONTROL_AND_STATUS, 0x0020, 0x0020));
        } else {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_AUXILIARY_CONTROL_AND_STATUS, 
    ...
    case VTSS_PHY_POWER_DYNAMIC:
        ...
        VTSS_RC(vtss_phy_page_std(port_no));
        if (ps->family == VTSS_PHY_FAMILY_COOPER) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, COOPER_VTSS_PHY_AUXILIARY_CONTROL_AND_STATUS, 0x0000, 0x0020));
        } else {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_AUXILIARY_CONTROL_AND_STATUS, 0x0000, 
                                            VTSS_F_PHY_AUXILIARY_CONTROL_AND_STATUS_ACTIPHY_MODE_ENABLE));
        }
...
static vtss_rc vtss_phy_conf_1g_set_private(const vtss_port_no_t port_no)
{
    vtss_phy_conf_1g_t *conf = &vtss_state->phy_state[port_no].conf_1g;
    u16                reg, reg_cont=0, reg_status=0;

    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(vtss_phy_rd(port_no, 9, &reg_cont));
    VTSS_RC(vtss_phy_rd(port_no, 10, &reg_status));

    reg  = ((conf->master.cfg ? 1 : 0) << 12) | ((conf->master.val ? 1 : 0) << 11);

    /* Commit 1000-BT Control values */
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_1000BASE_T_CONTROL, reg, 0x1800));
    ...
    if ((reg_cont ^ reg) & 0x1800) {
        if ((reg & 0x1000) || (reg_status & 0x8000)) { /* Only start Auto-neg if Manuel is selected or master/slave resolution failed */
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_MODE_CONTROL, 0x0200, 0x0200));
        }
    }
...
static vtss_rc vtss_phy_status_1g_get_private(const vtss_port_no_t  port_no,
                                              vtss_phy_status_1g_t  *const status)
{
    u16 reg = 0;

    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(vtss_phy_rd(port_no, 10, &reg));
...
static vtss_rc vtss_phy_clock_conf_set_private(const vtss_port_no_t         port_no,
                                               const vtss_phy_recov_clk_t   clock_port)
{
    ...
    if (clock_port == VTSS_PHY_RECOV_CLK1) {
        VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_RECOVERED_CLOCK_0_CONTROL, &reg_val));
    } else {
        VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_RECOVERED_CLOCK_1_CONTROL, &reg_val));
    }
    ...
    case VTSS_PHY_FAMILY_ATOM:
    case VTSS_PHY_FAMILY_LUTON26:
        reg_val |= ((((vtss_phy_chip_port(source) % 12) &0xF) <<11) | (conf.freq << 8) | (conf.squelch << 4));
        break;
    case VTSS_PHY_FAMILY_TESLA:
        reg_val |= ((((vtss_phy_chip_port(source) % 4) &0xF) <<11) | (conf.freq << 8) | (conf.squelch << 4));
        break;
    default:
        break;
    }
    ...
    if (clock_port == VTSS_PHY_RECOV_CLK1) {
        if (vtss_state->phy_state[port_no].family == VTSS_PHY_FAMILY_ENZO) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, VTSS_PHY_RECOVERED_CLOCK_0_CONTROL, reg_val, 0xFFFF, 0x7FFF));     /* On Enzo it is not possible to set bit 15 - do not check */
        } else {
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_RECOVERED_CLOCK_0_CONTROL, reg_val));
        }
    } else {
        if (vtss_state->phy_state[port_no].family == VTSS_PHY_FAMILY_ENZO) {
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED_CHK_MASK(port_no, VTSS_PHY_RECOVERED_CLOCK_1_CONTROL, reg_val, 0xFFFF, 0x7FFF));     /* On Enzo it is not possible to set bit 15 - do not check */
        } else {
            VTSS_RC(VTSS_PHY_WARM_WR(port_no, VTSS_PHY_RECOVERED_CLOCK_1_CONTROL, reg_val));
        }
    }

    VTSS_RC(vtss_phy_page_std(port_no));

    VTSS_RC(squelch_workaround_private(port_no, TRUE));
    return VTSS_RC_OK;
}
...
//
// Return :  VTSS_RC_OK is setup was preformed correct else VTSS_RC_ERROR
static vtss_rc vtss_phy_led_mode_set_private(const vtss_port_no_t port_no) {
  u16 reg19e_mode = 0;
  u16 reg29_mode = 0;
  ...
  case LED3:
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_MODE_SELECT, reg29_mode << 12, 0xF000));
    VTSS_RC(vtss_phy_page_ext(port_no));  
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_MODE_CONTROL, reg19e_mode << 15, 0x8000));
    break; 
  case LED2:
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_MODE_SELECT, reg29_mode << 8, 0x0F00));
    VTSS_RC(vtss_phy_page_ext(port_no));  
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_MODE_CONTROL, reg19e_mode << 14, 0x4000));
    break; 
  case LED1:
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_MODE_SELECT, reg29_mode << 4, 0x00F0));
    VTSS_RC(vtss_phy_page_ext(port_no));  
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_MODE_CONTROL, reg19e_mode << 13, 0x2000));
    break; 
  case LED0:
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_MODE_SELECT, reg29_mode, 0x000F));
    VTSS_RC(vtss_phy_page_ext(port_no));  
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_MODE_CONTROL, reg19e_mode << 12, 0x1000));
    break; 
  }
...
//
// Return :  VTSS_RC_OK is setup was preformed correct else VTSS_RC_ERROR
static vtss_rc vtss_phy_led_intensity_set_private(const vtss_port_no_t port_no)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    vtss_phy_family_t     family;
    ...
    case VTSS_PHY_FAMILY_SPYDER:
        ...
        if (vtss_state->led_intensity == 0) {
            // Turn LED off
            VTSS_RC(vtss_phy_wr_masked(port_no, 0x1D, 0xEEEE, 0xFFFF)); 
        } else {
            // Turn LED on
            VTSS_RC(vtss_phy_wr_masked(port_no, 0x1D, 0xEE64, 0xFFFF)); 
        }
    ...
    case VTSS_PHY_FAMILY_TESLA:
        ...
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_ENHANCED_LED_CONTROL, (vtss_state->led_intensity * 2) << 8, 0xFF00)); 
        break;
...
//
// Return :  VTSS_RC_OK is setup was preformed correct else VTSS_RC_ERROR
static vtss_rc vtss_phy_enhanced_led_control_init_private(const vtss_port_no_t port_no)
{
    vtss_phy_port_state_t *ps = &vtss_state->phy_state[port_no];
    vtss_phy_family_t     family;
    ...
    case VTSS_PHY_FAMILY_TESLA:
        VTSS_RC(vtss_phy_page_gpio(port_no));
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_ENHANCED_LED_CONTROL, vtss_state->enhanced_led_control.ser_led_output_2 << 7,0x80)); // Datasheet section 4.6.15, register 25G
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_ENHANCED_LED_CONTROL, vtss_state->enhanced_led_control.ser_led_output_1 << 6,0x40)); // Datasheet section 4.6.15, register 25G
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_ENHANCED_LED_CONTROL, vtss_state->enhanced_led_control.ser_led_frame_rate << 3, 0x38)); // Datasheet section 4.6.15, register 25G
        VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_ENHANCED_LED_CONTROL, vtss_state->enhanced_led_control.ser_led_select << 1, 0x06)); // Datasheet section 4.6.15, register 25G
        ...
        if (!vtss_state->enhanced_led_control.ser_led_output_2 && !vtss_state->enhanced_led_control.ser_led_output_1) {
            ...
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_BEHAVIOR, VTSS_F_PHY_LED_BEHAVIOR_LED_PULSING_ENABLE, VTSS_F_PHY_LED_BEHAVIOR_LED_PULSING_ENABLE));
        }  else {
            VTSS_N("ser_led_output_2:%d, ser_led_output_1:%d", vtss_state->enhanced_led_control.ser_led_output_2, vtss_state->enhanced_led_control.ser_led_output_1);
            VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_LED_BEHAVIOR, 0, VTSS_F_PHY_LED_BEHAVIOR_LED_PULSING_ENABLE));
        }
        break;
...
static vtss_rc vtss_phy_read_temp_reg (vtss_port_no_t port_no, u8 *temp_reading) {
    u16 reg;
    static u8 last_temp_reading[VTSS_PORTS];
    ...
    case VTSS_PHY_FAMILY_TESLA:
        ...
        VTSS_RC(vtss_phy_wr_masked(port_no,0x1A, 0x0, 0x0040));  
        VTSS_RC(vtss_phy_wr_masked(port_no,0x1A, 0x0040, 0x0040)); 

        // Read current register value (Temperature Registers 26G,27G and 28G)
        VTSS_RC(vtss_phy_rd(port_no, 0x1C, &reg)); 
        *temp_reading = reg & 0xFF; // See data sheet for Registers 26G,27G and 28G
        
        
        // Revision A special
        if (ps->revision == VTSS_PHY_ATOM_REV_A) {
            ...
            // changes.
            while (*temp_reading != last_temp_reading[port_no] && timeout != 0) {
                ...
                *temp_reading = reg & 0xFF; // See data sheet for Registers 26G,27G and 28G
                ...
                if (*temp_reading == 0xFF || *temp_reading == 0x00) {
                    // Do nothing 
                } else if (*temp_reading > last_temp_reading[port_no]) {
                    last_temp_reading[port_no] += 1;
                } else {
                    last_temp_reading[port_no] -= 1;
                }
                timeout--;
                
                VTSS_N("temp_reading = %d, last_temp_reading[port_no] = %d", *temp_reading, last_temp_reading[port_no]);
            }
            
            VTSS_RC(atom_patch_suspend(port_no, FALSE)); // Resume patch
        }
        break;
    ...
    VTSS_RC(vtss_phy_page_std(port_no)); // Back to standard page.
    return VTSS_RC_OK;
}
...
vtss_rc vtss_phy_chip_temp_get_private (const vtss_port_no_t  port_no, 
    ...
    case VTSS_PHY_FAMILY_TESLA:
        ...
        VTSS_RC(vtss_phy_read_temp_reg(port_no, &temp_reading));
...
vtss_rc vtss_phy_chip_temp_init_private(const vtss_port_no_t  port_no) {
    ...
    case VTSS_PHY_FAMILY_TESLA:
        // Read current register value (Temperature Registers 26G,27G and 28G)
        VTSS_RC(vtss_phy_page_gpio(port_no));
        VTSS_RC(vtss_phy_wr_masked(port_no,0x1A, 0x0080, 0x0080)); // Deassert TMON0, Reset and enable background monitoring 
        VTSS_RC(vtss_phy_wr_masked(port_no,0x1A, 0x00C0, 0x00C0)); // Disable TMON1
        VTSS_RC(vtss_phy_page_std(port_no));
        break;
    default:
        VTSS_E("Temperature reading not supported for family: %s",vtss_phy_family2txt(ps->family));
...
static vtss_rc vtss_phy_loopback_set_private(const vtss_port_no_t   	port_no)
    
{
    VTSS_RC(VTSS_PHY_WARM_WR_MASKED(port_no, VTSS_PHY_EXTENDED_PHY_CONTROL, 
...
static vtss_rc vtss_phy_statistic_get_private(const vtss_port_no_t port_no, vtss_phy_statistic_t *statistic) {
    u16 reg_val;


    // Update CRC good count
    VTSS_RC(vtss_phy_page_ext(port_no));
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_CU_MEDIA_CRC_GOOD_COUNTER, &reg_val));
    ...
    VTSS_RC(vtss_phy_page_ext3(port_no));
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MEDIA_SERDES_TX_GOOD_PACKET_COUNTER, &reg_val));
    ...
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_MEDIA_SERDES_TX_CRC_ERROR_COUNTER, &reg_val));
    ...
    VTSS_RC(vtss_phy_page_std(port_no));
    VTSS_RC(PHY_RD_PAGE(port_no, VTSS_PHY_ERROR_COUNTER_1, &reg_val));
                           ...
                           const vtss_port_no_t        port_no)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = vtss_phy_detect(port_no);

        if (rc == VTSS_RC_OK)
            rc = VTSS_RC_COLD(vtss_phy_pre_reset_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                            ...
                            const vtss_port_no_t        port_no)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        if (rc == VTSS_RC_OK)
            rc = VTSS_RC_COLD(vtss_phy_post_reset_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                       ...
                       const vtss_phy_reset_conf_t *const conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        if (rc == VTSS_RC_OK)
            rc = VTSS_RC_COLD(vtss_phy_reset_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                                     ...
                                     BOOL                        *eee_capable)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        *eee_capable = vtss_phy_can(port_no, VTSS_CAP_EEE);
    }
    VTSS_EXIT();
    return rc;
}
                         ...
                         const BOOL                  enable)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_eee_ena_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                                                     ...
                                                     u8  *advertisements)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = VTSS_RC_COLD(vtss_phy_eee_link_partner_advertisements_get_private(port_no, advertisements));
    }
    VTSS_EXIT();
    return rc;
}
                                          ...
                                          BOOL  *tx_in_power_save_state) 
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = VTSS_RC_COLD(vtss_phy_eee_power_save_state_get_private(port_no, rx_in_power_save_state, tx_in_power_save_state));
    }
    VTSS_EXIT();
    return rc;
}
                        ...
                        vtss_phy_type_t *phy_id)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = vtss_phy_id_get_private(port_no, phy_id);
    }
                                ...
                                const vtss_port_no_t port_no)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = vtss_phy_chip_temp_init_private(port_no);
    }
                              ...
                              i16                    *const temp)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = vtss_phy_chip_temp_get_private(port_no, &chip_temp);
        *temp = chip_temp;
    }
                          ...
                          const vtss_phy_conf_t *const conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_conf_set_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                             ...
                             const vtss_phy_conf_1g_t  *const conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_conf_1g_set_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                               ...
                               vtss_phy_status_1g_t  *const status)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_status_1g_get_private(port_no, status);
    VTSS_EXIT();
    return rc;
}
                                ...
                                const vtss_phy_power_conf_t  *const conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_power_conf_set_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                                ...
                                const vtss_phy_clock_conf_t *const conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        if (clock_port < VTSS_PHY_RECOV_CLK_NUM) { /* Valid clock port */
            if ((rc = vtss_phy_id_get_private(port_no, &phy_id)) == VTSS_RC_OK) {
                ...
                rc = VTSS_RC_COLD(vtss_phy_clock_conf_set_private(port_no, clock_port));
            }
        }
    }
    VTSS_EXIT();
...
#define VTSS_SYNC_RC(function) if ((rc = function) != VTSS_RC_OK) {sync_calling_private = FALSE; return rc;}
vtss_rc vtss_phy_sync(const vtss_port_no_t port_no)
{
    ...
    for (i = VTSS_PORT_NO_START; i < VTSS_PORTS; i++) {
        warm_start_reg_changed[i] = FALSE;
    }


    // Call the private function for updating H/W registers 
    VTSS_SYNC_RC(vtss_phy_conf_set_private(port_no));
#if defined(VTSS_FEATURE_LED_POW_REDUC)
    VTSS_SYNC_RC(vtss_phy_led_intensity_set_private(port_no));
    VTSS_SYNC_RC(vtss_phy_enhanced_led_control_init_private(port_no));
#endif
    VTSS_SYNC_RC(vtss_phy_power_conf_set_private(port_no));
    VTSS_SYNC_RC(vtss_phy_conf_1g_set_private(port_no));
    
#if defined(VTSS_FEATURE_EEE)
    VTSS_SYNC_RC(vtss_phy_eee_ena_private(port_no));
#endif

    VTSS_SYNC_RC(vtss_phy_reset_private(port_no)); 
    VTSS_SYNC_RC(vtss_phy_loopback_set_private(port_no));            
    VTSS_N("Calling vtss_phy_event_enable_private(%d)", port_no);
    VTSS_SYNC_RC(vtss_phy_event_enable_private(port_no));
    
    for (i=0; i<VTSS_PHY_RECOV_CLK_NUM; ++i)
        VTSS_SYNC_RC(vtss_phy_clock_conf_set_private(port_no, i));
                              ...
                              const vtss_phy_led_mode_select_t led_mode_select)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_led_mode_set_private(port_no));
    }
                                   ...
                                   const vtss_phy_led_intensity intensity)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_led_intensity_set_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                                           ...
                                           const vtss_phy_enhanced_led_control_t conf)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_enhanced_led_control_init_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                                   ...
                                   
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_coma_mode_private(port_no, TRUE));
    }
    VTSS_EXIT();
    return rc;
}
                          ...
                          u8                   cnt)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_i2c_rd_private(port_no, i2c_mux, i2c_reg_addr, i2c_device_addr, value, cnt);
    VTSS_EXIT();
    return rc;
}
                           ...
                           u8                   cnt)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_i2c_wr_private(port_no, i2c_mux, i2c_reg_addr, i2c_device_addr, value, cnt);
    VTSS_EXIT();
    return rc;
}
                          ...
                          u16                  *const value)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_mmd_rd(port_no, devad, addr, value);
    VTSS_EXIT();
    return rc;
}
                           ...
                           u16                  value)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_mmd_wr(port_no, devad, addr, value);
    VTSS_EXIT();
    return rc;
}
                               ...
                               vtss_phy_statistic_t *statistic)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = vtss_phy_statistic_get_private(port_no, statistic);
    }
    VTSS_EXIT();
    return rc;
}
                           ...
                           u16                  *const value)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc |= vtss_phy_wr(port_no, 31, page);
        rc |= vtss_phy_rd(port_no, addr, value);
        rc |= vtss_phy_page_std(port_no);
    }
    VTSS_EXIT();
    return rc;
                       ...
                       const u16            value)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_wr(port_no, addr, value);
    VTSS_EXIT();
    return rc;
}
                              ...
                              const u16            mask)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_wr_masked(port_no, addr, value, mask);
    VTSS_EXIT();
    return rc;
}
                                   ...
                                   const u16            mask)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        (void) vtss_phy_wr(port_no, 31, page);
        rc= vtss_phy_wr_masked(port_no, addr, value, mask);
        (void) vtss_phy_wr(port_no, 31, 0);
    }
    VTSS_EXIT();
    return rc;
}
...
static vtss_rc vtss_phy_veriphy_get_private(const vtss_port_no_t      port_no,
                               ...
                               const u8              mode)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_veriphy_task_start(port_no, mode);
    VTSS_EXIT();
    return rc;
}
                             ...
                             vtss_phy_veriphy_result_t  *const result)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = vtss_phy_veriphy_get_private(port_no, result);
    VTSS_EXIT();
    return rc;
}
                                  ...
                                  const BOOL                enable)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_RC_COLD(vtss_phy_event_enable_private(port_no));
    }
    VTSS_EXIT();
    return rc;
}
                            ...
                            vtss_phy_event_t      *const events)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        rc = vtss_phy_event_poll_private(port_no, events);
    }
...
vtss_rc vtss_squelch_workaround(const vtss_inst_t         inst,
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        rc = squelch_workaround_private(port_no, enable);
    VTSS_EXIT();
    return rc;
}
                                  ...
                                  BOOL                      ail)
{
    ...
    if (ail) {
        ...
        pr("Port  Family    Type  Revision  MediaIf                         MacIf         IntrEventPoll  EEE INT  Channel ID  Port Base No. 1588 Base No.  \n");
        for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count; port_no++) {
            VTSS_RC(vtss_phy_id_get_private(port_no, &phy_id));
            (void)vtss_phy_event_poll_private(port_no, &events);
            ps = &vtss_state->phy_state[port_no];
            pr("%-4u  %-8s  %-4u  %-9u %-31s %-13s 0x%-12X %-3d %-4d %-11d %-13u %-13u\n", 
               ...
               events,
               vtss_phy_can(port_no, VTSS_CAP_EEE),
               vtss_phy_can(port_no, VTSS_CAP_INT),
               phy_id.channel_id,
               phy_id.base_port_no,
                              ...
                              vtss_phy_loopback_t  loopback)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = vtss_phy_loopback_set_private(port_no);
    }
    VTSS_EXIT();
    return rc;
}
                                ...
                                u16                     expected_crc)
{
   ...
   VTSS_ENTER();
   if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
       rc = vtss_phy_is_8051_crc_ok_private(port_no, FIRMWARE_START_ADDR, code_length, expected_crc);
   }
   VTSS_EXIT();
   return rc;
}
                              ...
                              const u16               value)
{
   ...
   VTSS_ENTER();
   if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
       rc = vtss_phy_cfg_ob_post0_private(port_no, value);
   }
   VTSS_EXIT();
   return rc;
}
                              ...
                              const                   u8 ib_eq_mode)
{
   ...
   VTSS_ENTER();
   if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
       rc= vtss_phy_atom12_cfg_ib_cterm_ena_private(port_no, ib_cterm_ena, ib_eq_mode);
   }
   VTSS_EXIT();
   return rc;
}
    ...
    
{
   ...
   VTSS_ENTER();
   if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
       rc = vtss_phy_atom12_patch_setttings_get_private(port_no, mcb_bus, cfg_buf, stat_buf);
   }
   VTSS_EXIT();
   return rc;
}
