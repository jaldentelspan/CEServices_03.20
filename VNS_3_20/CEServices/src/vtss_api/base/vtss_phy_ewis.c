...
#define VTSS_PHY_EWIS_MMD_READ(port_no, mmd, addr, value)             VTSS_RC(vtss_state->init_conf.mmd_read(port_no, mmd, addr, value))
#define VTSS_PHY_EWIS_MMD_WRITE(port_no, mmd, addr, value)            VTSS_RC(vtss_state->init_conf.mmd_write(port_no, mmd, addr, value))
#define VTSS_PHY_EWIS_MMD_WRITE_MASK(port_no, mmd, addr, value, mask) VTSS_RC(vtss_phy_ewis_write_mask(port_no, mmd, addr, value, mask))
 ...
 *  Static Functions
 * ================================================================= */
static vtss_rc vtss_phy_ewis_write_mask(const vtss_port_no_t port_no,
                                             const u16            mmd,
                                             const u32            addr,
                                             const u16            value,
                                             const u16            mask)
{
    ...
    /* Read-modify-write */
    VTSS_PHY_EWIS_MMD_READ(port_no, mmd, addr, &val);
    VTSS_PHY_EWIS_MMD_WRITE(port_no, mmd, addr, (val & ~mask) | (value & mask));
...
static vtss_rc vtss_phy_ewis_events_force(vtss_port_no_t port_no,
                                          const BOOL enable,
...
static vtss_rc vtss_phy_ewis_static_conf_set(const vtss_port_no_t port_no)
{
    ...
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xEDFF, &reg_val);
...
static vtss_rc vtss_phy_ewis_force_conf_set(const vtss_port_no_t port_no)
{
    vtss_ewis_force_mode_t  force_mode;
    BOOL enable = FALSE;
...
static vtss_rc vtss_phy_ewis_tx_oh_set(const vtss_port_no_t port_no)
{
...
static vtss_rc vtss_phy_ewis_mode_conf_set(vtss_port_no_t port_no)
{
    ...
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_PMA, 0xA101, &reg_val);
    ...
    /* Reset */
    VTSS_PHY_WARM_WR_MASKED_CHK_MASK(pma_port(port_no), MMD_PMA, 0x8003, 0x0001, 0x0001, 0x0);
    /* Clear */
    VTSS_PHY_WARM_WR_MASKED_CHK_MASK(pma_port(port_no), MMD_PMA, 0x8003, 0x0000, 0x0001, 0x0);
    return VTSS_RC_OK;
}

static vtss_rc vtss_phy_ewis_reset(vtss_port_no_t port_no)
{
...
static vtss_rc vtss_phy_ewis_action_set(vtss_port_no_t port_no)
{
    vtss_ewis_cons_act_t acti;
    u16 reg_val = 0x00;
...
static vtss_rc vtss_phy_ewis_section_txti_set(vtss_port_no_t port_no)
{
...
static vtss_rc vtss_phy_ewis_path_txti_set(vtss_port_no_t port_no)
{
    vtss_ewis_tti_t txti;
    u16 reg_val = 0;
    u16 reg_ct = 0;
...
static vtss_rc vtss_phy_ewis_test_mode_set(vtss_port_no_t port_no)
{
    ...
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0007, &reg_val);
...
static vtss_rc vtss_phy_ewis_prbs31_err_inj_set(vtss_port_no_t port_no, const vtss_ewis_prbs31_err_inj_t *const inj)
{
...
static vtss_rc vtss_phy_ewis_test_status_get(vtss_port_no_t port_no, vtss_ewis_test_status_t *const test_status)
{
   u16 reg_val = 0x00;

    /* Read the register 2x0009 to read the test pattern error count */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0009, &reg_val);
    test_status->tstpat_cnt = reg_val;
    /* Read the register 2xEC51::0 E-WIS PRBS31 Analyzer Status (2xEC51)*/
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xEC51, &reg_val);
    test_status->ana_sync = (reg_val & 0x0001);

    return VTSS_RC_OK;
}

static vtss_rc vtss_phy_ewis_defects_get(vtss_port_no_t port_no, vtss_ewis_defects_t *const def)
{
    u16 reg_val = 0x00;
    ...
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0021, &reg_val);
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0021, &reg_val);
...
static vtss_rc vtss_phy_ewis_status_get(vtss_port_no_t port_no, vtss_ewis_status_t *const status)
{
    u16 reg_val = 0x00;

    /* WIS Status 1 (2x0001) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0001, &reg_val);
...
static vtss_rc vtss_phy_ewis_section_acti_get(vtss_port_no_t port_no, vtss_ewis_tti_t *const acti)
{
    vtss_ewis_tti_t txti;
    u16 reg_val = 0;
    ...
    for(reg_ct = 0; reg_ct < reg_len; reg_ct++) {
        if (reg_ct < 8) {
            ...
            VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, (0x0048 + reg_ct), &reg_val);
        } else {
            /* Section TTI 16-63 Accepted Bytes are read from registers 0xE900 - 0xE917 */
            VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, (0xE900 + (reg_ct - 8)), &reg_val);
        }
...
static vtss_rc vtss_phy_ewis_path_acti_get(vtss_port_no_t port_no, vtss_ewis_tti_t *const acti)
{
    vtss_ewis_tti_t txti;
    u16 reg_val = 0;
    i16 reg_ct = 0, reg_len = 0;
    ...
    for(reg_ct = 0; reg_ct < reg_len; reg_ct++) {
        if (reg_ct < 8) {
            ...
            VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, (0x002F + reg_ct), &reg_val);
        } else {
            /* Path TTI 16-63 Accepted Bytes are read from registers 0xEB00 - 0xEB17 */
            VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, (0xEB00 + (reg_ct - 8)), &reg_val);
        }
        txti.tti[(reg_ct * 2) + 1] = (u8)((reg_val & 0xff00) >> 8);
        txti.tti[(reg_ct * 2)] = (u8)(reg_val & 0x00ff);
    }
...
static vtss_rc vtss_phy_ewis_perf_get(vtss_port_no_t port_no, vtss_ewis_perf_t *const perf)
{
    u16 lsw = 0x0000,msw = 0x0000;


    /* E-WIS S-BIP Error Counter 1 (MSW) (2xECB0) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB0, &msw);
    /* E-WIS S-BIP Error Counter 0 (LSW) (2xECB1) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB1, &lsw);
    perf->pn_ebc_s = ((msw << 16) | lsw);

    /* E-WIS L-BIP Error Counter 1 (MSW) (2xECB2) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB2, &msw);
    /* E-WIS L-BIP Error Counter 0 (LSW) (2xECB3) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB3, &lsw);
    perf->pn_ebc_l = ((msw << 16) | lsw);

    /* E-WIS L-REI Counter 1 (MSW) (2xEC90) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xEC90, &msw);
    /* E-WIS L-REI Counter 0 (LSW) (2xEC91)  */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB5, &lsw);
    perf->pf_ebc_l = ((msw << 16) | lsw);

    /* E-WIS P-BIP Error Counter 1 (MSW) (2xECB4) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB4, &msw);
    /* E-WIS P-BIP Error Counter 0 (LSW) (2xECB5) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xECB5, &lsw);
    perf->pn_ebc_p = ((msw << 16) | lsw);
 
    /* E-WIS P-REI Counter 1 (MSW) (2xEC80) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xEC80, &msw);
    /* E-WIS P-REI Counter 0 (LSW) (2xEC81) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0xEC81, &lsw);
    perf->pf_ebc_p = ((msw << 16) | lsw);

    return VTSS_RC_OK;
}

static vtss_rc vtss_phy_ewis_counter_get(vtss_port_no_t port_no, vtss_ewis_counter_t *const counter)
{
    u16 reg_val = 0x00;

    /*   WIS S-BIP Error Count 2x003c */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x003c, &reg_val);
    counter->pn_ebc_s = reg_val;

    /* Read Near End BIP Error Count, first read MSW and then read LSW */
    reg_val = 0;
    /*   WIS L-BIP Error Count 1 MSW (2x0039) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x0039, &reg_val);
    counter->pn_ebc_l = (reg_val & 0xffff) << 15;
    reg_val = 0;
    /*   WIS L-BIP Error Count 0 LSW (2x003A) */
    VTSS_PHY_EWIS_MMD_READ(port_no, MMD_WIS, 0x003A, &reg_val);
...
static vtss_rc vtss_phy_ewis_counter_threshold_set(const vtss_port_no_t port_no)
{
...
static vtss_ewis_cons_act_t act_default = {
    .aisl = {
        .ais_on_los       = FALSE,
        .ais_on_lof       = FALSE,
    },
...
* ================================================================= */
static vtss_ewis_static_conf_t static_default = {
...
vtss_rc vtss_phy_inst_ewis_create(void)
{
...
vtss_rc vtss_phy_ewis_sync(vtss_port_no_t port_no)
{
    ...
    VTSS_SYNC_RC(vtss_phy_ewis_static_conf_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_mode_conf_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_force_conf_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_action_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_section_txti_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_path_txti_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_test_mode_set(port_no));
    VTSS_SYNC_RC(vtss_phy_ewis_counter_threshold_set(port_no));
