                              ...
                              const vtss_upi_event_t    ev_mask)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK ) {
        rc = VTSS_FUNC_COLD(upi_event_enable, port_no, enable, ev_mask);
    }
    VTSS_EXIT();
    return rc;
}
                            ...
                            vtss_upi_event_t      *const status)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK ) {
        rc = VTSS_FUNC_COLD(upi_event_poll, port_no, status);
    }
    VTSS_EXIT();
    return rc;
}
                                         ...
                                         vtss_upi_event_t      *const status)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK ) {
        rc = VTSS_FUNC_COLD(upi_event_poll_without_mask, port_no, status);
    }
    VTSS_EXIT();
    return rc;
}
                          ...
                          const vtss_upi_cfg_t *const cfg)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && cfg) {
        ...
        rc = VTSS_FUNC_COLD(upi_config_set, port_no);
    }
    VTSS_EXIT();
    return rc;
}
                            ...
                            vtss_upi_status_t *const status)
{
    ...
    VTSS_D("port_no: %u", port_no);
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && status) {
        rc = VTSS_FUNC(upi_status_get, port_no, status);
    }
    VTSS_EXIT();
    return rc;
}
                               ...
                               const vtss_upi_test_cfg_t *const cfg)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && cfg) {
        ...
        rc = VTSS_FUNC_COLD(upi_test_config_set, port_no);
    }
    VTSS_EXIT();
    return rc;
}
                                 ...
                                 vtss_upi_test_status_t *const status)
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && status) {
        rc = VTSS_FUNC(upi_test_status_get, port_no, status);
    }
    VTSS_EXIT();
    return rc;
}
                               ...
                               vtss_upi_txeq_cfg_t const *cfg)
{
    ...
    if (((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK)
        ...
        rc = VTSS_FUNC(upi_txeq_mode_set, port_no);
    }
    VTSS_EXIT();
    return rc;
}
                               ...
                               vtss_upi_txeq_cfg_t *const cfg)
{
    ...
    VTSS_D("port_no: %u", port_no);
    VTSS_ENTER();
    rc = vtss_inst_port_no_check(inst, port_no);
                                  ...
                                  BOOL polarity)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_FUNC(upi_txeq_coef_adjust, port_no, coef, polarity);
    }
    VTSS_EXIT();
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK){
        if ((power_mvppd < 127) && (power_mvppd > 20)) {
            vtss_state->upi_state[port_no].upi_tx_cfg.transmit_power = 
															power_mvppd;
            rc = VTSS_FUNC(upi_txamp_set, port_no);
    	}
        else {
            rc = VTSS_RC_ERROR;
        }
                           ...
                           u32 *power_mvppd)
{
    ...
    VTSS_D("port_no: %u", port_no);
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && power_mvppd) {
                            ...
                            u32 slew_rate)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_FUNC(upi_txslew_set, port_no);
    } else {
        rc = VTSS_RC_ERROR;
    }
                            ...
                            u32 *slew_rate)
{
    ...
    VTSS_D("port_no: %u", port_no);
    VTSS_ENTER();
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK && slew_rate) {
                               ...
                               vtss_upi_rxeq_cfg_t const *cfg)
{
    ...
    if ((rc = vtss_inst_port_no_check(inst, port_no)) == VTSS_RC_OK 
        ...
        rc = VTSS_FUNC(upi_rxeq_mode_set, port_no);
    }
    VTSS_EXIT();
    return rc;
}
