...
static struct work_struct work_q_ext0;
static struct work_struct work_q_ext1;
static struct work_struct work_q_devall;
...
static int  
port_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
...
static int __init vtss_switch_init(void)
{
    ...
    chip_id = readl(VTSS_DEVCPU_GCB_CHIP_REGS_CHIP_ID);
    ...
    if(board_type == VTSS_BOARD_LUTON10_REF || board_type == VTSS_BOARD_LUTON26_REF
    		|| board_type == VTSS_BOARD_VNS_6_REF || board_type == VTSS_BOARD_VNS_8_REF
                || board_type == VTSS_BOARD_VNS_12_REF || board_type == VTSS_BOARD_VESM_REF
                || board_type == VTSS_BOARD_VNS_16_REF
    		|| board_type == VTSS_BOARD_NCGU_REF || board_type == VTSS_BOARD_IGU_REF
    		|| board_type == VTSS_BOARD_MITR_REF || board_type == VTSS_BOARD_ESIR_REF) {
