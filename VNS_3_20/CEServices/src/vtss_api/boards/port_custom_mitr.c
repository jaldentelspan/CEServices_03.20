...
static vtss_rc lu26_reset(void)
{
    ...
    for (port_idx = 0; port_idx < 11; port_idx++) {
	...
	if (vtss_sgpio_conf_get(NULL, 0, 0, &conf) == VTSS_RC_OK) {
        ...
        if ((board_type == VTSS_BOARD_MITR_REF) ) {
            ...
            for (port = 0; port < VTSS_SGPIO_PORTS; port++) 
                conf.port_conf[port].enabled = 1;//(port < 3  ? 1 : 0);
...
BOOL vtss_board_probe_lu26(vtss_board_t *board, vtss_board_info_t *board_info)
{
    ...
    if (board_type != VTSS_BOARD_MITR_REF ) {
        board_type = VTSS_BOARD_MITR_REF;
    }
    ...
    board->name = "MITR";
    // MISC_CFG_SW_MODE  VTSS_PORT_MUX_MODE_1
