                               ...
                               port_custom_conf_t *port_conf)
{
    ...
    if (vtss_sgpio_conf_get(NULL, 0, 0, &conf) == VTSS_RC_OK) {
        for (; port_idx < process_cnt; port_idx++) {
            ...
            if (board_type == VTSS_BOARD_LUTON26_REF) {
                fiber = FALSE;
                if (port_custom_table[port_idx].cap & (PORT_CAP_SPEED_DUAL_ANY_FIBER)) {
...
BOOL vtss_board_probe_lu26(vtss_board_t *board, vtss_board_info_t *board_info)
{
...

    {
        ...
        for(port_idx = 0; port_idx < 8; port_idx++) {
            lu26_port_table[port_idx].map.chip_port = port_idx;
            lu26_port_table[port_idx].map.miim_controller = VTSS_MIIM_CONTROLLER_0;
            lu26_port_table[port_idx].map.miim_addr = port_idx;
            lu26_port_table[port_idx].mac_if = VTSS_PORT_INTERFACE_SGMII;
            lu26_port_table[port_idx].cap = PORT_CAP_TRI_SPEED_COPPER;
        }
        if(board->type == VTSS_BOARD_LUTON26_REF) {
            for(port_idx = 8; port_idx < 12 && port_idx < VTSS_PORTS; port_idx++) {
                lu26_port_table[port_idx].map.chip_port = port_idx;
                lu26_port_table[port_idx].map.miim_controller = VTSS_MIIM_CONTROLLER_0;
                lu26_port_table[port_idx].map.miim_addr = port_idx;
                lu26_port_table[port_idx].mac_if = VTSS_PORT_INTERFACE_SGMII;
...
#if ((VTSS_PORTS-2) > 12)
            for(port_idx = 12; port_idx < VTSS_PORTS-2; port_idx++) {
                lu26_port_table[port_idx].map.chip_port = port_idx;
                lu26_port_table[port_idx].map.miim_controller = VTSS_MIIM_CONTROLLER_1;
                lu26_port_table[port_idx].map.miim_addr = port_idx - 12;
                lu26_port_table[port_idx].mac_if = VTSS_PORT_INTERFACE_QSGMII;

                if (port_idx < 20) {
                    lu26_port_table[port_idx].cap = PORT_CAP_TRI_SPEED_COPPER;
                } else {
                    // Dual media ports (port 21-24)
                    if ((port_idx == 20) && (port_idx < VTSS_PORT_ARRAY_SIZE)) {
                        //SFP auto detection only works for port 21
                        lu26_port_table[port_idx].cap = PORT_CAP_TRI_SPEED_DUAL_ANY_FIBER;
                    } else if (port_idx < VTSS_PORT_ARRAY_SIZE) { /* Satisfy lint */
                        //SFP auto detection for ports 22-24 don't work due to a hardware board bug with the I2C clocks for these ports.
                        lu26_port_table[port_idx].cap = PORT_CAP_TRI_SPEED_DUAL_ANY_FIBER_FIXED_SFP_SPEED;
                    }
                }
            }
#endif /* (VTSS_PORTS-2) > 12 */
        }
...
#endif
        for(port_idx = VTSS_PORTS-2, chip_port = 24; port_idx < VTSS_PORTS; port_idx++,chip_port++) {
            lu26_port_table[port_idx].map.chip_port = chip_port;
