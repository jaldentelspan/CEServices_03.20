...
cyg_int32 handler_config_thermal_protect_config(CYG_HTTPD_STATE *p)
{
    ...
    thermal_protect_local_conf_t      local_conf, newconf;
    vtss_rc               rc;
    char                  form_name[50];
    int                   i;
    port_iter_t           pit;
    ...
    if (web_get_method(p, VTSS_MODULE_ID_THERMAL_PROTECT) == CYG_HTTPD_METHOD_POST) {
        ...
        thermal_protect_mgmt_get_switch_conf(&local_conf, isid); // Get current configuration
        newconf = local_conf;
        ...
        if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
            while (port_iter_getnext(&pit)) {
                ...
                newconf.port_prio[pit.iport] = httpd_form_get_value_int(p, form_name, THERMAL_PROTECT_TEMP_MIN, THERMAL_PROTECT_TEMP_MAX);
                T_D_PORT(pit.iport, "port_prio = %lu", newconf.port_prio[pit.iport]);
            }
        }

        // Set current configuration
        if (memcmp(&local_conf, &newconf, sizeof(newconf)) && (rc = thermal_protect_mgmt_set_switch_conf(&newconf, isid)) != VTSS_OK) {
            strcpy(err_msg, error_txt(rc)); // Set error message
        } else {
        ...
        thermal_protect_mgmt_get_switch_conf(&local_conf, VTSS_ISID_LOCAL); // Get current configuration
        for (i = 0; i < THERMAL_PROTECT_PRIOS_CNT; i++) {
            ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d",
                          local_conf.glbl_conf.prio_temperatures[i]);
            cyg_httpd_write_chunked(p->outbuffer, ct);
        ...
        if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
            while (port_iter_getnext(&pit)) {
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d",
                              local_conf.port_prio[pit.iport]);
                cyg_httpd_write_chunked(p->outbuffer, ct);
