    ...
    if (prio_temp) {
        if (!req->set) {
            ...
            for (prio_index = 0; prio_index < THERMAL_PROTECT_PRIOS_CNT; prio_index++) {
                ...
                sprintf(str_buf, "%-9d %d %s", prio_index, thermal_protect_local_conf.glbl_conf.prio_temperatures[prio_index], "C");
                CPRINTF("%-9s \n",  str_buf);
            }
        }
    }
        ...
        if (req->set) {
            ...
            if (prio_temp) {
                for (prio_index = 0; prio_index < THERMAL_PROTECT_PRIOS_CNT; prio_index++) {
                    ...
                    thermal_protect_local_conf.glbl_conf.prio_temperatures[prio_index] = thermal_protect_req->temp;
                }
            }
            ...
            if (port_prio) {
                ...
                if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
                    while (port_iter_getnext(&pit)) {
                        ...
                        thermal_protect_local_conf.port_prio[pit.iport]  = thermal_protect_req->prio;
                    }
                }
            }

            // Set the new configuration
            if ((rc = thermal_protect_mgmt_set_switch_conf(&thermal_protect_local_conf, isid)) != VTSS_OK) {
                CPRINTF("%s \n", error_txt(rc));
            }
        ...
        if (port_prio) {
            ...
            if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
                while (port_iter_getnext(&pit)) {
                    ...
                    sprintf(str_buf, "%-5u %d", pit.uport, thermal_protect_local_conf.port_prio[pit.iport]);
                    CPRINTF("%-11s \n", str_buf);
                }
            }
        }
