...
static thermal_protect_local_conf_t   thermal_protect_local_conf; // Current confiugration for this switch.
...
static BOOL thermal_protect_msg_rx(void *contxt, const void *const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
    ...
    case THERMAL_PROTECT_MSG_ID_CONF_SET_REQ: {
        ...
        THERMAL_PROTECT_CRIT_ENTER();
        thermal_protect_local_conf  =  (msg->local_conf); // Update configuration for this switch.
        T_DG(TRACE_GRP_CONF, "prio_temperatures[0] = %d",
             thermal_protect_local_conf.glbl_conf.prio_temperatures[0]);
        THERMAL_PROTECT_CRIT_EXIT();
        break;
    }
    ...
    case THERMAL_PROTECT_MSG_ID_PORT_SHUTDOWN_REQ:
        ...
        T_D_PORT(port_index, "Thermal protection - Shutting down port");
...
static void thermal_protect_msg_tx_switch_conf (vtss_isid_t slave_id)
{
    ...
    THERMAL_PROTECT_CRIT_ENTER();
    msg->local_conf = thermal_protect_stack_conf.local_conf[slave_id];
    msg->local_conf.glbl_conf = thermal_protect_stack_conf.glbl_conf;
...
// Transmit port shutdown to a slave switch
//
...
static BOOL thermal_protect_msg_tx_switch_status_req (vtss_isid_t slave_id)
{
    ...
    if (timeout) {
        ...
        if (msg_switch_exists(slave_id)) {
            ...
            T_W("Skipped thermal_protect_msg_tx due to isid:%d msg switch doesn't exist", slave_id);
...
static void thermal_protect_chk(void)
{
    ...
    for (prio_index = 0; prio_index < THERMAL_PROTECT_PRIOS_CNT; prio_index++) {
        ...
        if (port_iter_init(&pit, NULL, VTSS_ISID_LOCAL, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
            while (port_iter_getnext(&pit)) {
                THERMAL_PROTECT_CRIT_ENTER(); // Protect thermal_protect_local_conf
                port_prio = thermal_protect_local_conf.port_prio[pit.iport];
                prio_temp = thermal_protect_local_conf.glbl_conf.prio_temperatures[prio_index];
                THERMAL_PROTECT_CRIT_EXIT();
...
static void  thermal_protect_conf_read(BOOL create)
{
    ...
    if (do_create) {
        ...
        memset(thermal_protect_flash_conf, 0, sizeof(*thermal_protect_flash_conf)); // Set everything to 0. Non-zero default values will be set below.
        int prio_index;
        vtss_isid_t isid;
        for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {
            for (prio_index = 0; prio_index < THERMAL_PROTECT_PRIOS_CNT; prio_index++) {
                thermal_protect_flash_conf->stack_conf.glbl_conf.prio_temperatures[prio_index] = THERMAL_PROTECT_TEMP_MAX;
                thermal_protect_flash_conf->stack_conf.local_conf[isid].glbl_conf.prio_temperatures[prio_index] = THERMAL_PROTECT_TEMP_MAX;
            }
        }
...
static void thermal_protect_thread(cyg_addrword_t data)
{
    ...
    if (thermal_protect_init_temperature_sensor != VTSS_OK) {
        T_E("Could not initialise temperature sensor controller. Shutting down ports and terminating thermal protection thread");
        // Loop through all front ports
        if (port_iter_init(&pit, NULL, VTSS_ISID_LOCAL, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_OK) {
            while (port_iter_getnext(&pit)) {
                thermal_protect_msg_tx_port_shutdown(pit.iport, TRUE);
...
//
void thermal_protect_mgmt_get_switch_conf(thermal_protect_local_conf_t *switch_conf, vtss_isid_t isid)
{

    if (isid != VTSS_ISID_LOCAL) {
        // Get this switch's configuration
        THERMAL_PROTECT_CRIT_ENTER();
        memcpy(switch_conf, &thermal_protect_stack_conf.local_conf[isid], sizeof(thermal_protect_local_conf_t)); // Update the configuration for the switch
        THERMAL_PROTECT_CRIT_EXIT();

    } else {
        THERMAL_PROTECT_CRIT_ENTER(); // Protect  thermal_protect_local_conf
        *switch_conf = thermal_protect_local_conf;
        T_I("prio 0 temp = %d", thermal_protect_local_conf.glbl_conf.prio_temperatures[0]);
        THERMAL_PROTECT_CRIT_EXIT();
    }
}
...
vtss_rc thermal_protect_mgmt_set_switch_conf(thermal_protect_local_conf_t  *new_switch_conf, vtss_isid_t isid)
{
    ...
    THERMAL_PROTECT_CRIT_ENTER();
    memcpy(&thermal_protect_stack_conf.glbl_conf,  &new_switch_conf->glbl_conf, sizeof(thermal_protect_glbl_conf_t)); // Update the configuration for the switch
    memcpy(&thermal_protect_stack_conf.local_conf[isid], new_switch_conf, sizeof(thermal_protect_local_conf_t)); // Update the configuration for the switch
    THERMAL_PROTECT_CRIT_EXIT();
...
vtss_rc thermal_protect_mgmt_get_switch_status(thermal_protect_local_status_t *status, vtss_isid_t isid)
{
    ...
    if (thermal_protect_msg_tx_switch_status_req(isid)) {
        ...
        memset(status, 0, sizeof(thermal_protect_local_status_t)); // We have no real data, so we resets everything to 0.
