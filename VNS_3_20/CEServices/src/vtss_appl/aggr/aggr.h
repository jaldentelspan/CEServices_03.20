...
typedef enum {
    ...
    AGGR_MSG_ID_PORTMAP_REQ,  /* Request    */
    AGGR_MSG_ID_PORTMAP_RESP  /* Response   */
} aggr_msg_id_t;
...
/* AGGR_MSG_ID_PORTMAP_REQ */
typedef struct {
    aggr_msg_id_t          msg_id;    /* Message ID  */
} aggr_msg_portmap_req_t;

/* AGGR_MSG_ID_PORTMAP_RESP */
typedef struct {
    aggr_msg_id_t          msg_id;    /* Message ID  */
    i32                    chip_port[VTSS_PORTS];
} aggr_msg_portmap_resp_t;
...
/* AGGR message request timer in sec */
#define AGGR_REQ_TIMEOUT 10
...
typedef struct {
    union {
        ...
        aggr_msg_portmap_req_t     pm;        
    } req; 
} aggr_msg_req_t;


/* Respons message for size calc */
typedef struct {
    union {
        aggr_msg_portmap_resp_t pm;
    } resp; 
} aggr_msg_resp_t;
...
typedef struct {   
    ...
    struct {
        vtss_os_sem_t    sem; 
        uchar            msg[sizeof(aggr_msg_resp_t)];
    } reply;

    cyg_flag_t                      wait_flags[VTSS_ISID_END];
    u32                             aggr_callbacks;
    ...
    vtss_port_speed_t               aggr_group_speed[VTSS_ISID_END][AGGR_MGMT_GLAG_END];  
    aggr_conf_stack_aggr_t          aggr_config_stack;

#if defined(VTSS_FEATURE_VSTAX_V2) && VTSS_SWITCH_STACKABLE
    aggr_mgmt_group_no_t            active_aggr_ports[VTSS_ISID_END][VTSS_PORT_ARRAY_SIZE]; 
    i32                             port_map_global[VTSS_ISID_END][VTSS_PORT_ARRAY_SIZE]; 
