...
static void aggr_cli_cmd_mode(cli_req_t *req )
{
    ...
    if (cli_cmd_conf_slave(req) || aggr_mgmt_aggr_mode_get(&mode) != VTSS_OK)
        return;
    ...
    if (req->set) {
        ...
        if (aggr_mgmt_aggr_mode_set(&mode) != VTSS_OK)
                          ...
                          aggr_mgmt_group_no_t min, aggr_mgmt_group_no_t max, BOOL *first)
{
    ...
    while (1) {
        ...
        if (msg_switch_is_master() &&
            aggr_mgmt_lacp_members_get(isid, aggr_no, &lacp, next) == VTSS_OK &&
            (rc != VTSS_OK || lacp.aggr_no < grp->aggr_no)) {
            /* LACP group found and static group not found or bigger */
            grp = &lacp;
            rc = VTSS_OK;
        }
#endif /* VTSS_SW_OPTION_LACP */
        if (rc != VTSS_OK || ((aggr_no = grp->aggr_no) > max))
            break;
...
static void aggr_cli_cmd_add(cli_req_t *req )
{
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        if (mgmt_aggr_add_check(isid, aggr_add, iport_list.member, buf) != VTSS_OK) {
            CPRINTF("%s\n", buf);
            return;
        }
        ...
        aggr_add = (aggr_req->aggr_id ? mgmt_aggr_id2no(aggr_req->aggr_id) : aggr_req->aggr_id);
        for (error = 0, aggr_no = 0, aggr_free = AGGR_MGMT_GROUP_NO_START; !error && aggr_mgmt_port_members_get(isid, aggr_no, &group, 1) == VTSS_OK; ) {
        ...
        if ((rc = aggr_mgmt_port_members_add(isid, aggr_add, &group.entry)) != VTSS_OK) {
            cli_cmd_usid_print(usid, req, 0);
...
static void aggr_cli_cmd_aggr_del(cli_req_t *req )
{
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        if (aggr_mgmt_group_del(isid, mgmt_aggr_id2no(aggr_req->aggr_id)) != VTSS_OK)
...
static void aggr_cli_cmd_debug_aggr(cli_req_t *req )
{
    ...
    for (addr = 0x80, p=0; addr <= 0x9b; addr++,p++) {
        ...
        if (misc_debug_reg_read(VTSS_ISID_LOCAL, 0, reg_addr, &value) != VTSS_OK)
        ...
        if (misc_debug_reg_read(VTSS_ISID_LOCAL, 0, reg_addr, &value2) != VTSS_OK)
            CPRINTF("Debug reg read operation failed\n");
