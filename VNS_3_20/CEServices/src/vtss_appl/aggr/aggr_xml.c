...
static BOOL cx_lacp_match(const cx_table_context_t *context, ulong port_a, ulong port_b)
{
    ...
    return (lacp_get_port_config(context->isid, port_a, &conf_a) == VTSS_OK &&
            lacp_get_port_config(context->isid, port_b, &conf_b) == VTSS_OK &&
...
static vtss_rc cx_lacp_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    CX_RC(lacp_get_port_config(context->isid, port_no, &conf));
...
static vtss_rc aggr_cx_parse_func(cx_set_state_t *s)
{
    ...
    case CX_PARSE_CMD_PARM:
    {
        ...
        if (global && s->apply && aggr_mgmt_aggr_mode_get(&mode) != VTSS_OK)
            break;
        ...
        case CX_TAG_ENTRY:
        {
            ...
            if (!global && s->group == CX_TAG_AGGR_TABLE &&
                ...
                if (cx_parse_ports(s, port_list, 1) == VTSS_OK) {
                    ...
                    if (mgmt_aggr_add_check(s->isid, aggr_no, port_mem, s->msg) != VTSS_OK)
                        s->rc = CONF_XML_ERROR_FILE_PARM;

                    for (port_idx = VTSS_PORT_NO_START;
                         s->rc == VTSS_OK && port_idx < VTSS_PORT_NO_END; port_idx++) {
                ...
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    if (cx_parse_bool(s, "mode", &conf.enable_lacp, 1) == VTSS_OK) {
                        mode = 1;
                        aggr_state->line_lacp = s->line;
                    } else if (cx_parse_ulong_word(s, "key", &val, 1, 65535, "auto",
                                                   VTSS_LACP_AUTOKEY) == VTSS_OK) {
                        key = 1;
                        conf.port_key = val;
                    } else if (cx_parse_kw(s, "role", cx_kw_lacp_role, &val, 1) == VTSS_OK) {
                        role = 1;
                        conf.active_or_passive = val;
                    } else if (cx_parse_ulong(s, "prio", &val, 0, 65535) == VTSS_OK) {
                        prio = 1;
                        conf.port_prio = val;
                    } else if (cx_parse_kw(s, "timeout", cx_kw_lacp_timeout, &val, 1) == VTSS_OK) {
                        timeout = 1;
                        conf.xmit_mode = val;
                    } else
                        cx_parm_unknown(s);
        ...
        if (s->init) {
            ...
            for (port_idx = VTSS_PORT_NO_START; 
                ...
                if (!PORT_NO_IS_STACK(port_idx))
                    lacp_get_port_config(s->isid, 
                                         port_idx, &aggr_state->lacp[port_idx]);
            }
#endif /* VTSS_SW_OPTION_LACP */
             ...
             * lacp_set_port_config does not allow any changes if 
            ...
            for (port_idx = VTSS_PORT_NO_START; 
                ...
                if (!PORT_NO_IS_STACK(port_idx) &&
                        lacp_get_port_config(s->isid,
                         port_idx, &conf) == VTSS_OK &&
                        conf.enable_lacp) {
                    conf.enable_lacp = 0;
                    lacp_set_port_config(s->isid, port_idx, &conf);
                } /* end of if (!PORT....) */
            } /* end of for */
#endif /* VTSS_SW_OPTION_LACP */
            ...
            for (port_idx = VTSS_PORT_NO_START; port_idx < VTSS_PORT_NO_END; port_idx++)
                if (!PORT_NO_IS_STACK(port_idx))
                    lacp_set_port_config(s->isid, port_idx, &aggr_state->lacp[port_idx]);
#endif /* VTSS_SW_OPTION_LACP */
        }
        break;
    }
    ...
    switch (s->cmd) {
        ...
        case CX_GEN_CMD_SWITCH:
            ...
            cx_add_tag_line(s, CX_TAG_AGGR, 0);
            {
                ...
                while (aggr_mgmt_port_members_get(s->isid, aggr_no, &group, 1) == VTSS_OK) {
                    aggr_no = group.aggr_no;
