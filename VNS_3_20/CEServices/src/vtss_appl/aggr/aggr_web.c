...
{
    vtss_isid_t              asid, sid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    ...
    if(p->method == CYG_HTTPD_METHOD_POST) {
            ...
            for (iport = 0; iport < port_count; iport++) {
                if ( aggr[aggrgroup][iport] == 1 ) { // this group has members, add the group
                    ...
                    if((rc = aggr_mgmt_port_members_add(sid, (web_aggr_id2no(aggrgroup)), &groupMember)) < 0) {
                        T_D("aggr_mgmt_port_members_add(%u, %u): failed rc = %d", sid, aggrgroup, rc);
                        if (rc == AGGR_ERROR_GROUP_IN_USE) {
                            err = "The group is used by LACP";
                        } else if (rc == AGGR_ERROR_LACP_ENABLED) {
                            err = "Static aggregation and LACP can not both be enabled on the same ports";
                        } else if (rc == AGGR_ERROR_DOT1X_ENABLED) {
                            err = "Static aggregation cannot be enabed on ports whose 802.1X Admin State is not Authorized";
                        } else if (rc == AGGR_ERROR_HDX_SPEED_ERROR) {
                            err = "Port joining aggregation must be in the same speed and in full duplex";
                        } else if (rc == AGGR_ERROR_MEMBER_OVERFLOW) {
                            err = "To many members in the aggregation group";
                        }
                        
                        if (err != NULL) {
                            send_custom_error(p, "Aggregation Error", err, strlen(err));
                            return -1;
                        }
        ...
        for (aggrgroup = 1; aggrgroup < aggr_count; aggrgroup++){// Get each group members
            if (aggr_mgmt_port_members_get(sid, web_aggr_id2no(aggrgroup), &aggrMember, 0) == VTSS_OK) {
        ...
        if (aggr_mgmt_aggr_mode_get(&mode) != VTSS_OK) {
        ...
        for (asid = VTSS_ISID_START; asid < VTSS_ISID_END; asid++) {
            if (asid == sid) continue; /* exclude the selected switch for easier Java implementation */
            for (aggrgroup = 1; aggrgroup <= VTSS_GLAGS; aggrgroup++){// Get each group members
                if ( aggr_mgmt_port_members_get(asid, web_aggr_id2no(aggrgroup), &aggrMember, 0) == VTSS_OK ){
                    for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
