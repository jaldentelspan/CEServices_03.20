...
{
    upnp_msg_req_t *msg = malloc(size);
    if (msg) {
        msg->msg_id = msg_id;
    }
                    ...
                    ulong port_no)
{
    ...
    if (vtss_ip2_if_inject(ingress_vid, len, packet) != VTSS_OK) {
        T_W("Failed in vtss_ip2_if_inject(%u, %d)!\n", ingress_vid, len);
    }
#else
    ...
    /* The packet is replied to system, forward it to TCP/IP stack.
       The system UPNP module will process the packet via socket interface */
    ...
    /* Get rl by mapping from VID */
    if ((rl = ip_rl_get(vid))) {
        sc = rl->sc;        /* and get SC from rl */
        if (IPP_rx_packet(sc, ptr, &rx_info)) {
...
#else
    if (ip_mgmt_conf_get(&conf) != VTSS_OK) {
        T_E("ip_mgmt_conf_get failed");
        return;
    } else {
        intf_vid = conf.vid;
    }
...
void vtss_upnp_get_location(char *location)
{
    ...
    if (ip_mgmt_conf_cur_get(&ip_cfg) != VTSS_OK) {
        T_D("vtss_upnp_get_location: ip_mgmt_conf_cur_get fails");
        intf_adr = 0;
    } else {
        intf_adr = ip_cfg.ipv4_addr;
    }
#endif /* VTSS_SW_OPTION_IP2 */
...
int vtss_upnp_get_ip(char *ipaddr)
{
    ...
    if (ip_mgmt_conf_cur_get(&ip_cfg) != VTSS_OK) {
        T_D("vtss_upnp_get_ip: ip_mgmt_conf_cur_get fails");
    }

    if (ip_cfg.ipv4_addr != 0) {
        (void)misc_ipv4_txt(ip_cfg.ipv4_addr, ipaddr);
        return 0;
    } else {
        return 1;
    }
