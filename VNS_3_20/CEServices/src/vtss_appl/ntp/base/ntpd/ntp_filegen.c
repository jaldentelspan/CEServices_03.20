...
filegen_init(char *prefix, const char *basename, FILEGEN *fp)
{
    ...
    if ( fp->basename ) {
        ...
        msyslog(LOG_ERR, "filegen_init: memory insufficient\n");
    }
#else
	strcpy(fp->basename, basename);
#endif
...
#if 1 /* CP: avoid use NULL */
    if ( basename == NULL ) {
        msyslog(LOG_ERR, "filegen_open: memory insufficient\n");
        return;
    }
#endif
...
#if 1 /* CP: avoid use NULL */
        if ( gen->basename == NULL ) {
            msyslog(LOG_ERR, "filegen_config: memory insufficient\n");
            return;
        }
#endif
...
#if 1 /* CP: avoid use NULL */
        if ( (*f)->name == NULL ) {
            msyslog(LOG_ERR, "filegen_register: memory insufficient\n");
            free( *f );
            *f = NULL;
            return;
        }
