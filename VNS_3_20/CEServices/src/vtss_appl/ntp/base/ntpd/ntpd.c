...
int ntp_interval_max = NTP_MAXDPOLL;
int ntp_interval_min = NTP_MINDPOLL;

const char * Version = "ntpd 4.2.4p6@1.1549-o Wed Mar 18 17:04:40 UTC 2009 (83)";

ntp_ip_dns_mapping ntp_mapping[NTP_SERVER_MAX_NUM];
#endif
...
#ifdef NTP_ECOS
bool find_hostname_by_addr(struct sockaddr_storage *addr, u_char *hostname)
{
    int i;

    for (i = 0; i < NTP_SERVER_MAX_NUM; i++) {
        if (memcmp(addr, &ntp_mapping[i].ntp_sockaddr, sizeof(struct sockaddr_storage)) == 0) {
            strcpy(hostname, ntp_mapping[i].ip_addr_string);
            return TRUE;
        }
    }
    return FALSE;
...
int ntp_set_ntp_interval(int interval_min, int interval_max)
{
    static int is_initialized = 0;


    /* 
     * The interval could not be changed during running.
     * Only allowed to initial once. 
     */
    if (is_initialized != 0) {
        return 0;
    }
    else {
        sys_poll = ntp_interval_min = interval_min;
        ntp_interval_max = interval_max;
        is_initialized = 1;
        return 0;
    }
    ...
    int i;   
    for (i = 0; i < NTP_SERVER_MAX_NUM; i++) {
        if (strcmp(ntp_mapping[i].ip_addr_string, "") != 0) {
            if (ntp_mapping[i].ntp_sockaddr.ss_family == AF_INET)
                ntp_set_ntp_server(NTP_IPV4, ntp_mapping[i].ip_addr_string);
#ifdef ISC_PLATFORM_HAVEIPV6
            else if (ntp_mapping[i].ntp_sockaddr.ss_family == AF_INET6)
                ntp_set_ntp_server(NTP_IPV6, ntp_mapping[i].ip_addr_string);
#endif
            else
                return -1;
        }
...
int ntp_set_ntp_server(int ip_type, char *ip_addr_string)
{
    int peerversion;
    int minpoll;
	int maxpoll;
	...
	minpoll = ntp_interval_min;
#if 0	
    minpoll = global_ntpcfg.poll_interval;
	maxpoll = NTP_MAXPOLL;
#endif	
	maxpoll = ntp_interval_max;
	peerkey = 0;
	peerkeystr = (u_char *)"*";
    ...
    error = getaddrinfo(ip_addr_string, "ntp", &hints, &res);
    if(error) {
        printf("error : ntp_set_ntp_server\n");
        if (res) {
            freeaddrinfo(res);
        }    
        initializing = 0;
    ...
    if (ip_type == NTP_IPV4) {
        ...
        memmove(&host, res->ai_addr, res->ai_addrlen);
        freeaddrinfo(res);
        memcpy(&peeraddr, &host, sizeof(struct sockaddr_in));
        peeraddr.ss_family = AF_INET;     
       
    }     
#ifdef ISC_PLATFORM_HAVEIPV6
    else if (ip_type == NTP_IPV6){
        ...
        */
        host.sin6_scope_id = 2;
        memcpy(&peeraddr, &host, sizeof(struct sockaddr_in6));  
    ...
    if (peer_config(&peeraddr, ANY_INTERFACE_CHOOSE(&peeraddr), hmode,
				    peerversion, minpoll, maxpoll, peerflags,
				    ttl, peerkey, peerkeystr) == 0) {
					msyslog(LOG_ERR, "configuration of %s failed",
					stoa(&peeraddr));
    }
...
int ntp_unset_ntp_server(int ip_type, char *ip_addr_string)
{
    struct addrinfo      *res;
    struct addrinfo      hints;
    int                  error;
...
void *base_ntp_client( void *args )
{
    l_fp            now;
	struct recvbuf  *rbuf;
    sigset_t        set;
    double          ntp_old_drift = *((double *) args);
    int             first_update = 1;
    ...
    memset(&ntp_mapping, 0, sizeof(ntp_ip_dns_mapping)*NTP_SERVER_MAX_NUM);
	...
	loop_config(LOOP_DRIFTCOMP, ntp_old_drift / 1e6);
...
# ifndef HAVE_SIGNALED_IO
			rdfdes = activefds;
#  if defined(VMS) || defined(SYS_VXWORKS) || defined(NTP_ECOS)
...
#if 1 /* timer work around */
        /*
         * Here is the workaround for the issue that the timer stops calling
         * back after one day or two sometimes. The method is that if the
         * timer does not call more than 5 seconds, the timer will reset.  
         */       
        {
            static struct timespec  curr_time, last_updat;   
            
            clock_gettime(CLOCK_MONOTONIC, &curr_time);
            
            if ((curr_time.tv_sec - last_updat.tv_sec >= 5) && (!first_update)) {
                //printf("curr_time.tv_sec - %lu last_updat.tv_sec - %lu\n", curr_time.tv_sec , last_updat.tv_sec);
                reset_timer();
                clock_gettime(CLOCK_MONOTONIC, &last_updat);
                vtss_ntp_timer_reset_counter();
            }    
