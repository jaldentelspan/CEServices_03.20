...
{
	register struct peer *peer = NULL;	/* peer structure pointer */
...
/*
 * ntp_process_packet - Packet Procedure, a la Section 3.4.4 of the
 *	specification. Or almost, at least. If we're in here we have a
 *	reasonable expectation that we will be having a long term
 *	relationship with this host.
 */
 static double  gp_offset[500];
 static double  gp_offset_diff[500];
 static int  gp_offset_index = 0;
	...
	)
{
    ...
    gp_offset[gp_offset_index] = p_offset;
    gp_offset_diff[gp_offset_index] = p_offset - gp_offset[gp_offset_index - 1];
    gp_offset_index = (gp_offset_index + 1) % 500 ; 
    
#ifdef DEBUG
    if (debug) {   
        printf("ntp_process_packet: p_offset: %.6f\n",
			    p_offset);	
	    printf("ntp_process_packet: p_offset_diff: %.6f\n",
			    p_offset - gp_offset[gp_offset_index -2]);
	}
#endif	
#ifdef DEBUG
    if (debug) {   
	    printf("ntp_process_packet: t21: %.6f t34:%.6f\n",
			    t21, t34);
    }			    
#endif	
...
#ifdef DEBUG
	if (debug > 1) {
		printf("poll_update: sys_rootdispersion:%.6f  dtemp:%.6f  sys_peer->rootdispersion:%.6f  \n",
		    sys_rootdispersion, dtemp, sys_peer->rootdispersion);
    }		    
#endif			
...
#ifdef NTP_ECOS
        /* check initial state, if it's in initial state,
         * peer->nextdate depends on NTP_MINPOLL, else
         * peer->nextdate depends on peer configuration
         */
        if (strcmp((char *)&sys_refid, "INIT") == 0)
            peer->nextdate = peer->outdate + (1<<NTP_MINPOLL);
        else
...
#endif
    vtss_ntp_server_info(current_time, ntoa(&peer->srcadr), peer->flags,
		    peer->hpoll, peer->burst, peer->outdate, peer->nextdate, peer->offset);
}
...
#ifdef DEBUG
		if (debug)
		    printf("clock_filter(if (m ==0) return)\n");
#endif		
		return;
    }
...
clock_select(void)
{
	...
	if (nlist > list_alloc) {
        ...
        if ( endpoint == NULL || indx == NULL || peer_list == NULL ) {
            msyslog(LOG_ERR, "clock_select: memory insufficient\n");
            if ( endpoint ) {
                free( endpoint );
            }
            if ( indx ) {
...
#ifdef DEBUG			    
			    if (debug)
			        printf("clock_select: peer_unfit continue\n");           
#endif			
				continue;
            }
			/*
...
#ifdef DEBUG
    if (debug) {   
        printf("xxx clock_combine xxx: %d\n", npeers);
        printf("clock_combine: x : %.6f \n", x);
        printf("clock_combine: y : %.6f \n", y);
        printf("clock_combine: z : %.6f \n", z);
        printf("clock_combine: w : %.6f \n", w);
    	printf("clock_combine: sys_offset : %.6f \n", sys_offset);
	}
#endif
...
#ifdef DEBUG 
    if (debug > 11) { 	
        printf("sys_mindisp: %16.6f\t", sys_mindisp);
        printf("peer->rootdelay/dist: %16.6f\t", peer->rootdelay); 
        printf("peer->rootdispersion/dist: %16.6f\t", peer->rootdispersion); 
        printf("peer->delay: %16.6f\t", peer->delay);
        printf("peer->disp: %16.6f\t", peer->disp);
        printf("current_time: %16.6f\t", current_time);
        printf("peer->update: %16.6f\t", peer->update);
        printf("peer->jitter: %16.6f\t", peer->jitter);
        printf("clock_phi: %16.6f\t", clock_phi);
        printf("current_time %d\t", current_time);
        printf(" peer->update %d\n",  peer->update);
...
#ifndef NTP_ECOS // For DNS to get socket address
...
#ifdef DEBUG
    if (debug) {   
        double xxx = sys_maxdist + clock_phi *
    	            ULOGTOD(sys_poll);
    	double xxx_distance = root_distance(peer);            
    	            
    	printf("root_distance:%16.6f  xxx:%16.6f \n", xxx_distance, xxx);
    }
#endif	
