	...
	)
{
	...
	*/
    ...
    if ( temp_datum_pts_unit == NULL ) {
        msyslog(LOG_ERR, "datum_pts_start: memory insufficient");
        return 0;
    }
#endif
...
#if 1 /* CP: avoid use NULL */
    if ( datum_pts_unit[nunits] == NULL ) {
        msyslog(LOG_ERR, "datum_pts_start: memory insufficient");
        free( temp_datum_pts_unit );
        temp_datum_pts_unit = NULL;
        return 0;
    }
#endif
...
#if 1 /* CP: avoid use NULL */
                if ( temp_datum_pts_unit == NULL ) {
                    msyslog(LOG_ERR, "datum_pts_shutdown: memory insufficient");
                    return 0;
                }
#endif
