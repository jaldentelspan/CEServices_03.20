...
#define CYG_NTP_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))
...
init_io(void)
{
	...
	DPRINTF(11, ("init_io: maxactivefd %d\n", maxactivefd));
}
...
print_interface(struct interface *iface, char *pfx, char *sfx)
{
	...
	if (debug > 11)	/* in-depth debugging only */
...
new_asyncio_reader()
{
    ...
    if ( reader == NULL ) {
        msyslog(LOG_ERR, "new_asyncio_reader: memory insufficient\n");
        return NULL;
    }
#endif
...
#if 1 /* CP: avoid use NULL */
    if ( iface == NULL ) {
        msyslog(LOG_ERR, "new_interface: memory insufficient\n");
        return NULL;
    }
#endif
...
address_okay(struct interface *iface) {
	...
	DPRINTF(11, ("address_okay: listen Virtual: %d, IF name: %s\n", 
	...
	if((iface->flags & INT_LOOPBACK) != 0) {
		DPRINTF(11, ("address_okay: loopback - OK\n"));
		return (ISC_TRUE);
	}
	...
	if (specific_interface != NULL) {
		if (strcasecmp(iface->name, specific_interface) == 0) {
			DPRINTF(11, ("address_okay: specific interface name matched - OK\n"));
			return (ISC_TRUE);
		} else {
			DPRINTF(11, ("address_okay: specific interface name NOT matched - FAIL\n"));
			return (ISC_FALSE);
		}
	}
	else {
		if (listen_to_virtual_ips == 0  && 
			...
			DPRINTF(11, ("address_okay: virtual ip/alias - FAIL\n"));
			return (ISC_FALSE);
		}
	}

	DPRINTF(11, ("address_okay: OK\n"));
	return (ISC_TRUE);
}
...
set_wildcard_reuse(int family, int on)
{
	...
	if (fd != INVALID_SOCKET) {
		...
		DPRINTF(11, ("set SO_REUSEADDR to %s on %s\n", *onoff ? "ON" : "OFF",
			    stoa((family == AF_INET) ?
	...
	)
{
	...
	DPRINTF(11, ("update_interfaces(%d)\n", ntohs( (u_short) port)));
	     ...
	     result = isc_interfaceiter_next(iter))
	{
		...
		if (iface && refresh_interface(iface)) 
		{
			...
			DPRINT_INTERFACE(11, (iface, "updating ", " present\n"));
			ifi.action = IFS_EXISTS;
			ifi.interface = iface;
			if (receiver)
				receiver(data, &ifi);
		...
		else
		{
			...
			if (iface)
			{
				...
				DPRINT_INTERFACE(11, (iface, "updating ", " new - created\n"));
			}
			else
			{
				DPRINT_INTERFACE(11, (&interface, "updating ", " new - creation FAILED"));
		...
		while (interf != NULL)
		{
			...
			if (!(interf->flags & (INT_WILDCARD|INT_MCASTIF))) {
				...
				if (interf->phase != sys_interphase) {
					struct peer *peer;
					DPRINT_INTERFACE(11, (interf, "updating ", "GONE - deleting\n"));
	...
	)
{
	...
	DPRINTF(11, ("create_sockets: Total interfaces = %d\n", ninterfaces));
		 ...
		 )
{
	...
	DPRINT_INTERFACE(11, (interface, "created ", "\n"));
	return interface;
}
...
set_reuseaddr(int flag) {
	...
	for (interf = ISC_LIST_HEAD(inter_list);
		 ...
		 */
		DPRINTF(11, ("setting SO_REUSEADDR on %.16s@%s to %s\n", interf->name, stoa(&interf->sin), flag ? "on" : "off"));
...
addr_ismulticast(struct sockaddr_storage *maddr)
{
	...
	case AF_INET :
		if (!IN_CLASSD(ntohl(((struct sockaddr_in*)maddr)->sin_addr.s_addr))) {
			DPRINTF(11, ("multicast address %s not class D\n", stoa(maddr)));
			return (ISC_FALSE);
		}
		else
		{
	...
	case AF_INET6 :
		...
		if (!IN6_IS_ADDR_MULTICAST(&((struct sockaddr_in6*)maddr)->sin6_addr)) {
			DPRINTF(11, ("address %s not IPv6 multicast address\n", stoa(maddr)));
			return (ISC_FALSE);
		}
		else
		{
...
#endif
	DPRINTF(11, ("Added IPv4 multicast interface on socket %d, addr %s for multicast address %s\n",
...
#endif
		DPRINTF(11, ("Added IPv6 multicast interface on socket %d, addr %s, scope %d for multicast address %s\n",
			    iface->fd,  stoa(&iface->sin), iface->scopeid,
			    stoa(maddr)));
		break;
#else
...
socket_multicast_enable(struct interface *iface, int lscope, struct sockaddr_storage *maddr)
{
	...
	if (find_addr_in_list(maddr)) {
		DPRINTF(11, ("socket_multicast_enable(%s): already enabled\n", stoa(maddr)));
		return ISC_TRUE;
	}
	...
	case AF_INET:
		...
		DPRINTF(11, ("Added IPv4 multicast membership on socket %d, addr %s for %x / %x (%s)\n",
			    iface->fd, stoa(&iface->sin),
			    mreq.imr_multiaddr.s_addr,
			    mreq.imr_interface.s_addr, stoa(maddr)));
		break;
	...
	case AF_INET6:
		...
		DPRINTF(11, ("Added IPv6 multicast group on socket %d, addr %s for interface %d(%s)\n",
			    iface->fd, stoa(&iface->sin),
			    mreq6.ipv6mr_interface, stoa(maddr)));
		break;
#else
...
socket_multicast_disable(struct interface *iface, struct sockaddr_storage *maddr)
{
	...
	if (find_addr_in_list(maddr) == NULL) {
		DPRINTF(11, ("socket_multicast_disable(%s): not enabled\n", stoa(maddr)));
		return ISC_TRUE;
	}
...
# endif
			DPRINTF(11, ("Found interface #%d %s, scope: %d for address %s\n", iface->ifnum, iface->name, lscope, stoa(&addr)));
		}
		break;
	}
	...
	if (interface->fd != INVALID_SOCKET)
	{
		...
		((struct sockaddr_in*)&interface->mask)->sin_addr.s_addr =
						htonl(~(u_int32)0);
		DPRINT_INTERFACE(11, (interface, "multicast add ", "\n"));
	...
	)
{
	 ...
	 */
#ifndef NTP_ECOS	
    /* James: it is a fix for the Unix system. No need to apply it here */
    fd = move_fd(fd);
#endif	
...
#else
    int bind_cnt = 2;
    while((errval = bind(fd, (struct sockaddr *)addr, SOCKLEN(addr))) < 0) {
        //diag_printf("bind error, sleeping\n");
        cyg_thread_delay(CYG_NTP_MSEC2TICK(1000));
        bind_cnt--;
        if (bind_cnt == 0)
            break;
    }
...
# endif
...
#ifdef HAVE_TIMESTAMP
	{
		...
		{
			DPRINTF(11, ("setsockopt SO_TIMESTAMP enabled on fd %d address %s\n", fd, stoa(addr)));
		}
#endif
	}	
#endif
	DPRINTF(11, ("bind() fd %d, family %d, port %d, addr %s, flags=0x%x\n",
		   fd,
		   addr->ss_family,
	...
	DPRINTF(11, ("flags for fd %d: 0x%x\n", fd,
		    fcntl(fd, F_GETFL, 0)));
#endif /* SYS_WINNT || VMS */
...
#define badaddrs6 ((struct cache6 *)0)		/* Only used in empty loops! */
    ...
    /* Peter, 2011/12/8, Skip the loopback addresses */
	if (inter->flags & INT_LOOPBACK)	
	  return;
	...
	fetch_timestamp(struct recvbuf *rb, struct msghdr *msghdr, l_fp ts)
{
	...
	while (cmsghdr != NULL) {
		...
		case SCM_TIMESTAMP:
		{
			...
			double dtemp;
			l_fp nts;
			DPRINTF(11, ("fetch_timestamp: system network time stamp: %ld.%06ld\n", tvp->tv_sec, tvp->tv_usec));
...
#ifdef DEBUG_TIMING
			{
				...
				DPRINTF(11, ("fetch_timestamp: timestamp delta: %s (incl. prec fuzz)\n", lfptoa(&dts, 9)));
			}
#endif
		...
		default:
			DPRINTF(11, ("fetch_timestamp: skipping control message 0x%x\n", cmsghdr->cmsg_type));
			break;
		}
...
read_network_packet(SOCKET fd, struct interface *itf, l_fp ts)
{
	...
	if (rb == NULL || itf->ignore_packets == ISC_TRUE)
	{
		...
		DPRINTF(11, ("%s on (%lu) fd=%d from %s\n",
	...
	else if (buflen < 0)
	{
		...
		DPRINTF(11, ("read_network_packet: fd=%d dropped (bad recvfrom)\n", fd));
		freerecvbuf(rb);
		return (buflen);
	}
	...
	)
{
	...
	if (interface == NULL)
	{
		DPRINTF(11, ("Found no interface for address %s - returning wildcard\n",
			    stoa(addr)));
	...
	{
		DPRINTF(11, ("Found interface #%d %s for address %s\n",
	...
	)
{
	...
	DPRINTF(11, ("Finding interface for addr %s in list of addresses\n",
		    stoa(addr));)
...
# endif
	...
	DPRINTF(11, ("findlocalinterface: kernel maps %s to %s\n", stoa(addr), stoa(&saddr)));
	...
	)
{
	...
	if (nif) {
		DPRINTF(11, ("findlocalcastinterface: kernel recommends interface #%d %s\n", nif->ifnum, nif->name));
		return nif;
	}
	...
	)
{
	...
	DPRINTF(11, ("Finding broadcast/multicast interface for addr %s in list of addresses\n",
		    stoa(addr)));
	...
	if (interface != NULL)
	{
		DPRINTF(11, ("Found bcast-/mcast- interface index #%d %s\n", interface->ifnum, interface->name));
		return interface;
	}
...
#endif /* SIOCGIFCONF */
	if (interface == NULL) {
		DPRINTF(11, ("No bcast interface found for %s\n", stoa(addr)));
		return ANY_INTERFACE_CHOOSE(addr);
	} else {
		DPRINTF(11, ("Found bcast-/mcast- interface index #%d %s\n", interface->ifnum, interface->name));
		return interface;
...
#if 1 /* CP: avoid use NULL */
    if ( lsock == NULL ) {
        msyslog(LOG_ERR, "add_fd_to_list: memory insufficient\n");
        return;
    }
#endif
...
add_addr_to_list(struct sockaddr_storage *addr, struct interface *interface){
	...
	if (find_addr_in_list(addr) == NULL) {
...
#if 1 /* CP: avoid use NULL */
        if ( laddr == NULL ) {
            msyslog(LOG_ERR, "add_addr_to_list: memory insufficient\n");
            return;
        }
#endif
		...
		DPRINTF(11, ("Added addr %s to list of addresses\n",
			    stoa(addr)));
#ifdef DEBUG
	} else {
		DPRINTF(11, ("WARNING: Attempt to add duplicate addr %s to address list\n",
			    stoa(addr)));
	}
#endif
}
...
delete_addr_from_list(struct sockaddr_storage *addr) {
	...
	while(laddr != NULL) {
		...
		if(SOCKCMP(&laddr->addr, addr)) {
			...
			DPRINTF(11, ("Deleted addr %s from list of addresses\n",
				    stoa(addr)));
			free(laddr);
			break;
		}
...
delete_interface_from_list(struct interface *iface) {
	...
	while(laddr != NULL) {
		...
		if (laddr->interface == iface) {
			ISC_LIST_DEQUEUE_TYPE(remoteaddr_list, laddr, link, remaddr_t);
			DPRINTF(11, ("Deleted addr %s for interface #%d %s from list of addresses\n",
...
find_addr_in_list(struct sockaddr_storage *addr) {
	...
	remaddr_t *next;
	remaddr_t *laddr = ISC_LIST_HEAD(remoteaddr_list);
	DPRINTF(11, ("Searching for addr %s in list of addresses - ",
		    stoa(addr)));
...
find_flagged_addr_in_list(struct sockaddr_storage *addr, int flag) {
	...
	remaddr_t *next;
	remaddr_t *laddr = ISC_LIST_HEAD(remoteaddr_list);
	DPRINTF(11, ("Finding addr %s in list of addresses\n",
		    stoa(addr)));
...
process_routing_msgs(struct asyncio_reader *reader)
{
	...
	while ((p + sizeof(struct rt_msghdr)) <= (buffer + cnt))
	{
		...
		case RTM_IFANNOUNCE:
			 ...
			 */
			DPRINTF(11, ("routing message op = %d: scheduling interface update\n", rtm->rtm_type));
		...
		default:
			 ...
			 */
			DPRINTF(11, ("routing message op = %d: ignored\n", rtm->rtm_type));
			break;
		}
		p += rtm->rtm_msglen;
	}
