...
vtss_rc eth_link_oam_mgmt_port_remote_loopback_oper_conf_set(const vtss_isid_t isid, const vtss_port_no_t port_no, const BOOL conf)
{
    ...
    if (rc == VTSS_RC_OK) {
        ...
        T_E("Remote loopback operation is timed out on port(%u/%u)", isid, port_no);
    }
    return rc;
}
...

{
    do {
        ...
        if ( (trace_level <= VTSS_ETH_LINK_OAM_TRACE_LEVEL_NONE) ||
             (trace_level >= VTSS_ETH_LINK_OAM_TRACE_LEVEL_COUNT)) {
            break;
        }
...
u32 vtss_eth_link_oam_control_port_non_info_pdu_xmit(const u32 port_no, u8  *oam_client_pdu, const u16 oam_pdu_len, const u8 oam_pdu_code)
{
    ...
    do {
        ...
        if (rc == VTSS_ETH_LINK_OAM_RC_OK) {
            ...
            T_E("Error:%u occured while sending the OAM PDU on port(%u)",
                rc, port_no);
            free(bufref);
            break;
        }
...
static void eth_link_oam_client_thread(cyg_addrword_t data)
{
    ...
    do {
        ...
        if ( (rc != VTSS_ETH_LINK_OAM_RC_OK) &&
            ...
            /* LOG-Message */
            T_E(":%u occured while processing the OAM event:%u on port(%u)",
...
static void eth_link_oam_control_error_frame_event_send(const u32 port_no, BOOL *is_error_frame_xmit_needed, const vtss_port_counters_t port_counters)
{
    ...
    if (rc != VTSS_ETH_LINK_OAM_RC_OK) {
        T_E("Error:%u occured while calculating port(%u) timer values",
            rc, port_no);
        return;
    }
    if (is_timer_expired == TRUE) {
        ...
        if (rc != VTSS_ETH_LINK_OAM_RC_OK) {
            T_E("Error:%u occured while updating port(%u) timer configurations",
                rc, port_no);
            return;
        }
...
static void eth_link_oam_control_symbol_period_error_event_send(const u32 port_no, BOOL *is_symbol_period_xmit_needed, const vtss_port_counters_t port_counters)
{
    ...
    if (rc != VTSS_ETH_LINK_OAM_RC_OK) {
        ...
        T_E("Error:%u occured while calculating port(%u) timer configurations",
            rc, port_no);
        return;
    }
    if ( is_timer_expired == TRUE)  {
...
static void eth_link_oam_control_frame_period_error_event_send(const u32 port_no, BOOL *is_frame_period_xmit_needed, const vtss_port_counters_t port_counters)
{
    BOOL                          is_timer_expired  = FALSE;
    u32                           rc;
    ...
    if (rc != VTSS_ETH_LINK_OAM_RC_OK) {
        T_E("Error:%u occured while updating port(%u) timer configurations", rc, port_no);
        return;
    }
    if (is_timer_expired == TRUE)  {
        /* Update the PDU Error Events if Any */
...
static void eth_link_oam_control_error_frame_secs_summary_event_send(const u32 port_no, BOOL *is_error_frame_secs_summary_xmit_needed, const vtss_port_counters_t port_counters)
{
    BOOL                          is_timer_expired  = FALSE;
    u32                           rc;
    ...
    do {
        ...
        for (port_no = 0; port_no < VTSS_ETH_LINK_OAM_PHYS_PORTS_CNT; port_no++) {
            ...
            if ( (rc == VTSS_ETH_LINK_OAM_RC_OK) &&
...
#endif
                if (rc != VTSS_RC_OK) {
                    T_E("Error:%u occured while getting port(%u) counters",
                        rc, port_no);
                ...
                if (rc != VTSS_ETH_LINK_OAM_RC_OK) {
                    T_E("Unable to retrive the PDU max length of port(%u)",
                        port_no);
                    vtss_eth_link_oam_crit_oper_data_unlock();
                    continue;
                }
                ...
                if ((is_error_frame_xmit_needed == TRUE)   ||
                    ...
                    if (oam_pdu == NULL) {
                        T_E("Unable to allocate memory to send out frame on port(%u)",
                            port_no);
                        continue; //It's should be fatal
                    }
...
static void eth_link_oam_port_change_callback(vtss_port_no_t port_no, port_info_t *info)
{
    ...
    message = malloc(sizeof(vtss_eth_link_oam_message_t));
    if (message != NULL) {
        if (info->link) {
...
vtss_rc eth_link_oam_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_SWITCH_ADD:
        T_D("SWITCH_ADD");
        // Register here with external modules.

        //Get the system MAC address
        (void)conf_mgmt_mac_addr_get(sys_mac_addr, 0);

        //Restore the saved configuration
        if ( (blk = conf_sec_open(CONF_SEC_GLOBAL,
                                  CONF_BLK_ETH_LINK_OAM_CONF_TABLE, &size)) == NULL ||
             (size != sizeof(*blk))
           ) {

            T_W("conf_sec_open failed or size mismatch, creating defaults");
            blk = conf_sec_create(CONF_SEC_GLOBAL, CONF_BLK_ETH_LINK_OAM_CONF_TABLE, sizeof(*blk));
            blk->version = 0;
            set_conf_to_default(blk);
        }
        apply_configuration(blk);
