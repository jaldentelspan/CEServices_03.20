...
static vtss_rc rc_conv(int vcl_rc)
{
    ...
    case VTSS_VCL_ERROR_ENTRY_WITH_DIFF_VLAN:
        ...
        break;
    default:
        T_E("Invalid VCL module error is noticed:- %d\n", vcl_rc);
        break;
    }
    return rc;
}
...
vtss_rc vcl_mac_vlan_vce_entry_add(vcl_mac_vlan_msg_cfg_t *conf)
{
    vtss_vce_t           vce;
    u32                  i;
    ...
    if (conf == NULL) {
        T_E("vcl_mac_vlan_vce_entry_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }

    /* Check for valid VCE ID */
    if (vce_id >= VTSS_VCL_ID_END) {
        T_E("vcl_mac_vlan_vce_entry_add: invalid VCE ID\n");
        return VCL_ERROR_PARM;
    }

    memset(&vce, 0, sizeof(vtss_vce_t));
    /* Prepare key */
    vce.key.type = VTSS_VCE_TYPE_ANY;
...
vtss_rc vcl_proto_vlan_vce_entry_add(vcl_proto_vlan_local_sid_conf_t *conf)
{
    vtss_vce_t           vce;
    vtss_rc              rc = VTSS_RC_OK;
    ...
    /* Check for NULL pointer */
    if (conf == NULL) {
        T_E("vcl_proto_vlan_vce_entry_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }

    /* Check for valid VCE ID */
    if (vce_id >= VTSS_VCL_ID_END) {
        T_E("vcl_proto_vlan_vce_entry_add: invalid VCE ID\n");
        return VCL_ERROR_PARM;
    }
...
vtss_rc vcl_ip_vlan_vce_entry_add(vcl_ip_vlan_msg_cfg_t *conf, vtss_vce_id_t next_id)
{
    vtss_vce_t           vce;
    vtss_port_no_t       port;
    vtss_rc              rc = VTSS_RC_OK;
    ...
    /* Check for NULL pointer */
    if (conf == NULL) {
        T_E("vcl_ip_vlan_vce_entry_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    memset(&vce, 0, sizeof(vtss_vce_t));
    /* Prepare key */
                       ...
                       const vtss_module_id_t modid, const ulong isid)
{
    ...
    case VCL_MAC_VLAN_MSG_ID_CONF_SET_REQ: {
        ...
        T_D("vcl_msg_rx: VCL_MAC_VLAN_MSG_ID_CONF_SET_REQ\n");
    ...
    case VCL_MAC_VLAN_MSG_ID_CONF_ADD_REQ: {
        ...
        T_D("vcl_msg_rx: VCL_MAC_VLAN_MSG_ID_CONF_ADD_REQ mac is %02x:%02x:%02x:%02x:%02x:%02x,entry.vid = %d, vce_id = %u\n",
        ...
        if (rc == VTSS_RC_OK) {
            ...
            if (rc != VTSS_RC_OK) {
                T_W("VCE Add failed on the hardware. Please delete the entry from control database. To delete, use the command VCL"
                    "Mac Del command\n");
    ...
    case VCL_MAC_VLAN_MSG_ID_CONF_DEL_REQ: {
        vcl_msg_conf_mac_vlan_add_del_req_t *msg;
        vtss_vce_id_t                       vce_id;
        T_D("vcl_msg_rx: VCL_MAC_VLAN_MSG_ID_CONF_DEL_REQ\n");
        msg = (vcl_msg_conf_mac_vlan_add_del_req_t *)rx_msg;
    ...
    case VCL_PROTO_VLAN_MSG_ID_CONF_SET_REQ: {
        ...
        u32                                     id = 0;
        vtss_vce_id_t                           vce_id;
        T_D("vcl_msg_rx: VCL_PROTO_VLAN_MSG_ID_CONF_SET_REQ\n");
    ...
    case VCL_PROTO_VLAN_MSG_ID_CONF_ADD_REQ: {
        ...
        T_D("vcl_msg_rx: VCL_PROTO_VLAN_MSG_ID_CONF_ADD_REQ\n");
    ...
    case VCL_PROTO_VLAN_MSG_ID_CONF_DEL_REQ: {
        vcl_msg_conf_proto_vlan_add_del_req_t   *msg;
        vtss_vce_id_t                           vce_id;
        T_D("vcl_msg_rx: VCL_PROTO_VLAN_MSG_ID_CONF_DEL_REQ\n");
        msg = (vcl_msg_conf_proto_vlan_add_del_req_t *)rx_msg;
    ...
    case VCL_IP_VLAN_MSG_ID_CONF_SET_REQ: {
        ...
        T_D("vcl_msg_rx: VCL_IP_VLAN_MSG_ID_CONF_SET_REQ\n");
    ...
    case VCL_IP_VLAN_MSG_ID_CONF_ADD_REQ: {
        ...
        T_D("vcl_msg_rx: VCL_IP_VLAN_MSG_ID_CONF_ADD_REQ\n");
    ...
    case VCL_IP_VLAN_MSG_ID_CONF_DEL_REQ: {
        vcl_msg_conf_ip_vlan_add_del_req_t   *msg;
        vtss_vce_id_t                        vce_id;
        T_D("vcl_msg_rx: VCL_IP_VLAN_MSG_ID_CONF_DEL_REQ\n");
        msg = (vcl_msg_conf_ip_vlan_add_del_req_t *)rx_msg;
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            if (ports[sit.isid - VTSS_ISID_START][pit.iport - VTSS_PORT_NO_START] == 1) {
                if ((rc1 = vlan_mgmt_port_conf_set(sit.isid, pit.iport, &vlan_cfg, VLAN_USER_VCL)) != VTSS_OK) {
                    rc += rc1;
                    T_W("%u:%d: Unable to change VLAN tx tag type", sit.isid, pit.iport + 1);
                } /* if (vlan_mgmt_port_conf_set(sit.isid, pit.iport, &vlan_cfg, VLAN_USER_VCL) */
            } /* if (ports[sit.isid - VTSS_ISID_START][pit.iport - VTSS_PORT_NO_START] == 1)  */
        } /* while (port_iter_getnext(&pit)) */
    } /* while (switch_iter_getnext(&sit)) */
    return rc;
...
static vtss_rc vcl_stack_vcl_mac_vlan_conf_set(vtss_isid_t isid)
{
    ...
    T_D("vcl_stack_vcl_mac_vlan_conf_set: enter, isid: %d", isid);
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        for (indx = 0; indx < VCL_MAC_VLAN_MAX_ENTRIES; indx++) {
            ...
            if (rc == VTSS_RC_OK) {
                ...
                if (vcl_mac_vlan_set_vlan_tx_tag(entry.l2ports, TRUE) != VTSS_RC_OK) {
                    T_W("vcl_mac_vlan_set_vlan_tx_tag failed while applying configuration\n");
                } /* if (vcl_mac_vlan_set_vlan_tx_tag */
        ...
        T_D("vcl_stack_vcl_mac_vlan_conf_set: sending message to isid %d\n", sit.isid);
        msg->count = cnt;
        /* The below function also frees the msg after tx */
        vcl_msg_tx(msg, sit.isid, sizeof(*msg));
    }
    T_D("vcl_stack_vcl_mac_vlan_conf_set: exit, isid: %d", isid);
    return VTSS_RC_OK;
}
...
static vtss_rc vcl_stack_vcl_proto_vlan_conf_set(vtss_isid_t isid)
{
    ...
    T_D("vcl_stack_vcl_proto_vlan_conf_set: enter, isid: %d", isid);
    (void) switch_iter_init(&sit, isid, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        ...
        T_D("vcl_stack_vcl_proto_vlan_conf_set: sending message to isid %d\n", sit.isid);
        msg->count = cnt;
        /* The below function also frees the msg after tx */
        vcl_msg_tx(msg, sit.isid, sizeof(*msg));
    }
    T_D("vcl_stack_vcl_proto_vlan_conf_set: exit\n");
    return VTSS_RC_OK;
}
...
static vtss_rc vcl_stack_vcl_ip_vlan_conf_set(vtss_isid_t isid)
{
    ...
    T_D("vcl_stack_vcl_ip_vlan_conf_set: enter, isid: %d", isid);
    ...
    for (isid_temp = isid_start; isid_temp < isid_end; isid_temp++) {
        ...
        T_D("vcl_stack_vcl_ip_vlan_conf_set: sending message to isid %d\n", isid_temp);
        msg->count = cnt;
        /* The below function also frees the msg after tx */
        vcl_msg_tx(msg, isid_temp, sizeof(*msg));
    }
    T_D("vcl_stack_vcl_ip_vlan_conf_set: exit, isid: %d", isid);
    return VTSS_RC_OK;
}
...
static void vcl_mac_conf_read_stack(BOOL create)
{
    ...
    if ((do_create == 1) && (vcl_blk != NULL)) {
        ...
        if (vcl_mac_vlan_set_vlan_tx_tag(l2ports, FALSE) != VTSS_RC_OK) {
            T_W("vcl_mac_vlan_set_vlan_tx_tag failed while setting defaults\n");
        }
        changed = TRUE;
    }
    if (do_create == 0) {
        if (vcl_blk == NULL) {
            ...
            for (i = 0; i < vcl_blk->count; i++) {
                ...
                if ((rc = rc_conv(vcl_mac_vlan_entry_add(&mac_vlan_entry, &id, VCL_MAC_VLAN_USER_STATIC)))
                    == VTSS_RC_OK) {
                } /* if ((rc = */
            } /* for (i = 0; */
        } /* if (vcl_blk == */
    ...
    if (changed) {
        rc = vcl_stack_vcl_mac_vlan_conf_set(VTSS_ISID_GLOBAL);
    }
    if (rc != VTSS_OK ) {
        return;
    }
...
static void vcl_proto_conf_read_stack(BOOL create)
{
    ...
    T_D("vcl_proto_conf_read_stack: enter, create: %d", create);

    /* Read/create VCL Proto table configuration */
    blk_id = CONF_BLK_VCL_PROTO_VLAN_PRO_TABLE;
    if (((vcl_blk = conf_sec_open(CONF_SEC_GLOBAL, blk_id, &size)) == NULL) || (size != sizeof(*vcl_blk))) {
        T_W("vcl_proto_conf_read_stack: conf_sec_open failed or size mismatch, creating defaults");
        vcl_blk = conf_sec_create(CONF_SEC_GLOBAL, blk_id, sizeof(*vcl_blk));
        do_create = 1;
    } else if (vcl_blk->version != VCL_BLK_VERSION) {
        T_W("vcl_proto_conf_read_stack: version mismatch, creating defaults");
        do_create = 1;
    } else {
        do_create = create;
    }
    ...
    if (do_create == 0) {
        if (vcl_blk == NULL) {
            ...
            for (i = 0; i < vcl_blk->count; i++) {
                ...
                if ((rc = rc_conv(vcl_proto_vlan_proto_entry_add(&proto_vlan_entry,
                                                                 VCL_PROTO_VLAN_USER_STATIC))) != VTSS_RC_OK) {
                    T_N("vcl_proto_vlan_proto_entry_add Failed\n");
                } /* if ((rc = */
            } /* for (i = 0; */
        } /* if (vcl_blk == */
    } /* if (do_create */
...
static void vcl_proto_vlan_conf_read_stack(BOOL create)
{
    ...
    T_D("vcl_proto_vlan_conf_read_stack: enter, create: %d", create);

    /* Read/create VCL Proto table configuration */
    blk_id = CONF_BLK_VCL_PROTO_VLAN_VID_TABLE;
    if (((vcl_blk = conf_sec_open(CONF_SEC_GLOBAL, blk_id, &size)) == NULL) || (size != sizeof(*vcl_blk))) {
        T_W("vcl_proto_vlan_conf_read_stack: conf_sec_open failed or size mismatch, creating defaults");
        vcl_blk = conf_sec_create(CONF_SEC_GLOBAL, blk_id, sizeof(*vcl_blk));
        do_create = 1;
    } else if (vcl_blk->version != VCL_BLK_VERSION) {
        T_W("vcl_proto_vlan_conf_read_stack: version mismatch, creating defaults");
        do_create = 1;
    } else {
        do_create = create;
    }
    ...
    if (do_create == 0) {
        if (vcl_blk == NULL) {
            ...
            for (i = 0; i < vcl_blk->count; i++) {
                ...
                if ((rc = rc_conv(vcl_proto_vlan_group_entry_add(vlan_entry->isid, &entry, VCL_PROTO_VLAN_USER_STATIC)))
                    != VTSS_RC_OK) {
                    T_N("vcl_proto_vlan_proto_entry_add Failed\n");
                } /* if ((rc = */
            } /* for (i = 0; */
    ...
    if (changed == TRUE) {
        ...
        rc = vcl_stack_vcl_proto_vlan_conf_set(VTSS_ISID_GLOBAL);
        if (rc != VTSS_RC_OK) {
            T_D("vcl_stack_vcl_proto_vlan_conf_set returned error\n");
        }
    }
    return;
}
...
static void vcl_ip_conf_read_stack(BOOL create)
{
    ...
    if (do_create == 0) {
        if (vcl_blk == NULL) {
            ...
            for (i = 0; i < vcl_blk->count; i++) {
                ...
                if ((rc = rc_conv(vcl_ip_vlan_entry_add(&ip_vlan_entry, VCL_IP_VLAN_USER_STATIC)))
                    == VTSS_RC_OK) {
                } /* if ((rc = */
            } /* for (i = 0; */
        } /* if (vcl_blk == */
    ...
    if (changed) {
        rc = vcl_stack_vcl_ip_vlan_conf_set(VTSS_ISID_GLOBAL);
    }
    if (rc != VTSS_OK ) {
        return;
    }
...
u32 vcl_stack_vcl_proto_vlan_conf_add_del(vtss_isid_t isid, vcl_proto_vlan_entry_t *conf, BOOL add)
{
    ...
    T_D("vcl_stack_vcl_proto_vlan_conf_add_del: enter, isid: %d", isid);
    VCL_CRIT_ASSERT_LOCKED();
    msg = vcl_msg_alloc(1);
    if (add) {
    ...
    T_D("vcl_stack_vcl_proto_vlan_conf_add_del: exit, isid: %d", isid);
    return VTSS_RC_OK;
}
/**
...
static vtss_rc vcl_stack_vcl_mac_vlan_conf_add_del(vtss_isid_t isid, vcl_mac_vlan_msg_cfg_t *cfg, BOOL add)
{
    ...
    T_D("vcl_stack_vcl_mac_vlan_conf_add_del: enter, isid: %d", isid);
    ...
    T_D("vcl_stack_vcl_mac_vlan_conf_add_del: exit, isid: %d", isid);
...
vtss_rc vcl_mac_vlan_save_config(void)
{
    ...
    while ((rc_conv(vcl_mac_vlan_entry_get(&entry.smac, VCL_MAC_VLAN_USER_STATIC, next, first, &entry)))
           == VTSS_RC_OK) {
        T_D("mac is %02x:%02x:%02x:%02x:%02x:%02x, vid = %d\n", entry.smac.addr[0], entry.smac.addr[1], entry.smac.addr[2],
                                       ...
                                       vcl_mac_vlan_user_t user)
{
    ...
    do {
        ...
        if (mac_vlan_entry == NULL) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_add: NULL pointer\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        /**
        ...
        if ((user < VCL_MAC_VLAN_USER_STATIC) || (user >= VCL_MAC_VLAN_USER_ALL)) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_add: Invalid User\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        ...
        if (conf_mgmt_mac_addr_get(system_mac, 0) == 0) {
            ...
            if (i == 6) {
                T_D("vcl_mac_vlan_mgmt_mac_vlan_add: System MAC address cannot be used for MAC-based VLAN\n");
                rc = VCL_ERROR_PARM;
                break;
            }
        }
        ...
        if ((mac_vlan_entry->smac.addr[0] & 0x1) == 1) {
            T_D("vcl_mac_vlan_mgmt_mac_vlan_add: Multicast or Broadcast MAC address cannot be used for MAC-based VLAN\n");
            rc = VCL_ERROR_PARM;
            break;
        }

        T_D("vcl_mac_vlan_mgmt_mac_vlan_add ENTER: mac: %02x:%02x:%02x:%02x:%02x:%02x, vid: %u, isid_add: %u, user %d\n",
        ...
        if ((rc = rc_conv(vcl_mac_vlan_entry_add(&entry, &indx, user))) == VTSS_RC_OK) {
            ...
            if (rc == VTSS_RC_OK) {
                ...
                if ((rc = vcl_mac_vlan_set_vlan_tx_tag(entry.l2ports, TRUE)) != VTSS_RC_OK) {
                    T_W("vcl_mac_vlan_set_vlan_tx_tag FAILED\n");
                }
            ...
            T_W("MAC-based VLAN Table full\n");
        }
    } while (0);

    T_D("vcl_mac_vlan_mgmt_mac_vlan_add EXIT\n");
    return rc;
}
                                       ...
                                       vcl_mac_vlan_user_t user)
{
    ...
    do {
         ...
         **/
        if (mac_addr == NULL) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_del: NULL pointer\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        /**
         ...
         **/
        if ((user < VCL_MAC_VLAN_USER_STATIC) || (user >= VCL_MAC_VLAN_USER_ALL)) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_del: Invalid User\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        T_D("vcl_mac_vlan_mgmt_mac_vlan_del ENTER: mac: %02x:%02x:%02x:%02x:%02x:%02x, isid_del: %u, user %d\n",
        ...
        if ((rc = rc_conv(vcl_mac_vlan_entry_get(mac_addr, user,
            ...
            T_D("vcl_mac_vlan_mgmt_mac_vlan_del: entry doesn't exist\n");
            rc = VCL_ERROR_ENTRY_NOT_FOUND;
            break;
        }
        /**
        ...
        if ((rc = rc_conv(vcl_mac_vlan_entry_del(&entry, &indx, user))) == VTSS_RC_OK) {
            ...
            if (rc == VTSS_RC_OK) {
                ...
                } /* if (rc_conv(vcl_mac_vlan_entry_get */
            } /* if (rc == VTSS_RC_OK */
        } /* if ((rc = rc_conv(vcl_mac_vlan_entry_del(&entry, &indx, user) */
    } while (0);

    T_D("vcl_mac_vlan_mgmt_mac_vlan_del EXIT\n");
    return rc;
}

vtss_rc vcl_mac_vlan_mgmt_mac_vlan_get(vtss_isid_t isid_get, vcl_mac_vlan_mgmt_entry_get_cfg_t *mac_vlan_entry,
                                       vcl_mac_vlan_user_t user, BOOL next, BOOL first)
{
    vcl_mac_vlan_conf_entry_t      entry;
    vtss_rc                        rc = VTSS_RC_OK;
    ...
    do {
         ...
         **/
        if (mac_vlan_entry == NULL) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_get: NULL pointer\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        /**
         ...
         **/
        if ((user < VCL_MAC_VLAN_USER_STATIC) || (user > VCL_MAC_VLAN_USER_ALL)) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_get: Invalid User\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        //TODO - Display MAC address, VID and ports as well.
        T_D("vcl_mac_vlan_mgmt_mac_vlan_get ENTER: isid_get: %u, user %d\n", isid_get, user);
    ...
    T_D("vcl_mac_vlan_mgmt_mac_vlan_get EXIT\n");
    return rc;
}
...
vtss_rc vcl_mac_vlan_mgmt_mac_vlan_get_by_key(vtss_isid_t isid_get, u32 indx,
                                              vcl_mac_vlan_mgmt_entry_get_cfg_t *mac_vlan_entry, vcl_mac_vlan_user_t user)
{
    vcl_mac_vlan_conf_entry_t      entry;
    vtss_rc                        rc = VTSS_RC_OK;
    ...
    do {
         ...
         **/
        if ((user < VCL_MAC_VLAN_USER_STATIC) || (user >= VCL_MAC_VLAN_USER_ALL)) {
            T_E("vcl_mac_vlan_mgmt_mac_vlan_get_by_key: Invalid User\n");
            rc = VCL_ERROR_PARM;
            break;
        }
        /**
        ...
        if (indx >= VCL_MAC_VLAN_MAX_ENTRIES) {
            T_D("vcl_mac_vlan_mgmt_mac_vlan_get_by_key: Invalid Index\n");
            rc = VTSS_VCL_ERROR_PARM;
            break;  /* break from do-while */
        }
        //TODO - Display MAC address, VID and ports as well.
        T_D("vcl_mac_vlan_mgmt_mac_vlan_get_by_key ENTER: isid_get: %u, user %d\n", isid_get, user);
        /**
         * Call the base function to fetch the Mac-based VLAN entry.
         **/
    ...
    T_D("vcl_mac_vlan_mgmt_mac_vlan_get_by_key EXIT\n");
    return rc;
}
                                      ...
                                      vcl_proto_vlan_user_t            user)
{
    ...
    /* Check for NULL pointer */
    if (proto_grp == NULL) {
        T_E("vcl_proto_vlan_mgmt_proto_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_proto_add: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    T_D("vcl_proto_vlan_mgmt_proto_add: Enter\n");
    ...
    T_D("vcl_proto_vlan_mgmt_proto_add: EXIT\n");
    return rc;
}
                                         ...
                                         vcl_proto_vlan_user_t   user)
{
    ...
    /* Check for NULL pointer */
    if (proto == NULL) {
        T_E("vcl_proto_vlan_mgmt_proto_delete: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_proto_delete: Invalid User\n");
        return VCL_ERROR_PARM;
    }
                                      ...
                                      BOOL                             first)
{
    ...
    /* Check for NULL pointer */
    if (proto_grp == NULL) {
        T_E("vcl_proto_vlan_mgmt_proto_get: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_proto_get: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if ((rc = rc_conv(vcl_proto_vlan_proto_entry_get(proto_grp, user, next, first)))
        == VTSS_RC_OK) {
    }
    return rc;
}
                                            ...
                                            vcl_proto_vlan_user_t             user)
{
    ...
    /* Check for NULL pointer */
    if (grp_vlan == NULL) {
        T_E("vcl_proto_vlan_mgmt_group_entry_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_group_entry_add: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if (!(VTSS_ISID_LEGAL(isid_add) || (isid_add == VTSS_ISID_GLOBAL))) {
        T_E("Invalid ISID (%u). LEGAL expected", isid_add);
                                               ...
                                               vcl_proto_vlan_user_t             user)
{
    ...
    /* Check for NULL pointer */
    if (grp_vlan == NULL) {
        T_E("vcl_proto_vlan_mgmt_group_entry_delete: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_group_entry_delete: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if (!(VTSS_ISID_LEGAL(isid_del) || (isid_del == VTSS_ISID_GLOBAL))) {
        T_E("Invalid ISID (%u). LEGAL expected", isid_del);
                                            ...
                                            BOOL                              first)
{
    ...
    /* Check for NULL pointer */
    if (grp_vlan == NULL) {
        T_E("vcl_proto_vlan_mgmt_vlan_get: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_PROTO_VLAN_USER_STATIC) || (user >= VCL_PROTO_VLAN_USER_ALL)) {
        T_E("vcl_proto_vlan_mgmt_vlan_get: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if (!VTSS_ISID_LEGAL(isid_get) || (isid_get == VTSS_ISID_GLOBAL)) {
        T_E("Invalid ISID (%u). LEGAL expected", isid_get);
...
vtss_rc vcl_proto_vlan_mgmt_local_entry_get(vcl_proto_vlan_local_sid_conf_t *entry, u32 id, BOOL next)
{
    ...
    if ((rc = rc_conv(vcl_proto_vlan_local_entry_get(entry, id, next))) != VTSS_RC_OK) {
        T_N("vcl_proto_vlan_mgmt_local_entry_get Failed\n");
    }
    return rc;
}
/* Debug function */
                                         ...
                                         BOOL                              next)
{
    ...
    if ((rc = rc_conv(vcl_proto_vlan_hw_entry_get(isid_get, entry, id, user, next))) != VTSS_RC_OK) {
        T_N("vcl_proto_vlan_mgmt_hw_entry_get Failed\n");
    }
    return rc;
}
...
static vtss_rc vcl_stack_vcl_ip_vlan_conf_add_del(vtss_isid_t isid, vcl_ip_vlan_msg_cfg_t *cfg, BOOL add)
{
    ...
    T_D("vcl_stack_vcl_ip_vlan_conf_add_del: enter, isid: %u", isid);
    ...
    T_D("vcl_stack_vcl_ip_vlan_conf_add_del: exit, isid: %u", isid);
...
static vtss_rc vcl_ip_vlan_conf_commit(void)
{
    ...
    T_D("vcl_ip_vlan_conf_commit ENTER\n");
    blk_id = CONF_BLK_VCL_IP_VLAN_TABLE;
    if ((vce_blk = conf_sec_open(CONF_SEC_GLOBAL, blk_id, NULL)) == NULL) {
                                     ...
                                     vcl_ip_vlan_user_t           user)
{
    ...
    if (ip_vlan_entry == NULL) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_add: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_IP_VLAN_USER_STATIC) || (user >= VCL_IP_VLAN_USER_ALL)) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_add: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    ...
    T_D("isid = %u, user = %d, vce_id = %u\n", isid_add, user, ip_vlan_entry->vce_id);
    /* Check for valid VCE ID */
    if (ip_vlan_entry->vce_id > VCL_IP_VLAN_MAX_ENTRIES) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_add: Invalid VCE ID\n");
        return VCL_ERROR_PARM;
    }
    if ((ip_vlan_entry->mask_len > 32) || (ip_vlan_entry->mask_len == 0)) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_add: Mask length cannot be either greater than 32 or less than 1\n");
        return VCL_ERROR_PARM;
    }
    if ((ip_vlan_entry->vid > VLAN_MAX) || (ip_vlan_entry->vid == 0)) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_add: Invalid VLAN ID\n");
        return VCL_ERROR_PARM;
    }
    T_D("vcl_ip_vlan_mgmt_ip_vlan_add: ip = %u, mask length = %u, vid = %u vce id = %u\n",
    ...
    if (ip_vlan_entry->vce_id == 0) {
        if ((rc = rc_conv(vcl_ip_vlan_vce_id_get(&vce_id))) != VTSS_RC_OK) {
            T_D("vcl_ip_vlan_vce_id_get failed");
            return rc;
        }
        T_D("vcl_ip_vlan_mgmt_ip_vlan_add auto-generated vce_id = %u\n", vce_id);
    }
    entry.vce_id = vce_id;
    if ((rc = rc_conv(vcl_ip_vlan_entry_add(&entry, user))) == VTSS_RC_OK) { /* add or update */
        if (rc == VTSS_RC_OK && user == VCL_IP_VLAN_USER_STATIC) {
            if ((rc = vcl_ip_vlan_conf_commit()) != VTSS_RC_OK) {
                ...
                T_D("vcl_ip_vlan_mgmt_ip_vlan_add Error - vcl_ip_vlan_conf_commit failed\n");
                return rc;
            } /* if ((rc = vcl_ip_vlan_conf_commit()) != VTSS_RC_OK) */
        } /* if (rc == VTSS_RC_OK && user == VCL_IP_VLAN_USER_STATIC) */
        ...
        if ((rc = rc_conv(vcl_ip_vlan_entry_get(&entry, user, FALSE, FALSE))) == VTSS_RC_OK) {
            ...
            (void) switch_iter_init(&sit, isid_add, SWITCH_ITER_SORT_ORDER_ISID);
            while (switch_iter_getnext(&sit)) {
                T_D("vcl_stack_vcl_ip_vlan_conf_add_del for isid = %u\n", sit.isid);
                ...
                if ((rc = vcl_stack_vcl_ip_vlan_conf_add_del(sit.isid, &cfg, TRUE)) != VTSS_RC_OK) {
                    T_D("stack message sent failed for isid = %d\n", sit.isid);
                } /* if ((rc = vcl_stack_vcl_ip_vlan_conf_add_del(sit.isid, &cfg, TRUE)) != VTSS_RC_OK) */
            } /* while (switch_iter_getnext(&sit)) */
        } /* if ((rc = rc_conv(vcl_ip_vlan_entry_get(&entry, user, FALSE, FALSE))) == VTSS_RC_OK) */
    } /* if ((rc = rc_conv(vcl_ip_vlan_entry_add(&entry, user))) == VTSS_RC_OK) */
    T_D("vcl_ip_vlan_mgmt_ip_vlan_add: EXIT\n");
    return rc;
}
                                     ...
                                     vcl_ip_vlan_user_t         user)
{
    ...
    /* Check for NULL pointer */
    if (ip_vlan_entry == NULL) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_del: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_IP_VLAN_USER_STATIC) || (user >= VCL_IP_VLAN_USER_ALL)) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_del: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if (!(VTSS_ISID_LEGAL(isid_del) || (isid_del == VTSS_ISID_GLOBAL))) {
        T_E("Invalid ISID (%u). LEGAL expected", isid_del);
    ...
    /* Check for valid VCE ID */
    if (ip_vlan_entry->vce_id > VCL_IP_VLAN_MAX_ENTRIES) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_del: Invalid VCE ID\n");
        return VCL_ERROR_PARM;
    }
    T_D("vcl_ip_vlan_mgmt_ip_vlan_del: Enter\n");
    entry.vce_id = ip_vlan_entry->vce_id;
    T_D("vcl_ip_vlan_mgmt_ip_vlan_del isid_del = %u, user = %u, vce_id = %u\n", isid_del, user, ip_vlan_entry->vce_id);
    ...
    if ((rc = rc_conv(vcl_ip_vlan_entry_del(&entry, user))) == VTSS_RC_OK) {
        if (rc == VTSS_OK && user == VCL_IP_VLAN_USER_STATIC) {
            if ((rc = vcl_ip_vlan_conf_commit()) != VTSS_RC_OK) {
                T_D("vcl_ip_vlan_mgmt_ip_vlan_del Error - vcl_ip_vlan_conf_commit failed\n");
                return rc;
            } /* if ((rc = vcl_ip_vlan_conf_commit()) != VTSS_RC_OK) */
        } /* if (rc == VTSS_OK && user == VCL_IP_VLAN_USER_STATIC) */
        cfg.ip_addr = ip_vlan_entry->ip_addr;
    ...
    T_D("vcl_ip_vlan_mgmt_ip_vlan_del: Exit\n");
    return rc;
}
                                     ...
                                     BOOL                                 next)
{
    ...
    /* Check for NULL pointer */
    if (ip_vlan_entry == NULL) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_get: NULL pointer\n");
        return VCL_ERROR_PARM;
    }
    /* Check for valid User */
    if ((user < VCL_IP_VLAN_USER_STATIC) || (user >= VCL_IP_VLAN_USER_ALL)) {
        T_E("vcl_ip_vlan_mgmt_ip_vlan_get: Invalid User\n");
        return VCL_ERROR_PARM;
    }
    if (!(VTSS_ISID_LEGAL(isid_get))) {
        T_E("Invalid ISID (%u). LEGAL expected", isid_get);
    ...
    T_D("vcl_ip_vlan_mgmt_ip_vlan_get: isid_get = %u, user = %d, first = %d, next = %d, vce_id = %u\n",
    ...
    T_D("vcl_ip_vlan_mgmt_ip_vlan_get: Exit\n");
...
vtss_rc vcl_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        if ((rc = VCL_icfg_init()) != VTSS_OK) {
            T_D("Calling vcl_icfg_init() failed rc = %s", error_txt(rc));
        }
#endif
        break;
    case INIT_CMD_START:
        T_D("vcl_init : INIT_CMD_START\n");
    ...
    case INIT_CMD_CONF_DEF:
        T_D("vcl_init : INIT_CMD_CONF_DEF\n");
    ...
    case INIT_CMD_MASTER_UP:
        T_D("vcl_init : INIT_CMD_MASTER_UP\n");
    ...
    case INIT_CMD_MASTER_DOWN:
        T_D("vcl_init : INIT_CMD_MASTER_DOWN\n");
        break;
    case INIT_CMD_SWITCH_ADD:
        T_D("vcl_init : INIT_CMD_SWITCH_ADD\n");
        ...
        if (rc != VTSS_RC_OK) {
            T_D("vcl_stack_vcl_ip_vlan_conf_set returned error\n");
            return VTSS_RC_ERROR;
        }
        rc = vcl_stack_vcl_mac_vlan_conf_set(isid);
        if (rc != VTSS_RC_OK) {
            T_D("vcl_stack_vcl_mac_vlan_conf_set returned error\n");
            return VTSS_RC_ERROR;
        }
        break;
    case INIT_CMD_SWITCH_DEL:
        T_D("vcl_init : INIT_CMD_SWITCH_DEL\n");
        break;
    default:
        break;
    }
