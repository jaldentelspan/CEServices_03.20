...
//
//  WATCH OUT: DO NOT MIX WITH NON-SM FUNCTIONS, DUE TO THE #if/#endif
//  SURROUNDING THE BLOCK OF FUNCTIONS!!!
//
/****************************************************************************/
...
/*****************************************************************/
/* Description:Setup PVLAN port isolation configuration via      */ 
/* switch API.                                                   */
/* - Calling API function vtss_isolated_ports_set()              */
/* Input:                                                        */
/* Output: Return VTSS_OK if success                             */
/*****************************************************************/
static vtss_rc PI_conf_set(BOOL *conf)
{
    T_D("PVLAN port isolation change");
    return vtss_isolated_port_members_set(NULL, conf);
}
...
static BOOL PVLAN_msg_rx(void *contxt, const void * const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
    ...
    case PVLAN_MSG_ID_ISOLATE_CONF_SET_REQ:
    {
        ...
        if (changed) {
            ...
            T_D("Calling PI_conf_set from PVLAN_MSG_ID_ISOLATE_CONF_SET_REQ with isid %u",isid);
            (void)PI_conf_set(conf);
        }
        PVLAN_CRIT_EXIT();
        break;
    }
...
static void PVLAN_conf_read_stack(BOOL create) 
{
    ...
    blk_id = CONF_BLK_PVLAN_TABLE;
    if ((pvlan_blk = conf_sec_open(CONF_SEC_GLOBAL, blk_id, &size)) == NULL || 
        size != sizeof(*pvlan_blk)) {
        T_W("conf_sec_open failed or size mismatch, creating defaults");
...
#if VTSS_SWITCH_STACKABLE
    // Never isolate stack ports.
    if (port_isid_info_get(isid, &pinfo) == VTSS_RC_OK) {
        local_member[pinfo.stack_port_0] = 0;
        local_member[pinfo.stack_port_1] = 0;
    }
#endif
...
vtss_rc pvlan_mgmt_pvlan_add(vtss_isid_t isid_add, pvlan_mgmt_entry_t *pvlan_mgmt_entry)
{
    ...
    // Convert the pvlan_mgmt_entry_t to a SM_entry_conf_t
    if(isid_add == VTSS_ISID_GLOBAL) {
        isid_min = VTSS_ISID_START;
        isid_max = VTSS_ISID_END;
    } else {
