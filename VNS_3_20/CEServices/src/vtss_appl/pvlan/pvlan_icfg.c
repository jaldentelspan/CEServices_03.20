...
i8 *pvlan_ids_to_txt(u32 pvlan_ids[PVLAN_ID_END], i8 *buf)
{
    u32             pvlan_id, first_pvlan = 1;
    i8              *pvlan_buf, p[10];
    BOOL            first_pvlan_found = FALSE, more_vlans = FALSE;;

    if ((pvlan_buf = (i8 *)malloc(PVLAN_IDS_BUF_LEN)) == NULL) {
        memset(buf, 0, PVLAN_IDS_BUF_LEN);
        return buf;
    }
    memset(pvlan_buf, 0, sizeof(pvlan_buf));
    for (pvlan_id = PVLAN_ID_START; pvlan_id <= PVLAN_ID_END; pvlan_id++) {
        ...
        } /* if ((pvlan_id == VLAN_TAG_MAX_VALID) || (!VLAN_IS_BIT_SET(vlan_mask, pvlan_id))) */
    } /* for (pvlan_id = VLAN_TAG_MIN_VALID; pvlan_id <= VLAN_TAG_MAX_VALID; pvlan_id++) */
                                    ...
                                    vtss_icfg_query_result_t *result)
{
    ...
    for (pvlan_id = PVLAN_ID_START; pvlan_id < PVLAN_ID_END; pvlan_id++) {
        ...
        if ((rc = pvlan_mgmt_pvlan_get(isid, pvlan_id - 1, &conf, 0)) != VTSS_RC_OK) {
            continue; 
...
vtss_rc PVLAN_icfg_init(void)
{
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_PVLAN_PORT_CONF, PVLAN_ICFG_port_conf);
