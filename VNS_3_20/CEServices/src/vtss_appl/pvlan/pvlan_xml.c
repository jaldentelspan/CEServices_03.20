...
static vtss_rc pvlan_cx_gen_func(cx_get_state_t *s)
{
    ...
    case CX_GEN_CMD_SWITCH:
        /* Switch - PVLAN */
        T_D("switch - pvlan");
        cx_add_tag_line(s, CX_TAG_PVLAN, 0);
...
#if defined(PVLAN_SRC_MASK_ENA)
        {
            ...
            cx_add_tag_line(s, CX_TAG_PVLAN_TABLE, 0);

            /* Entry syntax */
            cx_add_stx_start(s);
            cx_add_stx_ulong(s, "pvlan_id", 1, VTSS_PVLANS);
            cx_add_stx_port(s);
            cx_add_stx_end(s);
            ...
            while ((first && pvlan_mgmt_pvlan_get(s->isid, conf.privatevid, &conf, 0) == VTSS_OK) || 
                ...
                cx_add_attr_ulong(s, "pvlan_id", (conf.privatevid + 1));
                cx_add_attr_txt(s, "port", mgmt_iport_list2txt(conf.ports, buf));
                CX_RC(cx_add_attr_end(s, CX_TAG_ENTRY));
            }
            cx_add_tag_line(s, CX_TAG_PVLAN_TABLE, 1);
        }
#endif /* PVLAN_SRC_MASK_ENA */
