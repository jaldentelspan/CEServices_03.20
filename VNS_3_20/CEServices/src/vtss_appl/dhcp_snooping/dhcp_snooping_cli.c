    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (req->clear) {
                ...
                CPRINTF("\n--------------------\n");
                cli_cmd_stati("Rx Discover", "Tx Discover", stats.discover_rx, stats.discover_tx);
                cli_cmd_stati("Rx Offer", "Tx Offer", stats.offer_rx, stats.offer_tx);
                cli_cmd_stati("Rx Request", "Tx Request", stats.request_rx, stats.request_tx);
                cli_cmd_stati("Rx Decline", "Tx Decline", stats.decline_rx, stats.decline_tx);
                cli_cmd_stati("Rx ACK", "Tx ACK", stats.ack_rx, stats.ack_tx);
                cli_cmd_stati("Rx NAK", "Tx NAK", stats.nak_rx, stats.nak_tx);
                cli_cmd_stati("Rx Release", "Tx Release", stats.release_rx, stats.release_tx);
                cli_cmd_stati("Rx Inform", "Tx Inform", stats.inform_rx, stats.inform_tx);
                cli_cmd_stati("Rx Lease Query", "Tx Lease Query", stats.leasequery_rx, stats.leasequery_tx);
                cli_cmd_stati("Rx Lease Unassigned", "Tx Lease Unassigned", stats.leaseunassigned_rx, stats.leaseunassigned_tx);
                cli_cmd_stati("Rx Lease Unknown", "Tx Lease Unknown", stats.leaseunknown_rx, stats.leaseunknown_tx);
                cli_cmd_stati("Rx Lease Active", "Tx Lease Active", stats.leaseactive_rx, stats.leaseactive_tx);
                CPRINTF("Rx Discarded from Untrusted: %9u\n", stats.discard_untrust_rx);
            }
        }
    }
}
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Configuration",
    NULL,
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Mode",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Mode [enable|disable]",
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Port Mode [<port_list>]",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Port Mode [<port_list>] [trusted|untrusted]",
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Statistics [<port_list>]",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Snooping Statistics [<port_list>] [clear]",
...
cli_cmd_tab_entry(
    "Debug DHCP Snooping IP Assigned Information",
    NULL,
    "Show DHCP snooping IP assigned informaiton",
