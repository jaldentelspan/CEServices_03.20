...
static void afi_tx_done(void *context, packet_tx_done_props_t *props)
{
    ...
    if (idx >= ARRSZ(afi_frame_ptrs)) {
        diag_printf("Internal Error: idx = %u. Expected [0; %zu]\n", idx, ARRSZ(afi_frame_ptrs));
        return;
    }
    ...
    if (!match) {
        diag_printf("Error: AFI frame pointer (idx = %u) didn't match. Called with %p, found %p in afi_frame_ptrs[] array\n", idx + 1, props->frm_ptr[0], p.frm_ptr);
        return;
    }

    if (p.state != AFI_UNDER_CANCELLATION) {
        diag_printf("Error: AFI frame (idx = %u, frm_ptr = %p) was not under cancellation (state = %d)\n", idx + 1, props->frm_ptr[0], p.state);
    }

    diag_printf("AFI Frame (idx = %u) cancelled.", idx + 1);
#if defined(VTSS_FEATURE_AFI_FDMA)
    if (p.counting_enabled) {
        diag_printf(" Transmitted %llu times.", props->frm_cnt);
    }

    if (p.sequence_enabled) {
        diag_printf(" Last sequence number = %u = 0x%08x.", props->afi_last_sequence_number, props->afi_last_sequence_number);
    }
#endif /* defined(VTSS_FEATURE_AFI_FDMA) */
...
static int32_t PACKET_cli_parse_vid(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
  ...
  req->parm_parsed = 1;
  error = cli_parse_ulong(cmd, &value, 0, VTSS_VID_RESERVED);
  if (!error) {
