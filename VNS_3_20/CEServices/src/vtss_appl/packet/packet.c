...
#include "vtss_mep_api.h"    /* For VTSS_MEP_INSTANCE_MAX */
...
#if defined(VTSS_MEP_INSTANCE_MAX) && VTSS_MEP_INSTANCE_MAX > 0
#define AFI_BUF_CNT (2 * VTSS_MEP_INSTANCE_MAX)
#else
#define AFI_BUF_CNT 100
#endif /* defined(VTSS_MEP_INSTANCE_MAX) && VTSS_MEP_INSTANCE_MAX > 0 */
...
// These two hold the counters for jumbo frames, which are discarded silently
static u64 rx_bytes_jumbo = 0;
...
/****************************************************************************/
static PACKET_INLINE u32 CX_ntohl(u8 *p)
{
...
static BOOL RX_filter_validate(const packet_rx_filter_t *filter)
{
    ...
    if ((filter->match & PACKET_RX_FILTER_MATCH_UDP_DST_PORT) && (filter->match & PACKET_RX_FILTER_MATCH_IPV4_PROTO) && (filter->ipv4_proto != IP_PROTO_UDP || filter->ipv4_proto_mask != 0)) {
        T_E("Filter matches against UDP port range, but ipv4_proto is not set to IP_PROTO_UDP");
        return FALSE;
    }
...
static BOOL RX_filter_insert(const packet_rx_filter_t *filter, void **filter_id)
{
    ...
    if ((new_l = malloc(sizeof(packet_rx_filter_item_t))) == NULL) {
        T_E("malloc(subscr_list_item_t) failed");
        return FALSE;
    }
    ...
    if (new_l->filter.match & PACKET_RX_FILTER_MATCH_UDP_DST_PORT) {
...
static int RX_do_callback(packet_rx_filter_item_t *l, u8 *data_ptr, vtss_fdma_list_t *packet)
{
    ...
    if (l->filter.cb) {
        ...
        l->filter.cb_adv(l->filter.contxt, data_ptr, packet->rx_info, packet);
...
#define PACKET_MAX_CALLBACK_TIME_MSECS 3000
    if (tick_cnt > VTSS_OS_MSEC2TICK(PACKET_MAX_CALLBACK_TIME_MSECS)) {
        T_W("Module %s has spent more than %d msecs (%llu msecs) in its Packet Rx callback", vtss_module_names[l->filter.modid], PACKET_MAX_CALLBACK_TIME_MSECS, VTSS_OS_TICK2MSEC(tick_cnt));
    }
...
static PACKET_INLINE BOOL RX_dispatch(vtss_fdma_list_t *packet)
{
    ...
    if (!RX_filter_list) {
        T_DG(TRACE_GRP_RX, "Discarding. No listeners");
        return detach_buffers; // No filters.
    }
    ...
    // Get the semaphore protecting the filter list
    ...
    l = RX_filter_list;
    while (l) {
        packet_rx_filter_t *filter = &l->filter;
        match  = filter->match;
        ...
        if (match & PACKET_RX_FILTER_MATCH_SRC_PORT) {
            if (VTSS_PORT_BF_GET(filter->src_port_mask, rx_info->port_no) == 0) {
...
no_match:
    ...
    if (match_default && !at_least_one_match) {
        while (l) {
            packet_rx_filter_t *filter = &l->filter;
            if (filter->match & PACKET_RX_FILTER_MATCH_DEFAULT) {
                at_least_one_match = TRUE;
...
static void RX_thread(cyg_addrword_t data)
{
    ...
    while (1) {
        ...
        while (vtss_fifo_cnt(&RX_fifo) == 0) {
            ...
            cyg_flag_wait(&RX_flag, 1, CYG_FLAG_WAITMODE_AND | CYG_FLAG_WAITMODE_CLR); // This waits for bit 0 in the flags to become 1 and clears them when that happens
            cyg_scheduler_lock();
        }
...
static void TX_done_thread(cyg_addrword_t data)
{
    ...
    while (1) {
        ...
        while (vtss_fifo_cnt(&TX_done_fifo) == 0) {
            cyg_scheduler_unlock();
            cyg_flag_wait(&TX_done_flag, 1, CYG_FLAG_WAITMODE_AND | CYG_FLAG_WAITMODE_CLR); // This waits for bit 0 in the flags to become 1 and clears them when that happens
            cyg_scheduler_lock();
        }
...
static void TX_do_tx(packet_tx_props_t *tx_props)
{
    ...
    // Save info that has to survice until the user's Tx done gets called back.
...
static void TX_pend_thread(cyg_addrword_t data)
{
    ...
    while (1) {
        ...
        if (vtss_fifo_cp_cnt(&TX_pend_fifo) == 0) {
            cyg_cond_wait(&TX_pend_cond); // Unlocks the TX_pend_mutex and waits for condition variable to be signaled, at which point in time it locks the mutex again.
        }
...
static void DBG_cmd_stat_module_print(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    dbg_printf("Tx packet buffers:\n");
    dbg_printf("Allocations  : %10u\n", a);
    dbg_printf("Deallocations: %10u\n", f);
    dbg_printf("Outstanding  : %10u\n\n", a - f);
}
...
{
    dbg_printf("Obsolete. Use 'debug api fdma'\n");
}
...
static void DBG_cmd_stat_port_print(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    for (prio = 0; prio < VTSS_PRIOS; prio++) {
        dbg_printf("Rx Prio %d  ", prio);
    }
    dbg_printf("Rx Super   Tx\n");
    ...
    for (iport = 0; iport <= VTSS_PORTS; iport++) {
        ...
        if (iport == VTSS_PORTS) {
            ...
            dbg_printf("Unknown   "); // Typically sFlow frames.
        } else {
            dbg_printf("%9u ", iport2uport(iport));
        }
        for (prio = 0; prio <= VTSS_PRIOS; prio++) {
            dbg_printf("%10u ", CX_port_counters.rx_pkts[idx][prio]);
        }
        if (iport == VTSS_PORTS) {
            dbg_printf("%10s ", "N/A");
        } else {
            dbg_printf("%10u ", CX_port_counters.tx_pkts[idx]);
        }
        dbg_printf("\n");
    }
    dbg_printf("Switched  ");
    ...
    dbg_printf("%10u ", CX_port_counters.tx_pkts[VTSS_PORTS]);
    dbg_printf("\nMulticast ");
    for (prio = 0; prio <= VTSS_PRIOS; prio++) {
        dbg_printf("%10s ", "N/A");
    }
    dbg_printf("%10u ", CX_port_counters.tx_pkts[VTSS_PORTS + 1]);
#if VTSS_SWITCH_STACKABLE
    for (iport = VTSS_GLAG_NO_START; iport < VTSS_GLAG_NO_START + VTSS_GLAGS; iport++) {
        dbg_printf("\nGLAG #%-2d  ", iport);
        for (prio = 0; prio <= VTSS_PRIOS; prio++) {
            dbg_printf("%10u ", CX_port_counters.rx_pkts[iport - VTSS_GLAG_NO_START + VTSS_PORTS][prio]);
        }
        dbg_printf("%10s ", "N/A");
    }
#endif /* VTSS_SWITCH_STACKABLE */
...
static void DBG_cmd_stat_packet_clear(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    dbg_printf("Packet module statistics cleared!\n");
}
...
static void DBG_cmd_stat_fdma_clear(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    dbg_printf("FDMA statistics cleared!\n");
}
...
static void DBG_cmd_stat_port_clear(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    PACKET_RX_FILTER_CRIT_EXIT();
    dbg_printf("Port statistics cleared!\n");
}
...
{
    if (parms_cnt == 0) {
        dbg_printf("Stack trace is currently %sabled\n", CX_stack_trace_ena ? "en" : "dis");
    } else if (parms_cnt == 1) {
...
vtss_rc packet_tx(packet_tx_props_t *tx_props)
{
...
#if VTSS_SWITCH_STACKABLE
        if (tx_props->tx_info.tx_vstax_hdr != VTSS_PACKET_TX_VSTAX_NONE) {
            // If the frame contains a stack header, then only one port can be specified and it must be a stack port
            PACKET_CHECK(port_cnt == 1 && (port_no == PORT_NO_STACK_0 || port_no == PORT_NO_STACK_1), return VTSS_RC_ERROR;);
        }
#endif /* VTSS_SWITCH_STACKABLE */
...
#if defined(VTSS_FEATURE_AFI_FDMA) || defined(VTSS_FEATURE_AFI_SWC)
    PACKET_CHECK(tx_props->fdma_info.afi_fps >= 0 && tx_props->fdma_info.afi_fps <= VTSS_AFI_FPS_MAX, return VTSS_RC_ERROR;);
    if (tx_props->fdma_info.afi_fps > 0) {
    ...
    if (tx_props->packet_info.filter.enable) {
        ...
        // We need to transfer additional filter information through the Tx Pend FIFO,
        // so we reuse a from-now-on unused field.
        // #src_port will become 0 if we shouldn't tag the frame, and otherwise the Ethertype of the tag.
        tx_props->tx_info.tag.tpid = filter[port_no].filter == VTSS_PACKET_FILTER_TAGGED ? filter[port_no].tpid : 0;
    }
...
u8 *packet_tx_alloc(size_t size)
{
    ...
    if ((buffer = malloc(size + VTSS_FDMA_HDR_SIZE_BYTES /* IFH */ + 4 /* FCS */))) {
...
u8 *packet_tx_alloc_extra(size_t size, size_t extra_size_dwords, u8 **extra_ptr)
{
    ...
    size = MAX(VTSS_FDMA_MIN_FRAME_SIZE_BYTES - FCS_SIZE_BYTES, size);
    if ((buffer = malloc(size + VTSS_FDMA_HDR_SIZE_BYTES /* IFH */ + 4 /* FCS */ + extra_size_bytes))) {
        *extra_ptr = buffer;
...
/******************************************************************************/
/******************************************************************************/
static packet_dbg_cmd_t packet_dbg_cmds[] = {
    {
...
void packet_dbg(packet_dbg_printf_t dbg_printf, u32 parms_cnt, u32 *parms)
{
    ...
    if (parms_cnt == 0) {
        dbg_printf("Usage: debug packet <cmd idx>\n");
