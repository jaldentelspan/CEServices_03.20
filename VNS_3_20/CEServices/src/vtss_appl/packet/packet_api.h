...
#define PACKET_RX_FILTER_MATCH_UDP_DST_PORT 0x00000200 /* packet_rx_filter_t member(s): udp_dst_port_min, upd_dst_port_max */
#define PACKET_RX_FILTER_MATCH_SFLOW        0x00000400 /* packet_rx_filter_t member(s): None                               */
#define PACKET_RX_FILTER_MATCH_IP_ANY       0x00000800 /* packet_rx_filter_t member(s): None. Matches IPv4, IPv6, ARP      */
...
typedef struct {
  ...
  // NOTE 2): The callback function must NOT call any of the packet_rx_filter_XXX()
  //          functions (a deadlock would occur).
  //   Luton26: Y
  //   Luton28: Y
  //   Jaguar1: Y
  //   Serval : Y
...
typedef struct {
   ...
   * dynamically allocated with malloc() or allocated with
   * packet_tx_alloc().
