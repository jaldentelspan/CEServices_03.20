                      ...
                      int exact, size_t *var_len, WriteMethod **write_method)
{
    ...
    T_NG(TRACE_GRP_SNMP, "entering var_lldpConfiguration");
...
static BOOL get_next_lldpLocManAddrTableEntryByKey(lldpLocManAddrTable_entry_t *table_entry)
{
    ...
    (void) ip_mgmt_conf_cur_get( &conf );
#endif /* VTSS_SW_OPTION_IP2 */
...
#else
        if ( addr_key >= htonl(conf.ipv4_addr) ) {
            return FALSE;
        }
#endif /* VTSS_SW_OPTION_IP2 */
    }
...
#else
    table_entry->lldpLocManAddrSubtype = IANA_ADDR_IPV4;
    table_entry->lldpLocManAddr_len = sizeof(addr_key);
    addr_key = htonl(conf.ipv4_addr);
#endif
...
#else
    table_entry->lldpLocManAddrIfId = 1;
#endif
                              ...
                              int exact)
{
    ...
    table_info.ifIndex = lldp_loc_port_num - 1;
    if (FALSE == ifIndex_get_next_valid(&table_info)) {
                                   ...
                                   WriteMethod **write_method)
{
    ...
    table_info.ifIndex = idx_num - 1;
    if (FALSE == ifIndex_get_next(&table_info) || (table_info.type != IFTABLE_IFINDEX_TYPE_PORT
                                                   && table_info.type != IFTABLE_IFINDEX_TYPE_LLAG && table_info.type != IFTABLE_IFINDEX_TYPE_GLAG)) {
        return VTSS_UNSPECIFIED_ERROR;
    }
                     ...
                     size_t *var_len, WriteMethod **write_method)
{
    ...
    */
    ...
    case LLDPLOCPORTDESC: {
        ...
        strcpy(VAR_STR, &port_desc_str[0]);
...
void snmpLLDPNotificationChange(vtss_isid_t isid, int port_index)
{
    ...
    if (msg_uptime_get(VTSS_ISID_LOCAL) - last_notification_change_time > lldp_mgmt_get_notification_interval(FALSE) ) {
