...
static BOOL port_get_next(ifIndex_id_t startIdx, iftable_info_t *info, BOOL check_exist)
{
    ...
    if ( isid >= VTSS_ISID_END || !msg_switch_exists(isid) || if_id >= port_isid_port_count(isid) ) {
        return FALSE;
    }
...
//    T_D("enter isid = %d, if_id = %d, port_count = %d", (int)isid, (int)if_id, (int)port_isid_port_count(isid));
    ...
    if ( isid >= VTSS_ISID_END || !msg_switch_exists(isid) ) {
        return FALSE;
    }
...
//    T_D("enter isid %d ID %d ", (int)info->isid, (int)if_id);
    ...
    if ( FALSE == check_exist ) {
        ...
        if (vlan_mgmt_vlan_get(VTSS_ISID_GLOBAL, vlan_mgmt_entry.vid, &vlan_mgmt_entry, TRUE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP)
            != VTSS_OK) {
            return FALSE;
        }
...
BOOL ifIndex_get( iftable_info_t *info )
{
    ...
    if ( info->ifIndex != ifIndex_get_by_cb( ifIndex_cb_id, info, TRUE ) ) {
        return FALSE;
    }
...
//    T_D("enter ifIndex = %lu", info->ifIndex);
    ...
    if ( info->ifIndex != ifIndex_get_by_cb( ifIndex_cb_id, info, FALSE ) ) {
        return FALSE;
    }
