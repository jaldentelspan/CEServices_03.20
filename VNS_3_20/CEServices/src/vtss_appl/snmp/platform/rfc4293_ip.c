...
static BOOL get_next_ipIfStatsTableEntryByKey(ipIfStatsTable_entry_t *table_entry)
{
    ...
    if ((u32)tmp.ipIfStatsIfIndex < info.ifIndex && VTSS_OK != vtss_ip2_cntr_intf_stat_iter_first( &version, &vidx, &entry)) {
        return FALSE;
...
#define ARP_MAX 1024
    ...
    status_v4 = calloc(ARP_MAX, sizeof(vtss_neighbour_status_t));
    ...
    status_v6 = calloc(ARP_MAX, sizeof(vtss_neighbour_status_t));
...
#define SIZE 2048
    ...
    if (entry->rt.type == VTSS_ROUTING_ENTRY_TYPE_IPV6_UC) {
        return FALSE;
    }

    rts_v4 = calloc(SIZE, sizeof(vtss_routing_status_t));
    if ( !rts_v4 ) {
        return FALSE;
    }

    rts_v6 = calloc(SIZE, sizeof(vtss_routing_status_t));
    if ( !rts_v6 ) {
        goto free_v4_return;
    }
...
static BOOL get_next_ipv6InterfaceTableEntryByKey(ipv6InterfaceTable_entry_t *table_entry)
{
    ...
    table_entry->ipv6InterfaceIdentifier_len = sizeof(if_status.u.ipv6.net.address.addr);
    memcpy (table_entry->ipv6InterfaceIdentifier, ipv6_addr, table_entry->ipv6InterfaceIdentifier_len );
