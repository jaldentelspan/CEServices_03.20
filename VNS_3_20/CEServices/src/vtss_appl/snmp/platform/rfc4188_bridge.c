...
next_vid:
    while ( VTSS_OK == vlan_mgmt_vlan_get(VTSS_ISID_GLOBAL, vlan_mgmt_entry.vid, &vlan_mgmt_entry, TRUE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP)) {
...
static int get_dot1dTpFdbTableEntry_by_mac (vtss_vid_mac_t *vid_mac)
{
    ...
    if (get_next_dot1dTpFdbTableEntry_by_mac(&buf) != VTSS_RC_OK) {
        return VTSS_RC_ERROR;
    }
