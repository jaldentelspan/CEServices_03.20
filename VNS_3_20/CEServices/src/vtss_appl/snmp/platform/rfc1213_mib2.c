...
/* peter, 2007/8, move to snmp_custom_api.h
#define SNMP_CONF_SYS_OBJECT_ID     {1, 3, 6, 1, 4, 1, 6300} */
...
static oid system_object_id[] = SNMP_CONF_SYS_OBJECT_ID;
           ...
           int exact, size_t *var_len, WriteMethod **write_method)
{
    ...
    case SYSORLASTCHANGE: {
        struct timeval sysOR_LastChangeTimeval;
        ucd_snmp_get_sysOR_LastChange(&sysOR_LastChangeTimeval);
        ulong_ret = (sysOR_LastChangeTimeval.tv_sec * 100) + (sysOR_LastChangeTimeval.tv_usec / 10000);
    ...
    case SYSOBJECTID: {
        *var_len = sizeof(system_object_id);
        return (u_char *)system_object_id;
    }
    case SYSDESCR: {
        char *string_p;
...
extern ulong get_vlan_num(void);
...
ulong rfc1213_get_ifNumber(void)
{
    ...
    while (TRUE == ifIndex_get_next(&table_info)) {
        if (IFTABLE_IFINDEX_TYPE_VLAN == table_info.type) {
            ...
            table_info.if_id = VLAN_MAX;
            (void) ifIndex_get_by_interface((&table_info));
            if_number += get_vlan_num();
        } else {
            if_number++;
        }
    }
...
static BOOL update_ifTable_entry(vtss_isid_t isid, vtss_port_no_t port_idx, ifTable_entry_t *table_entry_p, char mac_address[ETHER_ADDR_LEN], BOOL is_vlan)
{
    ...
    if (!memcmp(mac_address, table_entry_p->ifPhysAddress, ETHER_ADDR_LEN)) {
        for (mac_idx = 5, mac_add_value = port_idx; mac_idx >= 0; mac_idx--) {
...
BOOL get_ifTable_entry(iftable_info_t *table_info, ifTable_entry_t *table_entry_p)
{
    ...
    if (msg_switch_is_master()) {
        ...
        if ((tsl_p = malloc(sizeof(topo_switch_list_t))) &&
    ...
    case IFTABLE_IFINDEX_TYPE_VLAN:
        ...
        while (switch_iter_getnext(&sit)) {
            ...
            if (vlan_mgmt_vlan_get(sit.isid, table_info->if_id, &vlan_mgmt_entry, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
    ...
    case IFTABLE_IFINDEX_TYPE_IP: {
        ...
        struct arpcom  *ac;
        int i = -1;  /* then acquire up to date information and deal with the relevent keys (only): */
        struct ether_drv_stats x; /* struct ether_drv_stats declaration in eth_drv_stats.h */
        ...
        table_info->if_id = 1;
#endif /* VTSS_SW_OPTION_IP_ROUTING */
        ...
        if (NULL != ifnet_p->if_ioctl) {
            i = (*ifnet_p->if_ioctl)(ifnet_p, SIOCGIFSTATSUD, (caddr_t)&x);
        }

        ac = (struct arpcom *)ifnet_p;

        table_entry_p->ifMtu             = ifnet_p->if_mtu;
        bcopy(&ac->ac_enaddr, table_entry_p->ifPhysAddress, ETHER_ADDR_LEN);
        table_entry_p->ifAdminStatus     = 1; /* ifAdminStatus is always enabled (read-only) for IP interfaces */
        table_entry_p->ifOperStatus      = (ifnet_p->if_flags & IFF_UP) ? 1 : 2;
        //table_entry_p->ifLastChange      = 0;
        table_entry_p->ifInOctets        = ifnet_p->if_ibytes;
        table_entry_p->ifInUcastPkts     = ifnet_p->if_ipackets - ifnet_p->if_imcasts;
        table_entry_p->ifInNUcastPkts    = ifnet_p->if_imcasts;
        table_entry_p->ifInDiscards      = ifnet_p->if_iqdrops;
        table_entry_p->ifInErrors        = ifnet_p->if_ierrors;
        table_entry_p->ifInUnknownProtos = ifnet_p->if_noproto;
        table_entry_p->ifOutOctets       = ifnet_p->if_obytes;
        table_entry_p->ifOutUcastPkts    = ifnet_p->if_opackets - ifnet_p->if_omcasts;
        table_entry_p->ifOutNUcastPkts   = ifnet_p->if_omcasts;
        table_entry_p->ifOutDiscards     = x.tx_dropped;
        table_entry_p->ifOutErrors       = ifnet_p->if_oerrors;
        table_entry_p->ifOutQLen         = x.tx_queue_len;
#endif
        memset(table_entry_p->ifSpecific, 0x0, sizeof(table_entry_p->ifSpecific));
        table_entry_p->ifSpecific_len    = 2;
        break;
...
vtss_rc rfc1213_set_ifAdminStatus(vtss_isid_t isid, vtss_port_no_t port_no, BOOL status)
{
    ...
    port_mgmt_conf_get(isid, port_no, &port_conf);
