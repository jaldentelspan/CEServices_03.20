...

{
    ...
    memset(&conf, 0x0, sizeof(ip_conf_t));
#endif /* VTSS_SW_OPTION_IP2 */
    mode = table_index_2;
    if (mode) {
...
//            ipv4_value = *(vtss_ipv4_t*)&intf_param.active_querier;
        }
#else
        if (ip_mgmt_conf_get(&conf) < 0) {
            return FALSE;
        }
        ipv4_value = conf.ipv4_addr;
#endif /* VTSS_SW_OPTION_IP2 */
...
#else
            memcpy(table_entry->mgmdHostInterfaceQuerier, (char *)conf.ipv6_addr.addr, 16);
#endif /* VTSS_SW_OPTION_IP2 */
#endif
        } else {
...
#else
            memcpy(table_entry->mgmdRouterInterfaceQuerier, (char *)conf.ipv6_addr.addr, 16);
#endif /* VTSS_SW_OPTION_IP2 */
#endif
        } else {
