...
static cyg_int32 handler_config_trap(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        while (trap_mgmt_conf_get_next(&entry) == VTSS_OK) {
            ...
            sprintf(search_str, "delete_%s", entry.trap_conf_name);
...
#ifdef VTSS_SW_OPTION_IPV6
            ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%c%s/%d/%u/%s/%u", ch, entry.trap_conf_name, conf->enable, conf->trap_version,
...
//    uchar       str_buff[16];
    ...
    (void) cgi_unescape(var_string, trap_conf_name, len, sizeof(trap_conf_name));
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        */
        ...
        while (VTSS_RC_OK == trap_mgmt_conf_get_next(&trap_entry)) {
            if ( i++ == 0 ) {
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%s", trap_entry.trap_conf_name);
            } else {
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "|%s", trap_entry.trap_conf_name);
            }
            cyg_httpd_write_chunked(p->outbuffer, ct);
...
#ifdef VTSS_SW_OPTION_IPV6
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer), ",%s/%d/%u/%s/%s/%d/%u/%u/%u",
                      trap_entry.trap_conf_name, conf->enable,
                      conf->trap_version,
                      encoded_string,
