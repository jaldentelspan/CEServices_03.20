...
#define IFINDEX_OID {1, 3, 6, 1, 2, 1, 2, 2, 1, 1, 1, 0}
#define IFADMIN_OID {1, 3, 6, 1, 2, 1, 2, 2, 1, 1, 7, 0}
#define IFOPER_OID  {1, 3, 6, 1, 2, 1, 2, 2, 1, 1, 8, 0}
...
/*
        args[0] = snmp_trap_buff[trap_buff_read_idx].trap_if_info.if_idx;
        args[1] = snmp_trap_buff[trap_buff_read_idx].trap_if_info.if_admin;
        args[2] = snmp_trap_buff[trap_buff_read_idx].trap_if_info.if_oper;
*/
...
static vtss_rc trap_event_send_snmpTraps(int trap_type, long *args)
{
    BOOL enable;
    ...
    struct variable_list ifIndex_var, ifAdmin_var, ifOper_var;
    iftable_info_t info;
    ...
    (void) trap_mgmt_mode_get(&enable);
    ...
    ifIndex_oid[ifTable_len - 1] = args[0];
    ifAdmin_oid[ifTable_len - 1] = args[0];
    ifOper_oid[ifTable_len - 1] = args[0];
    ...
    ( void )snmp_set_var_value( &ifIndex_var, (u_char *)&args[0], sizeof(long) );
    ( void )snmp_set_var_value( &ifAdmin_var, (u_char *)&args[1], sizeof(long) );
    ( void )snmp_set_var_value( &ifOper_var, (u_char *)&args[2], sizeof(long) );
...
static vtss_rc trap_event_send_vars(snmp_vars_trap_entry_t *trap_entry_vars)
{
    ...
    // Therefore we use an SNMP allocation function, rather than malloc().
...
static vtss_rc snmpv3_engine_views_set(snmpv3_views_conf_t *conf, BOOL is_add)
{
    ...
    if (is_add) {
        memset(subtree_mask, 0xFF, sizeof(subtree_mask));
        sprintf(buf, "%s %s %s %s",
                conf->view_name,
                conf->view_type == SNMPV3_MGMT_VIEW_INCLUDED ? "included" : "excluded",
                misc_oid2str(conf->subtree, conf->subtree_len, subtree_mask, conf->subtree_len),
...
//    T_E("name = %s, ID = %lu", tmp->trap_conf_name, tmp->trap_conf.conf_id);
    conf_id = tmp->trap_conf.conf_id;

    if ( TRUE == delete ) {
//        T_E("delete (ID:%s)", trap_entry->trap_conf_name);
        ...
        if ( FALSE == update ) {
//            T_E("new (ID:%s)", trap_entry->trap_conf_name);
                ...
                if (SnmpdGetAgentState()) {
                    while (trap_buff_read_idx != trap_buff_write_idx) {
                        ...
                        case SNMP_TRAP_TYPE_AUTHFAIL: {
                            ...
                            args[1] = snmp_trap_buff[trap_buff_read_idx].trap_if_info.if_admin;
                            args[2] = snmp_trap_buff[trap_buff_read_idx].trap_if_info.if_oper;
                            (void) trap_event_send_snmpTraps((int) snmp_trap_buff[trap_buff_read_idx].trap_type, args);
                        }
                        break;
                        case SNMP_TRAP_TYPE_EVENT:
                            ...
                            event_send_trap(snmp_trap_buff[trap_buff_read_idx].event_trap_entry.is_rising,
                                            snmp_trap_buff[trap_buff_read_idx].event_trap_entry.alarm_index,
                                            snmp_trap_buff[trap_buff_read_idx].event_trap_entry.value,
                                            snmp_trap_buff[trap_buff_read_idx].event_trap_entry.the_threshold,
                                            (oid *)snmp_trap_buff[trap_buff_read_idx].event_trap_entry.alarmed_var,
                                            snmp_trap_buff[trap_buff_read_idx].event_trap_entry.alarmed_var_length,
                                            snmp_trap_buff[trap_buff_read_idx].event_trap_entry.sample_type);
                        ...
                        case SNMP_TRAP_TYPE_VARS:
                            (void) trap_event_send_vars(&snmp_trap_buff[trap_buff_read_idx].trap_entry);
                            snmp_free_varbind(snmp_trap_buff[trap_buff_read_idx].trap_entry.vars);
                            break;
...
//                                T_E("probe fail");
                                ...
                                tmp->trap_conf.trap_engineid_len = snmp_trap_buff[trap_buff_read_idx].trap_conf.trap_engineid_len;
                                memcpy(tmp->trap_conf.trap_engineid, snmp_trap_buff[trap_buff_read_idx].trap_conf.trap_engineid,
...
static void snmp_conf_read_stack(BOOL create)
{
    ...
    if (do_create) {
        ...
        if (smon_stat_conf_blk_p != NULL) {
            smon_stat_conf_blk_p->snmp_smon_stat_entry_num = 0;
            memset(smon_stat_conf_blk_p->snmp_smon_stat_entry, 0x0, sizeof(smon_stat_conf_blk_p->snmp_smon_stat_entry));
        }
    }
    ...
    if (do_create) {
        ...
        if (port_copy_conf_blk_p != NULL) {
            port_copy_conf_blk_p->snmp_port_copy_entry_num = 0;
            memset(port_copy_conf_blk_p->snmp_port_copy_entry, 0x0, sizeof(port_copy_conf_blk_p->snmp_port_copy_entry));
        }
    }
    ...
    for (idx = 0; idx < SNMPV3_MAX_COMMUNITIES; idx++) {
        if (communities_conf_blk_p && communities_conf_blk_p->communities_conf[idx].valid) {
            if (communities_conf_blk_p->communities_conf[idx].storage_type != SNMP_MGMT_STORAGE_PERMANENT ||
                communities_conf_blk_p->communities_conf[idx].status != SNMP_MGMT_ROW_ACTIVE) {
                communities_conf_blk_p->communities_conf[idx].valid = 0;
                communities_conf_blk_p->communities_conf_num--;
            }
        }
    }

    if (communities_conf_blk_p) {
    ...
    for (idx = 0; idx < SNMPV3_MAX_USERS; idx++) {
        if (users_conf_blk_p && users_conf_blk_p->users_conf[idx].valid) {
            if (users_conf_blk_p->users_conf[idx].storage_type != SNMP_MGMT_STORAGE_PERMANENT ||
                users_conf_blk_p->users_conf[idx].status != SNMP_MGMT_ROW_ACTIVE) {
                users_conf_blk_p->users_conf[idx].valid = 0;
                users_conf_blk_p->users_conf_num--;
            }
        }
    }
    ...
    for (idx = 0; idx < SNMPV3_MAX_GROUPS; idx++) {
        if (groups_conf_blk_p && groups_conf_blk_p->groups_conf[idx].valid) {
            if (groups_conf_blk_p->groups_conf[idx].storage_type != SNMP_MGMT_STORAGE_PERMANENT ||
                groups_conf_blk_p->groups_conf[idx].status != SNMP_MGMT_ROW_ACTIVE) {
                groups_conf_blk_p->groups_conf[idx].valid = 0;
                groups_conf_blk_p->groups_conf_num--;
            }
        }
    }
    ...
    for (idx = 0; idx < SNMPV3_MAX_VIEWS; idx++) {
        if (views_conf_blk_p && views_conf_blk_p->views_conf[idx].valid) {
            if (views_conf_blk_p->views_conf[idx].storage_type != SNMP_MGMT_STORAGE_PERMANENT ||
                views_conf_blk_p->views_conf[idx].status != SNMP_MGMT_ROW_ACTIVE) {
                views_conf_blk_p->views_conf[idx].valid = 0;
                views_conf_blk_p->views_conf_num--;
            }
        }
    }
    ...
    for (idx = 0; idx < SNMPV3_MAX_ACCESSES; idx++) {
        if (accesses_conf_blk_p && accesses_conf_blk_p->accesses_conf[idx].valid) {
            if (accesses_conf_blk_p->accesses_conf[idx].storage_type != SNMP_MGMT_STORAGE_PERMANENT ||
                accesses_conf_blk_p->accesses_conf[idx].status != SNMP_MGMT_ROW_ACTIVE) {
                accesses_conf_blk_p->accesses_conf[idx].valid = 0;
                accesses_conf_blk_p->accesses_conf_num--;
            }
        }
    }
...
/*lint -esym(459, vlan_num) */
static ulong vlan_num = 1;

static void ifmib_vlan_changed(vtss_isid_t isid, vtss_vid_t vid, vlan_user_t user,
...
{
    if ( VLAN_ACTION_ADD == action ) {
        vlan_num++;
    } else if ( VLAN_ACTION_DELETE == action ) {
        vlan_num--;
    }
}

ulong get_vlan_num(void)
{
    return vlan_num;
}
/* Module start */
static void snmp_start(BOOL init)
{
    ...
    snmp_conf_t         *conf_p;
    snmp_port_conf_t    *port_conf;
    oid                 system_object_id[] = SNMP_CONF_SYS_OBJECT_ID;
    vtss_rc             rc;
    ...
    if (init) {
        ...
        SnmpdSetVersionIdOid(system_object_id, sizeof(system_object_id) / sizeof(oid));
        cyg_net_snmp_init();
    } else {
        ...
        vlan_membership_change_register(ifmib_vlan_changed);
    }
    T_D("exit");
}

static void traverse_vlan_num(void)
{
    iftable_info_t table_info;

    table_info.type = IFTABLE_IFINDEX_TYPE_VLAN;
    (void) ifIndex_get_first_by_type((&table_info));
    vlan_num = 1;
...
static int snmpv3_trap_callback(int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (!pdu->securityEngineIDLen && snmp_global.snmp_conf.trap_security_engineid_len) {
        pdu->securityEngineID = malloc(snmp_global.snmp_conf.trap_security_engineid_len);
    ...
    pdu->securityName = malloc(strlen(user_conf.user_name) + 1);
    if (!pdu->securityName) {
        return 1;
    }
...
void vtss_snmp_mibs_init(void)
{
    ...
    init_ieee8021BridgeMib(); /* ieee8021QBridgeMib needs to reference ieee8021BridgeBasePortTable */
    init_ieee8021QBridgeMib();
#endif
...
#endif // #if RFC2863_SUPPORTED_IFMIB ifdef
#endif /* VTSS_SW_OPTION_SMB_SNMP */
...
#endif // #ifdef IEEE8021X_SUPPORTED_MIB
...
#endif /* RFC4668_SUPPORTED_RADIUS ifdef*/
...
#endif /* RFC4670_SUPPORTED_RADIUS */
#ifdef VTSS_SW_OPTION_MSTP
#if IEEE_8021_SUPPORTED_MSTP
    init_ieee8021MstpMib();
#endif /* IEEE_8021_SUPPORTED_MSTP */
