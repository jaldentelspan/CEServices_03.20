...
BOOL get_dot3adAggTable_entry(int table_index, dot3adAggTable_entry_t *table_entry_p)
{
    ...
    for (aid = VTSS_PORT_NO_START; aid < (VTSS_LACP_MAX_AGGR + VTSS_PORT_NO_START); aid++) {
        if (lacp_get_aggr_status(aid, &lacp_aggregatorstatus)) {
    ...
    if (lacp_get_system_config(&lacp_system_config) != VTSS_OK) {
        return FALSE;
    }
    ...
    for (l2port = VTSS_PORT_NO_START; l2port < (VTSS_LACP_MAX_PORTS + VTSS_PORT_NO_START); l2port++) {
        if (lacp_aggregatorstatus.port_list[l2port - VTSS_PORT_NO_START]) {
            ...
            if (lacp_get_port_status(l2port, &lacp_portstatus) != VTSS_OK) {
                return FALSE;
            }
...
BOOL get_dot3adAggPortTable_entry(int table_index, dot3adAggPortTable_entry_t *table_entry_p)
{
    ...
    if (lacp_get_port_config(table_info.isid, table_info.if_id, &lacp_port_config) != VTSS_OK) {
        return FALSE;
    }
    ...
    if (lacp_get_port_status(L2PORT2PORT(table_info.isid, table_info.if_id), &lacp_portstatus) != VTSS_OK) {
        return FALSE;
    }
    if (aggID_ifindex == 0) {
    ...
    (void) lacp_get_aggr_status(lacp_portstatus.actor_port_aggregator_identifier, &lacp_aggregatorstatus);
                                 ...
                                 oid *name, size_t name_len)
{
    ...
    if (lacp_set_port_config(table_info.isid, table_info.if_id, &lacp_port_config) != VTSS_OK) {
        return SNMP_ERR_RESOURCEUNAVAILABLE;
    }
