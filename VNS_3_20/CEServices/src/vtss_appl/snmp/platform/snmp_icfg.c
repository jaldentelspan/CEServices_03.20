                                          ...
                                          vtss_icfg_query_result_t *result)
{
    ...
    def_conf = malloc(sizeof(conf) * num);
    ...
    while ( snmpv3_mgmt_communities_conf_get(&conf, TRUE) == VTSS_OK ) {
        ...
        rc = vtss_icfg_printf(result, "%s%s %s %s\n",
                              ...
                              conf.community, misc_ipv4_txt(conf.sip, ip_buf), misc_ipv4_txt(conf.sip_mask, ip_buf));
    }
                                     ...
                                     vtss_icfg_query_result_t *result)
{
    ...
    /* command: snmp-server
        ...
        [md5 <md5_passwd:word8-32> | sha <sha_passwd:word8-40> ] { [ priv {des | aes} <word8-32>] }
    ...
    while ( snmpv3_mgmt_users_conf_get(&conf, TRUE) == VTSS_OK ) {
        ...
        if ( conf.auth_protocol != SNMP_MGMT_AUTH_PROTO_NONE) {
            sprintf( cmd, "%s%s%s ", cmd,
                     ( conf.auth_protocol == SNMP_MGMT_AUTH_PROTO_MD5 ) ? "md5 " : "sha ", conf.auth_password);
        }

        if ( conf.priv_protocol != SNMP_MGMT_PRIV_PROTO_NONE) {
            sprintf( cmd, "%s%s%s%s ", cmd, "priv ",
                     ( conf.auth_protocol == SNMP_MGMT_PRIV_PROTO_DES ) ? "des " : "aes ", conf.priv_password);
        }
                                     ...
                                     vtss_icfg_query_result_t *result)
{
    ...
    if (req->all_defaults ||
        ...
        rc = vtss_icfg_printf(result, "%s%s\n",
                              "snmp-server engineID local ", misc_engineid2str(conf.engineid, conf.engineid_len));
    }
#endif /* SNMP_SUPPORT_V3 */
    ...
    if (req->all_defaults ||
        (strcmp(sysconf.sys_name, def_sysconf.sys_name))) {
        rc = vtss_icfg_printf(result, "%s%s\n",
                              "snmp-server sysname ", sysconf.sys_name);
    }

    if (req->all_defaults ||
        (strcmp(sysconf.sys_contact, def_sysconf.sys_contact))) {
        rc = vtss_icfg_printf(result, "%s%s\n",
                              "snmp-server contact ", sysconf.sys_contact);
    }

    if (req->all_defaults ||
        (strcmp(sysconf.sys_location, def_sysconf.sys_location))) {
        rc = vtss_icfg_printf(result, "%s%s\n",
                              "snmp-server location ", sysconf.sys_location);
    }
...
vtss_rc snmp_icfg_init(void)
{
    ...
    */
    if ((rc = vtss_icfg_query_register(VTSS_ICFG_SNMP_GLOBAL_CONF, SNMP_ICFG_global_conf)) != VTSS_OK) {
        return rc;
    }
