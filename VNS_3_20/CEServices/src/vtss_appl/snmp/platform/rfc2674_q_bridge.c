...
{
    scalar_entry->dot1qMaxVlanId = VLAN_MAX;
    scalar_entry->dot1qMaxSupportedVlans = VTSS_VIDS;
...
static int get_next_dot1qFdbId(vtss_vid_t *vid)
{
    ...
    if (vlan_mgmt_vlan_get(VTSS_ISID_GLOBAL, *vid, &vlan_mgmt_entry, TRUE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP)
        != VTSS_OK) {
...
_get_dot1qVlanCurrentTableEntry(dot1qVlanCurrentTable_entry_t *table_entry, BOOL getnext)
{
    ...
    vlan_mgmt_entry.vid = table_entry->dot1qVlanIndex;
    ...
    table_entry->dot1qVlanIndex = vlan_mgmt_entry.vid;
    table_entry->dot1qVlanFdbId = vlan_mgmt_entry.vid;
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (vlan_mgmt_port_conf_get(sit.isid, pit.iport, &port_conf, VLAN_USER_ALL) == VTSS_OK) {
                if ((port_conf.untagged_vid == vlan_mgmt_entry.vid) || (port_conf.untagged_vid == 4096)) {
                    VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanCurrentUntaggedPorts, dot1port_start + pit.iport, 1);
                } else if (port_conf.untagged_vid == 4097) {
                    if (vlan_mgmt_entry.vid != port_conf.pvid) {
                        VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanCurrentUntaggedPorts, dot1port_start + pit.iport, 1);
                    }
                } else {
                    VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanCurrentUntaggedPorts, dot1port_start + pit.iport, 0);
                }
            }
        }
    }
...
_get_dot1qVlanStaticTableEntry(dot1qVlanStaticTable_entry_t *table_entry, BOOL getnext)
{
    ...
    table_entry->dot1qVlanIndex = vlan_mgmt_entry.vid;
    strcpy(table_entry->dot1qVlanStaticName, vlan_mgmt_entry.vlan_name);
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (vlan_mgmt_port_conf_get(sit.isid, pit.iport, &port_conf, VLAN_USER_STATIC) == VTSS_OK) {
                if ((port_conf.untagged_vid == vlan_mgmt_entry.vid) || (port_conf.untagged_vid == 4096)) {
                    VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanStaticUntaggedPorts, dot1port_start + pit.iport, 1);
                } else if (port_conf.untagged_vid == 4097) {
                    if (vlan_mgmt_entry.vid != port_conf.pvid) {
                        VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanStaticUntaggedPorts, dot1port_start + pit.iport, 1);
                    }
                } else {
                    VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanStaticUntaggedPorts, dot1port_start + pit.iport, 0);
                }
            }
        }
    }
...
set_dot1qVlanStaticTableEntry(dot1qVlanStaticTable_entry_t *table_entry)
{
    ...
    if (table_entry->dot1qVlanStaticRowStatus == 4) {
        ...
        (void) vlan_mgmt_vlan_del(VTSS_ISID_GLOBAL, table_entry->dot1qVlanIndex, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP);
...
static int
set_dot1qPortVlanTableEntry(dot1qPortVlanTable_entry_t *table_entry)
{
    vlan_port_conf_t  port_conf;
    dot1Port_info_t   info;
