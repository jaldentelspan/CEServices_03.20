...
static BOOL get_or_next_vlanMemberByPort (vtss_vid_t vid, iftable_info_t *key2_info)
{
    ...
    while (switch_iter_getnext(&sit) ) {
        if (vlan_mgmt_vlan_get(sit.isid, vid, &vlan_mgmt_entry, FALSE, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
            continue;
        }
...
static BOOL get_vlanMemberByLLAG (vtss_vid_t vid, iftable_info_t *key2_info)
{
    ...
    if (vlan_mgmt_vlan_get(key2_info->isid, vid, &vlan_mgmt_entry, FALSE, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
        return FALSE;
    }
