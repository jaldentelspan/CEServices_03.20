    ...
    */
    ...
    if ( VTSS_RC_OK  == trap_mgmt_conf_get(&trap_entry)) {
        ...
        if (req->all_defaults ||
            ...
            rc = vtss_icfg_printf( result, " informs retries %lu timeout %lu\n", conf->trap_engineid, conf->trap_security_name);
        }
        ...
        if (req->all_defaults ||
            ...
            if ( event->system.warm_start || event->system.cold_start) {
                sprintf(cmd, "%s%s%s%s", cmd, "switch ", event->system.warm_start ? "warmstart " : "", event->system.cold_start ? "coldstart " : "" );
...
vtss_rc trap_icfg_init(void)
{
    ...
    */
    if ((rc = vtss_icfg_query_register(VTSS_ICFG_TRAP_GLOBAL_CONF, TRAP_ICFG_global_conf)) != VTSS_OK) {
        return rc;
    }

    if ((rc = vtss_icfg_query_register(VTSS_ICFG_TRAP_HOST_CONF, TRAP_ICFG_host_conf)) != VTSS_OK) {
        return rc;
    }

    if ((rc = vtss_icfg_query_register(VTSS_ICFG_TRAP_PORT_CONF, TRAP_ICFG_port_conf)) != VTSS_OK) {
        return rc;
    }
