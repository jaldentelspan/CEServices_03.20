                    ...
                    size_t *var_len, WriteMethod **write_method)
{
    ...
    /* simple input length check */
    if ((*length) > (etherStatsEntryFirstIndexBegin + 1)) {
        return NULL;
    }
...
#define HISTORYCONTROLOWNER     6
    {HISTORYCONTROLOWNER, ASN_OCTET_STR, RWRITE, var_historyControlTable, 3, {1, 1, 6}},
                        ...
                        size_t *var_len, WriteMethod **write_method)
{
    ...
    /* simple input length check */
    if ((*length) > (historyControlEntryFirstIndexBegin + 1)) {
        return NULL;
    }
               ...
               int exact, size_t *var_len, WriteMethod **write_method)
{
    ...
    /* simple input length check */
    if ((*length) > (alarmEntryFirstIndexBegin + 1)) {
        return NULL;
    }
...
#define EVENTOWNER      6
    {EVENTOWNER, ASN_OCTET_STR, RWRITE, var_eventTable, 3, {1, 1, 6}},
               ...
               int exact, size_t *var_len, WriteMethod **write_method)
{
    ...
    /* simple input length check */
    if ((*length) > (eventEntryFirstIndexBegin + 1)) {
        return NULL;
    }
