...
static oid snmp_mpd_oid[]    = {1, 3, 6, 1, 6, 3, 11, 3, 1, 1};
static int snmp_mpd_oid_len  = sizeof(snmp_mpd_oid) / sizeof(oid);
static int snmp_mpd_module_count = 0;
...
ucd_snmp_init_snmpMPDStats(void)
{
     ...
     */
#if RFC1213_SUPPORTED_ORTABLE
    if (snmp_mpd_module_count) {
        UNREGISTER_SYSOR_TABLE(snmp_mpd_oid, snmp_mpd_oid_len);
        snmp_mpd_module_count = 0;
    }
    REGISTER_SYSOR_TABLE(snmp_mpd_oid, snmp_mpd_oid_len, "The MIB for Message Processing and Dispatching");
    snmp_mpd_module_count++;
#endif /* RFC1213_SUPPORTED_ORTABLE */
}
