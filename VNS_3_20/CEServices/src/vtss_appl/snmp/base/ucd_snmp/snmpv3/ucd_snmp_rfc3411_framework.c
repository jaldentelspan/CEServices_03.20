...
static oid snmp_framework_oid[]    = {1, 3, 6, 1, 6, 3, 10, 3, 1, 1};
static int snmp_framework_oid_len  = sizeof(snmp_framework_oid) / sizeof(oid);
static int snmp_framework_module_count = 0;
...
ucd_snmp_init_snmpEngine(void)
{
     ...
     */
#if RFC1213_SUPPORTED_ORTABLE
    if (snmp_framework_module_count) {
        UNREGISTER_SYSOR_TABLE(snmp_framework_oid, snmp_framework_oid_len);
        snmp_framework_module_count = 0;
    }
    REGISTER_SYSOR_TABLE(snmp_framework_oid, snmp_framework_oid_len, "The SNMP Management Architecture MIB");
    snmp_framework_module_count++;
#endif /* RFC1213_SUPPORTED_ORTABLE */
}
