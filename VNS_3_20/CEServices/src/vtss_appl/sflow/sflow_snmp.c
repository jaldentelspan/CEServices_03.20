...
static u_char *SFLOW_snmp_cb_agent(struct variable *vp, oid *name, size_t *length, int exact, size_t *var_len, WriteMethod **write_method)
{
  ...
  *write_method = NULL;
  ...
  // sFlow agent addres is hardcoded to 127.0.0.1, as there is no way to
  // explicit configure the sFlow agent address. This is tracked in bug#10215

  // This is where we do the value assignments for the MIB results.
  switch (vp->magic) {
  case SFLOWVERSION:
    strncpy(SFLOW_snmp_global_ret.string_ret, SFLOW_MIB_VERSION, sizeof(SFLOW_snmp_global_ret.string_ret));
    *var_len = strlen(SFLOW_snmp_global_ret.string_ret);
    return (u_char *)SFLOW_snmp_global_ret.string_ret;

  case SFLOWAGENTADDRESSTYPE:
    SFLOW_snmp_global_ret.long_ret = ip_addr.type; // Can be used directly according to its definition in ip_api.h
    *var_len = sizeof(SFLOW_snmp_global_ret.long_ret);
    return (u_char *)&SFLOW_snmp_global_ret.long_ret;

  case SFLOWAGENTADDRESS:
    SFLOW_snmp_ip2bin(&ip_addr, SFLOW_snmp_global_ret.string_ret, var_len);
    return (u_char *)SFLOW_snmp_global_ret.string_ret;
