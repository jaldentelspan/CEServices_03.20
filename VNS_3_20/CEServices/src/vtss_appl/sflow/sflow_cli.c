...
/******************************************************************************/
static void SFLOW_cli_display_cfg(cli_req_t *req, BOOL show_rcvr, BOOL show_fs, BOOL show_cp)
{
  sflow_cli_req_t *sflow_req = req->module_req;
  switch_iter_t   sit;
  vtss_rc         rc;
...
static void SFLOW_cli_cmd_conf(cli_req_t *req)
{
  ...
  SFLOW_cli_display_cfg(req, TRUE, TRUE, TRUE);
}
...
static void SFLOW_cli_cmd_receiver(cli_req_t *req)
{
  ...
  if (!req->set) {
    SFLOW_cli_display_cfg(req, TRUE, FALSE, FALSE);
    return;
  }
  ...
  if (sflow_req->rcvr_hostname_specified) {
    strncpy((char *)new_rcvr.hostname, (char *)sflow_req->rcvr_hostname, sizeof(new_rcvr.hostname));
  }
...
static void SFLOW_cli_cmd_flow_sampler(cli_req_t *req)
{
  ...
  if (!req->set) {
    SFLOW_cli_display_cfg(req, FALSE, TRUE, FALSE);
    return;
  }
...
static void SFLOW_cli_cmd_counter_poller(cli_req_t *req)
{
  ...
  if (!req->set) {
    SFLOW_cli_display_cfg(req, FALSE, FALSE, TRUE);
    return;
  }
  ...
  while (cli_switch_iter_getnext(&sit, req)) {
    ...
    while (cli_port_iter_getnext(&pit, req)) {
      ...
      for (inst = inst_min; inst <= inst_max; inst++) {
        ...
        cp.enabled = sflow_req->rcvr_idx != 0;
...
static int32_t SFLOW_cli_parse_rcvr_ipaddr_str(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
  ...
  // Check for IPv6 (should be embedded in above check but would cause to many incompatibilities with existing code).
