...
static void SFLOW_msg_tx_switch_cfg(vtss_isid_t isid)
{
  ...
  msg = malloc(sizeof(sflow_msg_local_switch_cfg_t));
  if (msg == NULL) {
...
/******************************************************************************/
static vtss_rc SFLOW_check_isid_port(vtss_isid_t isid, vtss_port_no_t port, BOOL allow_local)
{
...
/****************************************************************************/
static void SFLOW_dgram_header_update(u32 rcvr_idx)
{
  sflow_rcvr_state_t *rcvr_state = &SFLOW_rcvr_state[rcvr_idx];
  sflow_rcvr_t       *rcvr_cfg   = &SFLOW_rcvr_cfg[rcvr_idx];
  u32                *ptr        = rcvr_state->dgram;
  int                family      = rcvr_state->addrinfo->ai_family;
  u64                uptime_ms;

  SFLOW_ASSERT(family == AF_INET || family == AF_INET6);
  SFLOW_ASSERT(family == AF_INET ? rcvr_state->dgram_len_dwords >= 7 : rcvr_state->dgram_len_dwords >= 10);

  // Datagram version
  SFLOW_ENCODE_32(ptr, rcvr_cfg->datagram_version);

  // IP version (1 = IPv4, 2 = IPv6)
  SFLOW_ENCODE_32(ptr, family == AF_INET ? 1 : 2);

  // TODO, hardcoded hack... fix this when bug#10215 is fixed
  SFLOW_ENCODE_32(ptr, 0x7f000001);
...
static void SFLOW_dgram_init(sflow_rcvr_state_t *rcvr_state)
{
  ...
  if (rcvr_state->addrinfo) {
    // Reserve room for a header, whose size depends on IPv4/IPv6
    SFLOW_ASSERT(rcvr_state->addrinfo->ai_family == AF_INET || rcvr_state->addrinfo->ai_family == AF_INET6);
    rcvr_state->dgram_len_dwords = rcvr_state->addrinfo->ai_family == AF_INET ? 7 : 10;
  } else {
    rcvr_state->dgram_len_dwords = 0;
  }
}
  ...
  for (isid_iter = isid_min; isid_iter <= isid_max; isid_iter++) {
    for (port_iter = port_min; port_iter <= port_max; port_iter++) {
      for (inst_iter = inst_min; inst_iter <= inst_max; inst_iter++) {
        ...
        SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].sampling_rate   = 0;
        SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].max_header_size = SFLOW_FLOW_HEADER_SIZE_DEFAULT;
        SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].type            = VTSS_SFLOW_TYPE_ALL;
      }
    }
  }
}
...
static SFLOW_INLINE void SFLOW_fs_state_update(void)
{
  ...
  while (port_iter_getnext(&pit)) {
    ...
    if (new_api_conf.sampling_rate == 0) {
      // Default to VTSS_SFLOW_TYPE_ALL
      new_api_conf.type = VTSS_SFLOW_TYPE_ALL;
    }
...
static SFLOW_INLINE BOOL SFLOW_fs_construct(u32 *frm, vtss_port_no_t sample_port, vtss_port_no_t ingr_port, u32 copy_bytes, u32 orig, u32 stripped, u32 drops)
{
    ...
    if (port_state->fs_samples_left[inst]) {
      ...
      if (type == VTSS_SFLOW_TYPE_ALL || (is_tx && type == VTSS_SFLOW_TYPE_TX) || (is_tx == FALSE && type == VTSS_SFLOW_TYPE_RX)) {
        if (--port_state->fs_samples_left[inst] == 0) {
          ...
          SFLOW_ASSERT(port_state->fs_api_conf.sampling_rate != 0);
  ...
  // sFlow-marked frames were in the pipeline port this port, when it timed out or was reconfigured. This is not an error.
  return TRUE;
}
...
static SFLOW_INLINE BOOL SFLOW_msg_cp_collect(vtss_port_no_t port, u32 instance_mask, vtss_port_if_group_counters_t *counters)
{
  ...
  // u32 if_status.  0 = Port disabled, 1 = Port enabled, but link down, 2 = Invalid, 3 = Link up.
  // In reality we should also tell whether the port is administratively
  // up or down. Unfortunately that would require a call to the port module
  // for each sample, which we don't wanna do.
  // The port module does exhibit a callback when it's being administratively
  // disabled, but we'd also need the opposite callback.
  SFLOW_ENCODE_32(ptr, port_state->port_link ? 3 : 1);
...
static void SFLOW_dgram_flush(u32 rcvr_idx)
{
  ...
  for (rcvr_idx = rcvr_idx_min; rcvr_idx <= rcvr_idx_max; rcvr_idx++) {
    ...
    if (rcvr_state->sockfd != -1 && rcvr_state->sample_cnt > 0) {
      ...
      SFLOW_dgram_header_update(rcvr_idx);

      int err = sendto(rcvr_state->sockfd, rcvr_state->dgram, rcvr_state->dgram_len_dwords * sizeof(u32), 0, rcvr_state->addrinfo->ai_addr, rcvr_state->addrinfo->ai_addrlen);
      if (err <= 0) {
...
vtss_rc sflow_mgmt_rcvr_cfg_get(u32 rcvr_idx /* 1..N */, sflow_rcvr_t *cfg, sflow_rcvr_info_t *info)
{
  ...
  if ((result = SFLOW_check_isid_port(VTSS_ISID_START, VTSS_PORT_NO_START, FALSE)) != VTSS_RC_OK) {
    return result;
  }
...
vtss_rc sflow_mgmt_flow_sampler_cfg_get(vtss_isid_t isid, vtss_port_no_t port, u16 instance, sflow_fs_t *cfg)
{
  ...
  if ((result = SFLOW_check_isid_port(isid, port, FALSE)) != VTSS_RC_OK) {
    return result;
  }
...
vtss_rc sflow_mgmt_switch_statistics_get(vtss_isid_t isid, sflow_switch_statistics_t *statistics)
{
  ...
  // Statistics reading only allowed by master
  if ((result = SFLOW_check_isid_port(isid, VTSS_PORT_NO_START, FALSE)) != VTSS_RC_OK) {
    return result;
  }
