...
typedef struct {
   ...
   *
   * By default, we therefore set the sample type to VTSS_SFLOW_TYPE_ALL,
   * but for debug reasons, we publish this interface so that it can be
   * changed (preferably only by debug CLI commands).
   */
