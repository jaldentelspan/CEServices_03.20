...
static vtss_rc cx_dot1x_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    if (ports == NULL) {
        ...
        cx_add_stx_start(s);
        cx_add_stx_port(s);
        cx_add_stx_kw(s, "state", cx_kw_dot1x_state);
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_QOS
        cx_add_stx_bool(s, "backend_qos");
#endif
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_VLAN
        cx_add_stx_bool(s, "backend_vlan");
#endif
#ifdef VTSS_SW_OPTION_NAS_GUEST_VLAN
        cx_add_stx_bool(s, "guest_vlan");
#endif
    ...
    cx_add_attr_kw(s, "state", cx_kw_dot1x_state, port_cfg->admin_state);
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_QOS
    cx_add_attr_bool(s, "backend_qos",  port_cfg->qos_backend_assignment_enabled);
#endif
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_VLAN
    cx_add_attr_bool(s, "backend_vlan", port_cfg->vlan_backend_assignment_enabled);
#endif
#ifdef VTSS_SW_OPTION_NAS_GUEST_VLAN
    cx_add_attr_bool(s, "guest_vlan",   port_cfg->guest_vlan_enabled);
#endif
...
static vtss_rc dot1x_cx_parse_func(cx_set_state_t *s)
{
    ...
    case CX_PARSE_CMD_PARM:
    {
        ...
        case CX_TAG_MODE:
            if (global) {
                cx_parse_val_bool(s, &conf.enabled, 1);
            } else {
                s->ignored = 1;
            }
            break;
        ...
        case CX_TAG_REAUTH:
            if (global) {
                cx_parse_val_bool(s, &conf.reauth_enabled, 1);
            } else {
                s->ignored = 1;
            }
            break;
        ...
        case CX_TAG_BACKEND_QOS:
            if (global) {
                cx_parse_val_bool(s, &conf.qos_backend_assignment_enabled, 1);
            } else {
                s->ignored = 1;
            }
            break;
        ...
        case CX_TAG_BACKEND_VLAN:
            if (global) {
                cx_parse_val_bool(s, &conf.vlan_backend_assignment_enabled, 1);
            } else {
                s->ignored = 1;
            }
            break;
        ...
        case CX_TAG_GUEST_VLAN_ENA:
            if (global) {
                cx_parse_val_bool(s, &conf.guest_vlan_enabled, 1);
            } else {
                s->ignored = 1;
            }
            break;
        ...
        case CX_TAG_GUEST_VLAN_ALLOW_EAPOL:
            if (global) {
                cx_parse_val_bool(s, &conf.guest_vlan_allow_eapols, 1);
            } else {
                s->ignored = 1;
            }
            break;
    ...
    case CX_PARSE_CMD_SWITCH:
        if (s->init) {
            ...
            /* NAS state */
            dot1x_mgmt_switch_cfg_get(s->isid, &dot1x_state->dot1x);
        } else if (s->apply) {
            ...
            if (chgd) {
                dot1x_mgmt_switch_cfg_set(s->isid, &switch_conf);
            }

            /* Set new NAS state */
            dot1x_mgmt_switch_cfg_set(s->isid, &dot1x_state->dot1x);
        }
        break;
...
static vtss_rc dot1x_cx_gen_func(cx_get_state_t *s)
{
    ...
    case CX_GEN_CMD_GLOBAL:
        ...
        cx_add_tag_line(s, CX_TAG_NAS, 0);
        {
            dot1x_glbl_cfg_t conf;

            if (dot1x_mgmt_glbl_cfg_get(&conf) == VTSS_RC_OK) {
                cx_add_val_bool (s, CX_TAG_MODE, conf.enabled);
                cx_add_val_bool (s, CX_TAG_REAUTH, conf.reauth_enabled);
                cx_add_val_ulong(s, CX_TAG_PERIOD, conf.reauth_period_secs, DOT1X_REAUTH_PERIOD_SECS_MIN, DOT1X_REAUTH_PERIOD_SECS_MAX);
                cx_add_val_ulong(s, CX_TAG_TIMEOUT, conf.eapol_timeout_secs, DOT1X_EAPOL_TIMEOUT_SECS_MIN, DOT1X_EAPOL_TIMEOUT_SECS_MAX);
#ifdef NAS_USES_PSEC
                cx_add_val_ulong(s, CX_TAG_AGETIME, conf.psec_aging_period_secs, NAS_PSEC_AGING_PERIOD_SECS_MIN, NAS_PSEC_AGING_PERIOD_SECS_MAX);
                cx_add_val_ulong(s, CX_TAG_HOLDTIME, conf.psec_hold_time_secs, NAS_PSEC_HOLD_TIME_SECS_MIN, NAS_PSEC_HOLD_TIME_SECS_MAX);
#endif /* NAS_USES_PSEC */
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_QOS
                cx_add_val_bool (s, CX_TAG_BACKEND_QOS, conf.qos_backend_assignment_enabled);
#endif
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_VLAN
                cx_add_val_bool (s, CX_TAG_BACKEND_VLAN, conf.vlan_backend_assignment_enabled);
#endif
#ifdef VTSS_SW_OPTION_NAS_GUEST_VLAN
                cx_add_val_bool (s, CX_TAG_GUEST_VLAN_ENA, conf.guest_vlan_enabled);
                cx_add_val_ulong(s, CX_TAG_GUEST_VID, conf.guest_vid, 1, 4095);
                cx_add_val_ulong(s, CX_TAG_REAUTH_MAX, conf.reauth_max, 1, 255);
                cx_add_val_bool (s, CX_TAG_GUEST_VLAN_ALLOW_EAPOL, conf.guest_vlan_allow_eapols);
#endif
            }
        }
