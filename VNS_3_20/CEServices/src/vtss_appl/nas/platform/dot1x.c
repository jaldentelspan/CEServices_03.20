...
/******************************************************************************/
static void DOT1X_cfg_default_switch(dot1x_switch_cfg_t *cfg)
{
  vtss_port_no_t api_port;
...
/******************************************************************************/
static void DOT1X_cfg_default_glbl(dot1x_glbl_cfg_t *cfg)
{
...
static DOT1X_INLINE void DOT1X_vlan_awareness_set(nas_port_t nas_port, nas_port_control_t admin_state)
{
  ...
  if (vlan_mgmt_port_conf_set(isid, api_port, &vlan_cfg, VLAN_USER_DOT1X) != VTSS_RC_OK) {
    T_W("%u:%d: Unable to change VLAN awareness", isid, uport);
  }
}
...
static void DOT1X_vlan_membership_set(vtss_isid_t isid, vtss_port_no_t api_port, vtss_vid_t vid, BOOL add_membership)
{
  ...
  if (vid != 0) {
    ...
    if (vlan_mgmt_vlan_get(isid, vid, &entry, FALSE, VLAN_USER_DOT1X, VLAN_MEMBERSHIP_OP) != VTSS_RC_OK) {
      ...
      if (entry.ports[api_port] == (add_membership ? VLAN_MEMBER_PORT : VLAN_NON_MEMBER_PORT)) {
        T_W("%u:%d: Expected port member of vid = %d to be %s, but it isn't", isid, uport, vid, add_membership ? "cleared" : "set");
      }
    }

    entry.ports[api_port] = add_membership ? VLAN_MEMBER_PORT : VLAN_NON_MEMBER_PORT;
...
static DOT1X_INLINE void DOT1X_vlan_port_set(nas_port_info_t *port_info, vtss_isid_t isid, vtss_port_no_t api_port, vtss_vid_t new_vid)
{
  ...
  if (vlan_mgmt_port_conf_set(isid, api_port, &vlan_cfg, VLAN_USER_DOT1X) != VTSS_RC_OK) {
    T_W("%u:%d: Unable to set PVID volatile to %d", isid, iport2uport(api_port), new_vid);
  }
}
#endif /* NAS_USES_VLAN */
...
static void DOT1X_set_port_control(nas_port_t nas_port, nas_port_control_t new_admin_state, nas_stop_reason_t stop_reason)
{
  ...
  if (stop_reason == NAS_STOP_REASON_PORT_MODE_CHANGED || stop_reason == NAS_STOP_REASON_PORT_LINK_DOWN || stop_reason == NAS_STOP_REASON_SWITCH_DOWN) {
  ...
  for (zisid = zisid_start; zisid <= zisid_end; zisid++) {
    ...
    for (api_port = 0; api_port < VTSS_PORTS; api_port++) {
...
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_QOS
      {
        ...
        if (old_qos_enabled && !new_qos_enabled && NAS_PORT_CONTROL_IS_SINGLE_CLIENT(cur_admin_state)) {
          // Gotta revert the QoS on the port (if set).
...
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_VLAN
      {
        ...
        if (old_backend_vlan_enabled && !new_backend_vlan_enabled && NAS_PORT_CONTROL_IS_SINGLE_CLIENT(cur_admin_state)) {
      ...
      if (cur_admin_state != new_admin_state && DOT1X_stack_state.switch_state[zisid].port_state[api_port].flags & DOT1X_PORT_STATE_FLAGS_LINK_UP) {
        T_D("%d:%d: Chg admin state from %s to %s", zisid + VTSS_ISID_START, iport2uport(api_port + VTSS_PORT_NO_START), dot1x_port_control_to_str(cur_admin_state), dot1x_port_control_to_str(new_admin_state));
...
static void DOT1X_cfg_flash_read(vtss_isid_t isid_add, BOOL create_defaults)
{
  ...
  if (flash_read_failed || DOT1X_cfg_valid(&flash_cfg->cfg) != VTSS_RC_OK) {
...
{
  // It's by design that we don't unlock the dot1x mutex before we've read our configuration.
  DOT1X_CRIT_ASSERT_LOCKED();

  while (1) {
    if (msg_switch_is_master()) {
      // Read and apply our configuration from flash.
      // We get here only on master-up events.
      DOT1X_CRIT_ASSERT_LOCKED();
      DOT1X_cfg_flash_read(VTSS_ISID_GLOBAL, FALSE);

      // It confuses Lint that it's locked at this position, so we help it.
      // Lint says: "(Warning -- A thread mutex that had not been locked is being unlocked)".
      /*lint -e(455) */
      DOT1X_CRIT_EXIT(); // This might be the very first release of our mutex.
    ...
    // This whole enter/exit/assert locked construct confuses Lint.
    // Lint says: "(Warning -- Two execution paths are being combined with different mutex lock states)"
    /*lint -e(456) */
    DOT1X_CRIT_ASSERT_LOCKED();
...
/******************************************************************************/
static vtss_rc DOT1X_isid_port_check(vtss_isid_t isid, vtss_port_no_t api_port, BOOL allow_local)
{
...
static vtss_vid_t DOT1X_backend_check_vlan_rfc2868_rfc3580(u8 radius_handle)
{
  ...
  while (vtss_radius_tlv_iterate(radius_handle, &tlv_type, &tlv_len, (const u8 **)&tlv_ptr, start_over) == VTSS_RC_OK) {
    ...
    if (!DOT1X_isdigits(tlv_ptr, tlv_len)) {
      ...
      strncpy(buf, avp_ptr, avp_len);
      if (vlan_mgmt_vid_from_name(buf, &result) != VTSS_RC_OK) {
#ifdef VTSS_SW_OPTION_SYSLOG
        S_W("RADIUS-assigned VLAN: VLAN name (%s) not found", buf);
#endif
        T_D("RADIUS-assigned VLAN: VLAN name (%s) not found", buf);
      }
      break;
#else
#ifdef VTSS_SW_OPTION_SYSLOG
...
#define DOT1X_ADD_OPTIONAL_RADIUS_ATTRIB(t, l, v)                                                                 \
  ...
  DOT1X_ADD_OPTIONAL_RADIUS_ATTRIB(VTSS_RADIUS_ATTRIBUTE_USER_NAME, strlen(user), (u8 *)user);
...
/****************************************************************************/
u8 nas_os_get_eapol_challenge_timeout(void)
{
  DOT1X_CRIT_ASSERT_LOCKED();
...
// nas_os_get_eapol_challenge_timeout().
/****************************************************************************/
u8 nas_os_get_eapol_request_identity_timeout(void)
{
  DOT1X_CRIT_ASSERT_LOCKED();
...
/******************************************************************************/
char *dot1x_port_control_to_str(nas_port_control_t port_control)
{
  switch (port_control) {
...
vtss_rc dot1x_mgmt_glbl_cfg_get(dot1x_glbl_cfg_t *glbl_cfg)
{
  ...
  if ((rc = DOT1X_isid_port_check(VTSS_ISID_START, VTSS_PORT_NO_START, FALSE)) != VTSS_RC_OK) {
    return rc;
  }
...
vtss_rc dot1x_mgmt_switch_cfg_get(vtss_isid_t isid, dot1x_switch_cfg_t *switch_cfg)
{
  ...
  if ((rc = DOT1X_isid_port_check(isid, VTSS_PORT_NO_START, FALSE)) != VTSS_RC_OK) {
    return rc;
  }
...
vtss_rc dot1x_mgmt_port_status_get(vtss_isid_t isid, vtss_port_no_t api_port, nas_port_status_t *port_status)
{
  ...
  if ((rc = DOT1X_isid_port_check(isid, api_port, TRUE)) != VTSS_RC_OK) {
    return rc;
  }
...
vtss_rc dot1x_mgmt_multi_client_status_get(vtss_isid_t isid, vtss_port_no_t api_port, dot1x_multi_client_status_t *client_status, BOOL *found, BOOL start_all_over)
{
  ...
  *found = FALSE;
  ...
  if ((rc = DOT1X_isid_port_check(isid, api_port, FALSE)) != VTSS_RC_OK) {
    return rc;
  }
