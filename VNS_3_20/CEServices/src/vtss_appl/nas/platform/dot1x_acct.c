...
static void DOT1X_ACCT_radius_tx_request(struct nas_sm *sm, u8 status_type)
{
  ...
  T_NG(TRACE_GRP_ACCT, "enter, mode = %s, port = %u", dot1x_port_control_to_str(port_info->port_control), port_info->port_no);
...
void nas_os_acct_timer_tick(struct nas_sm *sm)
{
  ...
  T_RG(TRACE_GRP_ACCT, "enter, mode = %s, port = %u", dot1x_port_control_to_str(port_info->port_control), port_info->port_no);
...
void dot1x_acct_authorized_changed(nas_port_control_t admin_state, struct nas_sm *sm, BOOL authorized)
{
  ...
  T_NG(TRACE_GRP_ACCT, "enter, admin_state = %s, port = %u, authorized = %s", dot1x_port_control_to_str(admin_state), port_info->port_no, authorized ? "Up" : "Down");
