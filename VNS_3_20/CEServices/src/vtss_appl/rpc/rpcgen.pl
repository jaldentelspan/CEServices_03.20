...
sub ref_arg {
    my ($param, $deref) = @_;
    return is_struct($param->{'type'}) || ($param->{'out'} && ! $param->{'size'}) ? $deref : "";
}
...
sub namelist {
    my ($list) = shift;
    my ($prefix) = shift || "";
    return join( ", ", map { $prefix . $_->{name} } @{$list});
} 
...
sub namelist_p {
    ...
    my @args;
    for my $a (@{$list}) {
        push @args, $a->{out} || is_struct($a->{type}) ? "&" . $a->{name} : $a->{name};
    }
    return join(", ", @args);
} 
...
sub decllist {
    ...
    for my $param (@{$list}) {
        if($ptr) {
            push @params, join(" ",
                               $param->{'type'},
                               ($param->{'size'} ? "$param->{name}\[$param->{size}\]" :
                               "*$param->{name}"));
        } else {
            push @params, join(" ",
                               $param->{'out'} ? "" : "const",
