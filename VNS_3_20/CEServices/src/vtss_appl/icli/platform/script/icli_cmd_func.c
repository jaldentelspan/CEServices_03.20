    ...
    *(c + 1) = 0;
    ...
    while ( b_loop ) {
        ...
        case ICLI_RC_OK:
            ...
            // check length
            // -1 is for the starting delimiter
            len = icli_str_len(user_str) - 1 + icli_str_len(str);
            if ( len >= max_len ) {
                if ( icli_str_len(user_str) ) {
                    len = max_len - icli_str_len(user_str) + 1;
                } else {
                    len = max_len;
                }
                *(str + len) = 0;
                (void)icli_str_concat(user_str, str);
                // free memory
                free( str );
                return TRUE;
            }
