...
)
{
    ...
    if ( g_password[0] ) {
        if ( icli_enable_password_if_secret_get(priv) ) {
            rc = vtss_icfg_printf(result, "enable secret 7 level %d %s\n",
                                  priv, g_password);
            if ( rc != VTSS_RC_OK ) {
                T_E("fail to print to icfg\n");
                return VTSS_RC_ERROR;
...
vtss_rc icli_icfg_init(void)
{
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_GLOBAL_ICLI, _icli_icfg);
    if ( rc != VTSS_OK ) {
        return rc;
    }
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_LINE_ICLI, _icli_icfg);
    if ( rc != VTSS_OK ) {
        return rc;
    }
