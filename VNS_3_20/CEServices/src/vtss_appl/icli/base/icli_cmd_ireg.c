...
static BOOL _cmd_reg_add(void)
{
    ...
    */
    ...
    for ( i = 0; i < line; i++ ) {
        if ( include_line == -1 ) {
            if ( b_include ) {
                if ( line_buf[i][0] != '#' ) {
                    include_line = i;
                } else if ( icli_str_str(g_reg_include, line_buf[i]) != NULL ) {
                    reg_include_line = i;
                }
            } else {
                if ( line_buf[i][0] == '#' ) {
                    b_include = TRUE;
                    if ( icli_str_str(g_reg_include, line_buf[i]) != NULL ) {
                        reg_include_line = i;
                    }
                }
            }
        ...
        if ( b_start ) {
            ...
            if ( icli_str_sub("NULL", line_buf[i], TRUE, NULL) >= 0 ) {
                continue;
            }

            /* check end */
            if ( icli_str_cmp(line_buf[i], _CMD_REG_END) == 0 ) {
            ...
            if ( line_buf[i][j] == '&' ) {
                j++;
                if ( icli_str_sub(g_reg_api, &(line_buf[i][j]), TRUE, NULL) >= 0 ) {
                    //T_W("Command already has been registered.\n");
                    reg_api_line = i;
                    break;
                }
            } else {
            ...
            if ( icli_str_cmp(line_buf[i], _CMD_REG_BEGIN) == 0 ) {
                b_start = TRUE;
            }
        }
    }
    ...
    if ( b_add ) {
        ...
        b_if = FALSE;
        if ( icli_str_sub("#if ", line_buf[reg_include_line - 1], TRUE, NULL) >= 0 ) {
            b_if = TRUE;
        }
        for ( i = 0; i < line; i++ ) {
            if ( b_if ) {
