...
#define _SCROLL_NEEDED \
    (handle->runtime_data.cmd_len > _CMD_LINE_LEN)
...
#define _CURSOR_AT_LINE_END \
    ((handle->runtime_data.right_scroll) ? \
     (handle->runtime_data.cursor_pos == (_CMD_LINE_LEN - 2)) : \
     (handle->runtime_data.cursor_pos == (_CMD_LINE_LEN)))
...
{
    return handle->runtime_data.width - handle->runtime_data.prompt_len - handle->runtime_data.cursor_pos - 1;
}
...
)
{
    ...
    // go to the begin
    icli_xy_cursor_offset(handle, 0 - cursor_pos, 0);
    ...
    handle->runtime_data.cursor_pos = 0;
#else
    while ( handle->runtime_data.cursor_pos ) {
...
)
{
    ...
    // get current x,y
    icli_sutil_current_xy_get(handle, &nx, &ny);

    // go back to the original x,y
    icli_xy_cursor_offset(handle, x - nx, y - ny);
        ...
        if ( handle->runtime_data.left_scroll ) {
            if (handle->runtime_data.start_pos < _SCROLL_SIZE) {
                ...
                if ( (handle->runtime_data.cmd_len - handle->runtime_data.cmd_pos) < _SCROLL_SIZE ) {
                    if ( handle->runtime_data.start_pos < (_CMD_LINE_LEN - _SCROLL_SIZE) ) {
                        handle->runtime_data.start_pos = 0;
                    } else {
                        handle->runtime_data.start_pos -= (_CMD_LINE_LEN - _SCROLL_SIZE);
                    }
                } else {
...
)
{
    ...
    if ( handle->runtime_data.left_scroll ) {
        ...
        /* go to the end */
        icli_xy_cursor_offset(handle, 0 - handle->runtime_data.cursor_pos, 0);
...
)
{
    ...
    if ( handle->runtime_data.right_scroll ) {
        ...
        _del_line( handle );
        /* get start_pos */
        handle->runtime_data.start_pos = handle->runtime_data.cmd_len - (_CMD_LINE_LEN - _SCROLL_SIZE - 1);
        ...
        /* go to the end */
        icli_xy_cursor_offset(handle, offset, 0);
...
)
{
    ...
    /* update cmd */
    (void)icli_str_cpy(handle->runtime_data.cmd + cmd_pos, handle->runtime_data.cmd + handle->runtime_data.cmd_pos);
...
)
{
    ...
    /* add char */
    if ( icli_sutil_cmd_char_add(handle, c) == FALSE ) {
        return;
    }

    /* check if any char already in buffer */
    if ( icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) ) {
        b_buf = TRUE;
        while ( ICLI_NOT_(KEY_ENTER, c) && ICLI_NOT_(NEWLINE, c)) {
            (void)icli_sutil_cmd_char_add(handle, c);
            if ( icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) == FALSE ) {
                /* no char in buffer */
                b_buf = FALSE;
                break;
            }
        ...
        if ( b_end ) {
            if ( cmd_len ) {
                for ( i = handle->runtime_data.cursor_pos; i > 0; i-- ) {
                    icli_session_char_put(handle, ICLI_BACKSPACE);
                }
                for ( i = handle->runtime_data.cursor_pos; i > 0; i-- ) {
                    icli_session_char_put(handle, ICLI_SPACE);
                }
                for ( i = handle->runtime_data.cursor_pos; i > 0; i-- ) {
                    icli_session_char_put(handle, ICLI_BACKSPACE);
                }
            ...
            if ( handle->runtime_data.cmd_len <= _CMD_LINE_LEN ||
                 handle->runtime_data.cmd_pos <= (_CMD_LINE_LEN - _SCROLL_SIZE) ) {
                handle->runtime_data.start_pos = 0;
                handle->runtime_data.cursor_pos = handle->runtime_data.cmd_pos;
            } else {
                if ( handle->runtime_data.cursor_pos < _SCROLL_SIZE ) {
                    handle->runtime_data.cursor_pos = _SCROLL_SIZE;
                } else if ( handle->runtime_data.cmd_pos > (_CMD_LINE_LEN - _SCROLL_SIZE) ) {
                    handle->runtime_data.cursor_pos = _CMD_LINE_LEN - _SCROLL_SIZE;
                }
    ...
    while ( loop == 1 ) {
        ...
        */
        ...
        case ICLI_HYPER_TERMINAL:
            ...
            case ICLI_VKEY_HOME:
                //get next c
                if ( icli_sutil_usr_char_get(handle, _FKEY_WAIT_TIME, &c) ) {
