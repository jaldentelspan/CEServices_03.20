    ...
    for ( w_type = _first_word_get(&cmd, &w);
        ...
        case ICLI_WORD_TYPE_OPTIONAL_BEGIN:
            ...
            for (;;) {
                ...
                *(cmd - 1) = ICLI_EOS;
                ...
                if ( sibling_tail ) {
                    ...
                    for (; new_tree->sibling != NULL; ___SIBLING(new_tree) ) {
                        ;
                    }
            ...
            if ( w_type == ICLI_WORD_TYPE_OPTIONAL_BEGIN ) {
                // begin bit mask
                for (new_tree = m_tree; new_tree != NULL; ___SIBLING(new_tree) ) {
            ...
            if ( symbol == ICLI_MANDATORY_END ) {
                if ( ICLI_IS_RANDOM_MUST(cmd - 1) ) {
                    if ( icli_parsing_random_head(&g_cmd_node[current_begin_id], &g_cmd_node[current_end_id], &must_level) ) {
                        /* get must number */
                        must_number = *(cmd + 1) - '0';
                        ...
                        g_cmd_node[current_begin_id].random_must_head   = must_head;
                        g_cmd_node[current_begin_id].random_must_number = must_number;
                        g_cmd_node[current_begin_id].random_must_level  = must_level;

                        /* mark begin node's siblings to be 'must begin' */
                        for ( node = &(g_cmd_node[current_begin_id]); node; ___SIBLING(node) ) {
                            node->random_must_begin = TRUE;
                        }

                        /* mark each end node to be 'must end' */
                        for ( i = current_begin_id + 1; i <= current_end_id; i++ ) {
                            ...
                            if ( g_cmd_node[i].child == NULL ) {
                                g_cmd_node[i].random_must_end = TRUE;
                            }
                        }
...
)
{
    ...
    */
    ...
    // word
    if ( icli_str_cmp(n1->word, n2->word) != 0 ) {
        return FALSE;
    }
    // optional_begin
...
#if 1 /* CP, 2012/12/06 11:01, check loop overlap */
    BOOL                    b_loop = FALSE;
            ...
            if ( b_loop ) {
                if ( loop_overlap_cnt ) {
                    T_E("Word \"%s\" in loop not overlapped, but it should.\n"
                        ...
                        "*** Please refer to AN1047, Appendix B.1\n",
                        w,
                        icli_register_cmd_str_get(pre_tree->node_property.cmd_property->cmd_id),
                        cmd_register->original_cmd);
                    return ICLI_RC_ERROR;
                } else {
                    // restore loop syntax
...
/*
    ...
    comment -
        n/a
*/
icli_parsing_node_t *icli_parsing_tree_get(
    IN  icli_cmd_mode_t         mode
)
{
    if ( mode >= ICLI_CMD_MODE_MAX ) {
        T_E("icli_parsing_tree_get : invalid mode = %d\n", mode);
        return NULL;
    }
