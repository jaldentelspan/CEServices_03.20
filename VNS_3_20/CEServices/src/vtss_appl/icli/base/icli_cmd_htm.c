...
#define ___VALUE_STR_GET(p) \
    ...
    if ( p == NULL ) {\
        __T_E("malloc() for "#p" by length %d\n", icli_str_len(g_value_str)+1);\
        return ICLI_RC_ERROR;\
    }\
...
)
{
    ...
    while (_line_read(f) == ICLI_RC_OK) {
        /* get g_read_cmd_cnt */
        if ( icli_str_cmp(g_line_buf, _HTM_COMMAND_CNT) == 0 ) {
        ...
        /* get g_read_reflink */
        if ( icli_str_cmp(g_line_buf, _HTM_REFLINK_BEGIN) == 0 ) {
            h = (_htm_element_t *)malloc(sizeof(_htm_element_t));
            if ( h == NULL ) {
                __T_E("malloc\n");
                fclose( f );
                return ICLI_RC_ERROR;
            ...
            while (_line_read(f) == ICLI_RC_OK) {
                ___HTM_ADD_TO_TAIL(h->begin);
                if ( icli_str_cmp(g_line_buf, _HTM_REFLINK_END) == 0 ) {
        ...
        /* get g_read_command */
        if ( icli_str_cmp(g_line_buf, _HTM_COMMAND_BEGIN) == 0 ) {
            h = (_htm_element_t *)malloc(sizeof(_htm_element_t));
            if ( h == NULL ) {
                __T_E("malloc\n");
                fclose( f );
            ...
            while (_line_read(f) == ICLI_RC_OK) {
                ___HTM_ADD_TO_TAIL(h->begin);
                if ( icli_str_cmp(g_line_buf, _HTM_COMMAND_END) == 0 ) {
                    for (t = h->begin; t->next != NULL; ___NEXT(t)) {
                        ;
                    }
                    h->end = t;
...
)
{
    ...
    while ( h ) {
        ...
        for ( t = h->begin; t; t = t->next ) {
            if ( icli_str_sub("<a href=", t->value_str, 1, NULL) == 1 ) {
                break;
            }
        }
        if ( t == NULL ) {
    ...
    while ( h ) {
        ...
        // find <a name
        for ( t = h->begin; t; t = t->next ) {
            if ( icli_str_sub("<a name=", t->value_str, 1, NULL) == 1 ) {
                break;
            }
        }
        if ( t == NULL ) {
...
)
{
    ...
    for ( i = 2; i < argc; i++ ) {
        ...
        j++;
        // copy to g_value_str
        (void)icli_str_cpy(g_value_str, &(argv[i][j]));
        ...
        if ( list ) {
            for ( p = NULL, e = list; e != NULL; p = e, ___NEXT(e)) {
                if ( icli_str_cmp(e->value_str, t->value_str) == 1 ) {
                    break;
                }
            }
            if ( e ) {
    ...
    g_htm_file = malloc(icli_str_len(argv[1]) + 32);
    if ( g_htm_file == NULL ) {
...
int main(i32 argc, char *argv[])
{
    ...
    // get g_generate_path
    i = icli_str_len( argv[1] );
    g_generate_path = malloc(i + 1);
    if ( g_generate_path == NULL ) {
        __T_E("fail to allocate memory for g_generate_path\n");
        return ICLI_RC_ERROR;
    }
    (void)icli_str_cpy(g_generate_path, argv[1]);

    // get cmd_ref.htm
    g_htm_file = malloc(i + 32);
    if ( g_htm_file == NULL ) {
        __T_E("fail to allocate memory for g_htm_file\n");
        return ICLI_RC_ERROR;
    }
