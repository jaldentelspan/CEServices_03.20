...
)
{
    ...
    // get end x,y
    icli_sutil_end_xy_get(handle, &ex, &ey);

    // go to the end
    icli_xy_cursor_offset(handle, ex - x, ey - y);
}
...
)
{
    ...
    // go back to the original x,y
    icli_xy_cursor_offset(handle, nx - x, ny - y);
...
)
{
    ...
    // print rest string
    icli_session_str_put(handle, handle->runtime_data.cmd + cmd_pos);
    ...
    // go back to the original x,y
    icli_xy_cursor_offset(handle, handle->runtime_data.prompt_len - x, 0 - y);

    // update command
    (void)icli_str_cpy(handle->runtime_data.cmd, handle->runtime_data.cmd + cmd_pos);
...
)
{
    ...
    if ( icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) ) {
        ...
        icli_session_str_put(handle, handle->runtime_data.cmd + handle->runtime_data.cursor_pos);
        // update cursor_pos
        handle->runtime_data.cursor_pos = handle->runtime_data.cmd_pos;
        // update cursor
        if ( ! _CURSOR_AT_CMD_END ) {
            ...
            icli_sutil_current_xy_get(handle, &x, &y);
            // get end x,y
            icli_sutil_end_xy_get(handle, &nx, &ny);
            // go back to the original x,y
            icli_xy_cursor_offset(handle, x - nx, y - ny);
        }
    } else {
...
)
{
    ...
    icli_xy_cursor_offset(handle, x - ex, y - ey);

    // restore original cursor pos
    handle->runtime_data.cursor_pos = handle->runtime_data.cmd_pos;
}

void icli_a_line_clear(
    IN  icli_session_handle_t   *handle
)
{
    _del_line( handle );
