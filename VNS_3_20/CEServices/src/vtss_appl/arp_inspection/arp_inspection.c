    ...
    if (arp_inspection_global.arp_inspection_conf.port_mode_conf[entry->isid - VTSS_ISID_START].check_VLAN[entry->port_no] == ARP_INSPECTION_MGMT_VLAN_ENABLED) {
        ...
        if (arp_inspection_global.arp_inspection_conf.vlan_mode_conf[entry->vid].flags & ARP_INSPECTION_VLAN_MODE) {
            ...
            case ARP_INSPECTION_LOG_DENY:
                if (arp_inspection_global.arp_inspection_conf.vlan_mode_conf[entry->vid].flags & ARP_INSPECTION_VLAN_LOG_DENY) {
                    //T_E("ARP packet is denied(vlan), port %lu, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
                    S_I("ARP packet is denied, port %u, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
                }
                break;
            case ARP_INSPECTION_LOG_PERMIT:
                if (arp_inspection_global.arp_inspection_conf.vlan_mode_conf[entry->vid].flags & ARP_INSPECTION_VLAN_LOG_PERMIT) {
                    //T_E("ARP packet is permitted(vlan), port %lu, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
                    S_I("ARP packet is permitted, port %u, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
        ...
        case ARP_INSPECTION_LOG_DENY:
            if (arp_inspection_global.arp_inspection_conf.port_mode_conf[entry->isid - VTSS_ISID_START].log_type[entry->port_no] == ARP_INSPECTION_LOG_DENY ||
                ...
                //T_E("ARP packet is denied, port %lu, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
                S_I("ARP packet is denied, port %u, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
            }
            break;
        case ARP_INSPECTION_LOG_PERMIT:
            if (arp_inspection_global.arp_inspection_conf.port_mode_conf[entry->isid - VTSS_ISID_START].log_type[entry->port_no] == ARP_INSPECTION_LOG_PERMIT ||
                arp_inspection_global.arp_inspection_conf.port_mode_conf[entry->isid - VTSS_ISID_START].log_type[entry->port_no] == ARP_INSPECTION_LOG_ALL) {
                //T_E("ARP packet is permitted, port %lu, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
                S_I("ARP packet is permitted, port %u, vlan %u, mac %s, sip %s.", iport2uport(entry->port_no), entry->vid, misc_mac_txt(entry->mac, buf), misc_ipv4_txt(entry->assigned_ip, buf1));
            }
            break;
        default:
...
{
    arp_inspection_msg_req_t *msg = malloc(size);
...
char *arp_inspection_error_txt(vtss_rc rc)
{
    ...
    case ARP_INSPECTION_ERROR_ENTRY_EXIST_ON_DB:
        return "ARP Inspection: the entry maybe exists on the database.";
                              ...
                              ulong port_no)
{
    ...
    info.glag_no = VTSS_GLAG_NO_NONE;               /* Ingress GLAG number or zero */
                                              ...
                                              vtss_glag_no_t glag_no)
{
    ...
    if (msg_switch_is_master() && VTSS_ISID_LEGAL(master_isid)) {   /* Bypass message module! */
        arp_inspection_do_rx_callback(packet, len, vid, master_isid, switchport);
    } else {
...
static BOOL arp_inspection_rx_packet_callback(void *contxt, const uchar *const frm, const vtss_packet_rx_info_t *const rx_info)
{
    ...
    /* Fill out frame information for filtering */
    info.port_no = rx_info->port_no;                 /* Ingress port number or zero */
#if defined(VTSS_FEATURE_AGGR_GLAG)
    info.glag_no = VTSS_GLAG_NO_NONE;                   /* Ingress GLAG number or zero */
#endif /* VTSS_FEATURE_AGGR_GLAG */
...
//        T_E_HEX(frm, rx_info->length);
        ar = (vtss_arp_header *)(frm + ARP_INSPECTION_MAC_LENGTH + ARP_INSPECTION_MAC_LENGTH + 2);  //DA, SA, and ether type
//        T_E("src_port = %d, vid = %d, arp sa = %s, aip = %s", rx_info->port_no, rx_info->tag.vid, misc_mac_txt(ar->ar_sha,buf1), misc_ipv4_txt(htonl(ar->ar_spa),buf2));
        // checking database
        memcpy(entry.mac, ar->ar_sha, ARP_INSPECTION_MAC_LENGTH * sizeof(u8));
        entry.vid = rx_info->tag.vid;
        entry.assigned_ip = htonl(ar->ar_spa);
        entry.isid = master_isid;
        entry.port_no = rx_info->port_no;
...
#if defined(VTSS_SW_OPTION_IP2)
        if (vtss_ip2_ip_by_vlan(entry.vid, VTSS_IP_TYPE_IPV4, &ipv4_addr) != VTSS_OK) {
            T_D("Get Current IP Address failed !!!!");
            is_get_ip_conf = FALSE;
        } else {
            is_get_ip_conf = TRUE;
        }
#endif

        /* check gratuitours ARP or not */
        is_gratuitous_arp = (ar->ar_spa == ar->ar_tpa);
        ...
        if ((rc = arp_inspection_checking(&entry)) == VTSS_OK) {
            ...
            while (switch_iter_getnext(&sit)) {
                ...
                /* send ARP packet to enabled ports */
                (void) port_iter_init(&pit, NULL, sit.isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT);
                while (port_iter_getnext(&pit)) {
                    info.port_tx = pit.iport;
                    ...
                    if (msg_switch_is_local(sit.isid)) {
                        if (port_info_get(pit.iport, &port_info) != VTSS_OK ||
                            port_info.link == 0 ||
                            vtss_packet_frame_filter(NULL, &info, &filter) != VTSS_OK ||
                            filter == VTSS_PACKET_FILTER_DISCARD) {
                            //T_R("port_no %lu, link %d, filter %d", pit.iport, port_info.link, filter);
                            continue;
                        }
                    } else {
                    ...
                    /* Only forward ARP packet to enabled ports
                       when sender/target IP is matched ARP inspection entries or gratuitours ARP */
                    if (!is_gratuitous_arp) {
                        temp_entry.isid = sit.isid;
                        temp_entry.port_no = pit.iport;
                        //T_R("isid %u, vid %lu, port_no %lu", sit.isid, entry.vid, pit.iport);
                        if (arp_inspection_entry_checking_matched_ipv4(&temp_entry, htonl(ar->ar_tpa)) != VTSS_OK) {
                            continue;
                        }
                    }
                ...
                if (send_packet) {
                    /* Alloc memory for transmit ARP frame */
                    if ((pkt_buf = arp_inspection_alloc_xmit(rx_info->length, entry.vid, sit.isid, port_list, &bufref)) != NULL) {
                        memcpy(pkt_buf, frm, rx_info->length);
                        if (arp_inspection_xmit(pkt_buf, rx_info->length, entry.vid, sit.isid, port_list, bufref, TRUE)) {
                            T_W("arp_inspection_xmit() transmit failed");
                        }
                    }
                    // log here, permit log
                    //T_E("4 ARP packet is permited, port %lu, vlan %u, mac %s, sip %s, tip %s.", iport2uport(entry.port_no), entry.vid, misc_mac_txt(ar->ar_sha, buf), misc_ipv4_txt(htonl(ar->ar_spa), buf1), misc_ipv4_txt(htonl(ar->ar_tpa), buf2));
                    arp_inspection_log_entry(&entry, ARP_INSPECTION_LOG_PERMIT);
                } else {
                    // log here, deny log
                    //T_E("5 ARP packet is denied, port %lu, vlan %u, mac %s, sip %s, tip %s.", iport2uport(entry.port_no), entry.vid, misc_mac_txt(ar->ar_sha, buf), misc_ipv4_txt(htonl(ar->ar_spa), buf1), misc_ipv4_txt(htonl(ar->ar_tpa), buf2));
                    arp_inspection_log_entry(&entry, ARP_INSPECTION_LOG_DENY);
                }

                /* check IP address is myself or not */
                if (msg_switch_is_local(sit.isid)) {
...
#if defined(VTSS_SW_OPTION_IP2)
                    if (is_get_ip_conf && (htonl(ar->ar_tpa) == ipv4_addr.addr.ipv4)) {
                        is_myself_arp = TRUE;
                    }
#endif
                }
        ...
        if (rc == ARP_INSPECTION_ERROR_DATABASE_NOT_FOUND) {
            // log here, deny log
            //T_E("6 ARP packet is denied, port %lu, vlan %u, mac %s, sip %s, tip %s.", iport2uport(entry.port_no), entry.vid, misc_mac_txt(ar->ar_sha, buf), misc_ipv4_txt(htonl(ar->ar_spa), buf1), misc_ipv4_txt(htonl(ar->ar_tpa), buf2));
            arp_inspection_log_entry(&entry, ARP_INSPECTION_LOG_DENY);
        }

        if (is_gratuitous_arp || is_myself_arp) {
            return FALSE; // Pass gratuitours ARP or myself ARP to IP stack
        } else {
            return TRUE; // Stop process the incoming packet
        }
    } else {
...
static void arp_inspection_receive_register(void)
{
    ...
    if (msg_switch_is_master()) {
        ...
        while (dhcp_snooping_ip_assigned_info_getnext(mac, &info)) {
            memcpy(entry.mac, info.mac, ARP_INSPECTION_MAC_LENGTH);
            entry.vid = info.vid;
            entry.assigned_ip = info.assigned_ip;
            entry.isid = info.isid;
...
static BOOL arp_inspection_msg_rx(void *contxt, const void *const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
    ...
    case ARP_INSPECTION_MSG_ID_FRAME_RX_IND: {
        ...
        arp_inspection_do_rx_callback(&msg[1], msg->req.rx_ind.len, msg->req.rx_ind.vid, isid, msg->req.rx_ind.port_no);
        break;
    }
    case ARP_INSPECTION_MSG_ID_FRAME_TX_REQ: {
...
static void arp_inspection_conf_read_stack(BOOL create)
{
    ...
    blk_version = ARP_INSPECTION_CONF_BLK_VERSION;
