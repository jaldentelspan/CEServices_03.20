        ...
        case CX_TAG_ENTRY:
            ...
            if (s->group == CX_TAG_ARP_INSPECTION_TABLE) {
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    ...
                    } else if (cx_parse_long(s, "vid", &vid_temp, VTSS_VID_DEFAULT, VTSS_VID_RESERVED) == VTSS_OK) {
                ...
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    if (cx_parse_long(s, "vid", &vid_temp, VTSS_VID_DEFAULT, VTSS_VID_RESERVED) == VTSS_OK) {
