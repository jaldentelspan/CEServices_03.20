...
static BOOL SL_flash_load(void)
{
    ...
    while (flptr < (SL_flash_info.base_fladdr + SL_flash_info.size_bytes)) {
        ...
        if (entry->size    <= sizeof(SL_flash_entry_t)         || // Equal sign because the message following the header must be non-empty (at least a NULL character)
            entry->size + flptr >= (SL_flash_info.base_fladdr + SL_flash_info.size_bytes) ||
...
/****************************************************************************/
static char SL_reserved_msg[SYSLOG_RAM_MSG_MAX]; // The message buffer is used when malloc() failed.
...
static void SL_assert_cb(const char *file_name, const unsigned long line_num, const char *msg)
{
    ...
    if ((buf = malloc(total_sz_bytes)) == NULL) {
        buf = SL_reserved_msg;
    }
...
void syslog_flash_print(syslog_cat_t cat, syslog_lvl_t lvl, int (*print_function)(const char *fmt, ...))
{
    ...
    while (flptr < SL_flash_next_free_entry) { /*lint -e{449} ... We're aware of the realloc hazards */
        if ((cat == SYSLOG_CAT_ALL || entry->cat == cat) &&
            (lvl == SYSLOG_LVL_ALL || entry->lvl == lvl) &&
            control_flash_read(flptr, entry, sizeof(*entry)) == CYG_FLASH_ERR_OK) {
            ...
            if (msglen > msgbuf_len) {
                u8 *newbuf = realloc(msgbuf, msglen);
                if (newbuf) {
                    msgbuf_len = msglen;
                       ...
                       BOOL               convert)
{
    ...
    /* Local log access */
    SYSLOG_RAM_CRIT_ENTER();
    for (cur = (SL_ram.last == NULL ? NULL : (SL_ram_entry_t *)&SL_ram.log[0]), cur_id = 1;
         cur != NULL; cur = cur->next, cur_id++) {
...
static void SL_ram_clear(syslog_lvl_t lvl)
{
    ...
    if (lvl == SYSLOG_LVL_ALL) {
        ...
        for (cur = (SL_ram.last == NULL ? NULL : (SL_ram_entry_t *) &SL_ram.log[0]), prev = NULL;
             cur != NULL;) {
            if (cur->lvl != lvl) {
                prev = cur;
                cur = cur->next;
            ...
            if (cur == (SL_ram_entry_t *) &SL_ram.log[0]) {
                if (cur->next) {
                    cur->lvl = cur->next->lvl;
                    cur->mid = cur->next->mid;
                    cur->time = cur->next->time;
                    strcpy(cur->msg, cur->next->msg);
...
void syslog_ram_log(syslog_lvl_t lvl, vtss_module_id_t mid, const char *fmt, ...)
{
    ...
    if (SL_ram.last == NULL) {
        ...
        /* Next entry is on 4-byte aligned address */
        n = (strlen(SL_ram.last->msg) + 4 + sizeof(SL_ram_entry_t) - SYSLOG_RAM_MSG_MAX);
