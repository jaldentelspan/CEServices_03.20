                                     ...
                                     vtss_timeinterval_t *pps_save_delay)
{
    ...
    *pps_load_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*4LL)/250000000LL;
    *pps_save_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*3LL)/250000000LL;
#else
    *freq = VTSS_PHY_TS_CLOCK_FREQ_15625M;
    *src = VTSS_PHY_TS_CLOCK_SRC_CLIENT_RX;
    *pps_load_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*4LL)/156250000LL;
    *pps_save_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*3LL)/156250000LL;
#endif
    return VTSS_RC_OK;
}
                                 ...
                                 vtss_timeinterval_t *pps_save_delay)
{
    ...
    *pps_load_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*3LL)/250000000LL;
    *pps_save_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*2LL)/250000000LL;
#else
#if defined(VTSS_PHY_TS_SILABS_CLK_DLL)
    *freq = VTSS_PHY_TS_CLOCK_FREQ_250M;
    *src = VTSS_PHY_TS_CLOCK_SRC_INTERNAL;
    ...
    *src = VTSS_PHY_TS_CLOCK_SRC_CLIENT_TX;
    /* default 1pps latency for the phy is 3 clock cycles */
    *pps_load_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*4LL)/125000000LL;
    *pps_save_delay = (((vtss_timeinterval_t)VTSS_ONE_MIA<<16)*3LL)/125000000LL;
...
static void phy_ts_internal_mode(vtss_phy_ts_rxtimestamp_len_t *ts_len, vtss_phy_ts_tc_op_mode_t *tc_mode)
{
    ...
    switch (mode) {
            ...
            case VTSS_TOD_INTERNAL_TC_MODE_48BIT:
                *ts_len = VTSS_PHY_TS_RX_TIMESTAMP_LEN_30BIT;
                *tc_mode = VTSS_PHY_TS_TC_OP_MODE_A;
                T_WG(VTSS_TRACE_GRP_PHY_TS, "Mode 48BIT not supported in PHY's");
                break;
            default: 
                *ts_len = VTSS_PHY_TS_RX_TIMESTAMP_LEN_30BIT;
                *tc_mode = VTSS_PHY_TS_TC_OP_MODE_A;
...
static void port_data_conf_set(vtss_port_no_t port_no)
{
    ...
    vtss_phy_ts_rxtimestamp_len_t ts_len;
    vtss_phy_ts_tc_op_mode_t tc_mode;
    ...
    if (rc == VTSS_RC_OK && (phy_id.part_number == 0x8488 || phy_id.part_number == 0x8487) &&
        ...
        if (phy_10g_mode.oper_mode != VTSS_PHY_1G_MODE) {
            ...
            if (phy_id.phy_api_base_no != port_no) {
                ...
                port_data[port_no].topo.port_shared = FALSE;
            ...
            if (!port_data[port_no].phy_init) {
                ...
                if (phy_id.part_number == 0x8488) {
                    phy_conf.xaui_sel_8487 = VTSS_PHY_TS_8487_XAUI_SEL_0; /* Required for 8488 */
                } else {
                    phy_conf.xaui_sel_8487 = VTSS_PHY_TS_8487_XAUI_SEL_0 | VTSS_PHY_TS_8487_XAUI_SEL_1; /* Required for 8487 */
                }
            ...
            /* GPIO setup: pin 1 = Load/save, pin 0 = 1PPS_0, pin 11 = 1PPS_1 */
            ...
            T_DG(VTSS_TRACE_GRP_PHY_TS, "Port_no = %d, Phy_id = %x, rev = %d, channel_id = %d, ts_feature = %d, shared = %d, rc = %x", port_no, 
                 phy_id.part_number, 
                 phy_id.revision,
                        ...
                        switch (mod_man_slave_table[slave_idx].my_state) {
                            ...
                            case MOD_MAN_NORM:
                                ...
                                if (mode == MODULE_MAN_MODE_CENTRAL_CLOCK) {
                                    ...
                                    T_WG(VTSS_TRACE_GRP_MOD_MAN, "Only central clock mode supported");
                                }
                                break;
                            case MOD_MAN_TSYNC1:
                                T_NG(VTSS_TRACE_GRP_MOD_MAN, "ModuleManager state = MOD_MAN_TSYNC1");
    ...
    for (ts_idx = 0; ts_idx < PHY_TS_TABLE_SIZE; ++ts_idx) {
        ...
        if ((phy_ts_status[ts_idx].reserved_mask & port_mask) &&
            ...
            if (phy_ts_status[ts_idx].cb && phy_ts_status[ts_idx].context) {
                /* call out to the application */
                phy_ts_status[ts_idx].cb(phy_ts_status[ts_idx].context, port_no, &ts);
            } else {
                T_EG(VTSS_TRACE_GRP_MOD_MAN, "undefined TS callback port_idx %d, ts_idx %d", port_no, ts_idx);
            }
            T_DG(VTSS_TRACE_GRP_MOD_MAN, "port_no %d, ts_id %d, ts %d(%d)", port_no, ts.id, ts.ts, ts.ts_valid);
        }
            ...
            while (port_iter_getnext(&pit)) {
                ...
                if ((phy_ts_status[id].reserved_mask & port_mask)) {
                    ...
                    if (phy_ts_status[id].cb && phy_ts_status[id].context) {
                        ...
                        phy_ts_status[id].cb(phy_ts_status[id].context, port_idx, &ts);
                    } else {
                        T_NG(VTSS_TRACE_GRP_MOD_MAN, "undefined TS callback port_idx %d, ts_idx %d", port_idx, id);
                    }
                    T_NG(VTSS_TRACE_GRP_MOD_MAN, "port_no %d, ts_id %d, ts %d(%d)", port_idx, ts.id, ts.ts, ts.ts_valid);
                }
                ++ port_idx;
            }
