...
static void cli_cmd_debug_phy_1588_engine_ip1_comm_conf(cli_req_t *req)
{
    ...
    if (rc == VTSS_RC_OK) {
        if (ip1_conf->comm_opt.ip_mode != tod_req->ip_mode) {
            for (i = 0; i < 8; i++) {
                if (ip1_conf->flow_opt[i].flow_en) {
                    free(flow_conf);
                    CPRINTF("IP mode error! disable all the flows before changing the mode\n");
                    return;
                }
            }
        }
...
static void cli_cmd_debug_phy_1588_engine_ip2_comm_conf(cli_req_t *req)
{
    ...
    if (rc == VTSS_RC_OK) {
        if (req->set == TRUE) {
            if (ip2_conf->comm_opt.ip_mode != tod_req->ip_mode) {
                for (i = 0; i < 8; i++) {
