...
static cyg_handle_t       mac_table_thread_handle;
static cyg_thread         mac_table_thread_block;
static char               mac_table_thread_stack[THREAD_DEFAULT_STACK_SIZE];
#endif /* defined(VTSS_FEATURE_VSTAX_V1) && VTSS_SWITCH_STACKABLE */
...
static i32 port_map_global[VTSS_ISID_END][VTSS_PORTS];
...
#if defined(VTSS_FEATURE_VSTAX_V1) && VTSS_SWITCH_STACKABLE 
/* Stores all frontport mac addresses in RAM for fast readout. */
static void mac_table_thread(cyg_addrword_t data)
{
    ...
    for(;;) {
        (void)cyg_flag_timed_wait( &mac_table_flags, 
                                   1,
                                   CYG_FLAG_WAITMODE_CLR,
                                   cyg_current_time() + VTSS_OS_MSEC2TICK(10 * 1000));
        
        memset(&status,0,sizeof(status));
        if ((vtss_mac_table_status_get(NULL, &status)) == VTSS_OK && 
            (status.learned || status.replaced || status.moved || status.aged)) {

            memset(&vid_mac_tmp, 0, sizeof(vid_mac_tmp));
            MAC_RAM_CRIT_ENTER();
            mac_ram_table.mac_index = 0;
            while(vtss_mac_table_get_next(NULL, &vid_mac_tmp, &next_entry) == VTSS_OK) { 

                /* Search again from the next address  */
                vid_mac_tmp = next_entry.vid_mac;

                for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
                    if (port_isid_port_no_is_stack(isid, port_no) && next_entry.destination[port_no]) {
                        store_mac = 0;
                        break; /* only stack port is a destination - ignore it */
                    } else if (next_entry.destination[port_no]) {
                        store_mac = 1;
                        break;
                    }
                }
                ...
                if (store_mac) {
                    mac_ram_table.mac_list[mac_ram_table.mac_index] = next_entry;               
                    mac_ram_table.mac_index++;

                    /* If new addresses are continously learned, we could exceed the table */
                    if (mac_ram_table.mac_index == VTSS_MAC_ADDRS)
                        break;
                } else {
                    store_mac = 1;
                }
...
#if defined(VTSS_FEATURE_VSTAX_V1) && VTSS_SWITCH_STACKABLE     
static vtss_rc mac_local_mem_get_next(vtss_vid_mac_t *vid_mac, vtss_mac_table_entry_t *entry, 
                               mac_mgmt_addr_type_t *type, BOOL next)
{
    ...
    MAC_RAM_CRIT_ENTER();
    for (i=0; i<mac_ram_table.mac_index; i++) {        
        if (next) {
            if (vid_mac_bigger(&mac_ram_table.mac_list[i].vid_mac, vid_mac)) {
               
                if ((type->only_this_vlan) && (mac_ram_table.mac_list[i].vid_mac.vid != vid_mac->vid))
                    continue;
                
                if ((type->not_dynamic) && (!mac_ram_table.mac_list[i].locked))
                    continue;
                
                if ((type->not_static) && (mac_ram_table.mac_list[i].locked))
                    continue;
                
                if ((type->not_cpu) && (mac_ram_table.mac_list[i].copy_to_cpu))
                    continue;
                
                if ((type->not_mc) && (mac_ram_table.mac_list[i].vid_mac.mac.addr[0] & 0x1))
                    continue;
                
                if ((type->not_uc) && !(mac_ram_table.mac_list[i].vid_mac.mac.addr[0] & 0x1))
                    continue;
                
                *entry = mac_ram_table.mac_list[i];
                MAC_RAM_CRIT_EXIT();
                return VTSS_OK;        
            } 
        } else {                                    
            if (mac_ram_table.mac_list[i].vid_mac.vid == vid_mac->vid && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[0] == vid_mac->mac.addr[0] && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[1] == vid_mac->mac.addr[1] && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[2] == vid_mac->mac.addr[2] && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[3] == vid_mac->mac.addr[3] && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[4] == vid_mac->mac.addr[4] && 
                mac_ram_table.mac_list[i].vid_mac.mac.addr[5] == vid_mac->mac.addr[5])  {
...
{        
    ...
    T_N("Deleting mac entry:%02x-%02x-%02x-%02x-%02x-%02x vid:%d", 
        vid_mac->mac.addr[0],vid_mac->mac.addr[1],vid_mac->mac.addr[2],
        vid_mac->mac.addr[3],vid_mac->mac.addr[4],vid_mac->mac.addr[5],vid_mac->vid);
...
static vtss_rc mac_local_learn_mode_set(vtss_port_no_t port_no, vtss_learn_mode_t *learn_mode)
{
    ...
    T_I("Local. Port:%d auto:%d, cpu:%d, discard:%d", port_no,learn_mode->automatic,learn_mode->cpu,learn_mode->discard);
...
void flush_mac_list(BOOL force)
{
    mac_static_t           *mac;
...
static char *mac_msg_id_txt(mac_msg_id_t msg_id)
{
    ...
    case MAC_MSG_ID_UPSID_UPSPN_FLUSH_REQ:
        ...
        break;
    case MAC_MSG_ID_PORTMAP_REQ:
        txt = "MAC_MSG_ID_PORTMAP_REQ";
        break;
    case MAC_MSG_ID_PORTMAP_REP:
        txt = "MAC_MSG_ID_PORTMAP_REP";
        break;
     default:
        txt = "?";
        break;
...
static void mac_msg_tx_done(void *contxt, void *msg, msg_tx_rc_t rc)
{
    ...
    if (rc != MSG_TX_RC_OK) {
        T_D("Could not transmit msg. Got return code:%d from msg module",rc);        
...
static void mac_msg_tx(void *msg, vtss_isid_t isid, size_t len)
{
    ...
    T_D("Tx: %s, len: %zd, isid: %d", mac_msg_id_txt(msg_id), len, isid);
...
static BOOL mac_msg_rx(void *contxt, const void *const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
    ...
    T_D("Rx: %s, len: %zd, isid: %u", mac_msg_id_txt(msg_id), len, isid);
    ...
    case MAC_MSG_ID_GET_NEXT_STACK_REQ: {
        ...
        /* Do a local memory Get Next */
        rep_msg->rep.get_next.rc = mac_local_mem_get_next(&req_msg->req.get_next_stack.vid_mac, &rep_msg->rep.get_next.entry, &req_msg->req.get_next_stack.type, req_msg->req.get_next_stack.next);
    ...
    case MAC_MSG_ID_PORTMAP_REQ: {
        mac_msg_rep_t    *rep_msg = mac_msg_rep_alloc(MAC_MSG_ID_PORTMAP_REP);
        vtss_port_map_t  port_map[VTSS_PORT_ARRAY_SIZE];
        vtss_port_no_t   port_no;
        u32              port_count = port_isid_port_count(VTSS_ISID_LOCAL);

        if (vtss_port_map_get(NULL, port_map) != VTSS_RC_OK) {
            T_E("Unable to get port map");
            break;
        }

        for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
            rep_msg->rep.pm.chip_port[port_no] = port_map[port_no].chip_port;
        }
        mac_msg_tx(rep_msg, isid, sizeof(rep_msg->rep.pm));
        break;
    }
    ...
    case MAC_MSG_ID_PORTMAP_REP: {
        ...
        MAC_CRIT_ENTER();
        for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
            port_map_global[isid][port_no] = rep_msg->rep.pm.chip_port[port_no];
        }
        MAC_CRIT_EXIT();
        break;
    }
...
static BOOL mac_mgmt_port_sid_invalid(vtss_isid_t isid, vtss_port_no_t port_no, BOOL set)
{
...
static vtss_rc mac_del_stack(vtss_isid_t isid_del, vtss_vid_mac_t *entry, BOOL vol)
{
    ...
    if (mac_mgmt_port_sid_invalid(isid_del, VTSS_PORT_NO_START, 1)) {
        return MAC_ERROR_GEN;
    }     
  
    memset(&del_entry, 0, sizeof(&del_entry));
...
static void mac_static_switch_add(vtss_isid_t isid_add)
{
    ...
    vtss_port_no_t        port_no;
    switch_iter_t         sit;
    u32                   port_count = port_isid_port_count(isid_add);
   
    T_D("Enter mac_static_switch_add:%d",isid_add);

    for (vol = 0; vol <= 1; vol++) {

        /* Add static address to the new switch, both volatile and non-volatile */
        memset(&vid_mac, 0, sizeof(vid_mac));    /* start with all zero */
        while (mac_mgmt_static_get_next(isid_add, &vid_mac, &mac_entry, 1, vol) == VTSS_OK) { 
            vid_mac = mac_entry.vid_mac;        
            mac_add_stack(isid_add, &mac_entry);
        }
              
        /* Need to run through all static address in the stack because the new switch 
           must get all addresses added to its stack ports      */
        ...
        while (switch_iter_getnext(&sit)) {      
            ...
            T_D("ISID scan. Accessing isid:%d for mac_mgmt_static_get_next", sit.isid);
            memset(&vid_mac, 0, sizeof(vid_mac));    /* start with all zero */
            while (mac_mgmt_static_get_next(sit.isid, &vid_mac, &mac_entry, 1, vol) == VTSS_OK) {
                vid_mac = mac_entry.vid_mac;    
                /* If the address already exist on the isid_add switch then skip it */
                if (mac_mgmt_static_get_next(isid_add, &vid_mac, &mac_entry, 0, vol) == VTSS_OK) {
                    continue;                                               
                }
                if (mac_entry.not_stack_ports) {
                    continue;
                }
                
                for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
                    mac_entry.destination[port_no] = port_isid_port_no_is_stack(isid_add, port_no) ? 1 : 0; /* Add only to stack ports */
                }
                
                msg = mac_msg_req_alloc(MAC_MSG_ID_ADD_REQ, 1);
                msg->req.add.entry = mac_entry;
                mac_msg_tx(msg, isid_add, sizeof(msg->req.add));
                
            }    
        }        
    }
    T_D("Exit mac_static_switch_add:%d",isid_add);
}
...
static void mac_static_switch_del(vtss_isid_t isid_del)
{
    ...
    T_D("Enter mac_static_switch_del:%d",isid_del);
    ...
    T_D("Exit mac_static_switch_del:%d",isid_del);
}
...
vtss_rc mac_mgmt_table_get_next(vtss_isid_t isid, vtss_vid_mac_t *vid_mac, vtss_mac_table_entry_t *entry, BOOL next)
{
    ...
    if (mac_mgmt_port_sid_invalid(isid, VTSS_PORT_NO_START, 0)) {
        return MAC_ERROR_GEN;
    }     
    
    if (msg_switch_is_local(isid) || !msg_switch_is_master()) {
...
{
    int                    first = 1, result, i;
    mac_msg_req_t          *msg;
    vtss_rc                rc = VTSS_UNSPECIFIED_ERROR, rc_tmp;
    vtss_isid_t            local_isid;
    switch_iter_t          sit;
    vtss_mac_table_entry_t tmp_entry;
    ...
    (void)switch_iter_init(&sit, VTSS_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        if (sit.isid == local_isid) {
            rc_tmp = mac_local_mem_get_next(vid_mac, &tmp_entry, type, next);
            MAC_CRIT_ENTER();
            memcpy(&mac_config.get_next.entry, &tmp_entry, sizeof(vtss_mac_table_entry_t));
            mac_config.get_next.rc = rc_tmp;
            MAC_CRIT_EXIT();
        } else {
            msg = mac_msg_req_alloc(MAC_MSG_ID_GET_NEXT_STACK_REQ, 1);
            msg->req.get_next_stack.vid_mac = *vid_mac;
            msg->req.get_next_stack.type    = *type;
            msg->req.get_next_stack.next    = next;
            
            mac_msg_tx(msg, sit.isid, sizeof(msg->req.get_next_stack));
            ...
            if (flags == 0) {
                T_W("Timeout or could not tx, MAC_MSG_ID_GET_NEXT_REQ ISID:%d",sit.isid);
                return MAC_ERROR_REQ_TIMEOUT;
            }
        }
        ...
        if (next) {
            /*  GET NEXT request */
            if (mac_config.get_next.rc == VTSS_OK) {

                if (first) {
                    first = 0;
                    entry->vid_mac = mac_config.get_next.entry.vid_mac;
                    for (i = 0;i < VTSS_PORTS;i++)
                        entry->destination[sit.isid][i] = mac_config.get_next.entry.destination[i];

                    entry->copy_to_cpu = mac_config.get_next.entry.copy_to_cpu;
                    entry->locked = mac_config.get_next.entry.locked;
                    rc = VTSS_OK;
                } else {
                    result = vid_mac_cmp(&entry->vid_mac, &mac_config.get_next.entry.vid_mac);
                    if (result == 1) { /* smaller found */
                        memset(entry,0,sizeof(*entry));
                        entry->vid_mac = mac_config.get_next.entry.vid_mac;
                        entry->copy_to_cpu = mac_config.get_next.entry.copy_to_cpu;
                        entry->locked = mac_config.get_next.entry.locked;
                        for (i = 0;i < VTSS_PORTS;i++)
                            entry->destination[sit.isid][i] = mac_config.get_next.entry.destination[i];
                    } else if (result == 0) { /* Equal, store the destination list */
                        for (i = 0;i < VTSS_PORTS;i++)
                            entry->destination[sit.isid][i] = mac_config.get_next.entry.destination[i];
                    }
                }
...
static vtss_rc mac_table_add(vtss_isid_t isid, mac_mgmt_addr_entry_t *entry)
{
    ...
    uint               i=0;
    BOOL               place_found=0, newentry=0, vol;
    u32                port_count = port_isid_port_count(isid);
    ...
    if ( mac_mgmt_static_get_next(isid, &entry->vid_mac, &return_mac, 0, (vol?0:1) ) == VTSS_OK) 
        return MAC_ERROR_MAC_EXIST; /* MAC address was found in the other storage area, exiting..*/
    ...
    if (!vol) {
        if (mac_mgmt_static_get_next(isid, &entry->vid_mac, &return_mac, 0, vol) != VTSS_OK) {
    ...
    /* System address and broadcast address may not be changed, unless it's a volatile
     * entry, so that we know it's for debug purposes and not end-user controlled. */
    /* if (!vol) { */
    /*     if (conf_mgmt_mac_addr_get(system_mac, 0) != 0) { */
    /*         T_W("Could not get system address"); */
    /*     } */
    /*     for (i = 0; i < 6; i++) { */
    /*         if (system_mac[i] != entry->vid_mac.mac.addr[i]) */
    /*             addr1_ok = 1; */

    /*         if (0xff != entry->vid_mac.mac.addr[i]) */
    /*             addr2_ok = 1; */
    /*     } */
    /*     if (!addr1_ok || !addr2_ok) { */
    /*         T_D("Cannot change system or broadcast addresses.\n"); */
    /*         return MAC_ERROR_MAC_RESERVED; */
    /*     } */
    /* } */
...
vtss_rc mac_mgmt_table_port_del(vtss_isid_t isid, vtss_port_no_t port_no, BOOL vol)
{
    ...
    if (mac_mgmt_port_sid_invalid(isid, port_no, 1)) {
        return MAC_ERROR_GEN;
    }       
...
{
    if (mac_mgmt_port_sid_invalid(isid, VTSS_PORT_NO_START, 1)) {
        return MAC_ERROR_GEN;
    }       
...
//    return_mac->not_stack_ports = 0;
    ...
    if (next) {
        ...
        for (mac = mac_used_p,i=1; mac != NULL; mac = mac->next, i++) {                
            T_D("Entry %d Isid:%d:%02x-%02x-%02x-%02x-%02x-%02x vid:%d",i,mac->conf.isid, 
                mac->conf.vid_mac.mac.addr[0],mac->conf.vid_mac.mac.addr[1],mac->conf.vid_mac.mac.addr[2],
                mac->conf.vid_mac.mac.addr[3],mac->conf.vid_mac.mac.addr[4],mac->conf.vid_mac.mac.addr[5],mac->conf.vid_mac.vid);
        }
...
/* Read/create mac table configuration */

static void mac_conf_read(vtss_isid_t isid_add, BOOL force_default)
{
    mac_age_conf_t        conf;
    mac_conf_blk_t        *age_blk;
    mac_static_t          *mac;
    ...
    vtss_learn_mode_t      learn_mode;   
    u16                   i;
    BOOL                  do_create=0, learn_force_en = 0;
    vtss_isid_t           isid;
    vtss_port_no_t        port_no;
    u32                   size, port_count = port_isid_port_count(isid_add);
    ...
    if (force_default || (do_create && (static_blk = conf_sec_create(CONF_SEC_GLOBAL, 
                                                                     CONF_BLK_MAC_STATIC_CONF_TABLE,sizeof(*static_blk))))) {
        T_D("Setting mac static conf to default, isid:%d",isid_add);
        /* Use start values */        
        static_blk->count = 0;    
        ...
        /* Let each switch flush their addresses locally */
        if (isid_add == VTSS_ISID_GLOBAL) {
            for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {
                if (!msg_switch_exists(isid))
                    continue;

                if (mac_remove_locked(isid) != VTSS_OK) {
                    T_W("Could not removed locked addresses isid:%d",isid);
                }
            }
        } else {
            if (mac_remove_locked(isid_add) != VTSS_OK) {
                T_W("Could not removed locked addresses isid:%d",isid_add);
            }
        }       
        ...
        if (static_blk != NULL) {
            /* Else restore from flash  */
            T_D("Getting stored static mac addresses, isid:%d",isid_add);
    ...
    MAC_CRIT_ENTER();
    /* If create */
    if (force_default || (do_create && (learn_blk = conf_sec_create(CONF_SEC_GLOBAL, 
                                                                    CONF_BLK_MAC_LEARN_MODE_TABLE,sizeof(*learn_blk))))) {
        ...
        if (isid_add == VTSS_ISID_GLOBAL) {
            for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {           
                for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
                    VTSS_BF_SET(mac_learn_mode.learn_mode[isid][port_no], LEARN_AUTOMATIC, 1);
                    VTSS_BF_SET(mac_learn_mode.learn_mode[isid][port_no], LEARN_CPU, 0);
                    VTSS_BF_SET(mac_learn_mode.learn_mode[isid][port_no], LEARN_DISCARD, 0);   
                } 
            }
        } else {
            for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
                VTSS_BF_SET(mac_learn_mode.learn_mode[isid_add][port_no], LEARN_AUTOMATIC, 1);
                VTSS_BF_SET(mac_learn_mode.learn_mode[isid_add][port_no], LEARN_CPU, 0);
                VTSS_BF_SET(mac_learn_mode.learn_mode[isid_add][port_no], LEARN_DISCARD, 0);            
            }
        }

        T_D("Setting mac learn mode to default, isid:%d",isid_add);
        ...
        if (learn_force_en) {
            if (isid_add == VTSS_ISID_GLOBAL) {
                ...
                port_count = port_isid_port_count(isid_add);
                for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
                    if (!learn_force.enable[isid_add][port_no]) {
                        MAC_CRIT_EXIT();
                        if (mac_stack_learn_mode_set(isid_add, port_no, &learn_mode) != VTSS_OK) {
                            T_W("Could not set learn mode");
                        }
                        MAC_CRIT_ENTER();
                    }
            ...
            for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {           
                if (!msg_switch_exists(isid))
                    continue;
                if (mac_stack_learn_mode_set(isid, MAC_ALL_PORTS, &learn_mode) != VTSS_OK) {
                    T_W("Could not set learn mode");
                }
            }
            MAC_CRIT_ENTER();
    ...
    T_D("Exit: mac_config_read");
}

#if defined(VTSS_FEATURE_VSTAX_V2) && VTSS_SWITCH_STACKABLE
static void mac_stack_port_map_get(vtss_isid_t isid) {
    mac_msg_req_t *msg = mac_msg_req_alloc(MAC_MSG_ID_PORTMAP_REQ, 1);
    mac_msg_tx(msg, isid, 0);
}
...
static void topo_upsid_change_callback(vtss_isid_t isid)
{
    ...
    flush_mac_list(1);
    build_mac_list();
    mac_conf_read(VTSS_ISID_GLOBAL,0);
}
...
{   
    ...
    if(msg_switch_is_master() && !info->stack) {  
        if (!info->link) {
            MAC_CRIT_ENTER();
            upspn = port_map_global[isid][port_no];  
            if (upsid_table.upsid[isid][1] != VTSS_VSTAX_UPSID_UNDEF) {
                upsid = port_no < upsid_table.port_no ? upsid_table.upsid[isid][0] : upsid_table.upsid[isid][1];
            } else {
                upsid = upsid_table.upsid[isid][0];  
            }
            MAC_CRIT_EXIT();
...
vtss_rc mac_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
       ...
       cyg_thread_create(THREAD_DEFAULT_PRIO, 
                         ...
                         &mac_thread_block);
#if defined(VTSS_FEATURE_VSTAX_V1) && VTSS_SWITCH_STACKABLE 
       /* Create mac table thread */
       cyg_thread_create(THREAD_BELOW_NORMAL_PRIO,
                         mac_table_thread, 
                         0, 
                         "Mac Table Control", 
                         mac_table_thread_stack, 
                         sizeof(mac_table_thread_stack),
                         &mac_table_thread_handle,
                         &mac_table_thread_block);
       cyg_thread_resume(mac_table_thread_handle);  /* Only used for L28 */
#endif
       break;
    case INIT_CMD_START:
        T_D("INIT_CMD_START");
    ...
    case INIT_CMD_SWITCH_ADD:
...
#if defined(VTSS_FEATURE_VSTAX_V2) && VTSS_SWITCH_STACKABLE
        /* Store the Port Map for the added isid */
        mac_stack_port_map_get(isid); 
        ...
        for (port_no = VTSS_PORT_NO_START+1; (port_no < data->port_cnt) && (port_no != data->stack_ports[0]); port_no++) {
            ...
            if (upsid_table.upsid[isid][0] != upsid) {
                ...
                upsid_table.port_no = port_no; /* First port on second UPSID */
                break;
            }            
        }
#endif /* VTSS_FEATURE_VSTAX_V2 */
