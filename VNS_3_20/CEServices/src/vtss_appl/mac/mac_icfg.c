...
static vtss_rc mac_icfg_global_conf(const vtss_icfg_query_request_t *req,
                                     vtss_icfg_query_result_t *result)
{
    ...
    if (req->all_defaults || (conf.mac_age_time != MAC_AGE_TIME_DEFAULT)) {
        VTSS_RC(vtss_icfg_printf(result, "%smac address-table aging-time %lu\n", 
                                 conf.mac_age_time == MAC_AGE_TIME_DEFAULT ? "no " : "", conf.mac_age_time));

    } 

    /* Global level: mac address-table static */
        ...
        while (mac_mgmt_static_get_next(sit.isid, &vid_mac, &mac_entry, 1, 0) == VTSS_OK) {
            ...
            for (iport = VTSS_PORT_NO_START; iport < VTSS_PORTS; iport++) { 
                if (mac_entry.destination[iport]) {
                    (void)icli_port_info_txt(topo_isid2usid(sit.isid), iport2uport(iport), buf);
                    sprintf(dest_buf,"%s %s",dest_buf,buf);
                }
            }
            VTSS_RC(vtss_icfg_printf(result, "mac address table static %s vlan %lu interface %s\n",
...
static vtss_rc mac_icfg_intf_conf(const vtss_icfg_query_request_t *req,
                                  vtss_icfg_query_result_t *result)
{
    vtss_rc             rc = VTSS_OK;
    vtss_isid_t         isid;
    ...
    if (req->all_defaults || !mode.automatic) {
        if (mode.discard) {
            VTSS_RC(vtss_icfg_printf(result, " mac address secure\n"));
        } else {
            VTSS_RC(vtss_icfg_printf(result, " %smac address learning\n",mode.automatic ? "" : "no "));
        }
    }   
...
vtss_rc mac_icfg_init(void)
{
    ...
    if ((rc = vtss_icfg_query_register(VTSS_ICFG_MAC_INTERFACE_CONF, mac_icfg_intf_conf)) != VTSS_OK) {
        return rc;
    }

    if ((rc = vtss_icfg_query_register(VTSS_ICFG_MAC_GLOBAL_CONF, mac_icfg_global_conf)) != VTSS_OK) {
        return rc;
    }
