...
/* Structure for temporarily age time change */
typedef struct {
...
typedef enum {
    ...
    MAC_MSG_ID_PORTMAP_REQ,           /* request */
    MAC_MSG_ID_PORTMAP_REP            /* reply   */
} mac_msg_id_t;
...
typedef struct {
    ...
    union {
        ...
        /* MAC_MSG_ID_GET_NEXT_STACK_REQ */
        struct {
            vtss_vid_mac_t         vid_mac;    /* Mac addr query */
            mac_mgmt_addr_type_t   type;       /* Type of address  */
            BOOL                   next;       /* Get next or lookup */
        } get_next_stack;
#endif
...
#if defined(VTSS_FEATURE_VSTAX_V1) && VTSS_SWITCH_STACKABLE 
/* All addresses in the switch */
struct {
    vtss_mac_table_entry_t  mac_list[VTSS_MAC_ADDRS];
    int mac_index;
} mac_ram_table;
#endif
