...
static void DHCP_RELAY_stack_receive_callback(const void *packet, size_t len, vtss_vid_t vid, vtss_isid_t isid, vtss_port_no_t src_port, vtss_glag_no_t glag_no)
{
#if defined(VTSS_SW_OPTION_IP2)
    ...
    if (vtss_ip2_if_conf_get((vtss_if_id_vlan_t) vid, &if_conf) == VTSS_RC_OK) {
        ...
        for (ifid = VTSS_VID_DEFAULT; ifid < VTSS_VID_RESERVED; ifid++) {
    ...
    /* Get rl by mapping from mamaged VID because the packet should forward to
       socket first then goto \base\relay\dhcrelay\dp_realy4() */
...
static void DHCP_RELAY_engine_conf_set(dhcp_relay_conf_t *conf)
{
    ...
    iscdhcp_relay_mode_set(conf->relay_mode);
...
static void DHCP_RELAY_conf_apply(void)
{
    if (msg_switch_is_master()) {
        ...
        if (DHCP_RELAY_global.dhcp_relay_conf.relay_mode) {
            dhcp_helper_relay_receive_register(DHCP_RELAY_stack_receive_callback);
        } else {
            dhcp_helper_relay_receive_unregister();
        }
#if DHCP_RELAY_USING_ISCDHCP_PACKAGE
...
vtss_rc dhcp_relay_mgmt_conf_set(dhcp_relay_conf_t *glbl_cfg)
{
    ...
    if ((glbl_cfg->relay_mode && !glbl_cfg->relay_server_cnt) ||
        (glbl_cfg->relay_info_mode == DHCP_RELAY_MGMT_DISABLED && glbl_cfg->relay_info_policy == DHCP_RELAY_INFO_POLICY_REPLACE)) {
        return DHCP_RELAY_ERROR_INV_PARAM;
    }
...
static void DHCP_RELAY_thread(cyg_addrword_t data)
{
    ...
    /* The switch IP address can be static assignment or getting by DHCP client.
       Since the relay agent forward the DHCP discover message to DHCP server via
       unicast package. If the switch IP address is getting by DHCP client. It
       should be done before agent running. That is, the DHCP relay module *MUST*
       running after switch IP address active. */
    do {
        memset(&ip_conf, 0, sizeof(ip_conf));
        if (msg_switch_is_master()) {
            if (ip_mgmt_conf_cur_get(&ip_conf) != VTSS_OK) {
                T_D("Calling ip_mgmt_conf_cur_get() failed.\n");
            }
        } else {
            // No reason for using CPU ressources when we're a slave
            T_D("Suspending DHCP relay thread");
...
static int DHCP_RELAY_update_circuit_id_callback(u8 *mac, uint transaction_id, u8 *circuit_id)
{
    ...
    if (!circuit_id || !dhcp_helper_frame_info_lookup(mac, transaction_id, &client_info)) {
...
static int DHCP_RELAY_send_client_callback(char *raw, size_t len, struct sockaddr_in *to, u8 *mac, uint transaction_id)
{
    ...
    u32                         ifct;
#else
    ip_conf_t                   system_ip_conf;
#endif /* VTSS_SW_OPTION_IP2 */
    dhcp_helper_frame_info_t    client_info;
    static u16                  DHCP_RELAY_send_to_client_ip_id = 0;
    int                         rc;

    if (!dhcp_helper_frame_info_lookup(mac, transaction_id, &client_info)) {
        /* Could not find the client interface */
        return -1;
    }

    if ((pkt_buf = dhcp_helper_alloc_xmit(pkt_len, client_info.vid, client_info.isid, client_info.port_no, VTSS_PORT_NO_NONE, VTSS_GLAG_NO_NONE, &bufref)) == NULL) {
        /* Allocate fail */
        return -1;
    }
    ...
    *(pkt_buf + 13) = 0x0;
...
#else
    if (ip_mgmt_conf_cur_get(&system_ip_conf) == VTSS_OK) {
        ip_hdr->ip_src.s_addr = htonl(system_ip_conf.ipv4_addr);
    }
#endif /* VTSS_SW_OPTION_IP2 */
    ...
    rc = dhcp_helper_xmit(pkt_buf, pkt_len, client_info.vid, client_info.isid, client_info.port_no, VTSS_PORT_NO_NONE, VTSS_GLAG_NO_NONE, bufref, 1);
    return (rc);
}
...
static void DHCP_RELAY_fill_giaddr_callback(u8 *mac, uint transaction_id, vtss_ipv4_t *agent_ipv4_addr)
{
    ...
    if (agent_ipv4_addr && ip_mgmt_conf_cur_get(&ip_conf) == VTSS_OK) {
        if (ip_conf.ipv4_addr) {
            *agent_ipv4_addr = htonl(ip_conf.ipv4_addr);
        }
    }
