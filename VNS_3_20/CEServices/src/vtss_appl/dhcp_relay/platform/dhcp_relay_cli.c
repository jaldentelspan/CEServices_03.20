                                    ...
                                    BOOL relay_info_mode, BOOL relay_info_policy)
{
    ...
    if (req->set) {
        if (relay_mode) {
            if (req->enable && !conf.relay_server_cnt) {
                CPRINTF("At least one valid server\n");
                return;
            }
            conf.relay_mode = req->enable;
        }
        if (relay_server_add) {
            if (conf.relay_mode && req->ipv4_addr == 0) {
                CPRINTF("Parameter <ip_addr> doesn't allowed '0.0.0.0' when relay mode is enabled\n");
                return;
            }
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Configuration",
    NULL,
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Mode",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Mode [enable|disable]",
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Server",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Server [<ip_addr>]",
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Information Mode",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Information Mode [enable|disable]",
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Information Policy",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Information Policy [replace|keep|drop]",
    "Set or show the DHCP relay mode.\n"
...
cli_cmd_tab_entry(
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Statistics",
    VTSS_CLI_GRP_SEC_NETWORK_PATH "DHCP Relay Statistics [clear]",
