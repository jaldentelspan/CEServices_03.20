...
static void FPGA_i2c_debug_read(cli_req_t *req)
{
    ...
    reg_info.regnum = fpga_req->reg_addr;
    reg_info.shift = 0;
    if (_GetRegister(reg_info, &data) == 0)
    {
        CPRINTF("Data read from FPGA I2C = 0x%08X, reg_addr = %d \n",
                data,
                fpga_req->reg_addr);
    }
    else
...
static void FPGA_i2c_debug_write(cli_req_t *req)
{
    ...
    {
        CPRINTF("Wrote 0x%08X to register 0x%X.\n", fpga_req->data_word, fpga_req->reg_addr);
    }
    else
    {
    ...
    {  
                ...
                conf.dst_port == VTSS_PORT_NO_NONE) 
        {
            ...
            if( fpga_req->epe_config.bit_rate < VNS_EPE_BIT_RATE_MIN 
                    || fpga_req->epe_config.bit_rate > VNS_EPE_BIT_RATE_MIN )
            {
            ...
            if (!set_vns_fpga_epe_conf_debug(epe_conf) ) {
                /* CPRINTF("\n%s\n", error_txt(rc)); */
...
static cli_parm_t vns_fpga_cli_parm_table[] = {
    ...
    {
        "<bit_rate>",
        "bitrate: The bitrate is set as 2000-20000 kbps ",
        CLI_PARM_FLAG_SET,
        cli_epe_port_bit_rate,
        cli_cmd_epe_port
    },
    {
        "encode|decode|disable",
        "Selects PCM state",
        CLI_PARM_FLAG_SET,
        cli_epe_port_dis,
        cli_cmd_epe_port
...
cli_cmd_tab_entry (
        NULL,
        "Debug .WRITE <i2c_reg> <i2c_data_word> ",
