...
static port_cap_t port_isid_port_cap(vtss_isid_t isid, vtss_port_no_t port_no)
{
...
static vtss_rc phy_10g_speed_setup(vtss_port_no_t port_no, vtss_port_speed_t speed)
{
    ...
    if ((mode.oper_mode == VTSS_PHY_1G_MODE && speed == VTSS_SPEED_1G) || 
    ...
    if (speed == VTSS_SPEED_1G) {
        ...
        mode.oper_mode = VTSS_PHY_LAN_MODE; /* Default to 10G LAN mode */
...
static vtss_rc port_setup(vtss_port_no_t port_no, u32 change)
{
    ...
    status_cap = port.status[VTSS_ISID_LOCAL][i].cap;
    ...
    if (change & PORT_CONF_CHANGE_PHY) {
        ...
        if (port_phy(port_no)) {
            ...
            if (conf->enable) {                
                if ((conf->fiber_speed == VTSS_SPEED_FIBER_100FX || conf->fiber_speed == VTSS_SPEED_FIBER_1000X) &&
                    !conf->autoneg) {
                    /* Forced fiber mode */
    ...
    ps->if_type = (cap & PORT_CAP_SFP_DETECT) ? port_mac_sfp_interface(port_no) : port_mac_interface(port_no);
    ...
    if (ps->loop)
        vtss_port_state_set(NULL, port_no, 1);  /* Always enable port with active loop back */
    ...
    if ((cap & PORT_CAP_VTSS_10G_PHY) && (cap & PORT_CAP_1G_FDX)) {
        if (phy_10g_speed_setup(port_no, (conf->speed == VTSS_SPEED_2500M) ? VTSS_SPEED_1G : conf->speed) != VTSS_RC_OK) {
            rc = VTSS_RC_ERROR;
        }
    }
...
static void port_status_poll(vtss_port_no_t port_no, BOOL *changed, vtss_event_t *link_down, BOOL fast_link_failure)
{
    ...
    if (port_phy(port_no)) {               
        T_R_PORT(port_no, "phy status get");        
        ...
        if (port.status[VTSS_ISID_LOCAL][port_no - VTSS_PORT_NO_START].sfp.type == PORT_SFP_1000BASE_T) {
            ...
            if (board_sfp_i2c_lock(1) != VTSS_RC_OK) {
                ...
                T_DG_PORT(TRACE_GRP_SFP, port_no, "rc:0x%X", rc);
                ...
                T_DG_PORT(TRACE_GRP_SFP, port_no, "sfp_data:0x%X 0x%X, reg5:0x%X  phy_setup.aneg.speed_1g_fdx:%d", 
                          sfp_phy[0], sfp_phy[1], reg5, phy_setup.aneg.speed_1g_fdx);
    ...
    if ((!status->link || status->link_down) && old_status.link) {
        ...
        *link_down = 1;
        old_status.link = 0;
        if (!(conf->adv_dis & PORT_ADV_UP_MEP_LOOP))
            vtss_port_state_set(NULL, port_no, 0);  /* Do not disable port with active loop back */
        /* Note, vtss_mac_table_port_flush() is moved to the MAC module, see BZ#4993 */
        port_status->fiber = 0;
        ...
        if (old_status.link) {
            ...
            if (conf->autoneg) {
                if (!port_phy(port_no)) {
                    ...
                    if ((status->aneg.generate_pause != old_status.aneg.generate_pause) ||
                        (status->aneg.obey_pause     != old_status.aneg.obey_pause) ||
                        (status->speed               != old_status.speed) ||
                        (status->fdx                 != old_status.fdx)) {
            ...
            if (port_custom_table[port_no].cap & (PORT_CAP_DUAL_COPPER | PORT_CAP_DUAL_FIBER | PORT_CAP_DUAL_FIBER_100FX)) {
                /* Read AMS status */
                vtss_phy_read(PHY_INST, port_no, 20 | VTSS_PHY_REG_EXTENDED, &reg);
                port_status->fiber = (((reg >> 6) & 0x3) == 2 ? 1 : 0);
                T_I("port_status->fiber =%d, reg =0x%X", port_status->fiber, reg);
            }
        }
        *changed = 1;
        port_change_event(port_no, port_status, 1);
...
static u32 port_conf_change(port_conf_t *old, port_conf_t *new)
{
    u32 change;

    if (new->autoneg != old->autoneg ||
        new->fiber_speed != old->fiber_speed) {
        /* Full change */
        change = PORT_CONF_CHANGE_ALL;
    } else if (new->enable != old->enable ||
               new->speed != old->speed ||
               new->fdx != old->fdx ||
               new->flow_control != old->flow_control ||
#ifdef VTSS_SW_OPTION_PHY_POWER_CONTROL
               new->power_mode != old->power_mode ||
#endif /* VTSS_SW_OPTION_PHY_POWER_CONTROL */
               new->adv_dis != old->adv_dis) {
        /* PHY and MAC change */
        change = (PORT_CONF_CHANGE_PHY | PORT_CONF_CHANGE_MAC);
    } else if (new->exc_col_cont != old->exc_col_cont ||
               new->max_tags != old->max_tags ||
...
static BOOL port_vol_conf_changed(port_vol_conf_t *old, port_vol_conf_t *new)
{
    return (old->disable != new->disable || old->loop != new->loop ? 1 : 0);
}
...
static BOOL cisco_aneg_complete(vtss_port_no_t port_no)
{
    ...
    memset(&conf, 0, sizeof(vtss_port_status_t));
...
{  
    ...
    T_IG_PORT(TRACE_GRP_SFP, port_no, "sfp_type = %d", sfp.type);
...
static char *port_msg_id_txt(port_msg_id_t msg_id)
{
    char *txt;
...
{
    port_msg_req_t *msg = msg_buf_pool_get(port.request);
    msg->msg_id = msg_id;
    return msg;
}
...
{
    port_msg_rep_t *msg = msg_buf_pool_get(port.reply);
    msg->msg_id = msg_id;
    return msg;
}
...
static void phy_fiber_speed_update(vtss_port_no_t port_no, vtss_fiber_port_speed_t fiber_speed) {
    ...
    if (port_phy(port_no)) {
        ...
        case VTSS_SPEED_FIBER_AUTO:
            T_NG_PORT(TRACE_GRP_SFP, port_no, "phy_reset.media_if = VTSS_SPEED_FIBER_AUTO, sfp_type  =%d", port.status[VTSS_ISID_LOCAL][port_no].sfp.type);
        ...
        case VTSS_SPEED_FIBER_NOT_SUPPORTED:
        ...
        T_N_PORT(port_no, "fiber_speed:%d, auto:%d, media:%d", fiber_speed, conf->autoneg, phy_reset.media_if);
    }
}

/* Receive port message */
static BOOL port_msg_rx(void *contxt, const void * const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
                                        ...
                                        u32                            port_no)
{
    ...
    vtss_interrupt_source_hook_set(port_los_interrupt_function,
                                   source_id,
                                        ...
                                        u32                            port_no)
{
    /* hook up to the next interrupt */
    /* to achieve backpressure this should be done in the thread just before taken action (reading the actual status) on the interrupt */
    vtss_interrupt_source_hook_set(port_ams_interrupt_function,
                                   INTERRUPT_SOURCE_AMS,
                                   INTERRUPT_PRIORITY_NORMAL);
...
vtss_rc switch_iter_init(switch_iter_t *sit, vtss_isid_t isid, switch_iter_sort_order_t sort_order)
{
    ...
    if (sort_order > SWITCH_ITER_SORT_ORDER_ISID_ALL) {
    ...
    if (isid == VTSS_ISID_GLOBAL) {
        ...
        if (sort_order == SWITCH_ITER_SORT_ORDER_USID) {
            vtss_usid_t s;
        ...
        BOOL exists = msg_switch_exists(isid);
        if ((sort_order == SWITCH_ITER_SORT_ORDER_ISID_ALL) || exists) {
            sit->remaining = 1;
            sit->m_switch_mask = 1LU << isid;
            T_DG(TRACE_GRP_ITER, "add isid %u to m_switch_mask", isid);
            if (exists) {
                sit->m_exists_mask = 1LU << isid;
                T_DG(TRACE_GRP_ITER, "add isid %u to m_exists_mask", isid);
            }
        }
    }
...
BOOL switch_iter_getnext(switch_iter_t *sit)
{
    ...
    case SWITCH_ITER_STATE_NEXT:
        if (sit->m_switch_mask) {
            ...
            if (sit->m_order == SWITCH_ITER_SORT_ORDER_USID) {
...
vtss_port_no_t port_physical2logical(vtss_chip_no_t chip_no, uint chip_port, vtss_glag_no_t *glag_no)
{
    ...
    *glag_no = VTSS_GLAG_NO_NONE; // Undefined aggregation number
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < port.port_count[VTSS_ISID_LOCAL]; port_no++)
        if (port_custom_table[port_no].map.chip_no == chip_no && port_custom_table[port_no].map.chip_port == chip_port)
            return port_no;
...
vtss_rc port_vol_status_get(port_user_t user, vtss_isid_t isid,
                            vtss_port_no_t port_no, port_vol_status_t *status)
{
    ...
    for (usr = PORT_USER_STATIC; usr < PORT_USER_CNT; usr++) {
        ...
        if (usr == user || (vol_conf->disable == 0 && disable)) {
            ...
            if (usr == PORT_USER_ACL)
                strcpy(status->name, "ACL");
#endif /* VTSS_SW_OPTION_ACL */
#ifdef VTSS_SW_OPTION_THERMAL_PROTECT
            if (usr == PORT_USER_THERMAL_PROTECT)
                strcpy(status->name, "Thermal");
#endif /* VTSS_SW_OPTION_THERMAL_PROTECT */
#ifdef VTSS_SW_OPTION_LOOP_PROTECT
            if (usr == PORT_USER_LOOP_PROTECT)
                strcpy(status->name, "Loop");
#endif /* VTSS_SW_OPTION_LOOP_PROTECT */
#ifdef VTSS_SW_OPTION_MEP
            if (usr == PORT_USER_MEP)
                strcpy(status->name, "MEP");
#endif /* VTSS_SW_OPTION_LOOP_PROTECT */
        }
...
char *port_mgmt_mode_txt(vtss_port_speed_t speed, BOOL fdx, BOOL fiber)
{
    switch (speed) {
    case VTSS_SPEED_10M:
...
vtss_rc port_mgmt_conf_get(vtss_isid_t isid, vtss_port_no_t port_no, port_conf_t *conf)
{
    T_D("enter, isid: %d, port_no: %u", isid, port_no);
...
static vtss_rc do_phy_reset (vtss_port_no_t port_no, const port_cap_t cap) {
...
/* Reset PHY */
    ...
    if (rc != VTSS_OK) {
        T_E("vtss_phy_reset failed, port_no: %u", port_no);
    }
    return rc;
}

/* Set port configuration */
vtss_rc port_mgmt_conf_set(vtss_isid_t isid, vtss_port_no_t port_no, port_conf_t *conf)
{
    vtss_rc           rc = VTSS_OK;
    u32               i, change;
    port_conf_blk_t   *blk;
    ...
    vtss_fiber_port_speed_t fiber_speed = conf->fiber_speed;
    ...
    T_D("enter, isid: %d, port_no: %u, %s, %s mode, %d",
        ...
        conf->enable ? (conf->autoneg ? "auto" :
                        port_mgmt_mode_txt(speed, fdx, FALSE)) : "disabled",
    ...
    T_N_PORT(port_no, "fiber_speed = %d, cap = 0x%X, PORT_CAP_FIBER_1000X =0x%X, cap & PORT_CAP_FIBER_100F = 0x%X", fiber_speed, cap, PORT_CAP_FIBER_1000X, cap & PORT_CAP_FIBER_100FX);
    switch (fiber_speed) {

    case VTSS_SPEED_FIBER_100FX:
        mode_ok = ((cap & PORT_CAP_FIBER_100FX) != 0);
        break;

    case VTSS_SPEED_FIBER_1000X:
        mode_ok = ((cap & PORT_CAP_FIBER_1000X) != 0);
        break;
    ...
    if (!mode_ok) {
        T_D_PORT(port_no, "Fiber mode %s not supported");
        return PORT_ERROR_PARM;
    }
    ...
    if (!conf->autoneg) {
        ...
        if (!mode_ok) {
	    T_D("mode %s not supported on port_no %u", port_mgmt_mode_txt(speed, fdx, FALSE), port_no);
            return PORT_ERROR_PARM;
        }
    }
...
#ifdef VTSS_SW_OPTION_PHY_POWER_CONTROL
    if (conf->power_mode != VTSS_PHY_POWER_NOMINAL && !port_isid_phy(isid, port_no)) {
...
vtss_rc port_mgmt_status_get_all(vtss_isid_t isid, vtss_port_no_t port_no, port_status_t *status)
{
    return port_status_get(isid, port_no, status, 1);
}

/* Get port counters */
vtss_rc port_mgmt_counters_get(vtss_isid_t isid,
                               vtss_port_no_t port_no, vtss_port_counters_t *counters)
{
    vtss_rc rc = VTSS_OK;
...
/* Clear port counters */
vtss_rc port_mgmt_counters_clear(vtss_isid_t isid, vtss_port_no_t port_no)
{
    vtss_rc rc = VTSS_OK;
...
char *port_mgmt_veriphy_txt(vtss_phy_veriphy_status_t status)
{
    char *txt;
                                ...
                                port_veriphy_mode_t mode[VTSS_PORT_ARRAY_SIZE])
{
    vtss_port_no_t      port_no;
    port_veriphy_t      *veriphy;
    port_veriphy_mode_t port_mode[VTSS_PORT_ARRAY_SIZE], start = 0;
...
/* Get VeriPHY result */
vtss_rc port_mgmt_veriphy_get(vtss_isid_t isid, vtss_port_no_t port_no,
    ...
    for (timer = 0; ; timer++) {
        ...
        if (!veriphy->running) {
            *result = veriphy->result;
            ...
            rc = (veriphy->valid ? VTSS_OK : PORT_ERROR_GEN);
        }
        PORT_CRIT_EXIT();
...
/* Set defaults for local port */
static void port_conf_default(vtss_isid_t isid, vtss_port_no_t port_no, port_conf_t *conf)
{
    ...
    if (cap & PORT_CAP_SPEED_DUAL_ANY_FIBER) {
        conf->fiber_speed = VTSS_SPEED_FIBER_1000X;
    } else {
      conf->fiber_speed = VTSS_SPEED_FIBER_NOT_SUPPORTED;
    }
...
static void port_conf_check(vtss_isid_t isid, vtss_port_no_t port_no, port_conf_t *conf)
{
    ...
    if ((port_isid_port_cap(isid, port_no) & cap) == 0) {
        port_conf_default(isid, port_no, conf);
    }
}
...
static void port_conf_read(vtss_isid_t isid, BOOL do_create, BOOL *changed)
{
    ...
    T_D("enter, isid: %d, do_create: %u", isid, do_create);

    /* Open or create configuration block */
    if ((blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_PORT_CONF_TABLE, &size)) == NULL ||
        size != sizeof(*blk)) {
        T_W("conf_sec_open failed or size mismatch, creating defaults");
        blk = conf_sec_create(CONF_SEC_GLOBAL, CONF_BLK_PORT_CONF_TABLE, sizeof(*blk));
        do_create = 1;
    ...
    *changed = 0;
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < port.port_count[isid]; port_no++) {
        ...
        // VTSS_SPEED_FIBER_DISABLED is obsolete and VTSS_SPEED_FIBER_NOT_SUPPORTED should be used instead. This is for being backward compatible
        if (port_conf->fiber_speed == VTSS_SPEED_FIBER_DISABLED) {
          port_conf->fiber_speed = VTSS_SPEED_FIBER_NOT_SUPPORTED;
        }

        // VTSS_SPEED_FIBER_DISABLED is obsolete and VTSS_SPEED_FIBER_NOT_SUPPORTED should be used instead. This is for being backward compatible
        if (conf.fiber_speed == VTSS_SPEED_FIBER_DISABLED) {
          conf.fiber_speed = VTSS_SPEED_FIBER_NOT_SUPPORTED;
        }
...
static void port_init_ports(void)
{
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
       ...
       /* Update sfp interface with the default mac interface */
        ...
        port_conf_default(VTSS_ISID_LOCAL, port_no, &conf);
#if defined(VTSS_SW_OPTION_MEP_LOOP_PORT)
...
static void port_thread(cyg_addrword_t data)
{
    ...
    if(vtss_board_features() & VTSS_BOARD_FEATURE_LOS) {
        vtss_interrupt_source_hook_set(port_los_interrupt_function, INTERRUPT_SOURCE_FLNK, INTERRUPT_PRIORITY_NORMAL);
        vtss_interrupt_source_hook_set(port_los_interrupt_function, INTERRUPT_SOURCE_LOS, INTERRUPT_PRIORITY_NORMAL);
    }
    if(vtss_board_features() & VTSS_BOARD_FEATURE_AMS)
        vtss_interrupt_source_hook_set(port_ams_interrupt_function, INTERRUPT_SOURCE_AMS, INTERRUPT_PRIORITY_NORMAL);
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
            ...
            if (fast_port_no == VTSS_PORT_NO_NONE || fast_link_int[i]) {
                ...
                if (fast_port_no == VTSS_PORT_NO_NONE) {
                    ...
                    T_NG_PORT(TRACE_GRP_SFP, port_no, "port_custom_table[port_no].cap:0x%X", port_custom_table[port_no].cap);     
                    if (port_custom_table[port_no].cap & PORT_CAP_SFP_DETECT || port_custom_table[port_no].cap & PORT_CAP_DUAL_SFP_DETECT) {
                        ...
                        T_NG_PORT(TRACE_GRP_SFP, port_no, "New SFP :%d, old:%d, new%d", 
                                  sfp_status_old[port_no] != sfp_status_new[port_no], sfp_status_old[port_no], sfp_status_new[port_no]);
                        ...
                        if (sfp_status_old[port_no] != sfp_status_new[port_no]) {
                            ...
                            // Change MAC interface if SFP module is connected directly to a MAC interface (Not through a PHY)
                            if (!port_phy(port_no)) {
                                ...
                                phy_fiber_speed_update(port_no, port.config[VTSS_ISID_LOCAL][port_no].fiber_speed);
                            }
                        }
                    }                    
#endif /* VTSS_SW_OPTION_I2C */
                        ...
                        if (veriphy->running) {
                            ...
                            if (rc != VTSS_RC_INCOMPLETE) {
                                ...
                                for (i = 0; i  < 4; i++) {
                                    ...
                                    if (poe_port_status.port_status[p] != NO_PD_DETECTED && poe_port_status.port_status[p] != POE_NOT_SUPPORTED) {
...
vtss_rc port_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        /* The request buffer pool consists of two buffers because of the two requests sent upon a SWITCH_ADD event */
        port.request = msg_buf_pool_create(VTSS_MODULE_ID_PORT, "Request", 2, sizeof(port_msg_req_t));
    ...
    case INIT_CMD_SWITCH_ADD:
        ...
        /* Store ISID information */
        port.board_type[isid] = data->board_type;
        port.port_count[isid] = data->port_cnt;
        port.stack_port_0[isid] = data->stack_ports[0];
        port.stack_port_1[isid] = data->stack_ports[1];
