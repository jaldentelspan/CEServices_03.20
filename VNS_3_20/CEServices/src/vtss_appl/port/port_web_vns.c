...
void stat_portstate_switch(CYG_HTTPD_STATE* p, vtss_usid_t usid, vtss_isid_t isid)
{
    ...
    char               combo_port = 0;
    char               sfp_port = 0;
    BOOL	       lu26 = (vtss_board_type() == VTSS_BOARD_LUTON26_REF ||vtss_board_type() == VTSS_BOARD_VNS_12_REF ||vtss_board_type() == VTSS_BOARD_VNS_16_REF);

    /* Backround, SID, managed */
    ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d|%s|",
                  usid,
                  lu26 ? "switch_ies12.png" : "switch_small.png");
