...
/* Get indication if port is a PHY or not */
 ...
 *
 * If this function is called on a slave, it'll return VTSS_UNSPECIFIED_ERROR.
 ...
 *
 * If isid == VTSS_ISID_GLOBAL, switch_iter_getnext() the returned number of switches are 0 to VTSS_ISID_CNT:
 *   If sort_order != SWITCH_ITER_SORT_ORDER_ISID_ALL, all existing switches are returned by switch_iter_getnext().
 *   If sort_order == SWITCH_ITER_SORT_ORDER_ISID_ALL, all existing and non-existing switches are returned by switch_iter_getnext().
 *
 * If isid != VTSS_ISID_GLOBAL, switch_iter_getnext() the returned number of switches are 0 to 1:
 *   If sort_order != SWITCH_ITER_SORT_ORDER_ISID_ALL and the switch exists it is returned by switch_iter_getnext().
...
vtss_rc port_mgmt_status_get_all(vtss_isid_t isid, vtss_port_no_t port_no, 
