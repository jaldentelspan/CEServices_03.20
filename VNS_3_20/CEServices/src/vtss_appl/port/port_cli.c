...
static char *sfp_if2txt(sfp_tranceiver_t sfp)
{
    switch (sfp) {
...
void port_cli_init(void)
{
    ...
    txt = port_sfp_cmd;
    if ((info.cap & PORT_CAP_SFP_DETECT) == 0)
        txt += sprintf(txt, "Debug ");
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
            ...
            if (req->set || req->clear) {
                ...
                if (flags & PORT_FLAGS_FIBER_MODE){
                    if (info.cap & PORT_CAP_SPEED_DUAL_ANY_FIBER) {
                        ...
                        if (req->disable) {
                            ...
                            conf.fiber_speed = port_req->fiber_speed;
                        }
                    } else {
                        if (port_req->fiber_speed != VTSS_SPEED_FIBER_NOT_SUPPORTED) {
                ...
                if (flags & PORT_FLAGS_MODE) {
                    BOOL no_fiber = (conf.fiber_speed == VTSS_SPEED_FIBER_NOT_SUPPORTED);
                    T_I_PORT(iport, "no_fiber = %d, fiber speed = %d auto neg = %d, fiber_speed_tx = %s, port_status.fiber:%d", 
                             no_fiber, conf.fiber_speed,conf.autoneg, port_fiber_mgmt_mode_txt(conf.fiber_speed, conf.autoneg), port_status.fiber);
                    if (no_fiber) {
                        CPRINTF("%-13s",
                                conf.autoneg ? "Auto" : port_mgmt_mode_txt(conf.speed, conf.fdx, FALSE));
                    } else {
                        CPRINTF("%-13s",
                                port_fiber_mgmt_mode_txt(conf.fiber_speed, conf.autoneg));
                    }
                }
                ...
                if (flags & (PORT_FLAGS_MODE | PORT_FLAGS_FIBER_MODE)) {
                    if (status->link) {
                        sprintf(buf, "%s", 
                                port_mgmt_mode_txt(status->speed, status->fdx, port_status.fiber));
...
static void cli_cmd_port_veriphy(cli_req_t *req)
{
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
            uport = iport2uport(iport);
            mode[iport] = (req->uport_list[uport] == 0 ||
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
            ...
            T_N("%d, %d, %d %d", (req->uport_list[uport] == 0), (port_isid_port_info_get(isid, iport, &info) != VTSS_OK),
                (info.cap & PORT_CAP_1G_PHY) == 0, (port_mgmt_veriphy_get(isid, iport, &veriphy, 20) != VTSS_OK));

            if (req->uport_list[uport] == 0 ||
                port_isid_port_info_get(isid, iport, &info) != VTSS_OK ||
                (info.cap & PORT_CAP_1G_PHY) == 0 ||
                port_mgmt_veriphy_get(isid, iport, &veriphy, 40) != VTSS_OK)
                continue;
...
static void cli_cmd_port_debug_info(cli_req_t *req)
{
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        CPRINTF("%-6u%-6u%-28s(%u)  %-12u", 
				...
				type == VTSS_BOARD_VNS_6_REF ? "VNS_6_REF" :
				type == VTSS_BOARD_VNS_8_REF ? "VNS_8_REF" :
				type == VTSS_BOARD_VNS_12_REF ? "VNS_12_REF" :
				type == VTSS_BOARD_VNS_16_REF ? "VNS_16_REF" :
				type == VTSS_BOARD_NCGU_REF ? "NCGU_REF" :
				type == VTSS_BOARD_IGU_REF ? "IGU_REF" :
				type == VTSS_BOARD_MITR_REF ? "MITR_REF" :
				type == VTSS_BOARD_ESIR_REF ? "MITR_ESIR" :
				type == VTSS_BOARD_VESM_REF ? "VESM_REF" :
