...
)
{
    ...
    case ICLI_CMD_MODE_GLOBAL_CONFIG:
        ...
        while ( rmon_mgmt_alarm_entry_get(&alarm_entry, TRUE) == VTSS_OK ) {
            rc = vtss_icfg_printf(result, "rmon alarm %lu %s %lu %s rising-threshold %ld %lu falling-threshold %ld %lu %s\n",
                                  alarm_entry.id,
                                  _oid_2_str(alarm_entry.var_name.objid, alarm_entry.var_name.length, buf),
                                  alarm_entry.interval,
                                  (alarm_entry.sample_type == SAMPLE_TYPE_ABSOLUTE) ? "absoluteValue" : "deltaValue",
        ...
        while ( rmon_mgmt_event_entry_get(&event_entry, TRUE) == VTSS_OK ) {
            ...
            case EVENT_NONE:
                if ( event_entry.event_description ) {
                    rc = vtss_icfg_printf(result, "rmon event %lu description %s\n",
                                          event_entry.id,
                                          event_entry.event_description
                                         );
                } else {
                    rc = vtss_icfg_printf(result, "rmon event %lu\n",
                                          event_entry.id
                                         );
                }
                break;
            ...
            case EVENT_LOG:
                if ( event_entry.event_description ) {
                    rc = vtss_icfg_printf(result, "rmon event %lu log description %s\n",
                                          event_entry.id,
                                          event_entry.event_description
                                         );
                } else {
                    rc = vtss_icfg_printf(result, "rmon event %lu log\n",
                                          event_entry.id
                                         );
                }
                break;
            ...
            case EVENT_TRAP:
                if ( event_entry.event_description ) {
                    rc = vtss_icfg_printf(result, "rmon event %lu trap %s description %s\n",
                                          event_entry.id,
                                          event_entry.event_community,
                                          event_entry.event_description
                                         );
                } else {
                    rc = vtss_icfg_printf(result, "rmon event %lu trap %s\n",
                                          event_entry.id,
                                          event_entry.event_community
                                         );
                }
            ...
            case EVENT_LOG_AND_TRAP:
                if ( event_entry.event_description ) {
                    rc = vtss_icfg_printf(result, "rmon event %lu log trap %s description %s\n",
                                          event_entry.id,
                                          event_entry.event_community,
                                          event_entry.event_description
                                         );
                } else {
                    rc = vtss_icfg_printf(result, "rmon event %lu log trap %s\n",
                                          event_entry.id,
                                          event_entry.event_community
                                         );
                }
    ...
    case ICLI_CMD_MODE_INTERFACE_PORT_LIST:
        ...
        while ( rmon_mgmt_statistics_entry_get(&stat_entry, TRUE) == VTSS_OK ) {
            ...
            if ( iftable_info.isid == isid && iftable_info.if_id == iport ) {
                rc = vtss_icfg_printf(result, " rmon collection stats %lu\n",
                                      stat_entry.id
                                     );
                if ( rc != VTSS_RC_OK ) {
                    T_E("fail to print to icfg\n");
        ...
        while ( rmon_mgmt_history_entry_get(&history_entry, TRUE) == VTSS_OK ) {
            ...
            if ( iftable_info.isid == isid && iftable_info.if_id == iport ) {
                rc = vtss_icfg_printf(result, " rmon collection history %lu buckets %lu interval %lu\n",
                                      history_entry.id,
...
vtss_rc rmon_icfg_init(void)
{
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_GLOBAL_RMON, _rmon_icfg);
    if ( rc != VTSS_OK ) {
        return rc;
    }
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_INTERFACE_ETHERNET_RMON, _rmon_icfg);
    return rc;
}
