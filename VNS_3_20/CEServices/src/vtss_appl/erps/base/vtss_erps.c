...
#define VTSS_ERPS_DISABLE_RAPS_FORWARDING(port) \
    if (((port) != 0) || erpg->raps_virt_channel) { \
    if ((erpg->ring_type == ERPS_RING_TYPE_MAJOR) || erpg->raps_virt_channel) { \
        if (vtss_erps_raps_forwarding(port,erpg->group_id,FALSE) != VTSS_RC_OK) { \
            vtss_erps_trace(" Error in enabling forwarding on groupId =",erpg->group_id); \
            return (ERPS_ERROR_RAPS_ENABLE_FORWARDING_FAILED); \
        } \
        } else { \
            vtss_erps_trace("RAPS fwd not disabled - ring_type is Sub-ring without virtual channel = ",erpg->group_id); \
        } \
    }
...
{
    if (sf_state) {
        if (erps_handle_local_event(FSM_EVENT_LOCAL_SF,instance,lport) 
            != ERPS_RC_OK) {
            return (VTSS_RC_ERROR);
        }
    } else if (!sf_state) {
        if (erps_handle_local_event(FSM_EVENT_LOCAL_CLEAR_SF,
            instance,lport) != ERPS_RC_OK) {
            return (VTSS_RC_ERROR);
        }
    }
    return (VTSS_RC_OK);
                                      ...
                                      BOOL virtual_channel)
{
    ...
    erpg->wtr_time     = MIN_TO_MS(RAPS_WTR_TIMEOUT_MIN_MINITES); 
...
static i32 vtss_erps_apply_interconnection_flush_logic (u32 erpg_instance)
{
    ...
    if (erpg == NULL) {
        vtss_erps_trace("\n discarding incoming R-APS PUD as no protection group"
...
i32 vtss_erps_rx (vtss_port_no_t port, u16 pdu_len, u8 *raps_pdu,u32 erpg_instance)
{
    ...
    erpg = &erpg_instances[erpg_instance];
    if (erpg == NULL) {
        vtss_erps_trace("discarding incoming R-APS PUD as no protection group"
                "associated with it",0);
        ERPS_CRITD_EXIT();
        return (ERPS_ERROR_GROUP_NOT_EXISTS);
    }
    ...
    if (erpg != NULL) {
        ...
        vtss_erps_trace("\n erpg has invalid pointer, thus not calling FSM grp =", erpg->group_id);
    }
...
static i32 erps_handle_local_event (i32 event_in, u32 erpg_instance, vtss_port_no_t lport)
{
    ...
    if (!erpg) {
        vtss_erps_trace("<FSM>: discarding incoming local event as no protection group associated with it",0);
        ERPS_CRITD_EXIT();
        return (ERPS_ERROR_GROUP_NOT_EXISTS);
    }
