...
******************************************************************************/
static void apply_configuration(erps_config_blk_t  *lblk)
{
    ...
    blk = calloc(1,sizeof(erps_config_blk_t));

    if (!blk) {
        return;
    }
    memcpy(blk,lblk,sizeof(erps_config_blk_t));

    for ( instance = 0; instance < ERPS_MAX_PROTECTION_GROUPS; instance++ ) {
        if (blk->groups[instance].enable == TRUE) {
...
vtss_rc vtss_erps_vlan_ingress_filter (vtss_port_no_t l2port, BOOL enable)
{
    ...
    if(l2port2port(API2L2PORT(l2port), &isid, &switchport)) {
        ...
        vtss_rc rc = vlan_mgmt_port_conf_set(isid, switchport, &vlan_pconf, VLAN_USER_ERPS);
        if(rc != VTSS_OK) {
        }
    } else {
                                    ...
                                    BOOL enable )
{
    ...
    if (mep_id >= VTSS_MEP_INSTANCE_MAX ) {
        T_NG(TRACE_GRP_CTRL,"MEP: mep_raps_forwarding returning error VTSS_MEP_INSTANCE_MAX");
        return (VTSS_RC_OK);
    }

    ret = mep_raps_forwarding(mep_id,erps_instance,enable);

    T_NG(TRACE_GRP_CTRL,"MEP: mep_raps_forwarding mep_id = %u erps_instance = %u enable = %d ",mep_id,erps_instance,enable);
                                   ...
                                   BOOL enable)
{
    ...
    ret += mep_eps_sf_register(raps_mep,erps_instance,MEP_EPS_TYPE_ERPS,enable);

    T_NG(TRACE_GRP_CTRL,"MEP: mep_eps_sf_register mep_id = %u erps_instance = %u enable = %d ",mep_id,erps_instance,enable);
    T_NG(TRACE_GRP_CTRL,"MEP: mep_eps_sf_register raps_mep = %u erps_instance = %u enable = %d ",raps_mep,erps_instance,enable);
                                          ...
                                          u32 erps_instance, BOOL enable )
{
    ...
    T_NG(TRACE_GRP_CTRL,"MEP: vtss_erps_mep_raps_transmission raps_mep = %u erps_instance = %u enable = %d ",raps_mep,erps_instance,enable);
    return(ret);
}
                             ...
                             const BOOL sf_state, const BOOL sd_state)
{
    ...
    if (emep == mep_id) {
        if (vtss_erps_sf_sd_state_set(instance,
                                      sf_state,sd_state,
                                      e_port) == VTSS_RC_OK) {
            return (VTSS_RC_OK);
        } else {
...
{
    raps_pdu_in_t   *req = malloc(sizeof(raps_pdu_in_t));
...
/*****************************************************************************p
                              MGMT Interface 
******************************************************************************/
vtss_rc erps_mgmt_set_protection_group_request (vtss_erps_mgmt_conf_t mgmt_req)
{
    ...
    mgmt_req.group_id = CONV_MGMTTOERPS_INSTANCE(mgmt_req.group_id);
    ...
    switch (mgmt_req.req_type) {
        ...
        case ERPS_CMD_PROTECTION_GROUP_ADD:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_create_protection_group group_id = %u east_port = %u west_port = %u", \
                                      mgmt_req.group_id,mgmt_req.data.create.east_port,mgmt_req.data.create.west_port);
           ret = vtss_erps_create_protection_group(mgmt_req.group_id,
        ...
        case ERPS_CMD_PROTECTION_GROUP_DELETE:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_delete_protection_group group_id = %u", mgmt_req.group_id);
           if (vtss_erps_get_protection_group_status(mgmt_req.group_id, &grp_status) == VTSS_RC_OK) {
               if (grp_status == ERPS_PROTECTION_GROUP_ACTIVE) { 
                   memset(&pconf, 0, sizeof(pconf));
                   pconf.group_id = mgmt_req.group_id;
                   if (vtss_erps_get_protection_group_by_id(&pconf) == VTSS_RC_OK) {
                       /* disable R-APS forwarding for this protection group */
                       ...
                       ret = vtss_erps_delete_protection_group (mgmt_req.group_id);
                       if (ret == ERPS_RC_OK) {
                           erps_platform_clear_port_to_mep(mgmt_req.group_id);
                       } /* if (ret == ERPS_RC_OK) */
                   } /* if (vtss_erps_get_protection_group_by_id(&pconf) == VTSS_RC_OK) */
               } /* if (grp_status == ERPS_PROTECTION_GROUP_ACTIVE) */
           } /* if (vtss_erps_is_protection_group_active(mgmt_req.group_id) == VTSS_RC_OK) */
        ...
        case ERPS_CMD_ADD_MEP_ASSOCIATION:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_associate_group group_id = %u eastmep = %u westmep = %u east rapsmep = %u  \
                                 west rapsmep = %u", mgmt_req.group_id,mgmt_req.data.mep.east_mep_id,mgmt_req.data.mep.west_mep_id,\
                                 mgmt_req.data.mep.raps_eastmep,mgmt_req.data.mep.raps_westmep);
           /* update mep_ids in port_to_mep global strcture */
           CRIT_ENTER(crit_p);
           if (!port_to_mep[mgmt_req.group_id].active) {
           CRIT_EXIT(crit_p);
               erps_platform_set_erpg_meps(mgmt_req.data.mep.east_mep_id,
                                       ...
                                       mgmt_req.group_id);
           CRIT_ENTER(crit_p);
               port_to_mep[mgmt_req.group_id].active = TRUE;
           CRIT_EXIT(crit_p);
           } else {
               CRIT_EXIT(crit_p);
           }
           ...
           if (vtss_erps_get_protection_group_status(mgmt_req.group_id, &grp_status) == VTSS_RC_OK) {
               if ((grp_status == ERPS_PROTECTION_GROUP_ACTIVE) || (grp_status == ERPS_PROTECTION_GROUP_INACTIVE)){
                   ...
                   if (vtss_erps_get_protection_group_by_id(&pconf) == VTSS_RC_OK) {
                       ...
                       ret = vtss_erps_associate_group (mgmt_req.group_id);
                       if (ret == VTSS_RC_OK) {
                           /* register with MEP for incoming signal */
                           erps_instance_signal_in(mgmt_req.group_id);
                       }
                   }
               }
           }
           CRIT_ENTER(crit_p);
           if (port_to_mep[mgmt_req.group_id].active == TRUE && ret == ERPS_RC_OK) {
           CRIT_EXIT(crit_p);
               erps_platform_set_erpg_meps(mgmt_req.data.mep.east_mep_id,
                                       mgmt_req.data.mep.west_mep_id,
                                       mgmt_req.data.mep.raps_eastmep,
        ...
        case ERPS_CMD_SET_RPL_NEIGHBOUR:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_set_rpl_neighbour group_id = %u neighbour port = %u ", \
                                 mgmt_req.group_id,mgmt_req.data.rpl_block.rpl_port);
           ret = vtss_erps_set_rpl_neighbour(mgmt_req.group_id,
                                             mgmt_req.data.rpl_block.rpl_port);
           break;
        case ERPS_CMD_UNSET_RPL_NEIGHBOUR:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_unset_rpl_neighbour group_id = %u", mgmt_req.group_id);
           ret = vtss_erps_unset_rpl_neighbour(mgmt_req.group_id);
           break;
        ...
        case ERPS_CMD_ENABLE_INTERCONNECTED_NODE:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_set_rpl_block group_id = %u major_ring_id = %u ", \
                                 mgmt_req.group_id,mgmt_req.data.inter_connected.major_ring_id);
           ret = vtss_erps_enable_interconnected(mgmt_req.group_id,
                                                  CONV_MGMTTOERPS_INSTANCE(mgmt_req.data.inter_connected.major_ring_id));
           break;
        case ERPS_CMD_DISABLE_INTERCONNECTED_NODE:
           T_NG(TRACE_GRP_MGMT, "vtss_erps_set_rpl_block group_id = %u major_ring_id = %u ", \
                                 mgmt_req.group_id,mgmt_req.data.inter_connected.major_ring_id);
           ret = vtss_erps_disable_interconnected(mgmt_req.group_id,
                                                  CONV_MGMTTOERPS_INSTANCE(mgmt_req.data.inter_connected.major_ring_id));
           break; 
        case ERPS_CMD_ENABLE_NON_REVERTIVE:
           ret = vtss_erps_enable_non_reversion(mgmt_req.group_id);
           break; 
        case ERPS_CMD_DISABLE_NON_REVERTIVE:
           ret = vtss_erps_disable_non_reversion(mgmt_req.group_id);
           break; 
        case ERPS_CMD_FORCED_SWITCH:
           ret = vtss_erps_admin_command(mgmt_req.group_id,ERPS_ADMIN_CMD_FORCED_SWITCH,
                                         mgmt_req.data.create.east_port); 
           break; 
        case ERPS_CMD_MANUAL_SWITCH:
           ret = vtss_erps_admin_command(mgmt_req.group_id,ERPS_ADMIN_CMD_MANUAL_SWITCH,
                                         mgmt_req.data.create.east_port); 
           break;
        case ERPS_CMD_CLEAR:
           ret = vtss_erps_admin_command(mgmt_req.group_id,ERPS_ADMIN_CMD_CLEAR,0);
           break;
        case ERPS_CMD_TOPOLOGY_CHANGE_PROPAGATE:
           ret = vtss_erps_set_topology_change_propogation(mgmt_req.group_id,TRUE);
           break; 
        case ERPS_CMD_TOPOLOGY_CHANGE_NO_PROPAGATE:
           ret = vtss_erps_set_topology_change_propogation(mgmt_req.group_id,FALSE);
           break; 
        case ERPS_CMD_SUB_RING_WITH_VIRTUAL_CHANNEL:
           ret = vtss_erps_enable_virtual_channel(mgmt_req.group_id);
           break; 
        case ERPS_CMD_SUB_RING_WITHOUT_VIRTUAL_CHANNEL:
           ret = vtss_erps_disable_virtual_channel(mgmt_req.group_id);
           break; 
        case ERPS_CMD_ENABLE_VERSION_1_COMPATIBLE:
           ret = vtss_erps_set_protocol_version(mgmt_req.group_id,ERPS_VERSION_V1);
           break;
        case ERPS_CMD_DISABLE_VERSION_1_COMPATIBLE:
           ret = vtss_erps_set_protocol_version(mgmt_req.group_id,ERPS_VERSION_V2);
           break;
        default:
           break;
    }
...
void vtss_erps_print_error (vtss_rc error) 
{
    switch(error) 
    {
        ...
        case ERPS_ERROR_PG_IN_INACTIVE_STATE:
            CPRINTF("\n protection group not in active state \n");
            break;
        case ERPS_ERROR_NODE_NON_RPL:
            CPRINTF("\n this node is not rpl for given protection group \n");
            break;
        case ERPS_ERROR_SETTING_RPL_BLOCK:
            CPRINTF("\n failed in setting rpl block \n");
            break;
        case ERPS_ERROR_VLANS_CANNOT_BE_ADDED:
            CPRINTF("\n vlans can not be added for this protection group \n");
            break;
        case ERPS_ERROR_VLANS_CANNOT_BE_DELETED:
            CPRINTF("\n vlans can not be deleted for this protection group \n");
            break;
        case ERPS_ERROR_CONTROL_VID_PART_OF_ANOTHER_GROUP:
            CPRINTF("\n control vlan id is part of another group \n");
            break;
        ...
        case ERPS_ERROR_GROUP_IN_ACTIVE_MODE:
            CPRINTF("\n cannot configure when group is in active mode \n");
            break;
        ...
        case ERPS_ERROR_MAXIMUM_PVIDS_REACHED:
            CPRINTF("\n maximum number of vlans already configured for protection group \n");
            break;
        default:
            CPRINTF("\n incorrect error code = %u\n", error);
            break;
    }
}
...
static void erps_run_thread(cyg_addrword_t data)
{
    ...
    for (;;) {
        ...
        while((req = cyg_mbox_timed_get(erps_mbhandle, wakeup))) {
            ...
            T_NG(TRACE_GRP_ERPS_RX, "ERPS PUD received on mep_id = %u", req->mep_id);
