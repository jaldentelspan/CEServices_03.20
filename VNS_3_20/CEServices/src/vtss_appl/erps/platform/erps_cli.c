...
/******************************************************************************
 Helper Functions
******************************************************************************/
static void erps_print_protected_vlans(vtss_vid_t pvids[])
{
    u8  count = 0, i = 0;
...
static void erps_display_protection_group_info(vtss_erps_mgmt_conf_t conf_req)
{
    vtss_erps_config_erpg_t * erpg = NULL;
    vtss_erps_fsm_stat_t    * ferpg = NULL;
                                       ...
                                       char *cmd_org, cli_req_t *req)
{
    ...
    error = cli_parse_ulong(cmd, &value, RAPS_WTR_TIMEOUT_MIN_MINITES,
                                         RAPS_WTR_TIMEOUT_MAX_MINITES);
    if (!error) {
        erps_req->timeout = value;
        return (VTSS_RC_OK);
    }
