            ...
            if (s->group == CX_TAG_ACCESS_MGMT_TABLE) {
                ...
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    ...
                    if (cx_parse_long(s, "access_id", (long *)&access_id, ACCESS_MGMT_ACCESS_ID_START, ACCESS_MGMT_MAX_ENTRIES) == VTSS_OK) {
                        entry.valid = 1;
#if defined(VTSS_SW_OPTION_IP2)
                    } else if (cx_parse_ulong(s, "access_vid", &access_vid, VTSS_VID_DEFAULT, VTSS_VID_RESERVED) == VTSS_OK) {
...
#if defined(VTSS_SW_OPTION_IP2)
                CX_RC(cx_add_stx_ulong(s, "access_vid", VTSS_VID_DEFAULT, VTSS_VID_RESERVED));
#endif /* VTSS_SW_OPTION_IP2 */
