...
#ifdef VTSS_SW_OPTION_SYSLOG
    char                                        buf[IP_SOURCE_GUARD_BUF_LENGTH];
#endif
...
#ifdef VTSS_SW_OPTION_SYSLOG
#if VTSS_SWITCH_STACKABLE
        S_I("Dynamic IP Source Guard Table Full. Could not add entry <VLAN ID = %d, Switch ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
            entry->vid, topo_isid2usid(entry->isid), iport2uport(entry->port_no), misc_ipv4_txt(entry->assigned_ip, buf), misc_mac_txt(entry->assigned_mac, buf));
#else
        S_I("Dynamic IP Source Guard Table Full. Could not add entry <VLAN ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
            entry->vid, iport2uport(entry->port_no), misc_ipv4_txt(entry->assigned_ip, buf), misc_mac_txt(entry->assigned_mac, buf));
...
#ifdef VTSS_SW_OPTION_SYSLOG
#if VTSS_SWITCH_STACKABLE
        S_I("Dynamic IP Source Guard Table reach port %d limitation(%d). Could not add entry <VLAN ID = %d, Switch ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
            iport2uport(entry->port_no), current_port_dynamic_entry_cnt, entry->vid, topo_isid2usid(entry->isid), iport2uport(entry->port_no), misc_ipv4_txt(entry->assigned_ip, buf), misc_mac_txt(entry->assigned_mac, buf));
#else
        S_I("Dynamic IP Source Guard Table reach port %d limitation(%d). Could not add entry <VLAN ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
            iport2uport(entry->port_no), current_port_dynamic_entry_cnt, entry->vid, iport2uport(entry->port_no), misc_ipv4_txt(entry->assigned_ip, buf), misc_mac_txt(entry->assigned_mac, buf));
#endif /* VTSS_SWITCH_STACKABLE */
#endif
        return TRUE;
    }
...
static vtss_rc ip_source_guard_del_first_dynamic_entry_with_specific_port(vtss_isid_t isid, vtss_port_no_t port_no)
{
    ...
    if (ip_source_guard_mgmt_conf_get_first_dynamic_entry(&entry) == VTSS_OK) {
        if ((entry.isid == isid) && (entry.port_no == port_no)) {
            ...
            S_I("Dynamic IP Source Guard Table port limitation changed. should delete entry <VLAN ID = %d, Switch ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
                entry.vid, topo_isid2usid(entry.isid), iport2uport(entry.port_no), misc_ipv4_txt(entry.assigned_ip, buf), misc_mac_txt(entry.assigned_mac, buf));
#else
            /* Fill system log which entry had been eliminated */
            S_I("Dynamic IP Source Guard Table port limitation changed. should delete entry <VLAN ID = %d, Port = %d, IP Address = %s, MAC Address = %s> to Table",
                entry.vid, iport2uport(entry.port_no), misc_ipv4_txt(entry.assigned_ip, buf), misc_mac_txt(entry.assigned_mac, buf));
#endif /* VTSS_SWITCH_STACKABLE */
#endif
            del_done = TRUE;
