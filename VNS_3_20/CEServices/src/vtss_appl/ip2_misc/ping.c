          ...
          struct sockaddr_in *from, struct sockaddr_in *to)
{
    ...
    if (tv < *tp) {
        /* wrap around */
        int time_wrap = PING_TIME_STAMP_BASE - *tp + 1;
          ...
          size_t icmp_len, size_t count, size_t interval)
{
    ...
    struct icmp *icmp = (struct icmp *) malloc(PING_MAX_PACKET_LEN + 28);

    // Rx packet. The +28 refers to sizeof(normal IP header) = 20 plus
    // sizeof ICMP header (8 bytes)
    unsigned char *rx_pkt = malloc(PING_MAX_PACKET_LEN + 28);
    size_t seq;
           ...
           const struct sockaddr_in6 *from, const struct sockaddr_in6 *to)
{
    ...
    if (tv < *tp) {
        /* wrap around */
        time_wrap = PING_TIME_STAMP_BASE - *tp + 1;
...
{
    struct icmp6_hdr *icmp = (struct icmp6_hdr *) malloc(PING_MAX_PACKET_LEN + 28); // Tx packet. The +28 refers to sizeof(normal IP header) = 20 plus sizeof ICMP header (8 bytes)
    unsigned char *rx_pkt = malloc(PING_MAX_PACKET_LEN + 28);   // Rx packet. The +28 refers to sizeof(normal IP header) = 20 plus sizeof ICMP header (8 bytes)
    int icmp_len = length;
...
ping6_test_1(vtss_ip2_cli_pr *pr, struct sockaddr_in6 *host, size_t len, size_t count, size_t interval)
{
    ...
    if (host->sin6_addr.s6_addr[0] == 0xFF) {
        ...
        if (IN6_IS_ADDR_LINKLOCAL(ipa6)) {
            ipa6->__u6_addr.__u6_addr8[3] = 0x0;
        }
    }
...
BOOL ping6_test(vtss_ip2_cli_pr *pr, const vtss_ipv6_t *ipv6_address, size_t len, size_t count, size_t interval)
{
    ...
    host.sin6_port = 0;
    /* It is tricky. For pinging link-local addresses, the index of interface must
...
#ifdef VTSS_SW_OPTION_IPV6
    if (pReq->is_ipv6) {
        (void) ping6_test(cli_printf, &pReq->ipv6_address, pReq->len, pReq->cnt, pReq->interval);
    } else
#endif /* VTSS_SW_OPTION_IPV6 */
    {
...
#ifdef VTSS_SW_OPTION_WEB
static cli_iolayer_t *ping_create_thread(const char *ip_address, size_t len, size_t count, size_t interval, BOOL is_ipv6, const vtss_ipv6_t *ipv6_address)
{
    int i;
    cli_iolayer_t *io;
...
cli_iolayer_t *ping6_test_async(const vtss_ipv6_t *ipv6_address, size_t len, size_t count, size_t interval)
{
    return ping_create_thread("<ipv6>", len, count, interval, 1, ipv6_address);
}
...
{
    return ping_create_thread(ip_address, len, count, interval, 0, NULL);
}
#endif /* VTSS_SW_OPTION_WEB */
