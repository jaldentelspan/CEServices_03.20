...
{
    loop_protect_msg_t *msg = malloc(sizeof(loop_protect_msg_t));
    if(msg)
        msg->msg_id = msg_id;
...
loop_protect_conf_read(BOOL create)
{
    ...
    if (do_create) {
        ...
        blk->conf.global.enabled = FALSE;        /* Disabled */
        blk->conf.global.transmission_time = 5;  /* 5 seconds */
        blk->conf.global.shutdown_time = 60 * 3; /* 3 minutes */
        for(i = 0; i < VTSS_ISID_END; i++) {
            for(j = 0; j < VTSS_PORT_ARRAY_SIZE; j++) {
                blk->conf.ports[i][j].enabled = TRUE;
                blk->conf.ports[i][j].action = LOOP_PROTECT_ACTION_SHUTDOWN;
                blk->conf.ports[i][j].transmit = TRUE;
            }
        }
    }
...
{
    if(lprot_state.ports[port_no].last_disable != time(NULL)) {
        ...
        T_N("Skip disable, already sent this very second, port %d", port_no);
    }
}
...
static BOOL lprot_egress_filter(vtss_port_no_t egress_port)
{
    ...
    info.glag_no = VTSS_GLAG_NO_NONE; // glag_no is not used
...
#if defined(VTSS_FEATURE_AGGR_GLAG)
    info.glag_no = VTSS_GLAG_NO_NONE; // glag_no is not used in ingres filtering
#endif /* VTSS_FEATURE_AGGR_GLAG */
    info.vid = vid;
    info.port_tx = VTSS_PORT_NO_NONE; // rx filtering
       ...
       pdu->version != LPROT_PROTVERSION) {                 /* Proper version */
        ...
        if((pstate->ttl_timer > 0) || /* Port just came up */
           ...
           (VTSS_OS_TICK2MSEC(tdelta) < LPROT_MAX_TTL_MSECS)) { /* OR recent PDU */
             ...
             */
            if(valid && vtss_lb_port(in_port) && vtss_lb_port(in_port)) {
...
{
    LPROT_CRIT_ENTER();
    if(info->link && !info->stack && lprot_conf.global.enabled) {
