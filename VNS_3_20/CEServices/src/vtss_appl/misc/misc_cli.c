...
typedef struct {
    ...
    BOOL  ipv6;
#if defined(VTSS_SW_OPTION_IP) || defined(VTSS_SW_OPTION_IP2)
    vtss_ipv4_t sip;
    vtss_ipv4_t dip;
#endif /* VTSS_SW_OPTION_IP || VTSS_SW_OPTION_IP2 */
...
static void misc_cli_cmd_sfp_debug_read( cli_req_t * req )
{
    ...
    if (vtss_i2c_wr_rd(NULL, misc_req->i2c_addr, &misc_req->sfp_reg_addr[0], misc_req->i2c_addr_cnt, &i2c_data[0], misc_req->i2c_data_cnt, 100, misc_req->i2c_clk_sel) == VTSS_RC_OK) {
        for (i = 0; i < misc_req->i2c_data_cnt; i++) {
...
static void misc_cli_reg_value(misc_cli_req_t *misc_req, ulong value, BOOL header)
{
    int i;

    if (header)
        CPRINTF("31     24 23     16 15      8 7       0");
    else if (misc_req->binary)
        for (i = 31; i >= 0; i--)
            CPRINTF("%d%s", value & (1<<i) ? 1 : 0, (i % 4) || i == 0 ? "" : ".");
    else if (misc_req->decimal)
        CPRINTF("%10u", value);
    else
...
{
    int         i, blk, sub, addr, addr_count = 0, sub_count = 0, first = 1;
    ulong       reg_addr, value, failed = 0;
    vtss_isid_t isid;
    misc_cli_req_t *misc_req = req->module_req;
    ...
    for (i = 0; i < (req->set ? 1 : 2); i++) {
        for (blk = 0; blk < CLI_BLK_MAX; blk++) {
            if (misc_req->blk_list[blk] == 0)
                continue;

            for (sub = 0; sub < CLI_SUB_MAX; sub++) {
                if (misc_req->sub_list[sub] == 0)
                    continue;
                ...
                for (addr = 0; addr < CLI_ADDR_MAX; addr++) {
                    ...
                    reg_addr = misc_l28_reg_addr(blk, sub, addr);
                    if (req->set) {
                        /* Write */
                        if (misc_debug_reg_write(isid, 0, reg_addr, req->value) != VTSS_OK)
                            failed++;
                    } else if (i == 0) {
                        /* Count number of addresses to read */
                        if (sub_count == 1)
                            addr_count++;
                    } else if (misc_debug_reg_read(isid, 0, reg_addr, &value) != VTSS_OK) {
                        /* Read failure */
                        failed++;
                    } else {
                        /* Read success */
                        if (first && misc_req->binary) {
                            if (sub_count > 1)
                                CPRINTF("%-20s","");
                            if (sub_count > 1 || addr_count > 1)
                                CPRINTF("%-6s","");
                            first = 0;
                            misc_cli_reg_value(misc_req, 0, 1);
                        }
                        if (sub_count > 1)
                            CPRINTF("Blk %d, Sub %2d, Addr ", blk, sub);
                        if (sub_count > 1 || addr_count > 1)
                            CPRINTF("0x%02x: ", addr);
                        misc_cli_reg_value(misc_req, value, 0);
                    }
                }
            }
        }
...
static void misc_cli_cmd_debug_reg(cli_req_t *req)
{
    ulong          i, first, tgt, addr, reg_addr, addr_count, value, failed = 0;
    misc_cli_req_t *misc_req = req->module_req;
            ...
            for (tgt = 0; tgt < CLI_BLK_MAX; tgt++) {
                ...
                for (addr = 0; addr < CLI_ADDR_MAX; addr++) {
                    ...
                    if (req->set) {
                        ...
                        if (addr_count > 1)
                            CPRINTF("Tgt 0x%02x, Addr 0x%04x: ", tgt, addr);
                        misc_cli_reg_value(misc_req, value, 0);
                    }
                }
            }
...
static void misc_cli_cmd_debug_api(cli_req_t *req)
{
    ...
    if (misc_req->group == VTSS_DEBUG_GROUP_COUNT) {
        ...
        if (misc_chip_no_get(&info.chip_no) == VTSS_RC_OK &&
            vtss_debug_info_print(NULL, (vtss_debug_printf_t)cli_printf, &info) != VTSS_OK)
    ...
    for (i = 0; i < (req->set ? 1 : 2); i++) {
        for (iport = VTSS_PORT_NO_START; iport < VTSS_PORT_NO_END; iport++) {
            ...
            for (addr = 0; addr < CLI_PHY_MAX; addr++) {
                ...
                if (req->set) {
                    /* Write */
                    if (misc_debug_phy_write(isid, iport, addr, 
                                             misc_req->page, req->value, misc_req->phy_mmd_access, misc_req->devad) != VTSS_OK)
                        failed++;
                } else if (i == 0) {
                    /* Count number of addresses to read */
                    if (port_count == 1)
                        addr_count++;
                } else if (misc_debug_phy_read(isid, iport, addr, 
                                               misc_req->page, &value, misc_req->phy_mmd_access, misc_req->devad) != VTSS_OK) {
                    /* Read failure */
                    failed++;
                } else {
                    /* Read success */
...
static void misc_cli_cmd_debug_phy_i2c_rd ( cli_req_t * req )
{
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        ...
        if (vtss_phy_i2c_read(MISC_INST, port_no, misc_req->i2c_mux, misc_req->i2c_reg_addr, misc_req->i2c_device_addr, &value, 1) != VTSS_RC_OK) {
...
// For debug - Shows PHY statistics.
static void misc_cli_cmd_debug_phy_statistic(cli_req_t *req)
{
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        ...
        if (req->set) {
            // No Set for the function
        } else {

            if (first) {
                CPRINTF("%-5s %-10s %-13s %-14s %-17s %-14s\n","Port","CU Rx Good", "CU Rx CRC err", "SerDes Tx Good", "SerDes Tx CRC Err", "BASE-TX Rx Err");
                CPRINTF("%-5s %-10s %-14s %-14s %-17s %-14s\n","----","----------", "-------------", "--------------", "-----------------", "--------------");
                first = 0;
            }           

            if (vtss_phy_statistic_get(NULL, port_no, &statistics) == VTSS_RC_OK) {
                CPRINTF("%-5u %-10d %-13d %-14d %-17d %-14d\n",
                        port, 
                        statistics.cu_good,
                        statistics.cu_bad,
                ...
                CPRINTF("Could not get PHY statistics \n");
            }
        }
    }
}
...
#if defined(VTSS_ARCH_LUTON28)
static void misc_cli_cmd_debug_wm ( cli_req_t * req )
{
    vtss_uport_no_t  uport;
    vtss_port_no_t   iport;
    vtss_isid_t      isid = VTSS_ISID_LOCAL;
    ulong            addr, data, value, val1, val2, val3;
    int              i, q, a, blk, sub, sub_end, freepool_only=1, chip_port;
    int              low_only[6], qmask[6], wmaction[6], chkmin[6], cmpwith[6], level[6];
    vtss_init_data_t init_data;
    
    init_data.cmd = INIT_CMD_SUSPEND_RESUME;
    ...
    for (iport = VTSS_PORT_NO_START; iport < VTSS_PORT_NO_END; iport++) {
        ...
        blk = (chip_port > 15 ? 6 : 1);
        sub = (chip_port % 16);
        sub_end = sub;

#if VTSS_OPT_INT_AGGR
        /* Run through two sub blocks for stack ports */
        if (chip_port > 23)
            sub_end++;
#endif /* VTSS_OPT_INT_AGGR */
        
        for (; sub <= sub_end; sub++, chip_port++) {
            if (freepool_only) {
                // Freepool
                addr = misc_l28_reg_addr(blk, sub, 0xd8);
                if (misc_debug_reg_read(isid, 0, addr, &value) == VTSS_OK &&
                    (((value>>8) & 0xff) || ((value>>0) & 0xff))) {
                    CPRINTF("Port:%u (%d) Free slices:%u  Ingress used:%u  Egress used:%u\n",
                            uport, chip_port, (value>>16), ((value>>8)&0xff), ((value>>0)&0xff));
                }
            } else {
                if (req->uport_list[uport] == 0)
                    continue;

                // q_ingress_wm=0xE8-0xED: low_only,qmask,wmaction,chkmin,cmpwith,level
                addr = misc_l28_reg_addr(blk, sub, 0xe8);
                for (i = 0; i < 6; i++) {
                    if (misc_debug_reg_read(isid, 0, addr + i, &value) != VTSS_OK)
                        value = 0;
                    low_only[i] = value>>18;
                    qmask[i] = (value>>13 & 0x1f);
                    wmaction[i] =(value>>11 & 0x3);
                    chkmin[i] = (value>>10 & 0x1);
                    cmpwith[i] = (value>>8 & 0x3);
                    level[i] = (value & 0x1f);
                }

                // q_egress_wm=0xe0: imin, emin, early_tx
                addr = misc_l28_reg_addr(blk, sub, 0xe0);
                if (misc_debug_reg_read(isid, 0, addr, &value) != VTSS_OK)
                    continue;

                CPRINTF("wmaction: 0='Head Drop', 1='Tail Drop', 2:'Urgent' 3:'Activate FC'\n");
                CPRINTF("cmpwith: 0='Tot mem use', 1='Tot ingr use', 2:'Curr Q use' 3:'Tot egr use'\n");
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d\n","",0,1,2,3,4,5);
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d\n","low_only =",low_only[0],low_only[1],low_only[2],low_only[3],low_only[4], low_only[5]);
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d\n","qmask =",qmask[0],qmask[1],qmask[2],qmask[3],qmask[4], qmask[5]);
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d  Egress Q0 = %5u\n","wmaction =",wmaction[0],wmaction[1],wmaction[2],wmaction[3],wmaction[4], wmaction[5],(value>>0 & 0x1f));

                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d  Egress Q1 = %5u\n","chkmin =",chkmin[0],chkmin[1],chkmin[2],chkmin[3],chkmin[4], chkmin[5],(value>>8 & 0x1f));
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d  Egress Q2 = %5u\n","cmpwith =",cmpwith[0],cmpwith[1],cmpwith[2],cmpwith[3],cmpwith[4], cmpwith[5],(value>>16 & 0x1f));
                CPRINTF("%-15s %5d %5d %5d %5d %5d %5d  Egress Q3 = %5u\n","level =",level[0],level[1],level[2],level[3],level[4], level[5],(value>>24 & 0x1f));
                addr = misc_l28_reg_addr(blk, sub, 0xdf);
                if (misc_debug_reg_read(isid, 0, addr, &value) == VTSS_OK)
                    CPRINTF("%-15s %5u %5s %5s %5s %5s %5s  emin      = %5u\n","imin =",(value>>16 & 0x1f),"","","","","",(value>>8 & 0x1f));

                // Suspend port module
                init_data.resume = 0;
                if (init_modules(&init_data) != VTSS_OK)
                  continue;

                CPRINTF("\n    %8s %8s %8s %8s\n","In.used","Eg.used","Rx","Tx");
                addr = misc_l28_reg_addr(blk, sub, 0xdd);
                data = misc_l28_reg_addr(blk, sub, 0xdc);
                for (q = 0, i = 0, a = 0; i < 13; q++, i+=4, a+=2) {
                    if (
                        // Rx Class cnt
                        misc_debug_reg_write(isid, 0, addr, 0x80003030 + a) == VTSS_OK &&
                        misc_debug_reg_read(isid, 0, data, &value) == VTSS_OK &&
                        // Tx Class cnt
                        misc_debug_reg_write(isid, 0, addr, 0x80003066 + a) == VTSS_OK &&
                        misc_debug_reg_read(isid, 0, data, &val2) == VTSS_OK &&
                        // Ingr Queue size cnt
                        misc_debug_reg_write(isid, 0, addr, 0x8002 + i) == VTSS_OK &&
                        misc_debug_reg_read(isid, 0, data, &val1) == VTSS_OK &&
                        // Egr Queue size cnt
                        misc_debug_reg_write(isid, 0, addr, 0x8016 + i) == VTSS_OK &&
                        misc_debug_reg_read(isid, 0, data, &val3) == VTSS_OK)
                        CPRINTF("Q%d: %8u %8u %8u %8u\n",q,val1,val3,value,val2);
                }
                if (misc_debug_reg_write(isid, 0, addr, 0x8012) == VTSS_OK &&
                    misc_debug_reg_read(isid, 0, data, &value) == VTSS_OK &&
                    misc_debug_reg_write(isid, 0, addr, 0x8026) == VTSS_OK &&
                    misc_debug_reg_read(isid, 0, data, &val1) == VTSS_OK)
                    CPRINTF("Q4: %8u %8u\n",value,val1);
                ...
                addr = misc_l28_reg_addr(blk, sub, 0xd8);
                if (misc_debug_reg_read(isid, 0, addr, &value) == VTSS_OK)
                    CPRINTF("\nFree slices:%u  Ingress used:%u  Egress used:%u\n",(value>>16),((value>>8)&0xff),((value>>0)&0xff));

                // Clear counters
                addr = misc_l28_reg_addr(blk, sub, 0x52);
                (void)misc_debug_reg_write(isid, 0, addr, 1);
            }
        } /* Sub block loop */
    } /* Port loop */
}
...
#if defined(VTSS_ARCH_LUTON28)
static int32_t cli_misc_sub_list_parse(char *cmd, char *cmd2, char *stx,
                                       char *cmd_org, cli_req_t *req)
{
    misc_cli_req_t *misc_req = req->module_req;

    return cli_parm_parse_list(cmd, misc_req->sub_list, 0, CLI_SUB_MAX - 1, 0);
}
#endif /* VTSS_ARCH_LUTON28 */
...
/* Register read/write syntax depends on chip architecture */
#if defined(VTSS_ARCH_LUTON28)
#define REG_READ_STX  "Debug Register Read <blk_list> <sub_list> <addr_list> [binary|decimal]"
#define REG_WRITE_STX "Debug Register Write <blk_list> <sub_list> <addr_list> <value>"
#else
#define REG_READ_STX  "Debug Register Read <tgt_list> <addr_list> [binary|decimal]"
#define REG_WRITE_STX "Debug Register Write <tgt_list> <addr_list> <value>"
#endif /* VTSS_ARCH_LUTON28 */
...
cli_cmd_tab_entry (
  "Debug wm [<port_list>]",
  NULL,
  "Dump port watermarks and queue fillings",
  PRIO_DEBUG_WM_READ,
  CLI_CMD_TYPE_STATUS,
  VTSS_MODULE_ID_DEBUG,
  misc_cli_cmd_debug_wm,
  misc_cli_req_default_set,
  misc_cli_parm_table,
  CLI_CMD_FLAG_NONE
);
...
#if defined(VTSS_CHIP_10G_PHY)
cli_cmd_tab_entry (
  "Debug 10g_phy_status [<port_list>]",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_loopback [<port_list>] [a|b|c|d|e|f|g|h|j|k] [enable|disable]",
  NULL,
  "Debug 10g_phy_loopback",
...
cli_cmd_tab_entry (
  "Debug 10g_phy_failover [<port_list>] [a|b|c|d|e|f]",
  NULL,
  "Debug 10g_phy_failover",
...
cli_cmd_tab_entry (
  "Debug 10g_phy_power [<port_list>] [enable|disable]",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_mode [<port_list>] [lan|wan|1g]",
  NULL,
  "Debug 10G PHY mode",
...
cli_cmd_tab_entry (
  "Debug 10g_phy_gpio_role <port_list> [<gpio_list>] [input|output|pcs_rx_fault]",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_gpio_read <port_list> [<gpio_list>]",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_gpio_write <port_list> [<gpio_list>] <value>",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_fw_status <port_list> ",
  NULL,
...
#if defined(VTSS_FEATURE_PHY_TIMESTAMP)
cli_cmd_tab_entry (
  "Debug PHY 1588_Read <port_list> <blk_id_1588> <csr_reg_list> [binary|decimal]",
  NULL,
...
#ifdef VTSS_FEATURE_SYNCE_10G
cli_cmd_tab_entry (
  "Debug 10g_phy_synce_clkout <port_list> [enable|disable]",
  NULL,
...
cli_cmd_tab_entry (
  "Debug 10g_phy_xfp_clkout <port_list> [enable|disable]",
  NULL,
...
cli_cmd_tab_entry (
 "Debug 10g_phy_synce <port_list> [<mode>] [<clk_out>] [<hitless>] [<rclk_div>] [<sref_div>] [<wref_div>]",
  NULL,
