            ...
            while (critd_p) { 
                ...
                if (!critd_deadlock) {
                    ...
                    if (critd_p->lock_poll_cnt > (critd_p->max_lock_time / POLL_PERIOD)) {
                        ...
                        if (!(deadlock_str = malloc(DEADLOCK_STR_LEN))) {
                            T_E("malloc(%d) failed", DEADLOCK_STR_LEN);
                        } else {
...
#ifdef VTSS_SW_OPTION_CLI
                cli_print_thread_status(thread_status_printf, TRUE); // Print thread status including stack trace. If CLI is deadlocked, then this is the only way to get that info.
#endif // VTSS_SW_OPTION_CLI
            }
