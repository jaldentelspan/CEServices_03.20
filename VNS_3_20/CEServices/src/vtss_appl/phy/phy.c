...
static vtss_rc mmd_read(const vtss_port_no_t  port_no,
                        const u8              mmd,
                        const u16             addr,
                        u16                   *const value)
{
    ...
    if (phy.init_conf_default.mmd_read(port_no,  mmd,  addr, value) != VTSS_RC_OK) {
        T_I("MMD read Failed");
        return VTSS_RC_ERROR;
    }
    /* Convert the API instance pointer back to 'phy_inst' */
    if (vtss_init_conf_get(phy.current_inst, &phy.init_conf_default) != VTSS_RC_OK) {
        return VTSS_RC_ERROR;
    }
    return VTSS_RC_OK;
}

static vtss_rc mmd_write(const vtss_port_no_t  port_no,
                         const u8              mmd,
                         const u16             addr,
                         const u16             value)

{
    ...
    if (phy.init_conf_default.mmd_write(port_no, mmd, addr, value) != VTSS_RC_OK) {
...
static vtss_rc mmd_read_inc(const vtss_port_no_t  port_no,
                            const u8              mmd,
                            const u16             addr,
                            u16                   *buf,
                            u8                    count)
{
   /* Must use the MMD functions from the default API instance (and update the API pointer to the default state) */
    ...
    if (phy.init_conf_default.mmd_read_inc(port_no, mmd, addr, buf, count) != VTSS_RC_OK) {
                         ...
                         u16                  *const value)
{
    ...
    if (phy.init_conf_default.miim_read(port_no, addr, value) != VTSS_RC_OK) {
...

{
    ...
    if (phy.init_conf_default.miim_write(port_no, addr, value) != VTSS_RC_OK) {
...
//                VTSS_RC(vtss_ewis_static_conf_get(phy.current_inst, port_no, &p10g->ewis_mode.static_conf));
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
            p1g = &phy.store_1g[port_no];
            if (p1g->active) {
                T_I("Applying 1g phy conf port:%u",port_no);
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
            p10g = &phy.store_10g[port_no];
            if (p10g->active) {
                T_I("Applying 10g phy conf port:%u",port_no);
...
vtss_rc phy_mgmt_inst_restart(vtss_inst_t inst, phy_inst_restart_t restart)
{
    ...
    if ((rc = phy_inst_conf(0)) != VTSS_RC_OK) {
        T_E("Problem in applying stored API conf");
        PHY_CRIT_EXIT();
        return rc;
    }
...
static void phy_conf_read(vtss_isid_t isid_add, BOOL force_default)
{
    ...
    if ((phy_blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_PHY_CONF, &size)) == NULL) {
        create = 1;
        T_W("Could not open conf block. Defaulting.");
    } else {       
