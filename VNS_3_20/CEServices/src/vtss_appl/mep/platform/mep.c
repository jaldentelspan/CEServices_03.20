...
typedef struct
{
    ...
    u8                         *tx_buffer[2][2];                /* Extra transmit buffers for auto CCM/TST transmission on protection port or into major ring */
...
#if defined(VTSS_SW_OPTION_EVC)
#define EVC_VID_SIZE 4096
static u32              evc_vid[EVC_VID_SIZE];
#endif
...
void mep_phy_config_mac(const u8  is_del,
                        const u8  port,
                        const u8  is_ing,
...
static vtss_rc mep_phy_ts_update(u8 port)
{
	...
	/**/
        ...
        if (mep_phy_ts_port[port].engine_id == VTSS_PHY_TS_ENGINE_ID_INVALID) { /* if not already allocated */
            ...
            mep_phy_ts_port[port].engine_id = VTSS_PHY_TS_OAM_ENGINE_ID_2A;
...
static void port_data_initialize(void)
{
    ...
    for (i = 0; i < VTSS_PORTS; i++) {
        ...
        if (test){
            ...
            if (rc != VTSS_RC_OK) {
                T_E("Port_no = %d, vtss_phy_ts_init fail %x", i, rc);
#if !DEBUG_WO_PHY
                CRIT_EXIT(crit_p);                
                return;
#endif                
...
/*T_D("port - state  %lu  %u", port, state);*/
    CRIT_ENTER(crit_p);
...
//printf("conv_los_state  %u-%u-%u-%u-%u-%u-%u-%u-%u-%u-%u\n", conv_los_state[0], conv_los_state[1], conv_los_state[2], conv_los_state[3], conv_los_state[4], conv_los_state[5], conv_los_state[6], conv_los_state[7], conv_los_state[8], conv_los_state[9], conv_los_state[10]);
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            if ((instance_data[i].domain == VTSS_MEP_MGMT_EVC) && (in_port_conv[port] != 0xFFFF) && (instance_data[i].port[in_port_conv[port]]))
...
//printf("conv_los_state  %u-%u-%u-%u-%u-%u-%u-%u-%u-%u-%u-%u\n", conv_los_state[0], conv_los_state[1], conv_los_state[2], conv_los_state[3], conv_los_state[4], conv_los_state[5], conv_los_state[6], conv_los_state[7], conv_los_state[8], conv_los_state[9], conv_los_state[10], conv_los_state[11]);
    ...
    for (i=0; i<MEP_INSTANCE_MAX; ++i)	/* For all EVC MEP 'behind' this port protection */
        if ((instance_data[i].enable) && (instance_data[i].domain == VTSS_MEP_MGMT_EVC) && (instance_data[i].port[in_port_conv[port]]))
...
static void set_conf_to_default(mep_conf_t  *blk)
{
    ...
    for (i=0; i<MEP_INSTANCE_MAX; ++i)
    {
        ...
        blk->conf[i].enable = FALSE;
        blk->cc_conf[i].enable = FALSE;
        blk->lm_conf[i].enable = FALSE;
        blk->lm_conf[i].ended = VTSS_MEP_MGMT_DUAL_ENDED;
        blk->dm_conf[i].enable = FALSE;
        blk->aps_conf[i].enable = FALSE;
        blk->ais_conf[i].enable = FALSE;
        blk->ais_conf[i].period = VTSS_MEP_MGMT_PERIOD_1S;
        blk->ais_conf[i].client_domain = VTSS_MEP_MGMT_EVC;
        blk->lck_conf[i].enable = FALSE;
        blk->lck_conf[i].period = VTSS_MEP_MGMT_PERIOD_1S;
        blk->lck_conf[i].client_domain = VTSS_MEP_MGMT_EVC;
...
static void apply_configuration(mep_conf_t  *blk)
{
    ...
    for (i=0; i<MEP_INSTANCE_MAX; ++i)
    {
        rc += vtss_mep_mgmt_conf_set(i, &blk->conf[i]);
        rc += vtss_mep_mgmt_cc_conf_set(i, &blk->cc_conf[i]);
        rc += vtss_mep_mgmt_lm_conf_set(i, &blk->lm_conf[i]);
        rc += vtss_mep_mgmt_dm_conf_set(i, &blk->dm_conf[i]);
        rc += vtss_mep_mgmt_aps_conf_set(i, &blk->aps_conf[i]);
        rc += vtss_mep_mgmt_ais_conf_set(i, &blk->ais_conf[i]);
        rc += vtss_mep_mgmt_lck_conf_set(i, &blk->lck_conf[i]);
        ...
        else    instance_data[i].domain = VTSS_MEP_MGMT_PORT;
    }
...
static u32 rc_conv(u32 rc)
{
    switch (rc)
    {
        case VTSS_MEP_RC_OK:                   return(MEP_RC_OK);
        ...
        case VTSS_MEP_RC_APS_EGRESS:           return(MEP_RC_APS_EGRESS);
                      ...
                      const vtss_mep_mgmt_conf_t   *const conf)
{
    ...
    if (instance >= MEP_INSTANCE_MAX)      return(MEP_RC_INVALID_PARAMETER);

    if ((rc = rc_conv(vtss_mep_mgmt_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                         ...
                         const vtss_mep_mgmt_cc_conf_t    *const conf)
{
    ...
    if ((rc = rc_conv(vtss_mep_mgmt_cc_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
        blk->cc_conf[instance] = *conf;
        conf_sec_close(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE);
    }
    if (rc) T_D("inst %u  rc %u", instance, rc);
                         ...
                         const vtss_mep_mgmt_lm_conf_t  *const conf)
{
    ...
    if ((rc = rc_conv(vtss_mep_mgmt_lm_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                         ...
                         const vtss_mep_mgmt_dm_conf_t  *const conf)
{
    ...
    if ((rc = rc_conv(vtss_mep_mgmt_dm_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                          ...
                          const vtss_mep_mgmt_aps_conf_t    *const conf)
{
    ...
    if (conf->enable && (((conf->type == VTSS_MEP_MGMT_L_APS) && (instance_data[instance].eps_type == MEP_EPS_TYPE_ERPS)) ||
        ...
        invalid = TRUE;
    CRIT_EXIT(crit_p);
    if (invalid)        return(MEP_RC_INVALID_PARAMETER);

    if ((rc = rc_conv(vtss_mep_mgmt_aps_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                         ...
                         const vtss_mep_mgmt_lt_conf_t  *const conf)
{
    ...
    rc = rc_conv(vtss_mep_mgmt_lt_conf_set(instance, conf));
    if (rc) T_D("inst %u  rc %u", instance, rc);
                         ...
                         const vtss_mep_mgmt_lb_conf_t  *const conf)
{
    ...
    rc = rc_conv(vtss_mep_mgmt_lb_conf_set(instance, conf));
    if (rc) T_D("inst %u  rc %u", instance, rc);
                          ...
                          const vtss_mep_mgmt_ais_conf_t  *const conf)
{
    ...
    if ((rc = rc_conv(vtss_mep_mgmt_ais_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                          ...
                          const vtss_mep_mgmt_lck_conf_t  *const conf)
{
    ...
    if ((rc = rc_conv(vtss_mep_mgmt_lck_conf_set(instance, conf))) == MEP_RC_OK)
    {
        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size);
                          ...
                          const vtss_mep_mgmt_tst_conf_t  *const conf)
{
    ...
    rc = rc_conv(vtss_mep_mgmt_tst_conf_set(instance, conf));
    if (rc) T_D("inst %u  rc %u", instance, rc);
...
//T_D("rx aps  instance %u  %X %X\n",instance, aps[0], aps[1]);
    if (rc == MEP_RC_OK)
    {
        ...
        if (eps_type == MEP_EPS_TYPE_ELPS)
            if (eps_rx_aps_info_set(eps, instance, aps) != EPS_RC_OK)    T_D("Error during APS rx set %u", instance);
#endif
#if defined(VTSS_SW_OPTION_ERPS)
                         ...
                         const vtss_mep_mgmt_domain_t    domain)
{
    ...
    for (i=0; i<VTSS_PORT_ARRAY_SIZE; ++i)  /* Calculate SSF for this mep instance based on los state */
        ...
        {
            los = (domain == VTSS_MEP_MGMT_EVC) ? conv_los_state[i] : los_state[i];
            if (!los)   break;      /* Only report los active (SSF) if all related ports are failing */
        }

#if defined(VTSS_SW_OPTION_EPS)
    u32              rc=MEP_RC_OK, eps=0, forward, raps_tx;
    u8               aps_txdata[VTSS_MEP_APS_DATA_LENGTH];
    mep_eps_type_t   eps_type=MEP_EPS_TYPE_INV;
    ...
    {
        if (eps_type == MEP_EPS_TYPE_ELPS)
            if (eps_signal_in(eps, instance) != EPS_RC_OK)    T_D("Error during EPS signal %u", instance);
                              ...
                              const BOOL                     sd_state)
{
    ...
    if (eps_type == MEP_EPS_TYPE_ELPS)
    {
        for (i=0; i<eps_count; ++i)
            rc += eps_sf_sd_state_set(eps_inst[i], instance, sf_state, sd_state);
    }
#endif
#if defined(VTSS_SW_OPTION_ERPS)
    if (eps_type == MEP_EPS_TYPE_ERPS) {
        for (i=0; i<eps_count; ++i) {
            rc += erps_sf_sd_state_set(eps_inst[i], instance, sf_state, sd_state);
        }
    }
#endif
    if (rc)        T_D("Error during SF/SD set %u", instance);
#endif
}
                         ...
                         const BOOL             enable)
{
    ...
    return(invalid ? MEP_RC_INVALID_PARAMETER : MEP_RC_OK);
}
...
//    if (rc==MEP_RC_OK)   memcpy(instance_data[instance].aps_txdata, aps, sizeof(aps));
    if (rc==MEP_RC_OK)   memcpy(instance_data[instance].aps_txdata, aps, (instance_data[instance].eps_type == MEP_EPS_TYPE_ERPS) ? VTSS_MEP_SUPP_RAPS_DATA_LENGTH : VTSS_MEP_SUPP_LAPS_DATA_LENGTH);
    CRIT_EXIT(crit_p);
//T_D("tx aps  instance %lu  %X %X\n",instance, aps[0], aps[1]);
    if ((rc==MEP_RC_OK) && (vtss_mep_tx_aps_info_set(instance, aps, event) != VTSS_MEP_RC_OK))
                   ...
                   const u32   eps_inst)
{
    ...
    if ((rc==MEP_RC_OK) && (vtss_mep_signal_in(instance) != VTSS_MEP_RC_OK))
        rc = MEP_RC_INVALID_PARAMETER;
                        ...
                        const BOOL    enable)
{
    ...
    if ((instance_data[instance].aps_inst == APS_INST_INV) || (instance_data[instance].aps_inst != eps_inst))    rc = MEP_RC_INVALID_PARAMETER;
    if (rc==MEP_RC_OK)   instance_data[instance].raps_forward = enable;
    CRIT_EXIT(crit_p);

//T_D("forward enable  instance %lu  enable %u\n", instance, enable);
    if ((rc==MEP_RC_OK) && (vtss_mep_raps_forwarding(instance, enable) != VTSS_MEP_RC_OK))
        rc = MEP_RC_INVALID_PARAMETER;
                          ...
                          const BOOL    enable)
{
    ...
    if (instance_data[instance].eps_type != MEP_EPS_TYPE_ERPS)    rc = MEP_RC_INVALID_PARAMETER;
    if ((instance_data[instance].aps_inst == APS_INST_INV) || (instance_data[instance].aps_inst != eps_inst))    rc = MEP_RC_INVALID_PARAMETER;
    if (rc==MEP_RC_OK)   instance_data[instance].raps_tx = enable;
    CRIT_EXIT(crit_p);

//T_D("tx enable  instance %lu  enable %u\n", instance, enable);
    if ((rc==MEP_RC_OK) && (vtss_mep_raps_transmission(instance, enable) != VTSS_MEP_RC_OK))
        rc = MEP_RC_INVALID_PARAMETER;
...
void mep_port_protection_changed_evc_mep(u32 port)
{
    u32                   i;

    for(i=0; i<MEP_INSTANCE_MAX; ++i) {
        if ((instance_data[i].enable) && (instance_data[i].domain == VTSS_MEP_MGMT_EVC) && (instance_data[i].port[port])) { /* Check if an EVC on this port  */
            if (vtss_mep_mgmt_change_set(i) != VTSS_RC_OK)   continue;    /* This MEP needs to reconfigure as IS2 rules is depending on port protection */
        }
    }
}
                                ...
                                const BOOL        active)
{
    ...
    if ((active && (in_port_conv[w_port] == w_port)) || (!active && (in_port_conv[w_port] == 0xFFFF)))  /* Protection state changed */
        mep_port_protection_changed_evc_mep(w_port);
...
{
//T_D("mep_ring_protection_block port %lu  block %u\n", port,block);
    CRIT_ENTER(crit_p);
    CRIT_EXIT(crit_p);
}
...
BOOL vtss_mep_vlan_get(const u32 vid,   BOOL nni[VTSS_PORT_ARRAY_SIZE])
{
    ...
    if (vlan_mgmt_vlan_get(VTSS_ISID_START, vid, &conf, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) != VTSS_RC_OK)     return(FALSE);

    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_ARRAY_SIZE; port_no++) {
        nni[port_no] = (conf.ports[port_no] == VLAN_MEMBER_PORT) ? TRUE : FALSE;
    }
...
BOOL vtss_mep_vlan_member(u32 vid,  BOOL *ports,  BOOL enable)
{
    ...
    rc = vlan_mgmt_vlan_get(VTSS_ISID_START,  vid,  &vlan_entry,  FALSE,  VLAN_USER_MEP, VLAN_MEMBERSHIP_OP);
    if ((rc == VLAN_ERROR_PARM) || (rc == VLAN_ERROR_STACK_STATE))     return(FALSE);

    vlan_entry.vid = vid;
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_ARRAY_SIZE; port_no++) {
        vlan_entry.ports[port_no] = (ports[port_no] ? VLAN_MEMBER_PORT : VLAN_NON_MEMBER_PORT);
    }
    if (enable)     {if (vlan_mgmt_vlan_add(VTSS_ISID_START,  &vlan_entry,  VLAN_USER_MEP) != VTSS_OK)     return(FALSE);}
    else            {if (vlan_mgmt_vlan_del(VTSS_ISID_GLOBAL, vid, VLAN_USER_MEP, VLAN_MEMBERSHIP_OP) != VTSS_OK)    return(FALSE);}
...
//T_D("vtss_mep_acl_del  vid %lu\n", vid);
    ...
    do
    {
        if ((rc = acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  id,  &conf,  &counter,  TRUE)) == VTSS_OK)
        {
...
//    T_D("\n");
    ...
    if (acl_mgmt_ace_init(VTSS_ACE_TYPE_ETYPE, &conf) != VTSS_OK) {
        return FALSE;
    }
...
*/
    ...
    for (i=0; i<VTSS_PORT_ARRAY_SIZE; ++i) {
        if (acl->ing_port[i]) {
            if ((evc_vid[acl->vid] != 0xFFFFFFFF)) { /* Check if this VID is EVC */
...
#endif
    return(acl_mgmt_ace_add(ACL_USER_MEP,  ACE_ID_NONE,  &conf) == VTSS_OK);
}
...
BOOL vtss_up_mep_acl_add(u32 pag_oam, u32 pag_port, u32 customer, u32 first_egress_loop)
{
    ...
    conf.id = ACE_ID_NONE;
    if (acl_mgmt_ace_add(ACL_USER_MEP,  ACE_ID_NONE,  &conf) != VTSS_OK)    return(FALSE);
...
void vtss_mep_acl_raps_add(u32 vid,  BOOL *ports,  u8 *mac,  u32 *id)
{
    ...
    if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  ACE_ID_NONE,  &conf,  &counter,  TRUE) != VTSS_OK)    return;
    ...
    if (acl_mgmt_ace_add(ACL_USER_MEP,  next_id,  &conf) != VTSS_OK)      T_W("acl_mgmt_ace_add() failed for this VID  %u", vid);
...
{
    if (acl_mgmt_ace_del(ACL_USER_MEP, id) != VTSS_OK)      T_W("acl_mgmt_ace_del() failed for this ID  %u", id);
}
#endif
...
void vtss_mep_acl_dm_add(vtss_mep_mgmt_domain_t domain,  u32 vid,  u32 pag,  u8 level)
{
    ...
    if (domain == VTSS_MEP_MGMT_EVC) {
        ...
        conf.policy.mask = 0xFF;
        if (acl_mgmt_ace_add(ACL_USER_MEP,  ACE_ID_NONE,  &conf) != VTSS_OK)    T_W("acl_mgmt_ace_add() failed for this VID  %u", vid);
        conf.id = ACE_ID_NONE;
        conf.policy.mask = 0;
    }
...
void vtss_mep_acl_ccm_add(u32 instance,  u32 vid,  BOOL *port,  u32 level,  u8 *smac,  u32 pag)
{
    ...
    if (i < VTSS_PORT_ARRAY_SIZE) { /* One more entry is needed */
        conf.id = ACE_ID_NONE;
        conf.port_no = i;
        if (acl_mgmt_ace_add(ACL_USER_MEP,  conf.id,  &conf) != VTSS_OK)      T_W("acl_mgmt_ace_add() failed for this VID  %u", vid);
...
void vtss_mep_acl_mip_lbm_add(u32 instance,  u32 dmac_port,  BOOL *ports,  u32 mip_pag,  BOOL isdx_to_zero)
{
    ...
    if (acl_mgmt_ace_init(VTSS_ACE_TYPE_ETYPE, &conf) != VTSS_OK)     return;
...
*/
    if (acl_mgmt_ace_add(ACL_USER_MEP, ACE_ID_NONE, &conf) != VTSS_OK)      T_W("acl_mgmt_ace_add() failed for this id  %u", conf.id);
...
void vtss_mep_acl_evc_add(BOOL mep,  BOOL *mip_nni,  BOOL *mip_uni,  u32 mep_pag,  u32 mip_pag)
{
    ...
    {
        /* Get conf for id */
        if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  service_mip_ccm_ace_id,  &conf,  NULL,  FALSE) != VTSS_OK)    return;
    ...
    if (acl_mgmt_ace_add(ACL_USER_MEP, nxt_conf.id, &conf) != VTSS_OK)      T_W("acl_mgmt_ace_add() failed for this id  %u", conf.id);
    ...
    {
        /* Get conf for id */
        if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  service_mip_uni_ltm_ace_id,  &conf,  NULL,  FALSE) != VTSS_OK)    return;
...
*/
    ...
    {
        /* Get conf for id */
        if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  service_mip_nni_ltm_ace_id,  &conf,  NULL,  FALSE) != VTSS_OK)    return;
...
void vtss_mep_acl_ccm_hit(u32 instance)
{
    ...
    for (i=0; i<2; ++i) {
        ...
        {
            /* Get conf for id */
            if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  instance_data[instance].ccm_ace_id[i],  &conf,  &counter,  FALSE) != VTSS_OK)    return;
            conf.action.cpu_once = TRUE;
...
void vtss_mep_acl_tst_clear(u32 instance)
{
    ...
    for (i=0; i<2; ++i) {
        if (instance_data[instance].tst_ace_id[i] != ACE_ID_NONE) {
            ...
            /* Get conf for id */
            if (acl_mgmt_ace_get(ACL_USER_MEP,  VTSS_ISID_LOCAL,  instance_data[instance].tst_ace_id[i],  &conf,  &counter,  FALSE) != VTSS_OK)    continue;
            conf.action.cpu_once = TRUE;
...
void vtss_mep_supp_port_loop(const u32       instance,
                             const u32       port,
                             const BOOL      enable)
{
...
u32 vtss_mep_supp_get_active_port(const u32 port)
{
    u32 w_port;

    w_port = working_port_get(port);

    if ((out_port_conv[w_port] != w_port) && (in_port_conv[w_port] == 0xFFFF))  /* This port is protected and not active */
        return(out_port_conv[w_port]);   /* Return the protecting port */
    else
        return(w_port);                  /* Return the working port */
}
...
void vtss_mep_supp_packet_tx_cancel(u32 instance,  u8 *buffer)
{
    ...
    if (packet_tx_afi_cancel(buffer) != VTSS_OK)        T_D("Error during packet_tx_afi_cancel()");

    if ((instance_data[instance].tx_buffer[0][0] != NULL) && (instance_data[instance].tx_buffer[0][1] == buffer)) {
        if (packet_tx_afi_cancel(instance_data[instance].tx_buffer[0][0]) != VTSS_OK)        T_D("Error during packet_tx_afi_cancel()");
        instance_data[instance].tx_buffer[0][0] = NULL;
    }
    if ((instance_data[instance].tx_buffer[1][0] != NULL) && (instance_data[instance].tx_buffer[1][1] == buffer)) {
        if (packet_tx_afi_cancel(instance_data[instance].tx_buffer[1][0]) != VTSS_OK)        T_D("Error during packet_tx_afi_cancel()");
        instance_data[instance].tx_buffer[1][0] = NULL;
    }
#endif
}
...
static void packet_tx_done(void *context, packet_tx_done_props_t *props)
{
    ...
    vtss_mep_supp_timestamp_t tx_time;
                            ...
                            vtss_mep_supp_oam_type_t       oam_type)
{
    ...
    /* In case of rate <> 0 the frame is eighter CCM (>64) or a TST PDU. TST generator need to transmit frames smaller that 64 byte */ 
    ...
    if (instance_data[instance].domain == VTSS_MEP_MGMT_EVC)
    {
        for (i=0; i<VTSS_PORT_ARRAY_SIZE; ++i)
            if (port[i])
                port_mask |= port_protection_mask_calc(i);
    }
    ...
    if (!rate) {    /* Not automatically transmitted frame */
    ...
    if (port_mask)
    {
        ...
        if (!rate) {
            rc = packet_tx(&tx_props) == VTSS_RC_OK;   /* This is not HW based CCM */
            T_DG(TRACE_GRP_TX_FRAME, "TX frame:  instance %u  port_mask 0x%llX  isdx %u  qos %u  pcp %u  dp %u  len %u  maskarade %u  switched %u  oam_type %u  frame %X-%X-%X-%X-%X-%X-%X-%X-%X",
                 instance, tx_props.tx_info.dst_port_mask, tx_props.tx_info.isdx, tx_props.tx_info.cos, tx_props.tx_info.tag.pcp,
        ...
        { /* This is HW based CCM or TST - transmission on more ports demands more buffers */
#if defined(VTSS_FEATURE_AFI_FDMA) || defined(VTSS_FEATURE_AFI_SWC)
            u8 *tx_buffer = NULL;
            ...
            for (i=0, mask=1; i<VTSS_PORT_ARRAY_SIZE; ++i, mask<<=1)     if (port_mask & mask)    break;         /* Transmit frm on the first port */
            if (i == VTSS_PORT_ARRAY_SIZE)  {CRIT_EXIT(crit_p); return(1);}
            ...
            T_DG(TRACE_GRP_TX_FRAME, "TX frame:  instance %u  port_mask %llX  isdx %u  qos %u  pcp %u  dp %u  len %u  maskarade %u  switched %u  oam_type %u  frame %X-%X-%X-%X-%X-%X-%X-%X-%X",
                 instance, tx_props.tx_info.dst_port_mask, tx_props.tx_info.isdx, tx_props.tx_info.cos, tx_props.tx_info.tag.pcp,
                 tx_props.tx_info.dp, tx_props.packet_info.len[0], tx_props.tx_info.masquerade_port, tx_props.tx_info.switch_frm, tx_props.tx_info.oam_type,
                 frm[12+0], frm[12+1], frm[12+2], frm[12+3], frm[12+4], frm[12+5], frm[12+6], frm[12+7], frm[12+8]);

            /* Search for a second port */
            for (i=i+1, mask<<=1; i<VTSS_PORT_ARRAY_SIZE; ++i, mask<<=1)     if (port_mask & mask)    break;
            if (i < VTSS_PORT_ARRAY_SIZE)
            { /* Found - Allocate buffer for second port if needed */
                if (instance_data[instance].tx_buffer[0][0] == NULL) {
                    instance_data[instance].tx_buffer[0][0] = packet_tx_alloc(len);
                    instance_data[instance].tx_buffer[0][1] = frm;
                    tx_buffer = instance_data[instance].tx_buffer[0][0];
                }
                else if (instance_data[instance].tx_buffer[1][0] == NULL) {
                    instance_data[instance].tx_buffer[1][0] = packet_tx_alloc(len);
                    instance_data[instance].tx_buffer[1][1] = frm;
                    tx_buffer = instance_data[instance].tx_buffer[1][0];
                }
                if (tx_buffer != NULL) {
                    /* Transmit copy of frm on the second port */
                              ...
                              vtss_mep_supp_oam_type_t       oam_type)
{
    ...
    if (instance_data[instance].domain == VTSS_MEP_MGMT_EVC) {
        ...
        if (tx_props.tx_info.dst_port_mask) { /* Do not transmit if mask is all zero */
            rc = packet_tx(&tx_props) == VTSS_RC_OK;
            T_DG(TRACE_GRP_TX_FRAME, "TX frame:  instance %u  port_mask 0x%llX  isdx %u  qos %u  pcp %u  dp %u  len %u  maskarade %u  switched %u  oam_type %u  frame %X-%X-%X-%X-%X-%X-%X-%X-%X",
                 instance, tx_props.tx_info.dst_port_mask, tx_props.tx_info.isdx, tx_props.tx_info.cos, tx_props.tx_info.tag.pcp, tx_props.tx_info.dp,
                 tx_props.packet_info.len[0], tx_props.tx_info.masquerade_port, tx_props.tx_info.switch_frm, tx_props.tx_info.oam_type,
                 frm[12+0], frm[12+1], frm[12+2], frm[12+3], frm[12+4], frm[12+5], frm[12+6], frm[12+7], frm[12+8]);
        }
    }
...
static vtss_rc vtss_mep_ts_two_step_cb_reg(packet_tx_props_t *tx_props,
                                u64  port_mask,
                                void *context)
{
    vtss_rc rc;
    ...
    if (instance_data[instance].domain == VTSS_MEP_MGMT_EVC)
    {
        ...
        if (port_mask)
        {
            ...
            { 
                ...
                rc_reg = vtss_mep_ts_two_step_cb_reg(&tx_props, port_mask_ts, done); 
#endif 
            }
...
{
    vtss_mep_supp_onestep_extra_t *onestep_extra = tx_pre_contxt;
    u32 tc, diff, diff_ns;
...
{
    vtss_mep_supp_onestep_extra_t *onestep_extra = context;
    u32 now = 0;
    u32 lat;
...
static vtss_rc vtss_mep_ts_one_step_timed_cb_reg(packet_tx_props_t  *tx_props,
                                                 u64                port_mask,
                                                 void               *context)
{
    vtss_ts_id_t              ts_id;
...
#if defined(VTSS_ARCH_JAGUAR_1) || defined(VTSS_ARCH_LUTON26)
            port_mask_ts = port_mask;
            rc_reg = vtss_mep_ts_one_step_timed_cb_reg(&tx_props, port_mask_ts, (void *)buf_handle);
#endif  
            if ((rc_reg == VTSS_RC_OK)) {      
                rc = packet_tx(&tx_props) == VTSS_RC_OK;
                T_DG(TRACE_GRP_TX_FRAME, "TX frame:  instance %u  port_mask 0x%llX  isdx %u  qos %u  pcp %u  dp %u  len %u  maskarade %u  switched %u  oam_type %u  frame %X-%X-%X-%X-%X-%X-%X-%X-%X",
...
void vtss_mep_supp_timestamp_get(vtss_mep_supp_timestamp_t *timestamp, u32 *tc)
{
#if 1
...
i32 vtss_mep_supp_delay_calc(const vtss_mep_supp_timestamp_t *x,
                             const vtss_mep_supp_timestamp_t *y,
                             const BOOL                      is_ns)
{
    i32          diff=0;
...
static void evc_change_callback(u16  evc_id)
{
    ...
    delete = !vtss_mep_evc_flow_info_get(evc_id,  nni,  uni,  &vid,  &evid, &ttype, &tvid, &tunnel, &tag_cnt);

    CRIT_ENTER(crit_p);
    for(i=0; i<EVC_VID_SIZE; ++i)   /* Update the register of what EVC the VID is related to */
        if (evc_vid[i] == evc_id)   {evc_vid[i] = 0xFFFFFFFF; break;}
    if (!delete)    evc_vid[vid] = evc_id;
    ...
    for (i=0; i<VTSS_MEP_INSTANCE_MAX; ++i)
    {
        ...
        { /* This is a EVC MEP related to this EVC ID */
            ...
            if (delete || (!nni[conf.port] && !uni[conf.port]))
            {/* This is a delete of an EVC or this MEP is no longer on a EVC port - delete MEP in database */
                conf.enable = FALSE;
                if (mep_mgmt_conf_set(i, &conf) != VTSS_MEP_RC_OK)  T_D("Error during call to MEP base");
            }
            else
        ...
        { /* This is a Port MEP */
            ...
            if ((ais_conf.enable) && (ais_conf.client_domain == VTSS_MEP_MGMT_EVC))
            { /* AIS is enabled - check for AIS client EVC ID  */
                for (j=0; j<ais_conf.client_flow_count; ++j)    if (ais_conf.client_flows[j] == evc_id)     break;
                if (j < ais_conf.client_flow_count)
            ...
            if ((lck_conf.enable) && (lck_conf.client_domain == VTSS_MEP_MGMT_EVC))
            { /* LCK is enabled - check for LCK client EVC ID  */
                for (j=0; j<lck_conf.client_flow_count; ++j)    if (lck_conf.client_flows[j] == evc_id)     break;
                if (j < lck_conf.client_flow_count)
...
{
/*T_D("Port %lu   Link %u\n", port_no, info->link);*/
//printf("port_change_callback  port_no %lu  los %u\n", port_no, !info->link);
    los_state_set(port_no, !info->link);
}
...
static void vlan_port_change_callback(vtss_port_no_t port_no, const vlan_port_conf_t *new_conf)
{
    u32 i;
#if defined(VTSS_ARCH_SERVAL)
    vtss_rc  rc;
...
//printf("vlan_port_change_callback  port_no %u  port_type %u  port_type %u\n", port_no, new_conf->port_type, vlan_port_conf[port_no].port_type);
    if (vlan_port_conf[port_no].port_type != new_conf->port_type) {
        ...
        for (i=0; i<VTSS_MEP_INSTANCE_MAX; ++i)
        {
            if (instance_data[i].enable)
            {
                ...
                if ((instance_data[i].res_port == port_no) && (instance_data[i].domain == VTSS_MEP_MGMT_EVC)) {
                    CRIT_EXIT(crit_p);
                    rc = vtss_mep_mgmt_conf_get(i, mac, &mep_conf);
                    if ((mep_conf.mode == VTSS_MEP_MGMT_MEP) && (mep_conf.direction == VTSS_MEP_SUPP_UP))  /* This is a Up-MEP on this port */
                        if (rc == VTSS_RC_OK)    rc += vtss_mep_mgmt_change_set(i);
                    CRIT_ENTER(crit_p);
                }
#endif
            }
    ...
    if (memcmp(conf->tag_pcp_map, qos_port_conf[iport].tag_pcp_map, sizeof(conf->tag_pcp_map))) {
        ...
        for (i=0; i<VTSS_MEP_INSTANCE_MAX; ++i)
        {
            if (instance_data[i].enable && (instance_data[i].port[iport]) && (instance_data[i].domain == VTSS_MEP_MGMT_EVC))
            {
                CRIT_EXIT(crit_p);
                rc = vtss_mep_mgmt_conf_get(i, mac, &mep_conf);
                if (!mep_conf.voe && (mep_conf.mode == VTSS_MEP_SUPP_MEP) && (mep_conf.direction == VTSS_MEP_SUPP_DOWN))   /* This is a SW EVC Down-MEP on this port */
                    if (rc == VTSS_RC_OK)    rc += vtss_mep_mgmt_change_set(i);
                CRIT_ENTER(crit_p);
            }
        }
...
//printf("link_interrupt_function  source_id %u  instance_id %lu\n", source_id, instance_id);
...
/* This is only for L28 - frame is received with stripped TAG */
    u32                       port_conv;
    u32                       tl_idx;
    vtss_mep_supp_timestamp_t rx_time;
...
//    vtss_ts_timestamp_t ts;
    ...
    {   /* Received with a C or S TAG. Customer tagged will be treaded as S-tagged */
        ...
        BOOL evc=FALSE;
        CRIT_ENTER(crit_p);
        if (evc_vid[rx_info->tag.vid] != 0xFFFFFFFF)      /* Check if this VID is EVC */
        { /* This VID is a EVC */
            evc = TRUE;
...
vtss_rc mep_init(vtss_init_data_t *data)
{
    ...
    switch (data->cmd)
    {
        case INIT_CMD_INIT:
            ...
            memset(los_state, TRUE, sizeof(los_state));
            for (i=0; i<MEP_INSTANCE_MAX; ++i)
            {
                memset(&instance_data[i], 0, sizeof(instance_data_t));
                instance_data[i].aps_inst = APS_INST_INV;
        ...
        case INIT_CMD_SWITCH_ADD:
            T_D("SWITCH_ADD");
//            if ((blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size)) == NULL || ((size != sizeof(*blk)) && (size != sizeof(old_blk))))
            if ((blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, &size)) == NULL || size != sizeof(*blk))
            {
                T_W("conf_sec_open failed or size mismatch, creating defaults");
                blk = conf_sec_create(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, sizeof(*blk));

                blk->version = 0;
                set_conf_to_default(blk);
            }
#if 0
            if ((blk != NULL) && (size == sizeof(old_blk)) && (blk->version == 0)) {
                memcpy(&old_blk, blk, sizeof(old_blk));     /* Save old blk */

                blk = conf_sec_create(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE, sizeof(*blk));

                blk->version = 1;
                blk->up_mep_enabled = FALSE;
                memcpy(blk->conf, old_blk.conf, sizeof(blk->conf));
                memcpy(blk->cc_conf, old_blk.cc_conf, sizeof(blk->cc_conf));
                memcpy(blk->lm_conf, old_blk.lm_conf, sizeof(blk->lm_conf));
                memcpy(blk->aps_conf, old_blk.aps_conf, sizeof(blk->aps_conf));
                memcpy(blk->ais_conf, old_blk.ais_conf, sizeof(blk->ais_conf));
                memcpy(blk->lck_conf, old_blk.lck_conf, sizeof(blk->lck_conf));
            }
#endif
            apply_configuration(blk);
            conf_sec_close(CONF_SEC_GLOBAL, CONF_BLK_MEP_CONF_TABLE);
            ...
            vtssrc = port_change_register(VTSS_MODULE_ID_MEP, port_change_callback);
            if (vtssrc != VTSS_OK)        T_D("Error during port change register");
            ...
            if(vtss_board_features() & (VTSS_BOARD_FEATURE_AMS | VTSS_BOARD_FEATURE_LOS ))
            {
                /* In case of protection we need to hook on fast link fail and LOS detection */
                 vtssrc = vtss_interrupt_source_hook_set(link_interrupt_function,
                                                         INTERRUPT_SOURCE_FLNK,
                                                         INTERRUPT_PRIORITY_PROTECT);
                 vtssrc = vtss_interrupt_source_hook_set(link_interrupt_function,
                                                         INTERRUPT_SOURCE_LOS,
                                                         INTERRUPT_PRIORITY_PROTECT);
                 if (vtssrc != VTSS_OK)        T_D("Error during interrupt source hook");
            }
            ...
            vtssrc = evc_change_register(evc_change_callback);
            if (vtssrc != VTSS_OK)        vtss_mep_trace("vtss_mep_init - EVC register failed", 0, 0, 0, 0);

            memset(evc_vid, 0xFF, sizeof(evc_vid));
            evc_id = EVC_ID_FIRST;
            do
            {
                if ((rc = evc_mgmt_get(&evc_id, &conf, TRUE)) == VTSS_RC_OK)
                {
                    ...
                    {
                        evc_vid[conf.conf.network.pb.ivid] = evc_id;
                    }
#else
                    evc_vid[conf.conf.network.pb.ivid] = evc_id;
#endif
            ...
            vlan_port_conf_change_register(vlan_port_change_callback);

#if defined(VTSS_ARCH_SERVAL)
            vtssrc = qos_port_conf_change_register(FALSE, VTSS_MODULE_ID_MEP, qos_port_change_callback);
            if (vtssrc != VTSS_OK)        vtss_mep_trace("vtss_mep_init - QOS register failed", 0, 0, 0, 0);
#endif
            ...
            for (i = 0; i < VTSS_PORTS; i++) {
                if (mep_phy_ts_port[i].phy_ts_port == TRUE) {
                    ...
                    vtssrc = vtss_mep_phy_config_mac(i, 1, 1, mac); /* Add port mac to ingress side */  
                    vtssrc = vtss_mep_phy_config_mac(i, 1, 0, mac); /* Add port mac to egress side */ 
#ifdef MEP_DATA_DUMP                    
                    phy_ts_dump(i);
#endif                    
                    vtssrc = mep_phy_ts_update(i);  
                    if (vtssrc != VTSS_OK)        T_E("Error during config PHY timeStamp");                    
                }    
            }       
#endif                  
#endif
