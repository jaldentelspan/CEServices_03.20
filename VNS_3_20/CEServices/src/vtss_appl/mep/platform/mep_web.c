...
static char *mep_error_txt(u32 error)
{
    switch (error)
    {
        case MEP_RC_OK:                     return("MEP_RC_OK");
        case MEP_RC_INVALID_PARAMETER:      return("MEP_RC_INVALID_PARAMETER");
        case MEP_RC_NOT_ENABLED:            return("MEP_RC_NOT_ENABLED");
        case MEP_RC_CAST:                   return("MEP_RC_CAST");
        case MEP_RC_PERIOD:                 return("MEP_RC_PERIOD");
        case MEP_RC_PEER_CNT:               return("MEP_RC_PEER_CNT");
        case MEP_RC_PEER_ID:                return("MEP_RC_PEER_ID");
        case MEP_RC_MIP:                    return("MEP_RC_MIP");
        case MEP_RC_INVALID_EVC:            return("MEP_RC_INVALID_EVC");
        case MEP_RC_APS_EGRESS:             return("MEP_RC_APS_EGRESS");
        case MEP_RC_APS_DOMAIN:             return("MEP_RC_APS_DOMAIN");
        case MEP_RC_INVALID_VID:            return("MEP_RC_INVALID_VID");
        case MEP_RC_INVALID_COS_ID:         return("MEP_RC_INVALID_COS_ID");
        case MEP_RC_NO_VOE:                 return("MEP_RC_NO_VOE");
        case MEP_RC_PEER_MAC:               return("MEP_RC_PEER_MAC");
...
static cyg_int32 handler_config_mep_create(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST)
    {
        ...
        {
            if ((rc = mep_mgmt_conf_get(mep_id, mac, &eps_count, eps_inst, &config)) != MEP_RC_OK);
                if (error == MEP_RC_OK)     error = rc;

            if ((rc == MEP_RC_OK) && config.enable)
            {   /* Created */
                sprintf(buf, "del_%u", mep_id+1);
                if (cyg_httpd_form_varable_find(p, buf))
                {
                    config.enable = FALSE;
                    if ((rc = mep_mgmt_conf_set(mep_id, &config)) != MEP_RC_OK)
                        if (error == MEP_RC_OK)     error = rc;
                }
            }
        }
        ...
        if (cyg_httpd_form_varable_int(p, "new_mep", &mep_id))
        {
            ...
            if (cyg_httpd_form_varable_int(p, "level", &level))
            {
                ...
                if ((vtss_mep_mgmt_domain_t)domain == VTSS_MEP_MGMT_EVC)
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "port", &port))
                    {
                        ...
                        config.port = port-1;
                        if ((rc = mep_mgmt_conf_set(mep_id-1, &config)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
                else
    ...
    else
    {
        ...
        for (mep_id=0; mep_id<MEP_INSTANCE_MAX; ++mep_id)
        {
            ...
            {   /* Created */
                ...
                if ((rc = mep_mgmt_state_get(mep_id, &state)) != MEP_RC_OK);
                    if (error == MEP_RC_OK)     error = rc;
        ...
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%s", mep_error_txt(error));
        cyg_httpd_write_chunked(p->outbuffer, ct);
...
/*T_D("Post");*/
        ...
        {
            if ((rc = mep_mgmt_conf_get(mep_id-1, mac, &eps_count, eps_inst, &config)) != MEP_RC_OK);
                if (error == MEP_RC_OK)     error = rc;
            ...
            {   /* Created */
                ...
                cyg_httpd_form_varable_int(p, "evcpag", &evcpag);
                cyg_httpd_form_varable_int(p, "evcqos", &evcqos);
                cyg_httpd_form_varable_int(p, "vid", &vid);
                if (cyg_httpd_form_varable_int(p, "level", &level))
                if (cyg_httpd_form_varable_int(p, "format", &format))
                {
                    nameS=cyg_httpd_form_varable_string(p, "name", &n_len);
                    if ((n_len > 0) && (n_len <= (VTSS_MEP_MEG_CODE_LENGTH-1)))
                    {
                        megS=cyg_httpd_form_varable_string(p, "meg", &m_len);
                        if ((m_len > 0) && (m_len <= (VTSS_MEP_MEG_CODE_LENGTH-1)))
                        {
                            if (cyg_httpd_form_varable_int(p, "mep", &mep))
                            {
                                ...
                                memcpy(config.name, nameS, n_len);
                                memcpy(config.meg, megS, m_len);
                                config.mep = mep;
                ...
                if (cyg_httpd_form_varable_find(p, "cc"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "cc_rate", &cc_rate))
                    {
                        ...
                        if ((rc = mep_mgmt_cc_conf_set(mep_id-1,   &cc_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
                else
                ...
                if (cyg_httpd_form_varable_find(p, "aps"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "aps_octet", &aps_octet))
                    {
                        ...
                        if ((rc = mep_mgmt_aps_conf_set(mep_id-1,   &aps_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
                else
...
/*T_D("1 %lu", error);*/
    ...
    else
    {
        ...
        if ((rc = mep_mgmt_state_get(mep_id, &state)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_cc_conf_get(mep_id, &cc_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_aps_conf_get(mep_id, &aps_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
/*T_D("4 %lu", error);*/
        ...
        if (config.direction == VTSS_MEP_MGMT_EGRESS)        ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%u|", 1);
        ...
        for (i=0; i<len; ++i)
            sprintf(data_string,"%s%c", data_string,
                    config.name[i]);
        sprintf(data_string,"%s/", data_string);

        len = strlen(config.meg);
        for (i=0; i<len; ++i)
            sprintf(data_string,"%s%c", data_string,
                    config.meg[i]);
        sprintf(data_string,"%s/", data_string);
...
static cyg_int32 handler_config_fm_mep(CYG_HTTPD_STATE* p)
{
    ...
    char                       buf[32];
    uint                       mep_id, flow;
    uint                       lt_prio, lt_peer, lt_ttl, prio, rate, domain, level;
        ...
        if (cyg_httpd_form_varable_int(p, "mep_id_hidden", &mep_id))
        {
            ...
            {   /* Created */
                if (cyg_httpd_form_varable_find(p, "lt"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "lt_ttl", &lt_ttl))
                    {
                        ...
                        {   /* Unicast MAC found */
                            ...
                            lt_conf.ttl = lt_ttl;
                            if ((rc = mep_mgmt_lt_conf_set(mep_id-1,   &lt_conf)) != MEP_RC_OK)
                                if (error == MEP_RC_OK)     error = rc;
                        }
                    }
                }
                ...
                if (cyg_httpd_form_varable_find(p, "lb"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "lb_interval", &lb_interval))
                    {
                        ...
                        {   /* Unicast MAC found */
                            ...
                            if ((rc = mep_mgmt_lb_conf_set(mep_id-1,   &lb_conf)) != MEP_RC_OK)
                                if (error == MEP_RC_OK)     error = rc;
                        }
                    }
                }
                ...
                if (cyg_httpd_form_varable_int(p, "tst_pattern", &tst_pattern))
                {
                    ...
                    if ((rc = mep_mgmt_tst_conf_set(mep_id-1,   &tst_conf)) != MEP_RC_OK)
                        if (error == MEP_RC_OK)     error = rc;
                }
                ...
                if (cyg_httpd_form_varable_int(p, "c_level", &level))
                {
                    ais_conf.client_domain = (vtss_mep_mgmt_domain_t)domain;
                    ais_conf.client_level = level;
                    lck_conf.client_domain = (vtss_mep_mgmt_domain_t)domain;
                    lck_conf.client_level = level;
                    ...
                    for (i=0, j=0; i<VTSS_MEP_CLIENT_FLOWS_MAX; i++)
                    {
                        ...
                        {
                            ais_conf.client_flows[j] = flow - 1;
                            lck_conf.client_flows[j] = flow - 1;
                            j++;
                        }
                    }
                    ais_conf.client_flow_count = j;
                    lck_conf.client_flow_count = j;

#ifndef VTSS_ARCH_SERVAL
                    if (cyg_httpd_form_varable_int(p, "ais_prio", &prio))
#else
                    prio = 0; /* On Serval the highes EVC COS is used for injection on client layer */
#endif
                    if (cyg_httpd_form_varable_int(p, "ais_rate", &rate))
                    {
                        ...
                        ais_conf.client_prio = prio;
                        if ((rc = mep_mgmt_ais_conf_set(mep_id-1, &ais_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }

                    if (cyg_httpd_form_varable_int(p, "lck_prio", &prio))
                    if (cyg_httpd_form_varable_int(p, "lck_rate", &rate))
                    {
                        ...
                        lck_conf.client_prio = prio;
                        if ((rc = mep_mgmt_lck_conf_set(mep_id-1, &lck_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
            }
...
/*T_D("1 %lu", error);*/
    ...
    else
    {
        ...
        if ((rc = mep_mgmt_lt_conf_get(mep_id, &lt_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_lb_conf_get(mep_id, &lb_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_ais_conf_get(mep_id, &ais_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_lck_conf_get(mep_id, &lck_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_tst_conf_get(mep_id, &tst_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_lt_state_get(mep_id, &lt_state)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_lb_state_get(mep_id, &lb_state)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_tst_state_get(mep_id, &tst_state)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
/*T_D("4 %u", error);*/
        ...
        for (i=0; i<lt_state.transaction_cnt; ++i)
        {
            ...
            for (j=0; j<lt_state.transaction[i].reply_cnt; ++j)
                sprintf(data_string,"%s%u/%u/%u/%s/%02X-%02X-%02X-%02X-%02X-%02X/%02X-%02X-%02X-%02X-%02X-%02X/", data_string,
                        lt_state.transaction[i].reply[j].ttl,
                        (uint)lt_state.transaction[i].reply[j].mode,
                        (uint)lt_state.transaction[i].reply[j].direction,
                        (lt_state.transaction[i].reply[j].relayed) ? "Yes" : "No",
        ...
        data_string[0] = '\0';
        sprintf(data_string, "%u/%u/", (uint)ais_conf.client_domain, (uint)ais_conf.client_level);
        for (i=0; i<ais_conf.client_flow_count; i++)
            sprintf(data_string, "%s%u/", data_string, (uint)(ais_conf.client_flows[i]+1));
        for (i=0; i<(VTSS_MEP_CLIENT_FLOWS_MAX - ais_conf.client_flow_count); i++)
            sprintf(data_string, "%s%u/", data_string, 0);
            ...
            {   /* Created */
                if (cyg_httpd_form_varable_find(p, "lm"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "lm_prio", &lm_prio))
                    {
                        ...
                        if ((rc = mep_mgmt_lm_conf_set(mep_id-1,   &lm_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
                else
                ...
                if (cyg_httpd_form_varable_find(p, "dm"))
                {
                    ...
                    if (cyg_httpd_form_varable_int(p, "dm_act", &dm_act))   
                    {
                        ...
                        dm_conf.mep = dm_mep;
                        dm_conf.way = dm_way;
                        dm_conf.txway = dm_txway;
                        dm_conf.calcway = dm_calcway;
                        dm_conf.gap = dm_gap;
                        dm_conf.count = dm_count;
                        dm_conf.tunit = dm_tunit;
                        dm_conf.dm2fordm1 = (cyg_httpd_form_varable_find(p, "dm_d2ford1") != NULL);
                        dm_conf.act = dm_act;
                        
                              
                        if ((rc = mep_mgmt_dm_conf_set(mep_id-1,   &dm_conf)) != MEP_RC_OK)
                            if (error == MEP_RC_OK)     error = rc;
                    }
                }
                else
...
/*T_D("1 %u", error);*/
    ...
    else
    {
        ...
        if ((rc = mep_mgmt_lm_conf_get(mep_id, &lm_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_dm_conf_get(mep_id, &dm_conf)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;    
        if ((rc = mep_mgmt_lm_state_get(mep_id, &lm_state)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;
        if ((rc = mep_mgmt_dm_state_get(mep_id, &dmr_state, &dm1_state_far_to_near, &dm1_state_near_to_far)) != MEP_RC_OK)
            if (error == MEP_RC_OK)     error = rc;            
...
/*T_D("4 %u", error);*/
        ...
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u|",
                     ...
                     dm1_state_far_to_near.best_delay,     
                     dm1_state_far_to_near.worst_delay,    
                     ...
                     dmr_state.best_delay,     
                     dmr_state.worst_delay,    
                     ...
                     dm1_state_near_to_far.best_delay,     
                     dm1_state_near_to_far.worst_delay,    
