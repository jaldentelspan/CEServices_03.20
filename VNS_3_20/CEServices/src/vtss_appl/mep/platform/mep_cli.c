...
typedef struct
{
    ...
    vtss_mep_mgmt_lm_ended_t   ended;
    u32                        flr_interval;
    u32                        octet;

    vtss_mep_mgmt_dm_way_t     way;            
    vtss_mep_mgmt_dm_txway_t   txway;           
...
static void mep_print_error(u32   rc)
{
    switch (rc)
    {
        case MEP_RC_INVALID_PARAMETER:  CPRINTF("Invalid parameter error returned from MEP\n"); break;
        case MEP_RC_NOT_ENABLED:        CPRINTF("MEP instance is not enabled\n"); break;
        case MEP_RC_CAST:               CPRINTF("LM and CC sharing SW generated CCM must have same casting (multi/uni)\n"); break;
        case MEP_RC_PERIOD:             CPRINTF("LM and CC sharing SW generated CCM must have same period\n"); break;
        case MEP_RC_PEER_CNT:           CPRINTF("Invalid number of peer's for this configuration\n"); break;
        case MEP_RC_PEER_ID:            CPRINTF("Duplicate peer MEP ID\n"); break;
        case MEP_RC_MIP:                CPRINTF("Not allowed on a MIP\n"); break;
        case MEP_RC_INVALID_EVC:        CPRINTF("EVC flow was found invalid\n"); break;
        case MEP_RC_APS_EGRESS:         CPRINTF("APS not allowed on Egress MEP\n"); break;
        case MEP_RC_APS_DOMAIN:         CPRINTF("R-APS only allowed in Port domain\n"); break;
        case MEP_RC_INVALID_VID:        CPRINTF("VLAN is not created for this VID\n"); break;
        case MEP_RC_INVALID_COS_ID:     CPRINTF("Invalid COS ID (priority) for this EVC\n"); break;
        case MEP_RC_NO_VOE:             CPRINTF("No VOE available\n"); break;
        case MEP_RC_PEER_MAC:           CPRINTF("Peer Unicast MAC must be known to do HW based CCM on SW MEP\n"); break;
        default:                        CPRINTF("Unknown error returned from MEP\n"); break;
    }
}
    ...
    if (req->set)
    {
        ...
        if (!mep_req->enable)
        {
            ...
            for (i=0; i<mep_req->count; ++i)
                if ((rc = mep_mgmt_conf_set(mep_req->instance+i, &config)) != MEP_RC_OK)
                    mep_print_error(rc);
            return;
        }
        ...
        for (i=0; i<mep_req->count; ++i)
        {
            ...
            config.direction = VTSS_MEP_MGMT_INGRESS;
            ...
            if ((rc = mep_mgmt_cc_conf_set(mep_req->instance+i, &cc_config)) != MEP_RC_OK)
                mep_print_error(rc);
        }
    }
}
...
static void cli_cmd_id_mep_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        else
        {
            ...
            strncpy(config.name, "VITESS", VTSS_MEP_MEG_CODE_LENGTH);
            if (mep_req->flag & FLAG_PARAM_MEG_ID)      strncpy(config.meg, mep_req->meg_id, VTSS_MEP_MEG_CODE_LENGTH);
            if (mep_req->flag & FLAG_PARAM_MEG_NAME)    strncpy(config.name, mep_req->meg_name, VTSS_MEP_MEG_CODE_LENGTH);
//            if (mep_req->flag & FLAG_PARAM_VOE)         config.voe = mep_req->voe;
            config.voe = mep_req->voe;
    
            if ((rc = mep_mgmt_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
                mep_print_error(rc);
        }
    }
    else
        ...
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            else
            {
                if (config.enable)
                {
                    ...
                    {
                        case VTSS_MEP_MGMT_INGRESS:   CPRINTF("%14s", "Ingress");     break;
                        case VTSS_MEP_MGMT_EGRESS:    CPRINTF("%14s", "Egress");      break;
                    }
...
//                        case VTSS_MEP_MGMT_MPLS:       CPRINTF("%8s", "Mpls");     break;
                    ...
                    if (config.mode == VTSS_MEP_MGMT_MEP)
                    {
                        ...
                        CPRINTF("%11s", config.name);
                        CPRINTF("%11s", config.meg);
                        CPRINTF("%11u", config.mep);
                    }
                    else
                    {
                        CPRINTF("             ");
...
static void cli_cmd_id_mep_cc_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_cc_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_dm_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if (mep_req->flag & FLAG_PARAM_MEP)         config.mep = mep_req->mep;
        if (mep_req->flag & FLAG_PARAM_WAY)         config.way = mep_req->way;
        if (mep_req->flag & FLAG_PARAM_DM_GAP)      config.gap = mep_req->dm_gap;
        if (mep_req->flag & FLAG_PARAM_TXWAY)       config.txway = mep_req->txway;
        if (mep_req->flag & FLAG_PARAM_CALCWAY)     config.calcway = mep_req->calcway;
        if (mep_req->flag & FLAG_PARAM_COUNT)       config.count = mep_req->count;
        if (mep_req->flag & FLAG_PARAM_TUNIT)       config.tunit = mep_req->tunit;
        if (mep_req->flag & FLAG_PARAM_ACT)         config.act = mep_req->act;
        if (mep_req->flag & FLAG_PARAM_DM2FORDM1)   config.dm2fordm1 = mep_req->dm2fordm1;

        if ((rc = mep_mgmt_dm_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
        ...
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            else
            {
                if (config.enable)
                {
                    ...
                    switch (config.way)
                    {
                        case VTSS_MEP_MGMT_ONE_WAY:     CPRINTF("%10s", "One-way");   break;
                        case VTSS_MEP_MGMT_TWO_WAY:     CPRINTF("%10s", "Two-way");   break;
                        default:                        CPRINTF("%10s", "Unknown");
                    }
                    switch (config.txway)
                    {
                        case VTSS_MEP_MGMT_STD:         CPRINTF("%10s", "Std");   break;
                        case VTSS_MEP_MGMT_PROP:        CPRINTF("%10s", "Prop");   break;
                        default:                        CPRINTF("%10s", "Unknown");
                    }
                    ...
                    CPRINTF("%8u", config.gap);
                    CPRINTF("%8u", config.count);
                    switch (config.tunit)
                    {
...
static void cli_cmd_id_mep_lm_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_lm_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_aps_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_aps_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_client_config (cli_req_t * req)
{
    ...
    if (req->set) {
        ...
        if ((rc = mep_mgmt_ais_conf_get(mep_req->instance, &ais_config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return; 
        }

        if (mep_req->flag & FLAG_PARAM_DOMAIN)   ais_config.client_domain = mep_req->domain;
        if (mep_req->flag & FLAG_PARAM_LEVEL)    ais_config.client_level  = mep_req->level;
        if (mep_req->flag & FLAG_PARAM_CLIENT_FLOWS) {
            for (i=0; i<mep_req->client_flow_count; ++i)
                ais_config.client_flows[i] = mep_req->client_flows[i];
            ais_config.client_flow_count = mep_req->client_flow_count;
        }

        if ((rc = mep_mgmt_ais_conf_set(mep_req->instance, &ais_config)) != MEP_RC_OK)
            mep_print_error(rc);

        if ((rc = mep_mgmt_lck_conf_get(mep_req->instance, &lck_config)) != MEP_RC_OK)
            mep_print_error(rc);

        if (mep_req->flag & FLAG_PARAM_DOMAIN)   lck_config.client_domain = mep_req->domain;
        if (mep_req->flag & FLAG_PARAM_LEVEL)    lck_config.client_level  = mep_req->level;
        if (mep_req->flag & FLAG_PARAM_CLIENT_FLOWS) {
            for (i=0; i<mep_req->client_flow_count; ++i)
                lck_config.client_flows[i] = mep_req->client_flows[i];
            lck_config.client_flow_count = mep_req->client_flow_count;
        }

        if ((rc = mep_mgmt_lck_conf_set(mep_req->instance, &lck_config)) != MEP_RC_OK)
            mep_print_error(rc);
    } else {
        CPRINTF("\n");
        ...
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            else
            {
                ...
                {
                    CPRINTF("%9u", i+1);
                    switch (ais_config.client_domain)
                    {
...
//                        case VTSS_MEP_MGMT_MPLS:       CPRINTF("%11s", "Mpls");  break;
                    ...
                    CPRINTF("%10d", ais_config.client_level);
                    CPRINTF("     ");
                    for (j=0; j<(10-ais_config.client_flow_count); ++j)
                        CPRINTF("  ");
                    for (j=0; j<ais_config.client_flow_count; ++j)
                        CPRINTF("%u-", ais_config.client_flows[j]+1);
                    CPRINTF("\n");
                }
            }
        }
...
{
    u32                        rc;
    vtss_mep_mgmt_ais_conf_t   config;
    mep_cli_req_t              *mep_req = req->module_req;
    u32                        i;
    ...
    if (req->set) {
        ...
        if ((rc = mep_mgmt_ais_conf_get(mep_req->instance, &config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return;
        }
        ...
        config.enable = mep_req->enable;
        if (mep_req->flag & FLAG_PARAM_PERIOD)      config.period        = mep_req->period;
        if (mep_req->flag & FLAG_PARAM_PROTECTION)  config.protection    = mep_req->protection;
        if (mep_req->flag & FLAG_PARAM_PRIO)        config.client_prio   = mep_req->prio;

        if ((rc = mep_mgmt_ais_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    } else {
        CPRINTF("\n");
        ...
        {
            if ((mep_req->instance != 0xFFFFFFFF) && (mep_req->instance != i))      continue;
            if ((rc = mep_mgmt_ais_conf_get(i, &config)) == MEP_RC_INVALID_PARAMETER)
                mep_print_error(rc);
            else
            {
                if (config.enable)
                {
                    ...
                    if (config.protection)      CPRINTF("%15s", "Enabled");
...
{
    u32                        rc, i;
    vtss_mep_mgmt_lck_conf_t   config;
    mep_cli_req_t              *mep_req = req->module_req;
    ...
    if (req->set) {
        ...
        if ((rc = mep_mgmt_lck_conf_get(mep_req->instance, &config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return;
        }
        ...
        if ((rc = mep_mgmt_lck_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    } else {
        CPRINTF("\n");
        ...
        {
            if ((mep_req->instance != 0xFFFFFFFF) && (mep_req->instance != i))      continue;
            if ((rc = mep_mgmt_lck_conf_get(i, &config)) == MEP_RC_INVALID_PARAMETER)
                mep_print_error(rc);
            else
            {
                if (config.enable)
...
static void cli_cmd_id_mep_lt_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_lt_conf_get(mep_req->instance, &config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return;
        }
...
//why this !!        memset(config.mac, 0, sizeof(config.mac));
        ...
        if ((rc = mep_mgmt_lt_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_lb_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_lb_conf_get(mep_req->instance, &config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return;
        }
        ...
        if ((rc = mep_mgmt_lb_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_tst_config(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        if ((rc = mep_mgmt_tst_conf_get(mep_req->instance, &config)) != MEP_RC_OK) {
            mep_print_error(rc);
            return;
        }
        ...
        if (mep_req->flag & FLAG_PARAM_DEI)         config.dei = mep_req->dei;
        if (mep_req->flag & FLAG_PARAM_MEP)         config.mep = mep_req->mep;
        if (mep_req->flag & FLAG_PARAM_RATE)        config.rate = mep_req->rate;
        if (mep_req->flag & FLAG_PARAM_SIZE)        config.size = mep_req->size;
        ...
        if ((rc = mep_mgmt_tst_conf_set(mep_req->instance, &config)) != MEP_RC_OK)
            mep_print_error(rc);
    }
    else
    {
...
static void cli_cmd_id_mep_state(cli_req_t *req)
{
    ...
    for (i=0; i<MEP_INSTANCE_MAX; ++i)
    {
        ...
        if ((rc = mep_mgmt_conf_get(i, mac, &eps_count, eps_inst, &config)) == MEP_RC_INVALID_PARAMETER)
            mep_print_error(rc);
        if (config.mode == VTSS_MEP_MGMT_MIP)   continue;
        ...
        else
        {
            if (rc != MEP_RC_NOT_ENABLED)
            {
                ...
                if (config.peer_count == 0)    CPRINTF("\n");
            }
        }
    }
    CPRINTF("\n");
            ...
            if ((rc != MEP_RC_NOT_ENABLED) && (state.transaction_cnt != 0))
            {
                ...
                for (t=0; t<state.transaction_cnt; ++t)
                {
                    ...
                    for (j=0; j<state.transaction[t].reply_cnt; ++j)
                    {
                        ...
                        CPRINTF("%12s", state.transaction[t].reply[j].relayed ? "Yes" : "No");
        ...
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            else
            {
                ...
                {   
                    ...
                    CPRINTF(" %8u", dm1_state_far_to_near.best_delay);
                    CPRINTF(" %9u", dm1_state_far_to_near.worst_delay);
                    ...
                    CPRINTF(" %8u", dm1_state_near_to_far.best_delay);
                    CPRINTF(" %9u", dm1_state_near_to_far.worst_delay);
        ...
        for (i=0; i<MEP_INSTANCE_MAX; ++i)
        {
            ...
            else
            {
                if (rc != MEP_RC_NOT_ENABLED)
                {
                    ...
                    CPRINTF(" %8u", dmr_state.best_delay);
                    CPRINTF(" %9u", dmr_state.worst_delay);
...
static void cli_cmd_id_debug_mep_up_mep(cli_req_t *req)
{
    ...
    for (i=12; i<16; ++i) {
        ...
        if (vlan_mgmt_port_conf_set(VTSS_ISID_START, i, &vlan_conf, VLAN_USER_STATIC) != VTSS_OK)
            continue;
    }
...
static void cli_cmd_id_debug_mep_dm_state(cli_req_t *req)
{
    ...
    delay = malloc(sizeof(u32) * VTSS_MEP_DM_MAX);
    if (delay == NULL )
        return; 
    delay_var = malloc(sizeof(u32) * VTSS_MEP_DM_MAX);
    if (delay_var == NULL )
    {
        free(delay);
        return;  
    ...
    for (i=0; i<MEP_INSTANCE_MAX; ++i)
    {
        if ((rc = mep_mgmt_dm_conf_get(i, &config)) != MEP_RC_OK)
            continue;
        if ((rc = mep_mgmt_dm_db_state_get(i, delay, delay_var)) != MEP_RC_OK)
            continue;
        else
        {
            ...
            if (rc == MEP_RC_OK)
            {    
                count = (dmr_state.rx_cnt > config.count) ? config.count : dmr_state.rx_cnt;
...
static int cli_mep_meg_id_parse (char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    if (!(error = cli_parse_text(cmd_org, req->parm, VTSS_MEP_MEG_CODE_LENGTH)))
    {
        ...
        if (mep_req->format == VTSS_MEP_MGMT_ITU_ICC)       error = ((len != 6) && (len != 7));
        if (len > 8)    error = TRUE;

        strcpy(&mep_req->meg_id[0], req->parm);
...
static int cli_mep_meg_name_parse (char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    if (!(error = cli_parse_text(cmd_org, req->parm, VTSS_MEP_MEG_CODE_LENGTH)))
    {
        ...
        if (mep_req->format == VTSS_MEP_MGMT_ITU_ICC)       error = (len != 6);
        if (len > 8)    error = TRUE;

        strcpy(&mep_req->meg_name[0], req->parm);
...
static int cli_mep_direction_parse (char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    if(!found)      return 1;
    else if(!strncmp(found, "ingress", 7))      mep_req->direction = VTSS_MEP_MGMT_INGRESS;
    else if(!strncmp(found, "egress", 6))       mep_req->direction = VTSS_MEP_MGMT_EGRESS;
    else return 1;
...
static int cli_mep_way_parse (char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    if(!found)      return 1;
    else if(!strncmp(found, "oneway", 6))    mep_req->way = VTSS_MEP_MGMT_ONE_WAY;
    else if(!strncmp(found, "twoway", 6))    mep_req->way = VTSS_MEP_MGMT_TWO_WAY;
    else return 1;
...
static int cli_mep_txway_parse (char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    if(!found)      return 1;
    else if(!strncmp(found, "std", 3))    mep_req->txway = VTSS_MEP_MGMT_STD;
    else if(!strncmp(found, "prop", 4))   mep_req->txway = VTSS_MEP_MGMT_PROP;
    else return 1;
...
void mep_cli_def_req ( cli_req_t * req)
{
   ...
   mep_req->ended = VTSS_MEP_MGMT_DUAL_ENDED;
   mep_req->way = VTSS_MEP_MGMT_ONE_WAY;
   mep_req->txway = VTSS_MEP_MGMT_STD;
...
static cli_parm_t mep_cli_parm_table[] = {
    ...
    },
    {
        ...
        "       IEEE short name must be max. 8 chars\n"
        "       ITU UMC must be 6 or 7 chars - if 6 is chosen trailing NULL will be added",
        CLI_PARM_FLAG_SET,
    ...
    {
        "<name>",
        "IEEE Domain name or ITU ICC\n"
        "       IEEE Domain name must be max. 8 chars\n"
        "       ITU ICC must be 6 chars",
        CLI_PARM_FLAG_SET,
...
cli_cmd_tab_entry (
  "MEP config",
  "MEP config [<inst>] [mep|mip] [voe] [ingress|egress] [<port>] [domport|domevc] [<level>] [itu|ieee] [<meg_id>] [<name>] [<mep>] [<vid>] [<flow>] [enable|disable]",
