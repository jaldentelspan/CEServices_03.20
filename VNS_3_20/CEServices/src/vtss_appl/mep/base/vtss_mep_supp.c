...
#define EVENT_IN_MASK             0x000000000EFFFFFFLL
...
typedef struct
{
    ...
    vtss_mep_supp_timestamp_t      tx_dmm_fup_timestamp;                   /* DMM transmission time (follow-up) */
    vtss_mep_supp_timestamp_t      rx_dmm_timestamp;                       /* DMM receive time      */
    vtss_mep_supp_timestamp_t      rx_dmr_timestamp;                       /* DMR receive time      */
    vtss_mep_supp_timestamp_t      rx_dm1_timestamp;                       /* DM1 receive time      */
    ...
    vtss_mep_supp_timestamp_t      rxTimef;                                /* DMM receive time returned in DMR */
    vtss_mep_supp_timestamp_t      txTimeb;                                /* DMR transmit time extracted from DMR */
...
static BOOL voe_down_mep_present(u32 vid,  u32 *voe_idx);
#endif

/* Loop port is always the last port, convert to zero-based port number */
...
#ifdef VTSS_ARCH_SERVAL
#define evc_up(data) (((data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (data->config.direction == VTSS_MEP_SUPP_UP)) ? TRUE : FALSE)
#else
#define evc_up(data) ((data->config.voe) ? TRUE : FALSE)   /* This is always FALSE on not Serval - this is only done like this as Lint complains if this is just FALSE */
#endif

#define init_frame_info(frame_info) \
            frame_info.bypass = TRUE;\
            frame_info.maskerade_port = VTSS_PORT_NO_NONE;\
            frame_info.isdx = 0xFFFF;\
...
static u32 init_tx_frame(u32 instance,   u8 dmac[],   u8 smac[],   u32 prio,   BOOL dei,   u8 frame[],   BOOL insert_header)
{
    ...
    if (config->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) {  /* This is a EVC MEP/MIP */
        if ((config->mode == VTSS_MEP_SUPP_MEP) && (config->direction == VTSS_MEP_SUPP_UP)) {  /* This is a SW/VOE EVC Up-MEP - 'Dummy tag' must be added */
            ...
            dei_in = dei;
            prio_in = prio;
            for (prio=0; prio<VTSS_PRIO_ARRAY_SIZE; ++prio) {   /* Search for the QOS mapping thet gives correct classified qos and dp - This is only relevant if the PMF_4 fix is active. If not active the QOS port mapping must be 1:1 */
                dei = 0;
            ...
            insert_c_s_tag(&frame[size], (vlan_conf.port_type == VTSS_VLAN_PORT_TYPE_C) ? 0x8100 : 0x88A8, config->flow.vid, prio, dei);
            size+=4;
        }
        else
        if ((config->mode == VTSS_MEP_SUPP_MIP) && (config->direction == VTSS_MEP_SUPP_UP)) {  /* This is a SW EVC Up-MIP (subscriber MIP) */
            insert_c_s_tag(&frame[size], (config->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_CINVID) ? 0x8100 : 0x88A8, config->flow.in_vid, prio, dei);
            size+=4;
        }
        else
        if (!config->voe && (config->mode == VTSS_MEP_SUPP_MEP) && (config->direction == VTSS_MEP_SUPP_DOWN)) {  /* This is a SW EVC Down-MEP */
            ...
            insert_c_s_tag(&frame[size], (config->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_COUTVID) ? 0x8100 : 0x88A8, config->flow.out_vid, prio, dei);
            size+=4;
        }
        else
        if (config->voe && (config->mode == VTSS_MEP_SUPP_MEP) && (config->direction == VTSS_MEP_SUPP_DOWN)) {  /* This is a VOE EVC Down-MEP */
        }   /* Do nothing - transmit untagged in any case */
        else
        if ((config->mode == VTSS_MEP_SUPP_MIP) && (config->direction == VTSS_MEP_SUPP_DOWN)) {  /* This is a SW EVC Down-MIP (subscriber MIP) */
            insert_c_s_tag(&frame[size], (config->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_COUTVID) ? 0x8100 : 0x88A8, config->flow.out_vid, prio, dei);
            size+=4;
        }
    }
...
static u32 init_client_tx_frame(u32 instance,   u8 dmac[],   u8 smac[],   u32 prio,   BOOL dei,   u8 frame[])
{
    ...
    if (frame == instance_data[instance].tx_ais)
        flow = &(instance_data[instance].ais_config.flows[instance_data[instance].ais_inx]);
    if (frame == instance_data[instance].tx_lck)
    ...
    (void)vtss_qos_port_conf_get(NULL, config->port, &qos_conf);
    vtss_mep_supp_evc_mce_info_get(flow->evc_id, &nni_type, nni_outer_tag, nni_key);

    for (i=VTSS_MEP_SUPP_COS_ID_SIZE-1; i >=0; i--) /* Find the highes COS */
        if (nni_outer_tag[i].enable)  break;

    if (i < 0)
        i = prio;   /* this shoul never happen - no COS was found for this EVC */

    if (nni_type == VTSS_MEP_SUPP_EVC_NNI_TYPE_E_NNI) /* E-NNI - COS-ID is mapped according to ECE configuration */
        prio = nni_outer_tag[i].pcp;
    else { /* I-NNI - port based mapping */
        if (qos_conf.tag_remark_mode == VTSS_TAG_REMARK_MODE_MAPPED)
            prio = qos_conf.tag_pcp_map[i][dei];
    }
    insert_c_s_tag(&frame[size], (config->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_COUTVID) ? 0x8100 : 0x88A8, flow->out_vid, prio, dei);
    size+=4;
#else
    if (flow->mask & VTSS_MEP_SUPP_FLOW_MASK_COUTVID) {
        insert_c_s_tag(&frame[size], 0x8100, flow->out_vid, prio, dei);
...
static void megid_calc(supp_instance_data_t  *data, u8 *buf)
{
    u32  n_len, m_len;

    if (data->ccm_config.format == VTSS_MEP_SUPP_ITU_ICC)
    {
        buf[0] = 1;
        buf[1] = 32;
        buf[2] = 13;
        memcpy(&buf[3], data->ccm_config.name, 6);
        memcpy(&buf[3+6], data->ccm_config.meg, 7);
    }
    else
    {
        ...
        buf[0] = 4;
        buf[1] = n_len;
        memcpy(&buf[2], data->ccm_config.name, n_len);
        buf[2+n_len] = 2;
        buf[2+n_len+1] = m_len;
        memcpy(&buf[2+n_len+1+1], data->ccm_config.meg, m_len);
    }
}
...
static BOOL handle_dmr(supp_instance_data_t *data)
{
    ...
    if ((data->dmm_config.txway == VTSS_MEP_SUPP_STANDARDIZE && 
        data->dmm_config.calcway == VTSS_MEP_SUPP_RDTRIP) ||
        (data->dmm_config.txway == VTSS_MEP_SUPP_STANDARDIZE && 
             data->dmm_config.calcway == VTSS_MEP_SUPP_FLOW))
    {
    ...
    else if (data->dmm_config.txway == VTSS_MEP_SUPP_PROPRIETARY && 
             data->dmm_config.calcway == VTSS_MEP_SUPP_FLOW)    
    {
...
static void ccm_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
...
static void lmm_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void lmr_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void ltm_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void ltr_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void lbm_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void lbr_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void aps_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void ais_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void lck_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void tst_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void dmm_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    BOOL  new_dmr=FALSE;
...
static void dm1_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void dm1_fup_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{ 
    int                             rc;
...
static void dmr_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    vtss_mep_supp_crit_lock();
...
static void dmr_fup_done(u32 instance,  u8 *frame,  vtss_mep_supp_timestamp_t tx_time,  u64 frame_count)
{
    int                             rc;
    vtss_mep_supp_tx_frame_info_t   tx_frame_info;
...
static void voe_config(supp_instance_data_t *data)
{
    ...
    cfg.voe_type = (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) ? VTSS_OAM_VOE_SERVICE : VTSS_OAM_VOE_PORT;    /* Calculate if VOE is a Port or a EVC VOE */
    ...
    if (data->config.enable && (data->config.direction == VTSS_MEP_SUPP_UP) && voe_down_mep_present(data->config.flow.vid, &voe_idx)) {  /* This EVC Up-MEP enabled - must point to any present VOE Down-MEP */
...
//    cfg.loop_isdx_p = 
    ...
    cfg.generic[0].enable = TRUE;           /* AIS */
    cfg.generic[0].copy_to_cpu = TRUE;
    cfg.generic[0].forward = FALSE;
    cfg.generic[0].count_as_selected = FALSE;
    cfg.generic[0].count_as_data = FALSE;
    cfg.generic[1].enable = TRUE;           /* LCK */
    cfg.generic[1].copy_to_cpu = TRUE;
    cfg.generic[1].forward = FALSE;
    cfg.generic[1].count_as_selected = FALSE;
    cfg.generic[1].count_as_data = FALSE;
    cfg.generic[2].enable = TRUE;           /* LAPS */
    cfg.generic[2].copy_to_cpu = TRUE;
    cfg.generic[2].forward = FALSE;
    cfg.generic[2].count_as_selected = FALSE;
    cfg.generic[2].count_as_data = TRUE;
    cfg.generic[3].enable = TRUE;           /* RAPS */
    cfg.generic[3].copy_to_cpu = TRUE;
    cfg.generic[3].forward = FALSE;
    cfg.generic[3].count_as_selected = FALSE;
    cfg.generic[3].count_as_data = TRUE;
    ...
    megid_calc(data, cfg.ccm.megid.data);
    ...
    if (data->tst_start) {
        ...
        (void)vtss_oam_voe_counter_clear(NULL, data->voe_idx, VTSS_OAM_CNT_TST);
...
static u32 run_sync_config(u32 instance,   const vtss_mep_supp_conf_t  *const conf)
{
    ...
    if (conf->voe) {   /* VOE based MEP */
        ...
        if (conf->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) { /* This VOE relates to a EVC - Service VOE is required */
...
static u8 level_value_mask_calc(u32 vid, u32 level, u32 *mask)
{
    ...
    for (i=0; i<VTSS_MEP_SUPP_CREATED_MAX; i++) /* Find lower level on this vid */
        if (instance_data[i].config.enable && (instance_data[i].config.mode == VTSS_MEP_SUPP_MEP) && (instance_data[i].config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) &&
            (instance_data[i].config.flow.vid == vid) && (instance_data[i].config.level < level) && (instance_data[i].config.level >= low_level))
            low_level = instance_data[i].config.level+1;
...
static BOOL voe_down_mep_present(u32 vid,  u32 *voe_idx)
{
    ...
    for (i=0; i<VTSS_OAM_PATH_SERVICE_VOE_CNT; ++i)
        if ((voe_to_mep[i] < VTSS_MEP_SUPP_CREATED_MAX) && instance_data[voe_to_mep[i]].config.enable &&
            (instance_data[voe_to_mep[i]].config.flow.vid == vid) && (instance_data[voe_to_mep[i]].config.direction == VTSS_MEP_SUPP_DOWN)) {
            *voe_idx = i;
            return(TRUE);
        }
...
static void mce_rule_config(u32 instance)
{
    ...
    nni = 0;
    if (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) { /* EVC MEP/MIP - the NNI port nummer is calculated based on direction (up/down) and the active port */
        if (data->config.direction == VTSS_MEP_SUPP_UP) {   /* This is an Up-MEP/MIP */
            for (nni=0; nni<VTSS_PORT_ARRAY_SIZE; ++nni)  if (data->config.flow.port[nni])    break;
            if (nni >= VTSS_PORT_ARRAY_SIZE)    return;
            nni = vtss_mep_supp_get_active_port(nni);
        }
        else   /* This is an Down-MEP/MIP */
            nni = vtss_mep_supp_get_active_port(data->config.port);
    }
    ...
    if (data->config.enable && !data->config.voe && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID)) { /* EVC SW MEP/MIP NOT based on VOE but MCE only */
        if (data->config.mode == VTSS_MEP_SUPP_MEP) {   /* This is a SW MEP */
            if (data->config.direction == VTSS_MEP_SUPP_UP) {   /* This is an SW Up-MEP */
                ...
                mce.key.port_no = data->config.port;
                ...
                mce.key.mel.value = level_value_mask_calc(data->config.flow.vid, data->config.level, &mask);
                mce.key.mel.mask = mask;
                mce.key.injected = VTSS_VCAP_BIT_1;
    
                mce.action.port_no = VTSS_PORT_NO_NONE;     /* Egress on NNI we are going to hit the Service ES0 */
                ...
                if (voe_down_mep_present(data->config.flow.vid, &voe_idx))  /* This MCE should only point to a VOE if an VOE based Down-MEP exist */
                    mce.action.voe_idx = voe_idx;
                ...
                if (vtss_mce_port_info_get(NULL, mce.id, mce.key.port_no, &mce_info) != VTSS_RC_OK)  return; /* Get the transmit ISDX */
            ...
            mce.action.port_no = VTSS_PORT_NO_NONE;
            mce.action.voe_idx = VTSS_OAM_VOE_IDX_NONE;
            ...
            mce_idx_used[mce.id] = TRUE;
            if (vtss_mce_port_info_get(NULL, mce.id, mce.key.port_no, &mce_info) != VTSS_RC_OK)  return; /* Get the receive ISDX */
    ...
    if (data->config.enable && !(data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID)) { /* Port MEP is enabled  */
        (void)vtss_mce_init(NULL, &mce);            /******************************* Create extraction MCE rule ***************************/
...
//        mce.action.prio = 7;
    ...
    if (data->config.enable && data->config.voe && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID)) { /* EVC MEP is enabled and is VOE based */
        ...
        if (data->config.direction == VTSS_MEP_SUPP_UP) {   /* This MEP is an VOE Up-MEP */
            ...
            (void)vtss_evc_oam_port_conf_set(NULL, data->config.flow.evc_id, nni, &evc_conf);                
            ...
            mce.action.port_no = data->config.port;
            mce.action.voe_idx = data->voe_idx;
            mce.action.pop_cnt = 0;
            ...
            mce.key.port_no = VTSS_PORT_NO_CPU;
    ...
    if (data->config.enable && (data->config.mode == VTSS_MEP_SUPP_MEP) && (data->config.level != 7) && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (instance == mep_on_highest_level(data->config.flow.vid))) { /* EVC MEP is enabled and this is highes level */
        ...
        mce.key.inner_tag.s_tagged = VTSS_VCAP_BIT_0;
        mce.key.inner_tag.dei = VTSS_VCAP_BIT_0;
        mce.key.mel.value = level_value_mask_calc(data->config.flow.vid, 7, &mask);
        mce.key.mel.mask = mask;
        mce.key.injected = VTSS_VCAP_BIT_0;
...
//        mce_idx_used[mce.id] = TRUE;
    ...
    if ((!data->config.enable || !data->config.voe) && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (data->voe_idx < VTSS_OAM_VOE_CNT)) { /* EVC MEP is disabled or no longer VOE based, with a VOE reference */
        ...
        if (evc_conf.voe_idx == data->voe_idx) {    /* The EVC is hooked to this VOE */
            ...
            (void)vtss_evc_oam_port_conf_set(NULL, data->config.flow.evc_id, data->config.port, &evc_conf);  /* Un-Hook EVC to VOE */

            if (data->config.direction == VTSS_MEP_SUPP_UP) {   /* If this is an Up-MEP then the NNI might allso be un-hooked */
                (void)vtss_evc_oam_port_conf_get(NULL, data->config.flow.evc_id, nni, &evc_conf);
                if (evc_conf.voe_idx == data->voe_idx) {    /* The EVC is hooked to this VOE */
                    evc_conf.voe_idx = VTSS_OAM_VOE_IDX_NONE;
                    (void)vtss_evc_oam_port_conf_set(NULL, data->config.flow.evc_id, nni, &evc_conf);
                }
...
static void run_async_config(u32 instance)
{
    ...
    out_of_service = up_mep = FALSE;
    memset(nni, FALSE, sizeof(nni));
    for (i=0; i<VTSS_OAM_PATH_SERVICE_VOE_CNT; ++i) {   /* Search for any VOE based UP-MEP */
        if ((voe_to_mep[i] < VTSS_MEP_SUPP_CREATED_MAX) && (instance_data[voe_to_mep[i]].config.direction == VTSS_MEP_SUPP_UP)) {
            up_mep = TRUE;                          /* Register an UP-MEP was found */
            for (j=0; j<VTSS_PORT_ARRAY_SIZE; ++j)  /* Register any NNI with related UP-MEP */
                nni[j] = nni[j] || instance_data[voe_to_mep[i]].config.flow.port[j];
            if ((instance_data[voe_to_mep[i]].config.port == data->config.port) && (instance_data[voe_to_mep[i]].config.out_of_service))
                out_of_service = TRUE;              /* Register an UP-MEP is out of service on this UNI */
        }
    }

    vtss_mep_supp_port_loop(instance, data->config.port, out_of_service);  /* UNI port loop must be active if VOE UP-MEP out of service was found */

    ifh_conf.ena_inj_header = up_mep;
    ifh_conf.ena_xtr_header = FALSE;
    (void)vtss_port_ifh_conf_set(NULL, voe_up_mep_loop_port, &ifh_conf);   /* VOE UP-MEP loop port must have injection header capability enabled, if any UP-MEP was found */

    for (i=0; i<VTSS_PORT_ARRAY_SIZE; ++i) { /* All NNI with UP-MEP must have double tag key in second IS1 lookup */
        vcap_conf.key_type_is1_1 = (nni[i]) ? VTSS_VCAP_KEY_TYPE_DOUBLE_TAG : VTSS_VCAP_KEY_TYPE_NORMAL;
        (void)vtss_vcap_port_conf_set(NULL, i, &vcap_conf);
    }
    ...
    {   /* Disable of this MEP  */
        if (data->config.voe && (data->config.direction == VTSS_MEP_SUPP_UP))
            vtss_mep_supp_port_loop(instance, data->config.port, FALSE);     /* VOE UP-MEP out of service port loop must be cleared */
...
static u32 run_sync_ccm_hw_gen(u32 instance)
{
    ...
    {
        if (!(data->tx_ccm_hw = vtss_mep_supp_packet_tx_alloc((evc_up(data) ? VTSS_PACKET_HDR_SIZE_BYTES : 0) + HEADER_MAX_SIZE + CCM_PDU_LENGTH)))
            return(VTSS_MEP_SUPP_RC_NO_RESOURCES);
...
static void ccm_pdu_init(supp_instance_data_t  *data, u8 *pdu)
{
    ...
    megid_calc(data, &pdu[10]);
    if (data->ccm_config.format == VTSS_MEP_SUPP_ITU_ICC)
    {
        memcpy(data->exp_meg, &pdu[10], 16);
        data->exp_meg_len = 16;
    }
    else
    {
        n_len = strlen(data->ccm_config.name);
        m_len = strlen(data->ccm_config.meg);
        memcpy(data->exp_meg, &pdu[10], 2+n_len+2+m_len);
        data->exp_meg_len = 2+n_len+2+m_len;
    }
    idx = 10+data->exp_meg_len;
    memset(&pdu[idx], 0, (CCM_PDU_LENGTH-idx));
}
...
static void run_async_ccm_gen(u32 instance)
{
    ...
    supp_instance_data_t            *data;
    u8                              *packet;
    u32                             i, sw_ccm_period, header_size;
    ...
    if (data->ccm_gen.enable) {  /* CC or LM is enabled */
        if (hw_ccm_calc(data->ccm_gen.cc_period)) { /* HW CCM generation */
            ...
            {   /* HW CCM transmitting is on-going. This must be stopped before starting the new one */
                if (!(packet = vtss_mep_supp_packet_tx_alloc((evc_up(data) ? VTSS_PACKET_HDR_SIZE_BYTES : 0) + HEADER_MAX_SIZE + CCM_PDU_LENGTH)))    {vtss_mep_supp_trace("run_async_ccm_gen: packet tx alloc", 0, 0, 0, 0);   return;}
                memcpy(packet, data->tx_ccm_hw, ((evc_up(data) ? VTSS_PACKET_HDR_SIZE_BYTES : 0) + HEADER_MAX_SIZE + CCM_PDU_LENGTH));
                vtss_mep_supp_packet_tx_cancel(instance, data->tx_ccm_hw);
                data->tx_ccm_hw = packet;
            }

            header_size = init_tx_frame(instance, data->ccm_config.dmac, data->config.mac, data->ccm_config.prio, data->ccm_config.dei, data->tx_ccm_hw, evc_up(data));
            ...
            if (evc_up(data)) { /* EVC VOE Up-MEP must send hw generated frames to loop port */
...
static void run_dmm_config(u32 instance)
{
    ...
    if (data->dmm_config.enable)
    {
        ...
        if (data->dmm_config.txway == VTSS_MEP_SUPP_PROPRIETARY) {
        ...
        data->tx_dmm_timer = (data->dmm_config.gap * 10) / timer_res;
...
static void run_1dm_config(u32 instance)
{
    ...
    if (data->dm1_config.enable)
    {
        ...
        if (data->dm1_config.txway == VTSS_MEP_SUPP_PROPRIETARY)
        {
        ...
        data->tx_dm1_timer = (data->dm1_config.gap * 10) / timer_res;
        vtss_mep_supp_timer_start();
    }
    else {
...
static void run_aps_txdata(u32 instance)
{
    ...
    if (data->aps_config.enable)
    {
        if (!data->aps_event)   memcpy(&data->tx_aps[data->tx_header_size + 4], data->aps_txdata, APS_DATA_LENGTH);
        else                    data->tx_aps[data->tx_header_size + 4] = 0xE0;     /* Transmission of an ERPS event */
        data->aps_count = 2;
        instance_data[instance].event_flags |= EVENT_IN_TX_APS_TIMER;       /* Simulate run out of TX APS timer */
...
static void run_aps_forward(u32 instance)
{
    ...
    if (data->aps_config.enable)
    {
        ...
        (void)vtss_oam_voe_conf_get(NULL, data->voe_idx, &cfg);
        cfg.generic[3].forward = data->aps_forward;
        (void)vtss_oam_voe_conf_set(NULL, data->voe_idx, &cfg);
    }
}
#endif
...
static void run_ltm_config(u32 instance)
{
    ...
    if (data->ltm_config.enable)
    {
        ...
        {/* LTM is ready and a tx buffer has been allocated */
            ...
            tx_frame_info.bypass = !evc_up(data);
            tx_frame_info.maskerade_port = (evc_up(data)) ? data->config.port : VTSS_PORT_NO_NONE;
            tx_frame_info.isdx = data->tx_isdx;
...
static void run_lbm_config(u32 instance)
{
    u32                             rc, i, frame_rate, header_size;
    supp_instance_data_t            *data;
    u8                              *pdu;
    ...
    {
        inj_hdr = ((data->lbm_config.to_send == VTSS_MEP_SUPP_LBM_TO_SEND_INFINITE) && evc_up(data)) ? TRUE : FALSE;

        if ((data->tx_lbm == NULL) && (data->tx_lbm = vtss_mep_supp_packet_tx_alloc((inj_hdr ? VTSS_PACKET_HDR_SIZE_BYTES : 0) + HEADER_MAX_SIZE + LBM_PDU_LENGTH + data->lbm_config.size)))
        {/* LBM is ready and a tx buffer has been allocated */
            ...
            pdu[8] = 3;
            pdu[9] = (data->lbm_config.size & 0xFF00)>>8;
            pdu[10] = (data->lbm_config.size & 0xFF);
            for (i=0; i<data->lbm_config.size; ++i)     pdu[11+i] = 0xAA;
            pdu[11+i] = 0;
    
            tx_frame_info.bypass = (frame_rate || !evc_up(data)) ? TRUE : FALSE;
            tx_frame_info.maskerade_port = (!frame_rate && evc_up(data)) ? data->config.port : VTSS_PORT_NO_NONE;
            tx_frame_info.isdx = data->tx_isdx;
            tx_frame_info.qos = data->lbm_config.prio;
            tx_frame_info.pcp = data->lbm_config.prio;
            tx_frame_info.dp = data->lbm_config.dei;
            if (frame_rate && evc_up(data)) { /* EVC Up-MEP must send hw generated frames to loop port */
                memset(hw_port, FALSE, sizeof(hw_port));
                hw_port[voe_up_mep_loop_port] = TRUE;
            } else
                memcpy(hw_port, data->config.flow.port, sizeof(hw_port));
            ...
            if (data->lbm_config.to_send != VTSS_MEP_SUPP_LBM_TO_SEND_INFINITE) {
                rc = vtss_mep_supp_packet_tx(instance, &data->done_lbm, hw_port, (header_size + LBM_PDU_LENGTH + data->lbm_config.size), data->tx_lbm, &tx_frame_info, frame_rate, FALSE, 0, ((data->config.voe) ? VTSS_MEP_SUPP_OAM_TYPE_LBM : VTSS_MEP_SUPP_OAM_TYPE_NONE));
                if (rc)     data->tx_lbm_ongoing = TRUE;
            } else  /* Do not give CB pointer in case of AFI */
                rc = vtss_mep_supp_packet_tx(instance, NULL, hw_port, (header_size + LBM_PDU_LENGTH + data->lbm_config.size), data->tx_lbm, &tx_frame_info, frame_rate, FALSE, 0, ((data->config.voe) ? VTSS_MEP_SUPP_OAM_TYPE_LBM : VTSS_MEP_SUPP_OAM_TYPE_NONE));
...
static void run_tst_config (u32 instance)
{
    ...
    if (data->tst_config.enable)
    {
        ...
        if (payload_size > TST_PDU_LENGTH)   /* Check if pattern can be added to frame */
            pattern_size = payload_size - TST_PDU_LENGTH;

        if ((data->tx_tst == NULL) && (data->tx_tst = vtss_mep_supp_packet_tx_alloc(((evc_up(data) ? VTSS_PACKET_HDR_SIZE_BYTES : 0) + HEADER_MAX_SIZE + TST_PDU_LENGTH + pattern_size))))
        {/* TST is ready and a tx buffer has been allocated */
            ...
            header_size = init_tx_frame(instance, data->tst_config.dmac, data->config.mac, data->tst_config.prio, data->tst_config.dei, data->tx_tst, evc_up(data));
            ...
            frame_size = (data->tx_header_size + data->tst_config.add_cnt + TST_PDU_LENGTH + pattern_size + 4 + 8 + 12); /* Add CRC + Preamble + Inter fame gap */
#ifdef VTSS_ARCH_SERVAL
            if (data->config.voe && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (data->config.direction == VTSS_MEP_SUPP_DOWN)) /* This is VOE based EVC DOWN-MEP - Outer tag is added by ES0 */
                frame_size += 4;
#endif
            frame_rate = (data->tst_config.rate * 1000000) / (frame_size * 8);
            ...
            if (evc_up(data)) { /* EVC Up-MEP must send hw generated frames to loop port */
                memset(hw_port, FALSE, sizeof(hw_port));
                hw_port[voe_up_mep_loop_port] = TRUE;
            } else
                memcpy(hw_port, data->config.flow.port, sizeof(hw_port));
...
{
    supp_instance_data_t  *data;
    BOOL                  new, ccm_active, ais_lck_active;
    u32                   i;
    ...
    new = ccm_active = ais_lck_active = FALSE;
...
static void run_tx_aps_timer(u32 instance)
{
    ...
    {/* R-APS PDU must only be transmitted when 'active' is enabled from ERPS */
        ...
        rc = vtss_mep_supp_packet_tx(instance, &data->done_aps, data->config.flow.port, (data->tx_header_size + APS_PDU_LENGTH), data->tx_aps, &tx_frame_info, VTSS_MEP_SUPP_PERIOD_INV, FALSE, 0, ((data->config.voe) ? VTSS_MEP_SUPP_OAM_TYPE_GENERIC : VTSS_MEP_SUPP_OAM_TYPE_NONE));
...
static void run_tx_lbm_timer(u32 instance)
{
    ...
    if (data->lbm_count)
    {
        ...
        rc = vtss_mep_supp_packet_tx(instance, &data->done_lbm, data->config.flow.port, (data->tx_header_size + LBM_PDU_LENGTH + data->lbm_config.size), data->tx_lbm, &tx_frame_info, VTSS_MEP_SUPP_PERIOD_INV, FALSE, 0, ((data->config.voe) ? VTSS_MEP_SUPP_OAM_TYPE_LBM : VTSS_MEP_SUPP_OAM_TYPE_NONE));
...
static void run_tx_dmm_timer(u32 instance)
{
    u32                         rc;
    supp_instance_data_t        *data;
    vtss_mep_supp_timestamp_t   timestamp;
    u32                         tc=0;
    vtss_mep_supp_tx_frame_info_t   tx_frame_info;
...
static void run_tx_ais_timer(u32 instance)
{
    ...
    header_size = init_client_tx_frame(instance, data->ais_config.dmac, data->config.mac, data->ais_config.prio, data->ais_config.dei, data->tx_ais);
    data->tx_ais[header_size] = data->ais_config.level << 5;
    ...
    tx_frame_info.qos = data->ais_config.prio;
    tx_frame_info.pcp = data->ais_config.prio;
    ...
    data->ais_inx++;
    if (data->ais_inx >= data->ais_config.flow_count)
    {
        data->ais_inx = 0;
        if (data->ais_count < 3)    data->ais_count++;
    }
}
...
static void run_tx_lck_timer(u32 instance)
{
    ...
    header_size = init_client_tx_frame(instance, data->lck_config.dmac, data->config.mac, data->lck_config.prio, data->lck_config.dei, data->tx_lck);
    data->tx_lck[header_size] = data->lck_config.level << 5;    /* If this is source direction then it is on instance level */
    ...
    tx_frame_info.bypass = TRUE;
    tx_frame_info.maskerade_port = VTSS_PORT_NO_NONE;
    tx_frame_info.isdx = source_dir ? data->tx_isdx : 0;    /* The ISDX is only set in case of LCK in source direction */
    tx_frame_info.qos = data->lck_config.prio;
    tx_frame_info.pcp = data->lck_config.prio;
    tx_frame_info.dp = data->lck_config.dei;

    rc = vtss_mep_supp_packet_tx(instance, &data->done_lck, source_dir ? data->config.flow.port : data->lck_config.flows[data->lck_inx].port, (header_size + LCK_PDU_LENGTH), data->tx_lck, &tx_frame_info, VTSS_MEP_SUPP_PERIOD_INV, FALSE, 0, ((data->config.voe) ? VTSS_MEP_SUPP_OAM_TYPE_GENERIC : VTSS_MEP_SUPP_OAM_TYPE_NONE));

    if (!rc)       vtss_mep_supp_trace("run_tx_lck_timer: packet tx failed", instance, 0, 0, 0);
    if (rc)     data->tx_lck_ongoing = TRUE;

    data->lck_inx++;
    if (data->lck_inx > data->lck_config.flow_count)
    {
        data->lck_inx = 0;
        if (data->lck_count < 1)    data->lck_count++;
    }
}
...
{                                                                                                           \
    if (!(data->config.voe && (data->config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (data->config.direction == VTSS_MEP_SUPP_DOWN))) \
...
static void ltr_to_peer(u32 instance,   supp_instance_data_t *data,   u8 ltm[],   vtss_mep_supp_frame_info_t *frame_info,   u32 port,   BOOL forward,   BOOL mep)
{
    int                             rc;
    u8                              *tx_pdu;
    u32                             header_size;
...
static void dmr_to_peer(u32 instance,   supp_instance_data_t *data,   u8 smac[],   u8 dmm[],   vtss_mep_supp_frame_info_t *frame_info,   u32 port)
{
    ...
    i = DMM_PDU_LENGTH;
    if (!data->config.voe && (i == DMM_PDU_VTSS_LENGTH) && (memcmp(&dmm[DMM_PDU_STD_LENGTH], "VTSS", 4) == 0)) {
...
void vtss_mep_supp_rx_frame(u32 port,  vtss_mep_supp_frame_info_t *frame_info,  u8 frame[],  u32 len, vtss_mep_supp_timestamp_t *rx_time, u32 rx_count)
{
/* This is called by API when a OAM frame is received */
    ...
    vtss_mep_supp_timestamp_t  rxTimef, txTimeb;
...
//printf("vtss_mep_supp_rx_frame  port %lu  vid %lu  isdx %lu  tl_idx %lu  level %lu  pdu[1] %X  sec %u sec %lu nano %lu\n", port, frame_info->vid, frame_info->isdx, tl_idx, level, pdu[1], rx_time->sec_msb, rx_time->seconds, rx_time->nanoseconds);
    ...
    switch (pdu[1])
    {
        case OAM_TYPE_CCM:
            ...
            lm_counter_calc(&(instance_data[mep_lm].ccm_lm_counter),   near_CT2,   far_CT2,   near_CR2,   far_CR2);   /* Calculate LM counters */
            break;
        ...
        case OAM_TYPE_LMR:
            ...
            lm_counter_calc(&data->lmm_lm_counter,   near_CT2,   far_CT2,   near_CR2,   far_CR2);   /* Calculate LM counters */
            break;
            ...
            {   /* One or more MEP's got this expected frame header */
                ...
                {/* Ingress MEP on this level,port was found */
                    ...
                    if (!memcmp(&pdu[15], instance_data[meps[i]].config.mac, VTSS_MEP_SUPP_MAC_LENGTH)) {/* Reply if this is the target MAC */
                        ltr_to_peer(meps[i], &instance_data[meps[i]], pdu, frame_info, reply_port_calc(&instance_data[meps[i]], port), FALSE, TRUE);
                        break;
                    }
            ...
            {   /* One or more MIP's got this expected frame header */
                for (i=0; i<mips_idx; ++i) /* Check for MIP with target MAC */
                    if ((instance_data[mipss[i]].config.level == level) && !memcmp(&pdu[15], instance_data[mipss[i]].config.mac, VTSS_MEP_SUPP_MAC_LENGTH)) {/* Reply if this is the target MAC */
                        ...
                        ltr_to_peer(mipss[i], &instance_data[mipss[i]], pdu, frame_info, port, FALSE, FALSE);
                        break;
                    }
                ...
                {/* Forwarding is possible - means that we will reply but not necessarily forward (depending on TTL) */
                    ...
                    {/* Egress MIP was found on forwarding port */
                        instance_data[mipss[i]].is2_rx_cnt++;
                        ltr_to_peer(mipss[i], &instance_data[mipss[i]], pdu, frame_info, port, (pdu[8] > 1), FALSE);
        ...
        case OAM_TYPE_DMR:
            ...
            if (data->dmm_config.txway == VTSS_MEP_SUPP_STANDARDIZE && data->dmm_config.calcway == VTSS_MEP_SUPP_FLOW) {
...
#if F_2DM_FOR_1DM
            if (data->dmm_config.dm2fordm1 == TRUE) {            
                if (data->dmm_config.txway == VTSS_MEP_SUPP_PROPRIETARY && data->dmm_config.calcway == VTSS_MEP_SUPP_FLOW) {
...
//    if (!memcmp(&instance_data[instance].config, conf, sizeof(vtss_mep_supp_conf_t)))                      					                  return(VTSS_MEP_SUPP_RC_OK);
    if (instance_data[instance].config.enable && conf->enable &&
        ((instance_data[instance].config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) != (conf->flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID)))  return(VTSS_MEP_SUPP_RC_INVALID_PARAMETER);
...
#ifdef VTSS_ARCH_SERVAL
        u32 i;
        if ((instance_data[instance].config.flow.mask & VTSS_MEP_SUPP_FLOW_MASK_EVC_ID) && (instance_data[instance].config.mode == VTSS_MEP_SUPP_MEP)) { /* This is a EVC MEP - all MEP in this EVC must be re-configured to assure correct TCAM rules and VOE configuration */
                               ...
                               const vtss_mep_supp_ccm_conf_t     *const conf)
{
    ...
    if (conf->peer_count >= instance_data[instance].ccm_config.peer_count) {
        if (memcmp(conf->peer_mep, instance_data[instance].ccm_config.peer_mep, instance_data[instance].ccm_config.peer_count*sizeof(u16)))
            memset(instance_data[instance].peer_mac, 0, (VTSS_MEP_SUPP_PEER_MAX*VTSS_MEP_SUPP_MAC_LENGTH));
    }
    else
        if (memcmp(conf->peer_mep, instance_data[instance].ccm_config.peer_mep, conf->peer_count*sizeof(u16)))
            memset(instance_data[instance].peer_mac, 0, (VTSS_MEP_SUPP_PEER_MAX*VTSS_MEP_SUPP_MAC_LENGTH));
                               ...
                               const vtss_mep_supp_dm1_conf_t   *const conf)
{
    ...
    if (conf->enable && instance_data[instance].config.voe && (conf->txway == VTSS_MEP_SUPP_PROPRIETARY))  return(VTSS_MEP_SUPP_RC_INVALID_PARAMETER);
                                ...
                                vtss_mep_mgmt_dm_timestamp_t *const dm1_timestamp_near_to_far)
{
    ...
    vtss_mep_supp_crit_lock();
    if (instance_data[instance].dmm_config.dm2fordm1 == TRUE && instance_data[instance].dmr_ts_ok == TRUE &&
                          ...
                          vtss_mep_supp_dmr_status *const  status)
{
    ...
    if (instance_data[instance].dmm_config.dm2fordm1 == TRUE) {            
...
u32 vtss_mep_supp_init(const u32 timer_resolution)
{
    u32  i;
    ...
    cfg.pdu_type.generic[0].opcode = VTSS_OAM_OPCODE_AIS;
    cfg.pdu_type.generic[0].check_dmac = TRUE;
    cfg.pdu_type.generic[0].extract.to_front = FALSE;
    cfg.pdu_type.generic[0].extract.rx_queue = rx_queue;

    cfg.pdu_type.generic[1].opcode = VTSS_OAM_OPCODE_LCK;
    cfg.pdu_type.generic[1].check_dmac = TRUE;
    cfg.pdu_type.generic[1].extract.to_front = FALSE;
    cfg.pdu_type.generic[1].extract.rx_queue = rx_queue;

    cfg.pdu_type.generic[2].opcode = VTSS_OAM_OPCODE_LINEAR_APS;
    cfg.pdu_type.generic[2].check_dmac = TRUE;
    cfg.pdu_type.generic[2].extract.to_front = FALSE;
    cfg.pdu_type.generic[2].extract.rx_queue = rx_queue;

    cfg.pdu_type.generic[3].opcode = VTSS_OAM_OPCODE_RING_APS;
    cfg.pdu_type.generic[3].check_dmac = FALSE;
    cfg.pdu_type.generic[3].extract.to_front = FALSE;
    cfg.pdu_type.generic[3].extract.rx_queue = rx_queue;
    ...
    cfg.pdu_type.lbr.rx_queue = rx_queue;
