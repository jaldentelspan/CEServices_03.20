...
cyg_httpd_form_variable_mac(CYG_HTTPD_STATE* p, const char *name, vtss_mac_t *mac)
{
    ...
    uint i, mac_addr[6];
    if((value = cyg_httpd_form_varable_string(p, name, &len)) && len > 0 &&
       ((sscanf(value, "%2x-%2x-%2x-%2x-%2x-%2x", &mac_addr[0], &mac_addr[1], &mac_addr[2], &mac_addr[3], &mac_addr[4], &mac_addr[5]) == 6) ||
        (sscanf(value, "%2x.%2x.%2x.%2x.%2x.%2x", &mac_addr[0], &mac_addr[1], &mac_addr[2], &mac_addr[3], &mac_addr[4], &mac_addr[5]) == 6))) {
        for (i= 0 ; i < sizeof(mac->addr); i++)
            mac->addr[i] = (uchar) mac_addr[i];
        return TRUE;
    } else if (value && strlen(value) == 12) {
        char mac_string[3];
        for (i = 0; i < len; i++) {
            if (!cyg_httpd_is_hex(value[i])) {
                return FALSE;
            } else if (i % 2) {
                mac_string[1] = value[i];
                mac_string[2] = '\0';
                mac->addr[i / 2] = atoi(mac_string);
            } else {
                mac_string[0] = value[i];
            }
        }
        return TRUE;
    }
...
cyg_int32 __handler_lib_config_js(CYG_HTTPD_STATE* p)
{
    ...
    if (file_buffer == NULL) { // First time or cache disabled
        ...
        char *file_offset;
        file_buffer = malloc(file_alloc_size);
...
size_t webCommonBufferHandler(char **base_ptr, char **cur_ptr, size_t *length, const char *buff)
{
    ...
    if (ct >= avail) { 
        ...
        char *new_ptr = realloc(*base_ptr, new_len);
        VTSS_ASSERT(new_ptr != NULL); // realloc failed. 
        T_D("Realloc (avail %zu, need %zu, bp %p, np %p, oz %zu, nz %zu", avail, ct, *base_ptr, new_ptr, *length, new_len);
