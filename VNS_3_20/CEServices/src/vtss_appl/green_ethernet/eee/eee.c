...
#include "port_power_savings_icli_functions.h"  /* For eee_icfg_init() */
#endif
...
static void EEE_msg_tx_switch_state_get(vtss_isid_t isid)
{
  ...
  msg = malloc(sizeof(eee_msg_switch_state_get_t));
  if (msg == NULL) {
...
static void EEE_msg_tx_switch_state_set(void)
{
  ...
  msg = malloc(sizeof(eee_msg_switch_state_set_t));
  if (msg == NULL) {
    T_E("Allocation failed.\n");
    return;
  }
...
static void EEE_msg_tx_port_state_set(vtss_port_no_t port)
{
  ...
  msg = malloc(sizeof(eee_msg_port_state_set_t));
  if (msg == NULL) {
    T_E("Allocation failed.\n");
    return;
  }
...
static void EEE_msg_tx_switch_conf(vtss_isid_t isid)
{
  ...
  msg = malloc(sizeof(eee_msg_switch_conf_t));
  if (msg == NULL) {
    T_E("Allocation failed.\n");
    return;
  }
...
// EEE_check_isid_port()
/******************************************************************************/
vtss_rc EEE_check_isid_port(vtss_isid_t isid, vtss_port_no_t port, BOOL allow_local)
{
...
static void  EEE_read_from_flash(vtss_isid_t isid, BOOL create)
{
  ...
  if (!do_create) {
    ...
    for (isid_iter = VTSS_ISID_START; isid_iter < VTSS_ISID_END; isid_iter++) {
      if (!EEE_platform_conf_valid(&flash_conf->stack_conf.switch_conf[isid_iter - VTSS_ISID_START])) {
        do_create = TRUE;
...
{
  vtss_rc result;
  if ((result = EEE_check_isid_port(isid, VTSS_PORT_NO_START, TRUE)) != VTSS_RC_OK) {
    return result;
  }
...
vtss_rc eee_mgmt_switch_conf_set(vtss_isid_t isid, eee_switch_conf_t *switch_conf)
{
  ...
  while (port_iter_getnext(&pit)) {
    if (switch_conf->port[pit.iport].eee_ena && !EEE_stack_state[isid - VTSS_ISID_START].port[pit.iport].eee_capable) {
      T_E("Attempting to enable EEE on non-EEE-capable port (%u)", pit.uport);
      result = EEE_ERROR_VALUE;
      goto do_exit;
    }
  }
...
/******************************************************************************/
#define MSG_TIMEOUT cyg_current_time() + VTSS_OS_MSEC2TICK(20 * 1000)         // Wait for timeout (20 seconds) or synch. flag to be set 
vtss_rc eee_mgmt_switch_state_get(vtss_isid_t isid, eee_switch_state_t *switch_state)
{
  ...
  if (cyg_flag_timed_wait(&eee_msg_id_get_stat_req_flags, flag, CYG_FLAG_WAITMODE_OR, MSG_TIMEOUT) & flag ? 0 : 1) {
    T_W("EEE msg status timeout - Status values not valid");
  }
...
{
  vtss_rc result;
  if ((result = EEE_check_isid_port(isid, port, TRUE)) != VTSS_RC_OK) {
    return result;
  }
