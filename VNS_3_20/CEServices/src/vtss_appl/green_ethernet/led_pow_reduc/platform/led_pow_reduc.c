...
static void  led_pow_reduc_conf_read(BOOL create)
{
    ...
    if (do_create) {
        ...
        memset(led_pow_reduc_flash_conf, 0, sizeof(*led_pow_reduc_flash_conf)); // Set everything to 0. Non-zero default values will be set below.
        led_pow_reduc_flash_conf->stack_conf.glbl_conf.maintenance_time = LED_POW_REDUC_MAINTENANCE_TIME_DEFAULT;

        int timer_index;
        for (timer_index = LED_POW_REDUC_TIMERS_MIN; timer_index <= LED_POW_REDUC_TIMERS_MAX; timer_index++) {
            led_pow_reduc_flash_conf->stack_conf.glbl_conf.led_timer_intensity[timer_index] = LED_POW_REDUC_INTENSITY_DEFAULT;
        }
