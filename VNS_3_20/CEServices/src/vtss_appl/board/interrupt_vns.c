...
void interrupt_source_enable(vtss_interrupt_source_t     source_id)
{
    ...
    if (board_type == VTSS_BOARD_VNS_6_REF)   cu_port = 6;
    if (board_type == VTSS_BOARD_VNS_8_REF)   cu_port = 8;
    if (board_type == VTSS_BOARD_VNS_12_REF)   cu_port = 12;
    if (board_type == VTSS_BOARD_VNS_16_REF)   cu_port = 16;
    if (board_type == VTSS_BOARD_VESM_REF)   cu_port = 24;
    ...
    if (flags & INTERRUPT_SGPIO)
    {
        ...
        for (i=24; i<26; i++)
            if (sgpio_events[i])
            {
                ...
                /* interrupt_signal_source(INTERRUPT_SOURCE_LOS, i-24+( (board_type == VTSS_BOARD_VNS_6_REF) ? 6 : 12)); //VNS-6 ////i-24+((board_type == VTSS_BOARD_LUTON10_REF) ? 8 : 24)); */
                interrupt_signal_source(INTERRUPT_SOURCE_LOS, i-24+(VTSS_OPT_PORT_COUNT - 2)); // Might need to modify BOARD_VNS_ALL_REF ((BOARD_VNS_6_REF) || (BOARD_VNS_8_REF) || (BOARD_VNS_12_REF))
                *pending = TRUE;
            }
    }
    if (flags & INTERRUPT_DEV_ALL)
    {
        /* for (i=0; i < ((board_type == VTSS_BOARD_VNS_6_REF) ? 6 : 12); i++)//VNS-6 ////for (i=0; i<((board_type == VTSS_BOARD_LUTON10_REF) ? 8 : 12); i++) */
