...
void interrupt_source_enable(vtss_interrupt_source_t     source_id)
{
    ...
    if (board_type == VTSS_BOARD_NCGU_REF)			cu_port = 3;
    if (board_type == VTSS_BOARD_IGU_REF)			cu_port = 3;
    ...
    if (flags & INTERRUPT_SGPIO)
    {
        ...
        for (i=24; i<26; i++)
            if (sgpio_events[i])
            {
                ...
                interrupt_signal_source(INTERRUPT_SOURCE_LOS, i-24+(((board_type == VTSS_BOARD_NCGU_REF) || (board_type == VTSS_BOARD_IGU_REF)) ? 3 : 8));
                *pending = TRUE;
            }
    }
    if (flags & INTERRUPT_DEV_ALL)
    {
        for (i=0; i<(((board_type == VTSS_BOARD_NCGU_REF) || (board_type == VTSS_BOARD_IGU_REF)) ? 3 : 8); i++)
        { /* Poll internal PHY */
            rc = vtss_phy_event_poll(PHY_INST, i, &phy_events);
            rc = vtss_phy_event_enable_set(PHY_INST, i, phy_events, FALSE);
            if (phy_events & VTSS_PHY_LINK_FFAIL_EV)   interrupt_signal_source(INTERRUPT_SOURCE_FLNK, i);
