...
static port_10g_t port_10g[PORT_10G_MAX];
...
// This function is not working anymore as 10G setup in portmodule is no more behind critd        port_phy_wait_until_ready();
        ...
        for (i=0, inx88=0; i<PORT_10G_MAX; i++) {
            if (port_10g[i].phy) {
                port = port_10g[i].port;
                while (VTSS_RC_ERROR == vtss_phy_10g_id_get(PHY_INST, port, &phy_id))      VTSS_OS_MSLEEP(10);
    ...
    if (flags & INTERRUPT_EXT1) {
        for (i=0; i<PORT_10G_MAX; i++) {   // Polling of solded 8486 and 8484+87+88 in X2 slot
            if (port_10g[i].phy) {
                port = port_10g[i].port;
                rc = vtss_phy_10g_event_poll(PHY_INST, port, &phy10g_events);
                rc = vtss_phy_10g_event_enable_set(PHY_INST, port, phy10g_events, FALSE);
                ...
                if (phy10g_events & VTSS_PHY_10G_PCS_RECEIVE_FAULT_EV)   {
                ...
                rc = vtss_phy_10g_id_get(PHY_INST, port, &phy_id);
                if ((phy_id.part_number == 0x8484) || (phy_id.part_number == 0x8487) || (phy_id.part_number == 0x8488)) {
