/*
 * tsd_hdlc_encoder_2p0.c
 *
 *  Created on: May 9, 2022
 *      Author: eric
 */



/* #include <stdint.h> */
#include "main.h"
/* #include <stdbool.h> */
#include <stdio.h>
#include <math.h>
#include "mmdrv_printf.h"
#include "tsd_hdlc_encoder_2p0.h"
#include "tsd_register_common.h"

/*
 * prints a summary of the core's setup to stdout
 */
void tsd_hdlc_enc_debug_print(uint32_t base)
{
    double fpgaVersion, driverVersion;
    mmdrv_printf("HDLC Encoder Core at 0x%X\n", (unsigned)base);
    if(0 == tsd_hdlc_enc_get_version(base, &fpgaVersion, &driverVersion))
    {
        mmdrv_printf(" - FPGA Version:  %f\n", fpgaVersion);
        mmdrv_printf(" - Driver Version:  %f\n", driverVersion);
    }
    else
    {
        mmdrv_printf(" - Version:  FAILED TO READ\n");
    }

    mmdrv_printf(" - REGISTERS:\n");
    mmdrv_printf("    => %02X: VERSION REG        = 0x%08X\n", TSD_HDLC_ENC_VERSION_REG        , TSD_HDLC_ENC_IORD_VERSION(base));
    mmdrv_printf("    => %02X: IP ID REG          = 0x%08X\n", TSD_HDLC_ENC_IP_ID_REG          , TSD_HDLC_ENC_IORD_IP_ID(base));
    mmdrv_printf("    => %02X: STATUS REG         = 0x%08X\n", TSD_HDLC_ENC_STATUS_REG         , TSD_HDLC_ENC_IORD_STATUS(base));
    mmdrv_printf("    => %02X: CONTROL REG        = 0x%08X\n", TSD_HDLC_ENC_CTRL_REG           , TSD_HDLC_ENC_IORD_CONTROL(base));
    mmdrv_printf("    => %02X: BITRATE REG        = 0x%08X\n", TSD_HDLC_ENC_BITRATE_REG        , TSD_HDLC_ENC_IORD_BITRATE(base));
    mmdrv_printf("    => %02X: PACKET COUNT REG   = 0x%08X\n", TSD_HDLC_ENC_PACKET_CNT_REG     , TSD_HDLC_ENC_IORD_PACKET_CNT(base));
    mmdrv_printf("    => %02X: FIFO0 OFLOW CNT    = 0x%08X\n", TSD_HDLC_ENC_FIFO0_OFLOW_CNT_REG, TSD_HDLC_ENC_IORD_FIFO0_OFLOW_CNT(base));
    mmdrv_printf("    => %02X: FIFO0 UFLOW CNT    = 0x%08X\n", TSD_HDLC_ENC_FIFO0_UFLOW_CNT_REG, TSD_HDLC_ENC_IORD_FIFO0_UFLOW_CNT(base));
}

/*
 * Gets the Telspan HDLC2p0 core version
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_get_version(uint32_t base, double *fpgaVersion, double *driverVersion)
{
    uint32_t version = TSD_HDLC_ENC_IORD_VERSION(base);
    uint32_t vH, vL;

    if(version == 0xDEADBEEF)
    {
        return 1;
    }
    *driverVersion = (double)TSD_HDLC_ENC_DRIVER_VERSION;
    vH = (version >> 16);
    vL = (version & 0xFFFF);
    if(vL < 100)
        *fpgaVersion = (double)vH + (double)(vL/100);
    else
        *fpgaVersion = (double)vH + (double)(vL/1000);

    return 0;
}

/*
 * Enables the HDLC mode of the Telspan HDLC Encoder core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_enable(uint32_t base) {
    return set_register_bits(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_ENABLE_BIT);
}

/*
 * Disables the Telspan HDLC Encoder core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_disable(uint32_t base) {
    return clear_register_bits(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_ENABLE_BIT);
}

/*
 * val = true to remove the tse padding from the ethernet frames prior to encoding
 * val = false to not remove tse padding (meaning TSE is sending data without)
 * returns 0 on success.
 */
int tsd_hdlc_enc_remove_tse_padding(uint32_t base, bool val) {
    if(val) {
        return set_register_bits(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_REMOVE_TSE_PADDING);
    } else {
        return clear_register_bits(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_REMOVE_TSE_PADDING);
    }
}

/*
 * Sets other control bits in the Telspan HDLC Encoder core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_set_control_bits(uint32_t base, uint32_t ctrlBits) {
    return set_register_bits(base, TSD_HDLC_ENC_CTRL_REG, ctrlBits);
}

/*
 * Clears control bits in the Telspan HDLC Encoder core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_clear_control_bits(uint32_t base, uint32_t ctrlBits) {
    return clear_register_bits(base, TSD_HDLC_ENC_CTRL_REG, ctrlBits);
}

/*
 * Sets the hdlc ctrl/addr fields if enabled in the Telspan HDLC2p0 core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_set_ctrl_addr_values(uint32_t base, uint8_t ctrl, uint8_t addr) {
	int ret = 0;
	ret += set_register_region(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_CTRL_VAL_MASK, TSD_HDLC_ENC_CTRL_CTRL_VAL_SHFT, (uint32_t)ctrl);
	ret += set_register_region(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_ADDR_VAL_MASK, TSD_HDLC_ENC_CTRL_ADDR_VAL_SHFT, (uint32_t)addr);

	return ret;
}

/*
 * Gets the hdlc ctrl/addr values if enabled in the Telspan HDLC2p0 core located at base address 'base'
 *
 * returns 0 on success.
 */
int tsd_hdlc_enc_get_ctrl_addr_values(uint32_t base, uint8_t *ctrl, uint8_t *addr) {
	int ret = 0;
	uint32_t Ctrl, Addr;
	ret += get_register_region(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_CTRL_VAL_MASK, TSD_HDLC_ENC_CTRL_CTRL_VAL_SHFT, &Ctrl);
	ret += get_register_region(base, TSD_HDLC_ENC_CTRL_REG, TSD_HDLC_ENC_CTRL_ADDR_VAL_MASK, TSD_HDLC_ENC_CTRL_ADDR_VAL_SHFT, &Addr);
	*ctrl = (uint8_t)Ctrl;
	*addr = (uint8_t)Addr;
	return ret;
}

/*
 * Sets the HDLC encoder bitrate.
 * returns 0 on success
 */
int tsd_hdlc_enc_set_bitrate(uint32_t base, int bitrate, int sysclkRate) {
    uint64_t bitrate64 = ((uint64_t)bitrate) << 32;
    // TODO: Figure out round
    /* double setval = round((double)bitrate64 / (double)sysclkRate); */
    ulonglong setval = (ulonglong)bitrate64 / (ulonglong)sysclkRate;
    TSD_HDLC_ENC_IOWR_BITRATE(base, (uint32_t)setval);
    return 0;
}

/*
 * Gets the HDLC encoder bitrate.
 * returns 0 on success
 */
int tsd_hdlc_enc_get_bitrate(uint32_t base, int *bitrate, int sysclkRate) {
    uint32_t regval = 0;
    regval = TSD_HDLC_ENC_IORD_BITRATE(base);
    *bitrate = (int) ((uint64_t)((uint64_t)regval * (uint64_t)sysclkRate ) >> 32);
    return 0;
}

/*
 * Gets the number of eth frames received by the PCM from the Telspan HDLC Encoder core
 * located at base address 'base'
 * returns 0 on success.
 */
int tsd_hdlc_enc_get_rx_eth_frame_count(uint32_t base, uint32_t *cnt) {
    *cnt = TSD_HDLC_ENC_IORD_PACKET_CNT(base) & TSD_HDLC_ENC_COUNTER_MASK;
    return 0;
}

/*
 * Gets the FIFO0 underflow Count in the Telspan HDLC Decoder core
 * located at base address 'base'
 * returns 0 on success.
 */
int tsd_hdlc_enc_get_fifo0_underflow_count(uint32_t base, uint32_t *cnt) {
    uint32_t regVal = TSD_HDLC_ENC_IORD_FIFO0_UFLOW_CNT(base);
    if(regVal != 0xDEADBEEF) {
        *cnt = regVal & TSD_HDLC_ENC_COUNTER_MASK;
        return 0;
    }
    return 1;
}
/*
 * Gets the FIFO0 underflow Count in the Telspan HDLC Decoder core
 * located at base address 'base'
 * returns 0 on success.
 */
int tsd_hdlc_enc_get_fifo0_overflow_count(uint32_t base, uint32_t *cnt) {
    uint32_t regVal = TSD_HDLC_ENC_IORD_FIFO0_OFLOW_CNT(base);
    if(regVal != 0xDEADBEEF) {
        *cnt = regVal & TSD_HDLC_ENC_COUNTER_MASK;
        return 0;
    }
    return 1;
}

/*
 * Resets all of the HDLC Counters in the Telspan HDLC Decoder core
 * located at base address 'base'
 * returns 0 on success.
 */
int tsd_hdlc_enc_reset_counters(uint32_t base) {
    TSD_HDLC_ENC_RESET_PACKET_CNT(base);
    TSD_HDLC_ENC_RESET_FIFO0_UFLOW_CNT(base);
    TSD_HDLC_ENC_RESET_FIFO0_OFLOW_CNT(base);

    return 0;
}
