...
static void cli_cmd_lldpmed_conf(cli_req_t *req, BOOL location, BOOL civic, BOOL fast_start_repeat_count,
                                 BOOL ecs, BOOL policy_delete, BOOL policy_add,   BOOL info,  BOOL show_polices,
...
//
// Prints the LLDP-MED polices configuration
//
// In : conf - Pointer to the current configuration
//
//
static void cli_cmd_lldpmed_print_polices(const  lldp_struc_0_t conf)
{
                                 ...
                                 BOOL port_policy, BOOL datum)
{
    ...
    if (req->set) {
        ...
        if (policy_delete) {
            for (policy_index = LLDPMED_POLICY_MIN ; policy_index <= LLDPMED_POLICY_MAX; policy_index++) {
                if (lldpmed_req->policies_list[policy_index]) {
                    conf.policies_table[policy_index].in_use = FALSE;
                }
            }
        }
        ...
        if (policy_add) {
            ...
            for (policy_index = LLDPMED_POLICY_MIN ; policy_index <= LLDPMED_POLICY_MAX; policy_index++) {
                T_IG(TRACE_GRP_CLI, "conf.policies_table[%d].in_use:%d", policy_index, conf.policies_table[policy_index].in_use);
                if (conf.policies_table[policy_index].in_use == FALSE) {
                    policy_added = LLDP_TRUE;
                    conf.policies_table[policy_index].in_use = TRUE;
                    conf.policies_table[policy_index].application_type = lldpmed_req->application_type;
                    conf.policies_table[policy_index].tagged_flag = lldpmed_req->tagged;
                    ...
                    if (lldpmed_req->tagged) {
                        if (req->vid == 0) {
                            ...
                            conf.policies_table[policy_index].vlan_id = req->vid;
                        }
                    } else {
                        if (req->vid != 0) {
                    ...
                    conf.policies_table[policy_index].l2_priority = lldpmed_req->l2_prio;
                    conf.policies_table[policy_index].dscp_value = lldpmed_req->dscp;
                    break;
                }
            }

            if (!policy_added) {
                CPRINTF("Maximum number of polices supported (%d) reached. New policy was not added\n", LLDPMED_POLICIES_CNT);
            } else {
                if (update_conf) {
        ...
        if (update_conf) {
            ...
            vtss_rc rc;
            if ((rc = lldp_mgmt_set_config(&conf, VTSS_ISID_LOCAL)) != VTSS_OK) {
                CPRINTF("%s\n", error_txt(rc));
            }
        }
        ...
        T_IG(TRACE_GRP_CLI, "Setting ecs:%d, show_polices:%d, policy_add:%d", ecs, show_polices, policy_add);
        for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
            if ((isid = req->stack.isid[usid]) == VTSS_ISID_END) {
                continue;
            }
            ...
            if (civic) {
                if (lldpmed_req->country) {
                    strcpy(conf.location_info.ca_country_code, req->parm);
                }
        ...
        T_IG(TRACE_GRP_CLI, "Getting ecs:%d, show_polices:%d", ecs, show_polices);
        lldp_mgmt_get_config(&conf, VTSS_ISID_LOCAL);
        ...
        if (show_polices) {
            cli_cmd_lldpmed_print_polices(conf);
        }
