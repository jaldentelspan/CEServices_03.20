                              ...
                              BOOL is_cdp_aware)
{
    ...
    if (interval || hold || delay || reinit) {
        if (req->set) {
            if (interval) {
                conf.msgTxInterval = lldp_req->lldp;
            }
            if (hold) {
                conf.msgTxHold = lldp_req->lldp;
            }
            if (delay) {
                conf.txDelay = lldp_req->lldp;
            }
            if (reinit) {
                conf.reInitDelay = lldp_req->lldp;
            }
            conf.timingChanged = 1; /* Indicate timer change */
            if (conf.txDelay * 4 > conf.msgTxInterval) {
                CPRINTF("Delay must be less than 1/4 of Interval\n");
            } else if (lldp_mgmt_set_config(&conf, VTSS_ISID_LOCAL) != VTSS_OK) {
                T_W("Problem setting configuration");
            }
        } else {
            if (interval) {
                CPRINTF("Interval    : %d\n", conf.msgTxInterval);
