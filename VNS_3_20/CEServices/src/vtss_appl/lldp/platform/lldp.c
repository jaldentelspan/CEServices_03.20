...
static lldp_bool_t master_up = FALSE;
...
static void lldp_global_conf2local_conf(lldp_struc_0_t *conf, vtss_isid_t isid)
{
    ...
    conf->medFastStartRepeatCount = lldp_global.flash.medFastStartRepeatCount;
...
{
    lldp_global.flash.common.reInitDelay = conf->reInitDelay;
    lldp_global.flash.common.msgTxHold = conf->msgTxHold;
    lldp_global.flash.common.msgTxInterval = conf->msgTxInterval;
    lldp_global.flash.common.txDelay = conf->txDelay ;
...
#ifdef VTSS_SW_OPTION_LLDP_MED
    memcpy(lldp_global.flash.common.policies_table, conf->policies_table, sizeof(lldpmed_policies_table_t));
    memcpy(lldp_global.flash.common.ports_policies, conf->ports_policies, sizeof(lldpmed_ports_policies_t));
    memcpy(&lldp_global.flash.common.location_info, &conf->location_info, sizeof(lldpmed_location_info_t));
    lldp_global.flash.medFastStartRepeatCount = conf->medFastStartRepeatCount;
    memcpy(lldp_global.flash.common.lldpmed_optional_tlv, conf->lldpmed_optional_tlv, sizeof(lldpmed_optional_tlv_t));
#endif
...
char *lldp_error_txt(vtss_rc rc)
{
    ...
    case LLDP_ERROR_VOICE_VLAN_CONFIGURATION_MISMATCH:
        return "Can not set LLDP port mode to disabled or TX only when Voice-VLAN supports LLDP discovery protocol.";

    case LLDP_ERROR_VALUE:
...
// how to handle, we need to add the frame to the MAC table manually.
// The hash entry to the MAC table consist of both the DMAC and VLAN VID.
// Due to that the VLAN VID can be changed by the user this function MUST be called
// every time the VLAN VID changes.
static void cdp_add_to_mac_table(vtss_port_no_t port_no, const vlan_port_conf_t *vlan_conf)
{
    ...
    if (port_index < LLDP_PORTS) {
        LLDP_CRIT_ENTER();
        vid_t  pvid;   // Port VLAN ID for the port that has changed configuration
        ...
        for (i = VTSS_PORT_NO_START; i < VTSS_PORT_NO_END; i++) {
            ...
            // Do not check PVID against itself.
            if (i == port_no || i >= LLDP_PORTS) {
                continue;
            }

            (void)vlan_mgmt_port_conf_get(VTSS_ISID_LOCAL, i, &conf, VLAN_USER_ALL);
            if (conf.pvid == entry[port_index].vid_mac.vid && local_conf.cdp_aware[i - VTSS_PORT_NO_START]) {
...
static lldp_bool_t lldp_msg_rx(void *contxt, const void *const rx_msg, const size_t len, const vtss_module_id_t modid, const ulong isid)
{
    ...
    T_IG(TRACE_GRP_CONF, "Entering lldp_msg_rx msg_id: %d,  len: %zd, isid: %u", msg_id, len, isid);
    ...
    case LLDP_MSG_ID_CONF_SET_REQ: {
        ...
        T_IG(TRACE_GRP_CONF, "msg_id = LLDP_MSG_ID_CONF_SET_REQ");
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
            cdp_add_to_mac_table(port_no, NULL);
        }
#endif // VTSS_SW_OPTION_CDP
        break;
    }
    ...
    T_IG(TRACE_GRP_CONF, "Done isid = %u", isid);
    return TRUE;
}
...
#endif // defined(VTSS_SW_OPTION_IP) || defined(VTSS_SW_OPTION_IP2)
...
vtss_isid_t lldp_local_usid_set_get(BOOL set, vtss_isid_t  new_usid)
{
    ...
    T_IG(TRACE_GRP_CONF, "set:%d, new_usid:%d, usid:%d", set, new_usid, usid);
    return usid;
}
...
void lldp_system_name(char *sys_name, char get_name)
{
    ...
    if (get_name) {
        ...
        strcpy(local_system_name, sys_name);
    }
...
static lldp_bool_t cdp_rx_pkt(void  *contxt,
                              ...
                              const vtss_packet_rx_info_t *const rx_info)
{
    lldp_bool_t eat_frm = 0; // Default to allowing other subscribers to receive the packet
    ...
    if (*(frm_p + 20) == 0x20 && *(frm_p + 21) == 0x00) {
        ...
        eat_frm = 1; // This is a CDP frame -  don't Allow other subscribers to receive the packet
    } else {
        eat_frm = 0; // This is not a CDP frame (probably a "Dynamic Trunking Protocol" frame). Allow other subscribers to receive the packet
    }
    LLDP_CRIT_EXIT();
    return eat_frm;
}
...
static void cdp_pkt_subscribe(void)
{
    ...
    // For getting the CDP frames to the CPU we need to add the CDP DAMC to the MAC table. The MAC
    ...
    vlan_port_conf_change_register(cdp_add_to_mac_table);
}
#endif // VTSS_SW_OPTION_CDP
...
static void lldp_thread(cyg_addrword_t data)
{
    ...
    // Wait for master up event has happen.
    while (!master_up) {
        VTSS_OS_MSLEEP(1000);
    }
...
static void lldp_conf_read(lldp_bool_t create)
{
    lldp_master_flash_conf_t *blk;
    ulong              size;
    lldp_bool_t        do_create;

    T_DG(TRACE_GRP_CONF, "Entering lldp_conf_read");
    ...
    if (do_create) {
        ...
        lldp_global.flash.medFastStartRepeatCount = FAST_START_REPEAT_COUNT_DEFAULT;
...
// Because we want to get LLDP information/statistics from the slaves in one chunck
// and because multiple user interfaces can "ask" for the information at the same time
// we need to make sure that the information for the first user interface isn't overwritten by
// the information from the next user interface. This is done by having the user interfaces calling
// the lldp_mgmt_get_lock function (which locks for new requests) before requesting the information, and calling
// lldp_mgmt_get_unlock (which opens for new requests) when having finshed outputing the information.
...
#endif
    if (conf->txDelay > (conf->msgTxInterval / 4)) {
        lldp_global.flash.common.txDelay = lldp_global.flash.common.msgTxInterval / 4;
        T_W("txDelay must not be larger than 0.25 * TxInterval - IEEE 802.1AB-clause 10.5.4.2 - Setting txDelay to %d", lldp_global.flash.common.txDelay);
    }
...
#ifdef VTSS_SW_OPTION_SNMP
    snmpLLDPNotificationChange(VTSS_ISID_LOCAL, port_index);
#ifdef VTSS_SW_OPTION_SECURE_SNMP
#ifdef VTSS_SW_OPTION_LLDP_MED
    snmpLLDPXemMIBNotificationChange(VTSS_ISID_LOCAL, port_index);
#endif
#endif
#endif /* VTSS_SW_OPTION_SNMP */
}
