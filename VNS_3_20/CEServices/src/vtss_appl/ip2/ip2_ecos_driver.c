...
int vtss_ip2_ecos_driver_if_add(vtss_vid_t vlan, const vtss_mac_t *mac)
{
    ...
    VTSS_ASSERT(vlan > 0 && vlan < VTSS_VID_RESERVED);
    ...
    _driver = calloc(1, sizeof(driver_pair_t));
    if (!_driver) {
        E("Alloc error");
        if_no = -1;
        goto ERR;
...
int vtss_ip2_ecos_driver_if_del(vtss_vid_t vid)
{
    ...
    VTSS_ASSERT(vid < VTSS_VID_RESERVED);
...
#define RT HANDLE.rt
    ...
    if (dst->sa_family == AF_INET) {
        ...
        // netmask
        if (n != NULL && n->sin6_family == AF_INET6) {
