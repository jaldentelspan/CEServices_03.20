                          ...
                          cli_req_t *req)
{
    ...
    req->parm_parsed = 1;
    if (IP2_parse_uint_range(cmd, VTSS_VID_DEFAULT, VTSS_VIDS, &tmp)) {
        return 1;
    }
                               ...
                               cli_req_t *req)
{
    ...
    req->parm_parsed = 1;
    if (mgmt_txt2list_bf(cmd, r->vlan_list, VTSS_VID_DEFAULT,
                         VTSS_VID_RESERVED, FALSE, TRUE) == VTSS_RC_OK) {
...
static void IP2_configuration(cli_req_t *req)
{
    ...
    /* Interfaces */
    for (ct = 0, ifid = VTSS_VID_DEFAULT; ifid < VTSS_VID_RESERVED; ifid++) {
...
static void IP2_address_delete(cli_req_t *req)
{
    ...
    if (vtss_ip2_hasipv6() && r->ip_net.address.type == VTSS_IP_TYPE_IPV6) {
        if (vtss_ip2_if_exists(r->vlan) &&
            ...
            if (vtss_ip_net_equal(&r->ip_net, &conf.network)) {
        ...
        if (vtss_ip2_if_exists(r->vlan) &&
            ...
            if (vtss_ip_net_equal(&r->ip_net, &conf.network)) {
                DO(vtss_ip2_ipv4_conf_set(r->vlan, &dconf), "Delete IPv4 address");
            } else {
                CPRINTF("Delete failed: Address not found on interface.\n");
            }
        } else {
...
static void IP2_dns_status(cli_req_t *req)
{
    ...
    rc = vtss_ip2_dns_get_status(&addr);
...
static void IP2_dns_conf(cli_req_t *req)
{
    vtss_rc rc;
    char buf[128];
    vtss_ip_srv_conf_t conf;
    IP2_cli_req_t *r = (IP2_cli_req_t *)req->module_req;

    memset(&conf, 0, sizeof(conf));

    if (!req->set) {
        rc = vtss_ip2_dns_get_conf(&conf);
        if (rc != VTSS_RC_OK) {
    ...
    if (r->vlan_spec == CLI_SPEC_VAL) {
        conf.type = VTSS_IP_SRV_TYPE_DHCP_VLAN;
        conf.u.dhcp_vlan_conf.vlan = r->vlan;
    }

    if (r->vlan_spec == CLI_SPEC_ANY || r->dhcp_any) {
        conf.type = VTSS_IP_SRV_TYPE_DHCP_ANY;
    }

    if (r->dns_ip_spec != CLI_SPEC_NONE) {
        if (vtss_ip_addr_is_zero(&r->dns_ip)) {
            conf.type = VTSS_IP_SRV_TYPE_NONE;
        } else {
            conf.type = VTSS_IP_SRV_TYPE_STATIC;
            conf.u.static_conf.ip_srv_address = r->dns_ip;
        }
    }
    DO(vtss_ip2_dns_set_conf(&conf), "Set DNS configuration");
}
cli_cmd_tab_entry (
...
cli_cmd_tab_entry (
    "Debug LPM_STAT ipv4_get [<vlan_list>]",
    NULL,
    "HW router counter",
    CLI_CMD_SORT_KEY_DEFAULT,
    CLI_CMD_TYPE_STATUS,
...
cli_cmd_tab_entry (
    "Debug LPM_STAT ipv6_get [<vlan_list>]",
    NULL,
    "HW router counter",
    CLI_CMD_SORT_KEY_DEFAULT,
    CLI_CMD_TYPE_STATUS,
