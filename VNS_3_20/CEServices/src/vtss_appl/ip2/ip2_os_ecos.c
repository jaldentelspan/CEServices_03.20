...
static BOOL port_filter_rx_discard(u32 src_port, vtss_vid_t vid)
{
    ...
    info.vid = vid;
    info.port_tx = VTSS_PORT_NO_NONE; // only rx filtering
...
vtss_rc vtss_ip2_os_dns_srv_get(vtss_ip_addr_t       *address)
{
    unsigned long ip = cyg_get_current_dns_server();

    if (ip == 0) {
        address->type = VTSS_IP_TYPE_NONE;
    } else {
        address->type = VTSS_IP_TYPE_IPV4;
        address->addr.ipv4 = ip;
    }

    return VTSS_RC_OK;
}

vtss_rc vtss_ip2_os_dns_srv_set(const vtss_ip_addr_t *const address)
{
    switch (address->type) {
    case VTSS_IP_TYPE_NONE:
    ...
    case VTSS_IP_TYPE_IPV4:
        cyg_set_current_dns_server(address->addr.ipv4);
        return VTSS_RC_OK;
