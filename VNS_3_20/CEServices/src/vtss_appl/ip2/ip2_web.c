...
static bool ip2_config_differs(const vtss_ip_conf_t *ipc_old, const vtss_ip_conf_t *ipc_new)
{
    if (ipc_old->dhcpc || ipc_new->dhcpc) { /* Used/Using DHCP? */
        return ipc_old->dhcpc != ipc_new->dhcpc; /* Did this change? */
    }
    /* Not using DHCP, not before, not now */
...
static cyg_int32 handler_ip_config(CYG_HTTPD_STATE *p)
{
    ...
    (void) vtss_ip2_dns_get_conf(&dns_conf);
#endif
    ...
    route_ct = 0;
    if ((routes = calloc((ct = IP2_MAX_ROUTES), sizeof(*routes))) != NULL) {
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (cyg_httpd_form_varable_int(p, "ip_dns_src", &val) &&
            val >= VTSS_IP_SRV_TYPE_DHCP_ANY &&
            val <= VTSS_IP_SRV_TYPE_DHCP_VLAN) {
            BOOL set_cfg = TRUE;
            dns_conf.type = val;
            if (dns_conf.type == VTSS_IP_SRV_TYPE_STATIC &&
                (web_parse_ip_fmt(p, &dns_conf.u.static_conf.ip_srv_address, "ip_dns_value") != VTSS_OK)) {
                T_E("Invalid IP address, not changing DNS config");
                set_cfg = FALSE;
            }
            if (dns_conf.type == VTSS_IP_SRV_TYPE_DHCP_VLAN) {
                if (cyg_httpd_form_varable_int(p, "ip_dns_value", &val) &&
                    val >= VTSS_VID_DEFAULT && val < VTSS_VID_RESERVED) {
                    dns_conf.u.dhcp_vlan_conf.vlan = val;
                } else {
            ...
            if (set_cfg) {
                if ((rc = vtss_ip2_dns_set_conf(&dns_conf)) != VTSS_OK) {
                ...
                if (cyg_httpd_form_variable_int_fmt(p, &vid, "if_vid_%d", i)) {
                    ...
                    if (cyg_httpd_form_variable_check_fmt(p, "if_del_%d", i)) {
                        ...
                        if ((rc = vtss_ip2_ipv4_conf_get(vid, &ipconf_old)) == VTSS_OK) {
                            ...
                            if (ip2_config_differs(&ipconf_old, &ipconf)) {
...
#ifdef VTSS_SW_OPTION_DNS
        switch (dns_conf.type) {
        case VTSS_IP_SRV_TYPE_STATIC:
            (void) vtss_ip2_ip_addr_to_txt(buf, sizeof(buf), &dns_conf.u.static_conf.ip_srv_address);
            break;
        case VTSS_IP_SRV_TYPE_DHCP_VLAN:
            (void) snprintf(buf, sizeof(buf), "%d", dns_conf.u.dhcp_vlan_conf.vlan);
            break;
        default:
            buf[0] = '\0';
        }
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                      "|%d|%s|%d", dns_conf.type, buf, dns_proxy);
        cyg_httpd_write_chunked(p->outbuffer, ct);
#endif
        ...
        for (i = 0; routes && i < route_ct; i++) {
            ...
            case VTSS_ROUTING_ENTRY_TYPE_IPV4_UC:
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                              VTSS_IPV4_FORMAT "#%d#" VTSS_IPV4_FORMAT "|",
...
static cyg_int32 handler_diag_ping_ipv6(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        (void)cyg_httpd_form_varable_long_int(p, "count", &count);
        (void)cyg_httpd_form_varable_long_int(p, "interval", &interval);
        web_io = ping6_test_async(&ipv6_addr, ping_length, count, interval);
        if (web_io) {
