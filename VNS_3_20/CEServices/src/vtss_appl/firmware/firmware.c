...
{
    firmware_msg_t *message = malloc(sizeof(*message));
    if(message) {
    ...
    if(len == sizeof(firmware_msg_t)) {
        ...
        if(len > 256) {         /* Sanity check */
            ...
            if(result == VTSS_OK) {
                ...
                if((image = malloc(len))) { /* Must copy from RX indication message */
    ...
    for(;;) {
        ...
        if(FIRMWARE_MBMSG_CHECKGO == (firmware_mbmsg_t) pReq) {
            ...
            if(pReq) {
                ...
                // free(pReq), which means that the restart member is no longer valid
                // after the next call.
