    ...
    if(((idx_act = flash_mgmt_fis_lookup(running, &fis_act)) >= 0) && 
       ...
       strcmp(fis_bak.name, alt_name) == 0) {
        ...
        if(vtss_switch_standalone()) {
            (void) control_system_reset(TRUE, 0, restart_type);
        } else {
                         ...
                         uchar *date, size_t sz_date)
{
    ...
    inbuf = malloc(Z_BUFSIZ);
    outbuf = malloc(Z_BUFSIZ + Z_WINDOW);
                    ...
                    size_t *length)
{
    ...
    inet_aton(server, &host.sin_addr);
    host.sin_port = 0;
                      ...
                      vtss_restart_t restart)
{
    ...
    firmware_flash_args_t *pReq = malloc(sizeof(firmware_flash_args_t));
    if(!pReq) {
#ifdef VTSS_SW_OPTION_CLI
    ...
    if((ent = flash_mgmt_fis_lookup(name, &fis)) > 0 &&
       simg->img_len <= fis.size) {
        ...
        if(!simage_get_tlv_dword(simg, SIMAGE_TLV_ARCH, &arch) || arch != MY_CPU_ARC) {
            ...
            T_E("CHIPFAMILY mismatch (was %d, want %d, CHIP was %04x, have %04x)",
...
char *firmware_error_txt(vtss_rc rc)
{
  switch(rc) {
    ...
    case FIRMWARE_ERROR_INVALID:                 return "Image error";
