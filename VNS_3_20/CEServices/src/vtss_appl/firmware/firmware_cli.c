...
static void cli_cmd_load_file(cli_req_t *req)
{
    char server[sizeof "000.000.000.000"], *file = req->parm;
    vtss_ipv4_t ipv4_server;
    struct sockaddr_in host;
...
static u8 *firware_netload(size_t max_size, size_t *actual_size, const char *target)
{
    ...
    if ((url = fetchParseURL(target)) == NULL) {
        ...
        if ((fp = fetchXGet(url, &us, "v"))) {
            ...
            } else if ((buffer = malloc(buflen)) == NULL) {
...
#ifdef VTSS_SW_OPTION_VNS_FPGA
static void cli_cmd_load_file_http(cli_req_t *req)
{
    firmware_cli_req_t *firmware_req = req->module_req;
...
cli_cmd_tab_entry (
	    NULL,
	    "LoadFile <ip_addr_string> <file_name>",
...
cli_cmd_tab_entry (
    NULL,
    "FileNetLoad <url>" COOL_KW,
    "Load new firmware via http",
    PRIO_LOAD_FIRMWARE,
    CLI_CMD_TYPE_CONF,
    VTSS_MODULE_ID_MISC,
    cli_cmd_load_file_http,
    NULL,
    firmware_cli_parm_table,
    CLI_CMD_FLAG_NONE
);
