...
#define VTSS_AUTH_CLIENT_TIMEOUT 60 /* seconds that client waits for reply from master */
...
typedef struct {
    ...
    vtss_auth_msg_id_t     msg_id;                                  /* request or reply */
    ulong                  sequence_number;                         /* used for matching replies with requests */
...
static critd_t                 crit_global;           /* Other global data critical region protection */
static vtss_auth_conf_t        config;                /* Current configuration */
static BOOL                    config_changed = TRUE; /* Configuration has changed */

static ulong                   sequence_number; /* Sequence number - incremented for each client request */
...
static vtss_trace_grp_t trace_grps[TRACE_GRP_CNT] = {
    ...
    [TRACE_GRP_CRIT_GLOBAL] = {
        .name      = "crit_glob",
        .descr     = "Global data critical regions ",
        .lvl       = VTSS_TRACE_LVL_ERROR,
        .timestamp = 1,
    }
};
...
#define AUTH_CRIT_GLOBAL_ENTER() critd_enter(&crit_global, TRACE_GRP_CRIT_GLOBAL, VTSS_TRACE_LVL_NOISE, __FILE__, __LINE__)
#define AUTH_CRIT_GLOBAL_EXIT()  critd_exit( &crit_global, TRACE_GRP_CRIT_GLOBAL, VTSS_TRACE_LVL_NOISE, __FILE__, __LINE__)
#else
...
#define AUTH_CRIT_GLOBAL_ENTER() critd_enter(&crit_global)
#define AUTH_CRIT_GLOBAL_EXIT()  critd_exit( &crit_global)
 ...
 * ================================================================= */
static void vtss_auth_client_pool_init(vtss_auth_client_pool_t *p)
{
    T_N("enter");
    memset(p, 0, sizeof(vtss_auth_client_pool_t));
    cyg_mutex_init(&p->mutex);
    cyg_cond_init(&p->cond, &p->mutex);
    T_N("exit");
}
 ...
 * ================================================================= */
static vtss_auth_client_buf_t *vtss_auth_client_buf_find_free(vtss_auth_client_pool_t *p)
{
    int i;
    T_N("enter");
    for (i = 0; i < VTSS_AUTH_CLIENT_POOL_LENGTH; i++) {
        if (p->buf[i].in_use == FALSE) {
 ...
 * ================================================================= */
static vtss_auth_client_buf_t *vtss_auth_client_buf_alloc(vtss_auth_client_pool_t *p)
{
    vtss_auth_client_buf_t *buf;
    T_N("enter");
    (void) cyg_mutex_lock(&p->mutex);
    while ((buf = vtss_auth_client_buf_find_free(p)) == NULL) {
        T_N("waiting");
        (void) cyg_cond_wait(&p->cond);
    }
    buf->in_use = TRUE;
    (void) cyg_mutex_unlock(&p->mutex);
    T_N("exit");
    return buf;
}
 ...
 * ================================================================= */
static void vtss_auth_client_buf_free(vtss_auth_client_pool_t *p, vtss_auth_client_buf_t *buf )
{
    T_N("enter");
    (void) cyg_mutex_lock(&p->mutex);
    buf->in_use = FALSE;
    cyg_cond_signal(&p->cond);
    (void) cyg_mutex_unlock(&p->mutex);
    T_N("exit");
}
 ...
 * ================================================================= */
static vtss_auth_client_buf_t *vtss_auth_client_buf_lookup(vtss_auth_client_pool_t *p, ulong seq_number)
{
    ...
    (void) cyg_mutex_lock(&p->mutex);
    for (i = 0; i < VTSS_AUTH_CLIENT_POOL_LENGTH; i++) {
        if (p->buf[i].in_use && (p->buf[i].msg.sequence_number == seq_number)) {
            buf = &p->buf[i];
            T_N("match");
            break;
        }
                             ...
                             const vtss_module_id_t modid, const ulong isid)
{
    ...
    msg->isid = isid; /* remember where it came from */

    switch (msg->msg_id) {
    case VTSS_AUTH_MSG_ID_AUTH_REQUEST: {
        if (msg_switch_is_master()) {
            vtss_auth_msg_t *m = malloc(sizeof(vtss_auth_msg_t));
            if (m) {
    ...
    case VTSS_AUTH_MSG_ID_AUTH_REPLY: {
        AUTH_CRIT_GLOBAL_ENTER();
        vtss_auth_client_buf_t *buf = vtss_auth_client_buf_lookup(&pool, msg->sequence_number);
        AUTH_CRIT_GLOBAL_EXIT();

        if (buf) { /* if a match is found then copy return parameters and unblock the client */
            (void) cyg_mutex_lock(&buf->mutex);
            buf->msg.userlevel = msg->userlevel;
            buf->msg.rc = msg->rc;
...
static vtss_rc vtss_auth_radius_config_update(const vtss_auth_radius_conf_t *rconf, const vtss_auth_proto_conf_t *conf)
{
    ...
    for (i = 0; i < VTSS_AUTH_NUMBER_OF_SERVERS; i++) {
        ...
        if (conf->host_conf[i].auth_port) {
            strncpy(radius_cfg.servers_auth[i].host, conf->host_conf[i].host, sizeof(radius_cfg.servers_auth[i].host));
            radius_cfg.servers_auth[i].host[sizeof(radius_cfg.servers_auth[i].host) - 1] = '\0';
            ...
            strncpy(radius_cfg.servers_auth[i].key, conf->host_conf[i].key[0] ? conf->host_conf[i].key : conf->global_conf.key, sizeof(radius_cfg.servers_auth[i].key));
            radius_cfg.servers_auth[i].key[sizeof(radius_cfg.servers_auth[i].key) - 1] = '\0';
        }
        if (conf->host_conf[i].acct_port) {
            strncpy(radius_cfg.servers_acct[i].host, conf->host_conf[i].host, sizeof(radius_cfg.servers_acct[i].host));
            radius_cfg.servers_acct[i].host[sizeof(radius_cfg.servers_acct[i].host) - 1] = '\0';
            ...
            strncpy(radius_cfg.servers_acct[i].key, conf->host_conf[i].key[0] ? conf->host_conf[i].key : conf->global_conf.key, sizeof(radius_cfg.servers_acct[i].key));
            radius_cfg.servers_acct[i].key[sizeof(radius_cfg.servers_acct[i].key) - 1] = '\0';
        }
    }
    ...
    strncpy(radius_cfg.nas_identifier, rconf->nas_identifier, sizeof(radius_cfg.nas_identifier));
    radius_cfg.nas_identifier[sizeof(radius_cfg.nas_identifier) - 1] = '\0';
...
static vtss_rc vtss_auth_radius(vtss_auth_agent_t agent, char *username, char *passwd, int *userlevel)
{
    ...
    if ((rc = vtss_radius_tx(handle, NULL, vtss_auth_radius_rx_callback)) != VTSS_OK) {
        ...
        if (radius_data.res == VTSS_RADIUS_RX_CALLBACK_OK) {
            ...
            rc = radius_data.code == VTSS_RADIUS_CODE_ACCESS_ACCEPT ? VTSS_OK : VTSS_AUTH_ERROR_SERVER_REJECT;
    ...
    *userlevel = 1;
    ...
    if ((session = vtss_auth_tac_connect(conf)) == NULL) {
        ...
        if (tac_rc == 0) {
            ...
            tac_rc = tac_authen_get_reply(session, serv_msg, data_msg);
            if (tac_rc <= 0) {
    ...
    if (rc == VTSS_OK) { // Authentication succeeded - now try to get the authorization privilege level
        if ((session = vtss_auth_tac_connect(conf)) != NULL) {
            ...
            if (tac_rc == 0) {
                ...
                tac_rc = tac_author_get_response(session, serv_msg, data_msg, avpair);
                if (tac_rc <= 0) {
...
static vtss_rc vtss_auth_local(char *username, char *passwd, int *userlevel)
{
    ...
    strcpy(conf.password, passwd);
    if (vtss_users_mgmt_conf_get(&conf, 0) != VTSS_OK || strcmp(conf.password, passwd)) {
        rc = VTSS_AUTH_ERROR_SERVER_REJECT;
    } else {
        ...
        *userlevel = entry->userlevel;
        ...
        if (vtss_authenticate(VTSS_AUTH_AGENT_HTTP, username, password, userlevel) == VTSS_OK) {
            ...
            if (rc == VTSS_AUTH_ERROR_CACHE_INVALID) {
                strncpy(entry->username, username, VTSS_AUTH_MAX_USERNAME_LENGTH);
                strncpy(entry->password, password, VTSS_AUTH_MAX_PASSWORD_LENGTH);
            }
            status = 0;
        } else {
            T_D("Auth failure");
...
static void vtss_auth_thread(cyg_addrword_t data)
{
    ...
    for (;;) {
        vtss_auth_msg_t *msg = cyg_mbox_get(thread.mbox_handle);
        T_N("begin server");
        ...
        if (msg_switch_is_master()) {
            int                     m;
            vtss_rc                 rc;
            vtss_auth_agent_conf_t *client = &local_config.agent[msg->agent];
            if (msg->agent < VTSS_AUTH_AGENT_LAST) {
                for (m = 0; m < VTSS_AUTH_METHOD_LAST; m++) {
                    switch (client->method[m]) {
                    case VTSS_AUTH_METHOD_LOCAL:
                        rc = vtss_auth_local(msg->username, msg->password, &msg->userlevel);
                        break;

                    case VTSS_AUTH_METHOD_RADIUS:
#ifdef VTSS_SW_OPTION_RADIUS
                        rc = vtss_auth_radius(msg->agent, msg->username, msg->password, &msg->userlevel);
#else
...
#ifdef VTSS_SW_OPTION_TACPLUS
                        rc = vtss_auth_tacplus(msg->agent, &local_config.proto[VTSS_AUTH_PROTO_TACACS], msg->username, msg->password, &msg->userlevel);
#else
                ...
                if (m == VTSS_AUTH_METHOD_LAST) {
                    msg->rc = VTSS_AUTH_ERROR_SERVER_REJECT; /* Authentication not allowed */
                } else {
                    msg->rc = rc;
                }
            } else {
                T_E("Invalid client (%d)", msg->agent);
            }
            msg->msg_id = VTSS_AUTH_MSG_ID_AUTH_REPLY;
            msg_tx(VTSS_MODULE_ID_AUTH, msg->isid, msg, sizeof(vtss_auth_msg_t));
        } else {
            T_W("server request received on a slave");
            free(msg);
        }
...
static void vtss_auth_msg_tx_done(void *contxt, void *msg, msg_tx_rc_t rc)
{
    if (rc != MSG_TX_RC_OK && msg) {
        ...
        vtss_auth_msg_t *auth_msg = msg;
        T_N("msg_tx error");
        AUTH_CRIT_GLOBAL_ENTER();
        vtss_auth_client_buf_t *buf = vtss_auth_client_buf_lookup(&pool, auth_msg->sequence_number);
        AUTH_CRIT_GLOBAL_EXIT();
        if (buf) {
            (void)cyg_mutex_lock(&buf->mutex);
            buf->msg.rc = VTSS_AUTH_ERROR_CLIENT_MSG_TX_FAILED;
            cyg_cond_signal(&buf->cond);
            (void)cyg_mutex_unlock(&buf->mutex);
        }
    }
...
static void vtss_auth_conf_flash_read(BOOL create)
{
    ...
    if ((blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_AUTH_TABLE, &size)) == NULL || size != sizeof(*blk)) {
...
vtss_rc vtss_authenticate(vtss_auth_agent_t agent, char *username, char *password, int *userlevel)
{
    vtss_auth_client_buf_t *buf;
    vtss_rc rc;
    cyg_tick_count_t absolute_timeout;
    ...
    AUTH_CRIT_GLOBAL_ENTER();
    buf = vtss_auth_client_buf_alloc(&pool);
    AUTH_CRIT_GLOBAL_EXIT();
    ...
    buf->msg.msg_id = VTSS_AUTH_MSG_ID_AUTH_REQUEST;
    AUTH_CRIT_GLOBAL_ENTER();
    buf->msg.sequence_number = sequence_number++;
    AUTH_CRIT_GLOBAL_EXIT();
    buf->msg.agent = agent;
    strncpy(buf->msg.username, username, VTSS_AUTH_MAX_USERNAME_LENGTH - 1);
    buf->msg.username[VTSS_AUTH_MAX_USERNAME_LENGTH - 1] = '\0'; /* force null termination */
    strncpy(buf->msg.password, password, VTSS_AUTH_MAX_PASSWORD_LENGTH - 1);
    buf->msg.password[VTSS_AUTH_MAX_PASSWORD_LENGTH - 1] = '\0'; /* force null termination */
    buf->msg.rc = VTSS_AUTH_ERROR_CLIENT_WAITING; /* also used here as condition variable!!! */
    absolute_timeout = cyg_current_time() +  VTSS_OS_MSEC2TICK(VTSS_AUTH_CLIENT_TIMEOUT * 1000);

    /* Wait for a reply from server */
    (void) cyg_mutex_lock(&buf->mutex);
    while (buf->msg.rc == VTSS_AUTH_ERROR_CLIENT_WAITING) {
        ...
        if (cyg_cond_timed_wait(&buf->cond, absolute_timeout) == FALSE) {
            T_N("timeout");
            buf->msg.rc = VTSS_AUTH_ERROR_CLIENT_TIMEOUT;
        } else if (buf->msg.rc == VTSS_AUTH_ERROR_CLIENT_MSG_TX_FAILED) {
            T_N("msg_tx error");
            // Try again.
            buf->msg.rc = VTSS_AUTH_ERROR_CLIENT_WAITING;
        } else {
            T_N("All is well");
        }
    }
    /* Copy everything before releasing the buffer */
    *userlevel = buf->msg.userlevel;
    rc = buf->msg.rc;
    (void) cyg_mutex_unlock(&buf->mutex);

    AUTH_CRIT_GLOBAL_ENTER();
    vtss_auth_client_buf_free(&pool, buf);
    AUTH_CRIT_GLOBAL_EXIT();
    return rc;
}
...
vtss_rc vtss_auth_mgmt_host_add(vtss_auth_proto_t proto, vtss_auth_host_conf_t *const conf)
{
    ...
    if (rc == VTSS_OK) {
        ...
        if (proto == VTSS_AUTH_PROTO_RADIUS) {
            vtss_rc _rc = VTSS_OK;
            if ((_rc = vtss_auth_radius_config_update(&config.radius, &config.proto[VTSS_AUTH_PROTO_RADIUS])) != VTSS_OK) {
                T_W("vtss_auth_radius_config_update() failed: \"%s\"", error_txt(_rc));
            }
        }
#endif /* VTSS_SW_OPTION_RADIUS */
    }
...
vtss_rc vtss_auth_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        AUTH_CRIT_CACHE_EXIT();
        critd_init(&crit_global, "auth.crit.global", VTSS_MODULE_ID_AUTH, VTSS_TRACE_MODULE_ID, CRITD_TYPE_MUTEX);
        AUTH_CRIT_GLOBAL_EXIT();

        vtss_auth_client_pool_init(&pool);
#ifdef VTSS_SW_OPTION_SSH
