...
static vtss_rc synce_cx_parse_func(cx_set_state_t *s)
{
    ...
    case CX_PARSE_CMD_PARM:
    {
        ...
        case CX_TAG_SYNCE_MANUAL_CLK_SRC:
            if (global && cx_parse_ulong_dis(s, "val", &val, 0, CLOCK_INPUT_MAX -1, 0) == VTSS_OK)
            ...
            case CX_TAG_GROUP_TABLE:
                if (1) {
                    ...
                    for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                        ...
                        if (cx_parse_ulong_dis(s, "prio", &val, 0,SYNCE_PRIORITY_MAX -1, 0) == VTSS_OK) {
                            clk_prio_update = 1;
                            clk_prio = val;
                        }
                    ...
                    if (s->apply) {
                        if (synce_state->synce_count < SYNCE_NOMINATED_MAX) {
                            if (clk_prio_update) {
...
static vtss_rc synce_cx_gen_func(cx_set_state_t *s)
{
    switch (s->cmd) {
        case CX_GEN_CMD_GLOBAL:
            ...
            cx_add_val_ulong(s, CX_TAG_SYNCE_MANUAL_CLK_SRC, clock_config.source, 0, CLOCK_INPUT_MAX -1);
            ...
            /* nominate */
            cx_add_stx_start(s);
            cx_add_stx_ulong(s, "clk_src", 0, SYNCE_NOMINATED_MAX-1);
            ...
            cx_add_stx_ulong(s, "prio", 0, SYNCE_PRIORITY_MAX-1);
            cx_add_stx_end(s);

            cx_add_tag_line(s, CX_TAG_GROUP_TABLE, 0);
            int clk_src;
            for (clk_src = 0; clk_src < SYNCE_NOMINATED_MAX; clk_src++) {
