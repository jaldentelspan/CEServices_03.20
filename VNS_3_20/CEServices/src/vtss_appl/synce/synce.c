...
#define TRACE_GRP_PDU_RX       2
#define TRACE_GRP_PDU_TX       3
#define TRACE_GRP_API          4
#define TRACE_GRP_CLOCK        5
...
static void save_config_blk(void)
{
    ...
    blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_SYNCE_CONF_TABLE, &size);
...
static void set_conf_to_default(synce_mgmt_clock_conf_blk_t  *clockk,
                                synce_mgmt_port_conf_blk_t   *port)
{
    uint   i;
...
static void apply_configuration(conf_blk_t  *blk) 
{
    ...
    rc += synce_mgmt_nominated_selection_mode_set(blk->clock.selection_mode, blk->clock.source, blk->clock.wtr_time/60, blk->clock.ssm_holdover, blk->clock.ssm_freerun);
...
{
	if (port >= SYNCE_PORT_COUNT)  return SSM_QL_NONE;
...
static void set_wtr(uint source)
{
    ...
    if(source >= SYNCE_NOMINATED_MAX ) return;
...
{
	if (port >= SYNCE_PORT_COUNT || source >= SYNCE_NOMINATED_MAX) return(FALSE);
...
static void configure_master_slave(void)
{
    ...
    for (i=0; i<SYNCE_NOMINATED_MAX; ++i) { /* Check for ANEG */
...
static void set_clock_priority(void)
{
    ...
    for (i=0; i<SYNCE_NOMINATED_MAX; ++i)    prio_assigned[i] = SYNCE_NOMINATED_MAX;
    ...
    /* Check all qualities */
        ...
        for (prio=0; prio<SYNCE_NOMINATED_MAX; ++prio)
        {
    ...
    if (prio_count < SYNCE_NOMINATED_MAX)
    {
    /* NOT all possible priorities has been assigned - has to be done */
        for (i=0; i<SYNCE_NOMINATED_MAX; ++i)
        {
            if (prio_assigned[i] == SYNCE_NOMINATED_MAX)
            {
                prio_assigned[i] = prio_count;
                prio_count++;
            }
    ...
    if (prio_count != SYNCE_NOMINATED_MAX)
        T_E("invalid numer of priorities assigned %d", prio_count);
...
static void func_thread(cyg_addrword_t data)
{
    ...
    tx_buf[0] = 1;
    if (vtss_i2c_wr_rd(NULL, 0x75, tx_buf, 1, rx_buf, 2, 0xFF, NO_I2C_MULTIPLEXER) == VTSS_RC_OK) {
        ...
        if ((rx_buf[0] >= 0x03) && (rx_buf[1] == 0x33)) {   /* This PCB104 SyncE */
            ...
            if (status.restart==VTSS_RESTART_COLD) {
                ...
                if (vtss_i2c_wr_rd(NULL, 0x75, tx_buf, 1, rx_buf, 1, 0xFF, NO_I2C_MULTIPLEXER) == VTSS_RC_OK) {
                    rx_buf[0] &= ~0x1C;
                    rx_buf[0] |= 0x18;
                    tx_buf[1] = rx_buf[0];
                    if (vtss_i2c_wr(NULL, 0x75, tx_buf, 2, 0xFF, NO_I2C_MULTIPLEXER) != VTSS_RC_OK)   T_D("Failed SPI selector set to SI5326  %X", tx_buf[1]);
                }
                else    T_D("Failed CPLD SPI selector get");

                tx_buf[0] = 0xF6;  /* This is reset of Silabs 5338 */
                tx_buf[1] = 2;
                if (vtss_i2c_wr(NULL, 0x71, tx_buf, 2, 50, NO_I2C_MULTIPLEXER) != VTSS_RC_OK)   T_D("Failed reset of SI5338");
            }
        }
    }
    ...
    if ((blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_SYNCE_CONF_TABLE, &size)) == NULL || size != sizeof(*blk))
    {
        ...
        blk = conf_sec_create(CONF_SEC_GLOBAL, CONF_BLK_SYNCE_CONF_TABLE, sizeof(*blk));

        blk->version = 0;
        set_conf_to_default(&blk->clock, &blk->port);
    }
    ...
    for (;;)
    {
        ...
        if (VTSS_MTIMER_TIMEOUT(&onesec_timer)) {
            ...
            if (clock_old_selector_state == CLOCK_SELECTOR_STATE_FREERUN)
            {
            ...
            { /* Run the prefer master/slave timer */
                if (port_state[i].prefer_timer) {   /* Prefer timer is active */
                    ...
                    if (port_state[i].prefer_timer >= PREFER_TIMEOUT) {   /* Prefer time out */
                        ...
                        if (port_is_nominated(i, &source) && (clock_conf_blk.aneg_mode[source] != SYNCE_MGMT_ANEG_FORCED_SLAVE)) {
        ...
        if (holdoff_active) { /* Run the hold off timer */
            ...
            for (source=0; source<SYNCE_NOMINATED_MAX; ++source) { /* Check for holdoff time out on any clock source */
...
/*  SyncE interface                                                         */
/****************************************************************************/
uint synce_mgmt_nominated_source_set(const uint                           source,
                                     const BOOL                           enable,
                                     const uint                           port_no,
                                     ...
                                     const synce_mgmt_quality_level_t     ssm_overwrite)
{
    ...
    T_D("source = %d, port_no = %d, aneg_mode = %d, ssm_overwrite %d", source, port_no, aneg_mode, ssm_overwrite);
    CRIT_ENTER();
    if (source >= SYNCE_NOMINATED_MAX)                                                                                          {CRIT_EXIT(); return (SYNCE_RC_INVALID_PARAMETER);}
    ...
    if (!enable && !clock_conf_blk.nominated[source])                                                                           {CRIT_EXIT(); return (SYNCE_RC_OK);}

    if (!enable)
    {
    /* This is a de-nomination of the clock source */
        if (!clock_conf_blk.nominated[source])      {CRIT_EXIT();   return (SYNCE_RC_OK);}    /* allready not nominated */
        ...
        clock_conf_blk.ssm_overwrite[source] = QL_NONE;

        if (pcb104_synce) {
            tx_buf[0] = (source) ? 8 : 9; /* Set CPLD Clock input selector */
            tx_buf[1] = 0;
            if (vtss_i2c_wr(NULL, 0x75, tx_buf, 2, 50, NO_I2C_MULTIPLEXER) != VTSS_RC_OK)    T_D("Clock selector set failed  %u", source);
        }
    }
    else
    {
    /* This is a nomination of the clock source */
        ...
        for (i=0; i<SYNCE_NOMINATED_MAX; ++i)
        {
            if (clock_conf_blk.nominated[i] && (clock_conf_blk.port[i] == port_no) && (i != source))                       {CRIT_EXIT(); return(SYNCE_RC_NOM_PORT);}
        }
        ...
        if (pcb104_synce) {
            tx_buf[0] = (source) ? 8 : 9; /* Set CPLD Clock input selector */
            tx_buf[1] = (source) ? 0x21 : 0x22;
            if (vtss_i2c_wr(NULL, 0x75, tx_buf, 2, 50, NO_I2C_MULTIPLEXER) != VTSS_RC_OK)    T_D("Clock selector set failed  %u", source);
        }

        if (clock_frequency_set(source, CLOCK_FREQ_125MHZ) != VTSS_OK)    T_D("error returned");
                                             ...
                                             const synce_mgmt_quality_level_t  ssm_freerun)
{
    ...
    if (source >= SYNCE_NOMINATED_MAX)    return(SYNCE_RC_INVALID_PARAMETER);
...
uint synce_mgmt_nominated_priority_set(const uint     source,
                                       const uint     priority)
{
    uint  rc=SYNCE_RC_OK;
    ...
    if ((source >= SYNCE_NOMINATED_MAX) || (priority >= SYNCE_PRIORITY_MAX))    return (SYNCE_RC_INVALID_PARAMETER);
...
vtss_rc synce_init(vtss_init_data_t *data)
{
    ...
    T_W("SYNCE_PORT_COUNT: %d, SYNCE_NOMINATED_MAX: %d\n", SYNCE_PORT_COUNT, SYNCE_NOMINATED_MAX);
    switch (data->cmd)
    {
        case INIT_CMD_INIT:
            T_D("INIT");
            ...
            if((board_type == VTSS_BOARD_MITR_REF) ||
            		(board_type == VTSS_BOARD_VESM_REF) ||
            		(board_type == VTSS_BOARD_ESIR_REF) ||
            		(board_type == VTSS_BOARD_NCGU_REF) ||
            		(board_type == VTSS_BOARD_IGU_REF)  ||
            		(board_type == VTSS_BOARD_VNS_6_REF) ||
            		(board_type == VTSS_BOARD_VNS_8_REF) ||
            		(board_type == VTSS_BOARD_VNS_16_REF) ||
            ...
            if ((board_type == VTSS_BOARD_LUTON10_REF) || (board_type == VTSS_BOARD_LUTON26_REF)/* || (board_type == VTSS_BOARD_VNS_6_REF) || (board_type == VTSS_BOARD_VNS_12_REF)*/ )
            {
                ...
                for (i=0; i<(VTSS_OPT_PORT_COUNT - 2); ++i) // Might need to modify BOARD_VNS_ALL_REF ((BOARD_VNS_6_REF) || (BOARD_VNS_8_REF) || (BOARD_VNS_12_REF))
                {
                    source_port[0][i] = 1;
                    source_port[1][i] = 1;
                }
            ...
            set_conf_to_default(&clock_conf_blk, &port_conf_blk);
