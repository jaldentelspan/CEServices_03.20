...
#define IPMC_MVR_ICFG_REG(x, y, z)  (((x) = vtss_icfg_query_register((y), (z))) == VTSS_OK)
                                     ...
                                     vtss_icfg_query_result_t *result)
{
    ...
    if (req && result) {
        ...
        */
        ...
        if (state != MVR_CONF_DEF_GLOBAL_MODE) {
                                   ...
                                   vtss_icfg_query_result_t *result)
{
    ...
    if (req && result) {
        ...
        /*
            ...
            COMMAND = mvr name <mvr_name:word16> igmp-address <ipv4_nmcast>
            COMMAND = mvr vlan <vlan_list> igmp-address <ipv4_nmcast>
        ...
        while (mvr_mgmt_get_next_intf_entry(VTSS_ISID_GLOBAL, &mvr_icfg_intf) == VTSS_OK) {
            ...
            if (mvif_strlen) {
                ...
                if (req->all_defaults || (mvif->last_listener_query_interval != MVR_CONF_DEF_INTF_LLQI)) {
                    rc |= vtss_icfg_printf(result, "mvr name %s last-member-query-interval %lu\n",
                                           mvif->name,
                ...
                if (req->all_defaults || (mvif->last_listener_query_interval != MVR_CONF_DEF_INTF_LLQI)) {
                    rc |= vtss_icfg_printf(result, "mvr vlan %u last-member-query-interval %lu\n",
                                           mvif->vid,
                                           mvif->last_listener_query_interval);
                }
            }
...
vtss_rc ipmc_mvr_icfg_init(void)
{
    ...
    if (IPMC_MVR_ICFG_REG(rc, VTSS_ICFG_IPMC_MVR_INTF, _ipmc_mvr_icfg_intf)) {
        if (IPMC_MVR_ICFG_REG(rc, VTSS_ICFG_IPMC_MVR_PORT, _ipmc_mvr_icfg_port)) {
            if (IPMC_MVR_ICFG_REG(rc, VTSS_ICFG_IPMC_MVR_GLOBAL, _ipmc_mvr_icfg_global)) {
