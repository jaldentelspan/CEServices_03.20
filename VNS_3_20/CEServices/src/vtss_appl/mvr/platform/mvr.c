...
#define MVR_OBS2_GROUP_MAX      (16 * 8)
...
//            memcpy(&obs_mvr_conf_v2, conf_blk2, sizeof(mvr_conf_v2_t));
    ...
    if (blk_ver > MVR_CONF_VERSION) {
        ...
        if (sizeof(mvr_configuration_t) != conf_sz) {
            ...
            if (!conf_blk) {
                T_W("Invalid MVR Configuration Block");

                return FALSE;
            }

            memcpy(tar_conf, conf_blk, sizeof(mvr_configuration_t));
        }
    }
...
static void _mvr_stacking_reset_port_type(vtss_isid_t isid)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (vlan_mgmt_port_conf_set(isid, idx, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
            T_D("Failure in _mvr_stacking_reset_port_type on isid %d port %u", isid, idx);
        }
    }
}
...
static BOOL _mvr_stacking_set_port_type(vtss_isid_t isid, vtss_mvr_interface_t *entry)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (chk_role.ports[idx] == MVR_PORT_ROLE_INACT) {
            ...
            if (vlan_mgmt_port_conf_set(isid, idx, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
                T_D("Failure in MVR_PORT_ROLE_INACT on isid %d port %u", isid, idx);
            }
        ...
        T_D("Set isid-%d port-%u pvid=%u/uvid=%u/frm_t=%d/ing_fltr=%s/tx_tag_t=%d/port_t=%d/flag=%u",
            ...
            vlan_port_conf.flags);
        if (vlan_mgmt_port_conf_set(isid, idx, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
            T_D("Failure(%s) on isid %d port %u",
                (chk_role.ports[idx] == MVR_PORT_ROLE_SOURC) ? "SRC" : "RCV",
                isid, idx);
        }
    }
...
static BOOL _mvr_stacking_refine_port_type(vtss_isid_t isid)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        T_D("Refine isid-%d port-%u pvid=%u/uvid=%u/frm_t=%d/ing_fltr=%s/tx_tag_t=%d/port_t=%d/flag=%u",
            ...
            vlan_port_conf.flags);
        if (vlan_mgmt_port_conf_set(isid, idx, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
            T_D("Failure in setting isid %d port %u", isid, idx);
        }
    }
...
static char *mvr_msg_id_txt(mvr_msg_id_t msg_id)
{
    ...
    case MVR_MSG_ID_STP_PORT_CHANGE_REQ:
        ...
        break;
    case MVR_MSG_ID_VLAN_PORT_CHANGE_REQ:
        txt = "MVR_MSG_ID_VLAN_PORT_CHANGE_REQ";
...
static BOOL mvr_msg_rx(void *contxt, const void *const rx_msg, size_t len, vtss_module_id_t modid, u32 isid)
{
    ...
    case MVR_MSG_ID_VLAN_GROUP_ENTRY_WALK_REQ: {
        ...
        MVR_CRIT_ENTER();
        if (vtss_mvr_intf_group_get_next(&grp)) {
            valid = TRUE;
        }
        MVR_CRIT_EXIT();
    ...
    case MVR_MSG_ID_GROUP_SRCLIST_WALK_REQ: {
        ...
        memcpy(&grp.group_addr, &msg->group_addr, sizeof(vtss_ipv6_t));
        MVR_CRIT_ENTER();
        if (vtss_mvr_intf_group_get(&grp)) {
            valid = TRUE;
        }
        MVR_CRIT_EXIT();
    ...
    case MVR_MSG_ID_VLAN_PORT_CHANGE_REQ: {
        mvr_msg_vlan_port_change_set_req_t  *msg;
        u8                                  active_cnt;
        ...
        msg = (mvr_msg_vlan_port_change_set_req_t *) rx_msg;
        T_D("Receiving MVR_MSG_ID_VLAN_PORT_CHANGE_REQ");
        ...
        memset(&vlan_port_conf, 0x0, sizeof(vlan_port_conf_t));
        if (vlan_mgmt_port_conf_get(isid, iport, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
            break;
        }

        uvid_chg = msg->cfg_pvid;
        if (vlan_port_conf.untagged_vid != uvid_chg) {
            vlan_port_conf.untagged_vid = uvid_chg;
            if (vlan_mgmt_port_conf_set(isid, iport, &vlan_port_conf, MVR_VLAN_TYPE) != VTSS_OK) {
                T_D("Failure in MVR_MSG_ID_VLAN_PORT_CHANGE_REQ on isid %u port %u", isid, iport);
            }
        }
                                           ...
                                           ipmc_ip_version_t ipmc_version)
{
    ...
    if (action == IPMC_OP_DEL) {
        ...
        if (vlan_mgmt_vlan_get(isid, param->vid, &vlan_entry, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
            if (vlan_mgmt_vlan_del(isid, param->vid, MVR_VLAN_TYPE, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (entry->ports[idx] != MVR_PORT_ROLE_INACT) {
                vlan_entry.ports[idx] = VLAN_MEMBER_PORT;
            }
        }
    ...
    if (msg_switch_is_local(isid)) {
        ...
        p->valid = TRUE;
        memcpy(p->name, entry->name, VTSS_IPMC_NAME_MAX_LEN);
        p->mode = entry->mode;
        p->vtag = entry->vtag;
                                           ...
                                           ipmc_ip_version_t ipmc_version)
{
    ...
    if (!msg_switch_is_master() || !msg_switch_exists(isid_add) || !intf_param) {
        return VTSS_RC_ERROR;
    }
                                            ...
                                            vtss_rc *rc)
{
    ...
    *rc = VTSS_OK;
    ...
    if (action == IPMC_OP_ADD) {
        ...
        vlan_entry.vid = vid;
        if (vlan_mgmt_vlan_get(isid, vid, &vlan_entry, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
...
static mvr_vlan_port_chg_t  mvr_vlan_port_chg_buf[VTSS_PORT_ARRAY_SIZE];

void mvr_vlan_port_change_callback(vtss_port_no_t iport, const vlan_port_conf_t *new_conf)
{
    if (!mvrReady || !new_conf) {
        return;
    }

    MVR_CRIT_ENTER();
    mvr_vlan_port_chg_buf[iport].valid = TRUE;
    mvr_vlan_port_chg_buf[iport].pvid_cfg = new_conf->pvid;
    MVR_CRIT_EXIT();
...
static void _mvr_vlan_port_change_handler(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        MVR_CRIT_ENTER();
        if (!mvr_vlan_port_chg_buf[iport].valid) {
            MVR_CRIT_EXIT();
            continue;
        }

        mvr_vlan_port_chg_buf[iport].valid = FALSE;
        pvid_cfg = mvr_vlan_port_chg_buf[iport].pvid_cfg;
        MVR_CRIT_EXIT();

        mvr_msg_alloc(MVR_MSG_ID_VLAN_PORT_CHANGE_REQ, &buf, FALSE, TRUE);
        msg = (mvr_msg_vlan_port_change_set_req_t *) buf.msg;

        msg->msg_id = MVR_MSG_ID_VLAN_PORT_CHANGE_REQ;
        msg->port = iport;
        msg->cfg_pvid = pvid_cfg;

        mvr_msg_tx(&buf, 0, sizeof(*msg));
    }
...
static void _mvr_vlan_warning_handler(vtss_vid_t vid)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        if (vlan_mgmt_vlan_get(isid, vid, &vlan_conf, FALSE, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
            continue;
        }
        ...
        while (port_iter_getnext(&pit)) {
            iport = pit.iport;
            if (vlan_conf.ports[iport] == VLAN_NON_MEMBER_PORT) {
                continue;
            }
...
#else
    while (ip_status_get(&status) == VTSS_OK && !status.ip_thread_ready && !mvr_link && (delay_cnt < MVR_THREAD_START_DELAY_CNT)) {
#endif /* VTSS_SW_OPTION_IP2 */
#if VTSS_SWITCH_STACKABLE
...
vtss_rc mvr_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        for (idx = MVR_MSG_ID_GLOBAL_SET_REQ; idx < MVR_MSG_MAX_ID; idx++) {
            ...
            case MVR_MSG_ID_STP_PORT_CHANGE_REQ:
                ...
                break;
            case MVR_MSG_ID_VLAN_PORT_CHANGE_REQ:
                mvr_running.msize[idx] = sizeof(mvr_msg_vlan_port_change_set_req_t);
                break;
            default:
                /* Give the MAX */
                mvr_running.msize[idx] = sizeof(mvr_msg_vlan_set_req_t);
            ...
            mvr_running.msg[idx] = malloc(mvr_running.msize[idx]);
        ...
        vlan_port_conf_change_register(mvr_vlan_port_change_callback);
        ...
        T_D("MVR-INIT_CMD_INIT consumes ID%d:%u ticks", isid, (u32)(cyg_current_time() - exe_time_base));
    ...
    case INIT_CMD_START:
        ...
        T_D("MVR-INIT_CMD_START consumes ID%d:%u ticks", isid, (u32)(cyg_current_time() - exe_time_base));
    ...
    case INIT_CMD_CONF_DEF:
        ...
        if (msg_switch_is_master()) {
            if (run_time_action) {
                ...
                while (flag) {
                    memset(&vlan_entry, 0x0, sizeof(vlan_mgmt_entry_t));
                    if (vlan_mgmt_vlan_get(VTSS_ISID_GLOBAL, intf.vid, &vlan_entry, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
                        (void) vlan_mgmt_vlan_del(VTSS_ISID_GLOBAL, intf.vid, MVR_VLAN_TYPE, VLAN_MEMBERSHIP_OP);
                    }
    ...
    case INIT_CMD_MASTER_DOWN:
        ...
        T_D("MVR-INIT_CMD_MASTER_DOWN consumes ID%d:%u ticks", isid, (u32)(cyg_current_time() - exe_time_base));
