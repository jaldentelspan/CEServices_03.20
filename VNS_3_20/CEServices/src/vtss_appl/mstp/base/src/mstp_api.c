       ...
       (bridge->conf.msti = zmalloc(N_MSTI_MAX*n_ports*sizeof(mstp_msti_port_param_t))) != NULL) {
        ...
        if(bridge) {
            if(bridge->ist.portmap != NULL)
                free(bridge->ist.portmap);
            if(bridge->conf.cist != NULL)
                free(bridge->conf.cist);
            if(bridge->conf.msti != NULL)
                free(bridge->conf.msti);
        }
        free(bridge);
        bridge = NULL;
    }
                          ...
                          mstp_port_status_t *status)
{
    ...
    status->state = (int) (portEnabled(cist) ? 
                           ...
                           PORTSTATE_DISABLED);
    strncpy(status->statestr, rstp_portstate2str(port), sizeof(status->statestr));
    ...
    /* s) Port Role */
    strncpy(status->rolestr, rstp_portrole2str(port->role), sizeof(status->rolestr));
                           ...
                           mstp_port_vectors_t *vectors)
{
    ...
    strncpy(vectors->infoIs, rstp_info2str(port->infoIs), sizeof(vectors->infoIs));
