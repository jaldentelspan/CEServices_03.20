...
static vtss_rc cx_stp_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    if (ports == NULL) {
        ...
        if (port_no == VTSS_PORT_NO_NONE)
            cx_add_attr_txt(s, "port", "aggr");
        else
            cx_add_stx_port(s);
        cx_add_stx_bool(s, "mode");
        cx_add_stx_bool(s, "adminedge");
        cx_add_stx_bool(s, "autoedge");
        cx_add_stx_kw(s, "p2p", cx_kw_stp_p2p);
        cx_add_stx_bool(s, "restrictedrole");
        cx_add_stx_bool(s, "restrictedtcn");
        cx_add_stx_bool(s, "bpduguard");
    ...
    cx_add_attr_bool(s, "mode", enable);
    cx_add_attr_bool(s, "adminedge", conf.adminEdgePort);
    cx_add_attr_bool(s, "autoedge", conf.adminAutoEdgePort);
    cx_add_attr_kw(s, "p2p", cx_kw_stp_p2p, conf.adminPointToPointMAC);
    cx_add_attr_bool(s, "restrictedrole", conf.restrictedRole);
    cx_add_attr_bool(s, "restrictedtcn", conf.restrictedTcn);
    cx_add_attr_bool(s, "bpduguard", conf.bpduGuard);
...
static vtss_rc cx_stp_mstiport_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    if (ports == NULL) {
        ...
        if (port_no == VTSS_PORT_NO_NONE)
            ...
            cx_add_stx_port(s);
        cx_add_stx_ulong(s, "msti", 0, N_MSTI_MAX-1);
        cx_add_stx_ulong(s, "pathcost", 0, 200000000);
        cx_add_stx_ulong(s, "priority", 0, 0xff);
        return cx_add_stx_end(s);
    }
    ...
    cx_add_port_start(s, CX_TAG_ENTRY, ports);
    cx_add_attr_ulong(s, "msti", msti);
    cx_add_attr_ulong(s, "pathcost", conf.adminPathCost);
    cx_add_attr_ulong(s, "priority", conf.adminPortPriority);
    return cx_add_port_end(s, CX_TAG_ENTRY);
}
...
static vtss_rc mstp_cx_parse_func(cx_set_state_t *s)
{
    ...
    case CX_PARSE_CMD_PARM:
    {
        ...
        case CX_TAG_ENTRY:
            if (global && s->group == CX_TAG_MSTP_MSTI_TABLE) {
                if(cx_parse_ulong(s, "vid", &val, VTSS_VID_DEFAULT, VTSS_VID_RESERVED) == VTSS_OK) {
...
static vtss_rc mstp_cx_gen_func(cx_get_state_t *s)
{
    switch (s->cmd) {
        case CX_GEN_CMD_GLOBAL:
            /* Global - STP */
            T_D("global - mstp");
            cx_add_tag_line(s, CX_TAG_STP, 0);
            {
                mstp_bridge_param_t conf;

                if (mstp_get_system_config(&conf)) {
                    cx_add_val_ulong(s, CX_TAG_AGE, conf.bridgeMaxAge, 6, 40);
                    cx_add_val_ulong(s, CX_TAG_DELAY, conf.bridgeForwardDelay, 4, 30);
                    cx_add_val_kw(s, CX_TAG_VERSION, cx_kw_stp_version, 
                            conf.forceVersion);
                    cx_add_val_ulong(s, CX_TAG_TXHOLDCOUNT, conf.txHoldCount, 1, 10);
                    cx_add_val_ulong(s, CX_TAG_MAXHOPS, conf.MaxHops, 6, 40);
                    cx_add_val_bool(s, CX_TAG_MSTP_BPDUFILTER, conf.bpduFiltering);
                    cx_add_val_bool(s, CX_TAG_MSTP_BPDUGUARD, conf.bpduGuard);
                    cx_add_val_ulong(s, CX_TAG_MSTP_RECOVERY, conf.errorRecoveryDelay, 
                            0, 86400);
                }
            }

            cx_add_tag_line(s, CX_TAG_MSTP_MSTI_PRIORITIES, 0);
            {
                u8 priority;
                u8 msti;
                ...
                /* Entry syntax */
                cx_add_stx_start(s);
                cx_add_stx_ulong(s, "msti", 0, N_MSTI_MAX-1); /* CIST, MST1, ... */
                cx_add_stx_ulong(s, "priority", 0, 0xff);
                cx_add_stx_end(s);
                ...
                for (msti = 0; msti < N_MSTI_MAX; msti++) {
                    ...
                    cx_add_attr_ulong(s, "msti", msti);
                    cx_add_attr_ulong(s, "priority", priority);
                    CX_RC(cx_add_attr_end(s, CX_TAG_ENTRY));
                }
            }
            CX_RC(cx_add_tag_line(s, CX_TAG_MSTP_MSTI_PRIORITIES, 1));
            cx_add_tag_line(s, CX_TAG_MSTP_MSTI_TABLE, 0);
            {
                mstp_msti_config_t conf;
                if (mstp_get_msti_config(&conf, NULL)) {
                    cx_add_val_ulong(s, CX_TAG_VERSION, conf.revision, 0, 0xffff);
                    cx_add_val_txt(s, CX_TAG_NAME, conf.configname, NULL);

                    /* Entry syntax */
                    cx_add_stx_start(s);
                    cx_add_stx_ulong(s, "vid", VTSS_VID_DEFAULT, VTSS_VID_RESERVED);
                    cx_add_stx_ulong(s, "msti", 1, N_MSTI_MAX-1);
                    cx_add_stx_end(s);
                    ...
                    for(i = 0; i < ARRSZ(conf.map.map); i++) {
                        if(conf.map.map[i]) {
                            cx_add_attr_start(s, CX_TAG_ENTRY);
                            cx_add_attr_ulong(s, "vid", i);
                            cx_add_attr_ulong(s, "msti", conf.map.map[i]);
                            CX_RC(cx_add_attr_end(s, CX_TAG_ENTRY));
                        }
                    }
                }
