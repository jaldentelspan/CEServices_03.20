...
mstp_vlan_ingress_filter(l2_port_no_t l2port, BOOL enable)
{
    ...
    if(l2port2port(l2port, &isid, &switchport)) {
        ...
        (void) vlan_mgmt_port_conf_set(isid, switchport, &vlan_pconf, VLAN_USER_MSTP);
    } else {
 ...
 */
static void
mstp_get_members(u8 msti, vid_t vid)
{
    uint count = 0;
    ...
    (void) switch_iter_init(&sit, VTSS_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        if(vlan_mgmt_vlan_get(sit.isid, vid, &vconf, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
            l2port_iter_t l2pit;
            (void) l2port_iter_init(&l2pit, sit.isid, L2PORT_ITER_TYPE_PHYS);
            while (l2port_iter_getnext(&l2pit)) {
                if(vconf.ports[l2pit.iport] == VLAN_MEMBER_PORT) {
 ...
 */
static void
mstp_conf_read(BOOL create, const mstp_macaddr_t *sysmac)
{
    mstp_conf_blk_t *blk;
    ulong           size;
    BOOL            do_create;
    ...
    if (do_create) {
        ...
        blk->conf.sys.bridgeMaxAge = 20; /* 17.14 - Table 17-1: Default recommended value */
        blk->conf.sys.bridgeHelloTime = 2; /* 17.14 - Table 17-1: Default recommended value */
        blk->conf.sys.bridgeForwardDelay = 15; /* 17.14 - Table 17-1: Default recommended value */
        blk->conf.sys.forceVersion = 3; /* 17.13.4 - The normal, default value */
        blk->conf.sys.txHoldCount = 6; /* 17.14 - Table 17-1: Default recommended value */
        blk->conf.sys.MaxHops = 20; /* 13.37.3 MaxHops */
        (void) misc_mac_txt(sysmac->mac, blk->conf.msti.configname);
        blk->conf.msti.revision = 0;
        uint i,j;
        for(i = 0; i < N_MSTI_MAX; i++)
            blk->conf.bridgePriority[i] = 0x80; /* 17.14 - Table 17-2: Default recommended value */
        for(i = 0; i < ARRSZ(blk->conf.portconfig); i++) {
            mstp_port_param_t *pp = &blk->conf.portconfig[i];
            blk->conf.stp_enable[i] = TRUE;
            pp->adminEdgePort = FALSE;
            pp->adminAutoEdgePort = TRUE;
            pp->adminPointToPointMAC = P2P_AUTO;
            for(j = 0; j < N_MSTI_MAX; j++) {
                mstp_msti_port_param_t *mpp = &blk->conf.msticonfig[i][j];
        ...
        blk->conf.stp_enable[MSTP_PORT_CONFIG_AGGR] = TRUE;
        blk->conf.portconfig[MSTP_PORT_CONFIG_AGGR].adminPointToPointMAC = P2P_FORCETRUE;
    }
#if defined(VTSS_MSTP_FULL)
    /* No limitations */
#else                                   
    if(blk->conf.sys.forceVersion > MSTP_PROTOCOL_VERSION_RSTP)
        /* Only allowed to use RSTP/STP */
        blk->conf.sys.forceVersion = MSTP_PROTOCOL_VERSION_RSTP; 
#endif  /* VTSS_MSTP_FULL */
        ...
        const vtss_packet_rx_info_t *const rx_info)
{
    ...
    // Don't allow other subscribers to receive the packet
    return TRUE; 
}
...
/*
 * Signal a specific VLAN as having configuration changes.
 */
static void mstp_vlan_changed(vtss_isid_t isid, vtss_vid_t vid, vlan_user_t user,
 ...
 */
static void
mstp_master_initialize(mstp_macaddr_t *sysmac)
{
    MSTP_ASSERT_LOCKED();
 ...
 */
static void
mstp_master_process(mstp_macaddr_t *sysmac)
{
    while(msg_switch_is_master()) {
        ...
        while((flags = cyg_flag_timed_wait(&mstp_global.control_flags, 0xffff, 
            ...
            if(flags & CTLFLAG_MSTP_DEFCONFIG) {
                mstp_conf_read(TRUE, sysmac); /* Reset stack configuration */
...
static void
mstp_thread(cyg_addrword_t data)
{
    packet_rx_filter_t rx_filter;
    static void *filter_id = NULL;
    static mstp_macaddr_t system_macaddr;
    ...
    vlan_membership_change_register(mstp_vlan_changed);
    ...
    for(;;) {
        ...
        if(msg_switch_is_master()) {
            ...
            mstp_master_initialize(&system_macaddr); /* Initialize state */

            MSTP_UNLOCK(); /* We were locked initializing - but open here */

            mstp_master_process(&system_macaddr); /* Process while beeing master */
