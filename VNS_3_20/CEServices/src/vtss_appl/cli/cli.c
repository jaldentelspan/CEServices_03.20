...
/****************************************************************************/
static int cli_print_stack_backtrace(cyg_thread_info *info, int (*print_function)(const char *fmt, ...), BOOL compiled_with_O2)
{
    ...
    if (print_function) {
        (void)print_function("#%-2d 0x%08x\n", cnt, regs->d[31]); // Print value of RA register. This is the closest we get to where the thread was interrupted.
    }
    cnt++;

    // Then start with the innermost stack frame and work the way out. d[29] == sp = "stack pointer"
    for (x = (cyg_uint32 *)regs->d[29]; x >= (cyg_uint32 *)info->stack_base && x < top_of_stack; x++) {
...
void cli_print_thread_status(int (*print_function)(const char *fmt, ...), BOOL backtrace)
{
    ...
    if (backtrace) {
        ...
        while (cyg_thread_get_next(&thread, &id) != 0) {
            ...
            if (info.name && strcasecmp(info.name, "Critd") == 0) {
                ...
                compiled_with_O2 = cli_print_stack_backtrace(&info, NULL, FALSE) < 4;
                break;
            }
        }
    ...
    while (cyg_thread_get_next(&thread, &id) != 0) {
        ...
        (void)print_function(" %5d\n", info.stack_used);
#else
        (void)print_function("   N/A\n");
#endif

        if (backtrace) {
            (void)cli_print_stack_backtrace(&info, print_function, compiled_with_O2);
        }
    }
    cyg_scheduler_unlock();
}
...
BOOL cli_io_login(cli_iolayer_t *pIO, vtss_auth_agent_t agent, int timeout)
{
    ...
    cli_puts("\n\n***************************************************");
    cli_puts(  "\n* Warning: Configuration changes will not persist *");
    cli_puts(  "\n*          unless config mode is enabled          *");
    cli_puts(  "\n* To enable, execute: 'CONFIG MODE ENABLE'        *");
    cli_puts(  "\n***************************************************\n");
