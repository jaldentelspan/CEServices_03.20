...
#endif
    {
        cli_print_thread_status(cli_printf, req->all);
    }
}
...
static void cli_cmd_debug_heap(cli_req_t *req)
{
    ...
    cli_printf("Total=0x%08x (%d KBytes) Free=0x%08x (%d KBytes) Max=0x%08x (%d KBytes)\n",
    ...
    if (req->stack.master) {
        ...
        for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
            ...
            if (msg_switch_info_get(isid, &info) == VTSS_RC_OK) {
                cli_printf("%-2d   VSC%-5x  %s\n", usid, info.api_inst_id, info.version_string);
            }
        }
    }
...
enum {
    ...
    CLI_CMD_DEBUG_HEAP_PRIO        = CLI_CMD_SORT_KEY_DEFAULT,
...
static void cli_parse_command(cli_io_t *pIO)
{
    ...
    if (match_count == 0) {
        ...
        if (help) {
            ...
            mod_req = malloc(cli_max_req_size);
            if (mod_req == NULL) {
