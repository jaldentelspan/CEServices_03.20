...
static int conf_mgmt_board_set_(conf_board_t *board) 
{
    ...
    {
        sprintf(str2, "%s%u\n", flash_conf_tag[i].tag, board->ies_board[i]);
        strcat(buf, str2);
    }
...
static void conf_board_start(void)
{
...
#if defined (BOARD_VNS_12_REF )
#endif
    BOOL ies_board_found[IES_FLASH_CONF_ITEMS];
    int i;
    for(i=0; i< FLASH_CONF_END_TAG; i++)
       ies_board_found[i]= FALSE; 
    
    T_D("enter");
    if ((base = malloc(FLASH_CONF_BOARD_SIZE)) && 
        ...
        if((p = strstr(base, board_sig))) {
            ulong board_id;
            p += strlen(board_sig);
            if (sscanf(p, "%x", &board_id) == 1) {
                board->board_id = board_id;
                T_D("Board ID: 0x%X", board_id);
                board_found = TRUE;
            }
        }
        ...
        for(i=0; i< FLASH_CONF_END_TAG; i++)
        {
             if((p = strstr(base, flash_conf_tag[i].tag ))) {
                 p += strlen(flash_conf_tag[i].tag);
                 if (sscanf(p, "%u", &board->ies_board[i]) == 1) {
                     T_D("%s:\t\t%u", flash_conf_tag[i].tag, board->ies_board[i]);
                     ies_board_found[i] = TRUE;
                 }
             }
        }
    } else {
    ...
    for(i=0; i< FLASH_CONF_END_TAG; i++)
    {
        if( !ies_board_found[i] ) {
            T_D("%s now fouud, setting to default", flash_conf_tag[i].tag);
            board->ies_board[i] = 0;
        }
    }
...
static conf_blk_t *conf_create_(conf_sec_t sec, conf_blk_id_t id, ulong size)
{
    ...
    if ((new = (conf_blk_t *)malloc(sizeof(conf_blk_t) + size)) == NULL) {
...
#if defined(VTSS_SW_OPTION_CONF_OS_FILE_ONLY)
    if (id != CONF_BLK_OS_FILE_CONF)
        return NULL;
#endif
...
static void conf_sec_read(conf_sec_t sec, uchar *ram_base, size_t size) 
{
    ...
    if (flash) {
        if((ram_base = malloc(hdr.size)) == NULL) {
...
#define COMPRESS_OVERHEAD 1024
    if ((msg = malloc(size + COMPRESS_OVERHEAD)) == NULL) {
                        ...
                        const vtss_module_id_t modid, ulong id)
{
    ...
    /* Compare with flash CRC to avoid unneccessary write operations */
    sec = CONF_SEC_GLOBAL;
    section = &conf.section[sec];
...
vtss_rc conf_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        /* fixes lost configuration issue */
        conf.conf.flash_save = 0;
