...
vtss_ptp_offset_filter_handle_t ptp_servo_create(const ptp_clock_servo_con_ds_t *c)
{
    ...
    vtss_ptp_offset_filter_handle_t h = malloc(sizeof(vtss_ptp_offset_filter_t) + sizeof(ptp_clock_servo_ds_t));
    if (h) {
...
vtss_ptp_delay_filter_handle_t ptp_delay_filter_create(ptp_clock_servo_con_ds_t *c, int port_count)
{
    ...
    vtss_ptp_delay_filter_handle_t s = malloc(sizeof(vtss_ptp_delay_filter_t) + sizeof(ptp_delay_filter_ds_t));
    if (s) {
        ...
        df = s->private_data;
        df->owd_filt = (one_way_delay_filter*)calloc(port_count+1, sizeof(one_way_delay_filter));
        df->con = c;
...
void ptp_delay_filter_delete(vtss_ptp_delay_filter_handle_t delay)
{
    ...
    if (delay && df->owd_filt) free(df->owd_filt);
    if (delay) free(delay);
}
