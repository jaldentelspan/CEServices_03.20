    ...
    const vtss_ptp_default_filter_config_t *of, int port_count)
{
    ...
    vtss_ptp_delay_filter_handle_t df = malloc(sizeof(vtss_ptp_delay_filter_t) + sizeof(vtss_ptp_wl_delay_filterprivate_data_t));
    if (df) {
        ...
        filter->owd_filt = (one_way_wl_delay_filter*)calloc(port_count+1, sizeof(one_way_wl_delay_filter));
        filter->filt_conf = of;
...
{
    vtss_ptp_wl_delay_filterprivate_data_t *filter = df->private_data;
    if (df && filter->owd_filt) free(filter->owd_filt);
    if (df) free(df);
}
