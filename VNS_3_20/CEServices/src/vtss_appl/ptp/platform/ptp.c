...
static void port_data_initialize(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        T_IG(VTSS_TRACE_GRP_PHY_TS, "Port_no = %d, ts_feature %d, channel %d, shared_port %d, in_sync %d", i, 
...
static void ptp_oam_slave_timer(vtss_timer_handle_t timer, void *m)
{
    ...
    if (config_data.conf[instance].clock_init.deviceType == PTP_DEVICE_SLAVE_ONLY && config_data.conf[instance].clock_init.protocol == PTP_PROTOCOL_OAM) {
        if (vtss_non_ptp_slave_check_port(ptp) == NULL) {
            ...
            if (vtss_mep_mgmt_conf_get(mep_inst, mac, &conf) != VTSS_RC_OK) {
                T_W("MISSING MEP instance");
            } else {
                if (conf.enable && conf.mode == VTSS_MEP_MGMT_MEP && conf.direction == VTSS_MEP_MGMT_INGRESS && conf.domain == VTSS_MEP_MGMT_PORT && conf.voe) {
                    rc = vtss_mep_mgmt_dm_conf_get(mep_inst, &config);
                    if (rc == VTSS_MEP_RC_INVALID_PARAMETER) {
                        T_W("MISSING OAM monitor");
                    } else {
                        if (!config.enable || config.calcway != VTSS_MEP_MGMT_FLOW || config.tunit != VTSS_MEP_MGMT_NS || !config.dm2fordm1) {
                            T_I("OAM monitor not set up correctly");
                            config.enable = TRUE;
                            config.calcway = VTSS_MEP_MGMT_FLOW;
                            config.tunit = VTSS_MEP_MGMT_NS;
                            config.dm2fordm1 = TRUE;
                            rc = vtss_mep_mgmt_dm_conf_set(mep_inst, &config);
                            if (rc != VTSS_MEP_RC_OK) {
                                T_W("OAM monitor configuration failed");
                            }
                        } else {
                            oam_timer_value = (config.gap*128)/100;
                            T_N("OAM timer rate %d msec pr packet, timer valus %d", config.gap*10, oam_timer_value);
            ...
            if (wireless_status.remote_pre || wireless_status.local_pre) {
                ...
                wireless_status.pre_cnt = 0;
                rc = vtss_mep_mgmt_dm_timestamp_get(instance, &far_to_near, &near_to_far);
                if (rc == VTSS_MEP_RC_OK) {
                ...
                } else if (rc == VTSS_MEP_RC_NO_TIMESTAMP_DATA) {
...
void
vtss_1588_update_encap_header(uchar *buf, BOOL uni, u16 len, int instance)
{
    ...
    if (ether_type == ip_ether_type) {
        ...
        port = vtss_tod_unpack16(buf+i+IP_HEADER_SIZE+2);
...
ptp_conf_init(ptp_instance_config_t *conf, UInteger8 defDomain)
{
    ...
    conf->time_prop.currentUtcOffset = DEFAULT_UTC_OFFSET;
    conf->time_prop.currentUtcOffsetValid = FALSE;
    conf->time_prop.leap59 = FALSE;
    conf->time_prop.leap61 = FALSE;
    conf->time_prop.timeTraceable = FALSE;
    conf->time_prop.frequencyTraceable = FALSE;
    conf->time_prop.ptpTimescale = TRUE; /* default in ITU profile */
    conf->time_prop.timeSource = 0xa0; /* indicates internal oscillator */
    port_iter_init_local(&pit);
    while (port_iter_getnext(&pit)) {
        i = pit.iport;
    ...
    conf->default_of.period = 1;
    conf->default_of.dist = 2;
    /* default servo config */
    conf->default_se.display_stats = FALSE;
    conf->default_se.p_reg = TRUE;
    conf->default_se.i_reg = TRUE;
    conf->default_se.d_reg = TRUE;
    conf->default_se.ap = DEFAULT_AP;
    conf->default_se.ai = DEFAULT_AI;
    conf->default_se.ad = DEFAULT_AD;
    conf->default_se.ho_filter = 60;
    conf->default_se.stable_adj_threshold = 300;  /* = 30 ppb */
    /* default delay filter  config*/
    conf->default_df.delay_filter = DEFAULT_DELAY_S;
    for (j = 0; j < MAX_UNICAST_MASTERS_PR_SLAVE; j++) {
        conf->unicast_slave[j].duration = 100;
        conf->unicast_slave[j].ip_addr = 0;
    }
    conf->slave_cfg.stable_offset = 100;
    conf->slave_cfg.offset_ok = 200;
    conf->slave_cfg.offset_fail = 1000;
...
ptp_conf_read(BOOL create)
{
    ...
    if (do_create) {
        ...
        config_data.init_ext_clock_mode.one_pps_mode = VTSS_PTP_ONE_PPS_DISABLE;
        config_data.init_ext_clock_mode.clock_out_enable = FALSE;
        config_data.init_ext_clock_mode.vcxo_enable = FALSE;
        config_data.init_ext_clock_mode.freq = 1;
#if defined (VTSS_ARCH_SERVAL)
                             ...
                             int instance)
{
    ...
    //memcpy(&receiver.sin_addr, &dest_ip, sizeof(dest_ip));
...
//    ptp_global.my_ip.s_addr = ntohl(my_addr.sin_addr.s_addr);

    n=write(ptp_sock,buffer,size);
    if (n < 0) {
        T_EG(_I,"write returned %d",n);
        return 0;
    }
    return n;
}
    ...
    if (rules[i].deviceType != PTP_DEVICE_NONE) {
        ...
        if (rules[i].deviceType != PTP_DEVICE_NONE) {
            ...
            for (j=0; j < MAX_ACL_RULES_PR_PTP_CLOCK; j++) {
                ...
                T_DG(_I,"clear ACL rules for inst %d", i);
            }
        }
    } 
    if (config_data.conf[i].clock_init.deviceType != PTP_DEVICE_NONE) {
...
static BOOL ptp_eng_alloc_table [PTP_CLOCK_PORTS] [2];
...
static void phy_ts_rules_init(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        j = pit.iport;
        ptp_eng_alloc_table[j][0] = FALSE;
        ptp_eng_alloc_table[j][1] = FALSE;
    }
}

static vtss_phy_ts_engine_t ptp_eng_alloc(vtss_port_no_t port_no)
{
    vtss_phy_ts_engine_t i;
    for (i = VTSS_PHY_TS_PTP_ENGINE_ID_0; i <= VTSS_PHY_TS_PTP_ENGINE_ID_1; i++) {
        if (!ptp_eng_alloc_table[port_no][i]) {
            ptp_eng_alloc_table[port_no][i] = TRUE;
            if (port_data[port_no].topo.port_shared) {
                ptp_eng_alloc_table[port_data[port_no].topo.shared_port_no][i] = TRUE;
            }
            return i;
        }
    }
...
static void ptp_eng_free(vtss_port_no_t port_no, vtss_phy_ts_engine_t id)
{
    if (ptp_eng_alloc_table[port_no][id]) {
        ptp_eng_alloc_table[port_no][id] = FALSE;
        if (port_data[port_no].topo.port_shared) {
            ptp_eng_alloc_table[port_data[port_no].topo.shared_port_no][id] = FALSE;
        }
    }
}
 ...
 * \note This function is callew whenever the PTP configuration for a ptp
 *  instance change.
...
static vtss_rc ptp_phy_ts_update(int inst)
{
    ...
    port_iter_t       pit;
    port_iter_init_local(&pit);
    if (inst == 0 && phy_ts_rules[inst].deviceType == PTP_DEVICE_SLAVE_ONLY && config_data.conf[inst].clock_init.oneWay &&
        my_tc_instance > 0) {
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (phy_ts_rules[inst].port[j].phy_ts_port) {
            ...
            if (phy_ts_rules[inst].port[j].engine_id == VTSS_PHY_TS_ENGINE_ID_INVALID) { /* if not already allocated */
                phy_ts_rules[inst].port[j].engine_id = ptp_eng_alloc(j);
            ...
            if (engine_free && phy_ts_rules[inst].port[j].engine_id != VTSS_PHY_TS_ENGINE_ID_INVALID) { /* if not used any more */
                ...
                ptp_eng_free(j,phy_ts_rules[inst].port[j].engine_id);
...
ptp_port_link_state_initial(int instance)
{
    ...
    for (portidx = 0; portidx < config_data.conf[instance].clock_init.portCount; portidx++) {
        if (port_mgmt_status_get(VTSS_ISID_START, portidx, &ps) == VTSS_OK) {
...
static void one_pps_external_input_servo_action(i32                         freq_offset)
{
    if (one_pps_first_time) {
        one_pps_first_time = FALSE; /* ignore the first reading, as it will have some random value */
        return;
    } else {
...
ptp_thread(cyg_addrword_t data)
{
	...
	while (!tod_ready()) {
		T_D("wait until TOD module is ready");
        ...
        while (ptp_global.ptpi[0]) {
            ...
            for (i = 0; i < PTP_CLOCK_INSTANCES; i++) {
                if (config_data.conf[i].clock_init.deviceType != PTP_DEVICE_NONE) {
                    ...
                    if (config_data.conf[i].clock_init.protocol != PTP_PROTOCOL_OAM) {
...
#endif
#if defined(VTSS_ARCH_SERVAL)
    PTP_RC(vtss_ts_p2p_delay_set(API_INST_DEFAULT, ptp_api_to_l2port(portnum), &p2p_delay));
#endif
    }
}
...
#if defined(VTSS_ARCH_SERVAL)
        PTP_RC(vtss_ts_delay_asymmetry_set(API_INST_DEFAULT, ptp_api_to_l2port(portnum), &asymmetry));
#endif
...
ptp_clock_create(const ptp_init_clock_ds_t *initData, uint instance)
{
    ...
    if (PTP_READY() && (instance < PTP_CLOCK_INSTANCES)) {
        ...
        if (initData->deviceType == PTP_DEVICE_SLAVE_ONLY && initData->protocol == PTP_PROTOCOL_OAM) {
            ...
            T_W("Chip ID = %x, rev = %d",chip_id.part_number, chip_id.revision);
            ...
            if (ptp_ace_update(instance) != VTSS_OK) T_EG(0, "ACE add error");
    ...
    if (PTP_READY() && (instance < PTP_CLOCK_INSTANCES)) {
        if (config_data.conf[instance].clock_init.deviceType != PTP_DEVICE_NONE) {
            if (0 < portnum && portnum <= config_data.conf[instance].clock_init.portCount) {
                port_instances = 0;
                for (i = 0; i < PTP_CLOCK_INSTANCES; i++) {
                    if (config_data.conf[i].port_config[ptp_api_to_l2port(portnum)].initPortState) {
                        port_instances++;
                    }
                }
                /* only allow limited number of active instances pr port */
                if (port_instances < PTP_MAX_INSTANCES_PR_PORT) {
                    config_data.conf[instance].port_config[ptp_api_to_l2port(portnum)].initPortInternal = internal;
                    ok = vtss_ptp_port_ena(ptp_global.ptpi[instance], portnum);
...
ptp_uni_slave_conf_set(const vtss_ptp_unicast_slave_config_t *c, uint ix, uint instance)
{
    BOOL ok = FALSE;
    PTP_CORE_LOCK();
    if (PTP_READY() && (instance < PTP_CLOCK_INSTANCES) && ix < MAX_UNICAST_MASTERS_PR_SLAVE) {
        if (config_data.conf[instance].clock_init.deviceType != PTP_DEVICE_NONE) {
...
static void ext_clock_out_set(const vtss_ptp_ext_clock_mode_t *mode)
{
    ...
    if (mode->one_pps_mode == VTSS_PTP_ONE_PPS_INPUT) {
        ...
        default_get.clockQuality.clockAccuracy = 0x21;
...
/* Get serval rs424 external clock configuration. */
...
ptp_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_MASTER_UP:
        T_IG(0, "INIT_CMD_MASTER_UP Starting PTP thread - size global = %u bytes", sizeof(ptp_global));
        cyg_thread_resume(ptp_thread_handle);
        break;
    case INIT_CMD_SWITCH_ADD:
        T_IG(0, "INIT_CMD_SWITCH_ADD - ISID %u", isid);
...
BOOL get_ptp_master_conf_size(config_transport_t *conf)
{
	conf->data.size = sizeof(ptp_config_t);
	if(conf->data.size == 0)
		return FALSE;
	return TRUE;
}
BOOL get_ptp_master_config(const config_transport_t *conf)
{
	BOOL result = TRUE;
	size_t ptp_conf_size = sizeof(ptp_config_t);
	if(conf->data.size >= ptp_conf_size) {
		memcpy(conf->data.pointer.c, &config_data, ptp_conf_size);
	}
	else {
		result = FALSE;
	}
...
BOOL set_ptp_master_config(const config_transport_t *conf)
{
    ptp_config_t    *config;  /* run-time configuration data */
    ptp_config_t    *conf_dat = (ptp_config_t *)conf->data.pointer.v;
    ulong size;
    BOOL result = TRUE;
	size_t ptp_conf_size = sizeof(ptp_config_t);
    if(ptp_conf_size != conf->data.size) {
    	T_E("ptp_conf_size, %u, was not equal to buffer_size, %u", ptp_conf_size, conf->data.size);

    	return FALSE;
    }
    PTP_CORE_LOCK();
    if ((config = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_PTP_CONF, &size)) != NULL) {
        if (size == sizeof(*config)) {
            T_IG(0, "Saving configuration");
            *config = *conf_dat;
        }
        else {
           	T_E("size was not equal to sizeof(*config)");
                	result = FALSE;
        }
        conf_sec_close(CONF_SEC_GLOBAL, CONF_BLK_PTP_CONF);
        ptp_conf_read(FALSE); /* Reset configuration */
        /* Make PTP configuration effective in PTP core */
        ptp_conf_propagate();
    }
    else {
