...
static void ptp_display_clock_default_ds(cli_req_t *req)
{
    ...
    ptp_req = ( ptp_cli_req_t * )req->module_req;

    cli_table_header("ClockId  DeviceType  2StepFlag  Ports  ClockIdentity            Dom");
    ...
    if (ptp_get_clock_default_ds(&bs,ptp_req->ptp_instance_no)) {
        ...
        CPRINTF("%-9d%-12s\n", ptp_req->ptp_instance_no, DeviceTypeToString(PTP_DEVICE_NONE));
    }
}
...
static void ptp_display_slave_config(cli_req_t *req)
{
    ...
    cli_table_header("Stable Offset  Offset Ok      Offset Fail");

    if (ptp_get_clock_slave_config(&slave_cfg,ptp_req->ptp_instance_no)) {
...
static void ptp_display_clock_current_ds(cli_req_t *req)
{
    ...
    cli_table_header("stpRm  OffsetFromMaster    MeanPathDelay       ");

    if (ptp_get_clock_current_ds(&bs,ptp_req->ptp_instance_no)) {
        CPRINTF("%-7d%-20s%s\n",
...
static void ptp_display_clock_timeproperties_ds(cli_req_t *req)
{
    ...
    cli_table_header("UtcOffset  Valid  leap59  leap61  TimeTrac  FreqTrac  ptpTimeScale  TimeSource");
    if (ptp_get_clock_timeproperties_ds(&bs,ptp_req->ptp_instance_no)) {
        CPRINTF("%-11d%-7s%-8s%-8s%-10s%-10s%-14s%-10d\n",
                bs.currentUtcOffset,
...
static void ptp_display_port_ds(cli_req_t *req)
{
    ...
    cli_table_header("Port  Stat  MDR  PeerMeanPathDel  Anv  ATo  Syv  Dlm  MPR  DelayAsymmetry   IngressLatency   EgressLatency    Ver");
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        if (req->uport_list[iport2uport(port_no)] == 0)
            continue;
        if (ptp_get_port_ds(&bs, iport2uport(port_no),ptp_req->ptp_instance_no)) {
...
static void ptp_display_clock_parent_ds(cli_req_t *req)
{
    ...
    cli_table_header("ParentPortIdentity      port  Pstat  Var  ChangeRate  ");
    if (ptp_get_clock_parent_ds(&bs,ptp_req->ptp_instance_no)) {
        CPRINTF("%-24s%-6d%-7s%-5d%-12d\n",
        ...
        CPRINTF("\n");
        cli_table_header("GrandmasterIdentity      GrandmasterClockQuality    Pri1  Pri2");
        CPRINTF("%-25s%-27s%-6d%-6d\n",
...
static void ptp_display_port_foreign_masters(cli_req_t *req)
{
    ...
    cli_table_header("Port  ForeignmasterIdentity         ForeignmasterClockQality     Pri1  Pri2  Qualif  Best ");
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        if (req->uport_list[iport2uport(port_no)] == 0)
            continue;
        /* do */
        for (ix = 0; ix < DEFAULT_MAX_FOREIGN_RECORDS; ix++) {
            if (ptp_get_port_foreign_ds(&bs,iport2uport(port_no),ix,ptp_req->ptp_instance_no)) {
                CPRINTF("%-4d  %-27s%-4d  %-27s%-6d%-6d%-8s%-5s\n",
                        iport2uport(port_no),
...
static void cli_cmd_ptp_local_clock(cli_req_t *req)
{
    ...
    if (ptp_req->ptp_localClockCmd == 1) {
        ...
        vtss_local_clock_time_get(&t,ptp_req->ptp_instance_no, &hw_time);
        CPRINTF("PTP Time (%d)    : %s %s\n", ptp_req->ptp_instance_no, misc_time2str(t.seconds), vtss_tod_ns2str(t.nanoseconds, str,','));
        CPRINTF("Clock Adjustment method: %s\n", clock_adjustment_method_txt[vtss_ptp_adjustment_method(ptp_req->ptp_instance_no)]);
...
static void cli_cmd_ptp_clock_servo(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        cli_table_header("Display  P-enable  I-enable  D-enable  'P'constant  'I'constant  'D'constant");
        if (ptp_default_servo_parameters_get(&default_servo, ptp_req->ptp_instance_no)) {
            CPRINTF("%-9s%-10s%-10s%-10s%-13d%-13d%-13d\n",
...
static void cli_cmd_ptp_clock_servo_options(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        /* show the servo parameters  */
        cli_table_header("Option  threshold  'P'constant");
        if (ptp_default_servo_parameters_get(&default_servo, ptp_req->ptp_instance_no)) {
            CPRINTF("%-9s%-11d%-13d\n",
...
static void cli_cmd_ptp_clock_servo_holdover(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        cli_table_header("Holdover filter  Adj threshold (ppb)");
        if (ptp_default_servo_parameters_get(&default_servo, ptp_req->ptp_instance_no)) {
            CPRINTF("%15d  %17d.%d\n",
        ...
        if (ptp_default_servo_status_get(&status, ptp_req->ptp_instance_no)) {
            cli_table_header("Holdover Ok  Holdover offset (ppb)");
            CPRINTF("%-11s  %19d.%ld\n",
...
static void cli_cmd_ptp_clock_filter(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        cli_table_header("DelayFilter  period  dist");
        if (ptp_default_filter_parameters_get(&default_offset, ptp_req->ptp_instance_no) &&
                ptp_default_delay_filter_parameters_get(&default_delay, ptp_req->ptp_instance_no)) {
            CPRINTF("%-13d%-8d%-8d\n",
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        ...
        if (req->set) {
            if (req->enable || ptp_req->ptp_internal) {
                if (!ptp_port_ena(ptp_req->ptp_internal, iport2uport(port_no),ptp_req->ptp_instance_no) && !first_err) {
                    CPRINTF("Clock instance %d : does not exist or timestamp feature not available\n",
                            ptp_req->ptp_instance_no);
                    first_err = TRUE;
                }
            } else if (req->disable) {
            ...
            if (first) {
                ...
                p += sprintf(p, "Port  ");
                p += sprintf(p, "State  ");
                cli_table_header("Port  PTP-State  Internal  Link  Port-Timer  Vlan-forw  Phy-timestamper  Peer-delay");
                first = 0;
            }
            if (ptp_get_port_ds(&bs, iport2uport(port_no),ptp_req->ptp_instance_no) &&
                ptp_get_port_link_state(&ds, iport2uport(port_no), ptp_req->ptp_instance_no)) {
                CPRINTF("%4d  %-9s  %-8s  %-4s  %-10s  %-9s  %-15s  %-10s\n",
                        bs.portIdentity.portNumber,
...
static void cli_cmd_ptp_clock_unicast_conf(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        cli_table_header("index  duration  ip_address       grant  CommState");
        ix = 0;
        while (ptp_uni_slave_conf_state_get(&conf_state, ix++, ptp_req->ptp_instance_no)) {
            CPRINTF("%-7d%-10d%-17s%-7d%-9s\n",
                    ix-1,
                    conf_state.duration,
...
static void cli_cmd_ptp_clock_unicast_table(cli_req_t *req)
{
    ...
    cli_table_header("ip_addr          stat  mac_addr           port  sourceportidentity           grant ");
    while (ptp_clock_unicast_table_get(&uni_slave_table, ix++, ptp_req->ptp_instance_no)) {
        if (uni_slave_table.conf_master_ip) {
            if (uni_slave_table.comm_state >= PTP_COMM_STATE_CONN) {
                CPRINTF("%-17s%-6s%-19s%-6d%-24s,%-3d %-5d\n",
...
static void cli_cmd_ptp_clock_unicast_master_table(cli_req_t *req)
{
    ...
    cli_table_header("ip_addr          mac_addr           port  Ann  Sync ");
    while (ptp_clock_unicast_master_table_get(&uni_master_table, ix++, ptp_req->ptp_instance_no)) {
        if (uni_master_table.ann) {
            if (uni_master_table.sync) {
                CPRINTF("%-17s%-19s%-6d%-4d %-4d\n",
...
static void cli_cmd_ptp_ext_clock_mode(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        CPRINTF("PTP External One PPS mode: %s, Clock output enabled: %s, frequency : %d, VCXO enable: %s\n", 
...
static void cli_cmd_ptp_rs422_mode(cli_req_t *req)
{
    ...
    if (req->set) {
        ...
        CPRINTF("PTP RS422 clock mode: %s, delay : %d, Protocol: %s, port: %d\n", 
...
static void cli_cmd_ptp_wireless_delay(cli_req_t *req)
{
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < VTSS_PORT_NO_END; port_no++) {
        ...
        if (first) {
            cli_table_header("Port  Wireless Mode  Base_delay(ns)  Incr_delay(ns) ");
            first = 0;
        }
        if (ptp_port_wireless_delay_mode_get(&mode, iport2uport(port_no),ptp_req->ptp_instance_no) &&
                    ptp_port_wireless_delay_get(&delay_cfg, iport2uport(port_no), ptp_req->ptp_instance_no)) {
            CPRINTF("%4u  %-13s  %10d.%03d  %10d.%03d\n", 
                    iport2uport(port_no), mode ? "Enabled" : "Disabled",
...
static void ptp_display_clock_slave_ds(cli_req_t *req)
{
    ...
    cli_table_header("Slave port  Slave state    Holdover(ppb)  ");
    if (ptp_get_clock_slave_ds(&ss,ptp_req->ptp_instance_no)) {
        CPRINTF("%-10d  %-13s  %-13s\n",
                ss.port_number,
        ...
        char *cmd_org, cli_req_t *req)
{
    ...
    ptp_req = req->module_req;
    req->parm_parsed = 1;
    error = cli_parse_ulong(cmd, &value, VTSS_VID_NULL, VTSS_VID_RESERVED); 
    if (!error) {
...
static cli_parm_t ptp_cli_parm_table[] = {
    ...
    },
    {
        ...
        "        sub       : RS422 clock in sub mode (save time and load new time atL/S input.\n",
        CLI_PARM_FLAG_SET,
    ...
    },
    {
        ...
        "        pim       : Use pim protocol to transfer 1PPS informationover a switch port\n",
        CLI_PARM_FLAG_SET,
...
cli_cmd_tab_entry (
    "PTP PIM Statistics [<port_list>]",
    "PTP PIM Statistics [<port_list>] [clear]",
    "Show or clear the PIM protocil statistics",
    PRIO_ID_PTP_PIM_STATISTICS,
    CLI_CMD_TYPE_STATUS,
    VTSS_MODULE_ID_PTP,
