...
/* Tag table */
static cx_tag_entry_t ptp_cx_tag_table[CX_TAG_NONE + 1] =
{
    [CX_TAG_PTP] = {
        .name  = "ptp",
        .descr = "PTP",
...
static vtss_rc ptp_cx_parse_func(cx_set_state_t *s)
{
	int err;
    switch (s->cmd) {
...
static vtss_rc ptp_cx_gen_func(cx_get_state_t *s)
{
    switch (s->cmd) {
    ...
    case CX_GEN_CMD_SWITCH:
    	cx_add_tag_line(s, CX_TAG_PTP, 0);
    	{
    	...
    	CX_RC(cx_add_tag_line(s, CX_TAG_PTP, 1));
        break;
    default:
        T_E("Unknown command");
...
CX_MODULE_TAB_ENTRY(
    ...
    0,
    NULL,                /* init function       */
	ptp_cx_gen_func,     /* Generation fucntion */
	ptp_cx_parse_func    /* parse fucntion      */
);
