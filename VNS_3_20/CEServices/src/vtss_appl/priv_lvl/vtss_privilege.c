...
static void VTSS_PRIVILEGE_default_set(vtss_priv_conf_t *conf)
{
    int i;
...
static void VTSS_PRIVILEGE_conf_read_stack(BOOL create)
{
    BOOL                    do_create;
    u32                     size;
    vtss_priv_conf_t        new_privilege_conf;
    ...
    blk_version = PRIVILEGE_CONF_BLK_VERSION;
    ...
    if (do_create) {
        /* Use default values */
        VTSS_PRIVILEGE_default_set(&new_privilege_conf);
        if (conf_blk_p != NULL) {
            conf_blk_p->privilege_conf = new_privilege_conf;
        }
    } else {
        /* Use new configuration */
        new_privilege_conf = conf_blk_p->privilege_conf;
    }
    VTSS_PRIVILEGE_global.privilege_conf = new_privilege_conf;
    PRIVILEGE_CRIT_EXIT();
    if (conf_blk_p == NULL) {
        T_W("failed to open privilege table");
    } else {
...
static void VTSS_PRIVILEGE_start(void)
{
    ...
    VTSS_PRIVILEGE_default_set(conf_p);
...
{
    int i;
    char name_lc[32], module_name_lc[32];
    int  mid_match = -1;

    T_D("enter");

    /* Convert name to lowercase */
    strncpy(name_lc, name, 31);
    name_lc[31] = 0;
    ...
    for (i = 0; i < VTSS_MODULE_ID_NONE; i++) {
        ...
        strncpy(module_name_lc, vtss_module_names[i], 31);
        module_name_lc[31] = 0;
...
/**
  * \brief Getnext privilege group name
  *
  ...
  *    FALSE otherwise.\n
  */
BOOL vtss_privilege_group_name_getnext(char *name, vtss_module_id_t *module_id_p)
{
    int i, found = 0;
    char upper_name[32] = "";
    ...
    for (i = 0; i < VTSS_MODULE_ID_NONE; i++) {
        ...
        if (found) {
            if (strcmp(vtss_module_names[i], name) > 0 && strcmp(vtss_module_names[i], upper_name) < 0) {
                strcpy(upper_name, vtss_module_names[i]);
                *module_id_p = (vtss_module_id_t) i;
            }
        } else if (strcmp(vtss_module_names[i], name) > 0) {
            strcpy(upper_name, vtss_module_names[i]);
            *module_id_p = (vtss_module_id_t) i;
            found = 1;
        }
