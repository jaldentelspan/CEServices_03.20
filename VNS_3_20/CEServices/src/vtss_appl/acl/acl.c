                                ...
                                vtss_isid_t *isid_del)
{
    ...
    if (rc == VTSS_OK && new == NULL && list->free == NULL) {
        ...
        if ((new = malloc(sizeof(acl_ace_t))) == NULL) {
            T_D("exit");
            ACL_CRIT_EXIT();
...
static BOOL acl_packet_rx(void *contxt, const uchar *const frame, const vtss_packet_rx_info_t *const rx_info)
{
...
#ifdef VTSS_SW_OPTION_SYSLOG
        S_I("Port %u shut down", port_no + 1);
#endif
        T_D("Port %u shut down", port_no);
    ...
    if (found_matched && ACL_USER_ID_GET(ace->conf.id) != ACL_USER_STATIC) {
        return FALSE;
    }
