...
static void ACL_ICFG_ace_flag(acl_entry_conf_t *conf_p, char *ace_str_p, acl_flag_t flag, BOOL *first_flag_p, char *flag_str_p)
{
    ...
    strcpy(str_buf, mgmt_acl_flag_txt(conf_p, flag, TRUE));
                                    ...
                                    vtss_icfg_query_result_t *result)
{
    ...
    acl_entry_conf_t        ace_conf, next_ace_conf;
    vtss_usid_t             usid_idx, first_existing_usid = VTSS_ISID_GLOBAL;
    ...
    for (policer_idx = ACL_POLICER_NO_START; policer_idx < ACL_POLICER_NO_END; policer_idx++) {
        ...
        if (!policer_conf_all_the_same) {
            sprintf(str_buf, " %u", policer_idx);
        }

        //rate-limiter
        if (req->all_defaults ||
            (policer_conf_changed && policer_conf.packet_rate != def_policer_conf.packet_rate)) {
    ...
    //found first existing usid
    for (usid_idx = VTSS_USID_START; usid_idx < VTSS_USID_END; usid_idx++) {
        if (msg_switch_exists(topo_usid2isid(usid_idx))) {
            first_existing_usid = usid_idx;
            break;
        }
    }

    while (acl_mgmt_ace_get(ACL_USER_STATIC, VTSS_ISID_GLOBAL, ace_id, &ace_conf, NULL, TRUE) == VTSS_OK) {
        ace_id = ace_conf.id;

        sprintf(ace_str, "%s %d", ACL_ACE_TEXT, ace_id);

        //next_ace_id
        if (acl_mgmt_ace_get(ACL_USER_STATIC, VTSS_ISID_GLOBAL, ace_id, &next_ace_conf, NULL, TRUE) == VTSS_OK) {
            sprintf(ace_str + strlen(ace_str), " next %u", next_ace_conf.id);
        }
        ...
        if (ace_conf.isid != VTSS_ISID_GLOBAL && port_list_cnt == VTSS_PORT_NO_END) {
            ...
            sprintf(ace_str + strlen(ace_str), " %s %s %s",
                    ACL_INGRESS_TEXT,
                    ACL_INTERFACE_TEXT,
                    icli_port_list_info_txt(ace_conf.isid, ace_conf.action.port_list, str_buf));
        ...
        if (port_list_cnt != VTSS_PORT_NO_END) { //specific switch port No.
            sprintf(ace_str + strlen(ace_str), " %s %s %s",
                    ACL_INGRESS_TEXT,
                    ACL_INTERFACE_TEXT,
                    icli_port_list_info_txt(VTSS_ISID_START, ace_conf.action.port_list, str_buf));
        }
#endif /* VTSS_SWITCH_STACKABLE */
        ...
        //frametype
        if (acl_mgmt_ace_init(ace_conf.type, &next_ace_conf) != VTSS_OK) {
            continue;
        }
        switch (ace_conf.type) {
        case VTSS_ACE_TYPE_ETYPE:
            sprintf(ace_str + strlen(ace_str), " %s %s", ACL_FRAMETYPE_TEXT, ACL_ETYPE_TEXT);
            if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.etype)) == 0) {
                sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
            } else {
                //etype
        ...
        case VTSS_ACE_TYPE_ARP:
            ...
            if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.arp)) == 0) {
                sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
            } else {
                //sip
        ...
        case VTSS_ACE_TYPE_IPV4:
            if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv4)) == 0) {
                sprintf(ace_str + strlen(ace_str), " %s %s %s", ACL_FRAMETYPE_TEXT, ACL_ARP_TEXT, ACL_ANY_TEXT);
            } else {
                //ipv4_protocol
                next_ace_conf.frame.ipv4.proto.mask = 0xFF;
                switch (ace_conf.frame.ipv4.proto.value) {
                case 1: //icmp
                    sprintf(ace_str + strlen(ace_str), " %s %s", ACL_FRAMETYPE_TEXT, ACL_ICMP_TEXT);
                    next_ace_conf.frame.ipv4.proto.value = 1;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv4)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                case 6: //tcp
                    ...
                    next_ace_conf.frame.ipv4.proto.value = 6;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv4)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                case 17: //udp
                    ...
                    next_ace_conf.frame.ipv4.proto.value = 17;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv4)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                default:
                    sprintf(ace_str + strlen(ace_str), " %s %s %s %u",
                            ACL_FRAMETYPE_TEXT,
                            ACL_IP_TEXT,
        ...
        case VTSS_ACE_TYPE_IPV6:
            if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv6)) == 0) {
                sprintf(ace_str + strlen(ace_str), " %s %s %s", ACL_FRAMETYPE_TEXT, ACL_IPV6_TEXT, ACL_ANY_TEXT);
            } else {
#if defined(VTSS_FEATURE_ACL_V2)
                next_ace_conf.frame.ipv6.proto.mask = 0xFF;
                switch (ace_conf.frame.ipv6.proto.value) {
                case 58: //icmp
                    sprintf(ace_str + strlen(ace_str), " %s %s", ACL_FRAMETYPE_TEXT, ACL_IPV6_ICMP_TEXT);
                    next_ace_conf.frame.ipv6.proto.value = 1;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv6)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                case 6: //tcp
                    ...
                    next_ace_conf.frame.ipv6.proto.value = 6;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv6)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                case 17: //udp
                    ...
                    next_ace_conf.frame.ipv6.proto.value = 17;
                    if (memcmp(&ace_conf.frame, &next_ace_conf.frame, sizeof(ace_conf.frame.ipv6)) == 0) {
                        sprintf(ace_str + strlen(ace_str), " %s", ACL_ANY_TEXT);
                    }
                    break;
                                  ...
                                  vtss_icfg_query_result_t *result)
{
    ...
    if (req->all_defaults ||
        ...
        rc += vtss_icfg_printf(result, " %s%s %s%s%s%s%s\n",
                               ...
                               conf.action.port_action != VTSS_ACL_PORT_ACTION_REDIR ? "" : " ",
                               conf.action.port_action != VTSS_ACL_PORT_ACTION_REDIR ? "" : icli_port_list_info_txt(isid, conf.action.port_list, str_buf));
    }
#else //VTSS_FEATURE_ACL_V2
    if (req->all_defaults ||
...
vtss_rc acl_icfg_init(void)
{
    ...
    */
    if ((rc = vtss_icfg_query_register(VTSS_ICFG_ACL_GLOBAL_CONF, ACL_ICFG_global_conf)) != VTSS_OK) {
        return rc;
    }

    rc = vtss_icfg_query_register(VTSS_ICFG_ACL_PORT_CONF, ACL_ICFG_port_conf);
