...
static vtss_rc cx_poe_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    if (ports == NULL) {
        ...
        cx_add_stx_start(s);
        cx_add_stx_port(s);
        cx_add_attr_txt(s, "poe_mode", "disabled,poe_af or poe+");
        cx_add_attr_txt(s, "prio", "low, high or critical");
        cx_add_attr_txt(s, "max_power", "Power in deciWatts. 0-300");
        return cx_add_stx_end(s);
    }

    cx_add_port_start(s, CX_TAG_ENTRY, ports);
    cx_add_attr_kw(s, "poe_mode", cx_kw_poe_mode, poe_local_conf.poe_mode[port_no]);
    cx_add_attr_kw(s, "prio", cx_kw_poe_prio, poe_local_conf.priority[port_no]);
    cx_add_attr_long(s, "max_power", poe_local_conf.max_port_power[port_no]);
...
static vtss_rc poe_cx_gen_func(cx_get_state_t *s)
{
    ...
    case CX_GEN_CMD_GLOBAL:
        ...
        //////////////////
        T_D("global - poe");
        cx_add_tag_line(s, CX_TAG_POE, 0);
        ...
        // Help text
        cx_add_stx_start(s);
        cx_add_stx_kw(s, "poe_mgmt", cx_kw_poe_mgmt);
        cx_add_stx_end(s);

        cx_add_attr_start(s, CX_TAG_POE_MGMT);
        cx_add_attr_kw(s, "poe_mgmt", cx_kw_poe_mgmt,
                       poe_master_conf.power_mgmt_mode);
        cx_add_attr_end(s, CX_TAG_POE_MGMT);
    ...
    case CX_GEN_CMD_SWITCH:
        ...
        cx_add_tag_line(s, CX_TAG_POE, 0);
        cx_add_val_ulong(s, CX_TAG_POE_PRIMARY_POWER_SUPPLY,
                         poe_local_conf.primary_power_supply, 0,
                         POE_POWER_SUPPLY_MAX);
        cx_add_val_ulong(s, CX_TAG_POE_BACKUP_POWER_SUPPLY,
