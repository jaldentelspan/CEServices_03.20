...
#endif
#endif
    if (rc != VTSS_RC_OK)       vtss_eps_trace("run_create: protection config failed", instance_data[instance].param.w_flow, 0, 0, 0);
                                  ...
                                  const vtss_eps_mgmt_create_param_t    *const param)
{
    ...
    if (instance_data[instance].created)                       return(VTSS_EPS_RC_CREATED);
    if (param->domain != VTSS_EPS_MGMT_PORT)                   return(VTSS_EPS_RC_INVALID_PARAMETER);
    if ((param->architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1) && 
        (param->domain != VTSS_EPS_MGMT_PORT))                 return(VTSS_EPS_RC_ARCHITECTURE);
    if (param->w_flow == param->p_flow)                        return(VTSS_EPS_RC_W_P_EQUAL);
    ...
    for (i=0; i<VTSS_EPS_MGMT_CREATED_MAX; ++i)
    {
        if ((i != instance) && instance_data[i].created)
        {
            ...
            {
                if (param->w_flow == instance_data[i].param.w_flow)       return(VTSS_EPS_RC_WORKING_USED);
                if ((instance_data[i].param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1) || (param->architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1))
                    if (param->p_flow == instance_data[i].param.p_flow)   return(VTSS_EPS_RC_PROTECTING_USED);
            }
        }
    }
...
{
    if (instance >= VTSS_EPS_MGMT_CREATED_MAX)          return(VTSS_EPS_RC_INVALID_PARAMETER);
    if (!instance_data[instance].created)               return(VTSS_EPS_RC_NOT_CREATED);
                           ...
                           const vtss_eps_mgmt_conf_t   *const conf)
{
    ...
    if (!instance_data[instance].created)                                                 return(VTSS_EPS_RC_NOT_CREATED);
    if (instance_data[instance].configured &&
        !memcmp(&instance_data[instance].config, conf, sizeof(vtss_eps_mgmt_conf_t)))     return(VTSS_EPS_RC_OK);
    if (conf->restore_timer > VTSS_EPS_MGMT_WTR_MAX)                                      return(VTSS_EPS_RC_INVALID_PARAMETER);
    if (conf->hold_off_timer > VTSS_EPS_MGMT_HOFF_MAX)                                    return(VTSS_EPS_RC_INVALID_PARAMETER);
    if ((instance_data[instance].param.architecture ==  VTSS_EPS_MGMT_ARCHITECTURE_1F1) &&
        ((conf->directional != VTSS_EPS_MGMT_BIDIRECTIONAL) || !conf->aps))               return(VTSS_EPS_RC_INVALID_PARAMETER);
                           ...
                           vtss_eps_mgmt_conf_t              *const conf)
{
    ...
    *param = instance_data[instance].param;
    if (!instance_data[instance].configured)            return(VTSS_EPS_RC_NOT_CONFIGURED);
                              ...
                              const vtss_eps_mgmt_command_t     command)
{
    ...
    if (!instance_data[instance].created)                                                                                       return(VTSS_EPS_RC_NOT_CREATED);
    if (!instance_data[instance].configured)                                                                                    return(VTSS_EPS_RC_NOT_CONFIGURED);
    if (command == instance_data[instance].command)                                                                             return(VTSS_EPS_RC_OK);
    if ((instance_data[instance].command != VTSS_EPS_MGMT_COMMAND_NONE) && (command == VTSS_EPS_MGMT_COMMAND_NONE))             return(VTSS_EPS_RC_INVALID_PARAMETER);
    if (command == VTSS_EPS_MGMT_COMMAND_NONE)                                                                                  return(VTSS_EPS_RC_OK);
    if ((instance_data[instance].command == VTSS_EPS_MGMT_COMMAND_FREEZE) && (command != VTSS_EPS_MGMT_COMMAND_CLEAR))          return(VTSS_EPS_RC_INVALID_PARAMETER);
    if ((instance_data[instance].command == VTSS_EPS_MGMT_COMMAND_LOCK_OUT_LOCAL) && (command != VTSS_EPS_MGMT_COMMAND_CLEAR))  return(VTSS_EPS_RC_INVALID_PARAMETER);
    if ((command == VTSS_EPS_MGMT_COMMAND_FREEZE) && (instance_data[instance].command != VTSS_EPS_MGMT_COMMAND_NONE))           return(VTSS_EPS_RC_INVALID_PARAMETER);
    if ((command == VTSS_EPS_MGMT_COMMAND_LOCK_OUT_LOCAL) && (instance_data[instance].command != VTSS_EPS_MGMT_COMMAND_NONE))   return(VTSS_EPS_RC_INVALID_PARAMETER);
