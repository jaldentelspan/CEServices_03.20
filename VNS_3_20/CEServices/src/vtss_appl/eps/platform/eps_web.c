...
static cyg_int32 handler_config_eps_create(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST)
    {
        ...
        {
            rc = eps_mgmt_conf_get(eps_id, &param, &config);
        ...
        if (cyg_httpd_form_varable_int(p, "new_eps", &eps_id))
        {
            ...
            {
                param.param.domain = (vtss_eps_mgmt_domain_t)domain;
                param.param.architecture = (vtss_eps_mgmt_architecture_t)architecture;
                param.param.w_flow = w_flow-1;
                param.param.p_flow = p_flow-1;
                param.w_mep = w_mep-1;
                param.p_mep = p_mep-1;
                param.aps_mep = aps_mep-1;
                if ((rc = eps_mgmt_instance_create(eps_id-1,   &param)) != EPS_RC_OK)
                    if (error == EPS_RC_OK)     error = rc;
            }
        }
    ...
    else
    {
        ...
        for (eps_id=0; eps_id<EPS_MGMT_CREATED_MAX; ++eps_id)
        {
            ...
            {   /* Created */
                ...
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%u/%u/%u/%u/%u/%u/%u/%u/%s|",
                              eps_id+1,
                              (uint)param.param.domain,
                              (uint)param.param.architecture,
                              param.param.w_flow+1,
                              param.param.p_flow+1,
                              param.w_mep+1,
                              param.p_mep+1,
                              param.aps_mep+1,
...
static cyg_int32 handler_config_eps(CYG_HTTPD_STATE* p)
{
    ...
    if(p->method == CYG_HTTPD_METHOD_POST)
    {
        ...
        {
            rc = eps_mgmt_conf_get(eps_id-1,  &param,  &conf);
            if ((rc == EPS_RC_OK) || (rc == EPS_RC_NOT_CONFIGURED))
            {   /* Created */
                aps = revert = FALSE;
                ...
                {
                    conf.directional = (param.param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1F1) ? VTSS_EPS_MGMT_BIDIRECTIONAL : (vtss_eps_mgmt_directional_t)direct;
    ...
    else
    {
        ...
        conf_rc = eps_mgmt_conf_get(eps_id,  &param,  &conf);
        ...
        if (conf_rc == EPS_RC_NOT_CONFIGURED)
        {
            if (param.param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1F1)
            {
                conf.directional = VTSS_EPS_MGMT_BIDIRECTIONAL;
                conf.aps = TRUE;
            }
            if (param.param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1)
            {
                conf.directional = VTSS_EPS_MGMT_UNIDIRECTIONAL;
                conf.aps = FALSE;
            }
            conf.revertive = FALSE;
            conf.restore_timer = 0;
            conf.hold_off_timer = 0;
        }
