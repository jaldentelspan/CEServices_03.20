...
typedef struct
{
    ...
    eps_mgmt_create_param_t         param[EPS_MGMT_CREATED_MAX];
...
static void set_conf_to_default(eps_conf_t  *blk)
{
    ...
    for (i=0; i<EPS_MGMT_CREATED_MAX; ++i)
    {
        ...
        blk->command[i] = VTSS_EPS_MGMT_COMMAND_NONE;
...
static void apply_configuration(eps_conf_t  *blk)
{
    ...
    for (i=0; i<EPS_MGMT_CREATED_MAX; ++i)
    {
        if (blk->created[i])
        {
            ...
            rc += mep_eps_aps_register(blk->param[i].aps_mep, i, MEP_EPS_TYPE_ELPS, TRUE);
            rc += mep_eps_sf_register(blk->param[i].w_mep, i, MEP_EPS_TYPE_ELPS, TRUE);
            rc += mep_eps_sf_register(blk->param[i].p_mep, i, MEP_EPS_TYPE_ELPS, TRUE);
            CRIT_ENTER(crit_p);
            eps_data[i].created = TRUE;
            eps_data[i].w_mep = blk->param[i].w_mep;
            eps_data[i].p_mep = blk->param[i].p_mep;
            eps_data[i].aps_mep = blk->param[i].aps_mep;
            CRIT_EXIT(crit_p);
...
/****************************************************************************/
static void cancel_create(const u32                        instance,
                          const eps_mgmt_create_param_t    *const param)
{
    (void)vtss_eps_mgmt_instance_delete(instance);
    (void)mep_eps_aps_register(param->aps_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
    (void)mep_eps_sf_register(param->w_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
    (void)mep_eps_sf_register(param->p_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
}
                             ...
                             const eps_mgmt_create_param_t    *const param)
{
    ...
    if (param->w_mep == param->p_mep)                return(EPS_RC_W_P_SSF_MEP_EQUAL);
    if (param->w_mep == param->aps_mep)              return(EPS_RC_INVALID_APS_MEP);
    for (i=0; i<EPS_MGMT_CREATED_MAX; ++i)
    {
        if ((i != instance) && eps_data[i].created) {
            if ((eps_data[i].w_mep == param->w_mep) || (eps_data[i].p_mep == param->w_mep) || (eps_data[i].aps_mep == param->w_mep))             return(EPS_RC_INVALID_W_MEP);
            if (eps_data[i].w_mep == param->p_mep)                                                                                               return(EPS_RC_INVALID_P_MEP);
            if ((param->param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1) &&
                ((eps_data[i].p_mep == param->p_mep) || (eps_data[i].aps_mep == param->p_mep)))                                                  return(EPS_RC_INVALID_P_MEP);
            if ((eps_data[i].w_mep == param->aps_mep) || (eps_data[i].p_mep == param->aps_mep) || (eps_data[i].aps_mep == param->aps_mep))       return(EPS_RC_INVALID_APS_MEP);
        }
    }

    if ((rc = vtss_eps_mgmt_instance_create(instance, &param->param)) == VTSS_EPS_RC_OK)
    {
        if (mep_eps_aps_register(param->aps_mep, instance, MEP_EPS_TYPE_ELPS, TRUE) != MEP_RC_OK)   {cancel_create(instance , param); return(EPS_RC_INVALID_APS_MEP);}
        if (mep_eps_sf_register(param->w_mep, instance, MEP_EPS_TYPE_ELPS, TRUE) != MEP_RC_OK)      {cancel_create(instance , param); return(EPS_RC_INVALID_W_MEP);}
        if (mep_eps_sf_register(param->p_mep, instance, MEP_EPS_TYPE_ELPS, TRUE) != MEP_RC_OK)      {cancel_create(instance , param); return(EPS_RC_INVALID_P_MEP);}

        blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_EPS_CONF_TABLE, &size);
        ...
        if (param->param.domain == VTSS_EPS_MGMT_PORT)     /* This in order to inform MEP about port conversion in case of port protection */
            mep_port_protection_create((param->param.architecture == VTSS_EPS_MGMT_ARCHITECTURE_1P1) ? MEP_EPS_ARCHITECTURE_1P1 : MEP_EPS_ARCHITECTURE_1F1,  param->param.w_flow,  param->param.p_flow);

        CRIT_ENTER(crit_p);
        eps_data[instance].created = TRUE;
        eps_data[instance].w_mep = param->w_mep;
        eps_data[instance].p_mep = param->p_mep;
        eps_data[instance].aps_mep = param->aps_mep;
        CRIT_EXIT(crit_p);
    }
...
{
    u32                            rc, w_mep, p_mep, aps_mep, rcm=MEP_RC_OK;
    ulong                          size;
    eps_conf_t                     *blk;
    vtss_eps_mgmt_create_param_t   param;
    ...
    if ((rc = vtss_eps_mgmt_instance_delete(instance)) == VTSS_EPS_RC_OK)
    {
        ...
        rcm += mep_eps_aps_register(aps_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
        rcm += mep_eps_sf_register(w_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
        rcm += mep_eps_sf_register(p_mep, instance, MEP_EPS_TYPE_ELPS, FALSE);
        if (rcm != MEP_RC_OK)   return(EPS_RC_INVALID_PARAMETER);
    }
...
u32 eps_mgmt_conf_get(const u32                 instance,
                      eps_mgmt_create_param_t   *const param,
                      vtss_eps_mgmt_conf_t      *const conf)
{
    if (instance >= EPS_MGMT_CREATED_MAX)            return(EPS_RC_INVALID_PARAMETER);

    CRIT_ENTER(crit_p);
    param->w_mep = eps_data[instance].w_mep;
    param->p_mep = eps_data[instance].p_mep;
    param->aps_mep = eps_data[instance].aps_mep;
    CRIT_EXIT(crit_p);

    return(rc_conv(vtss_eps_mgmt_conf_get(instance, &param->param, conf)));
}
