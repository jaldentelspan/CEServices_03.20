...
static void cli_cmd_id_eps_config(cli_req_t *req)
{
    ...
    else
    {
        ...
        for (i=0; i<EPS_MGMT_CREATED_MAX; ++i)
        {
            ...
            if ((rc = eps_mgmt_conf_get(i, &param, &config)) == EPS_RC_INVALID_PARAMETER)
                eps_print_error(rc);
            else
            {
...
static void cli_cmd_id_eps_create(cli_req_t *req)
{
    ...
    if (req->set)
    {
        ...
        {
            param.param.architecture = eps_req->architecture;
            param.param.domain = eps_req->eps_domain;
            param.param.w_flow = eps_req->w_flow;
            param.param.p_flow = eps_req->p_flow;
            param.w_mep = eps_req->w_mep;
            param.p_mep = eps_req->p_mep;
            param.aps_mep = eps_req->aps_mep;
...
static int cli_parse_eps_domain(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    else if(!strncmp(found, "dompath", 7))       eps_req->eps_domain = VTSS_EPS_MGMT_PATH;
    else if(!strncmp(found, "domservice", 10))   eps_req->eps_domain = VTSS_EPS_MGMT_SERVICE;
    else if(!strncmp(found, "dommpls", 7))       eps_req->eps_domain = VTSS_EPS_MGMT_MPLS;
    else return 1;
...
static cli_parm_t eps_cli_parm_table[] = {
    ...
    {
        "domport|dompath|domservice|dommpls",
        "Flow domain\n",
        CLI_PARM_FLAG_SET,
        cli_parse_eps_domain,
        cli_cmd_id_eps_create,
...
cli_cmd_tab_entry (
  "EPS create",
  "EPS create [<inst>] [domport|dompath|domservice|dommpls] [1p1|1f1] [<flow_w>] [<flow_p>] [<mep_w>] [<mep_p>] [<mep_aps>] [enable|disable]",
  "EPS create",
  PRIO_EPS_CREATE,
  CLI_CMD_TYPE_CONF,
  VTSS_MODULE_ID_EPS,
