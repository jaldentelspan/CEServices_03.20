...
typedef struct {
    u8  vlans_updated[VTSS_BF_SIZE(VTSS_VID_RESERVED + 1)];
    u32 vlan_cnt;
} vlan_cx_set_state_t;
...
static BOOL cx_vlan_match(const cx_table_context_t *context, ulong port_a, ulong port_b)
{
    ...
    return (vlan_mgmt_port_conf_get(context->isid, port_a, &conf_a, VLAN_USER_STATIC) == VTSS_OK &&
            vlan_mgmt_port_conf_get(context->isid, port_b, &conf_b, VLAN_USER_STATIC) == VTSS_OK &&
...
static vtss_rc cx_vlan_print(cx_get_state_t *s, const cx_table_context_t *context, ulong port_no, char *ports)
{
    ...
    if (ports == NULL) {
        ...
        cx_add_stx_start(s);
        cx_add_stx_port(s);
        cx_add_stx_ulong_word(s, "pvid", "none", VTSS_VID_DEFAULT, VTSS_VID_RESERVED);
        cx_add_stx_kw(s, "frame_type", cx_kw_frame);
#ifdef VTSS_SW_OPTION_VLAN_INGRESS_FILTERING
        cx_add_stx_bool(s, "ingress_filter");
#endif  /* VTSS_SW_OPTION_VLAN_INGRESS_FILTERING */
        cx_add_stx_kw(s, "port_type", cx_kw_port);
        cx_add_stx_kw(s, "tx_tag", cx_kw_tx);
        return cx_add_stx_end(s);
    }

    vlan_mgmt_port_conf_get(context->isid, port_no, &conf, VLAN_USER_STATIC);
    cx_add_port_start(s, CX_TAG_ENTRY, ports);
    cx_add_attr_ulong_word(s, "pvid", conf.pvid, "none", 0);
    cx_add_attr_kw(s, "frame_type", cx_kw_frame, conf.frame_type);
#ifdef VTSS_SW_OPTION_VLAN_INGRESS_FILTERING
    cx_add_attr_bool(s, "ingress_filter", conf.ingress_filter);
#endif  /* VTSS_SW_OPTION_VLAN_INGRESS_FILTERING */
    cx_add_attr_kw(s, "port_type", cx_kw_port, conf.port_type);
    cx_add_attr_kw(s, "tx_tag", cx_kw_tx, conf.tx_tag_type);
    ...
    case CX_PARSE_CMD_PARM:
    {
        ...
        if (global) {
            ...
            if (s->id != CX_TAG_TPID) {
                ...
                if (cx_parse_val_ulong(s, &val, 0x600, 0xFFFF) == VTSS_OK && s->apply) {
                    CX_RC(vlan_mgmt_vlan_tpid_set((vtss_etype_t)val));
                }
                return s->rc;
            }
#endif
        ...
        case CX_TAG_ENTRY:
            if (s->group == CX_TAG_PORT_TABLE && cx_parse_ports(s, port_list, 1) == VTSS_OK) {
                ...
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    if (cx_parse_ulong_word(s, "pvid", &val, VTSS_VID_DEFAULT, VTSS_VID_RESERVED, "none", 0) == VTSS_OK) {
                        pvid = 1;
                        new_port_conf.pvid = val;
                    } else if (cx_parse_kw(s, "frame_type", cx_kw_frame, &val, 1) == VTSS_OK) {
                        frame = 1;
                        new_port_conf.frame_type = val;
                    } else if (cx_parse_bool(s, "ingress_filter", &new_port_conf.ingress_filter, 1) == VTSS_OK) {
#ifdef VTSS_SW_OPTION_VLAN_INGRESS_FILTERING
                        in_filter = 1;
#else
                        ...
                        in_filter = 0;
#endif  /* VTSS_SW_OPTION_VLAN_INGRESS_FILTERING */
                    } else if (cx_parse_kw(s, "port_type", cx_kw_port, &val, 1) == VTSS_OK) {
                        ptype = 1;
                        new_port_conf.port_type = val;
                    } else if (cx_parse_kw(s, "tx_tag", cx_kw_tx, &val, 1) == VTSS_OK) {
                        tx_tag = 1;
                ...
                for (port_idx = VTSS_PORT_NO_START; s->apply && port_idx < s->port_count; port_idx++) {
                    if (port_list[iport2uport(port_idx)] && vlan_mgmt_port_conf_get(s->isid, port_idx, &port_conf, VLAN_USER_STATIC) == VTSS_OK) {
                        if (pvid) {
                        ...
                        (void)vlan_mgmt_port_conf_set(s->isid, port_idx, &port_conf, VLAN_USER_STATIC);
                    }
                }
            } else if (s->group == CX_TAG_VLAN_TABLE && cx_parse_vid(s, &vid, 1) == VTSS_OK) {
                BOOL                       vid_set = FALSE;
                vlan_mgmt_entry_t          new_vlan_conf;
#ifdef VTSS_SW_OPTION_VLAN_NAME
                vlan_mgmt_vlan_name_conf_t name_conf;
                BOOL                       vlan_name = FALSE;
#endif
...
#ifdef VTSS_SW_OPTION_VLAN_NAME
                if (cx_parse_vlan_name(s, "vlan_name", new_vlan_conf.vlan_name) == VTSS_OK) {
                    vlan_name = TRUE;
                    name_conf.vid = vid;
                    strcpy(name_conf.vlan_name, new_vlan_conf.vlan_name);
                }
                s->p = s->next;
#endif
                ...
                if (cx_parse_ports(s, port_list, FALSE) == VTSS_OK) {
                    // Convert from xml port to api port
                    for (port_idx = VTSS_PORT_NO_START; port_idx < VTSS_PORT_NO_END; port_idx++) {
                        new_vlan_conf.ports[port_idx] = (port_list[iport2uport(port_idx)] ? VLAN_MEMBER_PORT : VLAN_NON_MEMBER_PORT);
                    }
#ifdef VTSS_SW_OPTION_VLAN_NAME
                    if (vlan_name == FALSE) {
                ...
                if (cx_parse_ports_name(s, forbidden_port_list, FALSE, "Forbidden_port") == VTSS_OK) {
                    for (port_idx = VTSS_PORT_NO_START; port_idx < VTSS_PORT_NO_END; port_idx++) {
                        if (new_vlan_conf.ports[port_idx] != VLAN_MEMBER_PORT) {
                ...
                if (s->apply) {
...
#ifdef VTSS_SW_OPTION_VLAN_NAME
                    if (vlan_name) {
                        vlan_mgmt_vlan_name_add(&name_conf, 0);
                    }
#endif
                    ...
                    vlan_exists = vlan_mgmt_vlan_get(s->isid, vid, &old_vlan_conf, FALSE, VLAN_USER_STATIC, VLAN_MIXED_OP) == VTSS_RC_OK;

                    add_vlan = vid_set && ((vlan_exists && memcmp(&new_vlan_conf.ports, &old_vlan_conf.ports, sizeof(new_vlan_conf.ports)) != 0) || !vlan_exists);

                    if (add_vlan) {
                        // Prevent port-clash
                        T_DG(VTSS_TRACE_GRP_VLAN, "VID=%d: Deleting forbidden to prevent port-clash", vid);
                        (void)vlan_mgmt_vlan_del(s->isid, vid, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP);
                        (void)vlan_mgmt_vlan_del(s->isid, vid, VLAN_USER_STATIC, VLAN_FORBIDDEN_OP);
    ...
    case CX_PARSE_CMD_SWITCH:
        if (s->init) {
            ...
            if (vlan_state->vlan_cnt) {
                ...
                while (vlan_mgmt_vlan_get(s->isid, vlan_conf.vid, &vlan_conf, 1, VLAN_USER_STATIC, VLAN_MIXED_OP) == VTSS_OK) {
                    if (VTSS_BF_GET(vlan_state->vlans_updated, vlan_conf.vid) == FALSE) {
                        ...
                        (void)vlan_mgmt_vlan_del(s->isid, vlan_conf.vid, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP);
                        (void)vlan_mgmt_vlan_del(s->isid, vlan_conf.vid, VLAN_USER_STATIC, VLAN_FORBIDDEN_OP);
                    } /* if (VTSS_BF_GET(vlan_state->vlans_updated, vlan_conf.vid) == FALSE) */
                } /* while (vlan_mgmt_vlan_get(s->isid, vlan_conf.vid, &vlan_conf, 1, VLAN_USER_STATIC, VLAN_MIXED_OP) == VTSS_OK) */
            } /* if (vlan_state->vlan_cnt) */
        } /* else  if (s->apply) */
...
static vtss_rc vlan_cx_gen_func(cx_get_state_t *s)
{
    ...
    case CX_GEN_CMD_SWITCH:
        /* Switch - VLAN */
        T_D("switch - vlan");
        cx_add_tag_line(s, CX_TAG_VLAN, 0);
        CX_RC(cx_add_port_table(s, s->isid, CX_TAG_PORT_TABLE, cx_vlan_match, cx_vlan_print));
        {
            vlan_mgmt_entry_t conf;
#ifdef VTSS_SW_OPTION_VLAN_NAME
            i8                name[VLAN_NAME_MAX_LEN];
#endif
#if VTSS_SWITCH_STACKABLE
            port_isid_info_t  pinfo;
#endif

            cx_add_tag_line(s, CX_TAG_VLAN_TABLE, 0);

            /* Entry syntax */
            cx_add_stx_start(s);
            cx_add_stx_ulong(s, "vid", VTSS_VID_DEFAULT, VTSS_VID_RESERVED);
#ifdef VTSS_SW_OPTION_VLAN_NAME
            sprintf(name, "0-%u characters", VLAN_NAME_MAX_LEN - 1);
            CX_RC(cx_add_attr_txt(s, "vlan_name", name));
#endif
            sprintf(name, "none or 1-4095 (list allowed, e.g. 1,3-5)");
            CX_RC(cx_add_attr_txt(s, "port", name));
            sprintf(name, "none or 1-4095 (list allowed, e.g. 1,3-5)");
            CX_RC(cx_add_attr_txt(s, "Forbidden_port", name));
            cx_add_stx_end(s);

            prev_vid = VTSS_VID_NULL;
            while(((vlan_mgmt_vlan_get(s->isid, prev_vid, &conf, 1, VLAN_USER_STATIC, VLAN_MIXED_OP)) == VTSS_OK)) {
                prev_vid = conf.vid;
                cx_add_attr_start(s, CX_TAG_ENTRY);
                cx_add_attr_ulong(s, "vid", conf.vid);
...
#endif
#ifdef VTSS_SW_OPTION_VLAN_NAME
                cx_add_attr_txt(s, "vlan_name", conf.vlan_name);
#endif
                if (mports_exist) {
                    cx_add_attr_txt(s, "port", mgmt_iport_list2txt(mports, buf));
                } else {
                    cx_add_attr_txt(s, "port", "none");
                }
                if (fports_exist) {
                    cx_add_attr_txt(s, "Forbidden_port", mgmt_iport_list2txt(fports, buf));
                } else {
                    cx_add_attr_txt(s, "Forbidden_port", "none");
                }
                CX_RC(cx_add_attr_end(s, CX_TAG_ENTRY));
            } /* while(((vlan_mgmt_vlan_get(s->isid, prev_vid, &conf, 1, VLAN_USER_STATIC, VLAN_MIXED_OP)) == VTSS_OK)) */
            cx_add_tag_line(s, CX_TAG_VLAN_TABLE, 1);
        }
        CX_RC(cx_add_tag_line(s, CX_TAG_VLAN, 1));
        break;
    default:
