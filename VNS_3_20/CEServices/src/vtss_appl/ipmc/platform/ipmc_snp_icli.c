...
static BOOL _icli_ipmc_snp_validate_isid(vtss_isid_t isx)
{
    return IPMC_LIB_ISID_EXIST(isx);
}
                                      ...
                                      BOOL inc_sfm, BOOL detail)
{
    ...
    BOOL                        iports[VTSS_PORT_ARRAY_SIZE];
    BOOL                        deny_found;
    i32                         fto;
    i8                          adrString[40], plistString[MGMT_PORT_BUF_SIZE];
    ...
    memset(plistString, 0x0, sizeof(plistString));
    memset(iports, 0x0, sizeof(iports));
    (void) port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_ALL);
    while (port_iter_getnext(&pit)) {
        iport = pit.iport;
        iports[iport] = VTSS_PORT_BF_GET(grp_db->port_mask, iport);
    }

    ICLI_PRINTF("\n\r%s is registered on VLAN %u\n", adrString, vid);
    ICLI_PRINTF("Port Members: %s\n", icli_iport_list_txt(iports, plistString));
    if (detail) {
                                        ...
                                        BOOL detail)
{
    ...
    if (ipmc_mgmt_get_intf_version(isid, &compver, version) == VTSS_OK) {
        ...
        if (detail && (param->cfg_compatibility == VTSS_IPMC_COMPAT_MODE_AUTO)) {
            ...
            ICLI_PRINTF("Older Version Querier Present Timeout: %lu second%s\n",
                        ovpt, (ovpt > 1) ? "s" : "");

            ovpt = _icli_ipmc_snp_ovpt_get(&param->hst_compatibility);
            ICLI_PRINTF("Older Version Host Present Timeout: %lu second%s\n",
                        ovpt, (ovpt > 1) ? "s" : "");
        }
    }
}
                                            ...
                                            BOOL detail)
{
    ...
    if (detail) {
        ...
        if (admin) {
            if (q->state == IPMC_QUERIER_ACTIVE) {
                ICLI_PRINTF("Querier Up time: %lu second%s; Query Interval: %lu second%s\n",
                            q->QuerierUpTime,
                            (q->QuerierUpTime > 1) ? "s" : "",
                            q->timeout,
                            (q->timeout > 1) ? "s" : "");
            } else if (q->state == IPMC_QUERIER_INIT) {
                ICLI_PRINTF("Startup Query Interval: %lu second%s; Startup Query Count: %lu\n",
                            q->timeout,
                            (q->timeout > 1) ? "s" : "",
                            q->StartUpCnt);

            } else {
                ICLI_PRINTF("Querier Expiry Time: %lu second%s\n",
    ...
    if (detail) {
        ...
        ICLI_PRINTF("PRI:%u / RV:%lu / QI:%lu / QRI:%lu / LMQI:%lu / URI:%lu\n",
                    param->priority,
                    q->RobustVari,
                    q->QueryIntvl,
                    q->MaxResTime,
                                        ...
                                        BOOL detail)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (sta_mr->ports[iport] || dyn_mr->ports[iport]) {
            ...
            if (sta_mr->ports[iport] && dyn_mr->ports[iport]) {
                ICLI_PRINTF("%sPort %u: Static and Dynamic Router Port\n",
                            port_isid_port_no_is_stack(isid, iport) ? "*Stacking " : "",
                            pit.uport);
            } else {
                ICLI_PRINTF("%sPort %u: %s Router Port\n",
                            port_isid_port_no_is_stack(isid, iport) ? "*Stacking " : "",
                            pit.uport,
...
#if 0
char *icli_ipmc_snp_ptype_txt(icli_port_type_t ptype, ipmc_text_cap_t cap)
{
    char    *txt;
        ...
        */
        ...
        if (from_db) {
            ...
            if (ipmc_mgmt_get_ssm_range(version, &ipmc_prefix) == VTSS_OK) {
                ...
                memset(adrString, 0x0, sizeof(adrString));
                ICLI_PRINTF("Groups in range %s/%lu follow %s SSM registration service model.\n\r",
                            (version == IPMC_IP_VERSION_IGMP) ?
                           ...
                           BOOL inc_sfm, BOOL detail)
{
    ...
    while (switch_iter_getnext(&sit)) {
        isid = sit.isid;
        if (!_icli_ipmc_snp_validate_isid(isid)) {
            continue;
        }
        usid = sit.usid;
