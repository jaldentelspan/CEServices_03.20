...
#define IPMC_SNP_ICFG_REG(x, y, z)  (((x) = vtss_icfg_query_register((y), (z))) == VTSS_OK)
        ...
        if (ipmc_mgmt_get_ssm_range(version, &ipmc_icli_prefix) == VTSS_OK) {
            if (req->all_defaults || ipmc_mgmt_def_ssm_range_chg(version, &ipmc_icli_prefix)) {
                ...
                if (version == IPMC_IP_VERSION_IGMP) {
                    rc |= vtss_icfg_printf(result, "ip igmp ssm-range %s %lu\n",
                                           misc_ipv4_txt(ipmc_icli_prefix.addr.value.prefix, adrString),
                                           ipmc_icli_prefix.len);
                } else {
                    rc |= vtss_icfg_printf(result, "ipv6 mld ssm-range %s %lu\n",
                                   ...
                                   ipmc_ip_version_t version)
{
    ...
    if (req && result) {
        ...
        /*
            ...
            COMMAND = ip igmp querier {election | address <ipv4_nmcast>}
        ...
        if (ipmc_mgmt_get_intf_state_querier(TRUE, &ipmc_icli_intf_vid,
            ...
            if (ipmc_mgmt_get_intf_info(isid, ipmc_icli_intf_vid, &ipmc_icli_intf_param, version) == VTSS_OK) {
                ...
                rc |= vtss_icfg_printf(result, " %s %s snooping compatibility %s\n",
                                       ...
                                       (ipmc_icli_intf_param.cfg_compatibility == VTSS_IPMC_COMPAT_MODE_SFM) ? "v3" : "auto");
                rc |= vtss_icfg_printf(result, " %s %s snooping priority %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_icli_intf_param.priority);
                rc |= vtss_icfg_printf(result, " %s %s snooping robustness-variable %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       q->RobustVari);
                rc |= vtss_icfg_printf(result, " %s %s snooping query-interval %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       q->QueryIntvl);
                rc |= vtss_icfg_printf(result, " %s %s snooping query-max-response-time %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       q->MaxResTime);
                rc |= vtss_icfg_printf(result, " %s %s snooping last-member-query-interval %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       q->LastQryItv);
                rc |= vtss_icfg_printf(result, " %s %s snooping unsolicited-report-interval %lu\n",
                                       ipmc_lib_ip_txt(version, IPMC_TXT_CASE_LOWER),
                                       ipmc_lib_version_txt(version, IPMC_TXT_CASE_LOWER),
                                       q->UnsolicitR);
#endif /* VTSS_SW_OPTION_SMB_IPMC */
...
vtss_rc ipmc_snp_icfg_init(void)
{
    ...
    if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_IGMP, _ipmc_snp_icfg_igmp_global)) {
        if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_IGMP_PORT, _ipmc_snp_icfg_igmp_port)) {
            if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_IGMP_INTF, _ipmc_snp_icfg_igmp_intf)) {
                if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_IGMP_CTRL, _ipmc_snp_icfg_igmp_ctrl)) {
...
#if defined(VTSS_SW_OPTION_SMB_IPMC)
    /* Register callback functions to ICFG module. */
    if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_MLD, _ipmc_snp_icfg_mld_global)) {
        if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_MLD_PORT, _ipmc_snp_icfg_mld_port)) {
            if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_MLD_INTF, _ipmc_snp_icfg_mld_intf)) {
                if (IPMC_SNP_ICFG_REG(rc, VTSS_ICFG_IPMC_SNP_MLD_CTRL, _ipmc_snp_icfg_mld_ctrl)) {
