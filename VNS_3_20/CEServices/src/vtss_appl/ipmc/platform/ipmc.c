...
static void ipmc_msg_tx(ipmc_msg_buf_t *buf, vtss_isid_t isid, size_t len)
{
    ...
    T_I("msg_id: %d->%s, len: %zd, isid: %s-%d",
        ...
        len,
        msg_switch_is_local(isid) ? "LOC" : "REM",
        isid);
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        if (next) {
            ...
            if (isid == VTSS_ISID_LOCAL) {
                while (vlan_mgmt_vlan_get(VTSS_ISID_LOCAL, vid, vlan_conf, TRUE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
                    vid = vlan_conf->vid;

                    chk_mvr.vid = vid;
                    if (vlan_mgmt_vlan_get(VTSS_ISID_LOCAL, vid, &chk_mvr, FALSE, VLAN_USER_MVR, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
                        found = TRUE;
                        break;
                    }
                }
            } else {
                while (vlan_mgmt_vlan_get(isid, vid, vlan_conf, TRUE, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
                    vid = vlan_conf->vid;

                    chk_mvr.vid = vid;
                    if (vlan_mgmt_vlan_get(isid, vid, &chk_mvr, FALSE, VLAN_USER_MVR, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
                        found = TRUE;
                        break;
                    }
                }
            ...
            if (isid == VTSS_ISID_LOCAL) {
                if (vlan_mgmt_vlan_get(VTSS_ISID_LOCAL, vid, vlan_conf, FALSE, VLAN_USER_MVR, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
                    return VTSS_RC_ERROR;
                }

                return vlan_mgmt_vlan_get(VTSS_ISID_LOCAL, vid, vlan_conf, FALSE, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP);
            } else {
                if (vlan_mgmt_vlan_get(isid, vid, vlan_conf, FALSE, VLAN_USER_MVR, VLAN_MEMBERSHIP_OP) == VTSS_OK) {
                    return VTSS_RC_ERROR;
                }

                return vlan_mgmt_vlan_get(isid, vid, vlan_conf, FALSE, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP);
            }
        }
#else
        if (isid == VTSS_ISID_LOCAL) {
            return vlan_mgmt_vlan_get(VTSS_ISID_LOCAL, vid, vlan_conf, next, VLAN_USER_ALL, VLAN_MEMBERSHIP_OP);
        } else {
            return vlan_mgmt_vlan_get(isid, vid, vlan_conf, next, VLAN_USER_STATIC, VLAN_MEMBERSHIP_OP);
        }
                                         ...
                                         ipmc_ip_version_t ipmc_version)
{
    ...
    if (*state_enable) {
        ...
        if (ipmc_resolve_vlan_member(VTSS_ISID_LOCAL, &vlan_conf, FALSE) == VTSS_OK) {
            (void) port_iter_init(&pit, NULL, VTSS_ISID_LOCAL, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_ALL);
            while (port_iter_getnext(&pit)) {
                VTSS_PORT_BF_SET(vlan_ports.member_ports, pit.iport, ((vlan_conf.ports[pit.iport] == VLAN_MEMBER_PORT) ? 1 : 0));
            }
        }
    }
            ...
            if (action != IPMC_OP_DEL) {
                if (*state_enable) {
                    ...
                    if (ipmc_resolve_vlan_member(isid, &vlan_conf, FALSE) == VTSS_OK) {
                        (void) port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_ALL);
                        while (port_iter_getnext(&pit)) {
                            VTSS_PORT_BF_SET(vlan_ports.member_ports, pit.iport,
                                             ((vlan_conf.ports[pit.iport] == VLAN_MEMBER_PORT) ? 1 : 0));
                        }
                    }
                }
...
vtss_rc ipmc_mgmt_get_intf_info(vtss_isid_t isid, vtss_vid_t vid, ipmc_prot_intf_entry_param_t *intf_param, ipmc_ip_version_t ipmc_version)
{
    ...
    memset(intf_param, 0x0, sizeof(ipmc_prot_intf_entry_param_t));
    if ((isid == VTSS_ISID_GLOBAL) || msg_switch_is_local(isid)) {
...
vtss_rc ipmc_mgmt_set_router_port(vtss_isid_t isid, ipmc_conf_router_port_t *router_port, ipmc_ip_version_t ipmc_version)
{
    ...
    if (!apply_flag && msg_switch_is_local(isid)) {
        IPMC_CRIT_EXIT();
        return VTSS_OK;
    }
...
static void ipmc_vlan_changed(vtss_isid_t isid_in,
                              vtss_vid_t vid,
                              vlan_user_t user,
                              ...
                              vlan_action_t action)
{
    ...
    vtss_isid_t             isid;
    port_iter_t             pit;
    u16                     intf_idx4, intf_idx6, isid_port_cnt, isid_port_idx;
    ipmc_port_bfs_t         vlan_ports, *in_ports;
    ipmc_conf_intf_entry_t  ipmc_intf;
    cyg_tick_count_t        exe_time_base;
    BOOL                    isid_add_ports[VTSS_PORT_ARRAY_SIZE];
    BOOL                    isid_del_ports[VTSS_PORT_ARRAY_SIZE];
    BOOL                    isid_cur_ports[VTSS_PORT_ARRAY_SIZE];
    char                    bufVMPortAdd[MGMT_PORT_BUF_SIZE];
    char                    bufVMPortDel[MGMT_PORT_BUF_SIZE];
    char                    bufVMPortCur[MGMT_PORT_BUF_SIZE];

    if ((user != VLAN_USER_STATIC) || !msg_switch_is_master()) {
        return;
    }

    /*
        VLAN Change
        CfgExist + VlanExist:   ipmc_stacking_set_intf (VLAN Member Change)
        CfgExist + !VlanExist:  ipmc_stacking_set_intf (Delete Running Tables)
        !CfgExist + VlanExist:  Do Nothing
        !CfgExist + !VlanExist: Do Nothing
    */
    IPMC_CRIT_ENTER();
    ...
    exe_time_base = cyg_current_time();
    T_I("Enter ISID:%d/VID:%u/ACTION:%d", isid_in, vid, action);
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        isid_port_cnt = 0;
        memset(isid_add_ports, 0x0, sizeof(isid_add_ports));
        memset(isid_del_ports, 0x0, sizeof(isid_del_ports));
        memset(isid_cur_ports, 0x0, sizeof(isid_cur_ports));
        (void) port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_ALL);
        while (port_iter_getnext(&pit)) {
            isid_port_cnt++;
            isid_port_idx = pit.iport;

            in_ports = (ipmc_port_bfs_t *)added_ports;
            in_ports += (isid - VTSS_ISID_START);
            if (VTSS_PORT_BF_GET(in_ports->member_ports, isid_port_idx)) {
                isid_add_ports[isid_port_idx] = TRUE;
            }
            in_ports = (ipmc_port_bfs_t *)deleted_ports;
            in_ports += (isid - VTSS_ISID_START);
            if (VTSS_PORT_BF_GET(in_ports->member_ports, isid_port_idx)) {
                isid_del_ports[isid_port_idx] = TRUE;
            }
            in_ports = (ipmc_port_bfs_t *)current_ports;
            in_ports += (isid - VTSS_ISID_START);
            if (VTSS_PORT_BF_GET(in_ports->member_ports, isid_port_idx)) {
                isid_cur_ports[isid_port_idx] = TRUE;
            }
        }

        T_I("ISID-%d->ADD:%s|DEL:%s|CURRENT:%s", isid,
            mgmt_iport_list2txt(isid_add_ports, bufVMPortAdd),
            mgmt_iport_list2txt(isid_del_ports, bufVMPortDel),
            mgmt_iport_list2txt(isid_cur_ports, bufVMPortCur));

        if (action != VLAN_ACTION_DELETE) {
            /* IGMP */
            if (intf_idx4 != IPMC_VID_INIT) {    /* CfgExist + VlanExist */
                IPMC_CRIT_ENTER();
                ...
                if (ipmc_intf.protocol_status) {
                    VTSS_PORT_BF_CLR(vlan_ports.member_ports);
                    for (isid_port_idx = 0; isid_port_idx < isid_port_cnt; isid_port_idx++) {
                        VTSS_PORT_BF_SET(vlan_ports.member_ports, isid_port_idx, isid_cur_ports[isid_port_idx]);
                    }

                    (void) ipmc_stacking_set_intf(isid, IPMC_OP_SET, &ipmc_intf, &vlan_ports, IPMC_IP_VERSION_IGMP);
                    T_D("IPMC(IGMP) update VID %u for ISID %d", vid, isid);
                }
            }

            /* MLD */
            if (intf_idx6 != IPMC_VID_INIT) {    /* CfgExist + VlanExist */
                IPMC_CRIT_ENTER();
                ...
                if (ipmc_intf.protocol_status) {
                    ...
                    (void) ipmc_stacking_set_intf(isid, IPMC_OP_SET, &ipmc_intf, &vlan_ports, IPMC_IP_VERSION_MLD);
                    T_D("IPMC(MLD) update VID %u for ISID %d", vid, isid);
                }
            }
        } else {
            /* IGMP */
            if (intf_idx4 != IPMC_VID_INIT) {    /* CfgExist + !VlanExist */
                ...
                if (ipmc_intf.protocol_status) {
                    VTSS_PORT_BF_CLR(vlan_ports.member_ports);
                    (void) ipmc_stacking_set_intf(isid, IPMC_OP_SET, &ipmc_intf, &vlan_ports, IPMC_IP_VERSION_IGMP);
                    T_D("IPMC(IGMP) delete VID %u Members for ISID %d", vid, isid);
                }
            }
            ...
            if (intf_idx6 != IPMC_VID_INIT) {    /* CfgExist + !VlanExist */
                ...
                if (ipmc_intf.protocol_status) {
                    VTSS_PORT_BF_CLR(vlan_ports.member_ports);
                    (void) ipmc_stacking_set_intf(isid, IPMC_OP_SET, &ipmc_intf, &vlan_ports, IPMC_IP_VERSION_MLD);
                    T_D("IPMC(MLD) delete VID %u Members for ISID %d", vid, isid);
                }
            }
        }
    }
...
//            memcpy(&obs_ipmc_conf_v3, conf_blk3, sizeof(ipmc_conf_v3_t));
    ...
    if (blk_ver > IPMC_CONF_VERSION) {
        ...
        obs34_snp_conf_intf_entry_t *obs4_intf;
        ipmc_conf_intf_entry_t      *intf;
        ...
        case 4:
            ...
            for (intf_idx = 0; intf_idx < IPMC_VLAN_MAX; intf_idx++) {
                obs4_intf = &obs_ipmc_conf_v4.ipmc_conf_intf_entries[intf_idx];
                if (!obs4_intf || !obs4_intf->valid ||
                    !obs4_intf->vid || !obs4_intf->protocol_status) {
                    continue;
                }
                ...
                intf->proxy_status = obs4_intf->proxy_status;
        ...
        if (sizeof(ipmc_configuration_t) != conf_sz) {
            ...
            memcpy(tar_conf, conf_blk, sizeof(ipmc_configuration_t));
        }
    }
...
void ipmc_stp_port_state_change_callback(vtss_common_port_t l2port, vtss_common_stpstate_t new_state)
{
    ...
    if (!IPMC_LIB_ISID_CHECK(isid)) {
        T_W("Failed to translate ISID(%d) on %s\n", isid, l2port2str(l2port));
        return;
    }
    l2_isid = isid;
...
#if 0 /* etliang: Reserve for API-EVENT */
...
#else
    while (ip_status_get(&status) == VTSS_OK && !status.ip_thread_ready && !ipmc_link && (delay_cnt < IPMC_THREAD_START_DELAY_CNT)) {
#endif /* VTSS_SW_OPTION_IP2 */
#if VTSS_SWITCH_STACKABLE
    ...
    while (ipmcReady && sys_mgmt) {
        ...
        if (events & IPMC_EVENT_SWITCH_DEL) {
            ...
            while (switch_iter_getnext(&sit)) {
                ...
                if (msg_switch_is_local(isid_del)) {
        ...
        if (events & IPMC_EVENT_SWITCH_ADD) {
            ...
            while (switch_iter_getnext(&sit)) {
                ...
                if (msg_switch_is_local(isid_add)) {
...
vtss_rc ipmc_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        for (idx = IPMC_MSG_ID_MODE_SET_REQ; idx < IPMC_MSG_MAX_ID; idx++) {
            ...
            ipmc_global.msg[idx] = malloc(ipmc_global.msize[idx]);
        ...
        vlan_membership_change_register(ipmc_vlan_changed);
        ...
        T_D("IPMC-INIT_CMD_INIT consumes ID%d:%u ticks", isid, (ulong)(cyg_current_time() - exe_time_base));
    ...
    case INIT_CMD_START:
        ...
        T_D("IPMC-INIT_CMD_START consumes ID%d:%u ticks", isid, (ulong)(cyg_current_time() - exe_time_base));
    ...
    case INIT_CMD_MASTER_UP:
        ...
        T_D("IPMC-INIT_CMD_MASTER_UP consumes ID%d:%u ticks", isid, (ulong)(cyg_current_time() - exe_time_base));
    ...
    case INIT_CMD_MASTER_DOWN:
        ...
        T_D("IPMC-INIT_CMD_MASTER_DOWN consumes ID%d:%u ticks", isid, (ulong)(cyg_current_time() - exe_time_base));

        break;
    case INIT_CMD_SWITCH_ADD:
        T_D("INIT_CMD_SWITCH_ADD (%d)", isid);
