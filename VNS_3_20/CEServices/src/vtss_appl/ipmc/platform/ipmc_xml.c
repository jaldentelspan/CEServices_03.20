...
static vtss_rc cx_add_stx_port_single(cx_get_state_t *s)
{
    ...
    sprintf(buf, "%u-%u", 1, VTSS_FRONT_PORT_COUNT);
    ...
    case CX_PARSE_CMD_PARM: {
        ...
        case CX_TAG_ENTRY:
            if (global && s->group == CX_TAG_VLAN_TABLE &&
                ...
                ipmc_conf_fast_leave_port_t     fastleave_port;
                ipmc_conf_throttling_max_no_t   ipmc_throttling_max_no;
                BOOL                            r_found = 0, l_found = 0, t_found = 0, router, leave = 0;
                ulong                           throttling_no = 0;

                s->p = s->next;
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    if (cx_parse_bool(s, "router", &router, 1) == VTSS_OK) {
                        r_found = 1;
                    } else if (cx_parse_bool(s, "fastleave", &leave, 1) == VTSS_OK) {
                        l_found = 1;
                    } else if (cx_parse_ulong(s, "limit_group_number", &throttling_no, 0, 10) == VTSS_OK) {
                        t_found = 1;
                    } else {
                        CX_RC(cx_parm_unknown(s));
                    }
                ...
                if (s->apply &&
                    ...
                    ipmc_mgmt_get_fast_leave_port(s->isid,
                                                  &fastleave_port, ipmc_ip_version) == VTSS_OK &&
                    ipmc_mgmt_get_throttling_max_count(s->isid,
                                                       &ipmc_throttling_max_no, ipmc_ip_version) == VTSS_OK) {
                    (void) port_iter_init(&pit, NULL, s->isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_NORMAL);
                    while (port_iter_getnext(&pit)) {
                        port_idx = pit.iport;
                        ...
                        if (port_list[iport2uport(port_idx)]) {
                            if (r_found) {
                                router_port.ports[port_idx] = router;
                            }
                            if (l_found) {
                                fastleave_port.ports[port_idx] = leave;
                            }
                            if (t_found) {
                                ipmc_throttling_max_no.ports[port_idx] = throttling_no;
                            }
                        }
                    }
                    ...
                    CX_RC(ipmc_mgmt_set_throttling_max_count(s->isid, &ipmc_throttling_max_no, ipmc_ip_version));
                }
            }
#ifdef VTSS_SW_OPTION_SMB_IPMC
                ...
                if (ipmc_ip_version == IPMC_IP_VERSION_IGMP) {
                    if (cx_parse_ipmcv4(s, "ssm_prefix", &pfx.addr.value.prefix) == VTSS_OK) {
                        s->p = s->next;
                        for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                            if (cx_parse_ulong(s, "ssm_length", &pfx.len, 4, 32) != VTSS_OK) {
                                CX_RC(cx_parm_unknown(s));
                            }
                        }
                    } else {
                    ...
                    if (cx_parse_ipmcv6(s, "ssm_prefix", &pfx.addr.array.prefix) == VTSS_OK) {
                        s->p = s->next;
                        for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                            if (cx_parse_ulong(s, "ssm_length", &pfx.len, 8, 128) != VTSS_OK) {
                                CX_RC(cx_parm_unknown(s));
                            }
                        }
                    } else {
                ...
                BOOL            pdx = FALSE, port_found = FALSE;
                vtss_uport_no_t uport;

                memset(&ipmc_xml_parse_pg_filtering, 0x0, sizeof(ipmc_conf_port_group_filtering_t));
                for (; s->rc == VTSS_OK && cx_parse_attr(s) == VTSS_OK; s->p = s->next) {
                    if (cx_parse_ulong(s, "port", &uport, 1, VTSS_FRONT_PORT_COUNT) == VTSS_OK) {
                        port_found = TRUE;
