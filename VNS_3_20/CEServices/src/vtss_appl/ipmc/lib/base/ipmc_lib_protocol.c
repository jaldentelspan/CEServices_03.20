...
#if 0 /* etliang */
static BOOL ipmc_lib_protocol_adjust_filter_timers(ipmc_db_ctrl_hdr_t *fltr,
                                                   ipmc_db_ctrl_hdr_t *p,
                                                   ipmc_group_entry_t *entry)
{
    ipmc_group_entry_t  *grp;
    ipmc_group_db_t     *grp_db, *entry_db;
    u32                 ndx, local_port_cnt;
    ipmc_time_t         cur_min;
    BOOL                tmr_chg;
    ...
    if ((grp = ipmc_lib_group_ptr_get(p, entry)) != NULL) {
        grp_db = &grp->info->db;
        entry_db = &entry->info->db;

        IPMC_TIMER_RESET(&cur_min);
        tmr_chg = FALSE;
        local_port_cnt = ipmc_lib_get_system_local_port_cnt();
        for (ndx = 0; ndx < local_port_cnt; ndx++) {
            /* Group Filter Timer */
            if (!IPMC_TIMER_EQUAL(&grp_db->ipmc_sf_filter_timer[ndx], &entry_db->tmr.fltr_timer.t[ndx])) {
                ipmc_lib_time_cpy(&entry_db->ipmc_sf_filter_timer[ndx], &grp_db->tmr.fltr_timer.t[ndx]);
            }

            if (IPMC_TIMER_GREATER(&entry_db->min_tmr, &entry_db->tmr.fltr_timer.t[ndx])) {
                tmr_chg = TRUE;

                if (IPMC_TIMER_ZERO(&cur_min)) {
                    ipmc_lib_time_cpy(&cur_min, &entry_db->tmr.fltr_timer.t[ndx]);
                } else {
                    if (IPMC_TIMER_GREATER(&cur_min, &entry_db->tmr.fltr_timer.t[ndx])) {
                        ipmc_lib_time_cpy(&cur_min, &entry_db->tmr.fltr_timer.t[ndx]);
                    }
                }
            }
        ...
        if (tmr_chg) {
            grp_db = entry_db;
            if (IPMC_LIB_DB_GET(fltr, grp_db)) {
                (void) IPMC_LIB_DB_DEL(fltr, grp_db);
            }

            ipmc_lib_time_cpy(&entry_db->min_tmr, &cur_min);
            (void) IPMC_LIB_DB_ADD(fltr, entry_db);
        }
    }
