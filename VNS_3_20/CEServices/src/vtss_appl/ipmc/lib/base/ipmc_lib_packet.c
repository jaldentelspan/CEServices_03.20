...
static ushort mld_chksum_tx(mld_ip6_hdr *ip, ulong offset, ulong len)
{
    ...
    mld = (ipmc_mld_packet_t *) ((char *)ip + offset);
    memcpy(&icmpChkSum.mld, mld, len);
    icmpChkSum.mld.common.checksum = 0x0;

    sdata = (ushort *)&icmpChkSum;
    len = len + IPV6_HDR_FIXED_LEN;
    if (len > sizeof(mld_icmp_pseudo_t)) {
        return FALSE;
    }
                           ...
                           size_t len)
{
    ...
    vtss_rc                     tx_status;
    BOOL                        is_stacking, priority_tag, priv_flag;
    ...
    */
    ...
    for (i = 0; i < local_port_cnt; i++) {
        ...
        if (!is_stacking && (filter[i].filter == VTSS_PACKET_FILTER_TAGGED)) {
            if (force_untag) {
#if 0 /* Bug#8040: Customer asks not to send priority tag */
                priority_tag = TRUE;
                tag_offset = 4;
#endif /* etliang */
            } else {
    ...
    if (tx_flag) {
        ...
        if (entry->ipmc_version == IPMC_IP_VERSION_MLD) {
            ...
            if ((entry->param.hst_compatibility.mode == VTSS_IPMC_COMPAT_MODE_GEN) ||
                ...
                if (mld_suppress) {
                    ip_mld_sfm_hdr->sfminfo.query.resv_s_qrv |= 0x8;
                }
            ...
            if ((entry->param.hst_compatibility.mode == VTSS_IPMC_COMPAT_MODE_OLD) ||
                ...
                if (igmp_suppress) {
                    ip_igmp_sfm_hdr->sfminfo.query.resv_s_qrv |= 0x8;
                }
                if (IPMC_TIMER_RV(entry) < 0x7) {
                               ...
                               BOOL force_untag)
{
    ...
    if (tx_flag) {
        ...
        if (entry->ipmc_version == IPMC_IP_VERSION_MLD) {
            ...
            if (mld_suppress) {
                ip_mld_hdr->sfminfo.query.resv_s_qrv |= 0x8;
            }
            if (IPMC_TIMER_RV(entry) < 0x7) {
            ...
            if (igmp_suppress) {
                ip_igmp_hdr->sfminfo.query.resv_s_qrv |= 0x8;
            }
            if (IPMC_TIMER_RV(entry) < 0x7) {
