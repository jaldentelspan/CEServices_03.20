...
#else
        intf_up = IPMC_IP_INTF_OPST_UP(intf_ifid);
        intf_vid = IPMC_IP_INTF_OPST_VID(intf_ifid);
        intf_adr = IPMC_IP_INTF_OPST_ADR4(intf_ifid);
#endif /* VTSS_SW_OPTION_IP2 */
...
BOOL ipmc_lib_instance_name_check(i8 *inst_name)
{
    BOOL    chk_st;
    int     i, str_len;
    ...
    chk_st = FALSE;
    for (i = 0; i < str_len; i++) {
        if ((inst_name[i] < 48) || (inst_name[i] > 122)) {
            chk_st = FALSE;
            break;
        } else {
            if (((inst_name[i] > 64) && (inst_name[i] < 91)) ||
                (inst_name[i] > 96)) {
                chk_st = TRUE;
            } else {
                if (inst_name[i] > 57) {
...
BOOL ipmc_lib_instance_desc_check(i8 *inst_desc)
{
    BOOL    chk_st;
    int     i, str_len;
    ...
    str_len = strlen(inst_desc);
    if (str_len >= VTSS_IPMC_DESC_MAX_LEN) {
        return FALSE;
    } else {
        if (!str_len) {
            return TRUE;
    ...
    chk_st = FALSE;
    for (i = 0; i < str_len; i++) {
        if ((inst_desc[i] < 45) || (inst_desc[i] > 122)) {  /* ASCII[45:- 122:z] */
            if (inst_desc[i] != 32) {
                chk_st = FALSE;
                break;
            }
        } else {
            /* Accept only alphabet, digits and - & _ */
            if ((inst_desc[i] == 96) ||
                ((inst_desc[i] >= 46) && (inst_desc[i] <= 47)) ||
                ((inst_desc[i] >= 58) && (inst_desc[i] <= 64)) ||
                ((inst_desc[i] >= 91) && (inst_desc[i] <= 94))) {
                chk_st = FALSE;
                break;
            } else {
                /* At least one alphabet must be present */
                if (((inst_desc[i] >= 65) && (inst_desc[i] <= 90)) ||
                    ((inst_desc[i] >= 97) && (inst_desc[i] <= 122))) {
                    chk_st = TRUE;
                }
            }
        }
...
static void _ipmc_lib_cmn_static_mem_init(void)
{
    ...
    mem_obj = &ipmc_mem_pool.obj;
    memset(mem_obj, 0x0, sizeof(mem_obj));
