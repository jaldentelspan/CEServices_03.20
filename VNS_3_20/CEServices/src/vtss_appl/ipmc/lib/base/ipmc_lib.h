...
#define IPMC_LIB_ISID_PASS(x, y)        (((x) == VTSS_ISID_GLOBAL) ? IPMC_LIB_ISID_EXIST((y)) : \
                                         (((x) == VTSS_ISID_LOCAL) ? msg_switch_is_local((y)) : ((x) == (y))))
...
#define IPMC_LIB_NAME_CHECK(x)          (ipmc_lib_instance_name_check((x)))
#define IPMC_LIB_DESC_CHECK(x)          (ipmc_lib_instance_desc_check((x)))
...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            ipmcv4addr          source_addr[IPMC_NO_OF_PKT_SRCLIST - 1]; /* Max. 366 for MTU 1500 */
        } report;
    } sfminfo;
...
typedef struct {
    ...
    vtss_ipv6_t                 buf[IPMC_NO_OF_PKT_SRCLIST];
