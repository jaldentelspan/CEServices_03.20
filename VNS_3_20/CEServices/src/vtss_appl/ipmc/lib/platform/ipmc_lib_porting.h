...
#else
#define IPMC_IP_INTF_IFID_TABLE_WALK(x)     if ((ip_mgmt_conf_cur_get((x)) == VTSS_OK) || (ip_mgmt_conf_get((x)) == VTSS_OK))
#define IPMC_IP_INTF_MAX_OPST               1
#define IPMC_IP_INTF_OPST_UP(x)             TRUE
#define IPMC_IP_INTF_OPST_VID(x)            ((x)->vid)
#define IPMC_IP_INTF_OPST_ADR4(x)           ((x)->ipv4_addr)
#define IPMC_IP_INTF_OPST_GET(x, y, z)
#define IPMC_IP_INTF_OPST_NGET_CTN(x, y, z)
...
#define IPMC_LIB_MSG_ISID_VALID_SLV(x)      (!msg_switch_is_local((x)) && msg_switch_exists((x)))
