...
static void HTTPS_cli_cmd_conf(cli_req_t *req, BOOL mode, BOOL redirect)
{
    ...
    if ((conf = malloc(sizeof(https_conf_t))) == NULL) {
        return;
    }
...
{
    char *cert_info = malloc(HTTPS_MGMT_MAX_CERT_LEN + 1);
...
{
    char *sess_info = malloc(20480);

    if (!sess_info) {
        return;
    }
    memset(sess_info, 0, sizeof(sess_info));
    if (https_mgmt_sess_info(sess_info, 20480) == VTSS_OK) {
...
static void HTTPS_cli_cmd_load_cert(cli_req_t *req)
{
    ...
    if ((buffer = malloc(HTTPS_MGMT_MAX_CERT_LEN + 1)) != NULL) {
        vtss_rc      rc;
        https_conf_t *https_conf, *newconf;

        if ((https_conf = malloc(sizeof(https_conf_t))) == NULL) {
            free(buffer);
            return;
        }
        if ((newconf = malloc(sizeof(https_conf_t))) == NULL) {
            free(buffer);
            free(https_conf);
            return;
        }
