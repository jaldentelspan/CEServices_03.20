...
void https_conf_mgmt_get_default(https_conf_t *conf)
{
    ...
    conf->mode = HTTPS_MGMT_DISABLED;
    conf->redirect = HTTPS_MGMT_DISABLED;
    conf->self_signed_cert = TRUE;
    conf->active_sess_timeout = 0; /* disable active session timeout */
    conf->absolute_sess_timeout = 0; /* disable absolute session timeout */
#if HTTPS_USING_DEFAULT_CERTIFICATE
...
static BOOL https_mgmt_check_cert_subject_name(void)
{
    ...
    cert_info = malloc(HTTPS_MGMT_MAX_CERT_LEN + 1);
    if (!cert_info) {
        return FALSE;
    }
