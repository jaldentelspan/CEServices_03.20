...
/******************************************************************************/
static vtss_rc PSEC_LIMIT_master_isid_port_check(vtss_isid_t isid, vtss_port_no_t port)
{
  ...
  /*lint -e{685, 568} */
  if (port < VTSS_PORT_NO_START || port >= port_isid_port_count(isid) + VTSS_PORT_NO_START || port_isid_port_no_is_stack(isid, port)) {
...
static psec_add_method_t PSEC_LIMIT_on_mac_add_callback(vtss_isid_t isid, vtss_port_no_t port, vtss_vid_mac_t *vid_mac, u32 mac_cnt_before_callback, psec_add_action_t *action)
{
  ...
  if ((rc = PSEC_LIMIT_master_isid_port_check(isid, port)) != VTSS_OK || action == NULL) {
...
/******************************************************************************/
static void PSEC_LIMIT_cfg_default_glbl(psec_limit_glbl_cfg_t *cfg)
{
...
/******************************************************************************/
static void PSEC_LIMIT_cfg_default_switch(psec_limit_switch_cfg_t *cfg)
{
  vtss_port_no_t port;
...
static void PSEC_LIMIT_cfg_flash_read(vtss_isid_t isid_add, BOOL create_defaults)
{
  ...
  if (flash_read_failed || PSEC_LIMIT_cfg_valid(&flash_cfg->cfg) != VTSS_OK) {
...
vtss_rc psec_limit_mgmt_glbl_cfg_get(psec_limit_glbl_cfg_t *glbl_cfg)
{
  ...
  if ((rc = PSEC_LIMIT_master_isid_port_check(VTSS_ISID_START, VTSS_PORT_NO_START)) != VTSS_OK) {
    return rc;
  }
...
vtss_rc psec_limit_mgmt_switch_cfg_get(vtss_isid_t isid, psec_limit_switch_cfg_t *switch_cfg)
{
  ...
  if ((rc = PSEC_LIMIT_master_isid_port_check(isid, VTSS_PORT_NO_START)) != VTSS_OK) {
    return rc;
  }
