...
static void qos_conf_apply(qos_conf_t *conf)
{
    ...
    if ((rc = vtss_qos_conf_get(NULL, &qos_ll_conf)) != VTSS_OK) {
...
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_V1)
        {
            int i;
            for (i = 0; i < 64; i++) {
                qos_ll_conf.dscp_remark[i] = 1;
            }
            qos_ll_conf.dscp_remark[0] = 0;
            qos_ll_conf.dscp_remark[8] = 0;
            qos_ll_conf.dscp_remark[16] = 0;
            qos_ll_conf.dscp_remark[24] = 0;
            qos_ll_conf.dscp_remark[32] = 0;
            qos_ll_conf.dscp_remark[40] = 0;
            qos_ll_conf.dscp_remark[48] = 0;
            qos_ll_conf.dscp_remark[56] = 0;
            qos_ll_conf.dscp_remark[46] = 0;
        }
...
static void qos_port_conf_default_set(qos_port_conf_t *conf)
{
    ...
    conf->qcl_no = QOS_PORT_DEFAULT_QCL;            /* assign default QCL ID to 1 */
    ...
    conf->dscp_remarking_val[VTSS_QUEUE_START] = 8;
    conf->dscp_remarking_val[VTSS_QUEUE_START + 1] = 16;
    conf->dscp_remarking_val[VTSS_QUEUE_START + 2] = 24;
    conf->dscp_remarking_val[VTSS_QUEUE_START + 3] = 32;
#endif /* VTSS_FEATURE_QOS_DSCP_REMARK_V1 */
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_V2)
...
#ifdef VTSS_FEATURE_QCL_V1
    if (conf->qcl_no > (QCL_ID_END + RESERVED_QCL_CNT)) {
        T_W("Invalid parameter");
        return QOS_ERROR_PARM;
    }
#endif /* VTSS_FEATURE_QCL_V1 */
...
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_V1)
    for (i = 0; i < QOS_PORT_QUEUE_CNT; i++) {
        if (conf->dscp_remarking_val[i] > 63) {
            T_W("Invalid parameter");
            return QOS_ERROR_PARM;
        }
    }
...
static void qos_port_conf_apply(vtss_port_no_t port_no, qos_port_conf_t *qos_port_conf)
{
    ...
    if ((rc = vtss_qos_port_conf_get(NULL, port_no, &qos_port_ll_conf)) != VTSS_OK) {
        ...
        qos_port_ll_conf.qcl_id = qos_port_conf->qcl_no - 1;           //driver layer works on 0-based now.
#endif /* VTSS_FEATURE_QCL_V1 */
        ...
        for (i = 0; i < QOS_PORT_TR_DPL_CNT; i++) {
            qos_port_ll_conf.tag_dp_map[i] = qos_port_conf->tag_dp_map[i];
        }
        for (i = VTSS_PRIO_START; i < QOS_PORT_PRIO_CNT; i++) {
            int dpl;
            for (dpl = 0; dpl < 2; dpl++) {
...
#if defined(VTSS_SW_OPTION_BUILD_SMB) || defined(VTSS_SW_OPTION_BUILD_CE)
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_V1)
        qos_port_ll_conf.dscp_mode = VTSS_DSCP_MODE_SEL;
        qos_port_ll_conf.dscp_remark = qos_port_conf->qos_remarking_enable;
        qos_port_ll_conf.dscp_map[VTSS_QUEUE_START] = qos_port_conf->dscp_remarking_val[VTSS_QUEUE_START];
        qos_port_ll_conf.dscp_map[VTSS_QUEUE_START + 1] = qos_port_conf->dscp_remarking_val[VTSS_QUEUE_START + 1];
        qos_port_ll_conf.dscp_map[VTSS_QUEUE_START + 2] = qos_port_conf->dscp_remarking_val[VTSS_QUEUE_START + 2];
        qos_port_ll_conf.dscp_map[VTSS_QUEUE_START + 3] = qos_port_conf->dscp_remarking_val[VTSS_QUEUE_START + 3];
#endif /* VTSS_FEATURE_QOS_DSCP_REMARK_V1 */
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_V2)
...
static vtss_rc qce_entry_add(vtss_qcl_id_t qcl_id, vtss_qce_id_t qce_id, qos_qce_entry_conf_t *conf)
{
    ...
    case VTSS_QCE_TYPE_ETYPE:
        qce.frame.etype.val = conf->frame.etype.val;
        qce.frame.etype.prio = conf->frame.etype.prio;
        break;
    case VTSS_QCE_TYPE_VLAN:
        qce.frame.vlan.vid = conf->frame.vlan.vid;
        qce.frame.vlan.prio = conf->frame.vlan.prio;
        break;
    case VTSS_QCE_TYPE_UDP_TCP:
        qce.frame.udp_tcp.low = conf->frame.udp_tcp.low;
        qce.frame.udp_tcp.high = conf->frame.udp_tcp.high;
        qce.frame.udp_tcp.prio = conf->frame.udp_tcp.prio;
        break;
    case VTSS_QCE_TYPE_DSCP:
        qce.frame.dscp.dscp_val = conf->frame.dscp.dscp_val;
        qce.frame.dscp.prio = conf->frame.dscp.prio;
        break;
    case VTSS_QCE_TYPE_TOS:
        for (i = 0; i < 8; i++) {
            qce.frame.tos_prio[i] = conf->frame.tos_prio[i];
        }
        break;
    case VTSS_QCE_TYPE_TAG:
        for (i = 0; i < 8; i++) {
            qce.frame.tag_prio[i] = conf->frame.tag_prio[i];
        }
        break;
#endif /* VTSS_FEATURE_QCL_V1 */
    ...
    case VTSS_QCE_TYPE_SNAP: {
        /* First 3 bytes in API structure are OUI, currently it supports
           '0' (Ethernet) mgmt at this moment */
        qce.key.frame.snap.data.value[0] = 0;
        qce.key.frame.snap.data.value[1] = 0;
        qce.key.frame.snap.data.value[2] = 0;
        qce.key.frame.snap.data.mask[0] = 0xFF;
        qce.key.frame.snap.data.mask[1] = 0xFF;
        qce.key.frame.snap.data.mask[2] = 0xFF;
        /* next 2 bytes are PID */
    ...
    case VTSS_QCE_TYPE_IPV6: {
        ...
        qce.key.frame.ipv6.sip = conf->key.frame.ipv6.sip;
 ...
 */
#if defined(VTSS_FEATURE_QCL_V1)
static vtss_rc qos_qce_entry_add(qcl_qce_list_table_t *list, const vtss_qce_id_t qce_id, qos_qce_entry_conf_t *conf)
{
    qcl_qce_t     *qce, *prev, *ins = NULL, *ins_prev = NULL, *new = NULL, *new_prev = NULL;
    char          id_used[QCE_MAX];

    QOS_CRIT_ASSERT_LOCKED();
    T_D("enter, qce_id: %u, conf->id: %u", qce_id, conf->id);

    /* Check that the QCE ID is valid */
    if (qce_id == conf->id && qce_id != 0) {
    ...
    for (qce = list->qce_used_list, prev = NULL; qce != NULL; prev = qce, qce = qce->next) {
        if (qce->conf.id == conf->id && conf->id != 0) {
        ...
        /* Mark ID as used */
        id_used[qce->conf.id - QCE_ID_START] = 1;
    }
    ...
    if (qce_id != QCE_ID_NONE && ins == NULL) {
        T_W("qce_id %d not found", qce_id);
        return QOS_ERROR_QCE_NOT_FOUND;
    }

    if (new == NULL) {
        /* Take new entry from free list */
        if ((new = list->qce_free_list) == NULL) {
            T_W("QCE is full");
    ...
    if (conf->id == QCE_ID_NONE) {
        vtss_qce_id_t i;
        /* Find and use lowest available ID */
        for (i = QCE_ID_START; i <= QCE_ID_END; i++) {
            if (!id_used[i - QCE_ID_START]) {
                conf->id = i;
                T_D("using next free ID %d", i);
                break;
            }
        }
    }
    memcpy(&new->conf, conf, sizeof(qos_qce_entry_conf_t));
    T_D("exit ok");
    return VTSS_OK;
}
 ...
 * \return : Return code
 */
static vtss_rc qos_qce_entry_del(qcl_qce_list_table_t *list, const vtss_qce_id_t qce_id)
{
    qcl_qce_t *qce, *prev;
...
#if defined(VTSS_FEATURE_QCL_V1)
static vtss_rc qos_qcl_full(const vtss_qcl_id_t qcl_id)
{
    int          i;
    qcl_qce_t    *entry;
    ...
    for (i = 0, entry = qos.qcl_qce_stack_list[qcl_id].qce_used_list;
         entry != NULL; i++, entry = entry->next) {

        switch (entry->conf.type) {
        case VTSS_QCE_TYPE_ETYPE:
        case VTSS_QCE_TYPE_VLAN:
        case VTSS_QCE_TYPE_TOS:
        case VTSS_QCE_TYPE_TAG:
            ...
            break;
        case VTSS_QCE_TYPE_UDP_TCP:
            if (entry->conf.frame.udp_tcp.low != entry->conf.frame.udp_tcp.high) {
            ...
            break;
        case VTSS_QCE_TYPE_DSCP:
            if (entry->next != NULL && entry->next->conf.type == VTSS_QCE_TYPE_DSCP) {
                /* If next entry is DSCP, merge */
                entry = entry->next;
            }
            break;
        default:
            T_E("Unknown QCE type (%d)", entry->conf.type);
            return QOS_ERROR_GEN;
        }
    }
    if (i > QOS_QCL_PORT_MAX) {
        return QOS_ERROR_QCE_TABLE_FULL;
    } else {
        return VTSS_OK;
    }
...
static BOOL qos_msg_rx(void *contxt, const void *const rx_msg, size_t len, vtss_module_id_t modid, ulong isid)
{
    ...
    case QOS_MSG_ID_QCE_ADD_REQ: {
        ...
        (void) qos_qce_entry_add(list, msg->req.qce_add.qce_id, &msg->req.qce_add.conf);
        QOS_CRIT_EXIT();
        (void) qce_entry_add(msg->req.qce_add.qcl_id, msg->req.qce_add.qce_id, &msg->req.qce_add.conf);
#endif
#if defined(VTSS_FEATURE_QCL_V2)
        qos_qce_entry_conf_t *conf;
        qcl_qce_list_table_t *list;
    ...
    case QOS_MSG_ID_QCE_DEL_REQ: {
        ...
        (void) qos_qce_entry_del(list, msg->req.qce_del.qce_id);
        QOS_CRIT_EXIT();
        (void) vtss_qce_del(NULL, msg->req.qce_del.qcl_id, msg->req.qce_del.qce_id);
#endif
#if defined(VTSS_FEATURE_QCL_V2)
        qcl_qce_list_table_t *list;
        vtss_isid_t           dummy;
    ...
    case QOS_MSG_ID_QCL_CLEAR_REQ: {
        ...
        for (i = 0; i < QCL_MAX + RESERVED_QCL_CNT; i++) {
            for (qce = qos.qcl_qce_switch_list[i].qce_used_list, prev = NULL; qce != NULL; prev = qce, qce = qce->next) {
                (void) vtss_qce_del(NULL, i, qce->conf.id);
            }
            /* Free old QCEs in the specific QCL */
            if (prev != NULL) {
                prev->next = qos.qcl_qce_switch_list[i].qce_free_list;
                qos.qcl_qce_switch_list[i].qce_free_list = qos.qcl_qce_switch_list[i].qce_used_list;
...
static vtss_rc qce_entry_stack_add(vtss_isid_t isid_add, vtss_qcl_id_t qcl_id, vtss_qce_id_t qce_id, qos_qce_entry_conf_t *conf)
{
    qos_msg_req_t *msg;
    switch_iter_t  sit;
    ...
    /* Allocate a message with a ref-count corresponding to the number of times switch_iter_getnext() will return TRUE. */
    if ((msg = qos_msg_alloc(QOS_MSG_ID_QCE_ADD_REQ, sit.remaining)) != NULL) {
        msg->req.qce_add.qcl_id = qcl_id;
        msg->req.qce_add.qce_id = qce_id;
...
static vtss_rc qos_stack_qcl_update(vtss_isid_t isid_add)
{
    ...
    for (i = 0; i < QCL_MAX + RESERVED_QCL_CNT; i++) {
        for (qce = qos.qcl_qce_stack_list[i].qce_used_list; qce != NULL; qce = qce->next) {
            ...
            (void) qce_entry_stack_add(isid_add, i, 0, &qce->conf);
        }
    }
    T_N("exit");
    return VTSS_OK;
...
vtss_rc qos_mgmt_qce_entry_get(vtss_qcl_id_t qcl_id, vtss_qce_id_t qce_id, qos_qce_entry_conf_t *conf, BOOL next)
{
    ...
    T_N("enter, qcl: %u, qce: %u, next: %d", qcl_id, qce_id, next);
...
vtss_rc qos_mgmt_qce_entry_add(vtss_qcl_id_t qcl_id, vtss_qce_id_t qce_id, qos_qce_entry_conf_t *conf)
{
    ...
    T_N("enter, qcl: %u, qce: %u", qcl_id, qce_id);
    ...
    list = &qos.qcl_qce_stack_list[qcl_id];
    rc = qos_qce_entry_add(list, qce_id, conf);

    if (rc == VTSS_OK) {
        rc = qos_qcl_full(qcl_id);
        if (rc != VTSS_OK) {
            (void) qos_qce_entry_del(list, conf->id);
        }
    }
    if (rc == VTSS_OK) {
    ...
    if (rc == VTSS_OK) {
        rc = qce_entry_stack_add(VTSS_ISID_GLOBAL, qcl_id, qce_id, conf);
    }
    QOS_CRIT_EXIT();
    T_N("exit");
    return rc;
...
vtss_rc qos_mgmt_qce_entry_del(vtss_qcl_id_t qcl_id, vtss_qce_id_t qce_id)
{
    ...
    QOS_CRIT_ENTER();
    list = &qos.qcl_qce_stack_list[qcl_id];
    rc = qos_qce_entry_del(list, qce_id);

    if (rc == VTSS_OK) {
        rc = qos_qcl_conf_commit();
    }
    if (rc == VTSS_OK) {
        rc = qce_entry_stack_del(VTSS_ISID_GLOBAL, qcl_id, qce_id);
    }
    QOS_CRIT_EXIT();
    T_N("exit");
    return rc;
...
static void QOS_port_conf_flash_read(vtss_isid_t isid_add, BOOL force_defaults)
{
    ...
    // Open or create configuration block
    if ((qos_port_blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_QOS_PORT_TABLE, &size)) == NULL || (size != sizeof(qos_port_conf_blk_t))) {
        T_W("isid:%d, conf_sec_open() failed or size mismatch, creating defaults", isid_add);
...
static void QOS_qce_conf_flash_read(vtss_isid_t isid_add, BOOL force_defaults)
{
    ...
    if (!do_create) {
        if ((qce_blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_QOS_QCL_QCE_TABLE, &size)) == NULL) {
            ...
            if ((size != (sizeof(qcl_qce_blk_t))) || (qce_blk->version != QOS_QCL_QCE_BLK_VERSION)) {
                do_create = 1;
        ...
        for (i = 0; i < QCL_MAX + RESERVED_QCL_CNT; i++) {
...
#if defined(VTSS_FEATURE_QCL_V1)
            vtss_qce_id_t id = VTSS_QCE_ID_LAST;
#endif
            ...
            for (j = qce_blk->count[i]; j != 0; j--) {
                ...
                if (force_defaults) {
                    /* apply this configuration to the whole stack */
#if defined(VTSS_FEATURE_QCL_V1)
                    (void) qce_entry_stack_add(VTSS_ISID_GLOBAL, i, id, &qce->conf);
                    id = qce->conf.id;
#else
...
static void qos_start(BOOL init)
{
    ...
    if (init) {
        ...
        for (i = 0; i < QCL_MAX + RESERVED_QCL_CNT; i++) {
            ...
            if ((qos.qcl_qce_stack_list[i].qce_list = malloc(sizeof(qcl_qce_t) * QCE_ID_END))) {
        ...
        for (i = 0; i < QCL_MAX + RESERVED_QCL_CNT; i++) {
            ...
            if ((qos.qcl_qce_switch_list[i].qce_list = malloc(sizeof(qcl_qce_t) * (QCE_MAX + RESERVED_QCE_CNT)))) {
