...
/****************************************************************************/
// SYMREG_cli_parse_keyword()
/****************************************************************************/
static int32_t SYMREG_cli_parse_keyword(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
  symreg_cli_req_t *symreg_req = req->module_req;
  char             *found = cli_parse_find(cmd, stx);

  if (found != NULL) {
    if (!strncmp(found, "binary", 6)) {
      symreg_req->binary = 1;
    } else if (!strncmp(found, "decimal", 7)) {
      symreg_req->decimal = 1;
    }
  }
...
/****************************************************************************/
// SYMREG_cli_reg_value()
/****************************************************************************/
static void SYMREG_cli_reg_value(symreg_cli_req_t *symreg_req, u32 value)
{
  int i;

  if (symreg_req->binary) {
    for (i = 31; i >= 0; i--) {
      CPRINTF("%d%s", value & (1 << i) ? 1 : 0, (i % 4) || i == 0 ? "" : ".");
    }
  } else if (symreg_req->decimal) {
    CPRINTF("%10u", value);
  } else {
...
/****************************************************************************/
static void SYMREG_cli_print_component(char *name, int repl, u32 repl_cnt, char *post_str)
{
  char buf[13];
  if (repl_cnt == 1) {
    buf[0] = '\0';
  ...
  CPRINTF("%s%s%s", name, buf, post_str);
}
...
static void SYMREG_cli_reg_loop(symreg_cli_req_t *symreg_req, const SYMREG_target_t *tgt, const SYMREG_reggrp_t *reggrp, int reggrp_repl, int reg_repl)
{
  ...
  int                reg_repl_min, reg_repl_max, reg_repl_iter;
  u32                value = 0;
  ...
  while (reg->name != NULL) {
    if ((symreg_req->names[SYMREG_CLI_REG][0] == '\0' || strcmp(symreg_req->names[SYMREG_CLI_REG], reg->name) == 0) &&
      ...
      if (symreg_req->verify) {
        // Update the number of addresses we expect to read or write in next iteration.
        symreg_req->addr_cnt += reg_repl_max - reg_repl_min + 1;
      } else {
        for (reg_repl_iter = reg_repl_min; reg_repl_iter <= reg_repl_max; reg_repl_iter++) {
          ...
          SYMREG_cli_print_component(tgt->name,    tgt->repl_number, tgt->repl_number == -1 ? 1 : 2, ":");
          SYMREG_cli_print_component(reggrp->name, reggrp_repl,      reggrp->repl_cnt,               ":");
          SYMREG_cli_print_component(reg->name,    reg_repl_iter,    reg->repl_cnt,                  " = ");
          if (rc == VTSS_RC_OK) {
            SYMREG_cli_reg_value(symreg_req, value);
          } else {
...
static cli_parm_t SYMREG_cli_parm_table[] = {
  ...
  {
    .txt            = "binary|decimal",
    .help           = "binary     : Show value in binary format\n"
                      "decimal    : Show value in decimal format\n"
                      "(default: Show value in hexadecimal format)",
    .flags          = CLI_PARM_FLAG_NO_TXT,
    .parm_parse_fun = SYMREG_cli_parse_keyword,
    .cmd_fun        = NULL
  },
  {
    .txt            = NULL,
...
cli_cmd_tab_entry(
  NULL,
  SYMREG_CLI_PATH "Read <reg_syntax> [binary|decimal]",
