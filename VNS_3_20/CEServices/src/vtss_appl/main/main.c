...
#ifdef VTSS_SW_OPTION_VNS_FPGA
#include "vns_fpga_api.h"
#endif

#ifdef VTSS_SW_OPTION_IES_2_0
#include "vns_fpga_api.h"
#endif

#ifdef VTSS_SW_OPTION_MITR_FPGA
#include "mitr_fpga_api.h"
#endif

#ifdef VTSS_SW_OPTION_GVCP
#include "gvcp_api.h"
#endif
...
const char *error_txt(vtss_rc rc)
{
    ...
    sprintf(txt_default, "module_id=%d, code=%d", module_id, code);
    T_D("%s",txt_default);
    if (rc <= VTSS_OK) {
        switch (module_id) {
...
vtss_rc init_modules(vtss_init_data_t *data)
{
    ...
    for(i = 0; i < (sizeof(initfun) / sizeof(initfun[0])); i++) {
        dbg_latest_init_modules_init_fun_idx = i;
        T_I("%s(%s, cmd: %d, isid: %x, flags: 0x%x)", __FUNCTION__, initfun[i].name, data->cmd, data->isid, data->flags);
               ...
               ulong isid)
{
    if(len == sizeof(control_msg_t)) {
        ...
        case CONTROL_MSG_ID_SYSTEM_REBOOT:
            (void) control_system_reset(TRUE, VTSS_ISID_LOCAL, mmsg->flags);
            break;
...
#endif
    if (dump_threads) {
        cli_print_thread_status(func_printf, TRUE);
    }
}
...
{
    cli_print_thread_status(diag_printf, TRUE);
    hal_vcoreiii_reset();
}
#endif
