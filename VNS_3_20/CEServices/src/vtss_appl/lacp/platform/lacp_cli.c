                       ...
                       BOOL sysprio)
{
    ...
    while(l2port_iter_getnext(&l2pit) && status) {
    ...
    if (sysprio) {            
        if (lacp_get_system_config(&sysconf) == VTSS_OK) {
            if (req->set) {
                ...
                if (lacp_set_system_config(&sysconf) != VTSS_OK) {
    ...
    for (usid = VTSS_USID_START; usid < VTSS_USID_END; usid++) {
        ...
        for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
            ...
            if (lacp_get_port_config(isid, iport, &conf) != VTSS_OK ||
                lacp_get_port_status(l2port, &stat) != VTSS_OK)
                continue;

            /* Handle 'statistics clear' command */
            if (req->clear) {
                lacp_clear_statistics(l2port);
                continue;
            }
            ...
            if (req->set) {
                ...
                rc = lacp_set_port_config(isid, iport, &conf);
            ...
            (void) l2port_iter_init(&l2pit, VTSS_ISID_GLOBAL, L2PORT_ITER_TYPE_PHYS);
            while(l2port_iter_getnext(&l2pit) && status) {
                if (lacp_get_aggr_status(l2pit.l2port, &aggr) && aggr.port_list[L2PORT2PORT(isid, iport) - VTSS_PORT_NO_START]) {
                    break;
                }
            } 
            CPRINTF("%-2u    ", uport);
