...
static cyg_int32 handler_config_lacp_ports(CYG_HTTPD_STATE* p)
{
    ...
    if(p->method == CYG_HTTPD_METHOD_POST) {
        ...
        while (port_iter_getnext(&pit)) {
            if (lacp_get_port_config(isid, pit.iport, &conf) != VTSS_OK)
                continue;
            ...
            if(memcmp(&new, old, sizeof(*old)) != 0) {
                *old = new;
                if ((rc = lacp_set_port_config(isid, pit.iport, &conf)) != VTSS_OK) {
...
static cyg_int32 handler_stat_lacp_port_status(CYG_HTTPD_STATE* p)
{
    ...
    for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
        if (port_isid_port_no_is_stack(isid,iport) || lacp_get_port_status(L2PORT2PORT(isid, iport), &pp) != VTSS_OK)
            continue;
        ...
        while(l2port_iter_getnext(&l2pit)) {
            if (lacp_get_aggr_status(l2pit.l2port, &aggr)) {
...
static cyg_int32 handler_stat_lacp_statistics(CYG_HTTPD_STATE* p)
{
    ...
    for (iport = VTSS_PORT_NO_START; iport < port_count; iport++) {
        ...
        if(var_clear[0])       /* Clear? */
            lacp_clear_statistics(L2PORT2PORT(isid, iport));

        lacp_get_port_status(L2PORT2PORT(isid, iport), &pp);
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                      "%u/%lu/%lu/%lu/%lu|",
                      iport2uport(iport),
