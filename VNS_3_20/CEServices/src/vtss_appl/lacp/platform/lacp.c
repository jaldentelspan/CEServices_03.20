...
void vtss_os_set_hwaggr(vtss_lacp_agid_t aid, vtss_common_port_t new_port) 
{
    ...
    if (aggr_mgmt_lacp_members_add(aid, new_port) == AGGR_ERROR_REG_TABLE_FULL)  {
        /* Not possible to aggregate this port.  Disabled LACP for that port */
    }
}
...
/* Port state callback function  This function is called if a port state change occur.  */
...
{   
    if(msg_switch_is_master() && !info->stack) {
        ...
        /* When a LACP enabled port get a link-up event, port is set to delayed forwarding. */
        /* This will give LACP time to form aggregation.                                    */
        (void)lacp_get_port_config(isid, port_no, &pconf);
        if (info->link) {
...
vtss_rc lacp_get_system_config(vtss_lacp_system_config_t *conf)
{
    if(!msg_switch_is_master())
...
vtss_rc lacp_set_system_config(const vtss_lacp_system_config_t *conf)
{
...
vtss_rc lacp_get_port_config(vtss_isid_t isid,
...
vtss_rc lacp_set_port_config(vtss_isid_t isid,
                             vtss_port_no_t port_no,
                             const vtss_lacp_port_config_t *pconf)
{
    l2_port_no_t l2port = L2PORT2PORT(isid, port_no);
    vtss_rc rc = VTSS_INCOMPLETE;
    lacp_conf_blk_t *blk;
...
vtss_common_bool_t lacp_get_aggr_status(unsigned int aid, 
                               vtss_lacp_aggregatorstatus_t *stat)
{
...
vtss_rc lacp_get_port_status(l2_port_no_t l2port,
...
void lacp_clear_statistics(l2_port_no_t l2port)
{
    vtss_lacp_clear_statistics(l2port);
}


static void lacp_conf_read(BOOL force_default)
{
    ...
    if ((lacp_blk = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_LACP_CONF, &size)) == NULL || 
        size != sizeof(*lacp_blk)) {
    ...
    if (do_create || force_default) {
        /* Use default values */
        lacp_blk->conf.system.system_prio = VTSS_LACP_DEFAULT_SYSTEMPRIO;
        memcpy(lacp_blk->conf.system.system_id.macaddr, VTSS_COMMON_ZEROMAC, sizeof(VTSS_COMMON_ZEROMAC));
           
        (void) l2port_iter_init(&l2pit, VTSS_ISID_GLOBAL, L2PORT_ITER_TYPE_PHYS);
        while(l2port_iter_getnext(&l2pit)) {
            lacp_blk->conf.ports[l2pit.l2port].port_prio = VTSS_LACP_DEFAULT_PORTPRIO;
            lacp_blk->conf.ports[l2pit.l2port].port_key = VTSS_LACP_AUTOKEY;
            lacp_blk->conf.ports[l2pit.l2port].enable_lacp = FALSE;
            lacp_blk->conf.ports[l2pit.l2port].xmit_mode = VTSS_LACP_DEFAULT_FSMODE;
            lacp_blk->conf.ports[l2pit.l2port].active_or_passive = VTSS_LACP_DEFAULT_ACTIVITY_MODE;
        }       
...
static BOOL rx_lacp(void  *contxt, const uchar *const frm, const vtss_packet_rx_info_t *const rx_info)
{
    ...
    // Check the subtype, is '1' for LACP
    if (frm[14] != 1) {
        return FALSE;  // Not a LACP frame        
    } 

    // NB: Null out the GLAG (port is 1st in aggr)
    l2_receive_indication(VTSS_MODULE_ID_LACP, frm, rx_info->length, rx_info->port_no, rx_info->tag.vid,
                          (vtss_glag_no_t) (VTSS_GLAG_NO_START-1));

    return TRUE; // Do not allow other subscribers to receive the packet
} 

/*  Master only. Receive packet from l2proto - send it to core  */
...
static void lacp_thread(cyg_addrword_t data)
{
    ...
    while (1) {     
        ...
        while(msg_switch_is_master()) {
            /* If LACP gets disabled it goes to sleep after after cleaning up the protocol states */
            if (lacp_disable) {
                lacp_sleep_soon++;                
            } 
            if (lacp_sleep_soon == 10) {
                cyg_flag_wait(&lacp_flags, 
                              FLAG_LACP_ENABLED,
...
vtss_rc lacp_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_SWITCH_ADD:
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < data->port_cnt; port_no++) {              
