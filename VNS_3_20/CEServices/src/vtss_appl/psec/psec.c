...
//   PSEC_LEARN_CPU_REASON_OTHER
/******************************************************************************/
static void PSEC_sec_learn_cpu_copy_check(vtss_isid_t isid, vtss_port_no_t port, psec_learn_cpu_reason_t reason)
{
...
static void PSEC_mac_del(vtss_isid_t isid, vtss_port_no_t port, psec_mac_state_t *mac_state, psec_del_reason_t reason)
{
  ...
  if (reason == PSEC_DEL_REASON_HW_ADD_FAILED ||
    ...
    if (reason == PSEC_DEL_REASON_HW_ADD_FAILED) {
      ...
      PSEC_sec_learn_cpu_copy_check(isid, port, PSEC_LEARN_CPU_REASON_OTHER);
      ...
      PSEC_sec_learn_cpu_copy_check(isid, port, PSEC_LEARN_CPU_REASON_MAC_ADDRESS_ALLOCATED);
    }
    ...
    PSEC_sec_learn_cpu_copy_check(isid, port, PSEC_LEARN_CPU_REASON_MAC_ADDRESS_FREED);
  }
...
vtss_rc psec_mgmt_port_state_get(vtss_isid_t isid, vtss_port_no_t port, psec_port_state_t *state, BOOL get_macs)
{
  ...
  *state = PSEC_stack_state.switch_state[isid - VTSS_ISID_START].port_state[port - VTSS_PORT_NO_START];
  if (get_macs) {
    // Caller wants the individual MAC states. We must malloc() and copy our current states
    ...
    if (mac_cnt) {
      ...
      new_macs = malloc(mac_cnt * sizeof(psec_mac_state_t));
      if (new_macs) {
        macs = state->macs;
...
#error "Invalid PSEC_MAC_ADDR_ENTRY_CNT"
  ...
  case INIT_CMD_SWITCH_ADD: {
    ...
    PSEC_sec_learn_cpu_copy_check(isid, VTSS_PORTS /* Doesn't matter */, PSEC_LEARN_CPU_REASON_SWITCH_UP_OR_DOWN);
