...
#ifdef VTSS_SW_OPTION_IP2
        vtss_if_status_t    *ops, ipst[DNS_IP_INTF_MAX_OPST];
        u32                 ops_cnt;
        BOOL                intf_up, dns2me;
        vtss_ipv4_t         intf_adr;
        u32                 ops_idx;
        i8                  adrBuf1[40], adrBuf2[40];

        T_D("RCV from INTF-VID-%u", rx_info->tag.vid);
        dns2me = FALSE;
        intf_ifid = 0;
        while (DNS_IP_INTF_IFID_GET_NEXT(intf_ifid)) {
            ops_cnt = 0;
            memset(ipst, 0x0, sizeof(ipst));
            if (!DNS_IP_INTF_OPST_GET(intf_ifid, ipst, ops_cnt)) {
                continue;
            }
            ...
            for (ops_idx = 0; ops_idx < ops_cnt; ops_idx++) {
                ...
                if (DNS_IP_INTF_OPST_UP(ops)) {
                    intf_up = TRUE;
                }
                if (ops->type == VTSS_IF_STATUS_TYPE_IPV4) {
                    intf_adr = DNS_IP_INTF_OPST_ADR4(ops);
                }
            }

            memset(adrBuf1, 0x0, sizeof(adrBuf1));
            memset(adrBuf2, 0x0, sizeof(adrBuf2));
            T_D("INTF-ADR-%s is %s (DST=%s)",
                misc_ipv4_txt(intf_adr, adrBuf1),
                intf_up ? "Up" : "Down",
                misc_ipv4_txt(frame_info.dip, adrBuf2));
            if (intf_up && intf_adr) {
                if (frame_info.dip == intf_adr) {
                    dns2me = TRUE;
                    break;
                }
            }
...
#else
        (void) ip_mgmt_conf_get(&ip_conf);
        (void) ip_mgmt_conf_cur_get(&ip_conf);
        if (ip_conf.ipv4_addr != frame_info.dip) {
            return FALSE;
        }
#endif /* VTSS_SW_OPTION_IP2 */
