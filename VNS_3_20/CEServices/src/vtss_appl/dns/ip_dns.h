...
#define DNS_IP_INTF_IFID_GET_NEXT(x)    (vtss_ip2_if_id_next((x), &(x)) == VTSS_OK)
#define DNS_IP_INTF_MAX_OPST            4
#define DNS_IP_INTF_OPST_UP(x)          ((x) ? (((x)->type == VTSS_IF_STATUS_TYPE_LINK) ? (((x)->u.link.flags&VTSS_IF_LINK_FLAG_UP) && ((x)->u.link.flags&VTSS_IF_LINK_FLAG_RUNNING)) : FALSE) : FALSE)
#define DNS_IP_INTF_OPST_VID(x)         ((x) ? (((x)->if_id.type == VTSS_ID_IF_TYPE_VLAN) ? (x)->if_id.u.vlan : 0) : 0)
#define DNS_IP_INTF_OPST_ADR4(x)        ((x) ? (((x)->type == VTSS_IF_STATUS_TYPE_IPV4) ? ((x)->u.ipv4.net.address) : 0) : 0)
#define DNS_IP_INTF_OPST_GET(x, y, z)   (vtss_ip2_if_status_get(VTSS_IF_STATUS_TYPE_ANY, (x), DNS_IP_INTF_MAX_OPST, &(z), (y)) == VTSS_OK)
#else
#define DNS_IP_INTF_OPST_UP(x)          ((x) ? TRUE : FALSE)
#endif /* VTSS_SW_OPTION_IP2 */

#define DNS_CONF_VERSION        2
