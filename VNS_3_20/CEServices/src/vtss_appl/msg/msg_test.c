...
static ushort msg_test_idx = 0; // Current test in progress
static ulong  msg_test_suite_args[3];
...
/****************************************************************************/
static BOOL msg_test_rx(void *contxt, const void * const msg, const size_t len, const vtss_module_id_t modid, const ulong id)
{
  int i;
  uchar *msg_ptr = (uchar *)msg;
  uchar test_idx = msg_ptr[0];
  uchar disid;
  ulong sz;

  T_I("msg_test_rx(): test_idx=%d, msg_ptr=%p, len=%zu, id=%u", test_idx, msg, len, id);

  VTSS_OS_CRIT_ENTER(&msg_test_crit);
  switch (test_idx) {
    case MSG_TEST_IDX_BASIC:
      T_N("RX(basic): m=\"%s\", l=%zu, SISID=%u", ((uchar *)msg)+1, len, (ulong)id);
      break;
    ...
    case MSG_TEST_IDX_JUMBO:
      ...
      for (i = 6; i < sz; i++) {
        if (*msg_ptr != (uchar)i) {
            T_E("Rx(jumbo): Unexpected value at offset = %zu. Expected %u, got %u", (size_t)(msg_ptr-(uchar *)msg), i, *msg_ptr);
          break;
        }
        msg_ptr++;
      }
...
{
  BOOL dynamic       = ((ulong)contxt & MSG_TEST_CONTXT_FLAG_DYNAMIC)       != 0;
  BOOL exp_cb        = ((ulong)contxt & MSG_TEST_CONTXT_FLAG_EXP_CB)        != 0;
  BOOL msg_mod_frees = ((ulong)contxt & MSG_TEST_CONTXT_FLAG_MSG_MOD_FREES) != 0;
  BOOL high_prio     = ((ulong)contxt & MSG_TEST_CONTXT_FLAG_HIGH_PRIO)     != 0;

  VTSS_OS_CRIT_ENTER(&msg_test_crit);
  T_N("TX DONE: rc=%d, m=\"%s\", d=%d, cb=%d, f=%d, p=%d", rc, (uchar *)msg, dynamic, exp_cb, msg_mod_frees, high_prio);
...
static void msg_test_basic_tx(BOOL dynamic, BOOL callback, BOOL msg_mod_frees, BOOL high_prio)
{
  ...
  if (dynamic && !callback && msg_mod_frees) {
    ...
    ulong contxt = MSG_TEST_CREATE_CONTXT(dynamic, callback, msg_mod_frees, high_prio);
    contxt |= (ulong)MSG_TEST_IDX_BASIC << 16;
...
{
  ushort      test_idx = (ulong)contxt >> 16;
  vtss_isid_t isid     = (ulong)contxt & 0xFFFF;
...
static void msg_test_jumbo(void)
{
  ...
  uchar       *long_msg[JUMBO_BUF_CNT], *msg_ptr[JUMBO_BUF_CNT];
  int         i, j, used_bufs = 0;
  ulong       contxt = (MSG_TEST_IDX_JUMBO << 16);
  ulong       sz = DEFAULT_JUMBO_MSG_SZ_BYTES;
  ...
  if (sz > MSG_MAX_LEN_BYTES) {
    sz = MSG_MAX_LEN_BYTES;
  }

  for (i = 0; i < JUMBO_BUF_CNT; i++) {
    msg_ptr[i] = long_msg[i] = malloc(sz);
    if (long_msg[i] == NULL) {
...
// msg_test_suite()
/******************************************************************************/
void msg_test_suite(ulong num, ulong arg0, ulong arg1, ulong arg2, int (*print_function)(const char *fmt, ...))
{
  int i;
  if (num == 0 || num > sizeof(msg_test_suites) / sizeof(msg_test_suites[0]) - 1) {
    print_function("Defined message test suites:\n");
    for (i = 1; i < sizeof(msg_test_suites) / sizeof(msg_test_suites[0]); i++) {
  ...
  if (msg_test_suite_in_progress) {
    print_function("A test is already in progress. Hold your horses\n");
    return;
  }
