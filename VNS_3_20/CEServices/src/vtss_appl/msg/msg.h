...
/****************************************************************************/
#define MSG_CFG_DEFAULT_MSYN_TIMEOUT_MS              2500
#define MSG_CFG_DEFAULT_MD_TIMEOUT_MS                2500
#define MSG_CFG_DEFAULT_MD_RETRANSMIT_LIMIT             1
#if MSG_HOP_BY_HOP
...
#define MSG_CFG_MAX_RETRANSMIT_LIMIT   10
...
typedef struct {
  ...
  ulong rx_pdu[MSG_PDU_TYPE_LAST_ENTRY][2];
  ulong rx_msg; // Successfully received user messages.
  ...
  ulong tx_pdu[MSG_PDU_TYPE_LAST_ENTRY][2];
  // Likewise, index 0 is for OK Tx, 1 for bad Tx:
  ulong tx_msg[2]; // Successfully/Unsuccessfully transmitted user messages.

  // Payload-only bytes.
  // Retransmissions are also counted.
  ulonglong tx_md_bytes;
...
typedef struct {
  ...
  // The second dimension use the first index ([][0]) for good frames, and
  // the last index ([][1]) for bad frames.
  ulong rx[MSG_PDU_TYPE_LAST_ENTRY + 1][2];

  // Payload-only bytes. Index 0 for OK and index 1 for bad MDs.
  // Retransmissions are also counted.
  ulonglong rx_md_bytes[2]; // Payload-only.
  ...
  ulong tx[MSG_PDU_TYPE_LAST_ENTRY][2][2];
...
typedef struct {
  ...
  ulong rx[VTSS_MODULE_ID_NONE+1][2];

  // -------------------------- TX ------------------------------
  // Entry [][0] are the good ones, [][1] are the bad ones.
  ulong tx[VTSS_MODULE_ID_NONE+1][2];

  ulong rxb[VTSS_MODULE_ID_NONE+1];
  ulong txb[VTSS_MODULE_ID_NONE+1];
  ...
  // in terms of RAM.
  ulong tx_per_return_code[MSG_TX_RC_LAST_ENTRY];
} msg_mod_per_state_usr_stat_t;
...
/****************************************************************************/
typedef struct {
  ulong rx;           // Total number of frames received for relaying (good and bad)
  ulong tx_ok;        // Number of frames successfully transmitted.
  ulong inv_port_err; // While relaying a frame: Topo returned an invalid stack port number. Frame dropped.
  ulong tx_err;       // While relaying a frame: Packet Module couldn't transmit the frame. Frame dropped.
  ulong htl_err;      // While relaying a frame: HTL field of incoming frame was 1. Frame dropped.
} msg_relay_stat_t;

#define SSP_HDR_SZ_BYTES              8 /* Includes 0x8880, EPID, SSPID, and RESERVED */
#define MSYN_HDR_SZ_BYTES            17 /* Starting with PDU type */
// 3 (for PDU_TYPE, HTL, CID) + 6 * 2 (TLV 0x0-0x5) + 8 * 2 (TL 0x80-0x87) + 18 (V 0x80-0x82 and 0x85-0x87) + max string lengths (V 0x83-0x84) = 3 + 12 + 16 + 18 + strs = 49 + strs
#define MSYNACK_MAX_HDR_SZ_BYTES    (49 + MSG_MAX_VERSION_STRING_LEN + MSG_MAX_PRODUCT_NAME_LEN) /* Starting with PDU type */
...
#if !defined(VTSS_VSTAX_HDR_SIZE)
  #define VTSS_VSTAX_HDR_SIZE 0
#endif /* !defined(VTSS_VSTAX_HDR_SIZE) */
...
typedef struct tag_msg_md_item_t {
  ...
  uchar hdr[VTSS_FDMA_HDR_SIZE_BYTES + MD_MAX_PROTO_SZ_BYTES];
...
typedef struct {
  ...
  uchar frm[VTSS_FDMA_HDR_SIZE_BYTES + MSYN_MAX_SZ_BYTES];
...
typedef struct {
  ...
  uchar frm[VTSS_FDMA_HDR_SIZE_BYTES + MSYNACK_MAX_SZ_BYTES];
...
typedef struct {
  ...
  ushort dseq_leftmost_unreceived;
  ...
  uchar dseq_ackd[VTSS_BF_SIZE(MSG_CFG_MAX_WINSZ)];
  ulong dseq_ackd_idx;
  ...
  uchar dmac[VTSS_MAC_ADDR_SZ_BYTES];
  ...
  uchar vs2_frm_hdr[VTSS_VSTAX_HDR_SIZE];
