...
typedef struct {
  ...
  ulong slv_uptime_secs;

  // The time in seconds that the master had been up when it received first frame from slave.
  ulong mst_uptime_secs;

  // Port count of slave
  ulong port_cnt;

  // The stack port numbers of the slave
  vtss_port_no_t stack_ports[2];

  // Board type enumeration, which uniquely identifies the slave board (SFP, Cu. 24-ported, 48-ported, etc.)
  int board_type;

  // The ID used to instantiate the API of the slave
  unsigned int api_inst_id;
} msg_switch_info_t;
...
/****************************************************************************/
typedef enum {
  // Prevent Message Module from calling free() on message after Tx
  ...
  // and then calls the RX callback function, before freeing the copy.
...
// to malloc(). The message module frees it after it has been transmitted.
...
//   transmitted the message, it automatically calls free() on @msg, unless
...
typedef struct {
  ...
  BOOL (*cb)(void *contxt, const void * const msg, size_t len, vtss_module_id_t modid, ulong id);
 ...
 *   After having initialized you switch iterator with switch_iter_init(),
