...
/**
 * \brief Sets 'Configuration Mode' 
 * \param Uses BOOL
 *                  Setting Mode to TRUE allows configuration changes to persist after reset
 * \return zero when successful, any other number signifies failure
 */
int set_config_mode(const BOOL* enable);

/**
 * \brief Gets 'Configuration Mode' 
 * \param Uses *BOOL
 *                  Gets current 'Configuration Mode'; TRUE meane Coniguration mode is enabled
 * \return zero when successful, any other number signifies failure
 */
int get_config_mode(BOOL* enable);
