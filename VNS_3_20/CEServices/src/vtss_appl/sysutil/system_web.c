...
static cyg_int32 handler_stat_sys(CYG_HTTPD_STATE *p)
{
    ...
    if (ct) {
        cyg_httpd_write_chunked(encoded_string, ct);
    }
    cyg_httpd_write_chunked(sep, 1);
    ...
    if (msg_switch_is_master()) {
        ...
        /*lint -e{422, 433} */
        if ((tsl_p = malloc(sizeof(topo_switch_list_t))) && topo_switch_list_get(tsl_p) == VTSS_OK) {
            ...
            for (i = 0; i < ARRSZ(tsl_p->ts); i++) {
                ...
                if (ts_p->vld && ts_p->present) {
                    if (VTSS_ISID_LEGAL(ts_p->isid) && msg_switch_info_get(ts_p->isid, &info) == VTSS_RC_OK) {
                        sprintf(chipid_str, "VSC%x", info.api_inst_id);
                    } else {
...
static cyg_int32 handler_fpga_config_mode(CYG_HTTPD_STATE* p)
{
    BOOL conf_mode; 
    char str1[32];
#ifdef VTSS_SW_OPTION_PRIV_LVL
    ...
    if (p->method == CYG_HTTPD_METHOD_POST)
    {
        ...
        if(cyg_httpd_form_varable_find(p,  "confMode")) /* "confMode" if checked */
        {
            T_D("Entering confMode TRUE" );
            conf_mode=TRUE;
        }
        else
        {
            T_D("Entering confMode FALSE" );
            conf_mode=FALSE;
        }
        if(set_config_mode(&conf_mode)) { T_E("conf_set failed\n"); }
        redirect(p, "/top.htm");
    }
    else 
    {
        ...
        { 
            sprintf(str1, "get_config_mode failed" );
            T_E("conf_get failed\n"); 
        }
        snprintf(str1, sizeof(str1),",%d",conf_mode);
        /* send_string(p, str1); */
    cyg_httpd_start_chunked("html");
    strcpy(p->outbuffer, str1);
...
CYG_HTTPD_HANDLER_TABLE_ENTRY(get_cb_FPGA_config_mode, "/config/fpga_config_mode", handler_fpga_config_mode);
