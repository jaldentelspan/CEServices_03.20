...
static void cli_cmd_system(cli_req_t *req, BOOL contact, BOOL name, BOOL location, BOOL password, BOOL timezone)
{
    ...
    if (req->set) {
        if (contact) {
            strcpy(conf.sys_contact, req->parm);
        }
        if (name) {
            strcpy(conf.sys_name, req->parm);
        }
        if (location) {
            strcpy(conf.sys_location, req->parm);
        }
#ifndef VTSS_SW_OPTION_USERS
        if (password) {
            strcpy(conf.sys_passwd, req->parm);
        }
...
static void cli_cmd_do_reboot(cli_req_t *req)
{
    ...
    if (vtss_switch_mgd() && vtss_switch_stackable() && msg_switch_is_master() && vtss_stacking_enabled()) {
        ...
        rc = control_system_reset(TRUE, VTSS_ISID_LOCAL, system_req->restart);
    }
...
static void system_cmd_system_restore_default(cli_req_t *req )
{
    ...
    ulong flags = system_req->keep_ip ? 0 : INIT_CMD_PARM2_FLAGS_IP;
    if (vtss_switch_stackable()) {
        /* Non-local operation */
        control_config_reset(req->usid_sel, flags);
    } else {
        /* Local operation only */
        control_config_reset(VTSS_ISID_LOCAL, flags);
    }
}
