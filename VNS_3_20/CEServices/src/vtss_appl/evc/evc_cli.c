...
typedef struct {
    ...
    vtss_uport_no_t           uport_rx;
    vtss_uport_no_t           uport_tx;
...
static void evc_cmd_ece_add(cli_req_t *req)
{
    ...
    if (add) {
        ...
        conf.conf.id = id;
#if defined(VTSS_ARCH_JAGUAR_1) || defined(VTSS_ARCH_SERVAL)
        ot->vid = VTSS_VID_DEFAULT;
        action->inner_tag.vid = VTSS_VID_DEFAULT;
#endif /* VTSS_ARCH_JAGUAR_1/SERVAL */
    }
...
static void evc_cmd_mce_add(cli_req_t *req)
{
    ...
    key->port_no = uport2iport(evc_req->uport_rx);
    ...
    /* Action */
    action->port_no = uport2iport(evc_req->uport_tx);
...
static void evc_cmd_mce_get(cli_req_t *req)
{
    ...
    if (vtss_mce_port_info_get(NULL, evc_req->mce_id, 
                               uport2iport(evc_req->uport_rx), &info) == VTSS_OK) {
        CPRINTF("ISDX: %u\n", info.isdx);
    }
}
...
static int evc_parse_vid(char *cmd, evc_cli_vid_t *vid)
{
    ...
    if ((error = cli_parse_ulong(cmd, &value, VTSS_VID_DEFAULT, VTSS_VID_RESERVED)) == 0) {
        vid->value = value;
        vid->valid = 1;
    }
    return error;
...
{
    return evc_parse_range(cmd, vid, 0, VTSS_VID_RESERVED, no_range);
}
...
static int evc_parse_port(char *cmd, vtss_uport_no_t *uport, BOOL cpu)
{
    int error;
    
    if ((error = cli_parse_none(cmd)) != 0) {
        if (cpu && (error = cli_parse_cpu(cmd)) == 0) {
            *uport = iport2uport(VTSS_PORT_NO_CPU);
        } else
            error = cli_parse_ulong(cmd, uport, 1, port_isid_port_count(VTSS_ISID_LOCAL));
    }
    return error;
}
...
static int evc_parse_port_rx(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    return evc_parse_port(cmd, &evc_req->uport_rx, 1);
}
...
static int evc_parse_port_tx(char *cmd, char *cmd2, char *stx, char *cmd_org, cli_req_t *req)
{
    ...
    return evc_parse_port(cmd, &evc_req->uport_tx, 0);
}
