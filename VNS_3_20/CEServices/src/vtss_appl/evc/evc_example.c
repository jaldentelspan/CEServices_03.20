...
static void evc_default_conf(u32 cmd)
{
    ...
    if ((cmd & EVC_TEST_CMD_DEF_CONF) && system_get_config(&conf) == VTSS_RC_OK) {
        control_config_reset(VTSS_ISID_LOCAL, 0);
...
static void evc_vlan_conf(vtss_port_no_t uni, vtss_port_no_t nni)
{
    ...
    vtss_vid_t        vid = VTSS_VID_DEFAULT;  /* Default VLAN */
    vtss_port_no_t    port_no;
    u32               i;

    /* Exclude UNI and NNI port from default VLAN */
    if (vlan_mgmt_vlan_get(isid, vid, &entry, 0, user, VLAN_MEMBERSHIP_OP) == VTSS_RC_OK) {
        entry.ports[uni] = VLAN_NON_MEMBER_PORT;
        entry.ports[nni] = VLAN_NON_MEMBER_PORT;
...
static void evc_oam_conf(vtss_evc_id_t evc_id, vtss_port_no_t uni, u32 cmd)
{
    ...
    /* Create EVC up MEP */
    if (mep_mgmt_conf_get(inst_evc_up, mac, &eps_count, eps_inst, &conf) == MEP_RC_OK) {
        conf.enable = 1;
        conf.mode = VTSS_MEP_MGMT_MEP;
        conf.direction = VTSS_MEP_MGMT_EGRESS;
        ...
        if (mep_mgmt_conf_set(inst_evc_up, &conf) == MEP_RC_OK &&
            mep_mgmt_cc_conf_get(inst_evc_up, &cc_conf) == MEP_RC_OK) {
