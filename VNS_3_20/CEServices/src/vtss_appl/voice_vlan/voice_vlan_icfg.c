    ...
    case ICLI_CMD_MODE_GLOBAL_CONFIG:
        ...
        if ( rc == VTSS_OK ) {
            ...
            if ( conf.age_time != VOICE_VLAN_MGMT_DEFAULT_AGE_TIME || req->all_defaults ) {
                (void)vtss_icfg_printf(result, "voice vlan aging-time %lu\n", conf.age_time);
            }

            // voice vlan class <cos:0-7>
            if ( conf.traffic_class != VOICE_VLAN_MGMT_DEFAULT_TRAFFIC_CLASS || req->all_defaults ) {
                (void)vtss_icfg_printf(result, "voice vlan class %lu\n", conf.traffic_class);
            }
        } else {
...
vtss_rc voice_vlan_icfg_init(void)
{
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_GLOBAL_VOICE_VLAN, _voice_vlan_icfg);
    if ( rc != VTSS_OK ) {
        return rc;
    }
    ...
    */
    rc = vtss_icfg_query_register(VTSS_ICFG_INTERFACE_ETHERNET_VOICE_VLAN, _voice_vlan_icfg);
    return rc;
}
