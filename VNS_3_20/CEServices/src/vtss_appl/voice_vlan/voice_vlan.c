...
static vtss_rc VOICE_VLAN_join(vtss_isid_t isid, vtss_port_no_t iport, vid_t vid)
{
    vtss_rc                 rc = VTSS_OK;
    ...
    /* Add to VOICE_VLAN */
    if (vlan_mgmt_vlan_get(isid, vid, &vlan_member, FALSE, VLAN_USER_VOICE_VLAN, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
        memset(&vlan_member, 0x0, sizeof(vlan_member));
        vlan_member.vid = vid;
    }
    if (vlan_member.ports[iport] == VLAN_NON_MEMBER_PORT) {
        vlan_member.ports[iport] = VLAN_MEMBER_PORT;
...
static vtss_rc VOICE_VLAN_disjoin(vtss_isid_t isid, vtss_port_no_t iport, vid_t vid)
{
    vtss_rc                 rc = VTSS_OK;
    vlan_mgmt_entry_t       vlan_member;
#if defined(VTSS_FEATURE_QCL_V1)
    ...
    if (vlan_member.ports[iport] == VLAN_MEMBER_PORT) {
        vlan_member.ports[iport] = VLAN_NON_MEMBER_PORT;
        rc = vlan_mgmt_vlan_add(isid, &vlan_member, VLAN_USER_VOICE_VLAN);
        if (rc == VLAN_ERROR_DEL_INSTEAD_OF_ADD) {
            rc = VTSS_OK;
        }
    }
...
{
    size_t msg_len = sizeof(voice_vlan_msg_req_t);
    voice_vlan_msg_req_t *msg = malloc(msg_len);
    if (msg) {
...
static vtss_rc VOICE_VLAN_conf_apply(BOOL init, vtss_isid_t isid)
{
    ...
    if (VOICE_VLAN_global.conf.mode == VOICE_VLAN_MGMT_ENABLED) {
        ...
        if (vlan_mgmt_vlan_get(isid, VOICE_VLAN_global.conf.vid, &vlan_member, FALSE, VLAN_USER_VOICE_VLAN, VLAN_MEMBERSHIP_OP) != VTSS_OK) {
            /* Create Voice VLAN */
            memset(&vlan_member, 0x0, sizeof(vlan_member));
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (VOICE_VLAN_global.port_conf[isid].port_mode[pit.iport] == VOICE_VLAN_PORT_MODE_DISABLED) {
            ...
            if ((rc = vlan_mgmt_port_conf_set(isid, pit.iport, &vlan_port_conf, VLAN_USER_VOICE_VLAN)) != VTSS_OK) {
                T_W("vlan_mgmt_port_conf_set(%u, %d): failed rc = %d", isid, pit.iport, rc);
            }
        } else {
        ...
        if (VOICE_VLAN_global.port_conf[isid].port_mode[pit.iport] == VOICE_VLAN_PORT_MODE_FORCED) {
            vlan_member.ports[pit.iport] = VLAN_MEMBER_PORT;
            vlan_member_cnt++;
        }
...
static vtss_rc VOICE_VLAN_conf_pre_changed_apply(int change_field, voice_vlan_conf_t *old_conf)
{
    ...
    if (old_conf->mode == VOICE_VLAN_MGMT_ENABLED &&
        ...
        if ((rc = vlan_mgmt_vlan_del(VTSS_ISID_GLOBAL, old_conf->vid, VLAN_USER_VOICE_VLAN, VLAN_MEMBERSHIP_OP)) != VTSS_OK) {
            return rc;
        }
            ...
            while (port_iter_getnext(&pit)) {
                if (change_field & VOICE_VLAN_CONF_CHANGE_MODE) {
                    ...
                    if ((rc = vlan_mgmt_port_conf_set(sit.isid, pit.iport, &vlan_port_conf, VLAN_USER_VOICE_VLAN)) != VTSS_OK) {
                        T_W("vlan_mgmt_port_conf_set(%u, %d): failed rc = %d", sit.isid, pit.iport, rc);
                    }
            ...
            if ((change_field[pit.iport] & VOICE_VLAN_PORT_CONF_CHANGE_PORT_MODE) ||
                ...
                if ((change_field[pit.iport] & VOICE_VLAN_PORT_CONF_CHANGE_PORT_MODE) ||
                    ...
                    if (new_conf->port_mode[pit.iport] == VOICE_VLAN_PORT_MODE_FORCED) {
                        ...
                        vlan_member.ports[pit.iport] = VLAN_NON_MEMBER_PORT;
                    }
                }
...
static void VOICE_VLAN_oui_default_set(voice_vlan_oui_conf_t *conf)
{
    ...
    /* Default entry 0 */
    conf->entry[0].valid = 1;
    conf->entry[0].oui_addr[0] = 0x00;
    conf->entry[0].oui_addr[1] = 0x01;
    conf->entry[0].oui_addr[2] = 0xE3;
    strcpy(conf->entry[0].description, "Siemens AG phones");

    /* Default entry 1 */
    conf->entry[1].valid = 1;
    conf->entry[1].oui_addr[0] = 0x00;
    conf->entry[1].oui_addr[1] = 0x03;
    conf->entry[1].oui_addr[2] = 0x6B;
    strcpy(conf->entry[1].description, "Cisco phones");

    /* Default entry 2 */
    conf->entry[2].valid = 1;
    conf->entry[2].oui_addr[0] = 0x00;
    conf->entry[2].oui_addr[1] = 0x0F;
    conf->entry[2].oui_addr[2] = 0xE2;
    strcpy(conf->entry[2].description, "H3C phones");

    /* Default entry 3 */
    conf->entry[3].valid = 1;
    conf->entry[3].oui_addr[0] = 0x00;
    conf->entry[3].oui_addr[1] = 0x60;
    conf->entry[3].oui_addr[2] = 0xB9;
    strcpy(conf->entry[3].description, "Philips and NEC AG phones");

    /* Default entry 4 */
    conf->entry[4].valid = 1;
    conf->entry[4].oui_addr[0] = 0x00;
    conf->entry[4].oui_addr[1] = 0xD0;
    conf->entry[4].oui_addr[2] = 0x1E;
    strcpy(conf->entry[4].description, "Pingtel phones");

    /* Default entry 5 */
    conf->entry[5].valid = 1;
    conf->entry[5].oui_addr[0] = 0x00;
    conf->entry[5].oui_addr[1] = 0xE0;
    conf->entry[5].oui_addr[2] = 0x75;
    strcpy(conf->entry[5].description, "Polycom phones");

    /* Default entry 6 */
    conf->entry[6].valid = 1;
    conf->entry[6].oui_addr[0] = 0x00;
    conf->entry[6].oui_addr[1] = 0xE0;
    conf->entry[6].oui_addr[2] = 0xBB;
    strcpy(conf->entry[6].description, "3Com phones");
}
...
vtss_rc voice_vlan_mgmt_conf_set(voice_vlan_conf_t *glbl_cfg)
{
    ...
    if ((glbl_cfg->mode != VOICE_VLAN_MGMT_ENABLED && glbl_cfg->mode != VOICE_VLAN_MGMT_DISABLED) ||
        (glbl_cfg->vid < VLAN_ID_START || glbl_cfg->vid > VLAN_ID_END) ||
...
static vtss_rc VOICE_VLAN_conf_read_stack(BOOL create)
{
    ...
    if (do_create) {
        ...
        /* Use new configuration */
        new_voice_vlan_conf = conf_blk_p->conf;
    }
    ...
    if (do_create) {
        ...
        /* Use new configuration */
        new_voice_vlan_oui_conf = oui_conf_blk_p->oui_conf;
    }
...
vtss_rc VOICE_VLAN_is_valid_voice_vid(vtss_vid_t voice_vid)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        /* Check conflict with static VID */
        vid = 0;
        while (vlan_mgmt_vlan_get(isid, vid, &vlan_conf, TRUE, VLAN_USER_STATIC) == VTSS_OK) {
            if (voice_vid == vlan_conf.vid) {
                return VOICE_VLAN_ERROR_VID_IS_CONFLICT_WITH_STATIC_VID;
            }
        }

        (void) port_iter_init(&pit, NULL, sit.isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_NORMAL);
        while (port_iter_getnext(&pit)) {
            /* Check conflict with PVID */
            if (vlan_mgmt_port_conf_get(isid, pit.iport, &vlan_port_conf, VLAN_USER_VOICE_VLAN) == VTSS_OK) {
                if (voice_vid == vlan_port_conf.pvid) {
                    return VOICE_VLAN_ERROR_VID_IS_CONFLICT_WITH_PVID;
                }
            }
        }
    }
