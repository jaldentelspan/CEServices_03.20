...
// Must be harmonized with max EAPOL size + RADIUS overhead of 72 bytes.
#define RADIUS_MAX_FRAME_SIZE_BYTES      1472
        ...
        if (server_cfg->host[0]) {
            if ((rc = RADIUS_create_socket(server_state, server_cfg->host, server_cfg->port)) != VTSS_OK) {
                if (rc == VTSS_RADIUS_ERROR_NO_NAME) {
                    ...
                    T_W("RADIUS server lookup failed for %s, port %d. Retrying later.", (char *)server_cfg->host, server_cfg->port);
                } else {
                    // No error or warning message here. This can happen if the IP stack isn't ready yet
                    result = FALSE;
                    break;
...
static BOOL RADIUS_add_or_update_required_tlvs(RADIUS_id_state_s *id_state, const struct sockaddr *sa)
{
    ...
    case AF_INET6: {
        ...
        if ((tlv_ptr = RADIUS_find_attribute(id_state->tx_frm, VTSS_RADIUS_ATTRIBUTE_NAS_IPV6_ADDRESS, 0))) {
            ...
            if (!RADIUS_cfg.nas_ip_address_enable && memcmp(tlv_ptr + 2, &sin6->sin6_addr, sizeof(sin6->sin6_addr))) {
...
char *vtss_radius_error_txt(vtss_rc rc)
{
    ...
    case VTSS_RADIUS_ERROR_NOT_ROOM_FOR_TLV:
        return "Returned by vtss_radius_tlv_set() when the TLV is too larget to fit into the pre-allocated Tx frame, or by vtss_radius_tlv_get() when there's not room in the supplied array";
