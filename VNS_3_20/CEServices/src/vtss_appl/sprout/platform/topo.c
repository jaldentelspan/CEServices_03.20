...
static vtss_rc cfg_activate(BOOL init_phase)
{
    ...
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_MST_ELECT_PRIO,             cfg.mst_elect_prio) );
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_MST_TIME_IGNORE,            (cfg.deci_secs_mst_time_ignore > 0)) );
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_SPROUT_UPDATE_INTERVAL_SLV, cfg.sprout_update_interval_slv) );
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_SPROUT_UPDATE_INTERVAL_MST, cfg.sprout_update_interval_mst) );
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_SPROUT_UPDATE_AGE_TIME,     cfg.sprout_update_age_time) );
    T_E_RC_NEG( vtss_sprout_parm_set(init_phase, VTSS_SPROUT_PARM_SPROUT_UPDATE_LIMIT,        cfg.sprout_update_limit)    );
    ...
    topo_parm_t parm)
{
    ...
    if (recreate_local || parm != TOPO_PARM_ISID_TBL) {
        ...
        if (recreate_local || parm == TOPO_PARM_MST_ELECT_PRIO) {
            conf_p[CFG_L_OFFSET_MST_ELECT_PRIO]         = 
                (cfg.mst_elect_prio == CFG_DEFAULT.mst_elect_prio) ?
                0 : cfg.mst_elect_prio;
        }
        if (recreate_local || parm == TOPO_PARM_SPROUT_UPDATE_INTERVAL_SLV) {
            conf_p[CFG_L_OFFSET_SPROUT_UPDATE_INTERVAL_SLV] = 
                (cfg.sprout_update_interval_slv == CFG_DEFAULT.sprout_update_interval_slv) ?
                0 : cfg.sprout_update_interval_slv;
        }
        if (recreate_local || parm == TOPO_PARM_SPROUT_UPDATE_INTERVAL_MST) {
            conf_p[CFG_L_OFFSET_SPROUT_UPDATE_INTERVAL_MST] = 
                (cfg.sprout_update_interval_mst == CFG_DEFAULT.sprout_update_interval_mst) ?
                0 : cfg.sprout_update_interval_mst;
        }
        if (recreate_local || parm == TOPO_PARM_SPROUT_UPDATE_AGE_TIME) {
            conf_p[CFG_L_OFFSET_SPROUT_UPDATE_AGE_TIME] = 
                (cfg.sprout_update_age_time == CFG_DEFAULT.sprout_update_age_time) ?
                0 : cfg.sprout_update_age_time;
        }
        if (recreate_local || parm == TOPO_PARM_SPROUT_UPDATE_LIMIT) {        
            conf_p[CFG_L_OFFSET_SPROUT_UPDATE_LIMIT]    = 
                (cfg.sprout_update_limit == CFG_DEFAULT.sprout_update_limit) ?
                0 : cfg.sprout_update_limit;
        }
        if (recreate_local || parm == TOPO_PARM_FAST_MAC_AGE_TIME) {
            conf_p[CFG_L_OFFSET_FAST_MAC_AGE_TIME]      = 
                (cfg.fast_mac_age_time == CFG_DEFAULT.fast_mac_age_time) ?
                0 : cfg.fast_mac_age_time;
        }
        if (recreate_local || parm == TOPO_PARM_FAST_MAC_AGE_COUNT) {
            conf_p[CFG_L_OFFSET_FAST_MAC_AGE_COUNT]     = 
                (cfg.fast_mac_age_count == CFG_DEFAULT.fast_mac_age_count) ?
                0 : cfg.fast_mac_age_count;
        }
    
        if (recreate_local || parm == TOPO_PARM_UID_0_0) {
            conf_p[CFG_L_OFFSET_UID_PREF + 0] = cfg.uid_pref[0][0];
        }
        if (recreate_local || parm == TOPO_PARM_UID_0_1) {
            conf_p[CFG_L_OFFSET_UID_PREF + 1] = cfg.uid_pref[0][1];
        }
        if (recreate_local || parm == TOPO_PARM_UID_1_0) {
            conf_p[CFG_L_OFFSET_UID_PREF + 2] = cfg.uid_pref[1][0];
        }
        if (recreate_local || parm == TOPO_PARM_UID_1_1) {
            conf_p[CFG_L_OFFSET_UID_PREF + 3] = cfg.uid_pref[1][1];
        }
    ...
    if (recreate_global || parm == TOPO_PARM_ISID_TBL) {
        ...
        if (recreate_global || parm == TOPO_PARM_ISID_TBL) {
            for (isid = 1; isid < VTSS_ISID_END; isid++) {
                ...
                memcpy(conf_p + offset, cfg.isid_tbl[isid].mac_addr, 6);
                conf_p[offset + 6] = cfg.isid_tbl[isid].usid;
            }
        }
    ...
    if (sec_mask & CFG_RD_SEC_MASK_LOCAL) {
        ...
        if (!conf_p) {
            ...
            if (conf_size != CFG_L_SIZE) {
                ...
                T_E("conf_size=%u, expected %d, local section => Recreating w. default values",
                    conf_size, CFG_L_SIZE);
                ...
                T_W("version=%d, expected %d, local section => Recreating w. default values",
                ...
                cfg.ver                        = conf_p[CFG_L_OFFSET_VER];
    
                cfg.mst_elect_prio             = conf_p[CFG_L_OFFSET_MST_ELECT_PRIO];
                cfg.sprout_update_interval_slv = conf_p[CFG_L_OFFSET_SPROUT_UPDATE_INTERVAL_SLV];
                cfg.sprout_update_interval_mst = conf_p[CFG_L_OFFSET_SPROUT_UPDATE_INTERVAL_MST];
                cfg.sprout_update_age_time     = conf_p[CFG_L_OFFSET_SPROUT_UPDATE_AGE_TIME];
                cfg.sprout_update_limit        = conf_p[CFG_L_OFFSET_SPROUT_UPDATE_LIMIT];
                cfg.fast_mac_age_time          = conf_p[CFG_L_OFFSET_FAST_MAC_AGE_TIME];
                cfg.fast_mac_age_count         = conf_p[CFG_L_OFFSET_FAST_MAC_AGE_COUNT];

                
                cfg.mst_elect_prio = 
                    cfg.mst_elect_prio ? cfg.mst_elect_prio : CFG_DEFAULT.mst_elect_prio;
                cfg.sprout_update_interval_slv = 
                    cfg.sprout_update_interval_slv ? cfg.sprout_update_interval_slv : CFG_DEFAULT.sprout_update_interval_slv;
                cfg.sprout_update_interval_mst = 
                    cfg.sprout_update_interval_mst ? cfg.sprout_update_interval_mst : CFG_DEFAULT.sprout_update_interval_mst;
                cfg.sprout_update_age_time = 
                    cfg.sprout_update_age_time ? cfg.sprout_update_age_time : CFG_DEFAULT.sprout_update_age_time;
                cfg.sprout_update_limit = 
                    cfg.sprout_update_limit ? cfg.sprout_update_limit : CFG_DEFAULT.sprout_update_limit;
                cfg.fast_mac_age_time = 
                    cfg.fast_mac_age_time ? cfg.fast_mac_age_time : CFG_DEFAULT.fast_mac_age_time;
                cfg.fast_mac_age_count = 
                    cfg.fast_mac_age_count ? cfg.fast_mac_age_count : CFG_DEFAULT.fast_mac_age_count;
    
                cfg.uid_pref[0][0]         = conf_p[CFG_L_OFFSET_UID_PREF + 0];
                cfg.uid_pref[0][1]         = conf_p[CFG_L_OFFSET_UID_PREF + 1];
                cfg.uid_pref[1][0]         = conf_p[CFG_L_OFFSET_UID_PREF + 2];
                cfg.uid_pref[1][1]         = conf_p[CFG_L_OFFSET_UID_PREF + 3];
    ...
    if (sec_mask & CFG_RD_SEC_MASK_GLOBAL) {
        ...
        if (!conf_p) {
            ...
            if (conf_size != CFG_G_SIZE) {
                ...
                cfg.ver              = conf_p[CFG_G_OFFSET_VER];
    
                cfg.isid_tbl[0] = CFG_DEFAULT.isid_tbl[0];
                for (isid = 1; isid < VTSS_ISID_END; isid++) {
                    offset = CFG_G_OFFSET_ISID_TBL + isid*CFG_G_SIZE_ISID_TBL_ENTRY;
                    memcpy(cfg.isid_tbl[isid].mac_addr, conf_p + offset, 6);
                    cfg.isid_tbl[isid].usid = conf_p[offset + 6];
                    if (cfg.isid_tbl[isid].usid > VTSS_USID_CNT) {
                        
                        recreate_global = 1;
                        break;
                    }
                    cfg.usid_tbl[cfg.isid_tbl[isid].usid] = isid;

                    if (memcmp(cfg.isid_tbl[isid].mac_addr, mac_addr_null, 6) != 0) {
                        cfg.isid_tbl[isid].assigned = 1;
                    } else {
                        cfg.isid_tbl[isid].assigned = 0;
                    }
                }
    ...
    topo_switch_t*           const    ts_p)
{
    ...
    if (me_mst) {
        ts_p->isid            = si_p->id;
        ts_p->usid            = cfg.isid_tbl[ts_p->isid].usid;
    } else {
        ts_p->isid            = 0;
        ts_p->usid            = 0;
    }
    ...
    const vtss_usid_t usid)
{
    ...
    if (isid == 0) {
        ...
        led_usid_set(usid);
        cfg.deci_secs_slave_usid_rx = SLAVE_USID_RX_TIMOUT*10;
        return 1;
    } else if (me_mst) {
        if (isid == me_mst_isid) {
            ...
            if ((topo_msg_p = malloc(sizeof(*topo_msg_p)))) {
                ...
                T_E("malloc failed, size=%zu", sizeof(*topo_msg_p));
                return 1;
            }
        }
    } else {
    ...
    const mac_addr_t mac_addr)
{
    ...
    if (vtss_switch_unmgd()) return isid;
    ...
    for (isid = 1; isid < VTSS_ISID_END; isid++) {
        if (memcmp(mac_addr, cfg.isid_tbl[isid].mac_addr, 6) == 0) {
            break;
        }
    }

    if (get_semaphore) TOPO_CFG_RD_CRIT_EXIT();
    ...
    const mac_addr_t mac_addr)
{
    ...
    for (isid = 1; isid < VTSS_ISID_END; isid++) {
        if (cfg.isid_tbl[isid].assigned) {
            if (memcmp(cfg.isid_tbl[isid].mac_addr, mac_addr, 6) == 0) {
    ...
    for (isid = 1; isid < VTSS_ISID_END; isid++) {
        if (!cfg.isid_tbl[isid].assigned) {
            memcpy(cfg.isid_tbl[isid].mac_addr, mac_addr, 6);
            cfg.isid_tbl[isid].assigned = 1;
...
static void isid_free(vtss_isid_t isid)
{
    ...
    cfg.isid_tbl[isid].assigned = 0;
    memset(&cfg.isid_tbl[isid].mac_addr, 0, sizeof(mac_addr_t));
    for (i = 0; i < VTSS_SPROUT_SIT_SIZE; i++) {
                                     ...
                                     vtss_port_no_t          port_no)
{
    ...
    vtss_interrupt_source_hook_set(topo_link_down_interrupt,
                                   source_id,
...
static void topo_msg_tx_done(void *contxt, void *msg, msg_tx_rc_t rc)
{
    ...
    case TOPO_MSG_TYPE_SLAVE_USID_SET:
        if (contxt != NULL) {
            ...
            TOPO_ASSERT(VTSS_ISID_START <= isid &&
                        isid            <  VTSS_ISID_END, 
                        "contxt=%d", isid);
    ...
    default:
        T_E("Unexpected msg_type=%d contxt=%p msg=%p", 
            topo_msg_base_p->msg_type, contxt, msg);
        break;
    }
...
static void topo_thread(cyg_addrword_t data)
{
    ...
    for (i = 0; i < vtss_board_chipcount(); i++) {
        sprout_switch_init.chip[i].upsid_pref            = UID2UPSID(cfg.uid_pref[i][0]);
    }
    ...
    if(vtss_board_features() & VTSS_BOARD_FEATURE_LOS) {
        vtss_interrupt_source_hook_set(topo_link_down_interrupt, INTERRUPT_SOURCE_LOS, INTERRUPT_PRIORITY_NORMAL);
    }
    ...
    while (1) {
        ...
        if (cfg.deci_secs_mst_time_ignore > 0) {
            if (--cfg.deci_secs_mst_time_ignore == 0) {
                TOPO_CFG_WR_CRIT_EXIT_RACKET();
                T_E_RC_NEG( vtss_sprout_parm_set(0, VTSS_SPROUT_PARM_MST_TIME_IGNORE, 
                                                 (cfg.deci_secs_mst_time_ignore > 0)) );
                TOPO_CFG_WR_CRIT_ENTER_RACKET();
            }
        }

        if (me_mst) {
            cfg.deci_secs_slave_usid_tx--;
            if (cfg.deci_secs_slave_usid_tx <= 0) {
                ...
                for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {
                    ...
                    if (managed_switch) {
                        ...
                        topo_msg_p->usid = cfg.isid_tbl[isid].usid;
                ...
                cfg.deci_secs_slave_usid_tx = SLAVE_USID_TX_TIMER*10;
            }
        } else {
            
            cfg.deci_secs_slave_usid_rx--;

            if (cfg.deci_secs_slave_usid_rx <= 0) {
...
static vtss_rc topo_sprout_cfg_save(vtss_sprout_cfg_save_t* sprout_cfg)
{
    ...
    cfg.uid_pref[0][0] = UPSID2UID(sprout_cfg->upsid[0]);
    if (vtss_board_chipcount() > 1) {
        cfg.uid_pref[1][0] = UPSID2UID(sprout_cfg->upsid[1]);
    }
...
static vtss_rc topo_sprout_state_change(uchar state_change_mask)
{
    ...
    if (state_change_mask & VTSS_SPROUT_STATE_CHANGE_MASK_TTL) {
        ...
        TOPO_CFG_RD_CRIT_EXIT_TMP(
            mac_age_time_set(cfg.fast_mac_age_time, 
                             cfg.fast_mac_age_time*cfg.fast_mac_age_count));
    }
#endif
    ...
    if (vtss_switch_mgd()) {
        if (me_mst ||
            ...
            if (!(sit_old_p = malloc(sizeof(topo_sit_t)))) {
                T_E("malloc failed, size=%zu", sizeof(topo_sit_t));
                TOPO_CFG_WR_CRIT_EXIT();
    ...
    if (vtss_switch_mgd()) {
        if (me_mst ||
            ...
            if (state_change_mask & VTSS_SPROUT_STATE_CHANGE_MASK_ME_MST) {
                ...
                topo_led_usid_set(0, cfg.isid_tbl[me_mst_isid].usid);
                TOPO_CFG_RD_CRIT_ENTER();
    ...
    const ulong            id)
{
    ...
    case TOPO_MSG_TYPE_SWITCH_STAT_REQ:
#if VTSS_SWITCH_MANAGED
        {
            ...
            topo_switch_stat_get__static(VTSS_ISID_LOCAL,
                                        &topo_rsp_msg_p->stat,
                                        FALSE); 
            msg_tx_adv(NULL,
                       &topo_msg_tx_done,
    ...
    case TOPO_MSG_TYPE_STACK_CONF_GET_REQ:
    {
        ...
        if ((conf_msg = malloc(sizeof(*conf_msg))) != NULL) {
...
vtss_rc topo_init(vtss_init_data_t *data)
{
    ...
    if (PORT_NO_STACK_0 == VTSS_PORT_NO_NONE) {
        ...
        TOPO_ASSERT(PORT_NO_STACK_1 == VTSS_PORT_NO_NONE, 
                    "PORT_NO_STACK_1=%u", PORT_NO_STACK_1);

        if (data->cmd == INIT_CMD_START) {
            
            isid = VTSS_ISID_START;
            if (conf_mgmt_mac_addr_get(my_mac, 0) == 0 &&
                (conf = conf_sec_open(CONF_SEC_GLOBAL, CONF_BLK_TOPO, &conf_size)) != NULL &&
                *conf == CFG_DEFAULT.ver && 
                conf_size == CFG_G_SIZE) {
                for (i = VTSS_ISID_START; i < VTSS_ISID_END; i++) {
                    mac_addr = (conf + CFG_G_OFFSET_ISID_TBL + i * CFG_G_SIZE_ISID_TBL_ENTRY);
                    if (memcmp(my_mac, mac_addr, 6) == 0) {
                        isid = i;
                        break;
                    }
                }
...
#ifdef VTSS_SW_OPTION_SYSLOG
            S_W("version_string too long for SPROUT: Length=%d, Max=%d. "
                "Firmware interoperability check will be incomplete.",
                strlen(misc_software_version_txt()), VTSS_SPROUT_FW_VER_LEN);
#endif
            T_W("version_string too long for SPROUT: Length=%d, Max=%d. "
                "Firmware interoperability check will be incomplete.",
                strlen(misc_software_version_txt()), VTSS_SPROUT_FW_VER_LEN);
        }
#endif
    ...
    case INIT_CMD_CONF_DEF:
    {
        ...
        if (isid == VTSS_ISID_LOCAL ||
            ...
            topo_cfg_t cfg_new;
            cfg_new = CFG_DEFAULT;
            cfg_new.deci_secs_mst_time_ignore = cfg.deci_secs_mst_time_ignore;
            cfg_new.mst_elect_prio            = cfg.mst_elect_prio;
            cfg_new.uid_pref[0][0]            = cfg.uid_pref[0][0];
            cfg_new.uid_pref[0][1]            = cfg.uid_pref[0][1];
            cfg_new.uid_pref[1][0]            = cfg.uid_pref[1][0];
            cfg_new.uid_pref[1][1]            = cfg.uid_pref[1][1];
            memcpy(cfg_new.isid_tbl, cfg.isid_tbl, sizeof(cfg_new.isid_tbl));
            memcpy(cfg_new.usid_tbl, cfg.usid_tbl, sizeof(cfg_new.usid_tbl));
    ...
    case INIT_CMD_SWITCH_ADD:
        if (me_mst) {
            if (me_mst_isid != isid) {
                TOPO_CFG_WR_CRIT_ENTER();
                topo_led_usid_set(isid, cfg.isid_tbl[isid].usid);
                TOPO_CFG_WR_CRIT_EXIT();
            }
        }
        break;
...
int topo_parm_get(const topo_parm_t parm)
{
    ...
    case TOPO_PARM_MST_ELECT_PRIO:
        return cfg.mst_elect_prio;
        break;
        
    case TOPO_PARM_MST_TIME_IGNORE:
        return (cfg.deci_secs_mst_time_ignore > 0);
        break;

    case TOPO_PARM_SPROUT_UPDATE_INTERVAL_SLV:
        return cfg.sprout_update_interval_slv;
        break;

    case TOPO_PARM_SPROUT_UPDATE_INTERVAL_MST:
        return cfg.sprout_update_interval_mst;
        break;

    case TOPO_PARM_SPROUT_UPDATE_AGE_TIME:
        return cfg.sprout_update_age_time;
        break;

    case TOPO_PARM_SPROUT_UPDATE_LIMIT:
        return cfg.sprout_update_limit;
        break;

#if defined(VTSS_SPROUT_V1)
    case TOPO_PARM_FAST_MAC_AGE_TIME:
        return cfg.fast_mac_age_time;
        break;

    case TOPO_PARM_FAST_MAC_AGE_COUNT:
        return cfg.fast_mac_age_count;
        break;
#endif
    ...
    case TOPO_PARM_UID_0_0:
        return cfg.uid_pref[0][0];
        break;

    case TOPO_PARM_UID_0_1:
        return cfg.uid_pref[0][1];
        break;

    case TOPO_PARM_UID_1_0:
        return cfg.uid_pref[1][0];
        break;
    ...
    if (rc == VTSS_OK) {
        ...
        case TOPO_PARM_MST_TIME_IGNORE:
            ...
            if (val != 0 && val != 1) {
                ...
                cfg.deci_secs_mst_time_ignore = (val ? VTSS_SPROUT_MST_TIME_IGNORE_PERIOD*10 : 0);
            }
            break;
        ...
        case TOPO_PARM_SPROUT_UPDATE_INTERVAL_SLV:
            ...
            if (val < TOPO_PARM_SPROUT_UPDATE_INTERVAL_MIN) {
                ...
                cfg.sprout_update_interval_slv = val;
            }
            break;
        ...
        case TOPO_PARM_SPROUT_UPDATE_INTERVAL_MST:
            ...
            if (val < TOPO_PARM_SPROUT_UPDATE_INTERVAL_MIN) {
                ...
                cfg.sprout_update_interval_mst = val;
            }
            break;
        ...
        case TOPO_PARM_SPROUT_UPDATE_AGE_TIME:
            ...
            if (val < TOPO_PARM_SPROUT_UPDATE_AGE_TIME_MIN) {
                ...
                cfg.sprout_update_age_time = val;
            }
            break;
        ...
        case TOPO_PARM_FW_VER_MODE:
            if (!TOPO_FW_VER_MODE_LEGAL(val)) {
                ...
                if (isid == VTSS_ISID_LOCAL) {
                    ...
                    vtss_isid_t i;
                    for (i = VTSS_ISID_START; i < VTSS_ISID_END; i++) {
                        if (!msg_switch_exists(i)) continue;
    ...
    if (me_mst) {
        for (i = 0; i < VTSS_SPROUT_SIT_SIZE; i++) {
            if (sit_copy.si[i].vld && 
                ...
                TOPO_ASSERT(cfg.isid_tbl[sit_copy.si[i].id].assigned, "isid=%d", isid);
    ...
    const vtss_isid_t isid)
{
    ...
    if (!me_mst) {
        ...
        goto end;
    } else if (!cfg.isid_tbl[isid].assigned) {
        rc = TOPO_ERROR_SID_NOT_ASSIGNED;
        goto end;
    ...
    const vtss_usid_t usidb)
{
    ...
    if (!me_mst) {
        ...
        for (isid = 1; isid < VTSS_ISID_END; isid++) {
            if (cfg.isid_tbl[isid].usid == usida) {
                TOPO_ASSERT(!usida_found, "!"); usida_found = 1;
                cfg.isid_tbl[isid].usid = usidb;
            } else if (cfg.isid_tbl[isid].usid == usidb) {
                TOPO_ASSERT(!usidb_found, "!"); usidb_found = 1;
                cfg.isid_tbl[isid].usid = usida;
            }
        }
        TOPO_ASSERT(usida_found && usidb_found,
                    "usida=%d, usidb=%d, usida_found=%d, usidb_found=%d",
                    usida, usidb, usida_found, usidb_found);
        
        isid = cfg.usid_tbl[usida];
        cfg.usid_tbl[usida] = cfg.usid_tbl[usidb];
        cfg.usid_tbl[usidb] = isid;

        cfg_wr(0, 0, TOPO_PARM_ISID_TBL);

        topo_led_usid_set(cfg.usid_tbl[usida], usida);
        topo_led_usid_set(cfg.usid_tbl[usidb], usidb);
    }
    ...
    mac_addr_t mac_addr)
{
    ...
    if (vtss_switch_unmgd() || topo_switch_mode == TOPO_SWITCH_MODE_STANDALONE) {
        conf_mgmt_mac_addr_get(mac_addr, 0);
        return VTSS_OK;
    }
    TOPO_ASSERT(topo_switch_mode != TOPO_SWITCH_MODE_UNKNOWN,   "Unknown switch mode");
    ...
    if (cfg.isid_tbl[isid].assigned) {
        memcpy(mac_addr, cfg.isid_tbl[isid].mac_addr, sizeof(mac_addr_t));
    } else {
        rc = TOPO_ERROR_GEN;
    }
                ...
                for (chip_idx = 0; chip_idx < sit_copy.si[i].chip_cnt; chip_idx++) {
                    for (ups_idx = 0; ups_idx < 2; ups_idx++) {
                        if (sit_copy.si[i].chip[chip_idx].upsid[ups_idx] != VTSS_VSTAX_UPSID_UNDEF &&
                            ...
                            TOPO_ASSERT(upsid == VTSS_VSTAX_UPSID_UNDEF,
                                        "Multiple UPSes in sit_copy with isid=%u, port_no=%u?! (i=%d)",
                                        isid, port_no, i);
    ...
    for (i = 0; rc != VTSS_OK && i < VTSS_SPROUT_SIT_SIZE; i++) {
        ...
        for (chip_no = 0; si->vld && chip_no < 2; chip_no++) {
            ...
            if (ci->upsid[0] == upsid || ci->upsid[1] == upsid) {
                sid_port->isid = si->id;
                sid_port->usid = cfg.isid_tbl[sid_port->isid].usid;
                rc = VTSS_OK;
                break;
            }
        }
        ...
        for (usid = 1; usid < VTSS_USID_END; usid++) {
            ...
            if (cfg.isid_tbl[isid].assigned) {
                ...
                if (!found) {
                    ...
                    memcpy(tsl_p->ts[ts_idx].mac_addr, cfg.isid_tbl[isid].mac_addr, 6);
                    ts_idx++;
                }
            }
        }    
    ...
    const topo_dbg_printf_t  dbg_printf)
{
    ...
    for (isid = 1; isid < VTSS_ISID_END; isid++) {
        dbg_printf("%6d  %4d  %s  %4d  %4u\n",
                   cfg.isid_tbl[isid].assigned,
                   isid,
                   mac_addr_to_str(cfg.isid_tbl[isid].mac_addr),
                   cfg.isid_tbl[isid].usid,
                   topo_isid2port(isid));

        if (topo_usid2isid(cfg.isid_tbl[isid].usid) != isid) {
            T_E("topo_usid2isid(%d) = %d  != %d",
                cfg.isid_tbl[isid].usid,
                topo_usid2isid(cfg.isid_tbl[isid].usid),
                isid);
        }
    }
    dbg_printf("\n");
    dbg_printf("me_mst=%d, me_mst_isid=%d\n", me_mst, me_mst_isid);
    dbg_printf("cfg.deci_secs_mst_time_ignore=%d\n", cfg.deci_secs_mst_time_ignore);
} 
