...
static vtss_rc sprout_cx_gen_func(cx_get_state_t *s)
{
    switch (s->cmd) {
        ...
        case CX_GEN_CMD_SWITCH:
            ...
            cx_add_tag_line(s, CX_TAG_STACK, 0);
            {
                topo_switch_list_t *tsl_p;
                topo_switch_t      *ts_p;
                vtss_usid_t         usid = topo_isid2usid(s->isid);

                if ((tsl_p = malloc(sizeof(topo_switch_list_t))) == NULL) {
                    T_W("malloc failed, size=%zu", sizeof(topo_switch_list_t));
                    return VTSS_UNSPECIFIED_ERROR;
                } else {
                    if (topo_switch_list_get(tsl_p) == VTSS_RC_OK) {
                        int i = -1;
                        while (tsl_p->ts[++i].vld && (i < ARRSZ(tsl_p->ts))) {
                            ts_p = &tsl_p->ts[i];
                            if (ts_p->usid == usid) {
                                T_I("Prio = %d, usid = %d, isid = %d",
                                        ts_p->mst_elect_prio, usid,s->isid);
                                cx_add_val_ulong(s, CX_TAG_MST_PRIO, ts_p->mst_elect_prio,
                                        TOPO_PARM_MST_ELECT_PRIO_MIN,
                                        TOPO_PARM_MST_ELECT_PRIO_MAX);
                            }
                        }
                    }
                }
            ...
            }
            {
                ...
                if ((rc = topo_stack_config_get(s->isid, &conf, &dirty)) == VTSS_RC_OK) {
                    cx_add_val_bool(s, CX_TAG_STACKING, conf.stacking);
                    cx_add_val_ulong(s, CX_TAG_PORT_0, conf.port_0, iport2uport(VTSS_PORT_NO_START), iport2uport(VTSS_PORT_NO_END-1));
                    cx_add_val_ulong(s, CX_TAG_PORT_1, conf.port_1, iport2uport(VTSS_PORT_NO_START), iport2uport(VTSS_PORT_NO_END-1));
                } else {
