...
{
    vtss_sprout__stack_port_state_t *sps_pa[2][2];
    vtss_sprout__ui_t               *ui_p     = NULL;
    BOOL                            mirror_fwd_new[2][2] = { { 0, 0 }, { 0, 0 } };
    uint                            sp_idx;
...
static BOOL mst_calc(void)
{
    ...
    while ((ui_p = vtss_sprout__ui_get_nxt(&uit, ui_p))) {
        ...
        if (ui_p->unit_idx != 0) continue;

        
        if (!ui_p->mst_capable) continue;
    ...
    const BOOL                     require_even_upsid)
{
    ...
    for (i = 0; i <= VTSS_VSTAX_UPSID_MAX; i++) {
        ...
        if (upsid_inuse_mask[upsid])             continue;
        if (require_even_upsid && is_odd(upsid)) continue;
...
)
{
    ...
    if ((rc = stack_setup()) < 0) { return rc; }

    update_local_uis();

    
    if (upsids_changed) {
        if ((rc = cfg_save()) < 0) { return rc; }
    }
    ...
    uint                              *i_p)
{
    ...
    *i_p += 1;
    ...
    if (vtss_sprout__switch_addr_cmp(switch_addr_p,
        ...
        if ((ri_p = vtss_sprout__ri_find(&rit, switch_addr_p, 1))) {
    ...
    switch_section_t        *switch_section_p)
{
    ...
    if (len - *i_p < section_len) {
        
        topo_protocol_error(
            "Port %d: Packet length (%d) shorter than section length (%d)", 
            port_no, len - *i_p, section_len);
        
        return VTSS_RC_ERROR;
    }

    pkt_get_switch_addr(pkt_p, i_p, len, &switch_section_p->switch_addr);
    ...
    const uint                          len)
{
    ...
    VTSS_SPROUT_ASSERT((byte = pkt_get_byte(pkt_p, &byte_idx)) == SPROUT_PDU_TYPE_UPDATE,
                       ("PDU Type=%d", byte));
    ...
    if (!found) {
        sps_p->sprout_update_rx_err_cnt++;
        topo_protocol_error(
            "Port %d: No switch section found for neighbour", port_no);
        return VTSS_RC_ERROR;
    }
    ...
    for (i = 0; i < ss_cnt && !found; i++) {
        ...
        for (j = 0; j < ss_cnt && !found; j++) {
            if (i != j) {
                if (vtss_sprout__switch_addr_cmp(&sa, &switch_section[j].switch_addr) == 0) {
                    sps_p->sprout_update_rx_err_cnt++;
                    topo_protocol_error(
                        "Port %d: Two switch sections for same switch address.",
                        port_no);
                    return VTSS_RC_ERROR;
                }
            }
    ...
    const uint                          len)
{
    ...
    VTSS_SPROUT_ASSERT((byte = pkt_get_byte(pkt_p, &byte_idx)) == SPROUT_PDU_TYPE_ALERT,
                       ("PDU Type=%d", byte));
...
static vtss_rc stack_setup(void)
{
    ...
    vtss_appl_api_lock();
    vtss_vstax_conf_get(NULL, &vstax_setup);
    for (unit_idx = 0; unit_idx < vtss_board_chipcount(); unit_idx++) {
        if (unit_idx == 0) 
    ...
    if (rit.changed) {
        ...
        while ((ui_p = vtss_sprout__ui_get_nxt(&uit, ui_p))) {
            ...
            for (glagid = 0; glagid < VTSS_GLAGS; glagid++) {
                ...
                if (mbr_cnt) { glag_mbrs_found = 1; break; };
            }
            if (mbr_cnt) { glag_mbrs_found = 1; break; };
        }
    }
    ...
    const vtss_sprout_switch_init_t* const setup)
{
    ...
    for (i = 0; i < ARRSZ(upsids_pref); i++) {
        if (upsids_pref[i] == VTSS_VSTAX_UPSID_UNDEF) continue;

        for (j = 0; j < ARRSZ(upsids_pref); j++) {
            if (i == j) continue;

            if (upsids_pref[i] == upsids_pref[j]) {
                T_W("Switch has same preferred UPSID %d for different local UPS?!", 
                    upsids_pref[j]);
    ...
    const uint                 glag_mbr_cnt)
{
    ...
    VTSS_SPROUT_ASSERT(glag_no >= VTSS_GLAG_NO_START, ("glag_no=%u", glag_no));
