...
struct Client : public AbstraceTimeHandler
{
    ...
    AckFrame ack_frame() const { return ack_frame_.get(); }
    AckFrame ack_frame(Trigger& t) { return ack_frame_.get(t); }
...
private:
    // Simply copy the entire ACK frame, such that it can be used be exposed for
    // subscriptors which may be interested in various options.
    template<typename LL>
...
Client<FS, TS, Lock>::Client(FS& tx, TS& ts, Lock & lock, u16 vlan) :
    ...
    timer_rebind(this), ack_frame_(ts), vlan_(vlan), xid_(0), xid_seed_(0)
{
                                       ...
                                       const Mac_t src_mac)
{
    ...
    case DHCP4C_STATE_SELECTING: {
        ...
        vtss_ipv4_t server_ip = op_server_identifier.ip().as_int();
        vtss_ipv4_network_t offered_ip;
        offered_ip.address = f.yiaddr().as_int();
        offered_ip.prefix_size = prefix;
        src_mac.copy_to(server_mac.addr);
        ...
        if (offer_list.valid_offers < VTSS_DHCP_MAX_OFFERS) {
            offer_list.list[offer_list.valid_offers].server_ip = server_ip;
            offer_list.list[offer_list.valid_offers].server_mac = server_mac;
            offer_list.list[offer_list.valid_offers].ip = offered_ip;
            offer_list.valid_offers ++;
            ack_frame_.signal(Trigger::success);

        } else {
            T_I("%u Offer overflow. Can not cache more than %d offers. "
                "Offer " VTSS_IPV4N_FORMAT "from " VTSS_IPV4_FORMAT " "
                VTSS_MAC_FORMAT " will be ignored", vlan_,
                VTSS_DHCP_MAX_OFFERS, VTSS_IPV4N_ARG(offered_ip),
                VTSS_IPV4_ARGS(server_ip), VTSS_MAC_ARGS(server_mac));
        }
    ...
    case DHCP4C_STATE_REQUESTING: {
        ...
        T_N("%u Latching timers", vlan_);
        if (!latch_timers(f)) {
            T_N("%u Failed to patch timers", vlan_);
        ...
        latch_settings(f); // will notify listners
        return;
    }
...
void Client<FS, TS, Lock>::latch_settings(const DhcpFrame<LL>& f) {
    T_I("%u setting ack frame", vlan_);
    ack_frame_.set(AckFrame(f));
}
