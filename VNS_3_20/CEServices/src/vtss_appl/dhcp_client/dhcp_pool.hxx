...
#define ACK_OR_RETURN_ERR(VID, FRAME)                                \
    ...
    typename DhcpClient_t::AckFrame FRAME(client[VID]->ack_frame()); \
    do {                                                             \
        if (! FRAME.valid()) {                                       \
            return VTSS_RC_ERROR;                                    \
        }                                                            \
...
public:
    ...
    {
        ACK_OR_RETURN_ERR(vid, f);
        fields->ciaddr = (*f)->ciaddr().as_int();
        fields->yiaddr = (*f)->yiaddr().as_int();
        fields->siaddr = (*f)->siaddr().as_int();
        fields->giaddr = (*f)->giaddr().as_int();
        (*f)->chaddr().copy_to(fields->chaddr.addr);
        (*f)->sname().copy_to((u8*)fields->sname);
        (*f)->file().copy_to((u8*)fields->file);

        return VTSS_RC_OK;
    }

    vtss_rc ip_get(vtss_vid_t vid, vtss_ipv4_network_t *ip)
    {
        ACK_OR_RETURN_ERR(vid, f);
        ...
        if (option_ip_get(vid, Option::CODE_SUBNET_MASK, &mask) != VTSS_RC_OK) {
            ...
            T_D("got option");
            vtss_rc rc = vtss_conv_ipv4mask_to_prefix(mask, &prefix);
            if (rc != VTSS_RC_OK) {
                return rc;
            }
        }

        T_D("prefix = %u", (unsigned)prefix);
        ip->address = ip_;
        ip->prefix_size = prefix;
        return VTSS_RC_OK;
    }

    vtss_rc option_get(vtss_vid_t     vlan,
                       u8             option_code,
                       unsigned       buf_length,
                       u8            *buf)
    {
        ACK_OR_RETURN_ERR(vlan, f);
        int res = (*f)->option_copy(option_code, buf, buf_length, 0);
        return res > 0 ? VTSS_RC_OK : VTSS_RC_ERROR;
    }

    vtss_rc option_ip_any_get(u8 o,
                              ...
                              vtss_ipv4_t *ip)
    {
        ...
        for (int i = 0; i < VTSS_VIDS; ++i) {
            vtss_ipv4_t this_ip;
            if (option_ip_get(i, o, &this_ip) == VTSS_RC_OK) {
                if (this_ip == prefered) {
                    *ip = this_ip;
                    return VTSS_RC_OK;
                } else {
    ...
    vtss_rc option_ip_get(vtss_vid_t          vlan,
                          u8                  option_code,
                          vtss_ipv4_t        *ip)
    {
        ACK_OR_RETURN_ERR(vlan, f);
        int res = (*f)->option_copy(option_code, (u8 *)ip, 4, 2);
        T_D("%u option_ip_get %d", vlan, option_code);

        if (res != 4) {
            T_D("%u option_ip_get %d error: %d", vlan, option_code, res);
            return VTSS_RC_ERROR;
        }

        *ip = ntohl(*ip);

        T_D("%u option_ip_get %d -> %x", vlan, option_code, (unsigned)*ip);
        return VTSS_RC_OK;
    }

    vtss_rc option_ipn_get(vtss_vid_t         vlan,
                           u8                 option_code,
                           u8                 option_number,
                           vtss_ipv4_t       *ip)
    {
        ACK_OR_RETURN_ERR(vlan, f);
        int res = (*f)->option_copy(option_code, (u8 *)ip, 4,
                                    2 + 4 * option_number);
