...
static void os_file_readdata(file_blk_flash_t *flash, int nfiles)
{
    ...
    u8 *ptr = &flash->data[0];
    for (i = 0; i < nfiles; i++) {
        int fd = open(flash->meta[i].name, O_RDONLY);
        ssize_t act_read;
        if (fd >= 0 &&
            (act_read = read(fd, ptr, flash->meta[i].size)) >= 0) { /* In one scoop */
            ptr += act_read;
...
void os_file_fs2flash(void)
{
    ...
    DIR *dirp;
    struct dirent *entry;
    file_blk_flash_t *flash = malloc(sizeof(*flash)), *nflash;
    size_t size = sizeof(*flash);

    if (!flash) {
        T_E("Unable to peristent data struct, size %zd", size);
        return;
    }
    ...
    for (files = 0; files < OS_FILE_FILES_MAX && (entry = readdir(dirp)) != NULL; ) {
        ...
        if (err < 0) {
            ...
            if (S_ISREG(sbuf.st_mode)) {
                ...
                strncpy(flash->meta[files].name, fullname, sizeof(flash->meta[files].name));
...
void os_file_init(vtss_init_data_t *data)
{
    switch (data->cmd) {
    case INIT_CMD_INIT:
