...
static void spi_transaction_begin(cyg_spi_device *device)
{
    ...
    // VTSS_DEVCPU_GCB_GPIO_GPIO_OE |= VTSS_BIT(vcoreiii_device->gpio_num);
    ...
    } else if (vcoreiii_device->gpio_num != VTSS_SPI_GPIO_NONE) {
        VTSS_DEVCPU_GCB_GPIO_GPIO_OUT_CLR = VTSS_BIT(vcoreiii_device->gpio_num);
    }
}
...
static void spi_transaction_end(cyg_spi_device *device)
{
    ...
    if (vcoreiii_device->cs_num != VTSS_SPI_CS_NONE) {
        ...
        VTSS_DEVCPU_GCB_GPIO_GPIO_OUT_SET = VTSS_BIT(vcoreiii_device->gpio_num);
    }
