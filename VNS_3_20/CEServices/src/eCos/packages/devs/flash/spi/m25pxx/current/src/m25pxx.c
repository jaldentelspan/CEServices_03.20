...
#define M25PXX_DELAY_MS(_msdelay_) cyg_thread_delay (\
        MAX(1, ((1000 * _msdelay_ * CYGNUM_HAL_RTC_DENOMINATOR) / (CYGNUM_HAL_RTC_NUMERATOR / 1000))))
...
static const m25pxx_params m25pxx_supported_devices [] = {
  ...
  { // Support for Macronix MX25L64. 64K byte sector size. (256 x page_size(256byte))
    sector_size  : 256,
    sector_count : 128,
    jedec_id     : 0xc22017
  },
  { // Support for Macronix MX25L128. 64K byte sector size. (256 x page_size(256byte))
    sector_size  : 256,
    sector_count : 256,
    jedec_id     : 0xc22018
  },
  { // Support for Winbond W25Q64.
    sector_size  : 256,
    sector_count : 128,
    jedec_id     : 0xef4017
  },
