...
// RBN: If using the JTAG probe to load the binary, we don't have the Redboot functions
//      available for doing this, and since we only have one serial connection, there's
//      no reason to try to change it to the debug one. B.t.w. the 
//      CYGSEM_HAL_VIRTUAL_VECTOR_SUPPORT must be defined, for other purposes, so it
//      wouldn't help to undef it globally.
...
#ifdef CYGPKG_IO_ETH_DRIVERS_WARN_NO_MBUFS
                START_CONSOLE();
                diag_printf("warning: eth_recv out of MBUFs [2]\n");
#ifdef CYGDBG_NET_SHOW_MBUFS                
                cyg_net_show_mbufs();
#endif
                END_CONSOLE();
...
#ifdef CYGPKG_IO_ETH_DRIVERS_WARN_NO_MBUFS
                START_CONSOLE();
                diag_printf("warning: eth_recv out of MBUFs [3]\n");
#ifdef CYGDBG_NET_SHOW_MBUFS                
                cyg_net_show_mbufs();
#endif
                END_CONSOLE();
