...
/* peter, 2007/12, remove warning message, defined but not used
...
#define CYG_SNMP_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))
 ...
 * Prototypes.
 */
int snmp_build (struct snmp_session *, struct snmp_pdu *, u_char *, size_t *, int *);
static int snmp_parse (void *, struct snmp_session *, struct snmp_pdu *, u_char *, size_t, int *);
...
extern void SnmpdGetPort(u_short *snmp_port);
...
struct snmp_session *
snmp_open_v6(struct snmp_session *session)
{
    struct session_list *slp;
    slp = (struct session_list *)snmp_sess_open_v6(session);
    if (!slp) return NULL;
...
#ifdef CYGPKG_NET_FREEBSD_INET6
/* extended open */
struct snmp_session *snmp_open_ex_v6 (
  struct snmp_session *session,
  int (*fpre_parse) (struct snmp_session *, snmp_ipaddr),
  int (*fparse) (struct snmp_session *, struct snmp_pdu *, u_char *, size_t),
  int (*fpost_parse) (struct snmp_session *, struct snmp_pdu *, int),
...
#endif
        /* peter, 2007/5/31,
           modify to wait forever for stackable architecture.
           The intface maybe not intail right now until master is selected,
           the method is similar to SNTP */
        while (bind(sd,(struct sockaddr *)&isp->me, addr_size) < 0) {
            cyg_thread_delay(CYG_SNMP_MSEC2TICK(3000));
        }
        /* original code
...
//#endif // not __ECOS
    ...
    if (session->version == SNMP_VERSION_3) {
      if (session->securityEngineIDLen == 0 &&
          /* peter, 2007/12, upgrade to new version 1.280.2.60
...
#ifdef CYGPKG_NET_FREEBSD_INET6
static void *
_sess_open_v6(struct snmp_session *in_session)
{
    struct session_list *slp;
    struct snmp_internal_session *isp;
    struct snmp_session *session;
    ...
    struct sockaddr_in6 *isp_addr, *meIp;
#ifdef CYGPKG_SNMPAGENT_V3_SUPPORT
    struct snmp_pdu *pdu, *response;
    int status;
#endif
    ...
    if ( isp->addr.sa_family == AF_UNSPEC ) {
        if ( session->peername && session->peername[0] == '/' ) {
            ...
            isp->addr.sa_family = AF_INET6;
            isp_addr = (struct sockaddr_in6 *)&(isp->addr);
            if (session->peername != SNMP_DEFAULT_PEERNAME) {
#if 0 //Anderson, 2008/11, IPv6 format use ":" to spilt.
			/* Try and extract an appended port number */
		cp = strchr( session->peername, ':' );
		if ( cp ) {
		    *cp = '\0';
...
#endif
			/* ... failing that, as an IP address ... */
                if (inet_pton(AF_INET6, session->peername, (void *)&addr)){
                    memmove(&isp_addr->sin6_addr, &addr, sizeof(isp_addr->sin6_addr));
                     ...
                     */
                    isp_addr->sin6_scope_id = 2;                    
                } else {
                    /* just log it, no DNS supported for IPv6 */
                    snmp_log(LOG_ERR,"%s:%d: _sess_open do not have get host by name - cannot resolve %s \n",
                            __FILE__,__LINE__,
                            session->peername);
                    return(0);
                ...
                if (session->remote_port == SNMP_DEFAULT_REMPORT){
                    short iport = ds_get_int(DS_LIBRARY_ID, DS_LIB_DEFAULT_PORT);
                    isp_addr->sin6_port = htons(iport);
                } else {
                    isp_addr->sin6_port = htons(session->remote_port);
                }
            } else {
                isp_addr->sin6_addr = in6addr_any;
            }
        }
    }
	...
	if ( isp->addr.sa_family == AF_INET6 ) {
			/*
			 * Remember to use the specified local port,
			 *   rather than the (default?) remote one.
			 * If no local interface address is specified,
			 ...
			 *   (SNMP_DEFAULT_ADDRESS)
			 */
	    meIp = (struct sockaddr_in6*)&(isp->me);
	    meIp->sin6_port = htons(session->local_port);
	    meIp->sin6_len = snmp_socket_length(isp->me.sa_family);
            if (session->peername == SNMP_DEFAULT_PEERNAME)
                meIp->sin6_addr = in6addr_any;
	}
    }
    else {
        memset(&isp->me, '\0', sizeof(isp->me));
        isp->me.sa_family = isp->addr.sa_family;
        if ( isp->me.sa_family == AF_INET6 ) {
	    meIp = (struct sockaddr_in6*)&(isp->me);
	    meIp->sin6_len = snmp_socket_length(isp->me.sa_family);
            meIp->sin6_addr = in6addr_any;
            meIp->sin6_port = htons(session->local_port);
...
#ifdef CYGPKG_NET_FREEBSD_INET6
void *
snmp_sess_open_v6(struct snmp_session *pss)
{
    void * pvoid;
    pvoid = _sess_open_v6(pss);
    if ( !pvoid) {
        SET_SNMP_ERROR(pss->s_snmp_errno);
    }
    return pvoid;
...
create_user_from_session(struct snmp_session *session)
{
  ...
  if (user == NULL) {
    ...
    /* +++ peter, 2007/12, prevent user->name and user->secName equal NULL */
    if (user->name == NULL || user->secName == NULL) {
        user->name = strdup("");
        user->secName = strdup("");
    }
	   ...
	   int  *af_family)
{
    ...
    if(*af_family == AF_INET)
        ...
        cp = snmp_pdu_build_v6(pdu, cp, out_length);
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    if (cp == NULL)
	return -1;
...
{
    int rc;
    rc = _snmp_build(pss,pdu,packet,out_length,af_family);
    if (rc) {
        if ( !pss->s_snmp_errno)
...
#ifdef CYGPKG_NET_FREEBSD_INET6
u_char *
snmp_pdu_build_v6 (struct snmp_pdu *pdu, u_char *cp, size_t *out_length)
{
  u_char *h1, *h1e, *h2, *h2e;
  struct variable_list *vp;
  struct sockaddr_in6 *pduIp = (struct sockaddr_in6 *)&(pdu->agent_addr);
  size_t length;
  ...
  if (pdu->command != SNMP_MSG_TRAP){
    ...
    cp = asn_build_string(cp, out_length,
			  (u_char)(ASN_IPADDRESS | ASN_PRIMITIVE),
			  (u_char *)&pduIp->sin6_addr,
			  sizeof(pduIp->sin6_addr));
    if (cp == NULL)
      return NULL;
     ...
     u_char 		**after_header)
{
  ...
  *length = pdu_buf_len;
  ...
  af_family = AF_INET;
  ret = snmp_pdu_parse(pdu, data, length,&af_family);
  DEBUGINDENTADD(-8);
...
snmpv3_make_report(struct snmp_pdu *pdu, int error)
{
  ...
  pdu->securityEngineID	= snmpv3_generate_engineID((int *)&pdu->securityEngineIDLen);
  SNMP_FREE(pdu->contextEngineID);
  /* peter 2007/12, remove warning message
  pdu->contextEngineID	= snmpv3_generate_engineID(&pdu->contextEngineIDLen); */
  pdu->contextEngineID	= snmpv3_generate_engineID((int *)&pdu->contextEngineIDLen);
	   ...
	   int *af_family)
{
	...
	result = snmp_pdu_parse(pdu, data, &length, af_family);
        if (result < 0) {
            /*
	   ...
	   int *af_family)
{
    ...
    rc = _snmp_parse(sessp,pss,pdu,data,length,af_family);
    if (rc) {
        if ( !pss->s_snmp_errno)
...
extern void SnmpdGetTrapMode(u_long *mode);
extern void SnmpdGetTrapInformMode(u_long *mode);
extern void SnmpdGetTrapVersion(int *trap_version);
#ifdef CYGPKG_SNMPAGENT_V3_SUPPORT
extern void SnmpdGetTrapSecurityEngineIdProbe(u_long *mode);
#endif /* CYGPKG_SNMPAGENT_V3_SUPPORT */

int
snmp_pdu_parse(struct snmp_pdu *pdu, u_char  *data, size_t *length, int *af_family) {
  u_char  type;
  u_char  msg_type;
  u_char  *var_val;
  int      badtype;
  size_t   len;
  size_t   four;
  u_long   trap_mode, trap_inform_mode;
  int      trap_version;
  struct variable_list *vp = NULL;
  struct sockaddr_in *pduIp = (struct sockaddr_in *)&(pdu->agent_addr);
#ifdef CYGPKG_NET_FREEBSD_INET6
  struct sockaddr_in6 *pduIp_v6 = (struct sockaddr_in6 *)&(pdu->agent_addr);
#endif /* CYGPKG_NET_FREEBSD_INET6 */
  ...
  switch (pdu->command) {
  case SNMP_MSG_TRAP:
    /* peter, 2009/11, we only implement SNMP a*/
    return -1;
    ...
    if(*af_family == AF_INET) {
       ...
       pduIp_v6->sin6_len = sizeof(*pduIp);
       data = asn_parse_string(data, length, &type,
			    (u_char *)&pduIp_v6->sin6_addr,
			    &four);
    }
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    if (data == NULL)
  ...
  case SNMP_MSG_REPORT:
    /* peter, 2010/1, we only implement SNMP agent.
       But need process the report packet when SNMPv3
       Trap Probe Security Engine ID is enabled */
#if CYGPKG_SNMPAGENT_V3_SUPPORT
    SnmpdGetTrapSecurityEngineIdProbe(&trap_security_engineid_probe);
    if (trap_security_engineid_probe && pdu->version == SNMP_VERSION_3)
        pdu->flags |= UCD_MSG_FLAG_RESPONSE_PDU;
    else
        return -1;
#else
  ...
  case SNMP_MSG_RESPONSE:
    /* peter, 2010/2, we only implement SNMP agent.
       But need process the trap response packet when
       trap inform mode is enabled.
       Note: don't let case SNMP_MSG_REPORT run it. */
    if (pdu->command == SNMP_MSG_RESPONSE) {
        SnmpdGetTrapMode(&trap_mode);
        SnmpdGetTrapInformMode(&trap_inform_mode);
        SnmpdGetTrapVersion(&trap_version);
        if (trap_mode && trap_inform_mode && trap_version != SNMP_VERSION_1)
            pdu->flags |= UCD_MSG_FLAG_RESPONSE_PDU;
        else
            return -1;
    }
  ...
  while((int)*length > 0){
    ...
    vp->name = 0;
    vp->index = 0; /* peter, 2007/7, upgrade to new version 1.280.2.60 */
    vp->data = 0; /* peter, 2007/7, upgrade to new version 1.280.2.60 */
		     ...
		     void *cb_data)
{
...
#ifdef CYGPKG_NET_FREEBSD_INET6
    struct sockaddr_in *pduIp = NULL;
    struct sockaddr_in6 *isp_addr_v6;
    struct sockaddr_in6 *pduIp_v6= NULL;
    char server_name[40];
    int cmp_stat = 0;
    struct in6_addr v6_addr_temp;
#endif /* CYGPKG_NET_FREEBSD_INET6 */
...
#endif
    if(isp->af_family == AF_INET)
       pduIp = (struct sockaddr_in *)&(pdu->address);
#ifdef CYGPKG_NET_FREEBSD_INET6
    else if(isp->af_family == AF_INET6)
       pduIp_v6 = (struct sockaddr_in6 *)&(pdu->address);
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    pdu->flags |= UCD_MSG_FLAG_EXPECT_RESPONSE;
	...
	isp_addr_v6 = (struct sockaddr_in6 *)&(isp->addr);
	memset(&v6_addr_temp,0,sizeof(struct in6_addr));
	cmp_stat = memcmp(&isp_addr_v6->sin6_addr,&v6_addr_temp,sizeof(isp_addr_v6->sin6_addr));
	if(cmp_stat == 0)
	    cmp_stat = 1;
	else
	    cmp_stat = 0;
	...
	result = snmp_build(session, pdu, packet, &length, &isp->af_family);
    if (result < 0){
	return 0;
    }
	...
	snmp_log(LOG_DEBUG, "\nSending %u bytes to %s:%hu\n", length,
        ...
        inet_ntop(AF_INET6, (void *)&pduIp_v6->sin6_addr,
		  server_name, sizeof(server_name));
	snmp_log(LOG_DEBUG, "\nSending %u bytes to %s:%hu\n", length,
	       server_name, ntohs(pduIp_v6->sin6_port));
	...
	if (isp->requestsEnd){
      ...
      snmp_res_unlock(MT_LIBRARY_ID, MT_LIB_SESSION);
        ...
        /* peter, 2007/7, upgrade to new version 1.280.2.60
           snmp_free_pdu(pdu); */
       if (reqid) {
...
snmp_read(fd_set *fdset)
{
    ...
    struct session_list *next = Sessions;
    snmp_callback callback;
    void *magic;
#endif
    ...
    snmp_res_lock(MT_LIBRARY_ID, MT_LIB_SESSION);
#ifdef HAVE_MULTI_TRAP_DEST
    for(slp = Sessions; slp && next; slp = next){
#else
    for(slp = Sessions; slp; slp = slp->next){
#endif
...
#ifdef HAVE_MULTI_TRAP_DEST
        callback = slp->session->callback;
        magic = slp->session->callback_magic;
        next = slp->next;
        if (callback) {
            snmp_res_unlock(MT_LIBRARY_ID, MT_LIB_SESSION);
            callback(DELETE_SESSION,slp->session, 0, NULL, magic);
            snmp_res_lock(MT_LIBRARY_ID, MT_LIB_SESSION);
        }
#endif
    }
	       ...
	       fd_set *fdset)
{
...
#ifdef CYGPKG_NET_FREEBSD_INET6
    struct sockaddr_in6 *fromIp_v6 = (struct sockaddr_in6 *)&from;
    char server_name[40];
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    size_t length = 0;
    struct snmp_pdu *pdu;
      ...
      if (!isp->newpkt)
        ...
        if (from.sa_family == AF_UNSPEC) {
            if(isp->af_family == AF_INET)
               ...
               from.sa_family = AF_INET6; /* bad bad bad OS, no bone! */
#endif /* CYGPKG_NET_FREEBSD_INET6 */
        }
#if !defined(__ECOS)
    }
    ...
    if (ds_get_boolean(DS_LIBRARY_ID, DS_LIB_DUMP_PACKET)){
            if(isp->af_family == AF_INET)
               ...
               inet_ntop(AF_INET6, (void *)&fromIp_v6->sin6_addr,
		  server_name, sizeof(server_name));
               snmp_log(LOG_DEBUG, "\nReceived %d bytes from %s:%hu\n", length,
               server_name, ntohs(fromIp_v6->sin6_port));
      }
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    ...
    if ( isp->hook_parse )
      ...
      ret = snmp_parse(sessp, sp, pdu, packetptr, length ,&isp->af_family);
    if ( isp->hook_post ) {
		      ...
		      int *block)
{
    ...
    /* peter, 2007/12, don't block, other requests maybe inital later */
#if 0
...
snmp_timeout (void)
{
	...
	snmp_sess_timeout((void *)slp);
        ...
        magic = slp->session->callback_magic;
        next = slp->next;
        if (callback && slp->session->s_snmp_errno == SNMPERR_TIMEOUT) {
            snmp_res_unlock(MT_LIBRARY_ID, MT_LIB_SESSION);
            callback(DELETE_SESSION,slp->session, 0, NULL, magic);
            snmp_res_lock(MT_LIBRARY_ID, MT_LIB_SESSION);
        }
		    ...
		    int incr_retries)
{
  ...
  /* retransmit this pdu */
  if (snmp_build(sp, rp->pdu, packet, &length, &isp->af_family) < 0){
  ...
  if (ds_get_boolean(DS_LIBRARY_ID, DS_LIB_DUMP_PACKET)){
    ...
    struct sockaddr_in6 *pduIp;
    pduIp = (struct sockaddr_in6 *)&(rp->pdu->address);
    inet_ntop(AF_INET6, (void *)&pduIp->sin6_addr,
		  server_name, sizeof(server_name));
    snmp_log(LOG_DEBUG, "\nResending %d bytes to %s:%hu\n", length,
	   server_name, ntohs(pduIp->sin6_port));
    }
#endif /* CYGPKG_NET_FREEBSD_INET6 */
    xdump(packet, length, "");
  }
		      ...
		      size_t len)
{
    ...
    /* peter, 2007/7, upgrade to new version 1.280.2.60 */
    vars->name_length = 0; vars->data = 0; vars->index = 0;
    ...
    size_t length)
{
    ...
    /* peter, 2007/7, upgrade to new version 1.280.2.60
       RFC3416: request-id      INTEGER (-214783648..214783647)
    if (Reqid == 0) {
