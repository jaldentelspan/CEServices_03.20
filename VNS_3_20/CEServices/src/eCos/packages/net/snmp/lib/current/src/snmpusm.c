		...
		u_char		*msgSalt)
{
	 ...
	 * Sanity check.
	 */
        /* peter, 2007/12, upgrade to new version 1.46.2.7
     ...
     size_t *wholeMsgLen)          /* IN/OUT - Len available, len returned. */
{
	...
	*/
	...
	if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
	{
		 ...
		 */
		if (( ISTRANSFORM(thePrivProtocol, DESPriv) &&
		    ( (encrypted_length != (theTotalLength - dataOffset))
				|| (salt_length != msgPrivParmLen) ))
#ifdef HAVE_AES
		    || (ISTRANSFORM(thePrivProtocol, AESPriv) && ( encrypted_length != scopedPduLen))
#endif
		   )
		{
	...
	void   **secStateRf)	   /* OUT    - Ref to security state.         */
{
	...
	if (secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
	{
        ...
        if (ISTRANSFORM(user->privProtocol, DESPriv)) {
            ...
            /* peter, 2009/12, fixed Bbugzilla#1512
               This test issues a GET and a GET-NEXT request on a large number
               of variable bindings so as to force a tooBig response. */
            if(*scopedPduLen < remaining)
                        ...
                        oid *privProtocol, size_t privProtocolLen)
{
  ...
  if ((newUser->engineID = snmpv3_generate_engineID((int *)&newUser->engineIDLen)) == NULL)
    return usm_free_user(newUser); 
...
void SnmpdDestroyAllUserEntries(void)
{
    struct usmUser *puserList = NULL;

    /* clear SNMP engine users data */
    while ((puserList = usm_get_userList()) != NULL) {
        usm_remove_user(puserList);
        usm_free_user(puserList);
    }
}

int SnmpdChangeAuthPd(u_char *engineID, size_t engineIDLen, char *name, char *newPassword)
{
    ...
    if ((user = usm_get_user(engineID, engineIDLen, name))) {
        ret = generate_Ku(user->authProtocol, user->authProtocolLen,
                          (u_char *)newPassword, strlen(newPassword),
                          userKey, &userKeyLen);
        if (ret != SNMPERR_SUCCESS) {
        ...
        authKeyLen = sc_get_properlength(user->authProtocol, user->authProtocolLen);
        if (authKeyLen < 0) {
            config_perror("Error getting proper key length for auth algorithm.");
            return 1;
        }

        authKey = (u_char *) malloc(authKeyLen);
        if (!authKey) {
            return 1;
        }
...
int SnmpdChangePrivPd(u_char *engineID, size_t engineIDLen, char *name, char *newPassword)
{
    struct usmUser *user = NULL;
    u_char	       userKey[SNMP_MAXBUF_SMALL];
    size_t	       userKeyLen = SNMP_MAXBUF_SMALL;
    ...
    if ((user = usm_get_user(engineID, engineIDLen, name))) {
        ...
        privKeyLen = sc_get_properlength(user->authProtocol, user->authProtocolLen);
        if (privKeyLen < 0) {
          config_perror("Error getting proper key length for priv algorithm.");
          return 1;
        }

        privKey = (u_char *) malloc(privKeyLen);
        if (!privKey) {
            return 1;
        }
        ret = generate_kul(user->authProtocol, user->authProtocolLen,
