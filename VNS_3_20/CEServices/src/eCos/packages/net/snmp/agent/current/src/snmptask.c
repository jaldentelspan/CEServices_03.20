...
#define STACK_SIZE CYGNUM_HAL_STACK_SIZE_TYPICAL + 27648 // 27K for SNMPv3 (include trap and IPv6)
...
void
cyg_net_snmp_thread_suspend(void)
{
    cyg_thread_suspend(netsnmp_thread_handle);
}

void
cyg_net_snmp_thread_resume(void)
{
    cyg_thread_resume(netsnmp_thread_handle);
}
