   ...
   if (entry != 0)
    {
        ...
        if (cyg_httpd_auth_callback) {
            ...
            char    *ptr;
            char    unescape_username[CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_USERNAME_LEN * 3 + 1];
            char    username[CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_USERNAME_LEN + 1];
            char    unescape_passwd[CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_PASSWORD_LEN * 3 + 1];
            char    passwd[CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_PASSWORD_LEN + 1];
            int     str_idx = 0;
            ssize_t id_len;
        
            memset(unescape_username, 0, sizeof(unescape_username));
            memset(username, 0, sizeof(username));
            memset(unescape_passwd, 0, sizeof(unescape_passwd));
            ...
            if (httpstate.cookies) {
                ...
                while (*ptr != ':' && *ptr != '\0' && str_idx < id_len) {
                    if (str_idx < CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_USERNAME_LEN)
                        unescape_username[str_idx++] = *ptr++;
                    else
                        break;
                }
                unescape_username[str_idx] = '\0';
                ...
                while (*ptr != '\0' && str_idx < id_len) {
                    if (str_idx < CYGOPT_NET_ATHTTPD_BASIC_AUTH_MAX_PASSWORD_LEN)
                        unescape_passwd[str_idx++] = *ptr++;
                    else
                        break;
                }
                unescape_passwd[str_idx] = '\0';

                (void) cyg_httpd_unescape(unescape_username, username, strlen(unescape_username), sizeof(username));
                (void) cyg_httpd_unescape(unescape_passwd, passwd, strlen(unescape_passwd), sizeof(passwd));
            }
