...
#define CYG_DNS_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))

#ifdef VTSS_DNS_ENHANCEMENT 
#define MAX_DNS_CACHED_ENTRIES  128
struct in_addr current_dns_server;
...
#ifdef VTSS_DNS_ENHANCEMENT
static struct host_entry host_tab[MAX_DNS_CACHED_ENTRIES];
#endif //VTSS_DNS_ENHANCEMENT
...
send_recv(unsigned char * msg, int len, int msglen)
{
    ...
    int written;
    int ret;
    unsigned char msg_recv[MAXDNSMSGSIZE];
    ...
    dns_hdr = (struct dns_header *) msg_recv;
...
retry:
        ...
        /* Timeout */
        /* Simon: change backoff upper range from 16 to 2 */
        /* it can avoid semphore locking issue when DNS Porxy enabled */
        if (ret == 0) {
            if (backoff < 5) {
                cyg_thread_delay(CYG_DNS_MSEC2TICK(100));
                goto retry;
            } else {
                h_errno = TRY_AGAIN;
                ret = -1;
        ...
        if (ret == 1) {
            ...
            else if (ntohs(dns_hdr->id) == (id-1)){
                memcpy(msg, msg_recv, ret);
            finished = true;
        }
            else
                continue;
...
#ifdef VTSS_DNS_ENHANCEMENT   
    /* free host cached table */
    for(i=0; i<MAX_DNS_CACHED_ENTRIES; i++) {
        host_tab[i].valid = 0;
    }
#endif //VTSS_DNS_ENHANCEMENT
...
#ifdef VTSS_DNS_ENHANCEMENT 
  memcpy(&current_dns_server, dns_server, sizeof(struct in_addr));
#endif //VTSS_DNS_ENHANCEMENT 
...
{
    return htonl(current_dns_server.s_addr);
}
...
cyg_set_current_dns_server(unsigned long dns_server_ip)
{
    ...
    dns_server.s_addr = htonl(dns_server_ip);
...
#ifdef VTSS_DNS_ENHANCEMENT    
    if (!cyg_valid_hostname(hostname)) {
#else //VTSS_DNS_ENHANCEMENT
